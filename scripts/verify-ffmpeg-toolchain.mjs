...
const defaultToolsDirectory = process.platform === 'win32'
  ? 'tools'
...
if (/--enable-gpl(?:\s|$)/u.test(versionOutput) || /--enable-nonfree(?:\s|$)/u.test(versionOutput)) {
  fail('FFmpeg build must not enable GPL or nonfree components for the Steam distribution.');
}
...
const encodersOutput = execFileSync(artifactPath, ['-hide_banner', '-encoders'], {
  encoding: 'utf8',
  timeout: ffmpegProbeTimeoutMs,
  windowsHide: true,
});
const requiredEncoders = Array.isArray(manifest.requiredEncoders) ? manifest.requiredEncoders : [];
for (const encoder of requiredEncoders) {
  const pattern = new RegExp(`(^|\\n)\\s*[A-Z.]{6}\\s+${String(encoder).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s+`, 'u');
  if (!pattern.test(encodersOutput)) {
    fail(`Required FFmpeg encoder is missing: ${encoder}`);
  }
}
