...
static void applyMp4IlstItem(
  ...
  if (item.type == mp4CopyrightType('n', 'a', 'm')) {
    ...
    const bool isPng =
      result.embeddedCover.size() >= 8U && result.embeddedCover[0] == 0x89U && result.embeddedCover[1] == 'P' &&
      result.embeddedCover[2] == 'N' && result.embeddedCover[3] == 'G';
    const bool isWebp =
      result.embeddedCover.size() >= 12U && result.embeddedCover[0] == 'R' && result.embeddedCover[1] == 'I' &&
      result.embeddedCover[2] == 'F' && result.embeddedCover[3] == 'F' && result.embeddedCover[8] == 'W' &&
      result.embeddedCover[9] == 'E' && result.embeddedCover[10] == 'B' && result.embeddedCover[11] == 'P';
    result.embeddedCoverMimeType = isPng ? "image/png" : (isWebp ? "image/webp" : "image/jpeg");
  } else if (item.type == "----") {
    unsupportedExtendedMetadata = true;
  }
}
