...
目标：生成一个 `echo.custom-theme` JSON 文件。用户在 ECHO 的 `设置 -> 外观 -> 自定义当前主题 -> 导入参数` 中导入后，就能得到一个“我的主题”。
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `schema` | 是 | 固定为 `echo.custom-theme` |
| `version` | 是 | 固定为 `2` |
...
matsuriLantern, ginzaNoir, frostJazz, celadonPorcelain, polarDaybreak,
plumVelvet, midnightCopper, taroBunny, milkTeaBear, ramuneGlass,
pistachioMousse, sakuraWagashi, kamakuraHydrangea, ruriKintsugi,
hinokiRyokan, nightGlass, boneIron, harborLamp, ashRose, FINAL
```
...
要求：
- 只输出一个 JSON 代码块。
- 使用 schema = "echo.custom-theme"，version = 2。
