...
ECHO EQ 是可播放、可解释、可关闭的 HiFi DSP 功能。它的第一原则不是“看起来专业”，而是让用户清楚知道：EQ 何时在改变声音、何时会禁用 bit-perfect、何时可能削波、何时已经真正 bypass。
...
ECHO 的 EQ 应该让声音调整更可控，而不是让声音链路更神秘。只要 EQ 开启，用户就应该清楚知道它改变了信号；只要 EQ 关闭，系统就应该真正回到不处理样本的路径。
