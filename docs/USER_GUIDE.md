...
ECHO 优先尊重你的本地文件和标签。专辑显示乱、歌手拆分、封面不对时，通常不是“软件不认识歌”，而是文件标签本来就不统一。
...
新的调音入口在左侧 `DSP` 工作台。第一次使用建议先读 [DSP 新手教程](./ECHO_DSP_BEGINNER_GUIDE.md)，只想快速知道 `Simple` 模式怎么用可以看 [DSP Simple 教程](./ECHO_DSP_SIMPLE_GUIDE.md)。
...
不要第一次就对巨大远程库做全量索引。远程库速度受服务器、网络、权限、证书、代理、防火墙影响，慢不一定是 ECHO 卡。
...
平台边界也要清楚：ECHO 不会绕过会员、版权或平台限制。遇到某个平台搜不到，先换关键词、换来源、检查代理，不要默认是本地曲库坏了。

## 浏览器源输出

如果你要直播、录屏，或想把当前歌词和曲目信息放到 OBS / 直播软件里，可以用浏览器源输出。它只输出透明网页画面，不播放音频，不会改变 ECHO 的实际播放链路。
...
这个链接默认只监听本机 `127.0.0.1`。如果浏览器源没有画面，先确认 ECHO 仍在运行、设置开关已打开、直播软件里的链接没有多余空格。它不是音频采集功能；直播声音仍然按你的系统、声卡或 OBS 音频设置来处理。
