...
适用范围：ECHO 本地插件系统，当前宿主支持 `apiVersion` 1 和 2，推荐新插件使用 `apiVersion: 2`。
...
```text
请按 ECHO 插件系统写一个本地插件。
先阅读 docs/ECHO_PLUGINS.md 和 docs/plugin-sdk/ForAIReadme.md。
...
- 必须是 `type: "echo-plugin-package"` 和 `version: 1`。
