# ECHO Library Core 指南

Library Core 是 ECHO 的本地曲库核心。它负责把磁盘上的音乐文件变成稳定、可分页、可搜索、可恢复的曲库数据，并保证扫描、封面、网络补全、实时更新等能力不会压垮播放。
...
- 本地专辑墙默认启用虚拟化；只有用户显式关闭时才使用完整网格，远程与低配模式始终虚拟化。
- 专辑墙先读第一页，滚动接近底部再追加。
- 不要循环请求所有页。
