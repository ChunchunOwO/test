...
> **当前状态**：本地 daemon direct playback 已把文件读取、libav 解码、ECHO SRC、Dither、SDM、FIFO 和输出 drain 迁入 native host。remote / CUE / chained playback 等 generic legacy PCM 路径仍保留。实现与协作边界以 [Native Audio Pipeline 迁移指南](./ECHO_NATIVE_AUDIO_PIPELINE.md) 为准；任何改动前先加载 `.agents/skills/echo-audio-architecture-refactor/SKILL.md`。

Audio Core 负责 ECHO 的播放、时钟、输出设备、解码、DSP 状态和 HiFi 可解释性。它是播放事实来源，不是 UI 的附属工具，也不是旧 ECHO `AudioEngine.js` 的简单搬运。
...
Windows Audio CD 不依赖打包 FFmpeg 的 `libcdio`。`echo-audio-host` 提供原生 CDDA TOC 查询与音频扇区读取命令，主进程通过 `AudioCdService` 选择音轨，并将 44.1 kHz 双声道 PCM 流交给 `AudioSession`。播放位置、缓冲、DSP、输出设备与结束状态仍由 Audio Core / Native Host 持有；Renderer 只展示宿主能力和发出播放请求。

原生读取按有界批次进行。短暂读取失败会原位重试；驱动不接受较大批次时会逐级缩小并在后续读取中保持已探测到的批次上限。单扇区仍失败时必须终止并上报，不能用静音数据掩盖划伤、弹盘或设备断开。主进程只有在读取子进程正常退出后才结束 PCM 流，避免把末尾硬件错误误判为正常播放完成。

其他平台仍可使用具备 `libcdio` 输入设备的 FFmpeg 构建。能力检测必须以宿主握手或 FFmpeg `-devices` 的真实结果为准，不能由 Renderer 猜测。
...
开发期 native host 可来自 ECHO 自己的构建目录，迁移期允许旧 ECHO fallback，但生产不能依赖 `../ECHO`。
...
```text
ECHO/electron-app/build/echo-audio-host.exe
```
...
尚未统一的范围是 remote URL、CUE、带 headers、gapless / automix chained playback 等 generic legacy PCM 路径；CUDA worker 基础设施也仍保留，但不在当前 native DSP 播放数据面。完整现状、文件地图、fail-closed 规则和验证矩阵见 [Native Audio Pipeline 迁移指南](./ECHO_NATIVE_AUDIO_PIPELINE.md)。
