...
这几天的重点不是堆新按钮，而是把 ECHO 往“更稳、更可信、更专业”的方向推了一大步：播放链路继续加固，资料库恢复和扫描更安全，歌词/MV/流媒体体验更完整，插件与诊断体系更像正式产品，EQ/HQPlayer/Connect 也开始进入更专业的控制层。
...
- 新增 `docs/ECHO_ULTIMATE_ANALYSIS_2026-05-20.md`，系统梳理当前能力、短板、性能风险、竞品差距和低风险优先级。
- 新增 `docs/ECHO_FEATURE_DIRECTION_HQPLAYER_DIGITAL_TRANSPORT.md`，明确 HQPlayer、专业播放和数字转盘方向。
- 更新 `docs/ECHO_PLUGINS.md` 与 `docs/plugin-sdk/echo-plugin.d.ts`，插件 SDK 和平台边界更清楚。
- 更新 `docs/ECHO_LINUX_BUILD.md`、`docs/FUNCTIONAL_EVALUATION.md`、`docs/PERFORMANCE_ISSUES.md`、`docs/ECHO_EQ.md` 等文档，方便后续继续低风险推进。
...
- `20bd3914` update
- `861d2085` Update ECHO features without packaged artifacts
- `9cf09ffd` update
