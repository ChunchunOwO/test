...
export type LibraryAlbumGroupingRefreshRequest = {
...
export type LibraryFileFormatFilter =
  | 'all'
  | 'flac'
  | 'mp3'
  | 'wav'
...
export type PlaybackStatsPeriodTotals = {
  playCount: number;
  playedSeconds: number;
  uniqueTracks: number;
};

export type PlaybackStatsInsights = {
  comparison: {
    current: PlaybackStatsPeriodTotals;
    previous: PlaybackStatsPeriodTotals;
  } | null;
  sessions: {
    sessionCount: number;
    averagePlayedSeconds: number;
    longestPlayedSeconds: number;
...
export type PlaybackStatsDashboard = {
  ...
  insights?: PlaybackStatsInsights;
};
