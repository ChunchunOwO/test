...
export type ConnectPreflightIssueCode =
  | 'device_unavailable'
  | 'source_missing'
  | 'source_not_found'
  | 'airplay_not_ready'
  | 'hqplayer_unavailable';

export type ConnectPreflightWarningCode =
  | 'transcode_required'
  | 'limited_controls'
  | 'hqplayer_will_enable';

export type ConnectPreflightResult = {
  deviceId: string;
  deviceName: string;
  protocol: ConnectProtocol;
  ready: boolean;
  ...
  delivery: 'direct' | 'transcode' | 'hqplayer' | 'unsupported';
  capabilities: ConnectDeviceCapabilities;
  issues: ConnectPreflightIssueCode[];
  warnings: ConnectPreflightWarningCode[];
};
