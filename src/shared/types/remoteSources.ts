...
export type RemoteWebAuthorizationResult = {
  ok: boolean;
  baseUrl: string;
  message: string;
};
...
export type RemoteBackgroundJobKind = 'metadata' | 'cover' | 'lyrics' | 'duration-backfill';
...
export type RemoteMountedRootGrant = {
  grantId: string;
  displayName: string;
};
...
export type RemoteSourceIssueKind = 'metadata' | 'cover' | 'lyrics' | 'missing';
