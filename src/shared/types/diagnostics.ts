...
export type DiagnosticMemoryPressureEvent = {
  ...
  reportPath: string;
  /** False for background maintenance that must not surface user-facing UI. */
  userNoticeRecommended?: boolean;
  /** False when only unused caches should be released without changing visible rendering. */
  rendererMitigationRecommended?: boolean;
  graphicsPressure?: {
