...
describe('audio extension constants', () => {
  ...
  it('does not scan cue sheets as standalone library tracks', () => {
    ...
    expect(isScannableAudioExtension('D:\\Music\\broadcast.bwf')).toBe(true);
    expect(isScannableAudioExtension('D:\\Music\\archive.w64')).toBe(true);
  ...
  it('includes cue sheets in the direct playback dialog list', () => {
    ...
    expect(SUPPORTED_AUDIO_DIALOG_EXTENSIONS).toContain('aifc');
    expect(SUPPORTED_AUDIO_DIALOG_EXTENSIONS).toContain('au');
    expect(SUPPORTED_AUDIO_DIALOG_EXTENSIONS).toContain('voc');
