...
describe('audio platform capabilities', () => {
  ...
  it('allows macOS CoreAudio shared output while keeping advanced Windows modes unavailable', () => {
    ...
    expect(normalizeAudioOutputModeForPlatform('exclusive', 'darwin')).toBe('shared');
    expect(normalizeAudioSharedBackendForPlatform('windows', 'darwin')).toBe('auto');
