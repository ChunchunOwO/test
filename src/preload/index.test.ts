...
describe('preload SMTC API', () => {
  beforeEach(async () => {
    ...
    vi.mocked(webFrame.clearCache).mockReset();
  ...
  it('routes remembered system audio playback through HTMLAudio instead of native IPC', async () => {
    ...
    exposedApi = null;
    fakeAudioInstances = [];
    window.localStorage.setItem('echo.audio-output-memory', JSON.stringify({ enabled: true, outputMode: 'system' }));
  ...
  it('keeps manual system audio pause and play immediate when track transition fade is enabled', async () => {
      vi.resetModules();
      exposedApi = null;
      fakeAudioInstances = [];
      window.localStorage.setItem('echo.audio-output-memory', JSON.stringify({ enabled: true, outputMode: 'system' }));
  ...
  it('keeps remembered system audio manual controls immediate by default', async () => {
    vi.resetModules();
    exposedApi = null;
    fakeAudioInstances = [];
    window.localStorage.setItem('echo.audio-output-memory', JSON.stringify({ enabled: true, outputMode: 'system' }));
  ...
  it('preserves the active exclusive output for native Automix requests without an explicit output mode', async () => {
    ...
    exposedApi = null;
    fakeAudioInstances = [];
    window.localStorage.setItem('echo.audio-output-memory', JSON.stringify({ enabled: true, outputMode: 'exclusive' }));
    vi.mocked(ipcRenderer.invoke).mockImplementation((channel: string) => {
      if (channel === IpcChannels.AudioGetStatus) {
  ...
  it('exposes app utility helpers through IPC', async () => {
  ...
  it('does not expose plugin activation through the Steam app API', () => {
    expect(exposedApi!.app.activateEchoProPlugin).toBeUndefined();
    expect(exposedApi!.app.releaseEchoProCurrentDevice).toBeUndefined();
  });

  it('exposes silent renderer memory reclamation through WebFrame', () => {
    expect(exposedApi!.diagnostics.releaseUnusedRendererMemory?.()).toBe(true);
    expect(webFrame.clearCache).toHaveBeenCalledTimes(1);
  });
  ...
  it('exposes hide-to-tray through typed IPC', async () => {
    await exposedApi!.app.hideToTray();

    expect(ipcRenderer.invoke).toHaveBeenCalledWith(IpcChannels.AppWindowHideToTray);
  });

  it('exposes the main-window minimized state as a typed push event', () => {
    const handler = vi.fn();

    const unsubscribe = exposedApi!.app.onMinimizedChange(handler);
    const listener = listeners.get(IpcChannels.AppWindowMinimizedChanged);
    listener?.({}, true);
    listener?.({}, 'yes');
    unsubscribe();
    ...
    expect(handler).toHaveBeenNthCalledWith(1, true);
    expect(handler).toHaveBeenNthCalledWith(2, false);
    expect(listeners.has(IpcChannels.AppWindowMinimizedChanged)).toBe(false);
  });

  it('exposes the main-window hidden state as a typed push event', () => {
    const handler = vi.fn();

    const unsubscribe = exposedApi!.app.onHiddenChange(handler);
    const listener = listeners.get(IpcChannels.AppWindowHiddenChanged);
    listener?.({}, true);
    listener?.({}, 'yes');
    unsubscribe();
    ...
    expect(handler).toHaveBeenNthCalledWith(1, true);
    expect(handler).toHaveBeenNthCalledWith(2, false);
    expect(listeners.has(IpcChannels.AppWindowHiddenChanged)).toBe(false);
  });
  ...
  it('does not expose plugin management APIs in the Steam preload', () => {
    expect(exposedApi!.plugins).toBeUndefined();
  });

  it('does not expose music-platform account APIs in the Steam preload', () => {
    expect(exposedApi!.accounts).toBeNull();
  });

  it('exposes lyrics APIs through IPC', async () => {
    const changedHandler = vi.fn();
    const snapshotRequest = {
      trackId: 'streaming:qqmusic:1',
      title: 'Snapshot Song',
      artist: 'Snapshot Artist',
      album: null,
      durationSeconds: 120,
      mediaType: 'streaming' as const,
      sourceId: '1',
      stableKey: 'streaming:qqmusic:1',
    };
    ...
    await exposedApi!.lyrics.applyCustomLrcForSnapshot?.(snapshotRequest, '[00:02.00]Snapshot line', 'snapshot.lrc');
    ...
    expect(ipcRenderer.invoke).toHaveBeenCalledWith(
      IpcChannels.LyricsApplyCustomLrcForSnapshot,
      snapshotRequest,
      '[00:02.00]Snapshot line',
      'snapshot.lrc',
    );
  ...
  it('does not expose MV APIs in the Steam preload', () => {
    expect(exposedApi!.mv).toBeNull();
  });
});
