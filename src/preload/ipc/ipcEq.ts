...
export function createEqApi(
  ...
  return {
    ...
    setAnalyzerActive: (enabled) => ipcRenderer.invoke(IpcChannels.EqSetAnalyzerActive, enabled),
    ...
    onCompressorTelemetry: (handler) => {
      const listener = (_event: Electron.IpcRendererEvent, telemetry: CompressorTelemetry): void => handler(telemetry);
      ipcRenderer.on(IpcChannels.EqCompressorTelemetry, listener);
      return () => ipcRenderer.removeListener(IpcChannels.EqCompressorTelemetry, listener);
    },
