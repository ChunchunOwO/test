...
describe('Sidebar direct import actions', () => {
  ...
  it('shows user-hidden routes in edit mode and lets them be restored', async () => {
    ...
    const hiddenRoute = screen.getByRole('button', { name: '显示Albums' });
    ...
    expect(onShowRoute).toHaveBeenCalledWith('albums');
  ...
  it('hides a regular route from the route context menu', async () => {
    const onHideRoute = vi.fn();
    ...
    fireEvent.contextMenu(screen.getByRole('button', { name: 'Albums' }));
    fireEvent.click(await screen.findByRole('menuitem', { name: /隐藏|闅愯棌/u }));

    expect(onHideRoute).toHaveBeenCalledWith('albums');
  });
});
