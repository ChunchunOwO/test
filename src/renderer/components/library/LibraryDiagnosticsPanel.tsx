...
const getLibraryLabHeaderCopy = (locale: string): { title: string; description: string; expand: string; collapse: string } => {
  switch (locale) {
    case 'zh-CN':
      return { title: '开发者工具', description: '仅在排查曲库问题时使用，默认关闭。', expand: '展开', collapse: '收起' };
    case 'zh-TW':
      return { title: '開發者工具', description: '僅在排查媒體庫問題時使用，預設關閉。', expand: '展開', collapse: '收起' };
    case 'ja-JP':
      return { title: '開発者ツール', description: 'ライブラリの問題を調査するときだけ使用します。', expand: '展開', collapse: '閉じる' };
    case 'ko-KR':
      return { title: '개발자 도구', description: '라이브러리 문제를 진단할 때만 사용합니다.', expand: '펼치기', collapse: '접기' };
    default:
      return { title: 'Developer Tools', description: 'Use only when diagnosing library issues.', expand: 'Expand', collapse: 'Collapse' };
  }
};
...
export const LibraryDiagnosticsPanel = (): JSX.Element => {
  ...
  const labHeaderCopy = getLibraryLabHeaderCopy(locale);
  ...
  return (
    <section className="settings-cache-panel settings-library-lab-panel" aria-labelledby="library-lab-title" data-expanded={isExpanded}>
      <div className="settings-cache-header">
        <div>
          <h3 id="library-lab-title">{labHeaderCopy.title}</h3>
          <p className="settings-inline-note">{labHeaderCopy.description}</p>
        </div>
        <button
        ...
        >
          <span>{isExpanded ? labHeaderCopy.collapse : labHeaderCopy.expand}</span>
