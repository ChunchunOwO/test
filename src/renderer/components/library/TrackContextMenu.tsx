...
import {
  clearWorkshopTrackContextActionsForTests,
  getWorkshopTrackContextActions,
  subscribeWorkshopTrackContextActions,
} from '../../workshop/WorkshopTrackContextActions';
...
export const TrackContextMenu = ({
  ...
  const workshopActions = useSyncExternalStore(
    subscribeWorkshopTrackContextActions,
    getWorkshopTrackContextActions,
    getWorkshopTrackContextActions,
  );
  ...
  }, [enabledActions, extraActionsEnabled, liked, position.x, position.y, selectionCount, showOpenOsuBeatmapset, showRemoveFromPlaylist, track.mediaType, workshopActions]);
  ...
  const workshopItems = isBatch
    ? []
    : workshopActions.filter((item) => item.ready && (!item.localOnly || isLocalFileTrack));
  const menuReady = extraActionsReady;
  ...
  return createPortal(
    <div className="track-menu-layer" role="presentation" onMouseDown={requestClose}>
      ...
      >
        ...
        {workshopItems.length > 0 ? <div className="track-menu-heading">创意工坊扩展</div> : null}
        {workshopItems.map((item) => (
          ...
          >
            <Blocks size={16} />
            <span>{item.pluginName} · {item.title}</span>
          </button>
        ))}
      </div>
      {playlistSubmenuOpen ? (
