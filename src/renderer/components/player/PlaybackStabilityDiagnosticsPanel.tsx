...
import { Activity, ChevronDown, Clipboard, RefreshCw, ShieldCheck, TriangleAlert } from 'lucide-react';
...
export const formatPlaybackDiagnosticsText = (diagnostics: AudioDiagnosticsFallback): string => {
  ...
  return ['ECHO Playback Stability Diagnostics', ...rows.map(([label, value]) => `${label}: ${formatValue(value)}`)].join('\n');
};
...
export const PlaybackStabilityDiagnosticsPanel = (): JSX.Element => {
  ...
  const hasProblem = diagnostics.state === 'error' || Boolean(diagnostics.error) || warnings.length > 0;
  const healthTone = hasProblem ? 'attention' : diagnostics.state === 'playing' ? 'stable' : 'idle';
  const healthLabel = hasProblem
    ? t('settings.playback.stability.status.attention')
    : diagnostics.state === 'playing'
      ? t('settings.playback.stability.status.stable')
      : t('settings.playback.stability.status.idle');
  const summaryCards = [
    [t('settings.playback.stability.summary.playback'), diagnostics.state],
    [t('settings.playback.stability.summary.output'), diagnostics.outputDeviceName ?? diagnostics.outputMode],
    [t('settings.playback.stability.summary.buffer'), diagnostics.nativeBufferedMs === null || diagnostics.nativeBufferedMs === undefined ? null : `${diagnostics.nativeBufferedMs} ms`],
    [t('settings.playback.stability.summary.recovery'), diagnostics.recentWatchdogRecoveryCount ?? 0],
  ] as const;
  ...
  return (
    <section className="playback-stability-panel" aria-label={t('settings.playback.stability.title')} data-expanded={isExpanded} data-health={healthTone}>
      <div className="playback-stability-header">
        <button
          className="playback-stability-toggle"
          type="button"
          aria-expanded={isExpanded}
          onClick={() => setIsExpanded((current) => !current)}
        >
          <span className="playback-stability-icon" aria-hidden="true">
            <Activity size={19} />
          </span>
          <span className="playback-stability-heading">
            <span>
              <strong>{t('settings.playback.stability.title')}</strong>
              <em className="playback-stability-health">
                {hasProblem ? <TriangleAlert size={13} /> : <ShieldCheck size={13} />}
                {healthLabel}
              </em>
            </span>
            <small>{t('settings.playback.stability.description')}</small>
          </span>
          <ChevronDown className="playback-stability-chevron" size={18} />
        </button>
        {isExpanded ? (
      ...
      {isExpanded ? (
        <div className="playback-stability-body">
          <div className="playback-stability-summary">
            {summaryCards.map(([label, value]) => (
              <span key={label}>
                <em>{label}</em>
                <strong>{formatValue(value, t('settings.playback.stability.value.unknown'))}</strong>
              </span>
            ))}
          </div>
          <details className="playback-stability-details">
            <summary>{t('settings.playback.stability.details')}</summary>
