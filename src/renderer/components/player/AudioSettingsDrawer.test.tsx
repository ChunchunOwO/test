...
const testTranslations: Record<string, string> = {
  ...
  'audioDrawer.deviceRefresh.failed': 'Refresh failed. Try again.',
  'audioDrawer.deviceRefresh.refreshing': 'Scanning all audio devices…',
  'audioDrawer.deviceRefresh.success': 'Refreshed {count} devices · {time}',
  ...
  useI18n: () => ({
    t: (key: string, options?: Record<string, string | number>) => {
      if (testTranslations[key]) {
        return testTranslations[key].replace(/\{(\w+)\}/g, (placeholder, name: string) =>
...
beforeEach(() => {
  ...
  window.localStorage.setItem('echo.audio-engine-meter-open', 'true');
...
describe('AudioSettingsDrawer controls', () => {
  it('handles Escape inside the drawer without closing the underlying route', () => {
    const onClose = vi.fn();
    const underlyingRouteHandler = vi.fn();
    window.addEventListener('keydown', underlyingRouteHandler);
    renderDrawer(baseStatus, undefined, undefined, undefined, undefined, { onClose });

    fireEvent.keyDown(window, { key: 'Escape' });

    expect(onClose).toHaveBeenCalledTimes(1);
    expect(underlyingRouteHandler).not.toHaveBeenCalled();
    window.removeEventListener('keydown', underlyingRouteHandler);
  });

  it('refreshes devices from the host without subscribing to browser media capture devices', async () => {
  ...
  it('forces a full host device rescan from the recommended output refresh button', async () => {
    renderDrawer(baseStatus);
    await waitFor(() => expect(window.echo?.audio.listDevices).toHaveBeenCalled());
    let resolveRefresh!: (devices: AudioDeviceInfo[]) => void;
    vi.mocked(window.echo!.audio.refreshDevices).mockImplementation(() => new Promise((resolve) => {
      resolveRefresh = resolve;
    }));

    fireEvent.click(screen.getByRole('button', { name: /refresh.*systemDevices/i }));

    await waitFor(() => expect(window.echo?.audio.refreshDevices).toHaveBeenCalledOnce());
    expect(screen.getByRole('status').textContent).toContain('Scanning all audio devices…');
    expect(screen.getByRole('button', { name: /refresh.*systemDevices/i }).getAttribute('aria-busy')).toBe('true');

    resolveRefresh([asioDevice, nativeAsioDevice]);

    await waitFor(() => expect(screen.getByRole('status').textContent).toContain('Refreshed 2 devices'));
    expect(screen.getByRole('button', { name: /refresh.*systemDevices/i }).getAttribute('aria-busy')).toBe('false');

    vi.mocked(window.echo!.audio.refreshDevices).mockRejectedValueOnce(new Error('enumeration failed'));
    fireEvent.click(screen.getByRole('button', { name: /refresh.*systemDevices/i }));

    await waitFor(() => expect(screen.getByRole('status').textContent).toContain('Refresh failed. Try again.'));
  });
  ...
  it('filters drawer sections from titles, descriptions, and hidden search keywords', async () => {
    ...
    expect(screen.queryByRole('listbox', { name: /matches/i })).toBeNull();
  ...
  it('shows native ASIO devices as dedicated output routes', async () => {
    renderDrawerWithDevices(baseStatus, [nativeAsioDevice]);

    const deviceButton = await screen.findByRole('button', { name: /TEAC ASIO/ });

    expect(deviceButton.dataset.outputMode).toBe('asio');
    expect(within(deviceButton).getByText(/ASIO driver/)).toBeTruthy();
  });

  it('applies an explicit native ASIO device route', async () => {
    const setOutput = vi.fn().mockResolvedValue({
      ...baseStatus,
      outputMode: 'asio',
      outputDeviceName: nativeAsioDevice.name,
    });
    renderDrawer(
      baseStatus,
      setOutput,
    ...
    fireEvent.click(await screen.findByRole('button', { name: /TEAC ASIO/ }));

    await waitFor(() => expect(setOutput).toHaveBeenCalledWith(expect.objectContaining({
      outputMode: 'asio',
      deviceIndex: nativeAsioDevice.index,
      deviceName: nativeAsioDevice.name,
    })));
  });
  ...
  it('keeps advanced output collapsed by default and remembers when opened', () => {
    ...
    expect(window.localStorage.getItem('echo.audio-advanced-output-open')).toBe('true');
  ...
  it('remembers the HiFi engine meter collapse state', () => {
    window.localStorage.removeItem('echo.audio-engine-meter-open');
    renderDrawer(baseStatus);
    ...
    expect(window.localStorage.getItem('echo.audio-engine-meter-open')).toBe('true');
  ...
  it('resets all audio settings from the drawer bottom', async () => {
    ...
    const settingsChanged = vi.fn();
    window.localStorage.setItem('echo.hidden-audio-devices', JSON.stringify(['asio:0:TEAC ASIO']));
    window.addEventListener('settings:changed', settingsChanged);
    ...
    expect(window.localStorage.getItem('echo.hidden-audio-devices')).toBe('[]');
  ...
  it('keeps low-level ASIO control-panel entry out of the playback drawer', async () => {
    renderDrawer(baseStatus);

    await screen.findByRole('button', { name: /TEAC ASIO/ });
    expect(screen.queryByRole('button', { name: /Open ASIO Panel/ })).toBeNull();
    expect(screen.queryByRole('checkbox', { name: /Show ASIO panel settings/ })).toBeNull();
  });
  ...
  it('keeps manual ASIO buffer controls out of the playback drawer', () => {
    renderDrawer({
      ...baseStatus,
      outputMode: 'shared',
      outputBackend: 'asio',
  ...
  it('applies a latency profile without inventing a manual ASIO buffer override', async () => {
    const status = {
      ...baseStatus,
      outputMode: 'shared' as const,
      outputBackend: 'asio',
    ...
    const setOutput = vi.fn().mockResolvedValue({ ...status, latencyProfile: 'balanced' });
    renderDrawer(status, setOutput);
    openBufferControls();

    fireEvent.click(screen.getByRole('button', { name: /Balanced/ }));

    await waitFor(() => expect(setOutput).toHaveBeenCalledWith({ latencyProfile: 'balanced' }));
  });
  ...
  it('recovers controls after an ASIO output switch fails', async () => {
    ...
    const handleNotice = (event: Event): void => {
      notices.push(event as CustomEvent);
    };
  ...
  it('saves the current output route when output settings are enabled', async () => {
    renderDrawer({
      ...baseStatus,
      outputMode: 'shared',
      outputBackend: 'asio',
    ...
    const remembered = JSON.parse(window.localStorage.getItem('echo.audio-output-memory') ?? '{}');
  ...
  it('clears incompatible remembered buffer size when switching to shared low latency', async () => {
    ...
    expect(JSON.parse(window.localStorage.getItem('echo.audio-output-memory') ?? '{}')).not.toHaveProperty('bufferSizeFrames');
  });
