...
import '../../styles/audio-drawer-polish.css';
import {
  AudioLines,
  Bluetooth,
  Cable,
...
type DeviceRefreshFeedback =
  | { kind: 'success'; count: number; completedAt: number }
  | { kind: 'error' };
...
const hiddenDeviceStorageKey = 'echo.hidden-audio-devices';
const advancedOutputOpenStorageKey = 'echo.audio-advanced-output-open';
const audioEngineMeterOpenStorageKey = 'echo.audio-engine-meter-open';
...
const formatDeviceRefreshTime = (completedAt: number): string =>
...
export const AudioSettingsDrawer = ({
  ...
  const [deviceRefreshFeedback, setDeviceRefreshFeedback] = useState<DeviceRefreshFeedback | null>(null);
  ...
  const kernelStreamingDevices = useMemo(() => visibleDevices.filter((device) => device.outputMode === 'ks'), [visibleDevices]);
  const advancedNativeOutputAvailable = useMemo(() => isAdvancedNativeOutputPlatform(rendererPlatform), [rendererPlatform]);
  const routeDeviceCount = sharedDevices.length + (advancedNativeOutputAvailable ? asioDevices.length + kernelStreamingDevices.length : 0);
  const deviceRefreshFeedbackText = isRefreshingDevices
    ? t('audioDrawer.deviceRefresh.refreshing')
    : deviceRefreshFeedback?.kind === 'success'
      ? t('audioDrawer.deviceRefresh.success', {
        count: deviceRefreshFeedback.count,
        time: formatDeviceRefreshTime(deviceRefreshFeedback.completedAt),
      })
      : deviceRefreshFeedback?.kind === 'error'
        ? t('audioDrawer.deviceRefresh.failed')
        : null;
  ...
  const kernelStreamingRouteActive = outputMode === 'ks' || status?.outputBackend === 'wdm-ks';
  ...
  const lockWasapiExclusive = !advancedNativeOutputAvailable || outputMode === 'system' || asioRouteActive || kernelStreamingRouteActive;
  ...
  const refreshAllDevices = useCallback(async (): Promise<void> => {
    ...
    if (!audio) {
      setError(copy.desktopBridgeUnavailable);
      setDevices([]);
      setDeviceRefreshFeedback({ kind: 'error' });
      return;
    }

    setIsRefreshingDevices(true);
    setDeviceRefreshFeedback(null);
    try {
      ...
      onStatusChange(nextStatus);
      setDevices(nextDevices);
      setDeviceRefreshFeedback({ kind: 'success', count: nextDevices.length, completedAt: Date.now() });
      setError(null);
    } catch (refreshError) {
      setDeviceRefreshFeedback({ kind: 'error' });
      setError(formatUserFacingError(refreshError, { context: 'audio' }));
  ...
  useEffect(() => {
    ...
    // Audio device truth belongs to the native host. Chromium's mediaDevices
    // listener can start a persistent Video Capture helper even though this
    // drawer never captures media, so use the existing lightweight host poll.
        ...
        <section className="audio-drawer-section audio-route-system-devices">
          <div className="audio-drawer-section-title">
            ...
            <span className="audio-device-section-meta">
              ...
              <button
                ...
                title={t('audioProfessional.action.refresh')}
                disabled={isBusy || isRefreshingDevices}
                aria-busy={isRefreshingDevices}
                onClick={() => void refreshAllDevices()}
              >
                <RefreshCw className={isRefreshingDevices ? 'spinning-icon' : undefined} size={13} />
              </button>
            </span>
          </div>
          {deviceRefreshFeedbackText ? (
            <p
              className="audio-device-refresh-feedback"
              data-state={isRefreshingDevices ? 'refreshing' : deviceRefreshFeedback?.kind}
              role="status"
              aria-live="polite"
            >
              {isRefreshingDevices ? <RefreshCw size={13} /> : deviceRefreshFeedback?.kind === 'success' ? <Check size={13} /> : <ShieldAlert size={13} />}
              <span>{deviceRefreshFeedbackText}</span>
            </p>
          ) : null}
          ...
          >
            ...
            <span>
              ...
              <small>{formatWasapiDeviceMode(exclusiveRouteActive)} / {t('audioDrawer.device.systemSelectedRoute')}</small>
            </span>
            <em>WASAPI</em>
            ...
            return (
              ...
              >
                ...
                <span>
                  ...
                  <small>{bluetooth ? 'Bluetooth / ' : ''}{formatWasapiDeviceMode(exclusiveRouteActive)} / {sampleRate || t('audioDrawer.status.sampleRatePending')}</small>
                </span>
                <em>{bluetooth ? 'Bluetooth' : device.isDefault ? 'DEFAULT' : 'WASAPI'}</em>
          ...
          {advancedNativeOutputAvailable ? kernelStreamingDevices.map((device) => {
            const isActive = !hqPlayerTakeoverEnabled && deviceMatchesStatus(device, status, 'ks');
            ...
            return (
              <button
                ...
                key={device.id}
                type="button"
                title={`${device.name} · WDM-KS experimental direct output`}
                disabled={isBusy || automaticOutputEnabled}
                onMouseDown={suppressNativeDeviceMenu}
                onContextMenu={(event) => openDeviceMenu(event, device)}
                onClick={() => applyDevice('ks', device)}
              >
                <Zap size={15} />
                <span>
                  <strong>{device.name}</strong>
                  <small>WDM-KS 直连 / 实验性 / 失败自动回退 WASAPI Shared</small>
                </span>
                <em>KS</em>
                {isActive ? <Check size={15} /> : null}
              </button>
