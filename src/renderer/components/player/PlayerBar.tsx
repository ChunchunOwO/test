...
import { Cable, Captions, CircleAlert, FileDown, Loader2, Monitor, X } from 'lucide-react';
...
import { beginPlaybackSeekSnapshot, getVisualPlaybackState, refreshPlaybackStatus, setPlaybackStatusSnapshot, useSharedPlaybackStatusForUi } from '../../stores/playbackStatusStore';
...
import { largeCoverUrlFromCachedVariant, playerCoverDisplayUrl } from '../../utils/coverDisplayUrl';
...
import { readActiveDetailReturnTarget } from '../../utils/albumNavigation';
...
import { WorkshopPlayerBarActionButtons } from '../../workshop/WorkshopPlayerBarActionButtons';
...
const defaultHiddenPlayerBarButtonIds: PlayerBarButtonId[] = ['sleepTimer', 'audioExport'];
...
const playerArtworkUrl = (track: { coverId: string | null; coverThumb: string | null } | null): string | null =>
  playerCoverDisplayUrl(track?.coverId, track?.coverThumb);
...
export const PlayerBar = ({
  ...
  const [allowLayoutMotion, setAllowLayoutMotion] = useState(true);
  ...
  const title =
    ...
    (filePath ? titleFromPath(filePath) : 'ECHO');
  const artist =
  ...
  const artworkUrl = receiverMetadata?.coverHttpUrl
    ...
    || playerCoverDisplayUrl(null, statusArtworkUrl);
  const displayArtworkUrl = artworkUrl === failedArtworkUrl ? null : artworkUrl;
  ...
  useEffect(() => {
    ...
    const retryTimer = window.setTimeout(() => setFailedArtworkUrl(null), 15_000);
  ...
  const isCurrentStreamingTrack = streamingTrackMediaType === 'streaming';
  ...
  const handleOpenCurrentArtist = useCallback((): void => {
    ...
    void openArtistDetailByName(currentLibraryArtistName, { returnTo: readActiveDetailReturnTarget() }).catch((error) => {
      ...
      void getSettings()
        .then((settings) => {
          if (!cancelled) {
            ...
            setAllowLayoutMotion(resolveEffectivePerformancePolicy(settings as Partial<AppSettings>).allowLayoutMotion);
  ...
  const commitSeek = useCallback(
    async (nextPositionSeconds: number): Promise<void> => {
      ...
      if (isAirPlayReceiverPlaybackActive) {
    ...
    [applyConnectPlaybackStatus, currentTrack, durationSeconds, filePath, isAirPlayReceiverPlaybackActive, isSpotifyCurrentTrack, refreshStatus, trackId],
  );
  ...
  const playerBarLayout = lyricsMiniPlayer || !allowLayoutMotion ? false : 'position';
    ...
    <>
      ...
      <div className="player-now">
        <motion.button
          ...
          data-empty={!displayArtworkUrl}
          type="button"
          aria-label={t('playerBar.aria.openLyrics')}
          title={t('playerBar.aria.openLyrics')}
          data-loading={isPlaybackPreparing ? 'true' : undefined}
          layoutId={allowLayoutMotion ? playerCoverLayoutId(trackId) : undefined}
          transition={springSoft}
          onClick={handleOpenLyrics}
        >
          {displayArtworkUrl ? (
            <img alt="" src={displayArtworkUrl} onError={() => setFailedArtworkUrl(displayArtworkUrl)} />
          ) : (
      ...
      <div className="player-center">
        ...
        <PlayerProgress
          disabled={isAirPlayReceiverPlaybackActive || isLiveStreamPlayback || (!filePath && !isSpotifyCurrentTrack)}
      ...
      <div className="output-status">
        <WorkshopPlayerBarActionButtons />
