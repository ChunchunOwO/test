...
const translations: Record<string, string> = {
  ...
  'settings.playback.stability.description': '查看当前输出、缓冲和自动恢复情况。',
  'settings.playback.stability.details': '查看完整技术信息',
  'settings.playback.stability.error.desktopBridgeUnavailable': '当前无法完成此操作',
  ...
  'settings.playback.stability.status.attention': '需要注意',
  'settings.playback.stability.status.idle': '等待播放',
  'settings.playback.stability.status.stable': '运行稳定',
  'settings.playback.stability.summary.buffer': '当前缓冲',
  'settings.playback.stability.summary.output': '输出设备',
  'settings.playback.stability.summary.playback': '播放状态',
  'settings.playback.stability.summary.recovery': '近期恢复',
...
const expandTechnicalDetails = (): void => {
  fireEvent.click(screen.getByText('查看完整技术信息'));
};
...
describe('PlaybackStabilityDiagnosticsPanel', () => {
  ...
  it('shows playing diagnostics fields', async () => {
    ...
    expect((await screen.findAllByText('playing')).length).toBeGreaterThan(0);
  ...
  it('copies complete diagnostic text', async () => {
    ...
    expect(copied).toContain('ECHO Playback Stability Diagnostics');
