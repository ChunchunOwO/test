...
describe('PlayerBar', () => {
  ...
  it('disables player bar layout projection in lightweight mode', async () => {
    window.echo = {
      app: {
  ...
  it('pauses the active HQPlayer Connect session even while takeover mode is enabled', async () => {
    window.localStorage.setItem('echo.hqplayer-takeover-enabled', 'true');
  ...
  it('loads a larger Subsonic player cover and falls back cleanly when it fails', async () => {
    const remoteTrack = makeTrack(8, {
      id: 'remote-track-8',
      path: 'remote://subsonic/song-8',
      title: 'Remote Cover Track',
      mediaType: 'remote',
      provider: 'subsonic',
      coverThumb: 'echo-image://subsonic-cover/track-8?size=160&cacheKey=album-8',
    });
    ...
    window.echo = {
      playback: {
        getStatus: vi.fn().mockResolvedValue({
          ...
          filePath: remoteTrack.path,
        }),
      },
      audio: {
        getStatus: vi.fn().mockResolvedValue(audioStatus(remoteTrack)),
      },
      library: {
        getLikedTrackIds: vi.fn().mockResolvedValue({ [remoteTrack.id]: false }),
      },
      app: {
        getSettings: vi.fn().mockResolvedValue({ smtcEnabled: true }),
      },
    ...
    const { container } = render(
      <PlaybackQueueProvider>
        <QueueSeed tracks={[remoteTrack]} />
      </PlaybackQueueProvider>,
    );

    await screen.findByText('Remote Cover Track');
    const coverImage = container.querySelector('.player-cover img');
    expect(coverImage?.getAttribute('src')).toBe(
      'echo-image://subsonic-cover/track-8?size=512&cacheKey=album-8',
    );

    fireEvent.error(coverImage!);
    await waitFor(() => expect(container.querySelector('.player-cover img')).toBeNull());
    expect(container.querySelector('.player-cover')?.getAttribute('data-empty')).toBe('true');
  });
  ...
  it('handles library, queue, search, playback-mode, mute, and auxiliary-window global shortcut commands', async () => {
    const track = makeTrack(1);
    const globalShortcutHandlers: Array<(command: GlobalShortcutAction) => void> = [];
    ...
    const petShow = vi.fn().mockImplementation(async () => {
      petVisible = true;
      return { visible: true };
    });
    const petHide = vi.fn().mockImplementation(async () => {
      petVisible = false;
      return { visible: false };
    });
    const getPetState = vi.fn().mockImplementation(async () => ({ visible: petVisible }));
    ...
    globalShortcutHandlers[0]?.('togglePet');
    await waitFor(() => expect(petShow).toHaveBeenCalledTimes(1));

    globalShortcutHandlers[0]?.('togglePet');
    await waitFor(() => expect(petHide).toHaveBeenCalledTimes(1));

    globalShortcutHandlers[0]?.('toggleShuffle');
    await waitFor(() => expect(window.localStorage.getItem('echo:playback-mode')).toContain('"isShuffleEnabled":true'));

    globalShortcutHandlers[0]?.('cycleRepeatMode');
    await waitFor(() => expect(window.localStorage.getItem('echo:playback-mode')).toContain('"repeatMode":"one"'));

    window.removeEventListener('app:navigate:queue', navigateQueue);
    window.removeEventListener('app:navigate:route', navigateRoute);
  });

  it('handles replay, lyrics, settings, liked, eq, and reveal shortcut commands', async () => {
    const track = makeTrack(1);
    const globalShortcutHandlers: Array<(command: GlobalShortcutAction) => void> = [];
    const seek = vi.fn().mockResolvedValue({
      state: 'playing',
    ...
    const setOutput = vi.fn().mockResolvedValue({ ...audioStatus(track), playbackRate: 1 });
    const getEqState = vi.fn().mockResolvedValue(eqState());
    const setEqEnabled = vi.fn().mockResolvedValue({ ...eqState(), enabled: true });
    const openPathInFolder = vi.fn().mockResolvedValue(undefined);
    const navigateLyrics = vi.fn();
    const navigateSettings = vi.fn();
    const navigateRoute = vi.fn();
    window.addEventListener('app:navigate:lyrics', navigateLyrics);
    ...
    window.echo = {
      ...
      app: {
        getSettings: vi.fn().mockResolvedValue({ smtcEnabled: true }),
        setSettings: vi.fn().mockResolvedValue({ smtcEnabled: true, playbackSpeed: 1, playbackSpeedMode: 'nightcore' }),
        onGlobalShortcutCommand: vi.fn((handler) => {
          globalShortcutHandlers[0] = handler as typeof globalShortcutHandlers[number];
          return () => {
            globalShortcutHandlers.length = 0;
      ...
      audio: {
        getStatus: vi.fn().mockResolvedValue({ ...audioStatus(track), playbackRate: 1.3, positionSeconds: 12 }),
        listDevices: vi.fn(),
        setOutput,
      },
      eq: {
    ...
    await screen.findByText('Song 1');
    globalShortcutHandlers[0]?.('replayCurrentTrack');
    await waitFor(() => expect(seek).toHaveBeenCalledWith(0));

    globalShortcutHandlers[0]?.('toggleLyrics');
    globalShortcutHandlers[0]?.('openSettings');
    globalShortcutHandlers[0]?.('openLiked');
    expect(navigateLyrics).toHaveBeenCalledWith(expect.objectContaining({ detail: { mode: 'lyrics' } }));
    expect(navigateSettings).toHaveBeenCalledTimes(1);
    expect(navigateRoute).toHaveBeenCalledWith(expect.objectContaining({ detail: 'liked' }));

    globalShortcutHandlers[0]?.('resetPlaybackSpeed');
    await waitFor(() => expect(setOutput).toHaveBeenCalledWith({ playbackRate: 1, playbackSpeedMode: 'nightcore' }));

    globalShortcutHandlers[0]?.('toggleEq');
    await waitFor(() => expect(setEqEnabled).toHaveBeenCalledWith(true));

    globalShortcutHandlers[0]?.('revealCurrentTrackInFolder');
    await waitFor(() => expect(openPathInFolder).toHaveBeenCalledWith(track.path));

    window.removeEventListener('app:navigate:lyrics', navigateLyrics);
  ...
  it('does not publish a browser media session before native SMTC diagnostics resolve', async () => {
    const track = makeTrack(1, {
      title: 'SMTC Startup Song',
      artist: 'SMTC Artist',
      album: 'SMTC Album',
    });
    const mediaSession = {
    ...
    window.echo = {
      ...
      smtc: {
        getDiagnostics: vi.fn(() => new Promise(() => undefined)),
        setEnabledActions: vi.fn().mockResolvedValue(undefined),
        onCommand: vi.fn(() => () => undefined),
      },
      eq: {
    ...
    await screen.findByText('SMTC Startup Song');
    await act(async () => {
      await Promise.resolve();
      await Promise.resolve();
    });
  ...
  it('keeps AirPlay receiver progress read-only when sender seek is unavailable', async () => {
  ...
  it('hides sleep timer and local audio export by default and shows them when enabled in appearance settings', async () => {
    ...
    expect(defaultContainer.querySelector('.sleep-timer-control')).toBeNull();
    ...
    expect(visibleContainer.querySelector('.sleep-timer-control')).toBeTruthy();
