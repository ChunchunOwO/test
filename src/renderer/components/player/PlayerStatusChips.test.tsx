...
describe('PlayerStatusChips', () => {
  ...
  it('surfaces remote playback loading even when codec chips are available', () => {
    render(<PlayerStatusChips status={null} state="loading" track={track({ mediaType: 'remote', sourceDisplayName: 'WebDAV' })} />);
  ...
  it('shows an Intelligent Transition chip only when the engine has an active transition plan', () => {
    ...
    expect(document.querySelector('.tag-automix')).toBeNull();
  });
});
