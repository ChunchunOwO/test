...
export const PlayerVolumeControl = ({
  ...
  const handleWheel = useCallback(
    (event: WheelEvent): void => {
      ...
      onOpenChange(true);
      if (fixedVolumeEnabled || event.deltaY === 0) {
        return;
      }
      const direction = event.deltaY > 0 ? -1 : 1;
      void commitVolume(volume + direction * 0.03);
    },
    [commitVolume, fixedVolumeEnabled, onOpenChange, volume],
  );
  ...
  return (
    <div className="volume-control" ref={rootRef} onMouseEnter={() => onOpenChange(true)}>
      <button
