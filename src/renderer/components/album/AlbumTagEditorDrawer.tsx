...
import type { EditableAlbumTags, LibraryAlbum, LibraryTrack, NetworkTagCandidate, TrackCoverSelection } from '../../../shared/types/library';
...
export const AlbumTagEditorDrawer = ({ album, isOpen, isSaving, error, onClose, onSave, onOpenInFolder, onDeleteAlbum }: AlbumTagEditorDrawerProps): JSX.Element | null => {
  ...
  const [hydratedGenre, setHydratedGenre] = useState('');
  const [genreHydration, setGenreHydration] = useState<{ status: 'pending' | 'ready'; mixed: boolean }>({
    status: 'pending',
    mixed: false,
  });
  ...
  const initialForm = useMemo(
    () => ({ ...stateFromAlbum(album), genre: hydratedGenre }),
    [album, hydratedGenre],
  );
  ...
  useEffect(() => {
    if (album) {
      ...
      setHydratedGenre('');
      setGenreHydration({ status: 'pending', mixed: false });
  ...
  useEffect(() => {
    ...
    if (!library?.getAlbumTracks) {
      setGenreHydration({ status: 'ready', mixed: false });
      return;
    }

    let cancelled = false;
    const pageSize = Math.min(Math.max(album.trackCount, 1), 500);
    void library.getAlbumTracks(album.id, { page: 1, pageSize }).then((result: { items: LibraryTrack[] }) => {
      if (cancelled) {
        return;
      }

      const snapshot = albumGenreEditorValue(result.items);
      setGenreHydration({ status: 'ready', mixed: snapshot.mixed });
      setHydratedGenre(snapshot.genre);
      setForm((current) => {
        if (genreEditedRef.current || (current.genre.trim().length > 0 && current.genre !== snapshot.genre)) {
          return current;
        }
        return current.genre === snapshot.genre ? current : { ...current, genre: snapshot.genre };
      });
    }).catch(() => {
      if (!cancelled) {
        setGenreHydration({ status: 'ready', mixed: false });
  ...
  const applyNetworkCandidate = (candidate: NetworkTagCandidate, selection: NetworkFieldSelection): void => {
    if (selection.genre && candidate.genre) {
      genreEditedRef.current = true;
    }
  ...
  const handleSubmit = (event: FormEvent): void => {
    ...
    const tags: EditableAlbumTags = {
      album: form.album,
      albumArtist: form.albumArtist,
      year: numberOrNull(form.year),
    };
    const shouldWriteGenre =
      genreEditedRef.current
      || form.genre.trim() !== hydratedGenre.trim()
      || (genreHydration.status === 'ready' && !genreHydration.mixed);
    if (shouldWriteGenre) {
      tags.genre = form.genre.trim() || null;
    }
