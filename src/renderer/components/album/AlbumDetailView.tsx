...
export const AlbumDetailView = ({ album, onBack }: AlbumDetailViewProps): JSX.Element => {
  ...
  const albumGenreLabels = useMemo(
    () => uniqueValues(loadedTracks.map((track) => track.genre)).slice(0, 3),
    [loadedTracks],
  );
  const albumFacts = useMemo(
    () => {
      ...
      return [
        ...
        ...(albumGenreLabels.length > 0
          ? [{ label: t('albumDetail.fact.genre'), value: albumGenreLabels.join(' · ') }]
          : []),
    ...
    [album.trackCount, album.year, albumGenreLabels, loadedTotal, loadedTracks.length, locale, onlineInfoState.info?.externalRatings, signalItems, t],
  );
  ...
  const loadRelatedAlbums = useCallback(async (): Promise<void> => {
    ...
    try {
      const sourceProvider = 'local' as const;
  ...
  const loadStreamingAlbums = useCallback(async (): Promise<void> => {
    ...
    if (!streaming?.search) {
      setStreamingAlbumsState({
        loading: false,
        albums: [],
        error: '桌面桥接不可用。请在 ECHO 桌面版中搜索流媒体专辑。',
        loadedForAlbumId: album.id,
      });
      return;
    }
    ...
    async (action: TrackMenuAction, track: LibraryTrack, playlistTarget?: LibraryPlaylist): Promise<void> => {
      ...
      if (action === 'clear-lyrics-cache') {
        ...
        if (!lyricsApi?.clearCache) {
          setPlayError('Desktop bridge unavailable. Open ECHO in Electron to clear lyrics cache.');
          return;
        }
  ...
  useEffect(() => {
    ...
    const loadStreamingAlbumDetail = async (): Promise<void> => {
      ...
      if (!streaming?.getAlbum) {
        setSelectedStreamingAlbumDetail(null);
        setStreamingAlbumDetailError('桌面桥接不可用。请在 ECHO 桌面版中读取流媒体专辑。');
        setIsStreamingAlbumDetailLoading(false);
        return;
      }
  ...
  const renderRelatedAlbums = (): JSX.Element | null => {
    if (album.mediaType === 'remote' || !displayAlbumArtist) {
      return null;
    }
