...
describe('ArtistAlbumGrid', () => {
  ...
  it('selects an artist album by click and keyboard', async () => {
    ...
    const albumCard = async (): Promise<HTMLElement> => screen.findByRole('button', { name: /Refrain/i });
    fireEvent.click(await albumCard());
    fireEvent.keyDown(await albumCard(), { key: 'Enter' });
    fireEvent.keyDown(await albumCard(), { key: ' ' });
  ...
  it('can sort artist albums by release year', async () => {
    const getArtistAlbums = vi.fn().mockResolvedValue(page([album()]));
    installLibrary(getArtistAlbums);

    renderAlbumGrid();
    await screen.findByRole('button', { name: /Refrain/i });
    expect(getArtistAlbums).toHaveBeenCalledWith('artist-1', { page: 1, pageSize: 12, sort: 'recent' });

    fireEvent.click(screen.getByRole('button', { name: /专辑排序|Album sort/u }));
    fireEvent.click(screen.getByRole('option', { name: /发行年份最新|Release year, newest/u }));

    await waitFor(() => expect(getArtistAlbums).toHaveBeenCalledWith('artist-1', { page: 1, pageSize: 12, sort: 'yearDesc' }));
  });
});
