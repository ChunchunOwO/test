...
describe('ArtistDetailView', () => {
  it('shows the loading state while artist tracks are being read', async () => {
    ...
    expect((document.querySelector('.artist-primary-action') as HTMLButtonElement).disabled).toBe(true);
  });
  ...
  it('renders online artist bio links and concert cards after the background insights load', async () => {
    ...
    installLibrary(vi.fn().mockResolvedValue(artist()), { onlineArtistInfoBandsintownAppId: 'echo', onlineArtistInfoRegion: 'HK' }, getArtistInsights);
    window.echo = {
      ...window.echo,
      app: {
        ...window.echo?.app,
