...
import {
  defaultWallImageMaxConcurrent,
  defaultWallImageRootMargin,
  isLowSpecModeDomActive,
  lowSpecWallImageMaxConcurrent,
  lowSpecWallImageRootMargin,
} from '../../../shared/utils/performancePolicy';
import { useRouteActivity } from './RouteActivityContext';
import { getPageScrollContainer } from './InfiniteScrollSentinel';

const wallImageIdleDelayMs = 180;
const wallImageSlotLeaseMs = 10_000;

const wallImageMaxConcurrent = (): number =>
  isLowSpecModeDomActive() ? lowSpecWallImageMaxConcurrent : defaultWallImageMaxConcurrent;

const resolveWallImageRootMargin = (rootMargin?: string): string =>
  rootMargin ?? (isLowSpecModeDomActive() ? lowSpecWallImageRootMargin : defaultWallImageRootMargin);

let activeWallImageLoads = 0;
const wallImageQueue: Array<() => void> = [];

const drainWallImageQueue = (): void => {
  while (activeWallImageLoads < wallImageMaxConcurrent() && wallImageQueue.length > 0) {
...
const requestWallImageSlot = (onGrant: (release: () => void) => void): (() => void) => {
  ...
  if (activeWallImageLoads < wallImageMaxConcurrent()) {
    grant();
  } else {
...
export const DeferredWallImage = ({
  ...
  const resolvedRootMargin = resolveWallImageRootMargin(rootMargin);
  ...
  }, [isRouteActive, priority]);

  useEffect(() => {
    if (!isRouteActive || priority || isInViewport) {
      return undefined;
    }
  ...
  }, [isInViewport, isRouteActive, priority]);

  useEffect(() => {
    if (!isRouteActive || priority || isNearViewport) {
      return undefined;
    }
    ...
    observer.observe(anchor);
    return () => observer.disconnect();
  }, [isNearViewport, isRouteActive, priority, resolvedRootMargin]);

  useEffect(() => {
    if (!isRouteActive || canLoad || !isNearViewport) {
      return undefined;
    }
  ...
  }, [canLoad, isInViewport, isNearViewport, isRouteActive, paused, priority]);
  ...
  return (
    <span className="deferred-wall-image" data-ready={canLoad ? 'true' : 'false'} ref={anchorRef}>
      {isRouteActive && canLoad ? <img {...imageProps} src={src} onError={handleError} onLoad={handleLoad} /> : null}
    </span>
  );
};
