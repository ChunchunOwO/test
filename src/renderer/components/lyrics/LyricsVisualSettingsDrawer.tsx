...
import '../../styles/lyrics-visual-drawer-polish.css';
import {
  Captions,
  Check,
  ChevronDown,
...
const LyricsVisualGroup = ({ className, children, description, icon, isOpen, onToggle, title }: LyricsVisualGroupProps): JSX.Element => {
  return (
    <div
      className={`lyrics-visual-group${isOpen ? ' lyrics-visual-group--open' : ''}${className ? ` ${className}` : ''}`}
    ...
    >
      ...
      <div className="lyrics-visual-group__shell" aria-hidden={!isOpen} {...(!isOpen ? { inert: '' } : {})}>
        <div className="lyrics-visual-group__content">{children}</div>
      </div>
    </div>
  );
};
...
export const LyricsVisualSettingsDrawer = ({ isOpen, onClose }: LyricsVisualSettingsDrawerProps): JSX.Element | null => {
  ...
  const activeLyricsBackgroundMode = appSettings.lyricsBackgroundModeOverrideEnabled === true
    ? appSettings.lyricsBackgroundMode
    : null;
  ...
  const lyricsPageStyleOptions = useMemo<Array<{ mode: LyricsPageStyle; label: string }>>(
    () => {
      const options: Array<{ mode: LyricsPageStyle; label: string }> = [
        ...
        { mode: 'folded', label: t('lyricsSettings.visual.pageStyleFolded') },
        { mode: 'roseVinyl', label: t('lyricsSettings.visual.pageStyleRoseVinyl') },
        { mode: 'cinemaStage', label: t('lyricsSettings.visual.pageStyleCinemaStage') },
        { mode: 'kineticPoster', label: t('lyricsSettings.visual.pageStyleKineticPoster') },
        { mode: 'coverStage', label: t('lyricsSettings.visual.pageStyleCoverStage') },
        { mode: 'cutBoard', label: t('lyricsSettings.visual.pageStyleCutBoard') },
      ];
      return options.filter((option) => isLyricsPageStyleVisible(option.mode));
    },
    [t],
  );
  const lyricsPageStyle = resolveVisibleLyricsPageStyle(
    appSettings.lyricsPageStyle ?? fallbackAppSettings.lyricsPageStyle ?? 'default',
  );
  const isLyricsMiniPlayerRequired = isLyricsMiniPlayerRequiredForPageStyle(lyricsPageStyle);
  const isLyricsMiniPlayerEnabled =
    isLyricsMiniPlayerRequired || appSettings.lyricsPlayerBarDrawerEnabled === true;
  ...
  const patchAppSettings = useCallback(async (patch: Partial<AppSettings>, optimistic = true): Promise<void> => {
    ...
    try {
      ...
      if (requestId === appSaveRequestIdRef.current) {
        const nextVisualSettings = selectAppSettings(overlayLyricsPageStylePatch(nextSettings, patch));
  ...
  const flushDebouncedSettings = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (requestId === debouncedSaveRequestIdRef.current) {
        setAppSettings(selectAppSettings(overlayLyricsPageStylePatch(nextSettings, patch)));
        setError(null);
        dispatchSettingsChanged(patch);
      }
    } catch (settingsError) {
          ...
          <section className="audio-drawer-section audio-drawer-options audio-drawer-options--open">
            ...
            <div className="lyrics-color-panel lyrics-page-style-panel" data-style={lyricsPageStyle}>
              ...
              <StyledSelect<LyricsPageStyle>
                ...
                leadingIcon={<Captions size={16} aria-hidden="true" />}
                onChange={(mode) => void patchAppSettings({ lyricsPageStyle: mode })}
              />
              <p>{t('lyricsSettings.visual.pageStyleDescription')}</p>
              <label className="audio-toggle-row lyrics-theme-filter-toggle">
                <span>
                  <Palette size={17} />
                  <strong>{t('lyricsSettings.visual.themeFilter')}</strong>
                </span>
                <input
                  type="checkbox"
                  checked={appSettings.lyricsThemeFilterEnabled !== false}
                  disabled={isBusy}
                  onChange={(event) => void patchAppSettings({ lyricsThemeFilterEnabled: event.currentTarget.checked })}
                />
              </label>
              <p>{t('lyricsSettings.visual.themeFilterDescription')}</p>
              {lyricsPageStyle === 'roseVinyl' ? (
                <div className="lyrics-page-style-panel__rose-controls">
                  ...
                  <label className="audio-toggle-row lyrics-rose-vinyl-gradient-toggle">
                    <span>
                      <Palette size={17} />
                      <strong>{t('lyricsSettings.visual.roseVinylGradient')}</strong>
                    </span>
                    <input
                      type="checkbox"
                      checked={appSettings.lyricsRoseVinylGradientEnabled === true}
                      disabled={isBusy}
                      onChange={(event) => void patchAppSettings({ lyricsRoseVinylGradientEnabled: event.currentTarget.checked })}
                    />
                  </label>
                  <p>{t('lyricsSettings.visual.roseVinylGradientDescription')}</p>
                </div>
              ) : null}
            </div>
            </LyricsVisualGroup>
            ...
            <LyricsVisualGroup
              className="lyrics-visual-group--background"
              icon={<ImageIcon size={17} />}
                    ...
                    {lyricsBackgroundModeOptions.map((option) => (
                      <button
                        ...
                        onClick={() => {
                          setIsBackgroundModeMenuOpen(false);
                          if (option.mode === null) {
                            void patchAppSettings(createLyricsBackgroundModeSelectionPatch(null));
                            return;
                          }
                          ...
                          void patchAppSettings(createLyricsBackgroundModeSelectionPatch(option.mode));
                        }}
                      >
