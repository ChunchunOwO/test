...
const presets: EqPreset[] = [
  ...
  { id: 'rock', name: 'Rock Drive', preampDb: -3, bands, createdAt: 'built-in', updatedAt: 'built-in', readonly: true },
  { id: 'harman-target', name: 'Harman Inspired · Balanced', preampDb: -5, bands, createdAt: 'built-in', updatedAt: 'built-in', readonly: true },
...
beforeEach(() => {
  ...
  window.localStorage.removeItem('echo.eq.uiMode');
  window.localStorage.removeItem('echo.eq.spectrumAnalyzer');
  window.localStorage.removeItem('echo.eq.analyzerMode');
  window.localStorage.removeItem('echo.eq.autoGainEnabled');
  ...
  window.echo = {
    eq: {
      ...
      setPreset: vi.fn().mockImplementation((presetId: string) => Promise.resolve(eqState({ presetId, presetName: presetId === 'rock' ? 'Rock Drive' : 'User Bright' }))),
...
describe('EqPanel', () => {
  it('routes EQ failures to the upper-left notice without rendering an inline error', async () => {
    ...
    try {
      ...
      expect((notices[0] as CustomEvent<{ message: string }>).detail.message).toBeTruthy();
      expect((notices[0] as CustomEvent<{ message: string }>).detail.message).not.toBe('eq bridge failed');
  ...
  it('keeps detached DSP modules out of the EQ-only surface', async () => {
    ...
    expect(await screen.findByTestId('eq-curve-node-0')).toBeTruthy();
    expect(screen.getByLabelText('Quick EQ preamp')).toBeTruthy();
    ...
    expect(container.querySelector('.eq-quick-strip')).toBeNull();
    expect(container.querySelector('.eq-signal-strip')).toBeNull();
    expect(container.querySelector('.eq-simple-summary')).toBeNull();
    expect(container.querySelector('.eq-curve-view')?.getAttribute('data-graphic')).toBe('true');
    expect(container.querySelector('.eq-curve-stem')).toBeTruthy();
    expect(container.querySelector('.eq-curve-focus-line')).toBeTruthy();
    expect(container.querySelector('.eq-curve-track')).toBeNull();
    expect(container.querySelector('.eq-curve-simple-hint')).toBeNull();
    expect(container.querySelector('.eq-curve-footer-actions')).toBeTruthy();
    expect(screen.getByTestId('eq-simple-reset-selected')).toBeTruthy();
    expect(screen.getByTestId('eq-simple-gain-up')).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Reset curve' })).toBeTruthy();
    expect(screen.getByTestId('eq-curve-node-0').getAttribute('style')).toContain('--eq-band-color-');
  ...
  it('lets the dedicated EQ surface flatten the whole curve', async () => {
    renderEqPanel(audioStatus, { surface: 'eq-only' });

    fireEvent.click(await screen.findByRole('button', { name: 'Reset curve' }));

    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalledWith({ band: 1, gainDb: 0 }));
    expect(window.echo.eq.reset).not.toHaveBeenCalled();
  });

  it('lets Simple mode on the dedicated EQ surface drag a band and auto-enable EQ', async () => {
    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });

    const curve = await screen.findByTestId('eq-curve-node-2');
    const svg = container.querySelector('.eq-curve-view') as SVGSVGElement;
    ...
    expect(svg.getAttribute('data-graphic')).toBe('true');
    fireEvent.pointerDown(curve, { clientX: 410, clientY: 94, pointerId: 1 });
    fireEvent.pointerUp(svg, { clientX: 410, clientY: 94, pointerId: 1 });

    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalled());
    await waitFor(() => expect(window.echo.eq.setEnabled).toHaveBeenCalledWith(true));
  });

  it('lets Simple EQ grab a fader, zero it with a double-click, and step the selected band', async () => {
    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });

    const fader = await screen.findByTestId('eq-curve-fader-4');
    const svg = container.querySelector('.eq-curve-view') as SVGSVGElement;
    svg.getBoundingClientRect = vi.fn(() => ({
      x: 0,
      y: 0,
    ...
    fireEvent.pointerDown(fader, { clientX: 410, clientY: 94, pointerId: 1 });
    fireEvent.pointerUp(svg, { clientX: 410, clientY: 94, pointerId: 1 });
    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalled());

    const boosted = screen.getByTestId('eq-curve-node-1');
    fireEvent.pointerDown(boosted, { clientX: 200, clientY: 180, pointerId: 2, detail: 2 });
    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalledWith({ band: 1, gainDb: 0 }));

    fireEvent.keyDown(screen.getByTestId('eq-curve-node-2'), { key: 'ArrowRight' });
    expect(screen.getByTestId('eq-curve-node-3').getAttribute('data-active')).toBe('true');

    fireEvent.click(screen.getByTestId('eq-simple-gain-up'));
    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalledWith({ band: 3, gainDb: 0.5 }));

    fireEvent.click(screen.getByTestId('eq-simple-reset-selected'));
    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalledWith({ band: 3, gainDb: 0 }));
  });

  it('lets Simple EQ drag a band left and right to change frequency', async () => {
    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });

    const node = await screen.findByTestId('eq-curve-node-4');
    const svg = container.querySelector('.eq-curve-view') as SVGSVGElement;
    svg.getBoundingClientRect = vi.fn(() => ({
      x: 0,
      y: 0,
    ...
    fireEvent.pointerDown(node, { clientX: 168, clientY: 174, pointerId: 1 });
    fireEvent.pointerMove(svg, { clientX: 410, clientY: 174, pointerId: 1 });
    fireEvent.pointerUp(svg, { clientX: 410, clientY: 174, pointerId: 1 });

    await waitFor(() => expect(window.echo.eq.setBandFrequency).toHaveBeenCalled());
    const frequencyCall = vi.mocked(window.echo.eq.setBandFrequency).mock.calls.at(-1)?.[0] as { band: number; frequencyHz: number };
    expect(frequencyCall.band).toBe(4);
    expect(frequencyCall.frequencyHz).toBeGreaterThan(80);
  });

  it('anchors every Simple EQ node to the combined response path', async () => {
    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });

    const node = await screen.findByTestId('eq-curve-node-1');
    const transform = node.getAttribute('transform') ?? '';
    const match = /^translate\(([-\d.]+) ([-\d.]+)\)$/.exec(transform);
    const curvePath = container.querySelector('.eq-curve-stroke')?.getAttribute('d') ?? '';

    expect(match).not.toBeNull();
    expect(curvePath).toContain(`${match?.[1]} ${match?.[2]}`);

    fireEvent.pointerEnter(node);
    expect((await screen.findByTestId('eq-simple-response-readout')).textContent).toContain('Response');
    expect(container.querySelector('.eq-curve-focus-line--horizontal')).toBeTruthy();
  });

  it('keeps a Simple EQ drag active when pointer events continue on the window', async () => {
    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });
    ...
    fireEvent.pointerDown(node, { clientX: 168, clientY: 220, pointerId: 41 });
    fireEvent.pointerMove(window, { clientX: 168, clientY: 94, pointerId: 41 });
    fireEvent.pointerUp(window, { clientX: 168, clientY: 94, pointerId: 41 });

    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalled());
    const gainCall = vi.mocked(window.echo.eq.setBandGain).mock.calls.at(-1)?.[0] as { band: number; gainDb: number };
    expect(gainCall.band).toBe(4);
    expect(gainCall.gainDb).toBeCloseTo(4.8, 5);
  });

  it('keeps Simple EQ frequency still when a fader is dragged vertically', async () => {
    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });
    ...
    fireEvent.pointerDown(node, { clientX: 168, clientY: 220, pointerId: 1 });
    fireEvent.pointerMove(svg, { clientX: 170, clientY: 94, pointerId: 1 });
    fireEvent.pointerUp(svg, { clientX: 170, clientY: 94, pointerId: 1 });

    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalled());
    expect(window.echo.eq.setBandFrequency).not.toHaveBeenCalled();
  });

  it('lets Pro EQ show the selected filter, Alt-drag Q, and Ctrl-click bypass', async () => {
    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });
    await showAdvancedEqTools();

    fireEvent.click(await screen.findByTestId('eq-curve-node-1'));
    expect(container.querySelector('.eq-curve-selected-band')).toBeTruthy();
    expect(container.querySelector('.eq-curve-selected-fill')).toBeTruthy();
    expect(container.querySelector('.eq-curve-focus-line')).toBeTruthy();
    expect(container.querySelector('.eq-spectrum-state')).toBeNull();
    ...
    const node = screen.getByTestId('eq-curve-node-1');
    fireEvent.pointerDown(node, { clientX: 200, clientY: 120, pointerId: 1, altKey: true });
    fireEvent.pointerMove(svg, { clientX: 360, clientY: 120, pointerId: 1, altKey: true });
    fireEvent.pointerUp(svg, { clientX: 360, clientY: 120, pointerId: 1, altKey: true });

    await waitFor(() => expect(window.echo.eq.setBandQ).toHaveBeenCalledWith(expect.objectContaining({ band: 1 })));
    expect(window.echo.eq.setBandGain).not.toHaveBeenCalled();

    fireEvent.pointerDown(node, { clientX: 200, clientY: 120, pointerId: 2, ctrlKey: true });
    await waitFor(() => expect(window.echo.eq.setBandEnabled).toHaveBeenCalledWith({ band: 1, enabled: false }));
  });
  ...
  it('does not render the removed Simple listening cards and actions', async () => {
    renderEqPanel();

    await screen.findByRole('img', { name: 'Draggable 31-band EQ frequency response' });
    expect(screen.queryByRole('button', { name: /Bass lift/i })).toBeNull();
    expect(screen.queryByLabelText('Simple current vibe summary')).toBeNull();
    expect(screen.queryByLabelText('Simple listening zone changes')).toBeNull();
    expect(screen.queryByLabelText('Simple safe headroom')).toBeNull();
    expect(screen.queryByRole('button', { name: 'Next vibe' })).toBeNull();
    expect(screen.queryByRole('button', { name: 'Save vibe' })).toBeNull();
  });
  ...
  it('updates PEQ band Q, filter type, and bypass state from the advanced inspector', async () => {
    ...
    const qInput = await screen.findByLabelText('Q');
    const typeSelect = container.querySelector('.eq-inspector-fields select') as HTMLSelectElement;
    const enabledInput = container.querySelector('.eq-inspector-toggle input') as HTMLInputElement;
    fireEvent.change(qInput, { target: { value: '2.4' } });
    fireEvent.blur(qInput);
    fireEvent.change(typeSelect, { target: { value: 'lowShelf' } });
  ...
  it('keeps a locked EQ node on its current frequency while dragging gain', async () => {
    renderEqPanel();
  ...
  it('uses a curve-first active filter stack on the dedicated Pro EQ surface', async () => {
    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });
    await showAdvancedEqTools();

    await waitFor(() => {
      expect(container.querySelector('.eq-panel')?.getAttribute('data-mode')).toBe('pro');
      expect(container.querySelectorAll('[data-testid^="eq-curve-node-"]')).toHaveLength(2);
      expect(container.querySelectorAll('.eq-band-chip:not(.eq-band-chip--add)')).toHaveLength(2);
    });
    expect((screen.getByTestId('eq-pro-frequency-snap') as HTMLInputElement).checked).toBe(false);
    expect(container.querySelector('.eq-header-analyzer .eq-analyzer-toggle')?.getAttribute('aria-pressed')).toBe('false');
    expect(container.querySelector('.eq-header-analyzer .eq-analyzer-mode')).toBeNull();

    const curve = container.querySelector('.eq-curve-view') as SVGSVGElement;
    curve.getBoundingClientRect = vi.fn(() => ({
      x: 0,
      y: 0,
      top: 0,
    ...
    fireEvent.doubleClick(curve, { clientX: 600, clientY: 120 });

    await waitFor(() => expect(window.echo.eq.setBandEnabled).toHaveBeenCalledWith({ band: 2, enabled: true }));
    await waitFor(() => expect(window.echo.eq.setEnabled).toHaveBeenCalledWith(true));
    expect(container.querySelector('.eq-pro-live-summary [data-state="live"]')).toBeTruthy();
    expect(container.querySelector('.eq-pro-command-bar')).toBeTruthy();
    expect(screen.getByTestId('eq-pro-band-editor')).toBeTruthy();
    expect(container.querySelector('.eq-pro-band-editor__width')).toBeTruthy();

    const wideRangeButton = container.querySelector('[data-q-preset="wide"]') as HTMLButtonElement;
    fireEvent.click(wideRangeButton);
    await waitFor(() => expect(window.echo.eq.setBandQ).toHaveBeenCalledWith(expect.objectContaining({ band: 2 })));

    fireEvent.click(container.querySelector('[data-pro-tool-target="profile"]') as HTMLButtonElement);
    expect(container.querySelector('.eq-profile-panel')?.getAttribute('data-pro-visible')).toBe('true');
    fireEvent.click(container.querySelector('[data-pro-tool-target="preset"]') as HTMLButtonElement);
    expect(container.querySelector('.eq-preset-console')?.getAttribute('data-pro-visible')).toBe('true');
    fireEvent.click(container.querySelector('[data-pro-tool-target="preset"]') as HTMLButtonElement);
    expect(container.querySelector('.eq-panel')?.getAttribute('data-pro-tool')).toBe('precision');

    const node = container.querySelector('[data-testid^="eq-curve-node-"]') as SVGGElement;
    const band = Number(node.getAttribute('data-testid')?.replace('eq-curve-node-', ''));
    fireEvent.wheel(node, { deltaY: -100 });
    await waitFor(() => expect(window.echo.eq.setBandQ).toHaveBeenCalledWith(expect.objectContaining({ band })));

    fireEvent.keyDown(node, { key: 'Delete' });
    await waitFor(() => expect(window.echo.eq.setBandEnabled).toHaveBeenCalledWith({ band, enabled: false }));
  });

  it('uses one visible sequence across curve nodes, editor, and newly added filters', async () => {
    const configuredIndexes = new Set([0, 2, 4, 6, 8, 10, 12, 14]);
    vi.mocked(window.echo.eq.getState).mockResolvedValueOnce(eqState({
      bands: bands.map((band, index) => configuredIndexes.has(index) ? { ...band, gainDb: index === 0 ? -1 : 1 } : band),
    }));

    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });
    await showAdvancedEqTools();

    await waitFor(() => expect(container.querySelectorAll('[data-testid^="eq-curve-node-"]')).toHaveLength(8));
    fireEvent.click(await screen.findByTestId('eq-curve-node-12'));

    expect(screen.getByTestId('eq-curve-node-12').querySelector('.eq-curve-node-type')?.textContent).toBe('7');
    expect(container.querySelector('.eq-pro-band-editor__index')?.textContent).toBe('7');
    expect(screen.getByText('Editing filter 7')).toBeTruthy();

    fireEvent.click(container.querySelector('.eq-pro-add-filter') as HTMLButtonElement);

    await waitFor(() => expect(window.echo.eq.setBandQ).toHaveBeenCalledWith({ band: 13, q: 1.4 }));
    await waitFor(() => expect(container.querySelectorAll('[data-testid^="eq-curve-node-"]')).toHaveLength(9));
    expect(container.querySelectorAll('.eq-band-chip:not(.eq-band-chip--add)')).toHaveLength(9);
    expect(container.querySelector('.eq-pro-band-editor__index')?.textContent).toBe('8');

    fireEvent.click(await screen.findByTestId('eq-curve-node-0'));
    expect(container.querySelectorAll('[data-testid^="eq-curve-node-"]')).toHaveLength(9);
  });
  ...
  it('does not let a late gain acknowledgement roll back a dragged frequency', async () => {
    let resolveGain!: (state: EqState) => void;
    (window.echo.eq.setBandGain as ReturnType<typeof vi.fn>).mockImplementationOnce(() => new Promise<EqState>((resolve) => {
      resolveGain = resolve;
    }));
    (window.echo.eq.setBandFrequency as ReturnType<typeof vi.fn>).mockImplementationOnce(({ band, frequencyHz }: { band: number; frequencyHz: number }) =>
      Promise.resolve(eqState({
        presetId: 'custom',
        presetName: 'Custom',
        bands: bands.map((item, index) => (index === band ? { ...item, frequencyHz } : item)),
      })),
    );
    ...
    await waitFor(() => expect(window.echo.eq.setBandFrequency).toHaveBeenCalled());
    const frequencyHz = (window.echo.eq.setBandFrequency as ReturnType<typeof vi.fn>).mock.calls.at(-1)?.[0].frequencyHz as number;
    resolveGain(eqState({
      presetId: 'custom',
      presetName: 'Custom',
      bands: bands.map((item, index) => (index === 2 ? { ...item, gainDb: 5.5 } : item)),
    }));

    await waitFor(() => expect((screen.getByLabelText('Frequency') as HTMLInputElement).value).toBe(String(Math.round(frequencyHz))));
    expect((screen.getByLabelText('Gain') as HTMLInputElement).value).toBe('5.5');
  });

  it('does not let a late initial refresh overwrite a Pro curve drag', async () => {
    let resolveRefresh!: (state: EqState) => void;
    vi.mocked(window.echo.eq.getState).mockImplementationOnce(() => new Promise<EqState>((resolve) => {
      resolveRefresh = resolve;
    }));

    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });
    await showAdvancedEqTools();
    const frequencySnap = await screen.findByTestId('eq-pro-frequency-snap') as HTMLInputElement;
    expect(frequencySnap.checked).toBe(false);
    ...
    fireEvent.pointerDown(node, { clientX: 650, clientY: 120, pointerId: 1 });
    fireEvent.pointerUp(curve, { clientX: 650, clientY: 120, pointerId: 1 });

    await waitFor(() => expect(window.echo.eq.setBandFrequency).toHaveBeenCalled());
    const draggedFrequency = vi.mocked(window.echo.eq.setBandFrequency).mock.calls.at(-1)?.[0].frequencyHz as number;
    resolveRefresh(eqState());

    const frequencyInput = container.querySelector('.eq-pro-band-editor__field input') as HTMLInputElement;
    await waitFor(() => expect(frequencyInput.value).toBe(String(Math.round(draggedFrequency))));
  });

  it('does not let a late add-filter acknowledgement reset a filter dragged immediately after adding it', async () => {
    let resolveAddedFilter!: (state: EqState) => void;
    vi.mocked(window.echo.eq.setBandEnabled).mockImplementationOnce(() => new Promise<EqState>((resolve) => {
      resolveAddedFilter = resolve;
    }));

    const { container } = renderEqPanel(audioStatus, { surface: 'eq-only' });
    await showAdvancedEqTools();
    await waitFor(() => expect(container.querySelectorAll('[data-testid^="eq-curve-node-"]')).toHaveLength(2));
    expect((await screen.findByTestId('eq-pro-frequency-snap') as HTMLInputElement).checked).toBe(false);
    ...
    fireEvent.doubleClick(curve, { clientX: 520, clientY: 150 });
    await waitFor(() => expect(window.echo.eq.setBandEnabled).toHaveBeenCalledWith({ band: 2, enabled: true }));

    const addedNode = await screen.findByTestId('eq-curve-node-2');
    fireEvent.pointerDown(addedNode, { clientX: 760, clientY: 90, pointerId: 2 });
    fireEvent.pointerUp(curve, { clientX: 760, clientY: 90, pointerId: 2 });
    await waitFor(() => expect(vi.mocked(window.echo.eq.setBandFrequency).mock.calls.filter(([request]) => request.band === 2).length).toBeGreaterThan(1));
    const draggedFrequency = vi.mocked(window.echo.eq.setBandFrequency).mock.calls.filter(([request]) => request.band === 2).at(-1)?.[0].frequencyHz as number;

    resolveAddedFilter(eqState({
      presetId: 'custom',
      presetName: 'Custom',
      bands: bands.map((item, index) => (index === 2 ? { ...item, frequencyHz: 1000, enabled: true } : item)),
    }));
  ...
  it('keeps pass and notch node drags horizontal by avoiding gain updates', async () => {
    ...
    fireEvent.click(screen.getByLabelText('Unlock frequency'));
    ...
    expect(window.echo.eq.setBandGain).not.toHaveBeenCalled();
    await waitFor(() => expect(window.echo.eq.setBandFrequency).toHaveBeenCalledWith({ band: 0, frequencyHz: expect.any(Number) }));
  });
  ...
  it('shows Auto Gain in Simple and Pro modes and persists the toggle', async () => {
    ...
    await waitFor(() => expect(window.localStorage.getItem('echo.eq.autoGainEnabled')).toBe('true'));
    await showAdvancedEqTools();
  ...
  it('automatically lowers preamp when Auto Gain sees realtime clipping risk', async () => {
    ...
    expect(await screen.findByText('Reducing')).toBeTruthy();
  });

  it('does not repeatedly lower preamp for a historical cumulative clip count', async () => {
    window.localStorage.setItem('echo.eq.autoGainEnabled', 'true');
    renderEqPanel({
      ...audioStatus,
      clippingRisk: true,
      audioLevels: {
        ...
        estimatedOutputRmsDb: -24,
        headroomDb: 12,
        clipCount: 4,
        lastClipAt: '2026-08-06T00:00:00.000Z',
        meterSource: 'pre_native_estimated_post_dsp',
      },
    });

    await screen.findByRole('button', { name: 'Auto Gain' });
    await waitFor(() => expect(window.echo.eq.getState).toHaveBeenCalled());
    expect(window.echo.eq.setPreamp).not.toHaveBeenCalled();
    expect(screen.getByText('Idle')).toBeTruthy();
  });
  ...
  it('adds and removes an APO-style filter slot through the advanced stack', async () => {
    ...
    await waitFor(() => expect(window.echo.eq.setBandGain).toHaveBeenCalledWith({ band: 2, gainDb: 0 }));
    await waitFor(() => expect(window.echo.eq.setBandQ).toHaveBeenCalledWith({ band: 2, q: 1.4 }));
  ...
  it('applies loudness-matched A/B restore through preamp compensation', async () => {
    ...
    const preampCallsBeforeRestore = vi.mocked(window.echo.eq.setPreamp).mock.calls.length;
    fireEvent.click(screen.getByRole('button', { name: 'Apply A' }));

    await waitFor(() => expect(vi.mocked(window.echo.eq.setPreamp).mock.calls.length).toBeGreaterThan(preampCallsBeforeRestore));
    expect(window.echo.eq.setPreamp).toHaveBeenLastCalledWith(-6);
  });
  ...
  it('selects presets, resets to Flat, and prevents built-in preset deletion', async () => {
    ...
    fireEvent.click(screen.getByRole('option', { name: 'Rock Drive' }));
  ...
  it('saves the current EQ to the in-app user preset list', async () => {
    renderEqPanel();

    fireEvent.change(await screen.findByLabelText('Preset name'), { target: { value: 'Desk Headphones' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save to presets' }));
    ...
    expect(window.echo.eq.listPresets).toHaveBeenCalled();
    expect(window.echo.eq.setPreset).toHaveBeenCalledWith('user-bright');
    expect(window.echo.eq.exportPreset).not.toHaveBeenCalled();
  });

  it('exports the current EQ as a preset file from the explicit export action', async () => {
    renderEqPanel();

    fireEvent.change(await screen.findByLabelText('Preset name'), { target: { value: 'Desk Headphones' } });
    fireEvent.click(screen.getByRole('button', { name: 'Export ECHO preset' }));
  ...
  it('filters presets by search and target curve category', async () => {
    ...
    expect(screen.getByRole('option', { name: 'Harman Inspired · Balanced' })).toBeTruthy();
  });
  ...
  it('renders EQ panel keys across supported locales', async () => {
    for (const locale of ['en-US', 'zh-CN', 'zh-TW', 'ja-JP']) {
      cleanup();
      window.localStorage.setItem('echo.locale', locale);
      renderEqPanel();
