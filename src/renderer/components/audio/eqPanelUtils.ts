...
export const eqAutoGainReductionIntervalMs = 500;
...
export const computeAutoGainPreamp = ({
  ...
  const safeReductionIntervalMs = Math.max(0, Number.isFinite(reductionIntervalMs) ? reductionIntervalMs : eqAutoGainReductionIntervalMs);
  ...
  if (realtimeRisk) {
    if (lastAdjustmentAtMs > 0 && nowMs - lastAdjustmentAtMs < safeReductionIntervalMs) {
    ...
    if (targetPreampDb < currentPreampDb - 0.05) {
      return {
        status: protectionEvent ? 'clipping' : 'reducing',
        targetPreampDb,
...
export const computeEqResponseGainDbFromCoefficients = (
  coefficients: BiquadCoefficients[],
  frequencyHz: number,
): number => {
  const safeFrequencyHz = clamp(frequencyHz, curveMinFrequencyHz, curveMaxFrequencyHz);
  const gainDb = coefficients.reduce(
    (sum, coefficient) => sum + computeBiquadGainDbAtFrequency(coefficient, safeFrequencyHz),
    0,
  );
...
export const computeEqResponseGainDbAtFrequency = (bands: EqBand[], frequencyHz: number): number =>
  computeEqResponseGainDbFromCoefficients(bands.map(makeEqBiquadCoefficients), frequencyHz);
...
export const resolveGraphicEqBandGainDb = (
  bands: EqBand[],
  index: number,
  frequencyHz: number,
  targetResponseGainDb: number,
): number => {
  const band = bands[index];
  if (!band || band.enabled === false || !isEqFilterGainEditable(band.filterType)) {
    return 0;
  }

  const safeFrequencyHz = clamp(frequencyHz, eqMinFrequencyHz, eqMaxFrequencyHz);
  const otherGainDb = bands.reduce((sum, candidate, candidateIndex) => (
    candidateIndex === index
      ? sum
      : sum + computeBiquadGainDbAtFrequency(makeEqBiquadCoefficients(candidate), safeFrequencyHz)
  ), 0);
  const responseAtGain = (gainDb: number): number => otherGainDb + computeBiquadGainDbAtFrequency(
    makeEqBiquadCoefficients({ ...band, frequencyHz: safeFrequencyHz, gainDb }),
    safeFrequencyHz,
  );
  const safeTargetGainDb = clamp(targetResponseGainDb, eqMinGainDb, eqMaxGainDb);
  const minResponseDb = responseAtGain(eqMinGainDb);
  const maxResponseDb = responseAtGain(eqMaxGainDb);

  if (safeTargetGainDb <= minResponseDb) {
    return eqMinGainDb;
  }
  if (safeTargetGainDb >= maxResponseDb) {
    return eqMaxGainDb;
  }

  let lowGainDb = eqMinGainDb;
  let highGainDb = eqMaxGainDb;
  for (let iteration = 0; iteration < 18; iteration += 1) {
    const candidateGainDb = (lowGainDb + highGainDb) / 2;
    if (responseAtGain(candidateGainDb) < safeTargetGainDb) {
      lowGainDb = candidateGainDb;
    } else {
      highGainDb = candidateGainDb;
    }
  }

  return clamp((lowGainDb + highGainDb) / 2, eqMinGainDb, eqMaxGainDb);
};
...
export const interpolateEqResponseGainDb = (
  samples: Array<{ x: number; gainDb: number }>,
  normalizedX: number,
): number => {
  if (samples.length === 0) {
    return 0;
  ...
  if (samples.length === 1) {
    return samples[0].gainDb;
  }

  const scaled = clamp(normalizedX, 0, 1) * (samples.length - 1);
  const leftIndex = Math.floor(scaled);
  const rightIndex = Math.min(samples.length - 1, leftIndex + 1);
  const mix = scaled - leftIndex;
  return samples[leftIndex].gainDb * (1 - mix) + samples[rightIndex].gainDb * mix;
};
...
export const computeEqSpectrumBars = (
  ...
  responseGainDbAtNormalizedX?: (normalizedX: number) => number,
): EqSpectrumBar[] => {
  if (!Array.isArray(spectrum) || spectrum.length === 0) {
    return [];
  }

  const coefficients = mode === 'postEq' && !responseGainDbAtNormalizedX
    ? bands.map(makeEqBiquadCoefficients)
    : null;
  ...
  return spectrum.map((value, index) => {
    ...
    const responseGainDb = mode !== 'postEq'
      ? 0
      : responseGainDbAtNormalizedX
        ? responseGainDbAtNormalizedX(x)
        : computeEqResponseGainDbFromCoefficients(coefficients ?? [], curveXToFrequency(x));
    return {
      x,
