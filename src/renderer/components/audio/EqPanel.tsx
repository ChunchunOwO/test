...
import { EqCurveView, getEqCurveBandColor } from './EqCurveView';
import { EqProBandEditor } from './EqProBandEditor';
import { resolveEqPresetLabel } from './eqPresetLabels';
import { EqPresetSelector } from './EqPresetSelector';
import '../../styles/eq-simple-workbench.css';
import {
  captureEqSnapshot,
...
type EqProToolPanel = 'precision' | 'profile' | 'preset';
type EqSimpleToneName = {
  id: 'bass' | 'vocal' | 'air' | 'warm' | 'flat';
...
const eqUiModeStorageKey = 'echo.eq.uiMode';
const eqAnalyzerStorageKey = 'echo.eq.spectrumAnalyzer';
const eqAnalyzerModeStorageKey = 'echo.eq.analyzerMode';
const eqAutoGainStorageKey = 'echo.eq.autoGainEnabled';
...
const readEqAnalyzerEnabled = (defaultValue = false): boolean => {
  try {
    const storedValue = window.localStorage.getItem(eqAnalyzerStorageKey);
...
const simpleToneNames: EqSimpleToneName[] = [
  { id: 'bass', labelKey: 'settings.eq.simpleTone.bass' },
  { id: 'vocal', labelKey: 'settings.eq.simpleTone.vocal' },
  { id: 'air', labelKey: 'settings.eq.simpleTone.air' },
  { id: 'warm', labelKey: 'settings.eq.simpleTone.warm' },
  { id: 'flat', labelKey: 'settings.eq.simpleTone.flat' },
];
...
export const EqPanel = ({ audioStatus, onAudioStatusRefresh, surface = 'full' }: EqPanelProps): JSX.Element => {
  ...
  const [proToolPanel, setProToolPanel] = useState<EqProToolPanel>('precision');
  const [spectrumAnalyzerEnabled, setSpectrumAnalyzerEnabled] = useState(() => readEqAnalyzerEnabled(false));
  ...
  const curveBandListRef = useRef<HTMLDivElement | null>(null);
  const profilePanelRef = useRef<HTMLElement | null>(null);
  const presetConsoleRef = useRef<HTMLElement | null>(null);
  const analyzerRefreshRef = useRef(onAudioStatusRefresh);
  const analyzerTranslationRef = useRef(t);
  ...
  const autoGainLastClipAt = useRef<string | null>(null);
  const autoGainProtectionActive = useRef(false);
  const autoGainEventsInitialized = useRef(false);
  const gainMutationRevisions = useRef<Record<number, number>>({});
  const frequencyMutationRevisions = useRef<Record<number, number>>({});
  const eqMutationRevision = useRef(0);
  const curveFirstLayoutActive = useRef(false);
  const curveEditEnablePending = useRef(false);
  const lowSpecModeEnabled = useLowSpecModeEnabled();
  const showAdvancedTools = eqUiMode === 'pro';
  const eqOnlySurface = surface === 'eq-only';
  const showEmbeddedDspModules = !eqOnlySurface;
  const simpleWorkbench = eqOnlySurface && !showAdvancedTools;
  const analyzerRuntimeEnabled = spectrumAnalyzerEnabled && !lowSpecModeEnabled;
  const curveSpectrumEnabled = (eqOnlySurface || showAdvancedTools) && analyzerRuntimeEnabled;

  useEffect(() => {
    analyzerRefreshRef.current = onAudioStatusRefresh;
    analyzerTranslationRef.current = t;
  }, [onAudioStatusRefresh, t]);
  ...
  const frequencyEditUnlocked = (simpleWorkbench || (showAdvancedTools && frequencyUnlocked)) && !headphoneCorrectionManaged;
  ...
  const analyzerStatusState = !analyzerRuntimeEnabled
    ? 'off'
  ...
  const curveHasGain = state.bands.some((band) => isEqFilterGainEditable(band.filterType) && Math.abs(band.gainDb) >= 0.05);
  const curveFirstLayout = eqOnlySurface && showAdvancedTools;
  const curveBandEntries = useMemo(
    () => state.bands
      .map((band, index) => ({ band, index }))
      .filter(({ band, index }) => !curveFirstLayout || index === selectedBandIndex || !isBandSlotAvailable(band, index))
      .sort((left, right) => (curveFirstLayout ? left.band.frequencyHz - right.band.frequencyHz : 0)),
    [curveFirstLayout, selectedBandIndex, state.bands],
  );
  const curveNodeIndexes = useMemo(
    () => (curveFirstLayout ? curveBandEntries.map(({ index }) => index) : undefined),
    [curveBandEntries, curveFirstLayout],
  );
  const curveTokenEntries = useMemo(
    () => (curveFirstLayout && curveNodeIndexes
      ? curveBandEntries.filter(({ index }) => curveNodeIndexes.includes(index))
      : curveBandEntries),
    [curveBandEntries, curveFirstLayout, curveNodeIndexes],
  );
  const selectedCurveDisplayNumber = Math.max(
    1,
    curveTokenEntries.findIndex(({ index }) => index === selectedBandIndex) + 1,
  );
  useEffect(() => {
    if (!curveFirstLayout) {
      return;
    ...
    curveBandListRef.current
      ?.querySelector<HTMLElement>(`[data-band-index="${selectedBandIndex}"]`)
      ?.scrollIntoView?.({ behavior: 'smooth', block: 'nearest', inline: 'nearest' });
  }, [curveFirstLayout, selectedBandIndex]);
  useEffect(() => {
    if (curveFirstLayout && !curveFirstLayoutActive.current) {
      setFrequencyUnlocked(true);
    }
    curveFirstLayoutActive.current = curveFirstLayout;
  }, [curveFirstLayout]);
  useEffect(() => {
    if (!curveFirstLayout || proToolPanel === 'precision') {
      return;
    }

    const frame = window.requestAnimationFrame(() => {
      (proToolPanel === 'profile' ? profilePanelRef.current : presetConsoleRef.current)
        ?.scrollIntoView?.({ behavior: 'smooth', block: 'nearest' });
    });
    return () => window.cancelAnimationFrame(frame);
  }, [curveFirstLayout, proToolPanel]);
  ...
  const translatedSimpleToneNames = simpleToneNames.map((option) => ({ id: option.id, name: t(option.labelKey) }));
  const activeSimpleTone = state.presetId === 'custom'
    ? translatedSimpleToneNames.find((option) => option.name === state.presetName)?.id ?? null
    : null;
  const displayPresetName = state.presetId === 'custom'
    ? activeSimpleTone
      ? state.presetName
      : t('settings.eq.preset.modified')
    : resolveEqPresetLabel({ id: state.presetId, name: state.presetName }, t);
  ...
  const refresh = useCallback(async (): Promise<void> => {
    const mutationRevisionAtStart = eqMutationRevision.current;
    try {
      ...
      if (!eq) {
        setPresets([]);
        setProfiles([]);
        setError(analyzerTranslationRef.current('settings.eq.error.bridgeControlEq'));
        return;
      }
      ...
      if (eqMutationRevision.current === mutationRevisionAtStart) {
        setState(nextState);
      }
  ...
  useEffect(() => {
    ...
    if (!eqOnlySurface || !bridge?.setAnalyzerActive) {
      return;
    }

    let disposed = false;
    void bridge.setAnalyzerActive(analyzerRuntimeEnabled)
      .then(() => {
        if (!disposed) analyzerRefreshRef.current?.();
      })
      .catch(() => {
        if (!disposed) setError(analyzerTranslationRef.current('settings.eq.error.bridgeControlEq'));
      });

    return () => {
      disposed = true;
      if (analyzerRuntimeEnabled) void bridge.setAnalyzerActive(false).catch(() => undefined);
    };
  }, [analyzerRuntimeEnabled, eqOnlySurface]);

  useEffect(() => () => {
    Object.values(debounceTimers.current).forEach((timer) => window.clearTimeout(timer));
    Object.values(frequencyDebounceTimers.current).forEach((timer) => window.clearTimeout(timer));
    debounceTimers.current = {};
    frequencyDebounceTimers.current = {};
  }, []);
  ...
  const commitState = useCallback(
    (nextState: EqState): void => {
      setState(nextState);
      analyzerRefreshRef.current?.();
    },
    [],
  );

  const commitBandFields = useCallback(
    (band: number, nextState: EqState, fields: Array<'gainDb' | 'frequencyHz' | 'q' | 'filterType' | 'enabled'>): void => {
      const acknowledgedBand = nextState.bands[band];
      if (!acknowledgedBand) {
        return;
      }
      ...
      setState((current) => ({
        ...current,
        clippingRisk: nextState.clippingRisk,
        bands: current.bands.map((item, index) => (
          index === band
            ? fields.reduce<EqBand>((updated, field) => ({ ...updated, [field]: acknowledgedBand[field] }), item)
            : item
        )),
      }));
      analyzerRefreshRef.current?.();
  ...
  const commitPreampField = useCallback(
    (nextState: EqState): void => {
      setState((current) => ({
        ...current,
        preampDb: nextState.preampDb,
        clippingRisk: nextState.clippingRisk,
      }));
      analyzerRefreshRef.current?.();
    },
  ...
  useEffect(() => {
    const currentClipAt = audioLevels?.lastClipAt ?? null;
    const currentProtectionActive = Boolean(
      state.clippingRisk ||
      roomCorrection.clippingRisk ||
      channelBalance.clippingRisk ||
      audioStatus?.dspClippingRisk ||
      audioStatus?.dspLimiterProtecting,
    );

    if (!autoGainEnabled) {
      autoGainBaselinePreampDb.current = null;
      autoGainLastAdjustmentAtMs.current = 0;
      autoGainLastClipAt.current = currentClipAt;
      autoGainProtectionActive.current = currentProtectionActive;
      autoGainEventsInitialized.current = true;
      setAutoGainStatus('idle');
      setAutoGainAdjustmentDb(0);
      return;
    }

    const protectionEvent = autoGainEventsInitialized.current && (
      (currentClipAt !== null && currentClipAt !== autoGainLastClipAt.current) ||
      (currentProtectionActive && !autoGainProtectionActive.current)
    );
    autoGainLastClipAt.current = currentClipAt;
    autoGainProtectionActive.current = currentProtectionActive;
    autoGainEventsInitialized.current = true;
    ...
    const result = computeAutoGainPreamp({
      eqState: { preampDb: state.preampDb, bands: state.bands },
      audioLevels,
  ...
  const setEnabled = (enabled: boolean): void => {
    ...
    eqMutationRevision.current += 1;
    setState((current) => ({ ...current, enabled }));

    if (!eq) {
      curveEditEnablePending.current = false;
      setState((current) => ({ ...current, enabled: previousEnabled }));
      setError(t('settings.eq.error.bridgeControlEq'));
      return;
    }

    void eq.setEnabled(enabled).then((nextState) => {
      setState((current) => ({ ...current, enabled: nextState.enabled, clippingRisk: nextState.clippingRisk }));
      setError(null);
      analyzerRefreshRef.current?.();
  ...
  const enableEqForCurveEdit = (): void => {
    if (!eqOnlySurface || state.enabled || curveEditEnablePending.current) {
      return;
    }

    curveEditEnablePending.current = true;
    setEnabled(true);
  };
  ...
  const toggleAutoGain = (): void => {
    ...
    autoGainLastClipAt.current = audioLevels?.lastClipAt ?? null;
    autoGainProtectionActive.current = Boolean(
      state.clippingRisk ||
      roomCorrection.clippingRisk ||
      channelBalance.clippingRisk ||
      audioStatus?.dspClippingRisk ||
  ...
  const sendBandGain = useCallback(
    (band: number, gainDb: number, revision: number): void => {
      const eq = getEqBridge();
      ...
      void eq.setBandGain({ band, gainDb })
        .then((nextState) => {
          if (gainMutationRevisions.current[band] === revision) {
            commitBandFields(band, nextState, ['gainDb']);
          }
        })
  ...
  const handleBandChange = (band: number, gainDb: number): void => {
    ...
    enableEqForCurveEdit();

    const revision = (gainMutationRevisions.current[band] ?? 0) + 1;
    gainMutationRevisions.current[band] = revision;
    eqMutationRevision.current += 1;
    beginEqEdit();
    ...
    debounceTimers.current[band] = window.setTimeout(() => sendBandGain(band, gainDb, revision), eqControlDebounceMs);
  };
  ...
  const handleBandCommit = (band: number, gainDb: number): void => {
    ...
    sendBandGain(band, gainDb, revision);
  };

  const sendBandFrequency = useCallback(
    (band: number, frequencyHz: number, revision: number): void => {
      const eq = getEqBridge();
      ...
      void eq.setBandFrequency({ band, frequencyHz })
        .then((nextState) => {
          if (frequencyMutationRevisions.current[band] === revision) {
            commitBandFields(band, nextState, ['frequencyHz']);
          }
        })
        .catch((bandError: unknown) => {
          setError(bandError instanceof Error ? bandError.message : String(bandError));
  ...
  const handleBandFrequencyChange = (band: number, frequencyHz: number): void => {
    ...
    const revision = (frequencyMutationRevisions.current[band] ?? 0) + 1;
    frequencyMutationRevisions.current[band] = revision;
    eqMutationRevision.current += 1;
    beginEqEdit();
    ...
    frequencyDebounceTimers.current[band] = window.setTimeout(() => sendBandFrequency(band, safeFrequencyHz, revision), eqControlDebounceMs);
  };
  ...
  const handleBandFrequencyCommit = (band: number, frequencyHz: number): void => {
    ...
    sendBandFrequency(band, safeFrequencyHz, revision);
  };
  ...
  const sendBandQ = useCallback(
    (band: number, q: number): void => {
      ...
      void eq.setBandQ({ band, q }).then((nextState) => commitBandFields(band, nextState, ['q'])).catch((bandError: unknown) => {
        setError(bandError instanceof Error ? bandError.message : String(bandError));
      });
    },
    [commitBandFields, t],
  ...
  const handleBandQChange = (band: number, q: number): void => {
    if (!requireCustomHeadphoneCorrection()) {
      return;
    }
    ...
    setState((current) => ({
      ...
      presetId: 'custom',
      presetName: 'Custom',
      bands: current.bands.map((item, index) => (index === band ? { ...item, q } : item)),
    }));
  };
  ...
  const toggleBandEnabled = (band: number, enabled: boolean): void => {
    ...
    void eq.setBandEnabled({ band, enabled }).then((nextState) => commitBandFields(band, nextState, ['enabled'])).catch((bandError: unknown) => {
      setError(bandError instanceof Error ? bandError.message : String(bandError));
    });
  };
  ...
  const addFilterSlot = (): void => {
    ...
    commitBandSlot(targetBand, {
      ...defaultBandSlot(targetBand),
      q: eqQPresetValues.peaking.normal,
    });
  };

  const addFilterAt = (frequencyHz: number, gainDb: number): void => {
    if (!requireCustomHeadphoneCorrection()) {
      return;
    }
    ...
    commitBandSlot(targetBand, {
      ...defaultBandSlot(targetBand),
      frequencyHz: resolveBandFrequency(frequencyHz, frequencyEditUnlocked),
      gainDb,
      q: eqQPresetValues.peaking.normal,
    });
  };

  const deleteFilterAt = (band: number): void => {
    if (!requireCustomHeadphoneCorrection()) {
      return;
    }
    ...
    commitBandSlot(band, bypassedBandSlot(band));
  };

  const deleteSelectedFilter = (): void => {
    deleteFilterAt(selectedBandIndex);
  };

  const curveCallbacksRef = useRef({
    ...
    onBandQCommit: handleBandQCommit,
    onBandDelete: deleteFilterAt,
    onAddBandAt: addFilterAt,
  });
  curveCallbacksRef.current = {
    onBandChange: handleBandChange,
    onBandCommit: handleBandCommit,
    onBandFrequencyChange: handleBandFrequencyChange,
    onBandFrequencyCommit: handleBandFrequencyCommit,
  ...
  const onCurveBandChange = useCallback((index: number, gainDb: number): void => {
    if (headphoneCorrectionManaged) {
      setError(t('settings.eq.headphoneCorrection.lockedError'));
      return;
    }
    curveCallbacksRef.current.onBandChange(index, gainDb);
  }, [headphoneCorrectionManaged, t]);

  const onCurveBandCommit = useCallback((index: number, gainDb: number): void => {
    if (headphoneCorrectionManaged) {
      setError(t('settings.eq.headphoneCorrection.lockedError'));
      return;
    }
    curveCallbacksRef.current.onBandCommit(index, gainDb);
  }, [headphoneCorrectionManaged, t]);

  const onCurveBandFrequencyChange = useCallback((index: number, frequencyHz: number): void => {
    if (headphoneCorrectionManaged) {
      setError(t('settings.eq.headphoneCorrection.lockedError'));
      return;
    }
    curveCallbacksRef.current.onBandFrequencyChange(index, frequencyHz);
  }, [headphoneCorrectionManaged, t]);

  const onCurveBandFrequencyCommit = useCallback((index: number, frequencyHz: number): void => {
    if (headphoneCorrectionManaged) {
      setError(t('settings.eq.headphoneCorrection.lockedError'));
      return;
    }
    curveCallbacksRef.current.onBandFrequencyCommit(index, frequencyHz);
  }, [headphoneCorrectionManaged, t]);

  const onCurveBandQCommit = useCallback((index: number, q: number): void => {
    if (headphoneCorrectionManaged) {
      setError(t('settings.eq.headphoneCorrection.lockedError'));
      return;
    }
    curveCallbacksRef.current.onBandQCommit(index, q);
  }, [headphoneCorrectionManaged, t]);

  const onCurveBandDelete = useCallback((index: number): void => {
    if (headphoneCorrectionManaged) {
      setError(t('settings.eq.headphoneCorrection.lockedError'));
      return;
    }
    curveCallbacksRef.current.onBandDelete(index);
  }, [headphoneCorrectionManaged, t]);

  const onCurveAddBandAt = useCallback((frequencyHz: number, gainDb: number): void => {
    curveCallbacksRef.current.onAddBandAt(frequencyHz, gainDb);
  }, []);
  ...
  const handlePreampChange = (preampDb: number): void => {
    ...
    void eq.setPreamp(preampDb).then(commitPreampField).catch((preampError: unknown) => {
  ...
  const saveUserPreset = async (): Promise<void> => {
    if (!saveName.trim()) {
      setError(t('settings.eq.error.presetName'));
      return;
    }
    ...
    try {
      ...
      setPresets(await eq.listPresets());
      commitState(await eq.setPreset(saved.id));
      setSaveName('');
      setExportNotice(null);
      setError(null);
    } catch (saveError) {
  ...
  return (
    <section className="eq-panel" aria-label="ECHO EQ panel" data-enabled={state.enabled} data-mode={eqUiMode} data-pro-tool={curveFirstLayout ? proToolPanel : undefined} data-surface={surface}>
      <header className="eq-header">
        <div className="eq-title-block">
          <span className="eq-title-icon">
            {curveFirstLayout ? <SlidersHorizontal size={18} /> : <AudioWaveform size={18} />}
          </span>
          <div className="eq-title-copy">
            <h2>{eqOnlySurface ? t('settings.eq.title.parametric') : t('settings.eq.title')}</h2>
            <p>{eqOnlySurface ? t('settings.eq.subtitle.parametric') : t('settings.eq.subtitle')}</p>
          </div>
          {eqOnlySurface ? (
            <label className="eq-enable-pill eq-enable-pill--title">
        ...
        <div className="eq-header-actions">
          ...
          {eqOnlySurface ? (
            <div className="eq-header-preset">
          ...
          {eqOnlySurface ? (
            <div className="eq-header-analyzer">
              <button
                className="eq-soft-button eq-analyzer-toggle"
                aria-pressed={spectrumAnalyzerEnabled}
              ...
              {spectrumAnalyzerEnabled ? <div className="eq-analyzer-mode" role="group" aria-label={t('settings.eq.analyzer.mode')}>
                <button className="eq-soft-button" aria-pressed={analyzerMode === 'input'} data-active={analyzerMode === 'input'} type="button" onClick={() => setAnalyzerMode('input')}>
                  {t('settings.eq.analyzer.input')}
                </button>
                <button className="eq-soft-button" aria-pressed={analyzerMode === 'postEq'} data-active={analyzerMode === 'postEq'} type="button" onClick={() => setAnalyzerMode('postEq')}>
                  {t('settings.eq.analyzer.postEq')}
                </button>
              </div> : null}
            </div>
          ) : null}
          {eqOnlySurface || (showAdvancedTools && !curveFirstLayout) ? (
            <>
      ...
      {curveFirstLayout ? (
        <section className="eq-pro-command-bar" aria-label={t('settings.eq.mode.pro')}>
          <div className="eq-pro-live-summary" aria-live="polite">
            <span data-state={state.enabled ? 'live' : 'off'}>
              <SlidersHorizontal size={12} aria-hidden="true" />
              {state.enabled ? t('settings.eq.state.eqEnabled') : t('settings.eq.state.eqDisabled')}
            </span>
            <span>{`${configuredBandCount} ${t('settings.eq.band.filters')}`}</span>
            <span data-risk={surfaceClippingRisk}>
              <ShieldCheck size={12} aria-hidden="true" />
              {surfaceClippingRisk ? t('settings.eq.status.warning') : t('settings.eq.status.safe')}
            </span>
          </div>
          <div className="eq-pro-tool-tabs" role="group" aria-label={t('settings.eq.mode.pro')}>
            <button className="eq-soft-button eq-pro-add-filter" type="button" disabled={headphoneCorrectionManaged} onClick={addFilterSlot}>
              <Plus size={13} aria-hidden="true" />
              {t('settings.eq.action.addFilter')}
            </button>
            <button
              className="eq-soft-button"
              data-testid="eq-reset-curve"
              disabled={headphoneCorrectionManaged || !curveHasGain}
              type="button"
              onClick={resetAllGains}
            >
              <RotateCcw size={13} aria-hidden="true" />
              {t('settings.eq.action.resetCurve')}
            </button>
          </div>
          <div className="eq-pro-tool-tabs" role="group" aria-label={t('settings.eq.proEditor.advanced')}>
            <button className="eq-soft-button" data-pro-tool-target="profile" aria-pressed={proToolPanel === 'profile'} data-active={proToolPanel === 'profile'} type="button" onClick={() => setProToolPanel((current) => current === 'profile' ? 'precision' : 'profile')}>
              <Headphones size={13} aria-hidden="true" />
              {t('settings.eq.profile.title')}
            </button>
            <button className="eq-soft-button" data-pro-tool-target="preset" aria-pressed={proToolPanel === 'preset'} data-active={proToolPanel === 'preset'} type="button" onClick={() => setProToolPanel((current) => current === 'preset' ? 'precision' : 'preset')}>
              <Save size={13} aria-hidden="true" />
              {t('settings.eq.proEditor.presetTools')}
            </button>
          </div>
        </section>
      ) : null}
        ...
        <section className="eq-headphone-lock" aria-label={t('settings.eq.headphoneCorrection.lockAria')}>
          ...
          {eqOnlySurface ? (
            <button
              ...
              onClick={toggleHeadphoneCorrectionCompare}
            >
              {state.enabled ? t('settings.eq.headphoneCorrection.compareOriginal') : t('settings.eq.headphoneCorrection.compareCorrected')}
            </button>
          ) : null}
      ...
      <div className="eq-workbench" data-managed={headphoneCorrectionManaged}>
        <div className="eq-curve-column">
          <div className="eq-preamp-bar" data-auto-gain={autoGainActive}>
            <div className="eq-preamp-value">
              <span>{curveFirstLayout ? t('settings.eq.preamp.aria') : t('settings.eq.preamp.inputSafety')}</span>
              {curveFirstLayout ? (
                <label className="eq-preamp-compact-field">
                  <input
            ...
            {!curveFirstLayout ? (
              <input
                aria-label={simpleWorkbench ? 'Quick EQ preamp' : t('settings.eq.preamp.aria')}
                type="range"
                min={eqMinPreampDb}
                max={eqMaxPreampDb}
                step="0.1"
            ...
            {simpleWorkbench ? (
              <div className="eq-curve-footer-actions">
                {selectedBand ? (
                  <span className="eq-simple-band-readout">
                    <button
                      aria-label={`${t('settings.eq.band.gain')} -`}
                      className="eq-simple-stepper"
                      disabled={headphoneCorrectionManaged || !selectedBandGainEditable}
                      type="button"
                      onClick={() => adjustSelectedGain(-0.5)}
                    >
                      −
                    </button>
                    <strong>{formatFrequencyLabel(selectedBand.frequencyHz)}</strong>
                    <em>{selectedBandGainEditable ? formatDb(selectedBand.gainDb) : t('settings.eq.band.gainFixed')}</em>
                    <button
                      aria-label={`${t('settings.eq.band.gain')} +`}
                      className="eq-simple-stepper"
                      data-testid="eq-simple-gain-up"
                      disabled={headphoneCorrectionManaged || !selectedBandGainEditable}
                      type="button"
                      onClick={() => adjustSelectedGain(0.5)}
                    >
                ...
                <span className="eq-compact-safety" data-risk={surfaceClippingRisk}>
                  <ShieldCheck size={14} aria-hidden="true" />
                  {surfaceClippingRisk ? t('settings.eq.warning.lowerPreamp') : t('settings.eq.status.safeHeadroomShort')}
                </span>
                <button
                  className="eq-soft-button"
                  data-testid="eq-simple-reset-selected"
                  disabled={headphoneCorrectionManaged || !selectedBandGainEditable || Math.abs(selectedBand?.gainDb ?? 0) < 0.05}
                  type="button"
                  onClick={() => handleBandCommit(selectedBandIndex, 0)}
                >
                ...
                <button
                  ...
                  onBlur={releaseBypass}
                >
                  {t('settings.eq.action.holdOriginal')}
                </button>
              </div>
            ) : null}
            {curveFirstLayout ? (
              <div className="eq-curve-footer-actions">
                ...
                <details className="eq-compact-ab">
                  <summary>A/B</summary>
                  <div className="eq-compact-ab-popover">
                    <label>
                    ...
                    {(['a', 'b'] as const).map((slot) => (
                      <div className="eq-compact-ab-row" key={slot}>
                        <strong>{slot.toUpperCase()}</strong>
                        <span>{abSlots[slot]?.presetName ?? t('settings.eq.ab.emptySlot')}</span>
                        <button className="eq-soft-button" type="button" onClick={() => storeAbSlot(slot)}>
                          {slot === 'a' ? t('settings.eq.action.storeA') : t('settings.eq.action.storeB')}
                        </button>
                        <button className="eq-soft-button" type="button" disabled={!abSlots[slot]} onClick={() => restoreAbSlot(slot)}>
                          {slot === 'a' ? t('settings.eq.action.applyA') : t('settings.eq.action.applyB')}
                        </button>
                      </div>
                    ))}
                  </div>
            ...
            {showAdvancedTools && !curveFirstLayout ? (
          ...
          <EqCurveView
            ...
            visibleBandIndexes={curveNodeIndexes}
            curveFirst={curveFirstLayout}
            graphicWorkbench={simpleWorkbench}
            spectrumEnabled={curveSpectrumEnabled}
            analyzerMode={analyzerMode}
            visualSpectrum={curveSpectrumEnabled ? audioLevels?.visualSpectrum : undefined}
            visualTelemetryState={curveSpectrumEnabled ? audioLevels?.visualTelemetryState : undefined}
            onBandSelect={setSelectedBandIndex}
            onBandChange={onCurveBandChange}
            onBandCommit={onCurveBandCommit}
            onBandFrequencyChange={onCurveBandFrequencyChange}
            onBandFrequencyCommit={onCurveBandFrequencyCommit}
            onBandQCommit={onCurveBandQCommit}
            onBandDelete={onCurveBandDelete}
            onAddBandAt={curveFirstLayout ? onCurveAddBandAt : undefined}
          />
          {showAdvancedTools ? (
          ...
          {showAdvancedTools ? (
            <div className="eq-advanced-grid">
              {curveFirstLayout ? (
                <EqProBandEditor
                  band={selectedBand}
                  displayNumber={selectedCurveDisplayNumber}
                  color={getEqCurveBandColor(Math.max(0, curveTokenEntries.findIndex(({ index }) => index === selectedBandIndex)))}
                  disabled={headphoneCorrectionManaged}
                  frequencyUnlocked={frequencyEditUnlocked}
                  gainEditable={selectedBandGainEditable}
                  qPresets={selectedBandQPresets}
                  onFilterTypeChange={(filterType) => handleBandFilterTypeChange(selectedBandIndex, filterType)}
                  onFrequencyChange={(frequencyHz) => handleBandFrequencyChange(selectedBandIndex, frequencyHz)}
                  onFrequencyCommit={(frequencyHz) => handleBandFrequencyCommit(selectedBandIndex, frequencyHz)}
                  onGainChange={(gainDb) => handleBandChange(selectedBandIndex, gainDb)}
                  onGainCommit={(gainDb) => handleBandCommit(selectedBandIndex, gainDb)}
                  onQChange={(q) => handleBandQChange(selectedBandIndex, q)}
                  onQCommit={(q) => handleBandQCommit(selectedBandIndex, q)}
                  onQPresetCommit={(q) => handleBandQPresetCommit(selectedBandIndex, q)}
                  onEnabledChange={(enabled) => toggleBandEnabled(selectedBandIndex, enabled)}
                  onFrequencySnapChange={(snapped) => setFrequencyUnlocked(!snapped)}
                  onReset={resetSelectedBand}
              ...
              <section className="eq-advanced-section eq-inspector" aria-label={t('settings.eq.band.inspector')}>
                ...
                <div className="eq-inspector-fields">
                  ...
                  <label>
                    ...
                    <input
                      ...
                      disabled={headphoneCorrectionManaged}
                      onChange={(event) => {
                        handleBandQChange(selectedBandIndex, Number(event.currentTarget.value));
                      }}
                ...
                <div className="eq-inspector-actions">
                  ...
                  <label className="eq-inspector-toggle">
                    <input
                      type="checkbox"
                      checked={curveFirstLayout ? !frequencyUnlocked : frequencyUnlocked}
                      disabled={headphoneCorrectionManaged}
                      onChange={(event) => setFrequencyUnlocked(curveFirstLayout ? !event.currentTarget.checked : event.currentTarget.checked)}
                    />
                    <span>{curveFirstLayout ? t('settings.eq.band.frequencySnapped') : t('settings.eq.action.unlockFrequency')}</span>
                  </label>
                  ...
                  {curveFirstLayout ? (
                    <button
                      className="eq-icon-action eq-inspector-delete"
                      type="button"
                      aria-label={t('settings.eq.action.deleteFilter')}
                      title={t('settings.eq.action.deleteFilter')}
                      disabled={headphoneCorrectionManaged || !selectedBand}
              ...
              <section
                className="eq-advanced-section eq-profile-panel"
                aria-label={t('settings.eq.profile.title')}
                data-pro-visible={curveFirstLayout && proToolPanel === 'profile'}
                ref={profilePanelRef}
              >
          ...
          <div className="eq-band-strip" aria-label={t('settings.eq.band.readoutsAria')}>
            ...
            <div className="eq-band-list" ref={curveBandListRef}>
              {curveTokenEntries.map(({ band, index }, displayIndex) => (
                <button
                ...
                >
                  <span className="eq-band-chip-index">{String(displayIndex + 1).padStart(2, '0')}</span>
              ...
              {curveFirstLayout ? (
                <button className="eq-band-chip eq-band-chip--add" type="button" disabled={headphoneCorrectionManaged} onClick={addFilterSlot}>
        ...
        <div className="eq-compare-panel">
          ...
          <div className="eq-compare-actions">
            ...
            <button className="eq-soft-button" type="button" onClick={() => setEnabled(!state.enabled)}>
              {curveFirstLayout ? 'A/B' : state.enabled ? t('settings.eq.action.toggleBypassOn') : t('settings.eq.action.toggleBypassOff')}
            </button>
            {curveFirstLayout ? (
              <div className="eq-history-actions">
                <button className="eq-icon-action" type="button" aria-label={t('settings.eq.action.undo')} title={t('settings.eq.action.undo')} disabled={undoStack.length === 0 || headphoneCorrectionManaged} onClick={undoEq}>
      ...
      <footer
        className="eq-preset-tools eq-preset-console"
        data-pro-visible={curveFirstLayout && proToolPanel === 'preset'}
        ref={presetConsoleRef}
      >
        <div className="eq-preset-console-main">
          ...
          <div className="eq-preset-actions">
            <button type="button" onClick={() => void saveUserPreset()}>
              <Save size={15} />
              {t('settings.eq.action.saveAs')}
            </button>
            <button type="button" onClick={() => void exportPreset()}>
              {t('settings.eq.action.exportPreset')}
            </button>
