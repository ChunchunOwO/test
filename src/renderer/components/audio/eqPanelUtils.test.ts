...
describe('eqPanelUtils', () => {
  ...
  it('reuses sampled response values for post-EQ analyzer bars', () => {
    const bands = [
      { frequencyHz: 1000, gainDb: 6, q: 1, filterType: 'peaking' as const, enabled: true },
    ];
    const direct = computeEqSpectrumBars([0.4, 0.5, 0.6], bands, 'postEq');
    const sampled = [
      { x: 0, gainDb: computeEqResponseGainDbAtFrequency(bands, 20) },
      { x: 0.5, gainDb: computeEqResponseGainDbAtFrequency(bands, 632.4555) },
      { x: 1, gainDb: computeEqResponseGainDbAtFrequency(bands, 20000) },
    ];
    const interpolated = computeEqSpectrumBars(
      [0.4, 0.5, 0.6],
      bands,
      'postEq',
      (normalizedX) => interpolateEqResponseGainDb(sampled, normalizedX),
    );

    expect(interpolated).toHaveLength(3);
    interpolated.forEach((bar, index) => {
      expect(bar.value).toBeCloseTo(direct[index].value, 1);
    });
  });
  ...
  it('solves graphic EQ band gain against the combined response while dragging', () => {
    const bands = [
      { frequencyHz: 400, gainDb: -6, q: 2, filterType: 'peaking' as const, enabled: true },
      { frequencyHz: 500, gainDb: -4, q: 2, filterType: 'peaking' as const, enabled: true },
    ];
    const targetResponseGainDb = -3;
    const gainDb = resolveGraphicEqBandGainDb(bands, 1, 500, targetResponseGainDb);
    const nextBands = bands.map((band, index) => index === 1 ? { ...band, gainDb } : band);

    expect(computeEqResponseGainDbAtFrequency(nextBands, 500)).toBeCloseTo(targetResponseGainDb, 1);
  });
  ...
  it('marks Auto Gain clipping risk and avoids NaN with extreme telemetry', () => {
    const result = computeAutoGainPreamp({
      eqState: { ...eqState([0]), preampDb: -11.8 },
      audioLevels: {
  ...
  it('does not treat a historical cumulative clip count as a new Auto Gain event', () => {
    const result = computeAutoGainPreamp({
      eqState: { ...eqState([0]), preampDb: -5 },
      audioLevels: {
        inputPeakDb: -18,
        inputRmsDb: -30,
    ...
    expect(result).toMatchObject({ status: 'idle', targetPreampDb: null, adjustmentDb: 0 });
  });

  it('holds repeated protection telemetry during the Auto Gain reduction cooldown', () => {
    const result = computeAutoGainPreamp({
      eqState: { ...eqState([0]), preampDb: -6 },
      audioLevels: {
        inputPeakDb: -1,
        inputRmsDb: -16,
        estimatedOutputPeakDb: 0,
      ...
      baselinePreampDb: -5,
      nowMs: 5200,
      lastAdjustmentAtMs: 5000,
      protectionEvent: true,
    });

    expect(result).toMatchObject({ status: 'holding', targetPreampDb: null, adjustmentDb: -1 });
  });
