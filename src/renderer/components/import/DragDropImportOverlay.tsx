...
const fileDropOwner = (event: DragEvent): string | null =>
  event.target instanceof Element
    ? event.target.closest<HTMLElement>('[data-file-drop-owner]')?.dataset.fileDropOwner ?? null
    : null;
...
const summarizeDroppedFilesImport = (
  ...
  if (parts.length === 0) {
    return t('import.dragDrop.files.empty');
  }

  const summary = parts.join(t('punctuation.clauseSeparator'));
  return result.storedFileCount > 0
    ? t('import.dragDrop.files.summaryWithOutput', { summary, outputDirectory: result.outputDirectory })
    : summary;
};
  ...
  useEffect(() => {
    ...
    const handleDrop = (event: DragEvent): void => {
      ...
      void library.importDroppedFiles(files)
        .then((result) => {
          result.folderScanStatuses.forEach(rememberLibraryScanStatus);
          onNotice(summarizeDroppedFilesImport(result, t));
          if (result.addedFolderCount > 0 || result.importedCount > 0) {
