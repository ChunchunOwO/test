...
import { albumDetailNavigationEvent, consumePendingAlbumDetailNavigation, dispatchDetailReturnNavigation, openAlbumDetail, openAlbumDetailForTrack, readActiveDetailReturnTarget, restoreHomeScrollEvent } from './albumNavigation';
...
describe('album navigation', () => {
  it('opens a current remote album without blocking navigation on readability probes', async () => {
    const remoteAlbum = album('remote-album:736f75726365', {
      mediaType: 'remote',
      sourceId: 'source-1',
      provider: 'subsonic',
    });
    ...
    window.echo = { library: { getAlbum, getAlbumTracks } } as unknown as Window['echo'];

    try {
      await openAlbumDetail(remoteAlbum, { returnTo: 'songs' });
      ...
      expect((navigateAlbum.mock.calls[0]?.[0] as CustomEvent).detail).toEqual(
        expect.objectContaining({ album: remoteAlbum, returnTo: 'songs' }),
      );
    } finally {
  ...
  it('reads the visible page-surface route as a detail return target', () => {
    document.body.innerHTML = `
      <main class="page-surface" data-route-id="songs" hidden></main>
      <main class="page-surface" data-route-id="queue"></main>
    `;

    expect(readActiveDetailReturnTarget()).toBe('queue');
    document.body.innerHTML = '';
  });

  it('dispatches the matching return route and restores home scroll', async () => {
    ...
    window.addEventListener(restoreHomeScrollEvent, restoreHomeScroll);

    try {
      expect(dispatchDetailReturnNavigation(null)).toBe(false);
      expect(dispatchDetailReturnNavigation('queue')).toBe(true);
      expect(dispatchDetailReturnNavigation('songs')).toBe(true);
      expect(dispatchDetailReturnNavigation('home')).toBe(true);
