...
export const getSteamBridge = (): Window['echo']['steam'] | null => getEchoBridge()?.steam ?? null;
...
const browserEqStorageKey = 'echo.browser-eq';
...
class BrowserEqBridge implements EqBridgeApi {
  ...
  async setAnalyzerActive(enabled: boolean): Promise<boolean> {
    return enabled;
  }
  ...
  onCompressorTelemetry(_handler: (telemetry: CompressorTelemetry) => void): () => void {
    return () => undefined;
  }
  ...
  async exportPreset(request: EqSavePresetRequest): Promise<string | null> {
    ...
    const fileName = `${sanitizePresetId(normalized.name) || 'echo-eq-preset'}.json`;
  ...
  async exportApoPreset(request: EqSavePresetRequest): Promise<string | null> {
    ...
    const fileName = `${sanitizePresetId(normalized.name) || 'echo-eq-preset'}.txt`;
  ...
  async exportApoGraphicEqPreset(request: EqSavePresetRequest): Promise<string | null> {
    ...
    const fileName = `${sanitizePresetId(normalized.name) || 'echo-eq-preset'}-graphic-eq.txt`;
  ...
  private readStorage(): BrowserEqStorage {
    ...
    try {
      ...
      return {
        state: refreshBuiltInEqState(normalizeState(parsed.state)),
...
const browserStreamingBridge: StreamingBridgeApi = {
  ...
  renameFavoriteCollection: async (): Promise<StreamingFavoriteCollectionRenameResult> => {
    throw new Error('Desktop bridge unavailable. Open ECHO in Electron to rename streaming favorite lists.');
  },
  syncFavoriteCollection: async (): Promise<StreamingFavoritesImportResult> => {
    throw new Error('Desktop bridge unavailable. Open ECHO in Electron to sync streaming favorite lists.');
  },
  deleteFavoriteCollection: async (): Promise<StreamingFavoriteCollectionDeleteResult> => {
    throw new Error('Desktop bridge unavailable. Open ECHO in Electron to delete streaming favorite lists.');
  },
