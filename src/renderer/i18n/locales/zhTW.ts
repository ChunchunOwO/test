import type { TranslationDictionary, TranslationKey } from '../locales';
import { communityWorkshopZhTW } from './communityWorkshopFamily.zhTW';
import { connectPageCoverageZhTW } from './connectPageCoverage';
import { likedPageZhTW, mediaLibraryZhTW, settingsAboutDangerZhTW, settingsIntegrationsExtraZhTW, songsPageZhTW } from './zhFamilyTW';

const audioSignalPathFlightRecorderZhTW = {
  'audioSignalPath.flight.collapse': '收起輸出記錄',
  'audioSignalPath.flight.count': '目前曲目 {current} 筆 / 共 {total} 筆',
  'audioSignalPath.flight.currentTrack': '目前曲目',
  'audioSignalPath.flight.detail.failure': '{device} / {route} / {reason}',
  'audioSignalPath.flight.detail.fallback': '{from} -> {to} / {reason}',
  'audioSignalPath.flight.detail.rateConversion': '{path} / {device}',
  'audioSignalPath.flight.detail.routeChange': '{from} -> {to} / {device}',
  'audioSignalPath.flight.detail.takeover': '{device} / {route} / {rate}',
  'audioSignalPath.flight.empty': '暫無輸出記錄',
  'audioSignalPath.flight.emptyDetail': '播放或切換輸出後，ECHO 會在這裡留下最近 20 筆唯讀記錄。',
  'audioSignalPath.flight.expand': '展開輸出記錄',
  'audioSignalPath.flight.eyebrow': '最近輸出',
  'audioSignalPath.flight.kind.failure': '開啟失敗',
  'audioSignalPath.flight.kind.fallback': '回到共享輸出',
  'audioSignalPath.flight.kind.rateConversion': '取樣率轉換',
  'audioSignalPath.flight.kind.routeChange': '輸出切換',
  'audioSignalPath.flight.kind.takeover': 'DAC 接管成功',
  'audioSignalPath.flight.ratePending': '取樣率待定',
  'audioSignalPath.flight.reasonUnknown': '原因未知',
  'audioSignalPath.flight.recentRoute': '最近輸出',
  'audioSignalPath.flight.routeUnknown': '上一次輸出',
  'audioSignalPath.flight.title': '輸出記錄',
} satisfies Partial<Record<TranslationKey, string>>;

const audioSignalPathExtrasZhTW = {
  ...audioSignalPathFlightRecorderZhTW,
  'audioSignalPath.atlas.eyebrow': '裝置圖譜',
  'audioSignalPath.atlas.collapse': '收合 DAC 能力圖譜',
  'audioSignalPath.atlas.expand': '展開 DAC 能力圖譜',
  'audioSignalPath.atlas.lastIssue': '最近問題',
  'audioSignalPath.atlas.lastIssueDetail': '最近發生在 {mode}',
  'audioSignalPath.atlas.lastIssueNone': '暫無錯誤',
  'audioSignalPath.atlas.lastIssueNoneDetail': 'ECHO 暫未記錄到這台裝置的輸出錯誤',
  'audioSignalPath.atlas.modeAria': '輸出模式穩定度',
  'audioSignalPath.atlas.modeIssues': '{count} 次問題',
  'audioSignalPath.atlas.modeMixed': '{success} 次成功 / {issues} 次問題',
  'audioSignalPath.atlas.modeNative': '{success} 次成功 / {native} 次原生',
  'audioSignalPath.atlas.modeStable': '{count} 次成功',
  'audioSignalPath.atlas.modeUnproven': '尚未觀測',
  'audioSignalPath.atlas.nativeDetail': '最近一次原生鏈路來自 {mode}',
  'audioSignalPath.atlas.nativeNone': '待證明',
  'audioSignalPath.atlas.nativeNoneDetail': '還沒有記錄到檔案取樣率等於裝置取樣率的成功鏈路',
  'audioSignalPath.atlas.nativeProof': '原生證明',
  'audioSignalPath.atlas.observedOnly': '來自 ECHO 觀察，不代表完整硬體規格',
  'audioSignalPath.atlas.observedRates': '見過的輸出',
  'audioSignalPath.atlas.pending': '等待觀察',
  'audioSignalPath.atlas.resample441To48': '44.1 -> 48 x{count}',
  'audioSignalPath.atlas.resampleDetail': '共記錄 {count} 次取樣率轉換',
  'audioSignalPath.atlas.resampleNone': '未發現',
  'audioSignalPath.atlas.resampleNoneDetail': '暫未看到固定 44.1 -> 48 傾向',
  'audioSignalPath.atlas.resampleTendency': '重取樣傾向',
  'audioSignalPath.atlas.title': 'DAC 能力圖譜',
  'audioSignalPath.doctor.blocker.rateConversion': '取樣率轉換',
  'audioSignalPath.doctor.bitPerfect.advice': '想盡量原樣輸出時可以保留這條路徑；目前沒有需要立刻處理的問題。',
  'audioSignalPath.doctor.bitPerfect.detail': '{mode}輸出，目前沒有開啟會改動聲音的 DSP。',
  'audioSignalPath.doctor.bitPerfect.title': 'Bit-perfect 候選',
  'audioSignalPath.doctor.collapse': '收合鏈路診斷',
  'audioSignalPath.doctor.echoSrc.advice': '想要升頻風格就保留 SRC；目標是原生取樣率輸出時就關閉 SRC。',
  'audioSignalPath.doctor.echoSrc.detail': '目前 DSP/SRC 模式已啟用升頻。',
  'audioSignalPath.doctor.sdm.advice': '這裡的 DoP 速率是載波時鐘，不是 SDM 的 1-bit 位元流率；判斷 SDM 檔位請看 DSD64/128/256 和 MHz 數值。',
  'audioSignalPath.doctor.sdm.title': 'PCM → SDM 轉換正在運行',
  'audioSignalPath.doctor.echoSrc.title': 'ECHO 升頻（SRC）已開啟',
  'audioSignalPath.doctor.expand': '展開鏈路診斷',
  'audioSignalPath.doctor.eyebrow.bitPerfect': 'Bit-perfect',
  'audioSignalPath.doctor.eyebrow.clock': '時鐘',
  'audioSignalPath.doctor.eyebrow.doctor': '診斷',
  'audioSignalPath.doctor.eyebrow.hqPlayer': 'HQPlayer',
  'audioSignalPath.doctor.eyebrow.level': '電平',
  'audioSignalPath.doctor.eyebrow.src': 'SRC',
  'audioSignalPath.doctor.eyebrow.sdm': 'SDM',
  'audioSignalPath.doctor.eyebrow.status': '狀態',
  'audioSignalPath.doctor.eyebrow.why': '原因',
  'audioSignalPath.doctor.headroom.advice': '啟用 EQ、FIR 或聲道平衡時，把前級增益再往下調一點通常更穩。',
  'audioSignalPath.doctor.headroom.detail': '餘量 {headroom}，峰值 {peak}；啟用 EQ/FIR/前級增益時要留意這裡。',
  'audioSignalPath.doctor.headroom.title': '餘量偏緊',
  'audioSignalPath.doctor.hintCount.many': '{count} 則提示',
  'audioSignalPath.doctor.hintCount.one': '{count} 則提示',
  'audioSignalPath.doctor.hqPlayer.error.advice': '優先檢查 HQPlayer Desktop / 控制埠；ECHO 不會自動改動外部渲染器鏈路。',
  'audioSignalPath.doctor.hqPlayer.error.detail': 'HQPlayer 回報了外部播放問題。',
  'audioSignalPath.doctor.hqPlayer.error.title': '外部鏈路異常',
  'audioSignalPath.doctor.hqPlayer.external.advice': '想改變這條鏈路時，在 HQPlayer 裡調整 profile / filter / shaper。',
  'audioSignalPath.doctor.hqPlayer.external.detail': 'ECHO 只交接傳輸；最終 DSP、取樣率、shaper 與輸出由 HQPlayer 決定。',
  'audioSignalPath.doctor.hqPlayer.external.title': '外部渲染器接管鏈路',
  'audioSignalPath.doctor.hqPlayer.output.advice': '如果速率看起來不對，優先檢查目前 HQPlayer profile，而不是 ECHO 本機輸出模式。',
  'audioSignalPath.doctor.hqPlayer.output.detail': '{output}；這個速率來自 HQPlayer 遙測，不是本機 Windows 輸出路徑。',
  'audioSignalPath.doctor.hqPlayer.output.title': 'HQPlayer 已回報輸出',
  'audioSignalPath.doctor.hqPlayer.waiting.advice': '等 HQPlayer 回報狀態，或直接打開 HQPlayer 確認目前輸出。',
  'audioSignalPath.doctor.hqPlayer.waiting.detail': '播放已經交給 HQPlayer，但它還沒有返回完整處理細節。',
  'audioSignalPath.doctor.hqPlayer.waiting.title': '等待 HQPlayer 詳情',
  'audioSignalPath.doctor.limiter.advice': '追求更大音量前，建議先降低 preamp、EQ 增益、FIR trim 或 ReplayGain 增益。',
  'audioSignalPath.doctor.limiter.detail': '安全限幅器已經在 DSP 之後保護輸出。',
  'audioSignalPath.doctor.limiter.title': '限幅器已介入',
  'audioSignalPath.doctor.notBitPerfect.advice': '想要更嚴格的 bit-perfect 路徑，優先用獨占或 ASIO 輸出，並關閉 DSP/SRC 模組。',
  'audioSignalPath.doctor.notBitPerfect.detail': '原因：{blockers}。',
  'audioSignalPath.doctor.notBitPerfect.fallback': 'ECHO 現在不能把這條鏈路標記為透明 1:1 輸出。',
  'audioSignalPath.doctor.notBitPerfect.title': '不是 bit-perfect',
  'audioSignalPath.doctor.outputError.advice': '先重試播放；如果還不行，切回 WASAPI Shared/相容輸出，或重新插拔目前的 DAC。',
  'audioSignalPath.doctor.outputError.title': '音訊輸出尚未準備好',
  'audioSignalPath.doctor.resampling.atlasAdvice': '裝置圖譜上次看到這台裝置用 {mode} 以 {rate} 原生輸出；想避開重取樣可以優先試這條路。',
  'audioSignalPath.doctor.resampling.advice': '如果想要原生取樣率播放，可以為這台 DAC 試試獨占輸出或 ASIO，而不是共享/系統輸出。',
  'audioSignalPath.doctor.resampling.detail': '{path}；ECHO 正在跟隨目前輸出時鐘/後端，所以不是 1:1 時鐘鏈路。',
  'audioSignalPath.doctor.resampling.title': '正在進行取樣率轉換',
  'audioSignalPath.doctor.sampleRateMismatch.advice': '先確認目標裝置是否支援這個取樣率；獨占或 ASIO 通常能給 ECHO 更乾淨的裝置時鐘。',
  'audioSignalPath.doctor.sampleRateMismatch.detail': '{path}；來源檔案和裝置時鐘沒有對齊。',
  'audioSignalPath.doctor.sampleRateMismatch.title': '取樣率不一致',
  'audioSignalPath.doctor.title': '鏈路診斷',
  'audioSignalPath.doctor.waiting.advice': '先播放一首歌。這裡只說明目前鏈路，不會自動改設定。',
  'audioSignalPath.doctor.waiting.detail': '開始播放後，ECHO 會在這裡解釋目前訊號鏈路。',
  'audioSignalPath.doctor.waiting.title': '等待播放',
  'audioSignalPath.meter.clipDetected.many': '偵測到 {count} 次削波',
  'audioSignalPath.meter.clipDetected.one': '偵測到 1 次削波',
  'audioSignalPath.meter.closeToZero': '輸出已經接近 0 dBFS',
  'audioSignalPath.meter.detail': '{source} / headroom {headroom} / peak {peak}',
  'audioSignalPath.meter.limiterHolding': '保護限幅器正在壓住輸出',
  'audioSignalPath.meter.note': '提示：削波造成的影響最終要用耳朵確認，面板數據只作為參考。',
  'audioSignalPath.meter.sourceFallback': '回退電平',
  'audioSignalPath.meter.sourceNative': '原生後 DSP 電平',
  'audioSignalPath.meter.sourcePcm': 'PCM 即時電平',
  'audioSignalPath.meter.sourcePriming': '電平表預熱中',
  'audioSignalPath.meter.waiting': '等待即時電平',
  'audioSignalPath.meter.notReported': '暫無資料',
  'audioSignalPath.meter.sdmDomain': 'PCM 域電平',
  'audioSignalPath.meter.sdmUnavailableDetail': 'native SDM 鏈路未上報 PCM 域電平；這裡不是 DAC 端 SDM/底噪實測。',
  'audioSignalPath.metric.bitPerfectCandidate': 'Bit-perfect 候選',
  'audioSignalPath.metric.clock': '時鐘',
  'audioSignalPath.metric.clockPending': '時鐘等待中',
  'audioSignalPath.metric.directPath': '直接路徑',
  'audioSignalPath.metric.external': '外部',
  'audioSignalPath.metric.externalRendererClock': '外部渲染器時鐘',
  'audioSignalPath.metric.hqPlayerClock': 'HQPlayer 時鐘',
  'audioSignalPath.metric.hqPlayerError': 'HQPlayer 回報了異常',
  'audioSignalPath.metric.hqPlayerOwnsGain': 'HQPlayer 接管增益與最終渲染',
  'audioSignalPath.metric.liveLevel': '即時電平',
  'audioSignalPath.metric.mediaMetadata': '媒體中繼資料',
  'audioSignalPath.metric.nativeClockPath': '原生時鐘路徑',
  'audioSignalPath.metric.noDspModules': '未啟用 DSP 模組',
  'audioSignalPath.metric.output': '輸出',
  'audioSignalPath.metric.processing': '處理',
  'audioSignalPath.metric.rateConversionActive': '取樣率轉換已啟用',
  'audioSignalPath.metric.source': '來源',
  'audioSignalPath.metric.sourceDeviceRateDiffers': '來源與裝置取樣率不同',
  'audioSignalPath.theater.title': '聲音鏈路',
};

export const zhTW: TranslationDictionary = {
  'common.skipToContent': '跳到主要內容',
  'userNotice.action.accept': '我已閱讀並同意',
  'userNotice.action.communityBoundaries': '社群邊界',
  'userNotice.action.decline': '不同意，退出',
  'userNotice.action.openDocs': '開啟文件',
  'userNotice.aiNotice': 'ECHO 使用了 AI（Codex 5.5、Claude Fable）來寫程式碼。如果您無法接受或認為智商大於它們，請立刻退出並拿下 ICPC World Final Winner 來證明自己的實力~',
  'userNotice.antiCrack': 'ECHO 會持續加強反破解和完整性保護；破解、繞過鎖定、傳播解鎖方式都不被接受。',
  'userNotice.boundariesPrefix': '強烈要求所有使用者閱讀社群邊界：',
  'userNotice.boundariesSuffix': '。',
  'userNotice.description': '繼續使用 ECHO 前必須先確認這些邊界。同意後才能進入；不同意將退出 ECHO。',
  'userNotice.dmca': 'ECHO 遵守 DMCA，不提供下載，也不會協助繞過會員、版權、DRM 或平台限制。',
  'userNotice.docsPrefix': '首先請閱讀 ECHO 文件：',
  'userNotice.docsSuffix': '。有不懂的內容，請優先查看文件。',
  'userNotice.error.desktopBridgeDecline': '不同意使用者須知時不能進入 ECHO。請關閉視窗退出。',
  'userNotice.error.desktopBridgeSave': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'userNotice.locked': '該事件讓 ECHO 鎖定了網盤、連接功能（AirPlay / DLNA / 電台 / HQPlayer）和部分主題。',
  'userNotice.partnersPrefix': '另外，以下是 OpenAI 的合作夥伴網路：',
  'userNotice.partnersSuffix': '。如果您無法忍受 vibe coding，請立刻停用並嚴肅抵制。',
  'userNotice.title': 'ECHO 使用者須知',
  ...audioSignalPathExtrasZhTW,
  ...mediaLibraryZhTW,
  ...likedPageZhTW,
  ...settingsAboutDangerZhTW,
  ...settingsIntegrationsExtraZhTW,
  ...songsPageZhTW,
  ...communityWorkshopZhTW,
  ...connectPageCoverageZhTW,
  'app.window.restore': '還原',
  'app.window.fullscreen': '全螢幕',
  'app.window.exitFullscreen': '退出全螢幕',
  'albumTagEditor.action.applyToForm': '套用到表單',
  'albumTagEditor.action.cancel': '取消',
  'albumTagEditor.action.chooseCover': '選擇封面',
  'albumTagEditor.action.close': '關閉編輯標籤',
  'albumTagEditor.action.deleteAlbum': '刪除專輯',
  'albumTagEditor.action.loadEmbedded': '重讀嵌入標籤',
  'albumTagEditor.action.loading': '讀取中',
  'albumTagEditor.action.loadNetwork': '從網路載入',
  'albumTagEditor.action.openInExplorer': '從檔案總管開啟',
  'albumTagEditor.action.saveTags': '儲存標籤',
  'albumTagEditor.action.saving': '儲存中',
  'albumTagEditor.action.searchCandidates': '搜尋候選',
  'albumTagEditor.action.searching': '搜尋中',
  'albumTagEditor.albumSummary': '{count} 首 / {duration}',
  'albumTagEditor.cover.embeddedSuffix': ' / 已從嵌入標籤重新載入封面',
  'albumTagEditor.cover.localSuffix': ' / 本機封面：{path}',
  'albumTagEditor.cover.networkSuffix': ' / 網路封面會在儲存時下載並寫入',
  'albumTagEditor.currentAlbum': '目前專輯',
  'albumTagEditor.currentAlbumAria': '目前專輯',
  'albumTagEditor.discard.continue': '繼續編輯',
  'albumTagEditor.discard.discard': '捨棄更改',
  'albumTagEditor.discard.prompt': '有未儲存更改，確認關閉並捨棄嗎？',
  'albumTagEditor.duration.hoursMinutes': '{hours} 小時 {minutes} 分鐘',
  'albumTagEditor.duration.minutes': '{minutes} 分鐘',
  'albumTagEditor.duration.unknown': '未知時長',
  'albumTagEditor.error.chooseCoverUnsupported': '目前執行環境不支援選擇封面。',
  'albumTagEditor.error.embeddedUnsupported': '目前執行環境不支援讀取嵌入標籤。',
  'albumTagEditor.error.fixYearBeforeSave': '請先修正年份，再儲存標籤。',
  'albumTagEditor.error.networkTemporary': '網路來源暫時不可用，請稍後再試。',
  'albumTagEditor.error.networkUnsupported': '目前執行環境不支援網路標籤搜尋。',
  'albumTagEditor.error.noReadableTrack': '這張專輯沒有可讀取標籤的歌曲。',
  'albumTagEditor.error.openFolderUnsupported': '目前執行環境不支援開啟檔案總管。',
  'albumTagEditor.error.positiveInteger': '{label}必須是正整數或留空',
  'albumTagEditor.error.readTracksUnsupported': '目前執行環境不支援讀取專輯曲目。',
  'albumTagEditor.field.album': '專輯',
  'albumTagEditor.field.albumArtist': '專輯演出者',
  'albumTagEditor.field.cover': '封面',
  'albumTagEditor.field.genre': '曲風',
  'albumTagEditor.field.year': '年份',
  'albumTagEditor.message.appliedNetwork': '已套用到表單，點擊儲存後才會寫入專輯內歌曲。',
  'albumTagEditor.message.noNetworkTags': '沒有找到合適的網路標籤。',
  'albumTagEditor.message.searchingNetwork': '正在搜尋網路標籤...',
  'albumTagEditor.network.aria': '網路候選對比',
  'albumTagEditor.network.column.candidate': '候選',
  'albumTagEditor.network.column.current': '目前',
  'albumTagEditor.network.column.field': '欄位',
  'albumTagEditor.network.selectAll': '全選',
  'albumTagEditor.network.selectFields': '選擇要套用到專輯的欄位',
  'albumTagEditor.network.title': '網路候選',
  'albumTagEditor.saveDescription': '儲存會寫入這張專輯內所有歌曲的嵌入標籤，並立即同步媒體庫。',
  'albumTagEditor.section.albumInfo': '專輯資訊',
  'albumTagEditor.section.albumInfoDescription': '會批次寫入這張專輯內的歌曲',
  'albumTagEditor.subtitle.albumBatch': '專輯級批次標籤',
  'albumTagEditor.subtitle.unsaved': '未儲存更改',
  'albumTagEditor.title': '編輯標籤',
  'albumTagEditor.value.albumCandidate': '專輯候選',
  'albumTagEditor.value.empty': '空',
  'albumTagEditor.value.existingCover': '已有封面',
  'albumTagEditor.value.networkCover': '網路封面',
  'albumTagEditor.value.unknownAlbum': '未知專輯',
  'albumTagEditor.value.unknownArtist': '未知演出者',
  'firstRun.action.finish': '完成設定',
  'firstRun.action.next': '下一步',
  'firstRun.action.previous': '上一步',
  'firstRun.action.skip': '跳過',
  'firstRun.action.skipWizard': '跳過精靈',
  'firstRun.aria.steps': '首次啟動步驟',
  'firstRun.aria.stepNotes': '目前步驟提示',
  'firstRun.aria.summary': '目前精靈選擇摘要',
  'firstRun.audio.asio.description': '需要 ASIO 裝置和可靠驅動。',
  'firstRun.audio.asio.hint': '專業',
  'firstRun.audio.asio.label': 'ASIO',
  'firstRun.audio.exclusive.description': '獨佔裝置，適合確認穩定的外接音效卡或 HiFi 調試。',
  'firstRun.audio.exclusive.hint': '進階',
  'firstRun.audio.exclusive.label': 'WASAPI Exclusive',
  'firstRun.audio.linuxShared.description': '透過 Linux 音訊堆疊使用 ECHO 原生輸出。',
  'firstRun.audio.linuxShared.hint': '進階',
  'firstRun.audio.linuxShared.label': 'Linux 共用輸出',
  'firstRun.audio.shared.description': '日常推薦路徑，使用 ECHO 原生共享輸出。',
  'firstRun.audio.shared.hint': '推薦',
  'firstRun.audio.shared.label': 'WASAPI Shared（推薦）',
  'firstRun.audio.system.description': '瀏覽器相容兜底，遇到裝置異常或原生輸出失敗時再使用。',
  'firstRun.audio.system.hint': '安全',
  'firstRun.audio.system.label': '安全模式',
  'firstRun.accounts.cookie.description': '如果平台登入視窗不可用，再手動貼上 Cookie 並點「手動儲存」，儲存後用「檢查」確認狀態。',
  'firstRun.accounts.cookie.title': '備用方式',
  'firstRun.accounts.login.description': '網易雲、QQ 音樂、Bilibili、SoundCloud 等帳號優先點「登入並同步」，登入完成後 ECHO 會儲存必要憑證。',
  'firstRun.accounts.login.title': '優先使用網頁登入',
  'firstRun.accounts.note': 'ECHO 尊重版權與 DMCA。請盡量使用本地媒體庫；串流支援為最低優先級，大多數串流問題將不會被回應。',
  'firstRun.accounts.open.description': '完成精靈後進入「設定 > 整合 > 帳號登入」，這裡集中管理串流、MV 和歌詞相關帳號。',
  'firstRun.accounts.open.title': '入口位置',
  'firstRun.accounts.spotify.description': 'ECHO 不會提供任何下載服務，並遵守 DMCA；也不會協助繞過會員、版權、DRM 或平台限制。',
  'firstRun.accounts.spotify.title': '版權與 DMCA',
  'firstRun.cache.chooseLocation': '選擇快取位置',
  'firstRun.cache.useDefault': '使用預設',
  'firstRun.currentSelection': '目前選擇',
  'firstRun.defaultLocation': '預設位置',
  'firstRun.description': '按順序確認媒體庫、快取、掃描、輸出和外觀。不確定的地方保留推薦值即可。',
  'firstRun.detail.accounts.later': '串流支援最低優先級',
  'firstRun.detail.accounts.local': '請優先使用本地媒體庫',
  'firstRun.detail.appearance.preview': '主題會立即預覽',
  'firstRun.detail.appearance.system': '跟隨系統更省心',
  'firstRun.detail.audio.advanced': '獨佔 / ASIO 適合穩定外接裝置',
  'firstRun.detail.audio.shared': '一般裝置先用共享輸出',
  'firstRun.detail.cache.changeLater': 'C 槽緊張就換到其他磁碟',
  'firstRun.detail.cache.space': '封面、歌詞和 MV 快取在這裡',
  'firstRun.detail.language.applyNow': '選擇後介面會立即切換',
  'firstRun.detail.language.changeLater': '之後可在設定中更改',
  'firstRun.detail.library.safe': '只讀取標籤和封面，不移動檔案',
  'firstRun.detail.library.scan': '勾選後完成即開始索引',
  'firstRun.detail.performance.optional': '可選功能，預設關閉',
  'firstRun.detail.performance.changeLater': '之後可在設定中調整',
  'firstRun.detail.scan.balanced': '均衡適合日常首次匯入',
  'firstRun.detail.scan.low': '低占用更適合邊聽邊掃',
  'firstRun.detail.summary.docs': '更多教程可隨時開啟文檔',
  'firstRun.detail.summary.save': '完成後儲存這些選擇',
  'firstRun.docs.action': '查看 ECHO 文檔',
  'firstRun.docs.description': '安裝、媒體庫、音訊輸出和排障教程都在這裡。',
  'firstRun.docs.title': 'ECHO 官方文檔',
  'firstRun.feature.albumWall.description': '用虛擬列表顯示大量專輯封面，捲動時更省資源。',
  'firstRun.feature.albumWall.hint': '大媒體庫',
  'firstRun.feature.albumWall.label': '專輯牆虛擬化',
  'firstRun.feature.enabled': '已開啟',
  'firstRun.feature.localDirect.description': '實驗功能：符合條件的本機 WAV/FLAC/MP3 會直接讀取，減少起播等待。',
  'firstRun.feature.localDirect.hint': '實驗',
  'firstRun.feature.localDirect.label': '本機直讀',
  'firstRun.feature.lowLoad.description': '播放時減少頻譜、封面、歌詞和背景刷新壓力，適合邊聽邊操作。',
  'firstRun.feature.lowLoad.hint': '省資源',
  'firstRun.feature.lowLoad.label': '減輕占用播放',
  'firstRun.language.choose': '選擇',
  'firstRun.language.current': '目前',
  'firstRun.language.en-US.description': '選單、引導和設定使用英語。',
  'firstRun.language.ja-JP.description': '選單、引導和設定使用日語。',
  'firstRun.language.ko-KR.description': '選單、引導和設定使用韓語。',
  'firstRun.language.zh-CN.description': '選單、教學和設定使用簡體中文。',
  'firstRun.language.zh-TW.description': '選單、教學和設定使用繁體中文。',
  'firstRun.pro.action': '開啟贊助渠道',
  'firstRun.pro.benefit.connect': '解鎖 ECHO 連接功能，可連接 ECHO Mobile 安卓端並共享媒體庫。',
  'firstRun.pro.benefit.dsp': '解鎖完整 DSP Center：Headroom、參數 / 圖形 EQ、OPRA 耳機校正、FIR 房間校正、聲道平衡與輸出安全監控。',
  'firstRun.pro.benefit.donators': '加入 ECHO Donators 群。',
  'firstRun.pro.benefit.mobile': '搶先體驗 ECHO 手機版。',
  'firstRun.pro.benefit.network': '解鎖 AirPlay / DLNA / 電台 / ECHO Connect 功能。',
  'firstRun.pro.benefit.plugins': '解鎖 ECHO 插件功能。',
  'firstRun.pro.benefit.storeKey': '如果未來上架 Steam / Microsoft Store，可免費獲得 key。',
  'firstRun.pro.benefit.themes': '解鎖所有主題。',
  'firstRun.pro.benefitsTitle': 'ECHO Pro 權益',
  'firstRun.pro.contributors': 'Tips：所有為項目做出貢獻的用戶將免費獲得此權益（GitHub Contributors Only）。',
  'firstRun.pro.description': '透過贊助渠道您可以獲得以下權益，該權益為永久。',
  'firstRun.pro.extrasTitle': '其他永久權益',
  'firstRun.pro.feature.connect.title': 'ECHO Connect',
  'firstRun.pro.feature.customize.title': '主題與插件',
  'firstRun.pro.feature.dsp.title': 'DSP Center',
  'firstRun.pro.feature.network.title': '無線播放與多端',
  'firstRun.pro.freeNotice': '當然，在不捐助的情況下，您仍可正常使用基本所有功能，ECHO 保證永久免費更新喵喵喵。',
  'firstRun.pro.kicker': '贊助 / 永久權益',
  'firstRun.pro.title': 'ECHO Pro',
  'firstRun.error.desktopBridgeCache': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'firstRun.error.desktopBridgeMusicFolder': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'firstRun.error.desktopBridgeSave': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'firstRun.library.chooseFolder': '選擇資料夾',
  'firstRun.library.noneSelected': '未選擇，稍後新增也可以。',
  'firstRun.library.scanAfterFinish': '完成後掃描',
  'firstRun.mascot.basics': '你好呀！先挑個順手的語言吧～',
  'firstRun.mascot.library': '把音樂資料夾交給我吧！',
  'firstRun.mascot.personalize': '換上你喜歡的顏色！',
  'firstRun.mascot.playback': '輸出方式我來幫你把關～',
  'firstRun.mascot.summary': '都準備好啦，出發！',
  'firstRun.message.saved': '首次啟動設定已儲存。',
  'firstRun.scan.balanced.description': '推薦。掃描速度和背景占用都比較穩。',
  'firstRun.scan.balanced.hint': '預設',
  'firstRun.scan.balanced.label': '均衡',
  'firstRun.scan.low.description': '更少打擾播放，掃描會慢一些。',
  'firstRun.scan.low.hint': '邊聽邊掃',
  'firstRun.scan.low.label': '低占用',
  'firstRun.scan.performance.description': '優先盡快建庫，適合電腦閒置時使用。',
  'firstRun.scan.performance.hint': '閒置時',
  'firstRun.scan.performance.label': '快速',
  'firstRun.step.audio.description': '一般耳機、藍牙和電腦喇叭建議使用 WASAPI Shared；安全模式只作為瀏覽器相容兜底，外接音效卡、獨佔模式或 ASIO 只建議在確認裝置可靠後再啟用。',
  'firstRun.step.audio.eyebrow': '5 / 8',
  'firstRun.step.audio.label': '輸出',
  'firstRun.step.audio.title': '選擇音訊輸出',
  'firstRun.step.appearance.description': '這裡先決定整體明暗模式和主題配色。淺色適合白天和辦公環境，深色適合夜間；不確定就跟隨系統。',
  'firstRun.step.appearance.eyebrow': '6 / 8',
  'firstRun.step.appearance.label': '外觀',
  'firstRun.step.appearance.title': '選擇主題和明暗模式',
  'firstRun.step.accounts.description': '串流帳號可以稍後登入，但它是最低優先級能力。大多數串流問題將不會被回應，請盡量使用本地媒體庫。',
  'firstRun.step.accounts.eyebrow': '7 / 8',
  'firstRun.step.accounts.label': '帳號',
  'firstRun.step.accounts.title': '串流帳號怎麼登入',
  'firstRun.step.cache.description': '封面、歌詞、MV 快取會占用磁碟空間。C 槽緊張時建議換到其他磁碟；之後也可以在設定裡調整。',
  'firstRun.step.cache.eyebrow': '3 / 8',
  'firstRun.step.cache.label': '快取',
  'firstRun.step.cache.title': '選擇快取位置',
  'firstRun.step.language.description': '先選擇 ECHO 的顯示語言，後面的引導、設定和選單會立刻按所選語言顯示。',
  'firstRun.step.language.eyebrow': '1 / 8',
  'firstRun.step.language.label': '語言',
  'firstRun.step.language.title': '選擇顯示語言',
  'firstRun.step.library.description': '選擇音樂根目錄後，ECHO 會讀取標籤、封面、時長和歌詞線索來建立媒體庫。檔案不會被移動或刪除，也可以先跳過。',
  'firstRun.step.library.eyebrow': '2 / 8',
  'firstRun.step.library.label': '音樂',
  'firstRun.step.library.title': '選擇音樂資料夾',
  'firstRun.step.performance.description': '這些開關預設關閉。電腦負載較高或媒體庫較大時可按需開啟；不確定就保持關閉。',
  'firstRun.step.performance.label': '效能',
  'firstRun.step.performance.title': '選擇可選效能功能',
  'firstRun.step.scan.description': '首次掃描會比較忙。日常推薦均衡；如果正在聽歌或電腦負載高，用低占用；電腦閒置時再用快速。',
  'firstRun.step.scan.eyebrow': '4 / 8',
  'firstRun.step.scan.label': '掃描',
  'firstRun.step.scan.title': '選擇掃描方式',
  'firstRun.step.summary.description': '確認後只儲存這些基礎設定。之後可以在設定裡重新開啟首次啟動指引，帳號也可以隨時補登入。',
  'firstRun.step.summary.eyebrow': '8 / 8',
  'firstRun.step.summary.label': '確認',
  'firstRun.step.summary.title': '確認設定',
  'firstRun.summary.addLater': '稍後新增',
  'firstRun.summary.after.enter': '進入 ECHO 主介面',
  'firstRun.summary.after.save': '儲存剛才選擇的設定',
  'firstRun.summary.after.scan': '依照目前選擇建立媒體庫索引',
  'firstRun.summary.afterTitle': '完成後',
  'firstRun.summary.accounts': '帳號',
  'firstRun.summary.accountsLater': '稍後在設定 > 整合登入',
  'firstRun.summary.cache': '快取',
  'firstRun.summary.configurationTitle': '本次設定',
  'firstRun.summary.features': '功能',
  'firstRun.summary.featuresDefault': '保持預設關閉',
  'firstRun.summary.kicker': '摘要',
  'firstRun.summary.music': '音樂',
  'firstRun.summary.noFileMove': '不會移動或刪除你的音樂檔案。',
  'firstRun.summary.output': '輸出',
  'firstRun.summary.readyDescription': '點擊完成後儲存設定和可選功能。若已選擇資料夾並勾選掃描，ECHO 會開始建立媒體庫索引。',
  'firstRun.summary.readyTitle': '可以開始了',
  'firstRun.summary.scan': '掃描',
  'firstRun.summary.scanWithFolder': '{mode}，完成後掃描',
  'firstRun.summary.theme': '外觀',
  'firstRun.summary.themeValue': '{mode}，{preset}',
  'firstRun.theme.ambient.description': '深藍黑玻璃氛圍，適合沉浸聽歌和夜間桌面。',
  'firstRun.theme.ambient.hint': '沉浸',
  'firstRun.theme.dark.description': '降低夜間亮度，適合暗光環境和 OLED 螢幕。',
  'firstRun.theme.dark.hint': '夜間',
  'firstRun.theme.light.description': '文字更清楚，適合白天、辦公和截圖。',
  'firstRun.theme.light.hint': '清爽',
  'firstRun.theme.modeTitle': '明暗模式',
  'firstRun.theme.presetTitle': '主題配色',
  'firstRun.theme.system.description': '跟隨 Windows 或系統外觀自動切換。',
  'firstRun.theme.system.hint': '省心',
  'firstRun.title': '歡迎使用 ECHO',
  'downloads.action.addToQueue': '加入佇列',
  'downloads.action.cancelJob': '取消任務',
  'downloads.action.changeFolder': '更換資料夾',
  'downloads.action.checkTools': '檢測環境',
  'downloads.action.chooseFolder': '選擇資料夾',
  'downloads.action.clearCompleted': '清除已完成',
  'downloads.action.clearQueue': '清空下載佇列',
  'downloads.action.creating': '建立中',
  'downloads.action.search': '搜尋',
  'downloads.action.searching': '搜尋中',
  'downloads.description': '搜尋 YouTube / Bilibili / osu!，並只下載最高可用音訊；osu! 會自動轉換為可直接播放的音訊。',
  'downloads.osu.description': '搜尋或貼上 osu! 譜面連結，轉換為帶有完整標籤與封面的 MP3。',
  'downloads.empty.noResults.description': '換個關鍵字再試試。',
  'downloads.empty.noResults.title': '暫無搜尋結果',
  'downloads.empty.queue.action.focusSearch': '定位到查找輸入框',
  'downloads.empty.queue.action.focusUrl': '定位到連結輸入框',
  'downloads.empty.queue.description': '貼上連結或從搜尋結果下載後，會在這裡看到真實進度。',
  'downloads.empty.queue.title': '佇列為空',
  'downloads.empty.searching.description': '正在查詢 {scope}。',
  'downloads.empty.searching.title': '正在搜尋',
  'downloads.empty.searchPrompt.description': '輸入歌曲名、藝術家、譜師或譜面 ID，找到後即可加入右側佇列。',
  'downloads.empty.searchPrompt.title': '搜尋 osu! 譜面',
  'downloads.error.cookieFallback': '無法讀取瀏覽器 Cookie，已自動嘗試不使用登入狀態搜尋。',
  'downloads.error.ipcUnavailable': '目前無法使用下載功能，請重新啟動 ECHO 後再試。',
  'downloads.error.operationFailed': '下載操作失敗',
  'downloads.folder.required': '請選擇下載資料夾',
  'downloads.job.imported': '已匯入媒體庫',
  'downloads.job.receiving': '即時接收中',
  'downloads.job.savedTo': '儲存到 {path}',
  'downloads.job.waitingProgress': '等待進度',
  'downloads.message.clearedTerminal': '已清除完成、失敗和取消的任務。',
  'downloads.message.clearedQueue': '下載佇列已清空。',
  'downloads.message.completed': '下載完成：{title}',
  'downloads.message.queued': '已加入下載佇列。',
  'downloads.message.resultQueued': '已加入佇列：{title}',
  'downloads.queue.summary': '{active} 個進行中 / 共 {total} 個',
  'downloads.queue.title': '下載佇列',
  'downloads.search.aria': '搜尋下載',
  'downloads.search.downloadAudio': '下載音訊',
  'downloads.search.joined': '已加入佇列',
  'downloads.search.placeholder': '搜尋歌曲、藝人、影片標題、osu! 譜面名或 id',
  'downloads.search.providerErrorItem': '{provider}：{error}',
  'downloads.search.providerErrors': '部分平台搜尋失敗：{errors}',
  'downloads.search.scopeAria': '搜尋平台',
  'downloads.search.title': '搜尋下載',
  'downloads.search.unknownUploader': '未知作者',
  'downloads.search.views': '{count} 次播放',
  'downloads.search.viewsWan': '{count} 萬次播放',
  'downloads.settings.audioStrategy': '音訊策略',
  'downloads.settings.bestAvailable': '最高可用音質',
  'downloads.settings.bindMvAfterImport': '匯入後綁定來源 URL 為 MV',
  'downloads.settings.importToLibrary': '完成後匯入媒體庫',
  'downloads.settings.outputDirectory': '下載資料夾',
  'downloads.settings.title': '下載設定',
  'downloads.status.bindingMv': '綁定 MV',
  'downloads.status.cancelled': '已取消',
  'downloads.status.completed': '已完成',
  'downloads.status.downloading': '下載中',
  'downloads.status.extractingAudio': '提取音訊',
  'downloads.status.failed': '失敗',
  'downloads.status.importing': '匯入媒體庫',
  'downloads.status.probing': '解析連結',
  'downloads.status.queued': '佇列中',
  'downloads.title': '下載',
  'downloads.tools.notBundled': '未隨應用程式安裝',
  'downloads.tools.notDetected': '未檢測到',
  'downloads.tools.title': '環境檢測',
  'downloads.url.placeholder': '貼上 YouTube / Bilibili / SoundCloud / osu! 連結',
  'downloads.url.title': '貼上連結下載',
  'albumMenu.action.addToPlaylist': '加入播放清單...',
  'albumMenu.action.addToQueue': '加入佇列',
  'albumMenu.action.copyArtist': '複製演出者',
  'albumMenu.action.copyCover': '複製專輯封面',
  'albumMenu.action.copyInfo': '複製專輯資訊',
  'albumMenu.action.copyPath': '複製路徑',
  'albumMenu.action.copyTitle': '複製標題',
  'albumMenu.action.copyTrackList': '複製曲目列表',
  'albumMenu.action.deleteAlbum': '刪除專輯',
  'albumMenu.action.editTags': '編輯標籤',
  'albumMenu.action.likeAlbum': '喜歡專輯',
  'albumMenu.action.playAlbum': '播放專輯',
  'albumMenu.action.playNext': '播放下一首',
  'albumMenu.action.saveCover': '儲存專輯封面',
  'albumMenu.action.showInFolder': '在資料夾中顯示',
  'albumMenu.action.unlikeAlbum': '取消喜歡專輯',
  'albumMenu.copySubmenu.aria': '複製專輯資訊',
  'albumMenu.playlistSubmenu.aria': '選擇播放清單',
  'albumMenu.playlistSubmenu.empty': '沒有本機播放清單',
  'albumMenu.playlistSubmenu.itemCount': '{count} 首',
  'albumMenu.playlistSubmenu.loading': '正在讀取播放清單...',
  'accountProvider.kugou': '酷狗音樂',
  'accountProvider.netease': '網易雲音樂',
  'accountProvider.unknown': '未知帳號',
  'desktopLyrics.aria.stage': '桌面歌詞',
  'desktopLyrics.control.close': '關閉',
  'desktopLyrics.control.colorSwatch': '顏色 {color}',
  'desktopLyrics.control.customColor': '自訂顏色',
  'desktopLyrics.control.decreaseFontSize': '縮小字號',
  'desktopLyrics.control.decreaseSecondaryFontSize': '縮小翻譯字號',
  'desktopLyrics.control.decreaseScale': '縮小',
  'desktopLyrics.control.increaseFontSize': '放大字號',
  'desktopLyrics.control.increaseSecondaryFontSize': '放大翻譯字號',
  'desktopLyrics.control.increaseScale': '放大',
  'desktopLyrics.control.lock': '鎖定',
  'desktopLyrics.control.pause': '暫停',
  'desktopLyrics.control.play': '播放',
  'desktopLyrics.control.resetPosition': '重設位置',
  'desktopLyrics.control.romanization': '桌面歌詞顯示羅馬音',
  'desktopLyrics.control.secondaryFontSizeValue': '譯 {size}px',
  'desktopLyrics.control.themeColor': '跟隨主題顏色',
  'desktopLyrics.control.textDirection': '切換橫排 / 直排',
  'desktopLyrics.control.translation': '桌面歌詞顯示翻譯',
  'desktopLyrics.control.translationShort': '譯',
  'desktopLyrics.primary.empty': '暫無歌詞',
  'desktopLyrics.primary.instrumental': '純音樂，請欣賞',
  'desktopLyrics.secondary.waiting': '等待播放',
  'lyricsView.empty.instrumental': '純音樂，請欣賞',
  'lyricsView.empty.noLyrics': '暫無歌詞',
  'mvPanel.action.close': '關閉',
  'mvPanel.action.copied': '已複製',
  'mvPanel.action.copy': '複製',
  'mvPanel.action.dismissUnavailable': '關閉 MV 不可用提示',
  'mvPanel.diagnostics.title': 'MV 診斷報告',
  'mvPanel.notice.unavailable': 'MV 不可用',
  'mvPanel.status.bilibiliBlocked': 'Bilibili 暫時拒絕直連解析。可以在 MV 設定裡開啟診斷報告、配置 Bilibili Cookie，或先外部開啟。',
  'mvPanel.status.databaseUnread': 'MV 資料庫不可讀。先到曲庫恢復裡修復資料庫；修復前可以在 MV 設定裡暫時綁定本機影片或自訂連結。',
  'mvPanel.status.externalRequired': '目前 MV 需要外部播放。點 MV 設定裡的「在外部開啟」，或換一個應用內候選。',
  'mvPanel.status.inAppUnavailable': '目前 MV 暫時不能在應用程式內播放。可以降低最高畫質、換來源，或外部開啟。',
  'mvPanel.status.loadFailed': 'MV 載入失敗',
  'mvPanel.status.loading': '正在載入 MV',
  'mvPanel.status.localUnsupported': '本機影片格式不支援。請換成 mp4/webm，或用系統播放器外部開啟。',
  'mvPanel.status.missingUrl': '缺少可播放位址。請在 MV 設定裡重新搜尋、手動貼上連結，或匯入本機影片。',
  'mvPanel.status.networkFailed': '網路 MV 請求失敗。先檢查代理/網路；也可以在 MV 設定裡切換 Bilibili/YouTube 來源、降低畫質後重試。',
  'mvPanel.status.notFound': '找不到可播放 MV。可以開啟 MV 設定手動搜尋、貼上 Bilibili/YouTube 連結，或匯入本機影片。',
  'mvPanel.status.temporaryPlayback': '臨時 MV 播放中，資料庫待修復',
  'mvPanel.status.unavailable': 'MV 不可用',
  'mvPanel.status.videoFailed': '影片載入失敗。請換一個候選、降低畫質，或外部開啟；本機影片可換成瀏覽器支援的 mp4/webm。',
  'miniPlayer.action.close': '關閉迷你播放器',
  'miniPlayer.action.closeQueue': '收起播放佇列',
  'miniPlayer.action.closeShort': '關閉',
  'miniPlayer.action.next': '下一首',
  'miniPlayer.action.openQueue': '開啟播放佇列',
  'miniPlayer.action.pause': '暫停',
  'miniPlayer.action.play': '播放',
  'miniPlayer.action.previous': '上一首',
  'miniPlayer.action.resetPosition': '重置位置',
  'miniPlayer.action.volume': '調整音量',
  'miniPlayer.aria.progress': '播放進度',
  'miniPlayer.aria.queue': '播放佇列',
  'miniPlayer.aria.shell': '迷你播放器',
  'miniPlayer.aria.volume': '音量',
  'miniPlayer.artist.unknown': '未知演出者',
  'miniPlayer.status.hqPlayerTakeover': 'HQPlayer 接管中',
  'miniPlayer.status.queueEmpty': '佇列為空',
  'miniPlayer.status.ready': '就緒',
  'pet.action.hide': '隱藏寵物',
  'pet.action.playPause': '播放 / 暫停',
  'pet.action.resetPosition': '重設位置',
  'pet.aria.character': 'ECHO 像素寵物',
  'pet.aria.window': 'ECHO 寵物',
  'playerTransport.action.like': '喜歡',
  'playerTransport.action.likeCurrent': '喜歡目前歌曲',
  'playerTransport.action.lyrics': '歌詞',
  'playerTransport.action.mv': 'MV',
  'playerTransport.action.next': '下一首',
  'playerTransport.action.pause': '暫停',
  'playerTransport.action.play': '播放',
  'playerTransport.action.playInOrder': '順序播放',
  'playerTransport.action.previous': '上一首',
  'playerTransport.action.queue': '播放佇列',
  'playerTransport.action.repeat': '循環模式',
  'playerTransport.action.repeatOne': '單曲循環',
  'playerTransport.action.shuffle': '隨機播放',
  'playerTransport.action.unlike': '取消喜歡',
  'playerTransport.action.unlikeCurrent': '取消喜歡目前歌曲',
  'playerStatus.audioSpecifications': '音訊規格',
  'playerStatus.ready': '就緒',
  'playerStatus.streaming': '串流媒體',
  'playerDacArrival.close': '關閉 DAC 登場卡片',
  'playerDacArrival.detail.asio': 'ECHO 正在透過 {mode} 驅動這台裝置',
  'playerDacArrival.detail.exclusive': 'ECHO 正在透過 {mode} 獨佔這台裝置',
  'playerDacArrival.eyebrow': 'DAC 抵達',
  'playerDacArrival.issue.label': '最近失敗',
  'playerDacArrival.issue.none': '暫無失敗',
  'playerDacArrival.native.label': '原生證明',
  'playerDacArrival.native.pending': 'Native 待證明',
  'playerDacArrival.native.rate': 'Native {rate}',
  'playerDacArrival.rates.label': '見過的輸出',
  'playerDacArrival.rates.pending': '等待觀測',
  'playerDacArrival.status.takeover': '已接管',
  'playerDacArrival.title': '{device} 已接管',
  'playerSpeed.label': '播放速度',
  'playerSpeed.reset': '重置播放速度',
  'playerVolume.fixed.disable': '關閉固定音量',
  'playerVolume.fixed.enable': '開啟固定音量',
  'playerVolume.fixed.enabled': '固定音量已開啟',
  'playerVolume.fixed.dsdAutoLocked': 'DSD 播放中已自動鎖定音量',
  'playerVolume.fixed.title': '固定音量',
  'import.dragDrop.desktopBridgeUnavailable': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'import.dragDrop.files.empty': '找不到可匯入的音訊檔案。',
  'import.dragDrop.files.failed': '{count} 個檔案匯入失敗',
  'import.dragDrop.files.ignored': '忽略 {count} 個不支援檔案',
  'import.dragDrop.files.imported': '已匯入 {count} 首歌曲',
  'import.dragDrop.files.summaryWithOutput': '{summary}。檔案已儲存到：{outputDirectory}',
  'import.dragDrop.noDroppedFiles': '未讀取到拖放檔案。',
  'import.dragDrop.overlay.description': '本機檔案保留原路徑，資料夾會直接掃描',
  'import.dragDrop.overlay.title': '拖入音樂、資料夾或 osu! 譜面以匯入曲庫',
  'import.dragDrop.plugin.cancelled': '已取消插件包匯入。',
  'import.dragDrop.plugin.desktopBridgeUnavailable': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'import.dragDrop.plugin.enabled': '插件 {pluginId} 已匯入並啟用。',
  'import.dragDrop.plugin.importedNeedsReview': '插件 {pluginId} 已匯入，自動啟用失敗：{reason}',
  'import.dragDrop.plugin.overlay.description': '放開後會匯入插件包並嘗試啟用',
  'import.dragDrop.plugin.overlay.title': '放開匯入 .echo 插件包',
  'import.dragDrop.paths.addedFolders': '已新增 {count} 個資料夾',
  'import.dragDrop.paths.empty': '找不到可匯入的音樂檔案或資料夾',
  'import.dragDrop.paths.failed': '{count} 個路徑匯入失敗',
  'import.dragDrop.paths.ignored': '忽略 {count} 個不支援檔案',
  'import.dragDrop.paths.importedFiles': '已匯入 {count} 個檔案',
  'import.dragDrop.paths.missing': '跳過 {count} 個無法存取的路徑',
  'import.dragDrop.paths.scannedAudioFolders': '已掃描 {count} 個音樂檔案所在資料夾',
  'notice.action.close': '關閉',
  'notice.action.closeNotice': '關閉提示',
  'notice.action.ignore': '忽略',
  'notice.action.openMarkdownReport': '開啟 Markdown',
  'notice.action.openReport': '開啟報告',
  'notice.action.openTextReport': '開啟 TXT',
  'notice.action.updateNow': '立即更新',
  'notice.action.viewUpdateDetails': '查看詳情',
  'notice.audioError.description': '下方顯示的是簡短說明；完整原始錯誤、裝置狀態與排查線索保存在本機報告裡。',
  'notice.audioError.title': '播放遇到問題',
  'notice.audioComponent.action.download': '前往官方下載',
  'notice.audioComponent.action.import': '選擇已下載元件',
  'notice.audioComponent.action.later': '暫不處理',
  'notice.audioComponent.description': '基礎音訊元件缺失或損壞，部分格式或播放功能暫時無法使用。可手動下載官方修復元件；ECHO 不會自動下載或強制安裝。',
  'notice.audioComponent.importFailed': '元件匯入失敗：{error}',
  'notice.audioComponent.installed': '音訊元件已安裝，請重新播放。',
  'notice.audioComponent.size': '元件安裝後約佔 230 MiB；可前往官方頁面自行下載。',
  'notice.audioComponent.title': '需要選用音訊元件',
  'notice.audioDefaultFormatWarning': '音訊設定提醒：Windows 預設格式設定偏高，目前是 {rate}。ECHO 只會提醒，不會修改系統設定；建議改到 48 kHz。',
  'notice.audioDeviceNotFoundFallback': '所選音訊裝置不存在，已自動回退到系統預設輸出（Shared）。',
  'notice.diagnosticsCrash.description': '上次 ECHO 未正常結束，已準備本機診斷報告；可以開啟 Markdown 或 TXT 查看完整細節。',
  'notice.memoryPressure.description': 'ECHO 目前佔用 {usage}，已達到 {threshold} 警戒線。最大程序：{process}（{processMemory}）。',
  'notice.memoryPressure.reportReady': '已自動產生本機記憶體分析報告。',
  'notice.memoryPressure.title': 'ECHO 記憶體佔用過高',
  'notice.upcomingTrack.coverAlt': '{title} 的封面',
  'notice.upcomingTrack.kicker': '下一首即將播放',
  'notice.importFiles.empty': '沒有可匯入的音訊檔案。',
  'notice.importFiles.failed': '{count} 個檔案匯入失敗',
  'notice.importFiles.imported': '已加入曲庫 {count} 個檔案',
  'notice.importFiles.skipped': '略過 {count} 個不支援或不可用檔案',
  'notice.openFiles.partial': '已開啟 {opened} 個檔案，略過 {rejected} 個不支援或不可用檔案。',
  'notice.reportOpened': 'Markdown 報告已開啟。',
  'notice.reportOpenedMarkdown': 'Markdown 報告已開啟。',
  'notice.reportOpenedMarkdownPath': 'Markdown 報告已開啟：{path}',
  'notice.reportOpenedPath': 'Markdown 報告已開啟：{path}',
  'notice.reportOpenedText': 'TXT 報告已開啟。',
  'notice.reportOpenedTextPath': 'TXT 報告已開啟：{path}',
  'notice.updateAvailable': '發現 ECHO 新版本。',
  'notice.updateAvailableVersion': '發現 ECHO 新版本 {version}。',
  'notice.updateDownloaded': 'ECHO 更新已下載完成，準備安裝。',
  'notice.updateDownloadedVersion': 'ECHO {version} 已下載完成，準備安裝。',
  'punctuation.clauseSeparator': '，',
  'punctuation.listSeparator': '、',
  'library.action.refresh': '重新整理',
  'albumDetail.action.addToQueue': '加入佇列',
  'albumDetail.action.back': '專輯',
  'albumDetail.action.likeAlbum': '喜歡專輯',
  'albumDetail.action.more': '更多專輯操作',
  'albumDetail.action.openSource': '開啟來源',
  'albumDetail.action.playNow': '立即播放',
  'albumDetail.action.readingAlbum': '正在讀取專輯',
  'albumDetail.action.refresh': '重新整理',
  'albumDetail.action.showInFolder': '在資料夾中顯示',
  'albumDetail.action.unlikeAlbum': '取消喜歡專輯',
  'albumDetail.action.viewCover': '查看完整封面',
  'albumDetail.aria.coverPreview': '{album} 封面原圖',
  'albumDetail.aria.details': '{album} 專輯詳情',
  'albumDetail.aria.info': '專輯資訊',
  'albumDetail.aria.metadata': '專輯中繼資料',
  'albumDetail.aria.openArtist': '開啟藝術家 {artist}',
  'albumDetail.aria.sections': '專輯分區',
  'albumDetail.aria.trackConsole': '{album} 曲目控制台',
  'albumDetail.artist.notFound': '找不到藝術家：{artist}',
  'albumDetail.count.albums': '{count} 張專輯',
  'albumDetail.count.loadedAlbums': '{loaded}/{total} 張專輯',
  'albumDetail.count.loadedTracks': '{loaded}/{total} 首歌',
  'albumDetail.count.tracks': '{count} 首歌',
  'albumDetail.dna.aria': '專輯聲紋卡',
  'albumDetail.dna.artistAria': '開啟藝人 {artist}',
  'albumDetail.dna.artistCount': '{count} 位相關藝人',
  'albumDetail.dna.artists': '藝人關係',
  'albumDetail.dna.bitDepth': '位元深度',
  'albumDetail.dna.codecMix': '格式組成',
  'albumDetail.dna.collapse': '收起聲紋卡',
  'albumDetail.dna.countTracks': '{count} 首',
  'albumDetail.dna.depthProfile': '位深画像',
  'albumDetail.dna.expand': '展開聲紋卡',
  'albumDetail.dna.format': '格式組成',
  'albumDetail.dna.integrity': '完整度',
  'albumDetail.dna.integrityComplete': '曲序完整',
  'albumDetail.dna.integrityDiscGaps': '斷裂 Disc',
  'albumDetail.dna.integrityLoaded': '載入曲目',
  'albumDetail.dna.integrityReview': '需要檢查',
  'albumDetail.dna.integrityStatus': '曲序狀態',
  'albumDetail.dna.integrityTrackGaps': '缺少曲號',
  'albumDetail.dna.integrityUnnumbered': '未寫曲號',
  'albumDetail.dna.kicker': '專輯聲紋卡',
  'albumDetail.dna.memory': '聽歌記憶',
  'albumDetail.dna.memoryEmpty': '暫無記憶',
  'albumDetail.dna.memoryLiked': '已收藏',
  'albumDetail.dna.memoryLoading': '讀取記憶中',
  'albumDetail.dna.memoryPlays': '{count} 次播放',
  'albumDetail.dna.memoryRecentTrack': '最近回潮',
  'albumDetail.dna.memorySkippedTrack': '常跳過',
  'albumDetail.dna.memorySkips': '{count} 次跳過',
  'albumDetail.dna.memoryStatus': '記憶狀態',
  'albumDetail.dna.memoryTopTrack': '最常播放',
  'albumDetail.dna.noArtists': '暫無關係',
  'albumDetail.dna.quality': '音質画像',
  'albumDetail.dna.qualityMixed44148': '44.1 / 48 混合',
  'albumDetail.dna.qualityMixedDepth': '位深混合',
  'albumDetail.dna.qualityMixedRate': '取樣率混合',
  'albumDetail.dna.qualityUnifiedDepth': '位深統一',
  'albumDetail.dna.qualityUnifiedRate': '取樣率統一',
  'albumDetail.dna.reading': '讀取中',
  'albumDetail.dna.replayGain': 'ReplayGain',
  'albumDetail.dna.replayGainValue': '{count}/{total}',
  'albumDetail.dna.resampleRisk': '重取樣風險',
  'albumDetail.dna.sampleRate': '取樣率',
  'albumDetail.dna.subtitle': '{album} 的本機檔案、曲序、聽歌記憶和藝人入口。',
  'albumDetail.dna.title': '專輯聲紋卡',
  'albumDetail.dna.unknown': '未知',
  'albumDetail.credit.role.arrangement': '編曲',
  'albumDetail.credit.role.engineering': '錄音與工程',
  'albumDetail.credit.role.label': '發行與廠牌',
  'albumDetail.credit.role.lyrics': '作詞',
  'albumDetail.credit.role.other': '其他貢獻',
  'albumDetail.credit.role.performer': '演奏',
  'albumDetail.credit.role.production': '製作',
  'albumDetail.credit.role.vocal': '演唱與聲部',
  'albumDetail.credit.source.album': '專輯貢獻',
  'albumDetail.credit.source.label': '廠牌',
  'albumDetail.credit.source.recording': '曲目貢獻',
  'albumDetail.credit.source.work': '作品貢獻',
  'albumDetail.credit.summary.arrangement': '編曲、配器和改編相關貢獻。',
  'albumDetail.credit.summary.composer': '來自發行、錄音或作品關係的音樂創作資訊。',
  'albumDetail.credit.summary.engineering': '錄音、混音、母帶和聲音工程資訊。',
  'albumDetail.credit.summary.label': '與發行相關的廠牌和目錄資訊。',
  'albumDetail.credit.summary.lyrics': '歌詞、文字、腳本和相關寫作貢獻。',
  'albumDetail.credit.summary.other': '線上中繼資料匹配到的其他貢獻。',
  'albumDetail.credit.summary.performer': '發行或單曲錄音中的演奏與表演貢獻。',
  'albumDetail.credit.summary.production': '製作人和製作側貢獻。',
  'albumDetail.credit.summary.vocal': '主唱、客座聲音和演唱相關貢獻。',
  'albumDetail.credits.count': '{count} 位貢獻者 / 組織',
  'albumDetail.credits.entries': '{count} 條',
  'albumDetail.credits.heading': '專輯貢獻',
  'albumDetail.credits.overviewAria': '貢獻概覽',
  'albumDetail.credits.trackPrefix': '曲目：{title}',
  'albumDetail.duration.hours': '{hours} 小時 {minutes} 分鐘',
  'albumDetail.duration.minutes': '{minutes} 分鐘',
  'albumDetail.fact.format': '格式',
  'albumDetail.fact.genre': '曲風',
  'albumDetail.fact.library': '曲庫',
  'albumDetail.fact.rating': '評分',
  'albumDetail.fact.released': '發行',
  'albumDetail.information.albumProfile': '專輯資料',
  'albumDetail.information.artistProfile': '藝術家資料',
  'albumDetail.information.atGlance': '概覽',
  'albumDetail.information.externalLinks': '外部連結',
  'albumDetail.information.overviewAria': '專輯與藝術家概覽',
  'albumDetail.label.album': '專輯',
  'albumDetail.online.emptyDescription': 'MusicBrainz 和 Wikipedia 沒有回傳足夠可靠的專輯匹配資料。',
  'albumDetail.online.emptyTitle': '沒有找到可靠的線上資訊',
  'albumDetail.online.match': 'MusicBrainz 匹配',
  'albumDetail.online.noSource': '沒有匹配來源',
  'albumDetail.online.possibleMatch': '可能的 MusicBrainz 匹配',
  'albumDetail.online.reading': '正在讀取線上專輯資訊...',
  'albumDetail.online.sources': '線上來源',
  'albumDetail.online.unavailable': '線上資訊不可用',
  'albumDetail.ratings.count': '{count} 個評分',
  'albumDetail.ratings.overviewAria': '外部專輯評分',
  'albumDetail.releases.count': '{count} 個發行版本',
  'albumDetail.releases.current': '目前匹配',
  'albumDetail.releases.currentHint': '標記目前本地專輯匹配到的 MusicBrainz 版本',
  'albumDetail.releases.heading': '版本 / 發行',
  'albumDetail.releases.overviewAria': '專輯發行版本概覽',
  'albumDetail.related.aria': '{artist} 在曲庫中的專輯',
  'albumDetail.related.heading': '我的曲庫',
  'albumDetail.related.loading': '正在載入專輯',
  'albumDetail.related.thisAlbum': '目前專輯',
  'albumDetail.sources.barcode': '條碼',
  'albumDetail.sources.catalogNumber': '目錄號',
  'albumDetail.sources.copyright': '版權資訊',
  'albumDetail.sources.kind.database': '資料庫',
  'albumDetail.sources.kind.official': '官方',
  'albumDetail.sources.kind.other': '網頁',
  'albumDetail.sources.kind.reference': '資料',
  'albumDetail.sources.kind.streaming': '串流',
  'albumDetail.sources.labels': '廠牌 / 目錄',
  'albumDetail.sources.linksAria': '專輯外部來源連結',
  'albumDetail.sources.releaseAria': '目前匹配發行資訊',
  'albumDetail.sources.releaseDetails': '目前發行',
  'albumDetail.status.addedToQueue': '已加入佇列 {count} 首。',
  'albumDetail.status.copiedCover': '已複製封面原圖',
  'albumDetail.status.libraryReady': '{value} 就緒',
  'albumDetail.status.readingSignal': '正在讀取訊號',
  'albumDetail.status.unknownGenre': '未知曲風',
  'albumDetail.status.unknownLength': '未知時長',
  'albumDetail.status.unknownYear': '未知年份',
  'albumDetail.tab.credits': '貢獻',
  'albumDetail.tab.information': '資訊',
  'albumDetail.tab.releases': '版本',
  'albumDetail.tab.sources': '來源',
  'albumDetail.tab.tracks': '曲目',
  'albumDetail.texture.discs': '{count} 張碟',
  'albumDetail.tracks.action.like': '喜歡 {title}',
  'albumDetail.tracks.action.likeTitle': '喜歡',
  'albumDetail.tracks.action.unlike': '取消喜歡 {title}',
  'albumDetail.tracks.action.unlikeTitle': '取消喜歡',
  'albumDetail.tracks.aria': '專輯曲目',
  'albumDetail.tracks.column.signal': '訊號',
  'albumDetail.tracks.column.time': '時長',
  'albumDetail.tracks.column.title': '標題',
  'albumDetail.tracks.confirm.delete': '刪除音樂檔案？\n{title}',
  'albumDetail.tracks.disc': 'DISC {number}',
  'albumDetail.tracks.discUnknown': 'DISC ?',
  'albumDetail.tracks.empty': '這張專輯沒有曲目。',
  'albumDetail.tracks.error.actionUnavailable': '這個曲目操作暫不可用。',
  'albumDetail.tracks.error.desktopBridgeActions': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'albumDetail.tracks.error.desktopBridgeEdit': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'albumDetail.tracks.error.desktopBridgeRead': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'albumDetail.tracks.error.noCoverSaved': '沒有儲存任何封面。',
  'albumDetail.tracks.error.noCoverToCopy': '這首歌沒有可複製的封面。',
  'albumDetail.tracks.error.remoteFileAction': '遠端曲目暫不支援本機檔案操作。',
  'albumDetail.tracks.formatAria': '曲目格式',
  'albumDetail.tracks.loadMore': '載入更多',
  'albumDetail.tracks.loading': '載入中...',
  'albumDetail.tracks.sort.aria': '曲目排序',
  'albumDetail.tracks.sort.default': '碟片 / 音軌號',
  'albumDetail.tracks.sort.filename': '檔案名稱',
  'albumDetail.tracks.status.addedToPlaylist': '已加入歌單：{playlist}',
  'albumDetail.tracks.status.albumNotFound': '已經在查看這張專輯：{title}',
  'albumDetail.tracks.status.notInQueue': '佇列中沒有這首歌：{title}',
  'albumDetail.tracks.status.reloadedTags': '已從內嵌標籤重新載入：{title}',
  'albumDetail.tracks.status.removedFromQueue': '已從佇列移除：{title}',
  'albumDetail.tracks.summaryAria': '曲目摘要',
  'library.albums.card.tracks': '{count} 首歌',
  'library.albums.confirm.deleteAlbumFiles': '刪除專輯檔案？\n{title}\n\n這會把 {count} 首歌曲移到系統回收站，並從媒體庫移除。',
  'library.albums.error.clipboardUnsupported': '目前執行環境不支援寫入剪貼簿。',
  'library.albums.error.coverNotSaved': '沒有儲存專輯封面。',
  'library.albums.error.desktopBridge': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'library.albums.error.noCopyableCover': '這張專輯沒有可複製的封面。',
  'library.albums.error.noPlayableTracks': '這張專輯沒有可播放的歌曲。',
  'library.albums.error.remoteEditUnsupported': '遠端專輯暫不支援編輯標籤或刪除伺服器檔案。',
  'library.albums.listAria': '專輯列表',
  'library.albums.loading': '正在載入專輯...',
  'library.albums.message.addedPlayNext': '已加入下一首：{count} 首',
  'library.albums.message.copiedToClipboard': '已複製到剪貼簿。',
  'library.albums.searchPlaceholder': '搜尋專輯 / 藝術家',
  'library.albums.sort.aria': '專輯排序',
  'library.albums.sort.artist': '藝術家',
  'library.albums.sort.group.general': '一般',
  'library.albums.sort.group.library': '曲庫',
  'library.albums.sort.group.listening': '聆聽',
  'library.albums.sort.lastPlayed': '最近播放',
  'library.albums.sort.playCountAsc': '播放次數最少',
  'library.albums.sort.playCountDesc': '播放次數最多',
  'library.albums.sort.recentAdded': '最近加入',
  'library.albums.sort.trackCountAsc': '曲目數量最少',
  'library.albums.sort.trackCountDesc': '曲目數量最多',
  'library.albums.sort.titleAsc': '標題 A-Z',
  'library.albums.sort.titleDesc': '標題 Z-A',
  'library.albums.sort.yearAsc': '發行年份最早',
  'library.albums.sort.yearDesc': '發行年份最新',
  'library.albums.title': '專輯',
  'libraryDiagnostics.lab.description': '這些功能用於開發測試即時媒體庫行為。預設關閉，不會影響一般使用者。請只在測試分支或測試曲庫中使用。',
  'library.artists.error.desktopBridge': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'library.artists.avatarPriority': '頭像優先',
  'library.artists.grouping.albumArtist': '專輯演出者',
  'library.artists.grouping.albumArtistHint': '不走合作藝人拆分，按歌曲原始署名整段顯示',
  'library.artists.listAria': '藝術家列表',
  'library.artists.loading': '正在載入藝術家...',
  'library.artists.meta.albums': '{count} 張專輯',
  'library.artists.meta.noTracks': '暫無歌曲',
  'library.artists.meta.tracks': '{count} 首歌',
  'library.artists.searchPlaceholder': '搜尋藝術家',
  'library.artists.sort.aria': '藝術家排序',
  'library.artists.sort.albumCountDesc': '專輯最多',
  'library.artists.sort.frequent': '歌曲最多',
  'library.artists.sort.group.view': '顯示',
  'library.artists.sort.nameAsc': '名稱 A-Z',
  'library.artists.sort.nameDesc': '名稱 Z-A',
  'library.artists.title': '藝術家',
  'library.genres.error.desktopBridge': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'library.genres.empty': '這個曲庫裡還沒有類型標籤。',
  'library.genres.listAria': '類型列表',
  'library.genres.loading': '正在載入類型...',
  'library.genres.meta.albums': '{count} 張專輯',
  'library.genres.meta.noTracks': '暫無歌曲',
  'library.genres.meta.tracks': '{count} 首歌',
  'library.genres.searchPlaceholder': '搜尋類型',
  'library.genres.sort.aria': '類型排序',
  'library.genres.sort.frequent': '歌曲最多',
  'library.genres.sort.nameAsc': '名稱 A-Z',
  'library.genres.sort.nameDesc': '名稱 Z-A',
  'library.genres.title': '類型',
  'library.genres.unclassified': '未分類',
  'genreDetail.action.addToQueue': '加入佇列',
  'genreDetail.action.back': '類型',
  'genreDetail.action.play': '立即播放',
  'genreDetail.action.shuffle': '隨機播放',
  'genreDetail.albums.aria': '{genre} 的專輯',
  'genreDetail.albums.count': '{count} 張專輯',
  'genreDetail.albums.empty': '這個類型還沒有歸檔專輯。',
  'genreDetail.albums.error.desktopBridge': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'genreDetail.albums.heading': '{genre} 的專輯',
  'genreDetail.albums.loadedCount': '已載入 {loaded}/{total} 張專輯',
  'genreDetail.aria.details': '{genre} 類型詳情',
  'genreDetail.aria.mosaic': '{genre} 的專輯封面',
  'genreDetail.error.desktopBridgeRead': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'genreDetail.label.genre': '類型',
  'genreDetail.meta.albums': '{count} 張專輯',
  'genreDetail.meta.tracks': '{count} 首歌',
  'genreDetail.tab.albums': '專輯',
  'genreDetail.tab.tracks': '歌曲',
  'genreDetail.tracks.column.album': '專輯',
  'genreDetail.tracks.empty': '這個類型還沒有歌曲。',
  'genreDetail.tracks.heading': '{genre} 的歌曲',
  'genreDetail.tracks.loading': '正在載入歌曲...',
  'genreDetail.tracks.unknownAlbum': '未知專輯',
  'artistDetail.action.addToQueue': '加入佇列',
  'artistDetail.action.back': '藝術家',
  'artistDetail.action.playArtist': '播放藝術家',
  'artistDetail.action.readingArtist': '正在讀取',
  'artistDetail.action.refreshInfo': '重新整理資訊',
  'artistDetail.action.shuffle': '隨機播放',
  'artistDetail.albums.aria': '{artist} 的專輯',
  'artistDetail.albums.count': '{count} 張專輯',
  'artistDetail.albums.empty': '這位藝術家尚未歸檔專輯。',
  'artistDetail.albums.error.desktopBridge': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'artistDetail.albums.heading': '{artist} 的專輯',
  'artistDetail.albums.loadedCount': '已載入 {loaded}/{total} 張專輯',
  'artistDetail.albums.showAll': '顯示所有專輯',
  'artistDetail.albums.showAllLoading': '正在顯示...',
  'artistDetail.aroundWeb.aria': '藝術家官網和社群媒體',
  'artistDetail.aroundWeb.heading': '網路上',
  'artistDetail.aria.details': '{artist} 藝術家詳情',
  'artistDetail.aria.events': '藝術家演出',
  'artistDetail.aria.facts': '藝術家資料',
  'artistDetail.aria.metadata': '藝術家中繼資料',
  'artistDetail.aria.onlineSources': '線上藝術家來源',
  'artistDetail.aria.overview': '藝術家概覽',
  'artistDetail.aria.relationshipMap': '藝人關係',
  'artistDetail.aria.sections': '{artist} 詳情分區',
  'artistDetail.constellation.collapse': '收起星圖',
  'artistDetail.constellation.collapseAria': '收起 {artist} 的藝人星圖',
  'artistDetail.constellation.empty': '暫時沒有更多本地關係',
  'artistDetail.constellation.error': '星圖讀取失敗：{message}',
  'artistDetail.constellation.expand': '展開星圖',
  'artistDetail.constellation.expandAria': '展開 {artist} 的藝人星圖',
  'artistDetail.constellation.loading': '正在讀取星圖...',
  'artistDetail.duration.hours': '已載入 {hours} 小時 {minutes} 分鐘',
  'artistDetail.duration.minutes': '已載入 {minutes} 分鐘',
  'artistDetail.duration.reading': '正在讀取時長',
  'artistDetail.empty.relationships': '暫未找到本地一跳關係。',
  'artistDetail.error.desktopBridgeRead': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'artistDetail.events.configureProviders': '可配置 Bandsintown、Ticketmaster 或 SeatGeek 金鑰以擴大演出涵蓋範圍；ECHO 仍會嘗試可用的免金鑰來源。',
  'artistDetail.events.collapse': '收起',
  'artistDetail.events.collapsedHint': '已找到 {count} 場演出，展開後查看日期、場館和票務入口。',
  'artistDetail.events.count': '{count} 場演出',
  'artistDetail.events.expand': '展開',
  'artistDetail.events.noConcerts': '暫未匹配到近期演出。',
  'artistDetail.events.noConcertsRegion': '暫未在 {region} 匹配到近期演出。',
  'artistDetail.events.openDetails': '開啟演出詳情',
  'artistDetail.events.providerKeysRequired': '需要配置來源金鑰',
  'artistDetail.events.timePending': '時間待公佈',
  'artistDetail.events.unavailable': '演出資訊暫時無法使用，請稍後重試。',
  'artistDetail.events.venuePending': '場地待公布',
  'artistDetail.fact.albums': '專輯',
  'artistDetail.fact.duration': '時長',
  'artistDetail.fact.loaded': '已載入',
  'artistDetail.fact.sources': '來源',
  'artistDetail.fact.tracks': '歌曲',
  'artistDetail.label.artist': '藝術家',
  'artistDetail.label.overview': '概覽',
  'artistDetail.meta.albums': '{count} 張專輯',
  'artistDetail.meta.loadedTracks': '已載入 {loaded}/{total}',
  'artistDetail.meta.tracks': '{count} 首歌',
  'artistDetail.missing.description': '返回藝術家頁並重新整理曲庫，即可查看最新目錄。',
  'artistDetail.missing.title': '藝術家不存在或已從曲庫移除。',
  'artistDetail.overview.about': '關於 {artist}',
  'artistDetail.overview.bioFallback': '來自你的本地曲庫。線上藝術家資訊會在背景輕量讀取。',
  'artistDetail.relation.bpm': 'BPM',
  'artistDetail.relation.collaboration': '合作',
  'artistDetail.relation.evidence': '{label} / {evidence}',
  'artistDetail.relation.genre': '流派',
  'artistDetail.relation.history': '播放歷史',
  'artistDetail.relation.link': '連結',
  'artistDetail.relation.local': '本地曲庫訊號',
  'artistDetail.relation.member': '成員',
  'artistDetail.relation.sameAlbum': '同專輯',
  'artistDetail.relation.similar': '相似',
  'artistDetail.section.concertInfo': '演出資訊',
  'artistDetail.section.events': '演出',
  'artistDetail.section.localNetwork': '本地網路',
  'artistDetail.section.relationshipMap': '藝人關係',
  'artistDetail.status.collectedLocally': '本地收藏',
  'artistDetail.status.linkedArtists': '{count} 位關聯藝術家',
  'artistDetail.status.loadingSignals': '正在讀取本地訊號',
  'artistDetail.status.localLibrary': '本地曲庫',
  'artistDetail.status.readingRelationships': '正在讀取藝術家關係...',
  'artistDetail.status.readySoon': '即將就緒',
  'artistDetail.tab.albums': '專輯',
  'artistDetail.tab.overview': '概覽',
  'artistDetail.tab.songs': '歌曲',
  'artistDetail.tracks.action.addToQueueAria': '將 {title} 加入佇列',
  'artistDetail.tracks.action.more': '更多',
  'artistDetail.tracks.action.moreAria': '{title} 的更多操作',
  'artistDetail.tracks.action.playNext': '下一首播放',
  'artistDetail.tracks.action.playNextAria': '下一首播放 {title}',
  'artistDetail.tracks.aria': '{artist} 的歌曲',
  'artistDetail.tracks.column.actions': '操作',
  'artistDetail.tracks.column.album': '專輯',
  'artistDetail.tracks.column.signal': '訊號',
  'artistDetail.tracks.column.time': '時長',
  'artistDetail.tracks.column.title': '標題',
  'artistDetail.tracks.confirm.delete': '刪除這個音樂檔案？\n{title}',
  'artistDetail.tracks.empty': '這位藝術家尚未歸檔歌曲。',
  'artistDetail.tracks.error.actionUnavailable': '這個歌曲操作暫不可用。',
  'artistDetail.tracks.error.desktopBridgeActions': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'artistDetail.tracks.error.desktopBridgeEdit': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'artistDetail.tracks.error.desktopBridgeRead': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'artistDetail.tracks.expandAll': '全部展開',
  'artistDetail.tracks.error.noCoverSaved': '沒有儲存任何封面圖。',
  'artistDetail.tracks.error.noCoverToCopy': '這首歌沒有可複製的封面圖。',
  'artistDetail.tracks.error.remoteFileAction': '遠端歌曲暫不支援本地檔案操作。',
  'artistDetail.tracks.formatAria': '歌曲格式',
  'artistDetail.tracks.heading': '{artist} 的歌曲',
  'artistDetail.tracks.loadedCount': '已載入 {loaded}/{total} 首歌',
  'artistDetail.tracks.loading': '正在載入歌曲...',
  'artistDetail.tracks.loadingTrack': '正在載入歌曲',
  'artistDetail.tracks.status.addedToPlaylist': '已加入歌單：{playlist}',
  'artistDetail.tracks.status.albumNotFound': '此藝術家視圖中找不到專輯：{album}',
  'artistDetail.tracks.status.notInQueue': '播放佇列中沒有這首歌：{title}',
  'artistDetail.tracks.status.reloadedTags': '已從內嵌標籤重新載入：{title}',
  'artistDetail.tracks.status.removedFromQueue': '已從播放佇列移除：{title}',
  'artistDetail.tracks.unknownAlbum': '未知專輯',
  'library.count.total': '共 {count} 個',
  'library.sort.createdAsc': '建立最早',
  'library.sort.createdDesc': '建立最新',
  'library.sort.default': '預設',
  'library.sort.durationAsc': '時長最短',
  'library.sort.durationDesc': '時長最長',
  'library.sort.fileModifiedAsc': '檔案修改最早',
  'library.sort.fileModifiedDesc': '檔案修改最新',
  'library.sort.random': '隨機',
  'library.sort.recent': '最近',
  'library.source.aria': '媒體庫來源',
  'library.source.local': '本機',
  'library.source.allRemote': '全部網路硬碟來源',
  'library.source.remote': '網路硬碟',
  'library.trackRow.action.addToPlaylist': '加入歌單',
  'library.trackRow.action.addToPlaylistLabel': '加入歌單 {title}',
  'library.trackRow.action.addToQueue': '加入佇列',
  'library.trackRow.action.addToQueueLabel': '加入佇列 {title}',
  'library.trackRow.action.download': '下載',
  'library.trackRow.action.downloadLabel': '下載 {title}',
  'library.trackRow.action.downloading': '下載中 {percent}%',
  'library.trackRow.action.downloadingLabel': '正在下載 {title} {percent}%',
  'library.trackRow.action.more': '更多',
  'library.trackRow.action.moreLabel': '更多 {title}',
  'library.trackRow.actions': '{title} 操作',
  'library.trackRow.audioSpecifications': '音訊規格',
  'library.trackRow.duplicateVersions.count': '有 {count} 個版本',
  'library.trackRow.duplicateVersions.title': '查看重複歌曲版本',
  'library.trackRow.openAlbum': '打開專輯：{album}',
  'library.trackRow.openArtist': '打開演出者：{artist}',
  'library.trackRow.status.playing': '播放中',
  'library.trackRow.status.unavailable': '不可用',
  'audioDrawer.action.close': '關閉音訊設定',
  'audioDrawer.action.copiedDiagnostics': '已複製播放診斷資訊',
  'audioDrawer.action.copyDiagnostics': '複製播放診斷資訊',
  'audioDrawer.action.hideDevice': '隱藏裝置',
  'audioDrawer.action.openAsioPanel': '開啟 ASIO 面板',
  'audioDrawer.action.resetEngine': '重置音訊引擎',
  'audioDrawer.action.resetEngineBusy': '正在重置音訊引擎',
  'audioDrawer.action.resetEngineDone': '音訊引擎已重置',
  'audioDrawer.action.restore': '復原',
  'audioDrawer.asioLatency.description': '根據目前開啟的緩衝估算輸出端會多等多久才播放。數值越低越即時，數值越高越穩、越不容易爆音。',
  'audioDrawer.asioLatency.recommended': '建議延遲',
  'audioDrawer.asioLatency.status': '要求 {requested} frames / 已開啟 {opened} frames',
  'audioDrawer.asioLatency.value': '{value} ms',
  'audioDrawer.asioRoutes.title': 'ASIO 輸出通道',
  'audioDrawer.badge.bitPerfectReady': 'Bit-perfect ready',
  'audioDrawer.badge.dspActive': 'DSP active',
  'audioDrawer.badge.resampling': 'Resampling',
  'audioDrawer.badge.soxrResampler': 'SOXR',
  'audioDrawer.badge.speedUp': 'Speed Up',
  'audioDrawer.badge.upsampling': '升頻',
  'audioDrawer.buffer.asio': 'ASIO 緩衝',
  'audioDrawer.buffer.auto': '自動',
  'audioDrawer.buffer.collapsedDescription': '預設收合；點開可調整延遲檔位與 ASIO 緩衝。',
  'audioDrawer.buffer.default': '預設',
  'audioDrawer.buffer.latencyProfile': '延遲檔位',
  'audioDrawer.buffer.low': '低',
  'audioDrawer.buffer.profileDefault': '跟隨檔位預設',
  'audioDrawer.buffer.safer': '更穩',
  'audioDrawer.buffer.stable': '穩定',
  'audioDrawer.buffer.title': '緩衝設定',
  'audioDrawer.buffer.ultraLow': '超低延遲',
  'audioDrawer.device.asioDriver': 'ASIO 驅動',
  'audioDrawer.device.lowLatency': '低延遲',
  'audioDrawer.device.selected': '已選取',
  'audioDrawer.device.systemAudio': '安全模式',
  'audioDrawer.device.systemAudioDescription': '系統相容輸出，僅在正常輸出異常時使用',
  'audioDrawer.device.systemDefault': '系統預設',
  'audioDrawer.device.systemDefaultOutput': '系統預設輸出',
  'audioDrawer.device.systemOutput': '系統輸出',
  'audioDrawer.device.systemSelectedRoute': '系統選取路徑',
  'audioDrawer.deviceRefresh.failed': '重新整理失敗，請再試一次',
  'audioDrawer.deviceRefresh.refreshing': '正在掃描所有音訊裝置…',
  'audioDrawer.deviceRefresh.success': '已重新整理 {count} 個裝置 · {time}',
  'audioDrawer.empty.asioDevices': '找不到 ASIO 輸出裝置。',
  'audioDrawer.empty.hiddenDevices': '沒有隱藏裝置。',
  'audioDrawer.empty.systemDevices': '找不到系統輸出裝置。',
  'audioDrawer.error.desktopBridgeUnavailable': '目前無法完成此操作',
  'audioDrawer.meter.direct': '直通',
  'audioDrawer.meter.chain': '鏈路',
  'audioDrawer.meter.mode': '模式',
  'audioDrawer.meter.output': '輸出',
  'audioDrawer.meter.rate': '取樣率',
  'audioDrawer.meter.resample': '重取樣',
  'audioDrawer.meter.source': '音源',
  'audioDrawer.meter.latency': '延遲',
  'audioDrawer.meter.nativeDirect': '本機直讀',
  'audioDrawer.meter.upsample': '升頻',
  'audioDrawer.guard.asioUnavailable.description': '預設關閉。遇到 No device found 後會短暫跳過同一個 ASIO 裝置，並改用安全的共享輸出。',
  'audioDrawer.guard.asioUnavailable.title': 'ASIO 不可用保護',
  'audioDrawer.guard.exclusiveInstability.description': '預設關閉。WASAPI 獨占持續 underrun 或裝置變得不穩定時，開啟後會從目前位置切到安全共享輸出。',
  'audioDrawer.guard.exclusiveInstability.title': '獨占不穩定自動切共享',
  'audioDrawer.guard.soxrFallback.description': '預設開啟。如果共享 SOXR 重取樣在 PCM 開始前不可用，會退回到 FFmpeg 預設重取樣。',
  'audioDrawer.guard.soxrFallback.title': 'SOXR 退回保護',
  'audioDrawer.latency.balanced': '均衡',
  'audioDrawer.latency.balancedDetail': '2048 frames',
  'audioDrawer.latency.lowLatency': '低延遲',
  'audioDrawer.latency.lowLatencyDetail': '1024 frames / 不穩升均衡',
  'audioDrawer.latency.stable': '穩定',
  'audioDrawer.latency.stableDetail': '8192 frames',
  'audioDrawer.mode.exclusive': '獨佔',
  'audioDrawer.mode.exclusiveCandidate': '獨佔候選',
  'audioDrawer.mode.directSound': 'DirectSound 相容',
  'audioDrawer.mode.shared': '共享',
  'audioDrawer.note.asio': '科普：WASAPI Exclusive 和 ASIO 在音質上並無差別；如果 ASIO 不穩定，請使用 WASAPI Exclusive。',
  'audioDrawer.note.currentOutput': '這裡顯示現在真正使用的輸出路徑；共享適合日常，ASIO 和 WASAPI 獨佔會以金色標出。',
  'audioDrawer.note.engine': '這裡快速查看輸出裝置、模式、取樣率、EQ 和重取樣狀態。',
  'audioDrawer.note.miniaudioOutput': '實驗功能，預設關閉。只影響 Windows 共享輸出的 PCM；獨佔、ASIO、DoP 和原生 DSD 不受影響。',
'audioDrawer.note.nativeDirectLocalPlayback': '預設關閉。共享輸出下直接讀取本機 WAV/FLAC/MP3，可縮短起播和跳轉等待；支援 EQ、播放速度和音量，不適用或啟動失敗時會自動回到穩定播放。',
  'audioDrawer.note.dsdDop': '預設開啟。本機 DSF 在 ASIO 或 WASAPI 獨占下使用 DoP，不相容時退回 PCM；ASIO 原生 DSD 需另行開啟實驗選項。最終以 DAC 顯示為準。',
  'audioDrawer.note.asioNativeDsd': '預設關閉。僅 ASIO + 本機 DSF + DoP 開啟且無 EQ/音量/變速/DSP 時嘗試；失敗會退回現有 DoP/PCM。',
  'audioDrawer.note.dsdAutoVolumeLock': '預設關閉。開啟後播放 DSD 時暫時鎖定 ECHO 音量為 100%，切回 PCM 後恢復到原本音量。',
  'audioDrawer.note.releaseExclusiveOnPause': '實驗功能。暫停時釋放 WASAPI 獨占，讓其他軟體暫時出聲；恢復播放會重新搶獨占，失敗時暫時降到共享。',
  'audioDrawer.option.miniaudioOutput': 'miniaudio WASAPI 輸出實驗',
  'audioDrawer.option.nativeDirectLocalPlayback': '本機直讀播放實驗',
  'audioDrawer.option.dsdDop': 'DSD 直通（DoP）',
  'audioDrawer.option.asioNativeDsd': 'ASIO 原生 DSD 實驗',
  'audioDrawer.option.dsdAutoVolumeLock': '播放 DSD 時自動鎖定音量',
  'audioDrawer.option.releaseExclusiveOnPause': '暫停釋放獨占實驗',
  'audioDrawer.option.active': '開啟',
  'audioDrawer.option.set': '設定',
  'audioDrawer.option.automix': '啟用智慧過渡',
  'audioDrawer.option.automixActive': '目前播放已進入智慧過渡。',
  'audioDrawer.option.automixDescription': 'Beta 功能，預設關閉。開啟後會依節拍、調性、樂句和能量自動銜接目前歌曲與下一首；不支援的場景會安全降級。',
  'audioDrawer.option.automaticOutput': '自動適配輸出（推薦新手）',
  'audioDrawer.option.automaticOutputDescription': '預設關閉。開啟後由音訊引擎跟隨系統預設裝置，使用共享模式與平衡延遲；裝置中斷或預設裝置變更時自動安全復原。開啟期間手動裝置與獨佔設定會鎖定。',
  'audioDrawer.option.automaticOutputStage.safeShared': 'WASAPI 已切換到穩定緩衝，目前仍由系統預設裝置輸出。',
  'audioDrawer.option.automaticOutputStage.directSound': 'WASAPI 無法啟動，已自動使用 DirectSound 相容輸出。',
  'audioDrawer.option.automaticOutputStage.systemRequired': '原生相容輸出仍然失敗，請切換到 System Audio 安全模式。',
  'audioDrawer.option.automaticOutputStage.failed': '自動復原已停止，以免掩蓋原生引擎錯誤；請查看音訊診斷。',
  'audioDrawer.option.rememberOutput': '儲存輸出設定',
  'audioDrawer.option.rememberOutputDescription': '下次啟動時復原所選輸出裝置、輸出模式與緩衝等參數。',
  'audioDrawer.option.fixedVolume': '固定音量',
  'audioDrawer.option.fixedVolumeDescription': '開啟後會將 ECHO 音量控制鎖定為 100%；ReplayGain 仍會獨立生效。',
  'audioDrawer.option.lowLoadPlaybackMode': '減輕占用播放',
  'audioDrawer.option.lowLoadPlaybackModeDescription': '建議大多數使用者開啟。打開後播放時會關掉即時頻譜、減少頁面重新整理、暫停 ReplayGain/BPM 重分析、降低歌詞刷新，並暫緩封面、藝人圖和 MV 預載。',
  'audioDrawer.option.lowLoadPlaybackEnhancements': '進一步減輕占用',
  'audioDrawer.option.lowLoadPlaybackEnhancementsDescription': '預設關閉。僅在「減輕占用播放」開啟時生效，會再減少狀態輪詢、桌面歌詞、診斷、後台曲庫任務、串流預熱和 MV 自動搜尋。',
  'audioDrawer.option.showAsioPanelSettings': '是否顯示 ASIO 面板設定',
  'audioDrawer.option.showAsioPanelSettingsDescription': '預設關閉。開啟後才會在 ASIO 裝置下顯示「開啟 ASIO 面板」按鈕。',
  'audioDrawer.option.alsaShared': 'ALSA',
  'audioDrawer.option.alsaSharedDescription': '透過 Linux ALSA 裝置輸出。',
  'audioDrawer.option.directSound': 'DirectSound 相容',
  'audioDrawer.option.directSoundDescription': '手動相容模式，延遲較大；只在 WASAPI 播放異常時嘗試。',
  'audioDrawer.option.linuxAutoShared': '自動',
  'audioDrawer.option.linuxAutoSharedDescription': '優先使用 ALSA，並尊重系統的 PipeWire/ALSA 相容層設定。',
  'audioDrawer.option.sharedBackend': '共享後端',
  'audioDrawer.option.wasapiShared': 'WASAPI Shared',
  'audioDrawer.option.wasapiSharedDescription': '日常 Windows 共享輸出路徑。',
  'audioDrawer.option.wasapiExclusive': 'WASAPI 獨佔模式',
  'audioDrawer.option.wasapiExclusiveDescription': '日常請用共享輸出。獨佔會略過系統混音器，只建議外接 DAC 或穩定驅動使用；板載 Realtek 較容易無聲、卡頓或切換失敗。',
  'audioDrawer.resetAll.action': '重置所有音訊設定',
  'audioDrawer.resetAll.busy': '正在重置音訊設定',
  'audioDrawer.resetAll.description': '復原共享輸出、預設緩衝、音訊引擎開關、減輕占用播放、固定音量和隱藏裝置列表。',
  'audioDrawer.resetAll.done': '音訊設定已重置',
  'audioDrawer.resetAll.title': '重置音訊設定',
  'audioDrawer.section.advancedOutput': '進階音訊設定',
  'audioDrawer.section.advancedOutputDescription': '適合外接音效卡、WASAPI Exclusive、ASIO 和 HiFi 調試',
  'audioDrawer.advancedGroup.outputPath.title': '輸出後端',
  'audioDrawer.advancedGroup.outputPath.description': 'JUCE / miniaudio 輸出路徑',
  'audioDrawer.advancedGroup.decodePath.title': '解碼與直讀',
  'audioDrawer.advancedGroup.decodePath.description': '原生解碼、本機直讀實驗',
  'audioDrawer.advancedGroup.dsdTransport.title': 'DSD 直出',
  'audioDrawer.advancedGroup.dsdTransport.description': 'DoP、原生 DSD 與音量鎖定',
  'audioDrawer.advancedGroup.protection.title': '穩定性保護',
  'audioDrawer.advancedGroup.protection.description': 'ASIO、獨占與重新取樣回退',
  'audioDrawer.advancedGroup.bufferAndDevice.title': '緩衝與裝置',
  'audioDrawer.advancedGroup.bufferAndDevice.description': '共享後端、延遲檔位與 ASIO 緩衝',
  'audioDrawer.advancedGroup.behavior.title': '一般行為',
  'audioDrawer.advancedGroup.behavior.description': '輸出記憶、固定音量與引擎維護',
  'audioDrawer.section.automix': '智慧過渡',
  'audioDrawer.section.asioDevices': 'ASIO 輸出裝置',
  'audioDrawer.section.currentOutput': '目前輸出',
  'audioDrawer.section.hiddenDevices': '隱藏裝置',
  'audioDrawer.section.systemDevices': '推薦輸出',
  'audioDrawer.signal.balanceDsp': 'Balance DSP',
  'audioDrawer.signal.bitPerfect': 'Bit-perfect',
  'audioDrawer.signal.dspOn': 'DSP On',
  'audioDrawer.signal.eqOff': 'EQ Off',
  'audioDrawer.signal.eqOn': 'EQ On',
  'audioDrawer.signal.asioSdkOutput': 'ASIO SDK 輸出',
  'audioDrawer.signal.ffmpegDecode': 'FFmpeg 解碼',
  'audioDrawer.signal.dsdDop': 'DSF 碼流 → DoP',
  'audioDrawer.signal.dsdDopFallback': 'DSD DoP 已降級',
  'audioDrawer.signal.dsdDopStandby': 'DoP 未適用',
  'audioDrawer.signal.miniaudioOutput': 'miniaudio WASAPI',
  'audioDrawer.signal.nativeDirectPlayback': '本機直讀',
  'audioDrawer.signal.nativeDirectPlaybackStandby': '本機直讀未適用',
  'audioDrawer.signal.nativeDirectPlaybackOff': '關閉',
  'audioDrawer.signal.nativeDirectPlaybackPending': '已開啟，等待下次播放套用',
  'audioDrawer.signal.nativeRate': '原生取樣率',
  'audioDrawer.signal.noActiveSource': '沒有作用中的音源',
  'audioDrawer.signal.pending': '等待中',
  'audioDrawer.signal.processed': '已處理',
  'audioDrawer.signal.sharedMixer': '共享混音器',
  'audioDrawer.signal.standardPath': '標準路徑',
  'audioDrawer.status.noTrack': '沒有曲目',
  'audioDrawer.status.ratePending': '取樣率待定',
  'audioDrawer.status.sampleRatePending': '取樣率待定',
  'audioDrawer.title': '音訊設定',
  'drawerSearch.clear': '清空搜尋',
  'drawerSearch.label': '搜尋側欄內容',
  'drawerSearch.next': '下一項',
  'drawerSearch.noResults': '沒有符合項目',
  'drawerSearch.placeholder': '搜尋設定',
  'drawerSearch.previous': '上一項',
  'drawerSearch.resultCount': '找到 {count} 項',
  'drawerSearch.resultLabel': '跳到 {result}',
  'drawerSearch.shortcutHint': 'Ctrl+F 聚焦，Enter 跳轉',
  'audioDrawer.warning.highOutputSampleRate': '目前音訊裝置取樣率過高，可能導致播放速度異常，建議改為 48 kHz。',
  'audioSignalPath.closeLabel': '關閉訊號路徑',
  'audioSignalPath.closeTitle': '關閉',
  'audioSignalPath.control.openLabel': '開啟音訊鏈路：{label}，{spec}',
  'audioSignalPath.decode.auto': '自動解碼',
  'audioSignalPath.decode.keepRate': '保持 {rate}',
  'audioSignalPath.decode.nativeDirect': '本機直讀',
  'audioSignalPath.decode.originalRate': '原取樣率',
  'audioSignalPath.decode.outputRate': '輸出取樣率',
  'audioSignalPath.decode.resamplingTo': '重取樣到 {rate}',
  'audioSignalPath.dsp.channelBalance': '聲道平衡',
  'audioSignalPath.dsp.roomCorrectionModule': 'FIR 房間校正',
  'audioSignalPath.dsp.safetyLimiter': '安全限幅',
  'audioSignalPath.header': '訊號路徑: {path}',
  'audioSignalPath.hqPlayer.error': 'HQPlayer 異常',
  'audioSignalPath.hqPlayer.externalChain': '外部處理鏈',
  'audioSignalPath.hqPlayer.externalProcessing': '外部處理',
  'audioSignalPath.hqPlayer.externalRendering': '外部渲染',
  'audioSignalPath.hqPlayer.input': 'HQPlayer 輸入',
  'audioSignalPath.hqPlayer.output': 'HQPlayer 輸出',
  'audioSignalPath.hqPlayer.outputDecided': '由 HQPlayer 決定',
  'audioSignalPath.hqPlayer.state.connecting': '連線中',
  'audioSignalPath.hqPlayer.state.paused': '已暫停',
  'audioSignalPath.hqPlayer.state.playing': '播放中',
  'audioSignalPath.hqPlayer.state.ready': '已就緒',
  'audioSignalPath.hqPlayer.state.stopped': '已停止',
  'audioSignalPath.layers': '{count} 層鏈路',
  'audioSignalPath.node.dataSource': '資料來源',
  'audioSignalPath.node.decode': '解碼',
  'audioSignalPath.node.output': '輸出',
  'audioSignalPath.node.playbackDevice': '播放裝置',
  'audioSignalPath.node.process': '處理',
  'audioSignalPath.node.source': '音源',
  'audioSignalPath.node.transition': '佇列銜接',
  'audioSignalPath.output.asio': 'ASIO 輸出',
  'audioSignalPath.output.ks': 'WDM-KS 直連輸出',
  'audioSignalPath.output.exclusive': '獨占輸出',
  'audioSignalPath.output.shared': '共享輸出',
  'audioSignalPath.output.system': '系統輸出',
  'audioSignalPath.output.systemDefaultDevice': '系統預設裝置',
  'audioSignalPath.outputMode.exclusive': '獨占',
  'audioSignalPath.outputMode.ks': 'WDM-KS 直連',
  'audioSignalPath.outputMode.shared': '共享',
  'audioSignalPath.outputMode.system': '系統音訊',
  'audioSignalPath.path.lossless': '無損',
  'audioSignalPath.path.sdm': 'SDM 輸出',
  'audioSignalPath.path.waiting': '等待',
  'audioSignalPath.process.echoChain': '經過 ECHO 處理鏈',
  'audioSignalPath.process.nativePath': '原生路徑',
  'audioSignalPath.gapless.active': '無縫播放中',
  'audioSignalPath.gapless.activeDetail': '音訊引擎正在按解碼 PCM 邊界無縫接上下一首',
  'audioSignalPath.gapless.enabled': '無縫播放已開啟',
  'audioSignalPath.gapless.pendingDetail': '遇到符合條件的本機同專輯相鄰曲目時自動準備',
  'audioSignalPath.process.noProcessing': '未啟用 EQ / FIR / 聲道處理',
  'audioSignalPath.processing.bitDepthConversion': '位元深度轉換',
  'audioSignalPath.processing.bitDepthTo': '64bit Float 至 {depth}',
  'audioSignalPath.processing.channelProcessing': '聲道處理',
  'audioSignalPath.processing.echoSrcUpsampling': 'ECHO SRC / 升頻',
  'audioSignalPath.processing.dopTransport': 'DoP 封裝與傳輸',
  'audioSignalPath.processing.sdmModulator': '1-bit SDM 調變器',
  'audioSignalPath.processing.sdmOversampling': 'SDM FIR 過採樣',
  'audioSignalPath.processing.firAcoustic': 'FIR / 聲學處理',
  'audioSignalPath.processing.fiveBands': '5 個頻段',
  'audioSignalPath.processing.parametricEq': '參數式 EQ',
  'audioSignalPath.processing.roomCorrection': '房間校正',
  'audioSignalPath.processing.volumeNormalization': '音量標準化',
  'audioSignalPath.source.audioSource': '音訊來源',
  'audioSignalPath.source.loading': '來源資訊準備中',
  'audioSignalPath.source.local': '本機媒體',
  'audioSignalPath.source.online': '線上來源',
  'audioSignalPath.source.remote': '遠端媒體',
  'audioSignalPath.source.signal': '訊號',
  'audioSignalPath.stability.actual': '實際鏈路',
  'audioSignalPath.stability.dsp': 'DSP',
  'audioSignalPath.stability.dspActive': 'DSP 已介入',
  'audioSignalPath.stability.dspBypassed': 'DSP 旁路',
  'audioSignalPath.stability.dspInPath': '訊號會經過 ECHO 處理鏈',
  'audioSignalPath.stability.sdmInPath': 'PCM 域 FIR 過採樣、1-bit SDM 調變與 DoP 封裝正在鏈路中運行',
  'audioSignalPath.stability.dspLimiter': '保護限幅器正在介入',
  'audioSignalPath.stability.dspOffDetail': 'EQ / FIR / 聲道處理未進入鏈路',
  'audioSignalPath.stability.dspRisk': '處理後電平接近風險區',
  'audioSignalPath.stability.eyebrow': '穩定性回饋',
  'audioSignalPath.stability.fallbackActive': '已從請求路徑回退',
  'audioSignalPath.stability.fallbackNone': '未觀察到回退',
  'audioSignalPath.stability.fallbackSystem': '安全模式使用瀏覽器音訊鏈路',
  'audioSignalPath.stability.fallbackWaiting': '等待播放後確認',
  'audioSignalPath.stability.headlineError': '目前鏈路有錯誤',
  'audioSignalPath.stability.headlineExternal': '外部渲染器接管',
  'audioSignalPath.stability.headlineFallback': '已切到可播放回退路徑',
  'audioSignalPath.stability.headlineLive': '目前輸出鏈路已確認',
  'audioSignalPath.stability.headlineSystem': '安全模式正在輸出',
  'audioSignalPath.stability.headlineWaiting': '等待播放確認鏈路',
  'audioSignalPath.stability.pending': '等待中',
  'audioSignalPath.stability.rateActual': '實際輸出 {rate}',
  'audioSignalPath.stability.rateConverted': '取樣率正在轉換',
  'audioSignalPath.stability.rateMismatch': '來源和裝置取樣率不一致',
  'audioSignalPath.stability.reasonClean': '沒有觀察到錯誤、回退或 DSP 介入。',
  'audioSignalPath.stability.reasonDsp': 'DSP 或音量處理已進入鏈路，目前不是純直通輸出。',
  'audioSignalPath.stability.reasonExternal': 'ECHO 負責交接，最終處理和輸出由外部渲染器回報。',
  'audioSignalPath.stability.reasonRateMismatch': '目前裝置沒有按來源取樣率輸出。',
  'audioSignalPath.stability.reasonResampling': 'ECHO 正在按目前輸出裝置時鐘轉換取樣率。',
  'audioSignalPath.stability.reasonSdm': '音訊引擎已確認 PCM → SDM 與 DoP 輸出鏈路。',
  'audioSignalPath.stability.reasonSystem': '安全模式使用系統相容輸出。',
  'audioSignalPath.stability.reasonWaiting': '開始播放後會顯示實際輸出鏈路、取樣率和處理狀態。',
  'audioSignalPath.stability.requested': '請求模式',
  'audioSignalPath.stability.requestedDetail': '來自目前輸出設定或最近一次切換',
  'audioSignalPath.stability.sampleRate': '取樣率',
  'audioSignalPath.stability.title': '播放穩定性回饋',
  'audioSignalPath.status.error': '異常',
  'audioSignalPath.summary.bitPerfectCandidate': '純淨鏈路',
  'audioSignalPath.summary.checkOutput': '先檢查輸出裝置',
  'audioSignalPath.summary.dspOff': '未啟用 DSP',
  'audioSignalPath.summary.enhanced': '已強化',
  'audioSignalPath.summary.limiterProtecting': '限幅保護輸出',
  'audioSignalPath.summary.nativePlayback': '原生播放',
  'audioSignalPath.summary.pathError': '輸出未接上',
  'audioSignalPath.summary.protecting': '保護中',
  'audioSignalPath.summary.resampling': '重取樣',
  'audioSignalPath.summary.sampleRateMismatch': '取樣率不一致',
  'audioSignalPath.summary.showAfterPlayback': '播放後顯示鏈路',
  'audioSignalPath.summary.sourceDeviceMismatch': '來源與裝置不一致',
  'audioSignalPath.summary.toRate': '到 {rate}',
  'audioSignalPath.summary.upsampling': '升頻',
  'audioSignalPath.summary.pcmToSdm': 'PCM → SDM · {target}',
  'audioSignalPath.summary.outputModeOutput': '{mode}輸出',
  'audioSignalPath.summary.waitingPlayback': '等待播放',
  'audioSignalPath.title': '訊號路徑',
  'audioSignalPath.unknown': '等待訊號',
  'audioProfessional.action.hideDetails': '收起專業詳情',
  'audioProfessional.action.refresh': '重新整理狀態',
  'audioProfessional.action.showDetails': '展開專業詳情',
  'audioProfessional.badge.bitPerfect': 'Bit-perfect',
  'audioProfessional.badge.dsp': 'DSP active',
  'audioProfessional.badge.protect': 'Protect',
  'audioProfessional.badge.replayGain': 'ReplayGain',
  'audioProfessional.badge.resampling': '重取樣',
  'audioProfessional.badge.sampleMismatch': '取樣率不符',
  'audioProfessional.badge.upsampling': '升頻',
  'audioProfessional.badge.warning': '裝置異常/警告',
  'audioProfessional.issue.reason': '異常原因',
  'audioProfessional.issue.audioLevelClipped': '即時電平偵測到削波，請降低前級增益或關閉增益類 DSP。',
  'audioProfessional.issue.audioLevelClippingRisk': '即時電平接近 0 dBFS，有削波風險。',
  'audioProfessional.issue.dspClippingRisk': 'DSP 鏈路輸出有削波風險，Protect limiter 已進入待命監控。',
  'audioProfessional.issue.dspLimiterProtecting': 'Protect limiter 正在保護輸出，表示 DSP 後級訊號已超過安全閾值。',
  'audioProfessional.issue.roomCorrectionBitPerfectDisabled': '房間校正會關閉 bit-perfect 輸出。',
  'audioProfessional.issue.roomCorrectionClippingRisk': '房間校正輸出有削波風險，請降低 FIR Trim 或啟用 Auto Gain。',
  'audioProfessional.issue.sharedMixRateTooHigh': 'Windows 共享取樣率過高：裝置是 {deviceRate}，ECHO 目前輸出 {decoderRate} PCM，可能導致變速。建議把 Windows 預設格式改到 48 kHz。',
  'audioProfessional.issue.windowsDefaultFormatUnusual': 'Windows 預設格式設定偏高：目前是 {deviceRate}。ECHO 只會提醒，不會修改系統設定；建議改到 48 kHz。',
  'audioProfessional.group.directDsp': '直通與 DSP',
  'audioProfessional.group.playbackChain': '播放鏈路',
  'audioProfessional.group.sampleRate': '取樣率鏈路',
  'audioProfessional.group.stability': '穩定性',
  'audioProfessional.signal.decode': '解碼',
  'audioProfessional.signal.dsp': 'DSP',
  'audioProfessional.signal.fir': 'FIR',
  'audioProfessional.signal.headroom': '餘量',
  'audioProfessional.signal.native': '原生',
  'audioProfessional.signal.output': '輸出',
  'audioProfessional.signal.source': '來源',
  'audioProfessional.row.actualBuffer': '實際 buffer',
  'audioProfessional.row.actualDeviceSampleRate': '實際裝置取樣率',
  'audioProfessional.row.bitDepth': '位元深度',
  'audioProfessional.row.bitPerfect': 'Bit-perfect',
  'audioProfessional.row.bitrate': '位元率',
  'audioProfessional.row.buffered': '目前緩衝',
  'audioProfessional.row.channelBalance': '聲道平衡',
  'audioProfessional.row.channels': '聲道',
  'audioProfessional.row.clippingProtection': '削波保護',
  'audioProfessional.row.codec': '格式',
  'audioProfessional.row.decodeBackend': '解碼後端',
  'audioProfessional.row.decoderOutputSampleRate': '解碼輸出',
  'audioProfessional.row.deviceBuffer': '裝置 buffer',
  'audioProfessional.row.eq': 'EQ',
  'audioProfessional.row.error': '錯誤',
  'audioProfessional.row.fileSampleRate': '音源取樣率',
  'audioProfessional.row.latencyProfile': '延遲檔位',
  'audioProfessional.row.outputBackend': '輸出後端',
  'audioProfessional.row.outputDevice': '輸出裝置',
  'audioProfessional.row.outputLatency': '輸出延遲',
  'audioProfessional.row.outputMode': '輸出模式',
  'audioProfessional.row.replayGain': 'ReplayGain',
  'audioProfessional.row.protectLimiter': '保護限幅',
  'audioProfessional.row.roomCorrection': '房間校正',
  'audioProfessional.row.requestedBuffer': '要求 buffer',
  'audioProfessional.row.requestedOutputSampleRate': '要求輸出',
  'audioProfessional.row.resampler': '重取樣器',
  'audioProfessional.row.resampling': '重取樣',
  'audioProfessional.row.sampleRateMismatch': '取樣率不符',
  'audioProfessional.row.upsampling': '升頻',
  'audioProfessional.row.signalPath': '訊號路徑',
  'audioProfessional.row.sharedDeviceSampleRate': '共享裝置取樣率',
  'audioProfessional.row.sharedStability': '共享穩定檔',
  'audioProfessional.row.soxr': 'SOXR',
  'audioProfessional.row.state': '狀態',
  'audioProfessional.row.underrun': 'Underrun',
  'audioProfessional.row.warnings': '警告',
  'audioProfessional.summary.pending': '等待音訊狀態',
  'audioProfessional.title': '專業播放狀態',
  'audioProfessional.value.disabled': '關閉',
  'audioProfessional.value.dspPath': 'DSP 路徑：{modules}',
  'audioProfessional.value.enabled': '開啟',
  'audioProfessional.value.nativePath': '原生直通',
  'audioProfessional.value.no': '否',
  'audioProfessional.value.pending': '待確認',
  'audioProfessional.value.ready': '可直通',
  'audioProfessional.value.sharedMixer': '共享混音',
  'audioProfessional.value.systemDefault': '系統預設輸出',
  'audioProfessional.value.unknown': 'n/a',
  'audioProfessional.value.yes': '是',
  'audioDrawer.todo.outputControls': '目標取樣率與緩衝控制',
  'audioDrawer.todo.outputControlsDescription': '這些控制已接入目前音訊引擎；切換失敗時會保留原輸出並顯示診斷。',
  'audioDrawer.troubleshooting.description': '如果聲音卡住或裝置列表不正常，點這裡。軟重啟不會影響其他應用程式。',
  'audioDrawer.troubleshooting.hardAction': '重啟 Windows 音訊服務',
  'audioDrawer.troubleshooting.hardBusy': '正在重啟 Windows 音訊服務',
  'audioDrawer.troubleshooting.hardConfirm': '這會中斷所有應用程式的聲音（Chrome、遊戲、通話），並需要系統管理員權限。是否繼續？',
  'audioDrawer.troubleshooting.hardDone': 'Windows 音訊服務已恢復，你可以重新開始播放',
  'audioDrawer.troubleshooting.softAction': '重啟音訊引擎',
  'audioDrawer.troubleshooting.softBusy': '正在重啟音訊引擎',
  'audioDrawer.troubleshooting.softDone': '音訊引擎已重啟，你可以重新開始播放',
  'audioDrawer.troubleshooting.title': '音訊故障排除',
  'trackMenu.action.addToPlaylist': '加入播放清單...',
  'trackMenu.action.playNext': '下一首播放',
  'trackMenu.action.addToQueue': '加入佇列',
  'trackMenu.action.like': '喜歡',
  'trackMenu.action.unlike': '取消喜歡',
  'trackMenu.action.removeFromQueue': '從播放佇列移除',
  'trackMenu.action.removeFromPlaylist': '從目前播放清單移除',
  'trackMenu.action.openOsuTiming': 'osu! Timing',
  'trackMenu.action.openOsuBeatmapset': '譜面詳情',
  'trackMenu.action.editTags': '編輯標籤',
  'trackMenu.action.reloadEmbeddedTags': '重掃內嵌標籤',
  'trackMenu.action.clearLyricsCache': '清理歌詞快取',
  'trackMenu.action.goToAlbum': '定位到專輯',
  'trackMenu.action.showInFolder': '在資料夾中顯示',
  'trackMenu.action.copyPath': '複製檔案路徑',
  'trackMenu.action.openSystem': '使用系統預設應用程式打開',
  'trackMenu.action.copyNameArtist': '複製歌名與演出者',
  'trackMenu.action.copyCover': '複製歌曲卡片圖片',
  'trackMenu.action.saveCover': '儲存歌曲卡片圖片',
  'trackMenu.action.deleteSong': '刪除歌曲',
  'folders.action.addScan': '加入並掃描',
  'folders.action.browse': '瀏覽',
  'folders.action.cancel': '取消',
  'folders.action.open': '打開',
  'folders.action.play': '播放全部',
  'folders.action.queue': '加入佇列',
  'folders.action.queueTail': '加入佇列尾端',
  'folders.action.random': '隨機播放',
  'folders.action.refresh': '重新整理資料夾',
  'folders.action.remove': '移除',
  'folders.action.repairAlbumIndex': '修復專輯索引',
  'folders.action.scan': '掃描',
  'folders.action.scanChanges': '增量掃描',
  'folders.action.rescanEmbeddedTags': '重掃內嵌標籤',
  'folders.confirm.deleteTrack': '刪除這個音樂檔？\n{title}',
  'folders.confirm.removeRoot': '從曲庫索引中移除「{name}」？音樂檔會保留在磁碟上。',
  'folders.count.tracks': '{count} 首',
  'folders.detail.importHint': '匯入音樂資料夾後，可以依路徑瀏覽曲庫。',
  'folders.detail.libraryFolders': '曲庫資料夾',
  'folders.detail.root': '根目錄',
  'folders.detail.selectFolder': '選擇資料夾',
  'folders.detail.subfolder': '子資料夾',
  'folders.duration.hours': '{count} 小時',
  'folders.duration.hoursMinutes': '{hours} 小時 {minutes} 分鐘',
  'folders.duration.minutes': '{count} 分鐘',
  'folders.empty.noScan': '這個根目錄還沒有執行過掃描。',
  'folders.empty.roots': '還沒有曲庫資料夾。',
  'folders.firstImport.complete.body': '掃描完成後可以先播放這個目錄，或繼續打開子資料夾確認封面、專輯和壞檔跳過情況。',
  'folders.firstImport.complete.includeSubfolders': '包含子資料夾',
  'folders.firstImport.complete.play': '播放這個目錄',
  'folders.firstImport.complete.title': '掃描完成後',
  'folders.firstImport.importHelp': '加入後會立即開始背景掃描；無法讀取的檔案會被跳過並顯示在掃描狀態裡。',
  'folders.firstImport.scanSkipHint': '掃描會繼續處理可播放檔案，壞檔或無權限檔案會計入錯誤並跳過。',
  'folders.firstImport.step.choose': '先選擇你的音樂資料夾，例如 D:\\Music 或外接硬碟裡的曲庫。',
  'folders.firstImport.step.scan': 'ECHO 會顯示掃描階段、已處理數量和錯誤數量；壞檔不會阻塞整次匯入。',
  'folders.firstImport.step.finish': '完成後可以直接播放、隨機播放，或進入子資料夾檢查專輯整理結果。',
  'folders.error.actionFailed': '資料夾操作失敗。',
  'folders.error.desktopEditTags': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'folders.error.desktopFileActions': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'folders.error.desktopImport': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'folders.error.desktopManage': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'folders.error.albumIndexRepairUnavailable': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'folders.error.noCoverSaved': '沒有儲存任何封面。',
  'folders.error.noCoverToCopy': '這首歌沒有可複製的封面。',
  'folders.error.notFolder': '選取的路徑不是資料夾。',
  'folders.error.pathMissing': '資料夾路徑不存在。',
  'folders.error.permission': 'ECHO 沒有權限存取這個資料夾。',
  'folders.error.trackActionUnavailable': '這個歌曲操作暫不可用。',
  'folders.filters.includeSubfolders': '包含子資料夾',
  'folders.filters.currentFolder': '目前資料夾',
  'folders.filters.label': '資料夾歌曲篩選',
  'folders.filters.scopeLabel': '搜尋範圍',
  'folders.filters.searchPlaceholder': '搜尋歌曲、藝人、專輯或檔名...',
  'folders.message.addedToPlaylist': '已加入播放清單：{name}',
  'folders.message.albumIndexRepairComplete': '專輯索引已修復：{albums} 張專輯 / {artists} 位藝人。',
  'folders.message.albumIndexRepairStarted': '正在直接修復專輯索引，介面和播放可能會短暫卡頓，請稍等。',
  'folders.message.alreadyScanning': '這個曲庫根目錄正在掃描。',
  'folders.message.folderAddedScanStarted': '資料夾已加入，掃描已在背景開始。',
  'folders.message.folderRemoved': '資料夾已從曲庫索引中移除。',
  'folders.message.loadedPartial': '已載入前 {loaded} / {total} 首，避免佔用過多記憶體。',
  'folders.message.loadedTracks': '已載入 {count} 首。',
  'folders.message.noPlayableTracks': '這個資料夾裡沒有可播放歌曲。',
  'folders.message.queuedTracks': '已加入佇列 {count} 首。',
  'folders.message.scanCancelled': '掃描已取消。',
  'folders.message.incrementalScanStarted': '增量掃描已開始，將處理新增、修改和移除的檔案。',
  'folders.message.embeddedTagRescanStarted': '內嵌標籤重掃已在背景開始。',
  'folders.message.scanStarted': '掃描已開始。',
  'folders.metrics.duration': '長度',
  'folders.metrics.label': '資料夾指標',
  'folders.metrics.size': '大小',
  'folders.metrics.subfolders': '子資料夾',
  'folders.metrics.tracks': '歌曲',
  'folders.panel.addFolder': '加入資料夾',
  'folders.panel.import': '匯入',
  'folders.panel.manage': '管理',
  'folders.panel.scan': '掃描',
  'folders.panel.selectedRoot': '已選根目錄',
  'folders.panel.status': '狀態',
  'folders.phase.checkingCache': '檢查快取',
  'folders.phase.discovering': '尋找檔案',
  'folders.phase.extractingCovers': '擷取封面',
  'folders.phase.finished': '已完成',
  'folders.phase.groupingAlbums': '整理專輯',
  'folders.phase.readingMetadata': '讀取標籤',
  'folders.phase.writingDatabase': '寫入資料庫',
  'folders.prompt.choosePlaylist': '選擇播放清單編號：\n{names}',
  'folders.prompt.createPlaylist': '還沒有播放清單。輸入名稱來建立一個：',
  'folders.queueSource.recursive': '{name} 資料夾',
  'folders.scan.progress': '{processed}/{total} 個檔案，{errors} 個錯誤',
  'folders.scan.errorDetails': '{count} 個失敗項目',
  'folders.scan.patientWarning': '掃描期間未回應是正常現象，請耐心等待！',
  'folders.scan.unresponsiveWarning': '正在掃描曲庫。掃描期間軟體可能短暫未回應，這是正常現象；請等待或使用取消掃描。',
  'folders.sidebar.kicker': '曲庫',
  'folders.sidebar.title': '資料夾',
  'folders.sort.album': '專輯',
  'folders.sort.artist': '演出者',
  'folders.sort.trackNumber': '音軌號',
  'folders.sort.duration': '長度',
  'folders.sort.quality': '音質',
  'folders.sort.random': '隨機',
  'folders.sort.recent': '最近更新',
  'folders.sort.title': '標題',
  'folders.status.cancelled': '已取消',
  'folders.status.completed': '完成',
  'folders.status.failed': '失敗',
  'folders.status.queued': '排隊中',
  'folders.status.running': '掃描中',
  'folders.statusLine.loadingTracks': '正在讀取資料夾歌曲...',
  'folders.statusLine.preparingQueue': '正在準備資料夾佇列...',
  'route.albums.label': '專輯',
  'route.artists.label': '演出者',
  'route.genres.label': '類型',
  'route.audioCd.description': '直接播放光碟音軌。',
  'route.audioCd.label': 'Audio CD',
  'route.audioSettings.label': '音訊設定',
  'route.connect.description': 'DLNA / AirPlay 無線播放。',
  'route.connect.label': '連接',
  'route.dsp.description': '訊號鏈調音工作台。',
  'route.dsp.label': '音效處理',
  'connectPage.controls.aria': 'Connect 控制',
  'connectPage.controls.disconnect': '中斷連接',
  'connectPage.controls.pause': '暫停',
  'connectPage.controls.play': '播放',
  'connectPage.controls.stop': '停止',
  'connectPage.controls.volume': '投送音量',
  'connectPage.deviceState.available': '可用',
  'connectPage.deviceState.connected': '已連接',
  'connectPage.deviceState.unavailable': '離線',
  'connectPage.deviceState.unsupported': '實驗',
  'connectPage.devices.allHidden': '目前設備都已隱藏',
  'connectPage.devices.aria': '設備列表',
  'connectPage.devices.collapse': '收起區域網串流設備',
  'connectPage.devices.empty': '未發現區域網設備',
  'connectPage.devices.emptyHint': '重新整理後會在這裡顯示串流器、TV、HQPlayer 和 AirPlay 入口。',
  'connectPage.devices.expand': '展開區域網串流設備',
  'connectPage.devices.hiddenAria': '已隱藏的區域網設備',
  'connectPage.devices.hiddenTitle': '已隱藏設備',
  'connectPage.devices.kicker': '區域網數播',
  'connectPage.devices.restoreAll': '全部恢復',
  'connectPage.devices.restoreHint': '可在上方恢復隱藏設備。',
  'connectPage.devices.summary': '{streamers} 台串流器 · {entries} 個入口 · 已隱藏 {hidden}',
  'connectPage.devices.streamerLabel': '串流器',
  'connectPage.devices.title': '區域網串流',
  'connectPage.header.autoStart': '啟動時自動開啟 AirPlay / DLNA',
  'connectPage.header.description': '把目前曲目投送到區域網串流器、電視或 HQPlayer。',
  'connectPage.header.descriptionMobile': '用手機遙控本機播放，或把狀態接到智慧家庭。',
  'connectPage.header.descriptionRadio': '收藏並播放網路電台流。',
  'connectPage.header.descriptionReceive': '讓手機或其他裝置把音樂推送到這台 ECHO。',
  'connectPage.header.descriptionHqPlayer': '把本機或遠端 HQPlayer 接成外部渲染器。',
  'connectPage.tabs.hqplayer': 'HQPlayer',
  'connectPage.header.kicker': 'Wireless playback',
  'connectPage.brand.subtitle': '投送與遙控',
  'connectPage.rail.output': '區域網串流',
  'connectPage.rail.hqplayer': '外部渲染器',
  'connectPage.rail.receive': 'AirPlay / DLNA',
  'connectPage.rail.mobile': '遙控與連動',
  'connectPage.rail.radio': '收藏電台',
  'connectPage.rail.groupCast': '投送',
  'connectPage.rail.groupIncoming': '接入',
  'connectPage.header.refresh': '重新整理設備',
  'connectPage.hqplayer.enable': '啟用 HQPlayer',
  'connectPage.hqplayer.disable': '停用 HQPlayer',
  'connectPage.hqplayer.aria': 'HQPlayer 連線',
  'connectPage.hqplayer.guideTitle': '把目前曲目交給 HQPlayer',
  'connectPage.hqplayer.guide1': '開啟 HQPlayer，並確認本機或遠端 Desktop 已打開控制連接埠。',
  'connectPage.hqplayer.guide2': '選擇本機或遠端模式，檢測連線後再播放目前曲目。',
  'connectPage.hqplayer.guide3': '交接成功後，這裡會顯示 HQPlayer 回報的進度和訊號路徑。',
  'connectPage.hqplayer.listeningHint': '已啟用。檢測連線後，從媒體庫播放即可交給 HQPlayer。',
  'connectPage.hqplayer.offlineHint': '先啟用 HQPlayer，再檢測本機或遠端 Desktop。',
  'connectPage.hqplayer.playbackAria': 'HQPlayer 投送狀態',
  'connectPage.hqplayer.playbackTitle': '外部渲染',
  'connectPage.hqplayer.waitingTitle': '等待交接',
  'connectPage.hqplayer.progressAria': 'HQPlayer 播放進度',
  'connectPage.hqplayer.debugTitle': '最近交接',
  'connectPage.hqplayer.debugEmpty': '暫無交接紀錄',
  'connectPage.hqplayer.debugNoneYet': '還沒有交接計畫',
  'connectPage.hqplayer.field.host': '主機',
  'connectPage.hqplayer.field.product': '產品',
  'connectPage.hqplayer.field.engine': '引擎',
  'connectPage.hqplayer.field.profile': '設定檔',
  'connectPage.hqplayer.field.handoff': '交接',
  'connectPage.hqplayer.field.control': '控制',
  'connectPage.hqplayer.field.send': '傳送',
  'connectPage.hqplayer.field.source': '音源',
  'connectPage.hqplayer.field.track': '曲目',
  'connectPage.hqplayer.field.headers': '請求頭',
  'connectPage.hqplayer.field.mediaUrl': '媒體位址',
  'connectPage.hqplayer.kicker': '外部渲染器',
  'connectPage.hqplayer.localDesktop': '本機 HQPlayer Desktop',
  'connectPage.hqplayer.state.available': '可連接',
  'connectPage.hqplayer.state.checking': '檢測中',
  'connectPage.hqplayer.state.disabled': '未啟用',
  'connectPage.hqplayer.state.notConfigured': '未設定連接埠',
  'connectPage.hqplayer.state.unavailable': '不可用',
  'connectPage.hqplayer.test': '檢測 HQPlayer',
  'connectPage.nowPlaying.aria': '目前投送',
  'connectPage.nowPlaying.chooseDevice': '選擇一台區域網串流器後開始投送',
  'connectPage.nowPlaying.coverReady': '封面 URL 已送出',
  'connectPage.nowPlaying.coverWaiting': '等待封面 URL',
  'connectPage.nowPlaying.dlnaPolling': '狀態輪詢約 3 秒',
  'connectPage.nowPlaying.emptyTitle': '目前沒有歌曲',
  'connectPage.nowPlaying.infoAria': '目前投送資訊',
  'connectPage.nowPlaying.infoWaiting': '等待投送資訊',
  'connectPage.nowPlaying.latency': '投送握手 {ms}ms',
  'connectPage.nowPlaying.noOutput': '未連接輸出',
  'connectPage.nowPlaying.progressAria': '投送進度',
  'connectPage.outgoing.aria': 'DLNA outgoing request log',
  'connectPage.outgoing.empty': 'No pulls yet',
  'connectPage.outgoing.note': '設備拉取封面或音訊後會顯示在這裡',
  'connectPage.outgoing.recent': '{count} recent',
  'connectPage.outgoing.title': 'DLNA Out',
  'connectPage.output.field.target': '目標',
  'connectPage.output.field.device': '裝置',
  'connectPage.output.field.signal': '投送',
  'connectPage.output.guideTitle': '投送到數播',
  'connectPage.output.guide1': '先在 ECHO 播放一首本地歌曲。',
  'connectPage.output.guide2': '在右側選擇同一區域網裡的數播、TV 或 HQPlayer。',
  'connectPage.output.guide3': '連接後，這台裝置接管輸出，本機揚聲器不再播放。',
  'connectPage.output.localHint': '目前由本機 Audio Core 播放。',
  'connectPage.output.liveHint': '播放、進度和裝置能力以這台數播回報的狀態為準。',
  'connectPage.receiver.aria': '接收來自手機',
  'connectPage.receiver.disable': '關閉接收',
  'connectPage.receiver.discoveryHint': '開啟後手機即可發現',
  'connectPage.receiver.enable': '開啟接收',
  'connectPage.receiver.fromClient': '來自 {address}',
  'connectPage.receiver.kicker': 'Receiver',
  'connectPage.receiver.noClient': '未連接手機',
  'connectPage.receiver.preparing': '正在準備區域網位址',
  'connectPage.receiver.progressAria': '接收播放進度',
  'connectPage.receiver.state.disabled': '未開啟',
  'connectPage.receiver.state.idle': '等待手機',
  'connectPage.receiver.state.loading': '載入中',
  'connectPage.receiver.state.playing': '手機投送中',
  'connectPage.receiver.state.ready': '已接收媒體',
  'connectPage.receiver.stop': '停止接收播放',
  'connectPage.receiver.title': '接收來自手機',
  'connectPage.receiver.waitingTitle': '等待手機投送',
  'connectPage.receiver.protocol': 'DLNA',
  'connectPage.receiver.field.state': '狀態',
  'connectPage.receiver.field.name': '裝置名',
  'connectPage.receiver.field.network': '網路',
  'connectPage.receiver.field.source': '來源',
  'connectPage.receiver.guideTitle': '從手機投送',
  'connectPage.receiver.guide1': '開啟接收後，這台電腦會出現在手機的 DLNA / 投屏清單裡。',
  'connectPage.receiver.guide2': '在手機上選擇上方顯示的裝置名。',
  'connectPage.receiver.guide3': '開始播放後，封面、進度和來源會顯示在這裡。',
  'connectPage.receiver.offlineHint': '接收關閉時，區域網裝置看不到這台 ECHO。',
  'connectPage.receiver.listeningHint': '已在區域網廣播，等待手機選擇這台裝置。',
  'connectPage.stage.aria': '快速投送',
  'connectPage.state.connecting': '連接中',
  'connectPage.state.discovering': '掃描設備',
  'connectPage.state.error': '錯誤',
  'connectPage.state.idle': '待機',
  'connectPage.state.paused': '已暫停',
  'connectPage.state.playing': '投送中',
  'connectPage.state.stopped': '已停止',
  'connectPage.airplay.state.idle': '等待 iPhone',
  'connectPage.airplay.state.playing': 'AirPlay 播放中',
  'connectPage.airplay.state.starting': '啟動中',
  'connectPage.airplay.state.unavailable': '原生後端不可用',
  'connectPage.airplay.waitingTitle': '等待 iPhone 投送',
  'connectPage.airplay.aria': 'AirPlay 實驗接收',
  'connectPage.airplay.copyDebug': '複製 AirPlay 診斷',
  'connectPage.airplay.currentAirplay1': '目前使用 AirPlay 1 / RAOP',
  'connectPage.airplay.currentAirplay2': '目前使用 AirPlay 2 實驗探索',
  'connectPage.airplay.debugAria': 'AirPlay 接收日誌',
  'connectPage.airplay.debugNoneYet': '暫無 AirPlay 事件',
  'connectPage.airplay.debugTitle': 'AirPlay 診斷',
  'connectPage.airplay.disable': '關閉 AirPlay',
  'connectPage.airplay.enable': '開啟 AirPlay',
  'connectPage.airplay.nativeLoaded': 'RAOP 後端已載入',
  'connectPage.airplay.nativeRequired': '需要可用的 AirPlay 原生後端',
  'connectPage.airplay.progressAria': 'AirPlay 播放進度',
  'connectPage.airplay.protocol.airplay1': 'AirPlay 1 / RAOP',
  'connectPage.airplay.protocol.airplay2': 'AirPlay 2 實驗',
  'connectPage.airplay.protocolAria': 'AirPlay 協定',
  'connectPage.airplay.seekHint': '傳送端支援遠端搜尋前，無法拖曳播放進度',
  'connectPage.airplay.stop': '停止 AirPlay',
  'connectPage.airplay.title': '接收來自 iPhone',
  'connectPage.airplay.waitingDevice': '等待 iPhone / iPad',
  'connectPage.airplay.protocolLabel': 'AirPlay',
  'connectPage.airplay.field.protocol': '協定',
  'connectPage.airplay.field.backend': '後端',
  'connectPage.airplay.guideTitle': '從 iPhone 投送',
  'connectPage.airplay.guide1': '開啟 AirPlay，並讓 iPhone 與這台電腦連同一 Wi-Fi。',
  'connectPage.airplay.guide2': '打開 iPhone 控制中心，輕點螢幕鏡像，選擇上方裝置名。',
  'connectPage.airplay.guide3': '開始播放後，這裡會顯示封面和進度。傳送端未開放遠端搜尋前不能拖曳進度。',
  'connectPage.airplay.offlineHint': 'AirPlay 關閉時，iPhone 控制中心不會列出這台 ECHO。',
  'connectPage.airplay.listeningHint': '已可被 iPhone / iPad 發現。',
  'connectPage.commandCenter.aria': 'Connect 總覽',
  'connectPage.commandCenter.collapse': '收合 Connect 總覽',
  'connectPage.commandCenter.entryCount': '{count} 个入口',
  'connectPage.commandCenter.expand': '展開 Connect 總覽',
  'connectPage.commandCenter.health.active': '正在投送',
  'connectPage.commandCenter.health.online': '中枢在线',
  'connectPage.commandCenter.health.warning': '需要关注',
  'connectPage.commandCenter.moreIssues': '另有 {count} 条连接事件',
  'connectPage.commandCenter.noRecentFailures': '最近没有连接失败',
  'connectPage.commandCenter.phonePairing': '手机配对',
  'connectPage.commandCenter.qrReady': '扫码连接手机 ECHO',
  'connectPage.commandCenter.qrWaiting': '等待 ECHO Link 启动',
  'connectPage.commandCenter.refreshAll': '全局刷新',
  'connectPage.commandCenter.route.output': '当前输出 / {target}',
  'connectPage.commandCenter.route.waiting': '等待选择输出',
  'connectPage.commandCenter.routeAria': '当前投送链路',
  'connectPage.commandCenter.webRemote': 'Web 遥控',
  'connectPage.common.copied': '已复制',
  'connectPage.common.copy': '复制',
  'connectPage.common.disable': '关闭',
  'connectPage.common.disabled': '未开启',
  'connectPage.common.enable': '开启',
  'connectPage.common.none': '无',
  'connectPage.common.notChecked': '未检测',
  'connectPage.common.notConfigured': '未配置',
  'connectPage.common.open': '打开',
  'connectPage.common.pendingCheck': '待检测',
  'connectPage.common.refresh': '刷新',
  'connectPage.common.unknown': '未知',
  'connectPage.device.format.all': '全格式接收',
  'connectPage.device.format.pending': '格式待探测',
  'connectPage.device.lanAddress': '局域网 {address}',
  'connectPage.device.modelUnknown': '型号待识别',
  'connectPage.device.support.airplay': '实验通道 · 元数据门控',
  'connectPage.device.support.basicDlna': '基础 DLNA 投送',
  'connectPage.device.support.direct': '可直连',
  'connectPage.device.support.hqplayer': '本机控制 · 高精度输出',
  'connectPage.device.support.metadata': '封面/元数据',
  'connectPage.device.support.seek': '可定位',
  'connectPage.device.support.transcode': '需要转码',
  'connectPage.device.support.volume': '可调音量',
  'connectPage.device.waitingAddress': '等待网络地址',
  'connectPage.devices.connect': '连接',
  'connectPage.devices.deviceInfoAria': '{name} 裝置資訊',
  'connectPage.devices.discoveryPending': '尚未完成发现',
  'connectPage.devices.hideHint': '右键隐藏此设备',
  'connectPage.devices.lastSeen': '最后发现 {time}',
  'connectPage.devices.reconnect': '重新投送',
  'connectPage.echoLink.address': '地址',
  'connectPage.echoLink.aria': 'ECHO Link',
  'connectPage.echoLink.authFailures': '认证失败',
  'connectPage.echoLink.backgroundClear': '清除',
  'connectPage.echoLink.backgroundChooseImage': '选择图片',
  'connectPage.echoLink.backgroundHint': '给 Album Sea 网页端换一张图片或一个静音循环影片背景。',
  'connectPage.echoLink.backgroundImage': '图片',
  'connectPage.echoLink.backgroundNone': '預設',
  'connectPage.echoLink.backgroundSave': '保存背景',
  'connectPage.echoLink.backgroundSaved': '已保存',
  'connectPage.echoLink.backgroundTitle': '网页背景',
  'connectPage.echoLink.backgroundType': '类型',
  'connectPage.echoLink.backgroundUrl': '媒体 URL',
  'connectPage.echoLink.backgroundUrlPlaceholder': 'https://example.com/background.webm',
  'connectPage.echoLink.backgroundVideo': '影片',
  'connectPage.echoLink.collapse': '折叠 ECHO Link',
  'connectPage.echoLink.copyWeb': '复制网页',
  'connectPage.echoLink.device': '设备',
  'connectPage.echoLink.diagnostics': 'ECHO Link 诊断',
  'connectPage.echoLink.discovery': '发现',
  'connectPage.echoLink.expand': '展开 ECHO Link',
  'connectPage.echoLink.hideToken': '隐藏 Token',
  'connectPage.echoLink.lanAddress': 'LAN 地址',
  'connectPage.echoLink.lastRange': '最后 Range',
  'connectPage.echoLink.mdnsAdvertising': 'mDNS 已广播',
  'connectPage.echoLink.mdnsError': 'mDNS 异常',
  'connectPage.echoLink.mdnsIdle': '未广播',
  'connectPage.echoLink.pairDisabled': 'echo://pair 未启用',
  'connectPage.echoLink.phone': '手机',
  'connectPage.echoLink.phoneNeverConnected': '尚未连接',
  'connectPage.echoLink.rotateToken': '轮换 Token',
  'connectPage.echoLink.showToken': '显示 Token',
  'connectPage.echoLink.state.error': '异常',
  'connectPage.echoLink.state.running': '运行中',
  'connectPage.echoLink.state.starting': '启动中',
  'connectPage.echoLink.tempStreams': '临时流',
  'connectPage.echoLink.title': 'ECHO Link 進階設定',
  'connectPage.echoLink.webAlbumSeaReady': 'Album Sea 已就绪',
  'connectPage.echoLink.webAvailableAfterEnable': '开启 ECHO Link 后可用',
  'connectPage.echoLink.webHint': '浏览器打开后可控制播放，并用专辑墙选择专辑。',
  'connectPage.echoLink.webReady': 'Web 遥控就绪',
  'connectPage.echoLink.webTitle': '网页控制端',
  'connectPage.echoLink.webWaiting': '等待 ECHO Link',
  'connectPage.error.airplayBridge': '暫時無法使用 AirPlay 接收。',
  'connectPage.error.airplayProtocolBridge': '暫時無法更改 AirPlay 協議設定。',
  'connectPage.error.copyAirPlayDebug': '複製 AirPlay 除錯資訊失敗。',
  'connectPage.error.copyAirPlayDebugWithMessage': '複製 AirPlay 除錯資訊失敗：{message}',
  'connectPage.error.desktopBridgeConnect': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'connectPage.error.desktopBridgeEchoLink': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'connectPage.error.desktopBridgeEchoLinkToken': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'connectPage.error.desktopBridgeHqPlayerConfig': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'connectPage.error.desktopBridgeHqPlayerTest': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'connectPage.error.desktopBridgeRadioPlay': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'connectPage.error.desktopBridgeRadioStop': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'connectPage.error.desktopBridgeSettings': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'connectPage.error.emptyMetadata': '请先播放或选中一首歌，Connect 不允许空元数据投送。',
  'connectPage.error.radioUrlInvalid': '电台 URL 无效，只支持 http/https 直播流。',
  'connectPage.error.radioUrlRequired': '请输入 http/https 电台直播流 URL，且不要带账号密码。',
  'connectPage.hqplayer.advanced': '高级设置',
  'connectPage.hqplayer.autoPort': '自动端口',
  'connectPage.hqplayer.backend.ask': '每次询问',
  'connectPage.hqplayer.backend.echoNative': '继续用 ECHO',
  'connectPage.hqplayer.backend.hqplayer': '优先 HQPlayer',
  'connectPage.hqplayer.backendAria': 'HQPlayer 預設交接',
  'connectPage.hqplayer.collapse': '折叠 HQPlayer',
  'connectPage.hqplayer.controlEndpoint': '控制端点',
  'connectPage.hqplayer.controlPort': '控制端口',
  'connectPage.hqplayer.defaultBackend': '預設交接',
  'connectPage.hqplayer.expand': '展开 HQPlayer',
  'connectPage.hqplayer.exposure.directHttp': '直连 HTTP',
  'connectPage.hqplayer.exposure.localFile': '本地文件',
  'connectPage.hqplayer.exposure.loopbackHttp': '本机流地址',
  'connectPage.hqplayer.exposure.mediaServer': 'ECHO 媒体服务',
  'connectPage.hqplayer.handoffReason.confirmationRequired': '需要确认',
  'connectPage.hqplayer.handoffReason.disabled': 'HQPlayer 未启用',
  'connectPage.hqplayer.handoffReason.echoNativeSelected': '当前选择 ECHO 输出',
  'connectPage.hqplayer.handoffReason.mediaServerNotReady': '媒体服务未就绪',
  'connectPage.hqplayer.handoffReason.portNotConfigured': '控制端口未配置',
  'connectPage.hqplayer.handoffReason.remoteRequiresMediaServer': '远程模式需要媒体服务',
  'connectPage.hqplayer.handoffReason.sourceRequiresHeaders': '音源需要请求头',
  'connectPage.hqplayer.handoffReason.sourceResolutionFailed': '音源解析失败',
  'connectPage.hqplayer.handoffReason.spotifySdkRequired': 'Spotify 需要 SDK 播放',
  'connectPage.hqplayer.handoffReason.streamingItemUnplayable': '串流曲目不可播放',
  'connectPage.hqplayer.handoffReason.streamingProxyRequired': '需要代理播放',
  'connectPage.hqplayer.handoffReason.unsupportedMediaType': '暂不支持的媒体类型',
  'connectPage.hqplayer.headersNotExposed': '不暴露请求头',
  'connectPage.hqplayer.headersRequireMediaServer': '需要媒体服务隐藏',
  'connectPage.hqplayer.lastChecked': '上次检测',
  'connectPage.hqplayer.localModeHint': '本机模式可直接交接本地文件或本机流地址',
  'connectPage.hqplayer.mediaPort': '媒体端口',
  'connectPage.hqplayer.mediaServer': '串流保护',
  'connectPage.hqplayer.mediaServerAria': 'HQPlayer 媒体服务',
  'connectPage.hqplayer.mode.localDesktop': '本机 Desktop',
  'connectPage.hqplayer.mode.remote': '远程 HQPlayer',
  'connectPage.hqplayer.modeAria': 'HQPlayer 连接模式',
  'connectPage.hqplayer.noCurrentTrack': '暂无当前歌曲',
  'connectPage.hqplayer.recentPlan': '最近交接计划',
  'connectPage.hqplayer.remoteModeHint': '远程模式会优先使用 ECHO 媒体服务',
  'connectPage.hqplayer.remoteState.stopRequested': '正在停止',
  'connectPage.hqplayer.remoteStatus': '远端状态',
  'connectPage.hqplayer.sendReason.controlPlanMissing': '还没有可发送的交接计划',
  'connectPage.hqplayer.sendReason.failed': '连接失败',
  'connectPage.hqplayer.sendReason.handoffNotReady': '交接未就绪',
  'connectPage.hqplayer.sendReason.portNotConfigured': '控制端口未配置',
  'connectPage.hqplayer.sendReason.protocolError': '协议响应异常',
  'connectPage.hqplayer.sendReason.refused': '连接被拒绝',
  'connectPage.hqplayer.sendReason.responseError': 'HQPlayer 返回错误',
  'connectPage.hqplayer.sendReason.sourceMissing': '音源缺失',
  'connectPage.hqplayer.sendReason.sourceRequiresHeaders': '音源需要私密请求头',
  'connectPage.hqplayer.sendReason.timeout': '连接超时',
  'connectPage.hqplayer.sendState.failed': '发送失败',
  'connectPage.hqplayer.sendState.notSent': '未发送',
  'connectPage.hqplayer.sendState.prepared': '已准备',
  'connectPage.hqplayer.sendState.sent': '已发送 · {ms}ms',
  'connectPage.hqplayer.sendState.unknownReason': '未知原因',
  'connectPage.hqplayer.signal.pendingFormat': '格式待回读',
  'connectPage.hqplayer.signalPath': '信号路径',
  'connectPage.hqplayer.testAvailable': '可用 · {ms}ms',
  'connectPage.hqplayer.testResult': '检测结果',
  'connectPage.lock.aria': 'Connect 需要 ECHO Pro',
  'connectPage.lock.copyHwid': '复制 HWID',
  'connectPage.lock.description': 'DLNA、AirPlay、HQPlayer、ECHO Link、手機掃碼和 Web 遙控都需要 ECHO Pro 帳號完成雲端驗證後才能使用。',
  'connectPage.lock.reason.hwidFileInvalid': '{hwidFile} 不是有效的 HWID 白名单。',
  'connectPage.lock.reason.hwidFileMissing': '插件缺少 {hwidFile} 白名单文件。',
  'connectPage.lock.reason.hwidNotAllowed': '当前设备 HWID 不在插件白名单内。',
  'connectPage.lock.reason.licenseInvalid': 'ECHO Pro 帳號未登入、未授權，或雲端驗證失敗。',
  'connectPage.lock.reason.pluginDisabled': '解锁插件已安装但未启用。',
  'connectPage.lock.reason.pluginError': '解锁插件异常或已被隔离。',
  'connectPage.lock.reason.pluginMissing': '需要先导入连接解锁插件。',
  'connectPage.lock.reason.unlocked': '已解锁。',
  'connectPage.lock.recheck': '重新校验',
  'connectPage.lock.title': 'Connect 已升級為 ECHO Pro Only',
  'connectPage.radio.aria': '网络电台',
  'connectPage.radio.collapse': '折叠网络电台',
  'connectPage.radio.deleteStation': '删除 {name}',
  'connectPage.radio.emptyDescription': '先支持手动电台收藏，避免目录接口拖慢 Connect。',
  'connectPage.radio.emptyTitle': '新增一個直播流 URL',
  'connectPage.radio.expand': '展开网络电台',
  'connectPage.radio.formAria': '网络电台表单',
  'connectPage.radio.lastPlayed': '上次播放 {time}',
  'connectPage.radio.name': '电台名',
  'connectPage.radio.namePlaceholder': '例如 Groove Salad',
  'connectPage.radio.neverPlayed': '未播放',
  'connectPage.radio.playStation': '播放 {name}',
  'connectPage.radio.playClipboard': '貼上並播放',
  'connectPage.radio.save': '收藏',
  'connectPage.radio.savedAria': '已收藏电台',
  'connectPage.radio.state.inactive': '未播放电台',
  'connectPage.radio.state.playing': '播放中',
  'connectPage.radio.state.preparing': '准备中',
  'connectPage.radio.streamUrl': '直播流 URL',
  'connectPage.radio.title': '网络电台',
  'connectPage.receiver.debugAria': 'DLNA 请求日志',
  'connectPage.receiver.debugEmpty': 'No requests',
  'connectPage.receiver.debugNoneYet': '暂无 DLNA 请求',
  'connectPage.receiver.debugTitle': 'DLNA Debug',
  'connectPage.room.airplay.title': 'AirPlay 接收器',
  'connectPage.room.aria': '局域网听音室地图',
  'connectPage.room.collapse': '折叠听音室地图',
  'connectPage.room.dlna.addressCount': '{count} 个 LAN 地址',
  'connectPage.room.dlna.title': 'DLNA 接收器',
  'connectPage.room.expand': '展开听音室地图',
  'connectPage.room.hqplayer.remote': '远端 {state}',
  'connectPage.room.nodeAria': '听音室节点 {title}',
  'connectPage.room.outputs.available': '可用渲染器',
  'connectPage.room.outputs.current': '当前输出',
  'connectPage.room.outputs.discovered': '已发现 {count} 个',
  'connectPage.room.outputs.summary': '输出：{dlna} DLNA / {airplay} AirPlay / {hqplayer} HQPlayer',
  'connectPage.room.outputs.target': '目标：{target}',
  'connectPage.room.phone.available': '遥控可用',
  'connectPage.room.phone.last': '上次手机 {time}',
  'connectPage.room.phone.offline': '配对服务离线',
  'connectPage.room.phone.title': '手机遥控',
  'connectPage.room.title': '局域网听音室地图',
  'connectPage.room.wallpaper.liveClients': '{count} 个实时视觉客户端',
  'connectPage.room.wallpaper.offline': 'Web 桌布橋離線',
  'connectPage.room.wallpaper.ready': '等待 Web 桌布連線（影片桌布不支援聯動）',
  'route.downloads.description': '搜尋下載、貼上連結、提取音訊並匯入曲庫。',
  'route.downloads.label': '下載',
  'route.osuDownloader.label': 'osu!',
  'route.folders.label': '資料夾',
  'historyPage.action.clear': '清空歷史',
  'historyPage.action.refreshInvalid': '刷新失效歌曲',
  'historyPage.confirm.clear': '要清空播放歷史嗎？這不會刪除你的音樂檔案，也不會清空曲庫。',
  'historyPage.date.none': '暫無',
  'historyPage.duration.hoursMinutes': '{hours} 小時 {minutes} 分鐘',
  'historyPage.duration.minutes': '{count} 分鐘',
  'historyPage.empty.description': '播放一首歌後，這裡會記錄你的最近收聽。',
  'historyPage.empty.title': '還沒有播放歷史。',
  'historyPage.error.desktopBridgeRead': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'historyPage.memory.aria': '聽歌記憶圖譜',
  'historyPage.memory.bucket.count': '{count} 次',
  'historyPage.memory.bucket.day': '白天',
  'historyPage.memory.bucket.evening': '夜晚',
  'historyPage.memory.bucket.lateNight': '深夜',
  'historyPage.memory.bucket.morning': '上午',
  'historyPage.memory.comeback.empty': '還沒發現回潮曲目',
  'historyPage.memory.comeback.title': '最近回潮',
  'historyPage.memory.coverage.outputMissing': '輸出裝置暫未寫入歷史；目前圖譜基於時間、完播、跳過、收藏和接歌關係。',
  'historyPage.memory.coverage.outputReady': '已包含輸出裝置歷史。',
  'historyPage.memory.detail.lastPlayed': '上次 {date}',
  'historyPage.memory.detail.plays': '{count} 次播放',
  'historyPage.memory.detail.playsDuration': '{count} 次 · {duration}',
  'historyPage.memory.detail.skipped': '{count} 次早退 · {rate}',
  'historyPage.memory.flow.aria': '最近聽歌流',
  'historyPage.memory.flow.title': '最近流',
  'historyPage.memory.forgotten.empty': '暫無沉睡曲目',
  'historyPage.memory.forgotten.title': '被遺忘的神曲',
  'historyPage.memory.gap.hours': '{count} 小時',
  'historyPage.memory.gap.lessThanMinute': '不到 1 分鐘',
  'historyPage.memory.gap.minutes': '{count} 分鐘',
  'historyPage.memory.generated': '更新於 {date}',
  'historyPage.memory.kicker': '本地聽歌畫像',
  'historyPage.memory.lateNight.empty': '深夜樣本還不夠',
  'historyPage.memory.lateNight.title': '深夜常聽',
  'historyPage.memory.liked.empty': '收藏列表裡還沒有足夠播放記錄',
  'historyPage.memory.liked.title': '收藏常聽',
  'historyPage.memory.skipped.empty': '暫無明顯跳過傾向',
  'historyPage.memory.skipped.title': '容易跳過',
  'historyPage.memory.summary.activeTime': '高頻時段',
  'historyPage.memory.summary.completion': '完播率',
  'historyPage.memory.summary.events': '已分析事件',
  'historyPage.memory.summary.eventsValue': '{count} 條',
  'historyPage.memory.summary.skip': '早退率',
  'historyPage.memory.summaryAria': '聽歌記憶摘要',
  'historyPage.memory.title': '聽歌記憶',
  'historyPage.memory.transition.detail': '{count} 次 · 平均間隔 {gap}',
  'historyPage.memory.transition.empty': '連續播放樣本還不夠',
  'historyPage.memory.transition.title': '常見接歌',
  'historyPage.filter.all': '全部',
  'historyPage.filter.completed': '只看完整播放',
  'historyPage.filter.month': '本月',
  'historyPage.filter.year': '今年',
  'historyPage.filter.today': '今天',
  'historyPage.filter.week': '本週',
  'historyPage.source.aria': '歷史來源篩選',
  'historyPage.source.all': '全部來源',
  'historyPage.source.local': '本地',
  'historyPage.source.streaming': '串流媒體',
  'historyPage.header.kicker': '最近播放記錄',
  'historyPage.header.title': '歷史',
  'historyPage.list.aria': '播放歷史列表',
  'historyPage.list.doubleClick': '雙擊播放',
  'historyPage.list.source': '來自 {source}',
  'historyPage.list.unknownAlbum': '未知專輯',
  'historyPage.list.unknownArtist': '未知藝術家',
  'historyPage.list.unknownSource': '來源未知',
  'historyPage.loadMore': '載入更多',
  'historyPage.loading': '正在讀取播放歷史...',
  'historyPage.loadingMore': '正在載入...',
  'historyPage.metric.plays': '{count} 次',
  'historyPage.metric.tracks': '{count} 首',
  'historyPage.refresh.none': '沒有發現失效歷史歌曲。',
  'historyPage.refresh.removed': '已移除 {count} 首失效歷史歌曲。',
  'historyPage.refresh.running': '正在刷新...',
  'historyPage.recent.aria': '最近播放列表',
  'historyPage.recent.count': '最近 {count} 首',
  'historyPage.recent.empty': '暫無最近播放。',
  'historyPage.recent.kicker': '按時間排序',
  'historyPage.recent.title': '最近播放列表',
  'historyPage.search.placeholder': '搜尋標題、藝術家、專輯或路徑',
  'historyPage.summary.all.count': '總播放',
  'historyPage.summary.all.duration': '總時長',
  'historyPage.summary.all.group': '按播放次數排序',
  'historyPage.summary.all.latest': '最近播放時間',
  'historyPage.summary.all.tracks': '歷史曲目',
  'historyPage.summary.aria': '歷史概覽',
  'historyPage.summary.completed.count': '完整播放',
  'historyPage.summary.completed.duration': '完整播放時長',
  'historyPage.summary.completed.group': '按完整播放次數排序',
  'historyPage.summary.completed.latest': '最近完整播放',
  'historyPage.summary.completed.tracks': '完整播放曲目',
  'historyPage.summary.month.count': '本月播放',
  'historyPage.summary.month.duration': '本月時長',
  'historyPage.summary.month.group': '本月按播放次數排序',
  'historyPage.summary.month.latest': '本月最近播放',
  'historyPage.summary.month.tracks': '本月曲目',
  'historyPage.summary.year.count': '今年播放',
  'historyPage.summary.year.duration': '今年時長',
  'historyPage.summary.year.group': '今年按播放次數排序',
  'historyPage.summary.year.latest': '今年最近播放',
  'historyPage.summary.year.tracks': '今年曲目',
  'historyPage.insights.aria': '收聽洞察',
  'historyPage.insights.title': '收聽洞察',
  'historyPage.insights.comparison.title': '對比上一週期',
  'historyPage.insights.comparison.plays': '播放次數',
  'historyPage.insights.comparison.duration': '收聽時長',
  'historyPage.insights.comparison.tracks': '不同曲目',
  'historyPage.insights.comparison.unavailable': '選擇今天、本週、本月或今年即可查看週期對比。',
  'historyPage.insights.sessions.title': '收聽工作階段',
  'historyPage.insights.sessions.count': '工作階段數',
  'historyPage.insights.sessions.average': '平均時長',
  'historyPage.insights.sessions.longest': '最長工作階段',
  'historyPage.insights.sessions.streak': '最長連續收聽',
  'historyPage.insights.discovery.title': '探索與回流',
  'historyPage.insights.discovery.new': '首次收聽曲目',
  'historyPage.insights.discovery.unique': '不同曲目',
  'historyPage.insights.discovery.returning': '回流曲目',
  'historyPage.insights.discovery.replayed': '重複收聽曲目',
  'historyPage.insights.discovery.rediscovered': '重新發現曲目',
  'historyPage.insights.discovery.repeatRate': '重複播放率',
  'historyPage.insights.days': '{count} 天',
  'historyPage.summary.today.count': '今日播放',
  'historyPage.summary.today.duration': '今日時長',
  'historyPage.summary.today.group': '今日按播放次數排序',
  'historyPage.summary.today.latest': '今日最近播放',
  'historyPage.summary.today.tracks': '今日曲目',
  'historyPage.summary.week.count': '本週播放',
  'historyPage.summary.week.duration': '本週時長',
  'historyPage.summary.week.group': '本週按播放次數排序',
  'historyPage.summary.week.latest': '本週最近播放',
  'historyPage.summary.week.tracks': '本週曲目',
  'historyPage.toolbar.aria': '歷史篩選',
  'route.home.description': '曲庫概覽與最近聆聽。',
  'route.home.label': '主頁',
  'home.artistLeaderboard.aria': '藝人排行榜',
  'home.artistLeaderboard.completionRate': '{rate}% 完播',
  'home.artistLeaderboard.emptyDescription': '播放更多音樂後，這裡會形成你的高頻藝人榜。',
  'home.artistLeaderboard.emptyTitle': '還沒有藝人排行',
  'home.artistLeaderboard.playCount': '{count} 次',
  'home.artistLeaderboard.title': '藝人排行榜',
  'home.count.tracks': '{count} 首',
  'home.date.none': '還沒有記錄',
  'home.date.unknown': '時間未知',
  'home.duration.hoursMinutes': '{hours} 小時 {minutes} 分鐘',
  'home.duration.hoursOnly': '{hours} 小時',
  'home.duration.minutes': '{count} 分鐘',
  'home.duration.zeroMinutes': '0 分鐘',
  'home.error.albumNotFound': '未找到專輯：{album}',
  'home.error.artistNotFound': '未找到藝人：{artist}',
  'home.error.desktopBridgeRandom': '桌面曲庫橋接不可用。請在 ECHO 桌面端生成隨機佇列。',
  'home.error.desktopBridgeRecommend': '桌面曲庫橋接不可用。請在 ECHO 桌面端刷新推薦。',
  'home.error.desktopBridgeView': '桌面曲庫橋接不可用。請在 ECHO 桌面端查看首頁。',
  'home.favoriteAlbums.aria': '你喜歡的專輯',
  'home.favoriteAlbums.emptyDescription': '播放更多專輯後，這裡會按聽過最多次數選出前四張。',
  'home.favoriteAlbums.emptyTitle': '還沒有常聽專輯',
  'home.favoriteAlbums.title': '你喜歡的專輯',
  'home.hero.action.continue': '繼續播放',
  'home.hero.action.viewQueue': '查看佇列',
  'home.hero.aria': '今日回聲',
  'home.hero.defaultTitle': '從你的音樂庫開始播放。',
  'home.hero.description.empty': '匯入音樂後，這裡會變成你的曲庫入口、最近播放和本週聆聽脈衝。',
  'home.hero.description.resume': '接上 {artist} 的「{title}」，或從最近入庫裡挑一張封面開始。',
  'home.hero.kicker': '今日回聲',
  'home.hero.nowPlaying': '正在播放',
  'home.hero.recentSignal': '最近訊號',
  'home.hero.statsAria': '曲庫統計',
  'home.metric.albums': '專輯',
  'home.metric.albumsDetail': '按作品聚合',
  'home.metric.artists': '藝人',
  'home.metric.folders': '資料夾',
  'home.metric.foldersDetail': '最近掃描 {date}',
  'home.metric.openAria': '打開{label}',
  'home.metric.songs': '歌曲',
  'home.metric.songsDetail': '總時長 {duration}',
  'home.month.label': '{month}月',
  'home.nowMeta.empty': '曲庫準備好後會顯示最近內容',
  'home.preferences.aria': '播放偏好',
  'home.recent.emptyAddedDescription': '匯入資料夾後，這裡會顯示最新進入曲庫的封面。',
  'home.recent.emptyAddedTitle': '還沒有最近入庫',
  'home.recent.emptyPlayedDescription': '開始播放後，這裡會出現最近聽過的專輯。',
  'home.recent.emptyPlayedTitle': '還沒有最近播放',
  'home.recent.nextPage': '下一頁',
  'home.recent.prevPage': '上一頁',
  'home.recent.tab.added': '新增於',
  'home.recent.tab.played': '已播放',
  'home.recent.tabsAria': '最近內容',
  'home.recent.title': '最近活動',
  'home.recommend.emptyDescription': '匯入專輯後，這裡會直接鋪滿你的專輯。',
  'home.recommend.emptyTitle': '還沒有可推薦專輯',
  'home.recommend.refresh': '刷新',
  'home.recommend.refreshing': '刷新中',
  'home.recommend.title': '為你推薦',
  'home.signalVisualizer.aria': '音訊視覺化',
  'home.status.loading': '正在整理首頁...',
  'home.week.emptyHint': '播放後，格子會按每週節奏被點亮。',
  'home.week.listenDuration': '聆聽時長',
  'home.week.playCount': '本週播放',
  'home.week.times': '次',
  'home.week.title': '本週回聲',
  'home.weekday.fri': '五',
  'home.weekday.mon': '一',
  'home.weekday.wed': '三',
  'home.weeklyHeatmap.activeWeeks': '{count} 週活躍',
  'home.weeklyHeatmap.aria': '近 {weeks} 週播放熱力圖',
  'home.weeklyHeatmap.dayAria': '{date}，{playCount} 次播放',
  'home.weeklyHeatmap.dayTitle': '{date} · {playCount} 次 · {duration}',
  'route.inbox.description': '每次掃描新增的歌曲。',
  'route.inbox.label': '收件箱',
  'inboxPage.action.addToQueue': '加入佇列',
  'inboxPage.action.generatePlaylist': '生成待聽歌單',
  'inboxPage.batch.latest': '最新掃描',
  'inboxPage.batch.recentAll': '最近全部掃描',
  'inboxPage.batch.selected': '指定掃描',
  'inboxPage.coverIntelligence.coolDetail': '冷色 {score}%',
  'inboxPage.coverIntelligence.disabledDescription': '預設關閉。開啟後 ECHO 才會分析封面主色，並把冷色封面歸入這個唱片箱。',
  'inboxPage.coverIntelligence.disabledEmpty': '封面色彩引擎預設關閉，開啟後才會生成真實冷色封面箱。',
  'inboxPage.coverIntelligence.disabledMessage': '已關閉封面色彩引擎，不會繼續分析封面。',
  'inboxPage.coverIntelligence.enabledMessage': '已開啟封面色彩引擎，ECHO 會輕量分析最近封面並寫入本地快取。',
  'inboxPage.coverIntelligence.offMetric': '預設關閉',
  'inboxPage.coverIntelligence.statusDisabled': '預設關閉',
  'inboxPage.coverIntelligence.statusEnabled': '已開啟',
  'inboxPage.coverIntelligence.toggle': '封面色彩引擎',
  'inboxPage.date.none': '尚無記錄',
  'inboxPage.empty.description': '完成一次曲庫掃描後，這裡會出現新增歌曲。',
  'inboxPage.empty.noMatch': '沒有符合的新歌',
  'inboxPage.empty.title': '還沒有新增記錄',
  'inboxPage.empty.tryFilter': '換個篩選條件再看看。',
  'inboxPage.error.desktopBridgeRead': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'inboxPage.facets.allAlbums': '全部專輯',
  'inboxPage.facets.allArtists': '全部藝人',
  'inboxPage.facets.allFolders': '全部資料夾',
  'inboxPage.facets.aria': '新歌收件箱維度篩選',
  'inboxPage.filter.all': '全部新增',
  'inboxPage.filter.aria': '問題分類篩選',
  'inboxPage.filter.clear': '清空篩選',
  'inboxPage.filter.metadataIssue': '資料異常',
  'inboxPage.filter.missingCover': '缺封面',
  'inboxPage.filter.suspiciousFile': '疑似異常',
  'inboxPage.filter.unknownAlbum': '未知專輯',
  'inboxPage.filter.unknownArtist': '未知藝人',
  'inboxPage.hero.kicker': '新入庫',
  'inboxPage.loading': '正在讀取收件箱...',
  'inboxPage.processing.aria': '本批待處理',
  'inboxPage.processing.pending': '待聽 / 待處理',
  'inboxPage.quality.aria': '入庫品質摘要',
  'inboxPage.quality.coverCompleteness': '封面完整率',
  'inboxPage.quality.metadataCompleteness': '資料完整率',
  'inboxPage.quality.unknownArtistAlbum': '未知藝人 / 專輯',
  'inboxPage.search.aria': '搜尋新歌收件箱',
  'inboxPage.search.placeholder': '搜尋標題、藝人、專輯、路徑',
  'inboxPage.smartCrates.aria': '智慧唱片箱',
  'inboxPage.smartCrates.cleanKicker': '待清理',
  'inboxPage.smartCrates.comeback.description': '最近又開始播放的歌，適合從這裡接著聽下去。',
  'inboxPage.smartCrates.comeback.detail': '最近又在聽',
  'inboxPage.smartCrates.comeback.empty': '多聽幾輪後，ECHO 會找出最近又開始聽的歌。',
  'inboxPage.smartCrates.comeback.title': '最近又在聽',
  'inboxPage.smartCrates.coolCovers.description': '最近帶封面的歌；冷色封面會優先放在這裡。',
  'inboxPage.smartCrates.coolCovers.empty': '還沒有足夠的封面可用於這個唱片箱。',
  'inboxPage.smartCrates.coolCovers.title': '封面冷色系',
  'inboxPage.smartCrates.coolCoversDetail': '封面候選',
  'inboxPage.smartCrates.forgotten.description': '曾經播放過、最近沉下去的歌，適合重新翻出來。',
  'inboxPage.smartCrates.forgotten.detail': '被遺忘',
  'inboxPage.smartCrates.forgotten.empty': '歷史還不夠長，暫時沒有被遺忘的神曲。',
  'inboxPage.smartCrates.forgotten.title': '被遺忘的神曲',
  'inboxPage.smartCrates.forgottenMetric': '可再聽',
  'inboxPage.smartCrates.hifi441.description': '本地無損 44.1kHz 曲目，適合安全模式和獨占輸出檢查。',
  'inboxPage.smartCrates.hifi441.empty': '最近曲庫樣本裡還沒找到 44.1kHz 無損曲目。',
  'inboxPage.smartCrates.hifi441.title': '44.1 kHz 無損',
  'inboxPage.smartCrates.kicker': '智慧唱片箱',
  'inboxPage.smartCrates.lateNight.description': '深夜經常出現的歌，適合夜裡聽。',
  'inboxPage.smartCrates.lateNight.detail': '深夜常聽',
  'inboxPage.smartCrates.lateNight.empty': '還沒有足夠的深夜播放記錄。',
  'inboxPage.smartCrates.lateNight.title': '深夜常聽',
  'inboxPage.smartCrates.loading': '整理中',
  'inboxPage.smartCrates.memoryKicker': '聆聽記憶',
  'inboxPage.smartCrates.playCountMetric': '{count} 次播放',
  'inboxPage.smartCrates.previewEmpty': '這個唱片箱還在等待更多本地記錄。',
  'inboxPage.smartCrates.qualityKicker': '本機音質',
  'inboxPage.smartCrates.queueAdded': '已把「{title}」的 {count} 首加入佇列。',
  'inboxPage.smartCrates.queueEmpty': '這個唱片箱暫時沒有可加入佇列的本地曲目。',
  'inboxPage.smartCrates.queueSelected': '加入這個唱片箱',
  'inboxPage.smartCrates.queueSource': '智慧唱片箱',
  'inboxPage.smartCrates.queueable': '{count} 首可加入佇列',
  'inboxPage.smartCrates.queueableShort': '{count} 首可播',
  'inboxPage.smartCrates.recentMetric': '最近命中',
  'inboxPage.smartCrates.skipCountMetric': '{count} 次跳過',
  'inboxPage.smartCrates.skipReview.description': '經常被跳過的曲目，適合決定保留、修標籤或移出常聽。',
  'inboxPage.smartCrates.skipReview.detail': '跳過待清理',
  'inboxPage.smartCrates.skipReview.empty': '還沒有明顯的常跳過曲目。',
  'inboxPage.smartCrates.skipReview.title': '常跳過待清理',
  'inboxPage.smartCrates.subtitle': '按常聽記錄、音質和封面，自動整理成幾組本地歌單。',
  'inboxPage.smartCrates.title': '智慧唱片箱',
  'inboxPage.smartCrates.trackCountMetric': '{count} 首候選',
  'inboxPage.smartCrates.visualKicker': '封面',
  'inboxPage.smartCrates.waitingMetric': '等待資料',
  'inboxPage.stats.aria': '新歌收件箱摘要',
  'inboxPage.stats.currentResults': '目前結果',
  'inboxPage.stats.newSongs': '新增歌曲',
  'inboxPage.status.all': '全部狀態',
  'inboxPage.status.aria': '處理狀態篩選',
  'inboxPage.status.ignored': '已忽略',
  'inboxPage.status.pending': '待處理',
  'inboxPage.status.processed': '已處理',
  'inboxPage.story.aria': '入庫故事',
  'inboxPage.story.clean': '{summary}。資料看起來很乾淨。',
  'inboxPage.story.empty': '完成一次掃描後，ECHO 會把新增歌曲、專輯和資料問題整理在這裡。',
  'inboxPage.story.kicker': '匯入故事',
  'inboxPage.story.newAlbums': '新增專輯',
  'inboxPage.story.newArtists': '新增藝人',
  'inboxPage.story.summaryAlbums': '{count} 張專輯',
  'inboxPage.story.summaryArtists': '{count} 位藝人',
  'inboxPage.story.summaryTracks': '{scope}新增 {count} 首',
  'inboxPage.story.totalDuration': '總時長',
  'inboxPage.story.warningMetadataIssue': '{count} 首資料異常',
  'inboxPage.story.warningMissingCover': '{count} 首缺封面',
  'inboxPage.story.withWarnings': '{summary}。其中 {warnings}。',
  'inboxPage.toolbar.aria': '新歌收件箱篩選',
  'inboxPage.toolbar.batch': '批次',
  'inboxPage.workbench.aria': '匯入整理工作台',
  'inboxPage.workbench.description': '新增歌曲、缺封面、資料異常和待聽狀態會在這裡集中處理。',
  'inboxPage.workbench.kicker': '匯入工作臺',
  'inboxPage.workbench.title': '匯入整理',
  'route.importFile.label': '匯入檔案',
  'route.importFolder.description': '選擇本機音樂資料夾。',
  'route.importFolder.label': '匯入資料夾',
  'importFolder.hero.note': '此頁面只用於本機曲庫匯入和掃描狀態查看。',
  'importFolder.onboarding.choose.body': '選擇保存音樂的根目錄；以後可以繼續加入多個資料夾。',
  'importFolder.onboarding.choose.title': '匯入目錄',
  'importFolder.onboarding.finish.body': '掃描完成後可以播放、隨機播放，或到資料夾頁檢查專輯與壞檔提示。',
  'importFolder.onboarding.finish.title': '開始使用',
  'importFolder.onboarding.kicker': '第一次匯入',
  'importFolder.onboarding.scan.body': '進度會顯示階段、處理數量和錯誤數量；壞檔會跳過，不會卡住整次匯入。',
  'importFolder.onboarding.scan.title': '觀察掃描',
  'importFolder.onboarding.title': '三步建立你的本機曲庫',
  'nowPlaying.action.openLyrics': '打開歌詞',
  'nowPlaying.description': '目前曲目概覽。歌詞請從底部播放器的麥克風按鈕進入獨立頁面。',
  'nowPlaying.emptyDescription': '從歌曲列表或專輯開始播放後，這裡會顯示目前曲目。',
  'nowPlaying.emptyTitle': '暫無播放',
  'nowPlaying.kicker': '正在播放',
  'nowPlaying.localFile': '本機檔案',
  'nowPlaying.ready': '就緒',
  'nowPlaying.state.idle': '閒置',
  'nowPlaying.state.playing': '播放中',
  'nowPlaying.title': '正在播放',
  'route.liked.label': '喜歡',
  'route.lyrics.description': '歌詞與沉浸播放。',
  'route.lyrics.label': '歌詞',
  'route.lyricsSettings.label': '歌詞設定',
  'lyricsSettings.action.choose': '選擇',
  'lyricsSettings.action.fonts': '字體',
  'lyricsSettings.action.match': '匹配',
  'lyricsSettings.action.music': '音樂',
  'lyricsSettings.action.reset': '重置',
  'lyricsSettings.action.search': '搜尋',
  'lyricsSettings.background.blur': '背景模糊度',
  'lyricsSettings.background.brightness': '背景亮度',
  'lyricsSettings.background.chooseWallpaper': '選擇自訂桌布',
  'lyricsSettings.background.clearWallpaper': '清除自訂桌布',
  'lyricsSettings.background.clearWallpaperHint': '恢復為跟隨主題',
  'lyricsSettings.background.emptyStateEffects': '無歌詞時顯示背景效果',
  'lyricsSettings.background.emptyStateEffectsDescription': '純音樂或暫無歌詞時繼續顯示封面漸層與動態背景；關閉後使用乾淨純色。',
  'lyricsSettings.background.immersiveCoverStyle': '沉浸式專輯封面樣式',
  'lyricsSettings.background.immersiveCoverStyleDescription': '在主歌詞頁鋪滿目前歌曲專輯封面，並自動增強文字對比；不會改動下方儲存的背景模式。',
  'lyricsSettings.background.immersiveCoverGlass': '沉浸封面毛玻璃',
  'lyricsSettings.background.immersiveCoverGlassDescription': '只為歌詞閱讀區域增加柔和毛玻璃，不改變封面鋪底；關閉時保持純封面背景。',
  'lyricsSettings.background.immersiveCoverGlassBlur': '毛玻璃模糊',
  'lyricsSettings.background.musicReactiveVisuals': '音樂響應視覺',
  'lyricsSettings.background.musicReactiveVisualsDescription': '在歌詞頁和桌面歌詞後方加入跟隨電平、瞬態、餘量和封面顏色變化的視覺層；減輕占用播放會自動暫停。',
  'lyricsSettings.background.mode.cover': '跟隨封面',
  'lyricsSettings.background.mode.coverColor': '封面取色',
  'lyricsSettings.background.mode.customWallpaper': '自訂桌布',
  'lyricsSettings.background.mode.theme': '跟隨主題',
  'lyricsSettings.background.modeAria': '歌詞背景模式',
  'lyricsSettings.background.modeDescription': '封面模式會使用目前歌曲封面；封面取色只提取目前封面主色；自訂桌布會儲存到應用程式資料目錄。',
  'lyricsSettings.background.opacity': '背景透明度',
  'lyricsSettings.background.readability': '歌詞可讀性增強',
  'lyricsSettings.background.readabilityDescription': '為沉浸式 MV 背景上的歌詞增加描邊和陰影；不用展開沉浸式 MV 背景設定也可以常駐開關。',
  'lyricsSettings.background.scale': '背景放大',
  'lyricsSettings.background.showControls': '顯示歌詞背景設定',
  'lyricsSettings.background.smartReadable': '智慧可讀顏色',
  'lyricsSettings.background.smartReadableDescription': '根據封面、桌布或 MV 畫面自動選擇高對比文字色，並視需要增加輕遮罩、描邊和陰影。關閉時繼續使用手動歌詞顏色。',
  'lyricsSettings.background.description': '封面沉浸、智慧可讀顏色、桌布和背景調節',
  'lyricsSettings.background.title': '歌詞背景',
  'lyricsSettings.background.tuning': '背景調節',
  'lyricsSettings.background.tuningDescription': '跟隨封面和自訂桌布都會使用這裡的透明度、模糊度和亮度。',
  'lyricsSettings.background.wallpaperSaved': '已儲存到應用桌布目錄',
  'lyricsSettings.visual.pageStyle': '歌詞介面樣式',
  'lyricsSettings.visual.pageStyleDefault': '預設',
  'lyricsSettings.visual.pageStyleDescription': '選擇主歌詞頁的整體版面；切換樣式不會修改其他背景和播放器設定。',
  'lyricsSettings.visual.themeFilter': '主題濾鏡',
  'lyricsSettings.visual.themeFilterDescription': '讓目前主題繼續調整歌詞背景的色彩和裝飾；關閉後，背景只使用歌詞樣式本身的效果。',
  'lyricsSettings.visual.pageStyleEditorial': '紙頁排版',
  'lyricsSettings.visual.pageStyleFolded': '摺頁微光',
  'lyricsSettings.visual.pageStyleRoseVinyl': '薔薇唱片',
  'lyricsSettings.visual.pageStyleCinemaStage': '電影舞台',
  'lyricsSettings.visual.pageStyleKineticPoster': '活字海報',
  'lyricsSettings.visual.pageStyleCoverStage': '封面舞台',
  'lyricsSettings.visual.pageStyleCutBoard': '動態切片',
  'lyricsSettings.visual.roseVinylBackgroundBlur': '薔薇背景模糊',
  'lyricsSettings.visual.roseVinylBackgroundBlurDescription': '只調整薔薇唱片歌詞樣式的封面背景模糊度，不影響通用背景設定。',
  'lyricsSettings.visual.roseVinylGradient': '主題漸層',
  'lyricsSettings.visual.roseVinylGradientDescription': '依照目前主題的強調色與面板色產生漸層；預設關閉。',
  'lyricsSettings.visual.title': '畫面視覺',
  'lyricsSettings.visual.group.background.description': '主題、封面、桌布和背景調節',
  'lyricsSettings.visual.group.background.title': '背景來源',
  'lyricsSettings.visual.group.cover.description': '專輯封面鋪底、毛玻璃和動態視覺',
  'lyricsSettings.visual.group.cover.title': '封面沉浸',
  'lyricsSettings.visual.group.display.description': '歌曲資訊、空狀態和角落控制項',
  'lyricsSettings.visual.group.display.title': '資訊顯示',
  'lyricsSettings.visual.group.layout.description': '歌詞頁版面、薔薇唱片和介面樣式',
  'lyricsSettings.visual.group.layout.title': '頁面樣式',
  'lyricsSettings.visual.group.miniPlayer.description': '歌詞頁底部播放列和自動隱藏',
  'lyricsSettings.visual.group.miniPlayer.title': '迷你播放列',
  'lyricsSettings.visual.group.mv.description': 'MV 背景、歌詞隱藏和畫面調節',
  'lyricsSettings.visual.group.mv.title': 'MV 沉浸背景',
  'lyricsSettings.visual.group.readability.description': '自動選色、文字增強和對比度',
  'lyricsSettings.visual.group.readability.title': '可讀性',
  'lyricsSettings.visual.group.typography.description': '字體、字號、行距和歌詞顏色',
  'lyricsSettings.visual.group.typography.title': '字體與歌詞',
  'lyricsSettings.candidate.allSources': '全部來源',
  'lyricsSettings.candidate.results': '歌詞搜尋結果',
  'lyricsSettings.candidate.risk.high': '差異較大',
  'lyricsSettings.candidate.risk.low': '較可能',
  'lyricsSettings.candidate.risk.medium': '需確認',
  'lyricsSettings.candidate.reason.albumMatch': '專輯匹配',
  'lyricsSettings.candidate.reason.artistExact': '藝人一致',
  'lyricsSettings.candidate.reason.artistMismatch': '藝人不符',
  'lyricsSettings.candidate.reason.autoAccept': '自動採用',
  'lyricsSettings.candidate.reason.candidateOnlyCover': '翻唱需確認',
  'lyricsSettings.candidate.reason.candidateOnlyDuration': '時長需確認',
  'lyricsSettings.candidate.reason.coverIntent': '翻唱候選',
  'lyricsSettings.candidate.reason.durationClose': '時長接近',
  'lyricsSettings.candidate.reason.durationExact': '時長精準',
  'lyricsSettings.candidate.reason.durationTolerated': '時長差可接受',
  'lyricsSettings.candidate.reason.durationMismatch': '時長不符',
  'lyricsSettings.candidate.reason.embeddedTag': '內嵌歌詞',
  'lyricsSettings.candidate.reason.localSidecar': '本地歌詞',
  'lyricsSettings.candidate.reason.rejectedByUser': '已拒絕',
  'lyricsSettings.candidate.reason.syncedDurationSafe': '同步安全',
  'lyricsSettings.candidate.reason.titleExact': '標題一致',
  'lyricsSettings.candidate.reason.titleSimilar': '標題接近',
  'lyricsSettings.candidate.reason.versionConflict': '版本衝突',
  'lyricsSettings.candidate.reason.versionMatch': '版本匹配',
  'lyricsSettings.candidate.sourceFilters': '歌詞來源篩選',
  'lyricsSettings.candidate.type.instrumental': '純音樂',
  'lyricsSettings.candidate.type.lyrics': '歌詞',
  'lyricsSettings.candidate.type.plain': '純文字',
  'lyricsSettings.candidate.type.synced': '逐行同步',
  'lyricsSettings.currentTrack.instrumentalMarked': '已標記為純音樂',
  'lyricsSettings.currentTrack.markInstrumental': '標記為純音樂',
  'lyricsSettings.currentTrack.markInstrumentalHint': '記住目前歌曲並停止自動歌詞匹配',
  'lyricsSettings.currentTrack.rematch': '重新匹配',
  'lyricsSettings.currentTrack.rematchHint': '清理目前快取並重新尋找',
  'lyricsSettings.currentTrack.restartOnApply': '套用歌詞後自動重播音樂',
  'lyricsSettings.currentTrack.restartOnApplyDescription': '預設關閉；開啟後，成功套用目前歌曲歌詞時會從頭播放，避免歌詞時間軸沿用舊進度而不同步。',
  'lyricsSettings.currentTrack.searchHint': '留空則使用目前歌曲資訊',
  'lyricsSettings.currentTrack.searchInput': '搜尋歌詞文字',
  'lyricsSettings.currentTrack.searchLyrics': '搜尋歌詞',
  'lyricsSettings.currentTrack.searchPlaceholder': '歌名 / 演出者 / 關鍵字',
  'lyricsSettings.currentTrack.description': '自動匹配、重新匹配和目前歌曲歌詞操作',
  'lyricsSettings.currentTrack.title': '目前歌曲',
  'lyricsSettings.display.autoOpenCandidatePanel': '自動彈出歌詞選擇欄',
  'lyricsSettings.display.chooseMiniPlayerColor': '選擇底欄顏色',
  'lyricsSettings.display.coverMiniPlayerHint': '會從目前歌曲封面擷取顏色，並自動壓暗成適合按鈕閱讀的玻璃色。',
  'lyricsSettings.display.customColor': '自訂顏色',
  'lyricsSettings.display.defaultMicrosoftYahei': '預設微軟雅黑，可換系統字體',
  'lyricsSettings.display.desktopFont': '桌面歌詞字體',
  'lyricsSettings.display.desktopHideWhenNoLyrics': '無歌詞時隱藏桌面歌詞',
  'lyricsSettings.display.desktopHideWhenNoLyricsDescription': '目前歌曲沒有歌詞或標記為純音樂時，桌面歌詞視窗不顯示「暫無歌詞 / 純音樂」佔位文字。',
  'lyricsSettings.display.desktopLyrics': '桌面歌詞',
  'lyricsSettings.display.desktopLyricsDescription': '開啟後用獨立透明視窗在桌面置頂顯示目前歌詞。',
  'lyricsSettings.display.desktopOpacity': '桌面歌詞透明度',
  'lyricsSettings.display.desktopOpacityDescription': '目前 {opacity}%，調低可減少遮擋。',
  'lyricsSettings.display.desktopPrimaryFontSize': '桌面歌詞主字號',
  'lyricsSettings.display.desktopPrimaryFontSizeDescription': '目前 {size}px，影響外語原文。',
  'lyricsSettings.display.desktopRomanization': '桌面歌詞顯示羅馬音',
  'lyricsSettings.display.desktopSecondaryFontSize': '桌面歌詞翻譯字號',
  'lyricsSettings.display.desktopSecondaryFontSizeDescription': '目前 {size}px，影響中文翻譯和羅馬音。',
  'lyricsSettings.display.desktopTextDirection': '桌面歌詞排版',
  'lyricsSettings.display.desktopTranslation': '桌面歌詞顯示翻譯',
  'lyricsSettings.display.disableMvTrackInfoAutoShow': '關閉 MV 自動顯示歌曲資訊',
  'lyricsSettings.display.cornerControlsAutoHide': '自動隱藏左上角按鈕',
  'lyricsSettings.display.cornerControlsAutoHideDescription': '開啟後滑鼠遠離左上角和右上角時，左上角按鈕會柔和收起；靠近任一上角會重新顯示，預設關閉。',
  'lyricsSettings.display.enableLyrics': '啟用歌詞',
  'lyricsSettings.display.enableLyricsDescription': '關閉後歌詞頁不會載入、搜尋或匹配歌詞。',
  'lyricsSettings.display.hideEmptyState': '隱藏純音樂提示',
  'lyricsSettings.display.hideEmptyStateDescription': '識別本機、線上與接收播放中的純音樂佔位內容，並隱藏歌詞頁中央的「純音樂，請欣賞」和「暫無歌詞」提示，預設開啟。',
  'lyricsSettings.display.hideTrackInfo': '隱藏歌曲資訊',
  'lyricsSettings.display.lockDesktopLyrics': '鎖定桌面歌詞',
  'lyricsSettings.display.lockDesktopLyricsDescription': '鎖定後滑鼠會穿透桌面歌詞，避免擋住桌面操作；回到這裡可解鎖。',
  'lyricsSettings.display.matchThreshold': '歌詞匹配度設定',
  'lyricsSettings.display.matchThresholdDescription': '線上結果達到 {threshold}% 才會自動套用',
  'lyricsSettings.groups.basics': '基礎顯示',
  'lyricsSettings.groups.basicsDescription': '歌詞頁開關、配對規則與迷你底欄',
  'lyricsSettings.groups.presentation': '顯示與外觀',
  'lyricsSettings.groups.presentationDescription': '文字樣式、桌面歌詞與背景效果',
  'lyricsSettings.groups.sources': '來源與同步',
  'lyricsSettings.groups.sourcesDescription': '線上配對、來源優先順序與時間校準',
  'lyricsSettings.display.miniPlayer': '迷你底欄',
  'lyricsSettings.display.miniPlayerAppearance': '外觀',
  'lyricsSettings.display.miniPlayerAutoHide': '自動隱藏迷你底欄',
  'lyricsSettings.display.miniPlayerAutoHideDescription': '開啟後滑鼠離開底欄一段距離會自動收起；移回底部附近會順滑顯示，預設關閉。',
  'lyricsSettings.display.miniPlayerCompactOnIdle': '遠離時收成進度膠囊',
  'lyricsSettings.display.miniPlayerCompactOnIdleDescription': '開啟後迷你底欄在滑鼠遠離時只顯示進度膠囊，靠近後展開完整控制。預設關閉。',
  'lyricsSettings.display.miniPlayerColor': '底欄顏色',
  'lyricsSettings.display.miniPlayerColorMode': '迷你底欄顏色模式',
  'lyricsSettings.display.miniPlayerDefaultDark': '預設深色',
  'lyricsSettings.display.miniPlayerDefaultLight': '預設淺色',
  'lyricsSettings.display.miniPlayerDescription': '開啟後歌詞頁會隱藏預設底部播放欄，改用貼在底部中央的小型控制條。',
  'lyricsSettings.display.miniPlayerBehavior': '自動行為',
  'lyricsSettings.display.miniPlayerHint': '預設開啟；適合想保留歌詞沉浸感、但仍要快速切歌和拖動進度時使用。',
  'lyricsSettings.display.miniPlayerOpacity': '底欄透明度',
  'lyricsSettings.display.miniPlayerPalette': '迷你底欄顏色調色盤',
  'lyricsSettings.display.miniPlayerShortcut': '使用快捷鍵顯示/隱藏迷你底欄',
  'lyricsSettings.display.miniPlayerShortcutDescription': '開啟並綁定後，迷你底欄只回應這個快捷鍵，不再隨滑鼠靠近自動顯示或隱藏。',
  'lyricsSettings.display.miniPlayerTrigger': '觸發方式',
  'lyricsSettings.display.preferUtatenKana': '優先 UtaTen 假名注音',
  'lyricsSettings.display.preferUtatenKanaDescription': '預設關閉；開啟後日文歌詞會嘗試用 UtaTen 的ふりがな替代羅馬音顯示，匹配不到會自動回退。',
  'lyricsSettings.display.resetDesktopPosition': '重置桌面歌詞位置',
  'lyricsSettings.display.resetDesktopPositionHint': '移回螢幕下方中央',
  'lyricsSettings.display.showRomanization': '顯示羅馬音',
  'lyricsSettings.display.showRomanizationDescription': '優先使用歌詞源提供的羅馬音；沒有時會為日文歌詞本機生成。',
  'lyricsSettings.display.showTranslation': '顯示中文翻譯',
  'lyricsSettings.display.showTranslationDescription': '優先顯示歌詞源提供的中文翻譯；沒有翻譯時不顯示額外文字。',
  'lyricsSettings.display.description': '歌詞載入、候選面板、歌曲資訊和空狀態',
  'lyricsSettings.display.title': '歌詞顯示',
  'lyricsSettings.display.useMiniPlayerColor': '使用底欄顏色 {color}',
  'lyricsSettings.drawer.aria': '歌詞設定',
  'lyricsSettings.drawer.close': '關閉歌詞設定',
  'lyricsSettings.drawer.title': '歌詞設定',
  'lyricsSettings.direction.horizontal': '橫排',
  'lyricsSettings.direction.vertical': '直排',
  'lyricsSettings.engine.autoMatch': '自動匹配',
  'lyricsSettings.engine.lyricsTitle': '歌詞名',
  'lyricsSettings.engine.provider': '來源',
  'lyricsSettings.engine.sourceLocal': '本地',
  'lyricsSettings.engine.sourceNetwork': '網路載入',
  'lyricsSettings.engine.sourceSummary': '本地或網路搜尋',
  'lyricsSettings.engine.threshold': '閾值',
  'lyricsSettings.engine.title': '歌詞引擎',
  'lyricsSettings.engine.wordTiming': '逐字歌詞',
  'lyricsSettings.engine.wordTiming.estimated': '按行估算',
  'lyricsSettings.engine.wordTiming.source': '原生逐字',
  'lyricsSettings.engine.wordTiming.unavailable': '未載入',
  'lyricsSettings.engine.wordTiming.unsupported': '不支援',
  'lyricsSettings.font.applySystem': '套用系統字體',
  'lyricsSettings.font.chooseInstalled': '選擇已安裝字體',
  'lyricsSettings.font.custom': '自訂',
  'lyricsSettings.font.desktopOnly': '只影響桌面歌詞',
  'lyricsSettings.font.importDesktop': '匯入桌面歌詞字體',
  'lyricsSettings.font.importFile': '匯入字體檔案',
  'lyricsSettings.font.lyricsOnly': '只影響歌詞頁和歌詞行',
  'lyricsSettings.font.restoreDesktopDefault': '恢復桌面歌詞預設字體',
  'lyricsSettings.font.restoreLyricsDefault': '恢復預設歌詞字體',
  'lyricsSettings.font.system': '系統字體',
  'lyricsSettings.fontPicker.aria': '選擇歌詞字體',
  'lyricsSettings.fontPicker.chooseFile': '從檔案選擇字體',
  'lyricsSettings.fontPicker.close': '關閉歌詞字體選擇',
  'lyricsSettings.fontPicker.preview': '歌詞字體預覽 Aa 你好',
  'lyricsSettings.fontPicker.searchPlaceholder': '搜尋已安裝字體',
  'lyricsSettings.fontPicker.title': '選擇歌詞字體',
  'lyricsSettings.provider.cached': '快取歌詞',
  'lyricsSettings.provider.amllTtml': 'AMLL TTML',
  'lyricsSettings.provider.amllTtmlDescription': '逐詞 TTML 歌詞庫，需要匹配網易雲 ID，預設關閉',
  'lyricsSettings.provider.chineseCatalogDescription': '中文曲庫補充',
  'lyricsSettings.provider.genius': 'Genius',
  'lyricsSettings.provider.kugou': '酷狗音樂',
  'lyricsSettings.provider.kuwo': '酷我音樂',
  'lyricsSettings.provider.local': '本地歌詞',
  'lyricsSettings.provider.subsonic': 'Navidrome / Subsonic 原生歌詞',
  'lyricsSettings.provider.lrclib': 'LRCLIB',
  'lyricsSettings.provider.lrclibDescription': '開放歌詞庫',
  'lyricsSettings.provider.manual': '手動歌詞',
  'lyricsSettings.provider.musixmatch': 'Musixmatch',
  'lyricsSettings.provider.netease': '網易雲音樂',
  'lyricsSettings.provider.none': '未套用歌詞',
  'lyricsSettings.provider.qqmusic': 'QQ 音樂',
  'lyricsSettings.preview.primary': '歌詞預覽',
  'lyricsSettings.preview.secondary': '輔助歌詞行',
  'lyricsSettings.online.autoSearch': '自動匹配歌詞',
  'lyricsSettings.online.autoSearchDescription': '開啟後會從已啟用的本地與網路來源自動查找；只有結果符合匹配條件才會自動套用，沒有匹配則不套用。',
  'lyricsSettings.online.autoSaveSidecar': '自動儲存本地歌詞檔',
  'lyricsSettings.online.autoSaveSidecarDescription': '預設關閉；線上或手動套用的歌詞會儲存為歌曲同名旁掛檔，不覆蓋既有 .lrc / .ttml / .txt。',
  'lyricsSettings.online.deepSearch': '深度優先搜尋',
  'lyricsSettings.online.deepSearchDescription': '開啟後多個線上平台會並行搜尋；較慢來源會在背景繼續，稍後返回的安全結果也可自動套用。',
  'lyricsSettings.online.enable': '啟用線上歌詞匹配',
  'lyricsSettings.online.enableDescription': '僅傳送標題、演出者、專輯和時長用於匹配。',
  'lyricsSettings.online.sources': '歌詞來源',
  'lyricsSettings.online.sourcesDescription': '本地歌詞會一直優先；未勾選的線上來源不會參與自動匹配或重新匹配。',
  'lyricsSettings.online.description': '線上匹配、深搜、歌詞來源順序和旁掛儲存',
  'lyricsSettings.online.title': '線上匹配',
  'lyricsSettings.status.applied': '已套用歌詞',
  'lyricsSettings.status.applying': '套用中',
  'lyricsSettings.status.auto': '自動',
  'lyricsSettings.status.markedInstrumental': '已標記為純音樂',
  'lyricsSettings.status.noCandidates': '未找到歌詞候選',
  'lyricsSettings.status.noPlayingTrack': '沒有正在播放的歌曲',
  'lyricsSettings.status.normal': '正常',
  'lyricsSettings.status.off': '關閉',
  'lyricsSettings.status.on': '開啟',
  'lyricsSettings.status.rematchingCandidates': '正在重新匹配歌詞...',
  'lyricsSettings.status.searchingCandidates': '正在搜尋歌詞候選...',
  'lyricsSettings.style.chooseLyricsColor': '選擇歌詞顏色',
  'lyricsSettings.style.contextOpacity': '上下文透明度',
  'lyricsSettings.style.fontSize': '歌詞字號',
  'lyricsSettings.style.lineMaxChars': '每行字數',
  'lyricsSettings.style.lineMaxCharsValue': '{count}字',
  'lyricsSettings.style.lineSpacing': '歌詞行距',
  'lyricsSettings.style.lyricsColor': '歌詞顏色',
  'lyricsSettings.style.lyricsColorPalette': '歌詞顏色調色盤',
  'lyricsSettings.style.lyricsFont': '歌詞字體',
  'lyricsSettings.style.secondaryFontSize': '輔助歌詞字號',
  'lyricsSettings.style.showControls': '顯示歌詞樣式設定',
  'lyricsSettings.style.showControlsDescription': '包含排版方向、輔助字號、歌詞字號、歌詞行距、上下文透明度和歌詞顏色。',
  'lyricsSettings.style.textDirection': '歌詞排版',
  'lyricsSettings.style.useColor': '使用顏色 {color}',
  'lyricsSettings.textStyle.description': '羅馬音、翻譯、逐字高亮、字體和顏色',
  'lyricsSettings.textStyle.title': '文字與樣式',
  'lyricsSettings.timing.defaultOffset': '新歌詞預設延遲',
  'lyricsSettings.timing.description': '預設延遲、全域偏移和智慧校準',
  'lyricsSettings.timing.globalOffset': '全域延遲',
  'lyricsSettings.timing.restoreDefaults': '恢復歌詞預設值',
  'lyricsSettings.timing.restoreDefaultsHint': '匹配閾值 50% / 延遲 0ms',
  'lyricsSettings.timing.showPerTrackOffset': '顯示本歌曲延遲校準',
  'lyricsSettings.timing.smartAlignment': '智慧歌詞校準',
  'lyricsSettings.timing.smartAlignmentDescription': '高可信時自動儲存目前歌曲延遲；異常漂移只提示換源，可撤回。',
  'lyricsSettings.timing.timelineCorrection': '套用歌詞時間軸校準',
  'lyricsSettings.timing.timelineCorrectionDescription': '全域延遲會影響所有歌曲；本歌曲延遲請在歌詞頁校準條裡調整，會跟隨目前歌曲單獨記憶。',
  'lyricsSettings.timing.title': '匹配與延遲',
  'lyricsSettings.wordHighlight.clarity': '逐字高亮清晰度',
  'lyricsSettings.wordHighlight.clarityDescription': '預設「正常」；調高會讓目前詞未唱到的部分更完整，調低會更有逐字推進感。',
  'lyricsSettings.wordHighlight.description': '開啟後會跟隨歌手演唱進度逐字高亮；優先使用真實逐字時間戳，只有逐行時間時會按行時長估算推進。',
  'lyricsSettings.wordHighlight.title': '逐字歌詞高亮',
  'route.mvSettings.description': 'MV 綁定與本地匹配設定。',
  'route.mvSettings.label': 'MV 設定',
  'mvSettings.action.chooseFile': '匯入本地影片',
  'mvSettings.action.close': '關閉 MV 設定',
  'mvSettings.action.collapseNetwork': '折疊網路來源',
  'mvSettings.action.dragReorder': '拖曳調整優先級',
  'mvSettings.action.dragSource': '拖曳 {provider} 調整優先級',
  'mvSettings.action.expandNetwork': '展開網路來源',
  'mvSettings.action.findLocal': '尋找本地',
  'mvSettings.action.openExternal': '在外部打開已選 MV',
  'mvSettings.action.refresh': '重新整理',
  'mvSettings.action.removeSelected': '移除已選 MV',
  'mvSettings.action.searchNetwork': '搜尋網路 MV',
  'mvSettings.aria.candidates': 'MV 候選列表',
  'mvSettings.aria.drawer': 'MV 設定',
  'mvSettings.aria.engineStatus': 'MV 引擎狀態',
  'mvSettings.aria.maxQuality': '最高畫質 {quality}',
  'mvSettings.aria.maxQualityOptions': '最高畫質選項',
  'mvSettings.aria.networkSources': '網路來源優先級',
  'mvSettings.aria.selectedQuality': '已選 MV 畫質 {quality}',
  'mvSettings.aria.selectedQualityOptions': '已選 MV 畫質選項',
  'mvSettings.badge.credentialsMain': '憑證保留在主行程',
  'mvSettings.badge.proxyOnly': '僅代理存取',
  'mvSettings.binding.selectedMv': '已選 MV',
  'mvSettings.binding.title': 'MV 來源',
  'mvSettings.candidate.external': '外部',
  'mvSettings.candidate.inApp': '應用內',
  'mvSettings.custom.apply': '套用自訂 MV',
  'mvSettings.custom.description': '貼上 YouTube 或 Bilibili 影片連結作為目前 MV。',
  'mvSettings.custom.directDash': '直連串流（DASH）',
  'mvSettings.custom.input': '自訂 MV 連結',
  'mvSettings.custom.placeholder': 'https://youtube.com/watch?v=... 或 BVxxxxxxxx',
  'mvSettings.custom.playing': '正在播放：{provider} - {sourceId}',
  'mvSettings.custom.title': '自訂 MV',
  'mvSettings.custom.videoTitle': '影片標題：{title}',
  'mvSettings.engine.mvTitle': 'MV 標題',
  'mvSettings.engine.network': '網路',
  'mvSettings.engine.quality': '畫質',
  'mvSettings.engine.selected': '已選',
  'mvSettings.engine.title': 'MV 引擎',
  'mvSettings.error.actionFailed': 'MV 操作失敗。請稍後重試，或先換成本地影片/自訂連結。',
  'mvSettings.error.bilibiliBlocked': 'Bilibili 解析被攔截。先開啟 MV 診斷報告，配置 Bilibili Cookie 或外部開啟；原始原因：{reason}',
  'mvSettings.error.noActiveTrackBinding': '沒有可用於 MV 綁定的目前曲庫歌曲',
  'mvSettings.error.noActiveTrackMatching': '沒有可用於 MV 匹配的目前曲庫歌曲',
  'mvSettings.error.noActiveTrackNetworkSearch': '沒有可用於網路 MV 搜尋的目前曲庫歌曲',
  'mvSettings.error.databaseUnavailable': 'MV 資料庫暫時無法讀取，請先到曲庫恢復裡修復資料庫。',
  'mvSettings.error.noLocalCandidates': '沒有找到本地 MV 候選',
  'mvSettings.error.noNetworkCandidates': '沒有找到網路 MV 候選',
  'mvSettings.error.networkFailed': '網路 MV 請求失敗。先檢查代理/網路，或切換來源、降低最高畫質後重試；原始原因：{reason}',
  'mvSettings.general.enabled': '啟用 MV',
  'mvSettings.immersive.blur': '毛玻璃模糊',
  'mvSettings.immersive.autoScale': '自動縮放',
  'mvSettings.immersive.autoScaleDescription': '依照 MV 寬高自動補償縮放，盡量避免背景黑邊。',
  'mvSettings.immersive.brightness': '背景亮度',
  'mvSettings.immersive.description': '開啟後，歌詞頁使用目前 MV 作為背景。',
  'mvSettings.immersive.dragHint': '也可以在歌詞頁空白處拖動調整。',
  'mvSettings.immersive.hideLyrics': 'MV 介面隱藏歌詞',
  'mvSettings.immersive.hideLyricsDescription': '開啟後，切到 MV 介面時不顯示歌詞文字；預設關閉。',
  'mvSettings.immersive.lyricsReadability': '歌詞可讀性增強',
  'mvSettings.immersive.lyricsReadabilityDescription': '為沉浸式 MV 上的歌詞增加描邊和投影。',
  'mvSettings.immersive.overlay': '暗色遮罩',
  'mvSettings.immersive.overlayHint': '越低越接近原片，越高歌詞越清晰。',
  'mvSettings.immersive.positionX': '橫向位置',
  'mvSettings.immersive.positionY': '縱向位置',
  'mvSettings.immersive.reset': '重置沉浸式背景',
  'mvSettings.immersive.title': '沉浸式 MV 背景',
  'mvSettings.immersive.tuning': '沉浸式背景調節',
  'mvSettings.immersive.visualHint': '用於調整沉浸式背景觀感。',
  'mvSettings.immersive.zoom': '背景縮放',
  'mvSettings.network.autoApply': '自動搜尋網路 MV',
  'mvSettings.network.autoApplyThreshold': '自動套用匹配度',
  'mvSettings.network.autoApplyThresholdDescription': '候選達到 {threshold} 以上才會自動套用。',
  'mvSettings.network.autoPreload': '是否預載 MV',
  'mvSettings.network.autoPreloadDescription': '開啟後，只要播放歌曲就會嘗試提前查找並準備目前歌曲的 MV。',
  'mvSettings.network.diagnosticsReport': 'MV 診斷報告',
  'mvSettings.network.diagnosticsReportDescription': '預設關閉；開啟後，MV 頁面無畫面時顯示可複製的本機診斷資訊。',
  'mvSettings.network.maxQuality': '最高畫質',
  'mvSettings.network.preferHighestViewCount': '按播放量匹配',
  'mvSettings.network.preferHighestViewCountDescription': '只在匹配度相同且已通過自動套用門檻的候選中優先播放量。',
  'mvSettings.network.titleOnlySearch': '只用歌曲名搜 MV',
  'mvSettings.network.titleOnlySearchDescription': '開啟後自動搜尋只用歌曲名；關閉後使用歌曲名和歌手。',
  'mvSettings.network.replayAudioOnChange': '切換 MV 後自動重播音樂',
  'mvSettings.network.replayAudioOnChangeDescription': '開啟後，手動選擇或綁定新的 MV 會重新播放目前歌曲，讓新 MV 立即生效。',
  'mvSettings.network.restartAudioOnLoad': 'MV 跟隨音樂進度',
  'mvSettings.network.restartAudioOnLoadDescription': '開啟後，會持續校準 MV 影片時間，不會 seek 或重啟音訊；歌詞同步偏移不會影響 MV。',
  'mvSettings.network.syncMode': '同步模式',
  'mvSettings.network.syncModeDescription': '輕微偏差用變速追平，大偏差才跳轉影片。',
  'mvSettings.network.syncMode.stable': '穩定',
  'mvSettings.network.syncMode.balanced': '均衡',
  'mvSettings.network.syncMode.precise': '精準',
  'mvSettings.network.title': '網路來源',
  'mvSettings.offset.aria': 'MV 同步延遲',
  'mvSettings.offset.description': '正數讓 MV 畫面提前，負數讓 MV 畫面延後；只記住目前歌曲的 MV。',
  'mvSettings.offset.earlier': 'MV 提前 {value}',
  'mvSettings.offset.earlierShort': '提前 {value}',
  'mvSettings.offset.input': '偏移秒數',
  'mvSettings.offset.later': 'MV 延後 {value}',
  'mvSettings.offset.laterShort': '延後 {value}',
  'mvSettings.offset.replay': '重播',
  'mvSettings.offset.replayTitle': '重新播放目前歌曲，並按 MV 起播點同步影片',
  'mvSettings.offset.reset': '重置 MV 延遲',
  'mvSettings.offset.resetShort': '重置',
  'mvSettings.offset.slider': 'MV 同步延遲滑桿',
  'mvSettings.offset.startDescription': '輸入影片裡的起點秒數，最高 600 秒；這首歌會直接從那裡對齊播放。',
  'mvSettings.offset.startInput': 'MV 起播秒數',
  'mvSettings.offset.startTitle': '從第幾秒開始播放',
  'mvSettings.offset.step': '調節步長',
  'mvSettings.offset.title': 'MV 音畫校準',
  'mvSettings.provider.local': '本地',
  'mvSettings.quality.max': '最高',
  'mvSettings.search.input': 'MV 搜尋關鍵字',
  'mvSettings.search.placeholder': '輸入 MV 搜尋關鍵字',
  'mvSettings.search.useCurrentSong': '使用目前歌曲和歌手搜尋',
  'mvSettings.status.auto': '自動',
  'mvSettings.status.noActiveTrack': '沒有目前歌曲',
  'mvSettings.status.none': '無',
  'mvSettings.status.off': '關閉',
  'mvSettings.status.on': '開啟',
  'mvSettings.title': 'MV 設定',
  'route.playlists.label': '播放清單',
  'playlistsPage.action.importFile': '匯入 M3U/M3U8 歌單',
  'playlistsPage.action.newLocal': '新建本地歌單',
  'playlistsPage.daily.subtitle': '網易雲帳號推薦',
  'playlistsPage.daily.title': '每日推薦',
  'playlistsPage.empty.create': '新建歌單',
  'playlistsPage.empty.createFirst': '建立第一個本地歌單',
  'playlistsPage.empty.local': '還沒有本地歌單。',
  'playlistsPage.empty.streamingFiltered': '沒有匹配的串流歌單。',
  'playlistsPage.error.desktopBridgeDailyRecommend': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'playlistsPage.filter.all': '全部',
  'playlistsPage.filter.aria': '歌單類型篩選',
  'playlistsPage.filter.local': '本地',
  'playlistsPage.filter.netease': '網易雲',
  'playlistsPage.filter.qqmusic': 'QQ 音樂',
  'playlistsPage.filter.streaming': '串流',
  'playlistsPage.form.cancel': '取消新建',
  'playlistsPage.form.create': '建立',
  'playlistsPage.form.nameAria': '本地歌單名稱',
  'playlistsPage.form.placeholder': '新建本地歌單',
  'playlistsPage.importStreaming.add': '添加歌單',
  'playlistsPage.importStreaming.adding': '添加中',
  'playlistsPage.importStreaming.placeholder': '貼上網易雲 / QQ 音樂 / Spotify 歌單連結',
  'playlistsPage.importStreaming.title': '添加串流歌單',
  'playlistsPage.prompt.newLocalName': '新建本地歌單名稱',
  'playlistsPage.status.createdLocal': '本地歌單已建立',
  'playlistsPage.status.importedStreamingPlaylist': '已添加歌單：{name}，共 {count} 首',
  'playlistsPage.status.importingStreamingPlaylist': '正在添加串流歌單...',
  'playlistsPage.status.loading': '正在讀取歌單...',
  'playlistsPage.status.noAccountPlaylists': '沒有讀取到{provider}歌單',
  'playlistsPage.status.refreshedDaily': '已刷新每日推薦：{count} 首',
  'playlistsPage.status.refreshingDaily': '正在刷新網易雲每日推薦...',
  'playlistsPage.status.syncedAccountPlaylists': '已同步{provider}歌單：{playlists} 個歌單 / {tracks} 首',
  'playlistsPage.status.syncedAccountPlaylistsPartial': '已同步{provider}歌單：{playlists} 個成功，{failed} 個失敗 / {tracks} 首',
  'playlistsPage.status.syncingAccountPlaylists': '正在同步{provider}歌單...',
  'playlistsPage.status.syncingAccountPlaylistsProgress': '正在同步{provider}歌單：{current}/{total}',
  'playlistsPage.view.aria': '播放清單視圖',
  'playlistsPage.view.local': '本地歌單',
  'playlistsPage.view.streamingFavorites': '串流收藏',
  'route.plugins.description': '本機可編輯外掛。',
  'route.plugins.label': '外掛',
  'route.queue.label': '佇列',
  'queue.action.clear': '清空佇列',
  'queue.action.currentItem': '正在播放',
  'queue.action.dragLabel': '拖曳 {title}',
  'queue.action.dragTitle': '拖曳排序',
  'queue.action.generateFromHistory': '依播放歷史產生佇列',
  'queue.action.generateRandom': '產生隨機佇列',
  'queue.action.generatingHistory': '產生中',
  'queue.action.generatingRandom': '產生中',
  'queue.action.autoFill': '自動補齊佇列',
  'queue.continuousPlay.toggle': '本機智慧電台',
  'queue.continuousPlay.source': '本機智慧電台',
  'queue.continuousPlay.local': '完全在本機產生，不使用 AI 雲端服務',
  'queue.continuousPlay.filling': '正在選擇接下來的歌曲…',
  'queue.continuousPlay.clearPreferences': '重設 {count} 條降頻',
  'queue.continuousPlay.removeRecommendations': '清除推薦曲目',
  'queue.continuousPlay.mode.similar': '相似',
  'queue.continuousPlay.mode.deep-cuts': '冷門收藏',
  'queue.continuousPlay.mode.recently-added': '最近新增',
  'queue.continuousPlay.mode.night': '夜間',
  'queue.continuousPlay.mode.headphone-test': '耳機測試',
  'queue.continuousPlay.reason.same-artist': '同一歌手',
  'queue.continuousPlay.reason.same-album': '同一專輯',
  'queue.continuousPlay.reason.same-genre': '同風格 {value}',
  'queue.continuousPlay.reason.similar-bpm': 'BPM 接近（{value}）',
  'queue.continuousPlay.reason.liked': '已收藏',
  'queue.continuousPlay.reason.rarely-played': '只播放過 {value} 次',
  'queue.continuousPlay.reason.not-heard-recently': '{value} 天沒聽',
  'queue.continuousPlay.reason.recently-added': '{value} 天內新增',
  'queue.continuousPlay.reason.night-favorite': '夜間播放過 {value} 次',
  'queue.continuousPlay.reason.high-completion': '完播率 {value}%',
  'queue.continuousPlay.reason.high-resolution': '{value}',
  'queue.continuousPlay.reason.lossless': '無損 {value}',
  'queue.continuousPlay.reason.unheard': '還沒有完整聽過',
  'queue.continuousPlay.reduce.title': '降低出現頻率',
  'queue.continuousPlay.reduce.artist': '少放這位歌手',
  'queue.continuousPlay.reduce.album': '少放這張專輯',
  'queue.continuousPlay.reduce.genre': '少放這個風格',
  'queue.action.like': '喜歡',
  'queue.action.more': '更多',
  'queue.action.openFolder': '打開所在資料夾',
  'queue.action.play': '立即播放 {title}',
  'queue.action.playNext': '下一首播放 {title}',
  'queue.action.remove': '移除 {title}',
  'queue.action.shuffle': '隨機播放',
  'queue.action.startFromHere': '從這裡開始 {title}',
  'queue.action.startFromHereShort': '從這裡開始',
  'queue.confirm.clear': '清空目前佇列？這會移除 {count} 首待播歌曲，不會刪除音樂檔案。',
  'queue.count': '{count} 首',
  'queue.empty.description': '播放歌曲、加入佇列或選擇下一首播放後，這裡會出現佇列內容。',
  'queue.empty.title': '還沒有接下來播放的歌曲',
  'queue.error.desktopBridge': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'queue.error.noHistoryTracks': '還沒有可用來產生佇列的播放歷史。',
  'queue.error.noRandomTracks': '曲庫裡還沒有可加入隨機佇列的歌曲。',
  'queue.header.kicker': '播放佇列',
  'queue.header.title': '佇列',
  'queue.historySource': '歷史常聽',
  'queue.now.actions': '目前曲目操作',
  'queue.now.emptyDescription': '從歌曲或專輯開始播放後，這裡會顯示目前曲目。',
  'queue.now.emptyTitle': '還沒有正在播放的歌曲',
  'queue.now.kicker': '正在播放',
  'queue.now.quality': '音訊品質',
  'queue.now.sourceFallback': '佇列',
  'queue.now.waitingAudio': '等待音訊資訊',
  'queue.nextPreview.empty': '目前佇列沒有下一首。',
  'queue.nextPreview.kicker': '下一首',
  'queue.nextPreview.repeatOneDetail': '單曲循環已開啟，會繼續播放目前歌曲。',
  'queue.nextPreview.shuffleDetail': '會從 {scope} 中挑選，並避開最近 {count} 首。',
  'queue.nextPreview.shuffleTitle': '隨機播放已開啟',
  'queue.nextPreview.trackDetail': '{artist} · 來自 {source}',
  'queue.notice.addedToNext': '已加入下一首',
  'queue.notice.addedToTail': '已加入佇列尾端',
  'queue.notice.nextDetail': '已經排到目前播放後面',
  'queue.notice.removedMatches': '已從佇列移除 {count} 首',
  'queue.notice.tailDetail': '已經排到佇列末尾',
  'queue.quality.unknown': '未知',
  'queue.randomSource': '隨機佇列',
  'queue.repeat.all': '佇列',
  'queue.repeat.mode': '循環模式',
  'queue.repeat.off': '關閉',
  'queue.repeat.one': '單曲',
  'queue.tools': '佇列工具',
  'queue.upNext.kicker': '接下來',
  'queue.upNext.title': '接下來播放',
  'queue.upNext.waitingCount': '{count} 首等待',
  'queue.unknownAlbum': '未知專輯',
  'queue.unknownArtist': '未知演出者',
  'route.remote.label': '網路硬碟 / 遠端',
  'route.settings.label': '設定',
  'route.songs.label': '曲目',
  'songs.action.clearList': '清空列表',
  'songs.action.openRecovery': '前往復原助手',
  'songs.duplicatesOnly': '只看重複歌曲',
  'songs.osuOnly': 'osu!',
  'songs.osuHiFi.action.play': '播放高音質音源',
  'songs.osuHiFi.action.matching': '匹配中...',
  'songs.osuHiFi.action.title': '按歌名和歌手精確匹配已登入串流帳號裡的無損音源',
  'songs.osuHiFi.toggle.label': '高音質音源',
  'songs.osuHiFi.toggle.off': '關',
  'songs.osuHiFi.toggle.on': '開',
  'songs.osuHiFi.toggle.title': '開啟後 osu! 歌曲會優先用網易雲精確匹配無損音源；關閉後播放本機檔案',
  'songs.osuHiFi.status.enabled': 'osu! 高音質音源已開啟，將優先使用網易雲匹配。',
  'songs.osuHiFi.status.disabled': 'osu! 高音質音源已關閉，將播放本機檔案。',
  'songs.osuHiFi.status.matching': '正在匹配無損音源：{title} - {artist}',
  'songs.osuHiFi.status.playing': '已播放 {provider} 無損音源：{title} - {artist}',
  'songs.osuHiFi.queueSource': 'osu! 高音質 / {provider}',
  'songs.osuHiFi.error.noTarget': '目前 osu! 列表沒有可匹配的歌曲。',
  'songs.osuHiFi.error.bridgeUnavailable': '串流橋接不可用，請在 ECHO 桌面端重試。',
  'songs.osuHiFi.error.noAccount': '請先在設定裡登入支援播放的串流帳號，然後再匹配無損音源。',
  'songs.osuHiFi.error.notFound': '沒有找到歌名和歌手完全一致的無損音源：{title} - {artist}',
  'songs.osuHiFi.error.playFailed': '無損音源匹配或播放沒有成功。',
  'songs.filter.button': '篩選',
  'songs.filter.menuAria': '歌曲篩選',
  'songs.filter.section': '篩選',
  'songs.filter.audioFormat': '音訊格式',
  'songs.filter.audioFormat.all': '不限格式',
  'songs.filter.audioFormat.other': '其他格式',
  'songs.filter.sampleRate': '取樣率',
  'songs.filter.sampleRate.all': '不限取樣率',
  'songs.filter.audioFormat.sampleRate44100': '44.1 kHz',
  'songs.filter.audioFormat.sampleRate48000': '48 kHz',
  'songs.filter.audioFormat.sampleRate88200': '88.2 kHz',
  'songs.filter.audioFormat.sampleRate96000': '96 kHz',
  'songs.filter.audioFormat.sampleRate176400': '176.4 kHz',
  'songs.filter.audioFormat.sampleRate192000': '192 kHz',
  'songs.filter.audioFormat.sampleRate352800': '352.8 kHz',
  'songs.filter.audioFormat.sampleRate384000': '384 kHz',
  'songs.filter.audioFormat.dsd': 'DSD 檔案',
  'songs.filter.audioFormat.otherSampleRate': '其他取樣率',
  'songs.error.albumNotFound': '未找到專輯：{album}',
  'songs.error.artistNotFound': '未找到藝人：{artist}',
  'songs.error.desktopBridgeClear': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'songs.error.desktopBridgeFileActions': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'songs.error.desktopBridgeLyricsCache': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'songs.error.desktopBridgeMaintain': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'songs.error.desktopBridgePlay': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'songs.error.desktopBridgePlaylists': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'songs.error.desktopBridgeRead': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'songs.error.desktopBridgeVersions': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'songs.error.osuBeatmapsetMissing': '找不到這首歌對應的 osu! beatmapset 連結。',
  'songs.empty.action.importFile': '匯入音訊檔案',
  'songs.empty.action.importFolder': '匯入音樂資料夾',
  'songs.importHint': '也可以直接把音樂檔案或資料夾拖入視窗。支援 MP3、FLAC、WAV、ALAC、AAC、OPUS、OGG、APE、WV、DSF、DFF、CUE 等格式，更多格式會自動識別。',
  'songs.queueSource.local': '歌曲列表',
  'songs.queueSource.remote': '網盤歌曲',
  'songs.search.placeholder': '搜尋曲目 / 藝人 / 專輯...',
  'songs.sort.album': '按專輯',
  'songs.sort.audioSpecAsc': '音訊規格（低到高）',
  'songs.sort.audioSpecDesc': '音訊規格（高到低）',
  'songs.sort.artist': '按藝人',
  'songs.sort.artistAlbum': '按藝人 / 專輯',
  'songs.sort.bpmAsc': 'BPM（慢到快）',
  'songs.sort.bpmDesc': 'BPM（快到慢）',
  'songs.sort.createdAsc': '建立時間（正序）',
  'songs.sort.createdDesc': '建立時間（倒序）',
  'songs.sort.default': '預設排序',
  'songs.sort.durationAsc': '時長（短到長）',
  'songs.sort.durationDesc': '時長（長到短）',
  'songs.sort.fileModifiedAsc': '檔案修改時間（舊到新）',
  'songs.sort.fileModifiedDesc': '檔案修改時間（新到舊）',
  'songs.sort.group.audio': '音訊',
  'songs.sort.group.browse': '瀏覽',
  'songs.sort.group.library': '曲庫',
  'songs.sort.group.listening': '聆聽',
  'songs.sort.lastPlayed': '最近播放',
  'songs.sort.menuAria': '歌曲排序',
  'songs.sort.multi.label': '多選排序',
  'songs.sort.multi.hint': '依選取順序決定優先順序',
  'songs.sort.multiCount': '{count} 項排序',
  'songs.sort.playCountAsc': '播放次數最少',
  'songs.sort.playCountDesc': '播放次數最多',
  'songs.sort.qualityAsc': '歌曲品質/大小（小到大）',
  'songs.sort.qualityDesc': '歌曲品質/大小（大到小）',
  'songs.sort.random': '隨機排序',
  'songs.sort.recent': '最近更新',
  'songs.sort.titleAsc': '歌曲名（A-Z）',
  'songs.sort.titleDesc': '歌曲名（Z-A）',
  'songs.sort.yearAsc': '發行年份（最早）',
  'songs.sort.yearDesc': '發行年份（最新）',
  'songs.trackList.aria': '歌曲列表',
  'songs.trackList.empty': '沒有可顯示的歌曲。匯入音樂資料夾後，這裡會顯示曲庫列表。',
  'route.streaming.description': '串流音樂來源。',
  'route.streaming.label': '串流',
  'streaming.empty.notFoundAlbum': '找不到匹配的專輯。',
  'streaming.empty.notFoundArtist': '找不到匹配的歌手。',
  'streaming.empty.notFoundPlaylist': '找不到匹配的歌單。',
  'streaming.empty.notFoundTrack': '找不到匹配的串流歌曲。',
  'streaming.empty.searchHint': '輸入關鍵字開始搜尋。播放時才會解析真實位址，佇列不會保存臨時 URL。',
  'streaming.hero.currentProvider': '目前來源',
  'streaming.hero.description': '搜尋線上曲庫，播放前臨時解析音訊位址。',
  'streaming.hero.meterAria': '目前串流狀態',
  'streaming.hero.preparingSearch': '準備搜尋',
  'streaming.hero.title': '串流音樂',
  'streaming.provider.available': '可用',
  'streaming.provider.disabled': '未啟用',
  'streaming.provider.loggedIn': '已登入 {name}',
  'streaming.provider.notLoggedIn': '未登入',
  'streaming.providers.aria': '串流平台',
  'streaming.quality.high': '高音質',
  'streaming.quality.highDescription': '優先 320kbps',
  'streaming.quality.hires': 'Hi-Res',
  'streaming.quality.hiresDescription': '平台可用時啟用',
  'streaming.quality.label': '音質',
  'streaming.quality.lossless': '無損',
  'streaming.quality.losslessDescription': '優先 FLAC',
  'streaming.quality.max': 'Max',
  'streaming.quality.maxDescription': '預設最高音質',
  'streaming.quality.menuAria': '音質',
  'streaming.quality.standard': '標準',
  'streaming.quality.standardDescription': '相容性更好',
  'streaming.quality.switchFailed': '切換音質失敗',
  'streaming.quality.switched': '已切換音質：{quality}',
  'streaming.result.count': '{count} 項結果',
  'streaming.result.searching': '搜尋中',
  'streaming.result.searchingEllipsis': '搜尋中...',
  'streaming.search.placeholder': '搜尋歌曲、歌手、專輯',
  'streaming.tab.album': '專輯',
  'streaming.tab.artist': '歌手',
  'streaming.tab.playlist': '歌單',
  'streaming.tab.track': '單曲',
  'streaming.tabs.aria': '結果類型',
  'settings.general.language.title': '顯示語言',
  'settings.general.language.description': '選擇選單、應用程式內設定與系統對話框的顯示語言。',
  'settings.about.version.title': '版本號',
  'settings.about.version.description': '目前安裝的 ECHO 版本。',
  'settings.about.pro.title': 'ECHO Pro',
  'settings.about.pro.description': '贊助後可獲得 ECHO Pro，解鎖連接功能（HQPlayer / AirPlay / 廣播），並獲得最新支援。',
  'settings.about.pro.descriptionUnlocked': '已解鎖 ECHO Pro，感謝你支持 ECHO。連接功能（HQPlayer / AirPlay / 廣播）已為你開放。',
  'settings.about.pro.action': '贊助 / 解鎖 ECHO Pro',
  'settings.about.pro.actionUnlocked': '已解鎖，感謝支持',
  'settings.about.updates.title': '自動檢查更新',
  'settings.about.updates.description': '啟動後自動檢查 GitHub Release，發現更新時會在左上角提醒，但不會自動下載。點擊左上角更新圖示後才會下載；關閉後不再檢查或提醒。',
  'settings.about.updates.currentVersion': '目前版本',
  'settings.about.updates.latestVersion': '最新版本',
  'settings.about.updates.status': '狀態',
  'settings.about.updates.lastChecked': '上次檢查',
  'settings.about.updates.autoCheck': '自動檢查更新',
  'settings.about.updates.action.check': '檢查更新',
  'settings.about.updates.action.checking': '檢查中...',
  'settings.about.updates.action.afdian': '愛發電',
  'settings.about.updates.action.history': '查看歷史更新日誌',
  'settings.about.updates.action.qq': '加入 QQ 群聊',
  'settings.about.updates.action.discord': '加入 Discord',
  'settings.about.updates.progress.ready': '下載完成，準備安裝',
  'settings.about.updates.progress.downloading': '正在下載更新',
  'settings.about.updates.progress.aria': 'OTA 更新下載進度 {percent}%',
  'settings.about.updates.releaseNotes': '更新日誌',
  'settings.about.updates.releaseNotesPending': '更新日誌稍後顯示...',
  'settings.about.updates.releaseNotesEmpty': '更新日誌會在 GitHub Release 回傳 release notes 後顯示。',
  'settings.about.updates.state.idle': '待檢查',
  'settings.about.updates.state.checking': '正在檢查',
  'settings.about.updates.state.available': '發現新版本',
  'settings.about.updates.state.downloading': '下載中',
  'settings.about.updates.state.downloaded': '下載完成，正在安裝',
  'settings.about.updates.state.notAvailable': '已是最新',
  'settings.about.updates.state.error': '檢查失敗',
  'settings.about.updates.state.disabled': '已關閉',
  'settings.general.dataBackup.title': '自動資料備份',
  'settings.general.dataBackup.description': '備份設定、曲庫索引、播放記憶、帳號本機狀態、桌布、封面快取與中繼資料；備份前會檢查曲庫資料庫，損壞資料會被拒絕。',
  'settings.general.dataBackup.status.enabled': '自動備份已開啟',
  'settings.general.dataBackup.status.disabled': '自動備份未開啟',
  'settings.general.dataBackup.hint.directoryReady': '目錄已設定，可手動備份或開啟定期備份',
  'settings.general.dataBackup.hint.chooseDirectory': '請先選擇備份目錄',
  'settings.general.dataBackup.frequency.aria': '自動備份週期',
  'settings.general.dataBackup.frequency.days': '{days} 天',
  'settings.general.dataBackup.frequency.monthly': '每月',
  'settings.general.dataBackup.meta.directory': '目錄',
  'settings.general.dataBackup.meta.lastBackup': '上次備份',
  'settings.general.dataBackup.meta.nextRun': '下次執行',
  'settings.general.dataBackup.meta.notSet': '未設定',
  'settings.general.dataBackup.meta.noneYet': '暫無自動備份',
  'settings.general.dataBackup.meta.nextRunPending': '選擇目錄並開啟後生效',
  'settings.general.dataBackup.meta.atPath': '{time} · {path}',
  'settings.general.dataBackup.progress.preparing': '準備中',
  'settings.general.dataBackup.progress.snapshot': '建立快照',
  'settings.general.dataBackup.progress.scanning': '掃描資料',
  'settings.general.dataBackup.progress.writing': '寫入備份',
  'settings.general.dataBackup.progress.finalizing': '收尾中',
  'settings.general.dataBackup.progress.completed': '已完成',
  'settings.general.dataBackup.progress.failed': '失敗',
  'settings.general.dataBackup.progress.measuring': '統計中',
  'settings.general.dataBackup.progress.waiting': '等待寫入',
  'settings.general.dataBackup.action.chooseDirectory': '選擇目錄',
  'settings.general.dataBackup.action.choosingDirectory': '選擇中...',
  'settings.general.dataBackup.action.backupNow': '立即備份',
  'settings.general.dataBackup.action.backingUp': '備份中...',
  'settings.general.dataBackup.action.importBackup': '匯入備份',
  'settings.general.dataBackup.action.importingBackup': '匯入中...',
  'settings.general.dataBackup.action.openDirectory': '開啟目錄',
  'settings.general.dataPackage.title': '一鍵匯出 / 遷移 ECHO 資料包',
  'settings.general.dataPackage.description': '匯出設定、曲庫索引、歌單快照、封面快取路徑與帳號狀態說明。不會複製音樂檔案，也不會匯出登入金鑰。',
  'settings.general.dataPackage.action.export': '匯出 ECHO 資料包',
  'settings.general.dataPackage.action.exporting': '打包中...',
  'settings.general.dataPackage.action.recovery': '恢復入口',
  'settings.general.dataPackage.note': '恢復前請先在危險區建立健康快照；遷移包裡的 RESTORE.md 會說明每個檔案的用途。',
  'settings.header.searchEmpty': '沒有符合的設定',
  'settings.header.searchClear': '清除搜尋',
  'settings.header.searchPlaceholder': '搜尋設定...',
  'settings.status.aria': '設定狀態概覽',
  'settings.status.asio.active': 'ASIO 正在輸出',
  'settings.status.asio.available': '偵測到 {count} 個 ASIO 裝置',
  'settings.status.asio.checking': '正在偵測輸出裝置',
  'settings.status.asio.missing': '未偵測到 ASIO 裝置',
  'settings.status.asio.title': 'ASIO',
  'settings.status.asio.unsupported': '目前平台不支援 ASIO',
  'settings.status.discord.checking': '正在讀取 Discord 狀態',
  'settings.status.discord.connected': '已連接 Rich Presence',
  'settings.status.discord.disabled': 'Discord 狀態已關閉',
  'settings.status.discord.error': '連接異常：{error}',
  'settings.status.discord.notConnected': '已啟用，但尚未連接',
  'settings.status.discord.notRunning': 'Discord 未執行',
  'settings.status.discord.title': 'Discord',
  'settings.status.downloadTools.checking': '正在檢查 yt-dlp / ffmpeg',
  'settings.status.downloadTools.locked': '下載功能未解鎖',
  'settings.status.downloadTools.missing': '缺少 {tools}',
  'settings.status.downloadTools.ready': '下載工具可用',
  'settings.status.downloadTools.title': '下載工具',
  'settings.status.downloadTools.unavailable': '執行環境暫不可檢查',
  'settings.status.eq.active': 'EQ 正在改變聲音：{preset}',
  'settings.status.eq.checking': '等待播放鏈路狀態',
  'settings.status.eq.clipping': 'DSP 有削波風險',
  'settings.status.eq.custom': '自訂',
  'settings.status.eq.dspActive': 'DSP 正在改變聲音',
  'settings.status.eq.off': 'EQ / DSP 未介入',
  'settings.status.eq.title': 'EQ / DSP',
  'settings.nav.general.label': '一般',
  'settings.nav.general.description': '語言、視窗與基礎行為',
  'settings.nav.experimental.label': '實驗室',
  'settings.nav.experimental.description': '可能提升效能，也可能帶來奇怪 BUG',
  'settings.performance.title': '效能',
  'settings.performance.description': '降低播放負載、專輯牆虛擬化、本機直讀都在這裡統一管理。專輯牆虛擬化預設開啟；另外兩項預設關閉，不確定時保持預設即可，本機直讀不適用時會按原路徑退回。',
  'settings.nav.playback.label': '播放',
  'settings.nav.playback.description': '輸出、緩衝與播放控制',
  'settings.nav.accounts.label': '帳號',
  'settings.nav.accounts.description': '音樂服務、進階帳號與憑證',
  'settings.nav.integrations.label': '連動',
  'settings.nav.integrations.description': 'Discord、Windows 與行動裝置',
  'settings.nav.steamPresence.label': 'Steam 設定',
  'settings.nav.steamPresence.description': '集中管理 Rich Presence、成就進度、個人統計與排行榜',
  'settings.nav.remote.label': '網路硬碟 / 遠端',
  'settings.nav.appearance.label': '外觀',
  'settings.nav.accessibility.label': '無障礙',
  'settings.nav.accessibility.description': '視覺、鍵盤、螢幕閱讀器',
  'settings.nav.library.label': '媒體庫',
  'settings.nav.plugins.label': '外掛',
  'settings.nav.plugins.description': '本機擴充、權限和腳本',
  'settings.plugins.card.title': '本地外掛',
  'settings.plugins.card.description': '外掛檔案預設放在應用程式資料目錄的 plugins 資料夾，可直接編輯設定和指令碼。',
  'settings.plugins.meta.runtime': '執行方式',
  'settings.plugins.meta.runtimeValue': '本地沙箱',
  'settings.plugins.meta.defaultState': '預設狀態',
  'settings.plugins.meta.defaultStateValue': '手動啟用',
  'settings.plugins.meta.permissions': '權限',
  'settings.plugins.meta.permissionsValue': '啟用時確認',
  'settings.plugins.meta.playbackSafety': '播放安全',
  'settings.plugins.meta.playbackSafetyValue': '不會插入播放音訊處理',
  'settings.plugins.action.openPage': '打開外掛頁',
  'settings.plugins.action.openDirectory': '打開外掛目錄',
  'settings.plugins.action.createExample': '新建示例外掛',
  'settings.plugins.action.openDocs': '查看外掛文件',
  'settings.plugins.note': '啟用、停用和日誌在外掛頁管理；這裡只放入口和安全說明。',
  'settings.nav.about.label': '關於',
  'settings.nav.advancedCustom.label': '進階設定',
  'settings.nav.advancedCustom.description': '實驗功能、效能與低頻選項',
  'settings.nav.danger.label': '危險操作',
  'settings.danger.intro.title': '這些操作只改 ECHO 本機資料',
  'settings.danger.intro.body': '只改本機資料。音樂檔案不會被刪除。不可逆操作會先確認，並要求輸入確認詞。',
  'settings.danger.keepFiles': '音樂檔案不會被刪除',
  'settings.danger.confirm.cancel': '取消',
  'settings.danger.confirm.run': '確認執行',
  'settings.danger.confirm.wordLabel': '輸入確認詞「{word}」',
  'settings.danger.database.group.inspect': '檢查與保護',
  'settings.danger.database.group.recover': '復原',
  'settings.danger.database.events.title': '最近維護',
  'settings.danger.database.events.empty': '還沒有維護紀錄',
  'settings.danger.database.event.manual-repair': '手動重建空庫',
  'settings.danger.database.event.manual-delete': '刪除曲庫資料庫',
  'settings.danger.database.event.manual-restore': '復原健康快照',
  'settings.danger.database.event.manual-scrub-quarantined': '修復隔離庫',
  'settings.danger.database.event.manual-discard-quarantined': '歸檔問題曲目',
  'settings.danger.database.event.startup-protected': '啟動時已保護',
  'settings.danger.database.event.startup-poisoned': '啟動時已隔離',
  'settings.danger.database.event.startup-auto-repair': '啟動自動修復',
  'settings.danger.database.event.scan-health-failed': '掃描健康檢查失敗',
  'settings.danger.database.event.scan-auto-restore': '掃描後自動復原',
  'settings.danger.duplicates.confirmWord': '清理重複歌曲',
  'settings.danger.duplicates.confirmMessage': '會把掃描結果中的 {tracks} 首低評分重複版本移入資源回收筒，並從曲庫索引移除；每組保留評分最高的一首。',
  'settings.danger.duplicates.keep': '音樂檔案會優先進入資源回收筒，不會直接永久刪除',
  'settings.danger.hardwareAcceleration.status.enabled': '已開啟',
  'settings.danger.hardwareAcceleration.status.disabled': '已關閉',
  'settings.danger.hardwareAcceleration.restartHint': '切換後需要完全結束並重新啟動 ECHO 才會生效',
  'settings.eq.action.addFilter': '新增濾波器',
  'settings.eq.action.autoPreamp': '自動 {value}',
  'settings.eq.action.delete': '刪除',
  'settings.eq.action.deleteFilter': '移除濾波器',
  'settings.eq.action.holdBypass': '按住旁路 EQ',
  'settings.eq.action.holdOriginal': '按住聽原聲',
  'settings.eq.action.hideAdvanced': '隱藏 PEQ 控制台',
  'settings.eq.action.importPreset': '匯入預設 / APO',
  'settings.eq.action.exportApoGraphicEqPreset': '匯出 GraphicEQ',
  'settings.eq.action.exportApoPreset': '匯出 APO',
  'settings.eq.action.exportPreset': '匯出 ECHO 預設',
  'settings.eq.action.pasteApoPreset': '貼上 APO',
  'settings.eq.action.resetChannelBalance': '重置聲道平衡',
  'settings.eq.action.resetDelaysOnly': '重置延遲',
  'settings.eq.action.save': '儲存',
  'settings.eq.action.showAdvanced': 'PEQ 控制台',
  'settings.eq.autoGain.adjustment': 'Auto {value}',
  'settings.eq.autoGain.status': '狀態',
  'settings.eq.autoGain.status.clipping': '削波中',
  'settings.eq.autoGain.status.holding': '維持中',
  'settings.eq.autoGain.status.idle': '閒置',
  'settings.eq.autoGain.status.recovering': '恢復中',
  'settings.eq.autoGain.status.reducing': '降低中',
  'settings.eq.autoGain.toggle': '自動增益',
  'settings.eq.room.active': '已啟用',
  'settings.eq.room.channelMode': '聲道',
  'settings.eq.room.clear': '清除',
  'settings.eq.room.disable': '停用 FIR',
  'settings.eq.room.empty': '未載入',
  'settings.eq.room.enable': '啟用 FIR',
  'settings.eq.room.error': '錯誤',
  'settings.eq.room.error.invalidImpulse': 'IR 無效',
  'settings.eq.room.error.invalidWav': 'WAV 無效',
  'settings.eq.room.error.missingFile': 'IR 檔案遺失',
  'settings.eq.room.error.missingIr': '未載入 IR',
  'settings.eq.room.error.tooLong': 'IR 過長',
  'settings.eq.room.import': '匯入 IR',
  'settings.eq.room.ir': 'IR',
  'settings.eq.room.loaded': 'Loaded',
  'settings.eq.room.sampleRate': '取樣率',
  'settings.eq.room.short': 'FIR',
  'settings.eq.room.tapCount': 'Taps',
  'settings.eq.room.title': '房間校正',
  'settings.eq.room.trim': 'Trim',
  'settings.eq.analyzer.overlayAria': 'EQ 即時頻譜疊加',
  'settings.eq.analyzer.input': 'Input',
  'settings.eq.analyzer.mode': '頻譜模式',
  'settings.eq.analyzer.postEq': 'Post EQ',
  'settings.eq.analyzer.status': '狀態',
  'settings.eq.analyzer.status.live': 'Live',
  'settings.eq.analyzer.status.noSignal': 'No signal',
  'settings.eq.analyzer.status.off': 'Off',
  'settings.eq.analyzer.status.priming': 'Priming',
  'settings.eq.analyzer.toggle': '頻譜',
  'settings.eq.band.console': '選中頻段控制台',
  'settings.eq.band.enabledShort': '啟用',
  'settings.eq.band.filters': '濾波器',
  'settings.eq.band.filterStack': 'APO 濾波器鏈',
  'settings.eq.band.gainFixed': '此類型不使用增益',
  'settings.eq.band.qPresets': 'Q 快捷值',
  'settings.eq.band.qPresetWide': 'Wide',
  'settings.eq.band.qPresetNormal': 'Normal',
  'settings.eq.band.qPresetNarrow': 'Narrow',
  'settings.eq.band.matrix': 'PEQ 頻段矩陣',
  'settings.eq.band.modeFree': '自由頻率',
  'settings.eq.band.modeStandard': '標準頻點',
  'settings.eq.bitPerfect.channelDisabled': 'DSP 已啟用：bit-perfect 已關閉。',
  'settings.eq.bitPerfect.disabled': 'DSP 已啟用：bit-perfect 已關閉{reason}。',
  'settings.eq.bitPerfect.readyPath': '可保留 bit-perfect 路徑。',
  'settings.eq.bitPerfect.sourceBoth': 'EQ + 聲道平衡',
  'settings.eq.bitPerfect.sourceChannel': '聲道平衡',
  'settings.eq.bitPerfect.sourceEq': 'EQ',
  'settings.eq.bitPerfect.sourceRoom': '房間校正',
  'settings.eq.channel.active': '啟用',
  'settings.eq.channel.center': '置中',
  'settings.eq.channel.constantPower': '恆定功率',
  'settings.eq.channel.description': 'Balance 用於左右偏移；L/R Gain 校正響度；L/R Delay 校正到達時間；Mono Sum 和 Invert 用於監聽檢查。',
  'settings.eq.channel.group.timing': 'Timing',
  'settings.eq.channel.invertLeft': '左聲道反相',
  'settings.eq.channel.invertRight': '右聲道反相',
  'settings.eq.channel.leftDelay': '左延遲',
  'settings.eq.channel.mono.off': '關閉',
  'settings.eq.channel.mono.sum': '合併',
  'settings.eq.channel.rightDelay': '右延遲',
  'settings.eq.channel.title': '聲道平衡',
  'settings.eq.channel.wizard.apply': '套用測量校準',
  'settings.eq.channel.wizard.clear': '清空測量',
  'settings.eq.channel.wizard.empty': '等待測量',
  'settings.eq.channel.wizard.imageLeft': '聲像偏左',
  'settings.eq.channel.wizard.imageRight': '聲像偏右',
  'settings.eq.channel.wizard.leftDistance': '左距離',
  'settings.eq.channel.wizard.leftLouder': '左邊較響',
  'settings.eq.channel.wizard.leftSpl': '左 SPL',
  'settings.eq.channel.wizard.monitor': '空間校準監聽',
  'settings.eq.channel.wizard.nudge': '空間校準微調',
  'settings.eq.channel.wizard.previewCenter': '中心檢查',
  'settings.eq.channel.wizard.previewLeft': '左檢查',
  'settings.eq.channel.wizard.previewRight': '右檢查',
  'settings.eq.channel.wizard.ready': '可套用',
  'settings.eq.channel.wizard.rightDistance': '右距離',
  'settings.eq.channel.wizard.rightLouder': '右邊較響',
  'settings.eq.channel.wizard.rightSpl': '右 SPL',
  'settings.eq.channel.wizard.title': '空間校準嚮導',
  'settings.eq.comfort.direct': '原聲直通',
  'settings.eq.comfort.directDetail': 'DSP 關閉時不壓低音量、不改動取樣，適合放心聽原聲。',
  'settings.eq.comfort.risk': '需要留意電平',
  'settings.eq.comfort.riskDetail': '偵測到潛在削波或保護動作，建議先預留 -6 dB Headroom。',
  'settings.eq.comfort.tuned': '正在修飾聲音',
  'settings.eq.comfort.tunedDetail': '只處理已啟用的 DSP 模組；關閉後會回到原生播放路徑。',
  'settings.eq.curve.aria': '可拖動 31 段 EQ 頻響曲線',
  'settings.eq.curve.addFilterHint': '按兩下空白處新增；新濾波器預設 0 dB，不會改變聲音',
  'settings.eq.curve.dragBand': '拖動 {frequency} EQ 頻段',
  'settings.eq.curve.dragHint': '上下拖動圓點就能聽到變化；編輯時會自動開啟 EQ',
  'settings.eq.curve.bandSetting': '頻段設定 {value}',
  'settings.eq.curve.totalResponse': '總響應 {value}',
  'settings.eq.proEditor.advanced': '進階參數',
  'settings.eq.proEditor.amount': '增減 / 增益',
  'settings.eq.proEditor.dragHelp': '新濾波器預設 0 dB，不改變聲音；上下拖動圓點即可試聽',
  'settings.eq.proEditor.position': '位置 / 頻率',
  'settings.eq.proEditor.presetTools': '更多工具',
  'settings.eq.proEditor.range': '影響範圍',
  'settings.eq.proEditor.rangeHelp': '寬：整體潤色；窄：精準處理附近頻率',
  'settings.eq.proEditor.selected': '正在調整濾波器 {index}',
  'settings.eq.filter.highShelf': '高架',
  'settings.eq.filter.highPass': '高通',
  'settings.eq.filter.lowShelf': '低架',
  'settings.eq.filter.lowPass': '低通',
  'settings.eq.filter.notch': '陷波',
  'settings.eq.filter.peaking': '峰值',
  'settings.eq.error.bridgeChannelBalance': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'settings.eq.error.bridgeControlEq': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'settings.eq.error.bridgeDeletePreset': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'settings.eq.error.bridgeSavePreset': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'settings.eq.error.presetName': '請輸入預設名稱。',
  'settings.eq.export.successApo': '已匯出 Equalizer APO 設定：{path}',
  'settings.eq.export.successGraphicEq': '已匯出 GraphicEQ 設定：{path}',
  'settings.eq.export.successPreset': '已匯出 EQ 預設：{path}',
  'settings.eq.import.filters': 'Filter',
  'settings.eq.import.filtersValue': '{count} 匯入 / {skipped} 跳過',
  'settings.eq.import.filterPreview': 'Filter 明細',
  'settings.eq.import.filterEnabledAria': '匯入 Filter {index} 啟用狀態',
  'settings.eq.import.filterFrequencyAria': '匯入 Filter {index} 頻率',
  'settings.eq.import.filterGainAria': '匯入 Filter {index} 增益',
  'settings.eq.import.filterQAria': '匯入 Filter {index} Q',
  'settings.eq.import.filterTypeAria': '匯入 Filter {index} 類型',
  'settings.eq.import.estimatedPeak': '預計峰值',
  'settings.eq.import.graphicEq': 'GraphicEQ 點',
  'settings.eq.import.includes': 'Include 檔案',
  'settings.eq.import.includesValue': '{count} 展開 / {skipped} 跳過',
  'settings.eq.import.maxBoost': '最大提升',
  'settings.eq.import.maxCut': '最大削減',
  'settings.eq.import.apoDirectives': 'APO 指令',
  'settings.eq.import.apoDirectiveDetails': '指令明細',
  'settings.eq.import.apoDirectivesValue': '{count} 識別 / {skipped} 聲道 Filter 跳過',
  'settings.eq.import.bandwidthFilters': 'BW 換算',
  'settings.eq.import.bandwidthFiltersValue': '{count} 條轉為 Q',
  'settings.eq.import.compatibility': '相容性',
  'settings.eq.import.compatibility.adjusted': '已換算',
  'settings.eq.import.compatibility.clean': '完整匯入',
  'settings.eq.import.compatibility.partial': '部分匯入',
  'settings.eq.import.moreFilters': '還有 {count} 條 Filter 未顯示',
  'settings.eq.import.noFilters': '沒有需要顯示的有效濾波器。',
  'settings.eq.import.preamp': 'Preamp',
  'settings.eq.import.preampAria': '匯入預設 Preamp',
  'settings.eq.import.safePreamp': '建議餘量',
  'settings.eq.import.safetyDetail': '依目前 Filter 粗略估算，套用前可先留出安全餘量。',
  'settings.eq.import.safetyNeedsHeadroom': '需要安全餘量',
  'settings.eq.import.safetyOk': '餘量看起來安全',
  'settings.eq.import.safetyTitle': '匯入聽感安全',
  'settings.eq.import.audition': '試聽匯入',
  'settings.eq.import.auditionPresetName': '試聽 {name}',
  'settings.eq.import.restoreAudition': '恢復原設定',
  'settings.eq.import.clearPaste': '清空',
  'settings.eq.import.applyPreview': '套用匯入',
  'settings.eq.import.cancelPreview': '取消匯入',
  'settings.eq.import.pasteDefaultName': '貼上的 APO',
  'settings.eq.import.pasteEmpty': '請先貼上 Equalizer APO 設定。',
  'settings.eq.import.pasteFileName': '貼上內容',
  'settings.eq.import.pasteIncludeWarning': '貼上匯入不會讀取 Include 檔案；如需展開 Include，請使用檔案匯入。',
  'settings.eq.import.pasteTitle': 'APO 文字',
  'settings.eq.import.previewPaste': '預覽 APO',
  'settings.eq.import.previewTitle': '匯入預覽',
  'settings.eq.import.reportTitle': '匯入完成',
  'settings.eq.import.source': '來源',
  'settings.eq.import.sourceApo': 'Equalizer APO',
  'settings.eq.import.sourceEcho': 'ECHO JSON',
  'settings.eq.import.updateAudition': '更新試聽',
  'settings.eq.import.useSafePreamp': '使用建議餘量',
  'settings.eq.headphoneCorrection.ab': '耳機校正 A/B',
  'settings.eq.headphoneCorrection.compareCorrected': '校正後',
  'settings.eq.headphoneCorrection.compareOriginal': '對比原聲',
  'settings.eq.headphoneCorrection.convert': '轉為自訂 EQ',
  'settings.eq.headphoneCorrection.customName': '自訂 EQ - {name}',
  'settings.eq.headphoneCorrection.lockAria': '耳機校正 EQ 鎖定提示',
  'settings.eq.headphoneCorrection.lockDetail': '{name} 由耳機校正管理。要手動改參數，先轉為自訂 EQ。',
  'settings.eq.headphoneCorrection.lockedError': '這個 OPRA preset 由耳機校正管理。請先點「轉為自訂 EQ」再修改參數。',
  'settings.eq.headphoneCorrection.managed': '由耳機校正管理',
  'settings.eq.preamp.inputSafety': 'Headroom 管理',
  'settings.eq.preamp.safeHeadroom': '安全餘量',
  'settings.eq.preset.readonly': '內建預設為唯讀。',
  'settings.eq.preset.savePlaceholder': '儲存為使用者預設',
  'settings.eq.status.clippingRisk': '削波風險',
  'settings.eq.status.headroom': '餘量',
  'settings.eq.status.processor': '處理器',
  'settings.eq.status.realtimeIir': '即時 IIR',
  'settings.eq.status.safeHeadroomShort': '安全餘量',
  'settings.eq.signal.armed': '待命',
  'settings.eq.signal.bitPerfectOutput': 'Bit-perfect 路徑',
  'settings.eq.signal.dspActive': 'DSP 訊號鏈已啟用',
  'settings.eq.signal.dspOutput': 'DSP 輸出',
  'settings.eq.signal.input': '輸入',
  'settings.eq.signal.limiter': '保護',
  'settings.eq.signal.output': '輸出',
  'settings.eq.signal.peq': 'PEQ',
  'settings.eq.signal.preamp': '前級',
  'settings.eq.signal.protecting': '保護中',
  'settings.eq.signal.title': '訊號鏈',
  'settings.eq.simpleAction.customPresetName': '我的 EQ 聽感',
  'settings.eq.simpleAction.lastTweak': '上一個調整',
  'settings.eq.simpleAction.nextVibe': '換個聽感',
  'settings.eq.simpleAction.nextVibeDetail': '試試看',
  'settings.eq.simpleAction.savedPresetName': '{name} {amount}%',
  'settings.eq.simpleAction.saveVibe': '儲存聽感',
  'settings.eq.simpleAction.saveVibeDetail': '儲存目前',
  'settings.eq.simpleInsight.amount': '強度',
  'settings.eq.simpleInsight.aria': 'Simple 目前聽感摘要',
  'settings.eq.simpleInsight.custom': '手動聽感',
  'settings.eq.simpleInsight.mainChange': '主要變化',
  'settings.eq.simpleInsight.ready': '就緒',
  'settings.eq.simpleInsight.vibe': '目前聽感',
  'settings.eq.simpleSafety.action': '留出餘量',
  'settings.eq.simpleSafety.aria': 'Simple 安全餘量',
  'settings.eq.simpleSafety.risk': '需要餘量',
  'settings.eq.simpleSafety.riskDetail': '峰值 {peak} / 建議 {preamp}',
  'settings.eq.simpleSafety.safe': '餘量安心',
  'settings.eq.simpleSafety.safeDetail': '峰值 {peak} / 前級 {preamp}',
  'settings.eq.simpleTone.air': '空氣感',
  'settings.eq.simpleTone.airDetail': '更亮更開闊',
  'settings.eq.simpleTone.amount': '味道強度',
  'settings.eq.simpleTone.amountAria': 'Simple 聽感強度',
  'settings.eq.simpleTone.aria': 'Simple 聽感快捷調整',
  'settings.eq.simpleTone.bass': '低頻彈性',
  'settings.eq.simpleTone.bassDetail': '鼓點更飽滿',
  'settings.eq.simpleTone.flat': '恢復平直',
  'settings.eq.simpleTone.flatDetail': '回到原點',
  'settings.eq.simpleTone.less': '淡一點',
  'settings.eq.simpleTone.more': '濃一點',
  'settings.eq.simpleTone.vocal': '人聲靠前',
  'settings.eq.simpleTone.vocalDetail': '歌詞更清楚',
  'settings.eq.simpleTone.warm': '暖一點',
  'settings.eq.simpleTone.warmDetail': '柔和耐聽',
  'settings.eq.simpleZone.air': '空氣感',
  'settings.eq.simpleZone.airDetail': '亮度和空間',
  'settings.eq.simpleZone.applyAria': '套用{zone}聽感，目前{value}',
  'settings.eq.simpleZone.aria': 'Simple 聽感區域變化',
  'settings.eq.simpleZone.low': '低頻',
  'settings.eq.simpleZone.lowDetail': '鼓點和厚度',
  'settings.eq.simpleZone.neutral': '平直',
  'settings.eq.simpleZone.vocal': '人聲',
  'settings.eq.simpleZone.vocalDetail': '歌詞和存在感',
  'settings.eq.routing.action.armHeadroom6': '預設 -6dB 餘量',
  'settings.eq.routing.action.nativeDirect': '原生直通',
  'settings.eq.routing.armed': '已待命',
  'settings.eq.routing.dspPath': 'DSP 路徑',
  'settings.eq.routing.dspPathDetail': '目前處理會明確顯示在鏈路中。',
  'settings.eq.routing.headroomActiveDetail': '餘量正在 DSP 路徑中生效。',
  'settings.eq.routing.headroomStandby': '餘量待命',
  'settings.eq.routing.headroomStandbyDetail': '只會在啟用 DSP 後生效。',
  'settings.eq.routing.modules': '模組',
  'settings.eq.routing.modulesActiveDetail': '下方會顯示每個啟用的處理階段。',
  'settings.eq.routing.modulesBypassed': '已旁路',
  'settings.eq.routing.modulesBypassedDetail': 'EQ / FIR / Balance 均關閉。',
  'settings.eq.routing.nativeBypassComfort': 'DSP 旁路時會保持原生直通，樣本不會被效果鏈改動。',
  'settings.eq.routing.nativeDirect': '原生直通',
  'settings.eq.routing.nativeDirectDetail': 'DSP 旁路時不會改動原生播放。',
  'settings.eq.routing.nativeNoGainDetail': '原生直通下不做增益衰減。',
  'settings.eq.routing.playbackPath': '播放路徑',
  'settings.eq.routing.protectActive': '保護中',
  'settings.eq.routing.protectActiveDetail': 'Limiter 正在防止過載。',
  'settings.eq.routing.safety': '安全',
  'settings.eq.headroom.dsp': 'DSP 餘量',
  'settings.eq.headroom.nativeBypassNote': '僅在 EQ / FIR / Balance 啟用時生效，原生旁路保持直通。',
  'settings.eq.headroom.presetsAria': 'DSP 餘量快捷檔位',  'settings.eq.mode.aria': 'EQ 顯示模式',
  'settings.eq.mode.current': '模式',
  'settings.eq.mode.pro': 'Pro',
  'settings.eq.mode.simple': 'Simple',
  'settings.eq.section.channel': '聲道與監聽工具',
  'settings.eq.section.compare': 'A/B 與旁路對比',
  'settings.eq.subtitle': '聲音曲線、安全餘量與進階調音',
  'settings.eq.subtitle.parametric': '高精度 31 段參數均衡器',
  'settings.eq.title': 'EQ',
  'settings.eq.title.parametric': '參數 EQ',
  'settings.eq.warning.channelClipping': '存在削波風險：降低增益或前級可獲得更安全的餘量。',
  'settings.eq.warning.lowerPreamp': '降低前級可避免削波。',
  'settings.general.artistInfoSources.description': '選擇重新整理藝人簡介時使用的百科來源；百度百科更適合中文網路環境，Wikipedia 可作為國際藝人備援。',
  'settings.nav.shortcuts.label': '快捷鍵',
  'settings.nav.shortcuts.description': '一般快捷鍵、全域快捷鍵、播放控制',
  'settings.shortcuts.action.clear': '清除',
  'settings.shortcuts.action.bossKey.description': '立即隱藏視窗，並把 ECHO 音量降到 0。',
  'settings.shortcuts.action.bossKey.title': '老闆鍵',
  'settings.shortcuts.action.nextTrack.description': '切到目前播放佇列裡的下一首。',
  'settings.shortcuts.action.nextTrack.title': '下一首',
  'settings.shortcuts.action.openAudioSettings.description': '打開底部播放器的音訊設定抽屜。',
  'settings.shortcuts.action.openAudioSettings.title': '打開音訊設定',
  'settings.shortcuts.action.openLyricsSettings.description': '打開歌詞設定抽屜。',
  'settings.shortcuts.action.openLyricsSettings.title': '打開歌詞設定',
  'settings.shortcuts.action.openMvSettings.description': '打開 MV 設定抽屜。',
  'settings.shortcuts.action.openMvSettings.title': '打開 MV 設定',
  'settings.shortcuts.action.locateCurrentTrack.description': '把目前列表捲動到正在播放的歌曲位置。',
  'settings.shortcuts.action.locateCurrentTrack.title': '定位到目前播放歌曲',
  'settings.shortcuts.action.cycleRepeatMode.description': '在順序播放和單曲循環之間切換。',
  'settings.shortcuts.action.cycleRepeatMode.title': '切換循環模式',
  'settings.shortcuts.action.openPlaybackQueue.description': '打開目前播放佇列。',
  'settings.shortcuts.action.openPlaybackQueue.title': '打開播放佇列',
  'settings.shortcuts.action.openSearch.description': '打開搜尋頁面。',
  'settings.shortcuts.action.openSearch.title': '打開搜尋',
  'settings.shortcuts.action.openSettings.description': '打開應用程式設定頁面。',
  'settings.shortcuts.action.openSettings.title': '打開設定',
  'settings.shortcuts.action.openLiked.description': '打開「我喜歡的」歌曲列表。',
  'settings.shortcuts.action.openLiked.title': '打開我喜歡的',
  'settings.shortcuts.action.playPause.description': '在全域範圍切換播放與暫停。',
  'settings.shortcuts.action.playPause.title': '播放 / 暫停',
  'settings.shortcuts.action.previousTrack.description': '切到目前播放佇列裡的上一首。',
  'settings.shortcuts.action.previousTrack.title': '上一首',
  'settings.shortcuts.action.record': '錄製',
  'settings.shortcuts.action.restoreGlobalRecommended': '恢復全域',
  'settings.shortcuts.action.restoreLocalRecommended': '恢復一般',
  'settings.shortcuts.action.restoreRecommended': '恢復推薦',
  'settings.shortcuts.action.restoreRecommendedConfirm': '恢復全部推薦快捷鍵會覆蓋一般和全域快捷鍵設定，是否繼續？',
  'settings.shortcuts.action.seekBackward.description': '目前歌曲向後退 10 秒。',
  'settings.shortcuts.action.seekBackward.title': '快退 10 秒',
  'settings.shortcuts.action.seekForward.description': '目前歌曲向前進 10 秒。',
  'settings.shortcuts.action.seekForward.title': '快進 10 秒',
  'settings.shortcuts.action.showMainWindow.description': '把 ECHO 主視窗帶回前景。',
  'settings.shortcuts.action.showMainWindow.title': '顯示主視窗',
  'settings.shortcuts.action.speedDown.description': '每次把播放速度降低 0.1x。',
  'settings.shortcuts.action.speedDown.title': '播放減速',
  'settings.shortcuts.action.speedUp.description': '每次把播放速度提高 0.1x。',
  'settings.shortcuts.action.speedUp.title': '播放加速',
  'settings.shortcuts.action.resetPlaybackSpeed.description': '把播放速度恢復為 1.0x。',
  'settings.shortcuts.action.resetPlaybackSpeed.title': '重設播放速度',
  'settings.shortcuts.action.replayCurrentTrack.description': '從頭重新播放目前歌曲。',
  'settings.shortcuts.action.replayCurrentTrack.title': '重播目前歌曲',
  'settings.shortcuts.action.revealCurrentTrackInFolder.description': '在檔案總管中顯示目前播放檔案。',
  'settings.shortcuts.action.revealCurrentTrackInFolder.title': '打開目前歌曲所在資料夾',
  'settings.shortcuts.action.stop.description': '停止目前播放並釋放播放狀態。',
  'settings.shortcuts.action.stop.title': '停止播放',
  'settings.shortcuts.action.toggleDesktopLyrics.description': '打開或關閉桌面歌詞視窗。',
  'settings.shortcuts.action.toggleDesktopLyrics.title': '打開 / 關閉桌面歌詞',
  'settings.shortcuts.action.toggleDesktopLyricsLock.description': '切換桌面歌詞滑鼠穿透鎖定狀態。',
  'settings.shortcuts.action.toggleDesktopLyricsLock.title': '鎖定 / 解鎖歌詞',
  'settings.shortcuts.action.toggleCurrentTrackLiked.description': '收藏或取消收藏目前播放歌曲。',
  'settings.shortcuts.action.toggleCurrentTrackLiked.title': '收藏目前歌曲',
  'settings.shortcuts.action.toggleEq.description': '打開或關閉均衡器。',
  'settings.shortcuts.action.toggleEq.title': '開關均衡器',
  'settings.shortcuts.action.toggleLyrics.description': '打開或關閉歌詞頁面。',
  'settings.shortcuts.action.toggleLyrics.title': '打開 / 關閉歌詞',
  'settings.shortcuts.action.toggleMiniPlayer.description': '顯示或隱藏迷你播放器視窗。',
  'settings.shortcuts.action.toggleMiniPlayer.title': '打開 / 關閉迷你播放器',
  'settings.shortcuts.action.togglePet.description': '顯示或隱藏 ECHO 桌面寵物。',
  'settings.shortcuts.action.togglePet.title': '打開 / 關閉 ECHO 寵物',
  'settings.shortcuts.action.toggleMute.description': '在靜音和正常音量之間切換。',
  'settings.shortcuts.action.toggleMute.title': '靜音 / 取消靜音',
  'settings.shortcuts.action.toggleShuffle.description': '打開或關閉隨機播放。',
  'settings.shortcuts.action.toggleShuffle.title': '切換隨機播放',
  'settings.shortcuts.action.volumeDown.description': '把 ECHO 音量降低 5%。',
  'settings.shortcuts.action.volumeDown.title': '音量降低',
  'settings.shortcuts.action.volumeUp.description': '把 ECHO 音量提高 5%。',
  'settings.shortcuts.action.volumeUp.title': '音量提高',
  'settings.shortcuts.column.function': '功能說明',
  'settings.shortcuts.column.local': '一般快捷鍵',
  'settings.shortcuts.column.global': '全域快捷鍵',
  'settings.shortcuts.description': '一般快捷鍵只有 ECHO 視窗聚焦時生效；全域快捷鍵在背景也生效，啟用前會檢查系統占用。',
  'settings.shortcuts.empty': '未綁定',
  'settings.shortcuts.filter.all': '全部',
  'settings.shortcuts.filter.aria': '快捷鍵篩選',
  'settings.shortcuts.filter.empty': '目前篩選下沒有快捷鍵動作。',
  'settings.shortcuts.filter.enabled': '已啟用',
  'settings.shortcuts.filter.issues': '有提示',
  'settings.shortcuts.filter.unbound': '未綁定',
  'settings.shortcuts.group.all': '全部分類',
  'settings.shortcuts.group.audio': '音訊',
  'settings.shortcuts.group.collapseAll': '全部摺疊',
  'settings.shortcuts.group.expandAll': '全部展開',
  'settings.shortcuts.group.jumpAria': '快捷鍵分類',
  'settings.shortcuts.group.library': '曲庫與尋找',
  'settings.shortcuts.group.lyrics': '歌詞',
  'settings.shortcuts.group.meta': '{visible} 項 · {enabled} 已啟用',
  'settings.shortcuts.group.playback': '播放控制',
  'settings.shortcuts.group.speed': '播放速度',
  'settings.shortcuts.group.volume': '音量',
  'settings.shortcuts.group.windows': '視窗與桌面',
  'settings.shortcuts.localUnavailable': '僅全域',
  'settings.shortcuts.mouse.back': '側鍵（後退）',
  'settings.shortcuts.mouse.forward': '側鍵（前進）',
  'settings.shortcuts.mouse.middle': '中鍵',
  'settings.shortcuts.message.duplicate': '這個快捷鍵已經綁定到其他動作。',
  'settings.shortcuts.message.duplicateWithAction': '這個快捷鍵已經綁定到「{action}」。',
  'settings.shortcuts.message.empty': '請先錄製一個快捷鍵。',
  'settings.shortcuts.message.invalid': '這個按鍵目前不能作為快捷鍵。',
  'settings.shortcuts.message.safe': '這個快捷鍵可以使用。',
  'settings.shortcuts.message.unavailable': '這個快捷鍵已被系統或其他應用占用，因此保持關閉。',
  'settings.shortcuts.message.unsafe': '這個按鍵目前不能作為快捷鍵；可以讓巨集鍵盤輸出標準鍵盤鍵或媒體鍵。',
  'settings.shortcuts.note': '支援單鍵、組合鍵、媒體鍵、F13 等巨集鍵盤鍵，以及滑鼠中鍵和側鍵。點按綁定框後按下目標鍵，Esc 取消。一般快捷鍵不會在輸入框裡觸發。',
  'settings.shortcuts.recording': '按下鍵盤、媒體鍵或滑鼠側鍵… Esc 取消',
  'settings.shortcuts.scope.local': '一般快捷鍵',
  'settings.shortcuts.scope.global': '全域快捷鍵',
  'settings.shortcuts.summary.aria': '快捷鍵摘要',
  'settings.shortcuts.summary.globalEnabled': '全域已啟用 {count}',
  'settings.shortcuts.summary.issues': '提示 {count}',
  'settings.shortcuts.summary.localEnabled': '一般已啟用 {count}',
  'settings.shortcuts.summary.unbound': '未綁定 {count}',
  'settings.shortcuts.title': '快捷鍵',
  'settings.general.artistInfoSources.title': '藝人資訊來源',
  'settings.general.artistStreamingAlbums.description': '開啟後，藝人詳情的專輯頁會在本地專輯下方按需搜尋並顯示串流專輯；預設關閉，避免增加頁面與網路壓力。',
  'settings.general.artistStreamingAlbums.title': '串流專輯',
  'settings.general.osuDownloaderFeature.description': '預設關閉。開啟後會在側邊欄顯示 osu! 入口，用於搜尋 beatmap 並將音訊提取到下載流程。',
  'settings.general.osuDownloaderFeature.title': 'osu! 入口',
  'settings.general.streamingFeature.description': '預設開啟。關閉後會從側邊欄隱藏串流頁面；進入線上音樂搜尋和收藏前仍需同意串流功能須知。',
  'settings.general.streamingFeature.title': '串流入口',
  'streamingConsentNotice.acceptance': '繼續表示你理解並接受：本功能是個人本機用戶端輔助入口，不構成法律意見、版權授權、平台代理關係或內容可用性承諾。',
  'streamingConsentNotice.authorizedUse': '你應只使用自己有權存取的帳號、訂閱和內容，不得用本功能規避平台條款、下載限制、DRM、帳號風控、付費牆或版權保護。',
  'streamingConsentNotice.bodyIntro': 'ECHO 的串流入口只提供搜尋、帳號狀態、收藏、歌單匯入和播放入口整合。ECHO 不擁有、託管、出售或重新分發任何第三方串流平台的音訊、影片、封面、歌詞或中繼資料版權。',
  'streamingConsentNotice.cancel': '取消',
  'streamingConsentNotice.close': '關閉須知',
  'streamingConsentNotice.confirm': '我同意並繼續',
  'streamingConsentNotice.consentPhrase': '我同意',
  'streamingConsentNotice.disclaimer': '免責聲明：ECHO 不對使用者使用第三方平台造成的帳號限制、服務中斷、版權爭議、DMCA 通知、資料遺失、地區不可用或其它損失承擔責任。',
  'streamingConsentNotice.dmca': 'DMCA 是美國版權法中的通知與移除機制。若權利人認為某內容、連結或資料使用侵犯其權利，應透過對應平台或合法管道提交通知；使用者也應在收到權利通知或平台要求時停止相關使用。',
  'streamingConsentNotice.inputLabel': '輸入「{phrase}」以繼續',
  'streamingConsentNotice.license': 'ECHO 的程式碼和外掛受專案授權條款約束；禁止破解、繞過授權、偽造權益、移除完整性校驗，或把 ECHO 用作未經授權存取付費內容的工具。',
  'streamingConsentNotice.localFirst': 'ECHO 本質上是本機音樂播放器，串流入口的優先級最低。關於「能不能加入某某平台 / 某某音源」的訴求會被拒絕；串流出現問題時，請優先使用本機音樂庫。',
  'streamingConsentNotice.noBypass': 'ECHO 不會為任何平台提供繞過會員、破解試聽、規避付費或損害平台及權利人利益的能力；如果歌曲只能播放 30 秒，請檢查你的平台會員、訂閱、地區和帳號狀態。',
  'streamingConsentNotice.platformControl': '第三方平台、商標、曲庫、API、帳號、訂閱、Cookie、DRM、地區限制和播放授權均由對應平台及權利人控制。ECHO 不代表這些平台，也不授予任何平台內容的使用許可。',
  'streamingConsentNotice.title': '串流功能須知',
  'streamingConsentNotice.uninstall': '如果你認為串流平台會員費用過高，請停止使用本功能並卸載軟體；任何破解會員、繞過授權、規避版權保護或其它不合法訴求都會被拒絕。',
  'settings.general.closeToTray': '關閉時隱藏到系統匣',
  'settings.general.launchAtLogin.title': '開機自動啟動',
  'settings.general.launchAtLogin.description': '登入 Windows 後自動啟動 ECHO。預設關閉，需要手動開啟。',
  'settings.general.lyricsMvGraphicsPressureGuard.title': '歌詞/MV 圖形記憶體保護',
  'settings.general.lyricsMvGraphicsPressureGuard.description': '預設開啟。歌詞頁或 MV 頁占用顯示記憶體過高時，本次執行會自動關掉額外的 MV 背景和耗資源歌詞效果；比關閉硬體加速更溫和。',
  'settings.general.sidebarAutoHide.title': '隱藏側邊欄',
  'settings.general.sidebarAutoHide.description': '開啟後側邊欄會收進螢幕邊緣；滑鼠移到左側邊緣時自動抽出。預設關閉。',
  'settings.general.settingsHideSidebar.title': '進入設定時隱藏側邊欄',
  'settings.general.settingsHideSidebar.description': '開啟後進入設定會完全隱藏側邊欄，給設定頁更多空間；設定標題左側會出現返回圖示。預設關閉。',
  'settings.general.sidebarIconOnly.title': '側邊欄僅顯示圖示',
  'settings.general.sidebarIconOnly.description': '開啟後側邊欄保持顯示，但導覽入口只顯示圖示；懸停仍可查看名稱。預設關閉。',
  'settings.general.settingsOptionalSections.title': '顯示外掛 / 網路硬碟 / EQ 設定欄目',
  'settings.general.settingsOptionalSections.description': '預設隱藏。開啟後在設定欄目中顯示外掛、網路硬碟 / 遠端和 EQ；功能入口仍保留在側邊欄。',
  'settings.general.notificationsDisabled.title': '關閉所有通知',
  'settings.general.notificationsDisabled.description': '開啟後停用 ECHO 內的提醒卡片、左上角通知和播放列通知。預設關閉。',
  'settings.general.upcomingTrackNotice.title': '下一首預告',
  'settings.general.upcomingTrackNotice.description': '開啟後，每首歌快播完時在左上角顯示下一首的封面、歌曲名、藝人和專輯。預設關閉。',
  'settings.general.trackContextMenuExtraActions.title': '右鍵選單擴充操作',
  'settings.general.trackContextMenuExtraActions.description': '開啟後在曲目右鍵選單中顯示 osu! Timing、使用系統預設應用程式打開、複製/儲存歌曲卡片圖片。預設隱藏以保持選單精簡。',
  'settings.general.touchKeyboard.title': '啟用螢幕鍵盤',
  'settings.general.touchKeyboard.description': '輸入欄位可自動顯示螢幕上的 Windows 鍵盤。此功能適用於觸控螢幕。',
  'settings.general.fastStartup.description': '開啟後，啟動時只做輕量唯讀曲庫驗證；完整資料保護快照會在視窗開啟後於背景完成。預設關閉。',
  'settings.general.fastStartup.title': '快速啟動',
  'settings.general.sqliteBalancedDurability.description': '預設關閉，曲庫寫入較穩妥。開啟後下次啟動會加快大量掃描時的寫入，但斷電或系統崩潰時保護會弱一些。',
  'settings.general.sqliteBalancedDurability.title': '掃描寫入效能模式',
  'settings.general.firstRunWizard.action': '開啟新手教程',
  'settings.general.firstRunWizard.description': '重新開啟第一次啟動時的向導，可選擇 WASAPI Shared、安全模式、Exclusive 或 ASIO，並快速了解本地媒體庫、快取、外觀和帳號入口。',
  'settings.general.firstRunWizard.title': '新手教程',
  'settings.general.echoProActivation.action': '網頁解綁 / 備用',
  'settings.general.echoProActivation.description': '在客戶端輸入愛發電訂單或 Pro Key 後連網啟用，成功後自動匯入並啟用 ECHO Pro 外掛；解除綁定 HWID 仍需要去網頁。',
  'settings.general.echoProActivation.title': 'Pro 啟用',
  'settings.general.userNotice.action': '重新閱讀用戶須知',
  'settings.general.userNotice.description': '重新開啟進入 ECHO 前必須同意的用戶須知，包含文件、社群邊界、DMCA、反破解和 AI 程式碼說明。',
  'settings.general.userNotice.title': '用戶須知',
  'settings.general.playerWaveformProgress.description': '開啟後，底部播放列會用輕量波形樣式顯示進度；預設關閉，不解碼音訊也不增加背景分析。',
  'settings.general.playerWaveformProgress.title': '波形進度條',
  'settings.general.signalPathControl.description': '在底部播放列顯示聲音鏈路入口；預設開啟，歌詞頁一律隱藏。',
  'settings.general.signalPathControl.title': '訊號路徑',
  'settings.general.homeWaveformVisualizer.description': '控制首頁「今日回聲」的即時波形圖。關閉後不渲染波形，也會跳過首頁波形使用的頻譜分析。',
  'settings.general.homeWaveformVisualizer.title': '首頁波形圖',
  'settings.general.rememberWindowSize.description': '開啟後會記住你上次拖曳後的視窗寬高，下次啟動自動恢復。',
  'settings.general.rememberWindowSize.title': '記住視窗尺寸',
  'settings.general.backup.title': '設定參數備份',
  'settings.general.backup.export': '匯出設定',
  'settings.general.backup.import': '匯入設定',
  'settings.playback.outputMode.asio': 'ASIO',
  'settings.playback.outputMode.ks': 'WDM-KS（實驗性）',
  'settings.playback.outputMode.description': '普通耳機、藍牙和電腦喇叭建議使用 WASAPI Shared。如果切到 Exclusive / ASIO 後沒聲音，先切回 WASAPI Shared。安全模式是瀏覽器相容兜底；ASIO / Exclusive 適合外接音效卡和 HiFi 調試。',
  'settings.playback.outputMode.exclusive': 'Exclusive',
  'settings.playback.outputMode.shared': 'WASAPI Shared（推薦）',
  'settings.playback.outputMode.system': '安全模式',
  'settings.playback.outputMode.title': '輸出模式',
  'settings.playback.outputStatus.failed': '切換失敗：{reason}',
  'settings.playback.outputStatus.fallback': '已保存，但實際輸出已從 {requested} 回退到 {actual}。',
  'settings.playback.outputStatus.saved': '已保存並切換到 {mode}。',
  'settings.playback.recommendedPath.advanced.body': 'Exclusive / ASIO、DSD、SRC 和原生解碼放在進階播放設定裡，預設收起，適合外接音效卡和排障。',
  'settings.playback.recommendedPath.advanced.title': '需要 HiFi / 排障時再展開進階項',
  'settings.playback.recommendedPath.description': '普通使用者只需要按這條路徑走；高風險和實驗項預設收起。',
  'settings.playback.recommendedPath.device.body': '先選預設裝置或實際耳機/DAC；沒聲音時先回到 WASAPI Shared，再看音訊狀態。',
  'settings.playback.recommendedPath.device.title': '再選輸出裝置',
  'settings.playback.recommendedPath.shared.body': '適合耳機、藍牙、電腦喇叭和日常播放，也是沒聲音時的回退起點。',
  'settings.playback.recommendedPath.shared.title': '先用 WASAPI Shared',
  'settings.playback.recommendedPath.title': '推薦播放路徑',
  'settings.playback.automaticOutput.title': '自動適配輸出',
  'settings.playback.automaticOutput.description': '預設關閉。適合螢幕喇叭、藍牙耳機與一般音效卡：跟隨系統預設裝置，採用共享輸出、平衡延遲與安全回退；開啟時鎖定手動輸出選項。',
  'settings.playback.automaticOutput.recommended': '推薦不熟悉音訊設定的使用者開啟',
  'settings.playback.automaticOutput.statusEnabled': '自動適配已開啟，音訊引擎正在使用系統預設共享輸出。',
  'settings.playback.automaticOutput.statusDisabled': '自動適配已關閉，可以手動選擇輸出模式與裝置。',
  'settings.playback.advancedPanel.action.collapse': '收起進階播放設定',
  'settings.playback.advancedPanel.action.expand': '展開進階播放設定',
  'settings.playback.advancedPanel.description': '減輕占用、診斷、DSD、匯出和細節播放控制預設收起，避免干擾常用輸出設定。',
  'settings.playback.advancedPanel.memory': '會記住展開狀態',
  'settings.playback.advancedPanel.title': '進階播放設定',
  'settings.playback.asioNativeDsd.description': '預設關閉。僅在 ASIO + 本地 DSF + DoP 開啟且沒有 EQ/音量/變速/DSP 時嘗試；失敗會退回現有 DoP/PCM。',
  'settings.playback.asioNativeDsd.title': 'ASIO 原生 DSD 實驗',
  'settings.playback.dsdDop.description': '預設開啟，僅對符合條件的本機 DSF 生效。ASIO 與 WASAPI 獨占使用 DoP，不相容時退回 PCM；ASIO 原生 DSD 需另行開啟實驗選項。直通路徑不重採樣，最終以 DAC 顯示為準。',
  'settings.playback.dsdDop.requiresAsio': 'ASIO 與 WASAPI 獨占支援 DoP',
  'settings.playback.dsdDop.title': 'DSD 直通（DoP）',
  'settings.playback.exportFormat.description': '底部播放列的匯出按鈕會使用這個格式；匯出速度跟隨目前播放速度。',
  'settings.playback.exportFormat.title': '音訊匯出格式',
  'settings.playback.fixedVolume.description': '開啟後會將 ECHO 音量控制鎖定為 100%；ReplayGain 仍會獨立生效。',
  'settings.playback.fixedVolume.status.fixed': '已固定',
    'settings.playback.fixedVolume.title': '固定音量',
    'settings.playback.autoPlayOnStartup.description': '啟動 ECHO 後自動繼續播放上次佇列中的曲目，並從已儲存的進度繼續。',
    'settings.playback.autoPlayOnStartup.title': '啟動時自動播放',
    'settings.playback.preventSleepWhilePlaying.description': '音樂載入或播放時阻止系統休眠和關閉螢幕；暫停或停止後立即恢復系統電源策略。',
    'settings.playback.preventSleepWhilePlaying.title': '播放時保持喚醒',
  'settings.playback.gapless.description': '同一張專輯的連續曲目會無停頓接上；無法無縫銜接時自動用一般方式切歌。預設關閉。',
  'settings.playback.gapless.title': '專輯無縫播放',
  'settings.playback.shuffleCredibility.count': '避開最近 {count} 首',
  'settings.playback.shuffleCredibility.current': '目前：{mode} · {detail}',
  'settings.playback.shuffleCredibility.description': '選擇隨機播放是否避開剛聽過的歌曲。抽取範圍仍是目前的歌曲列表、資料夾或歌單。',
  'settings.playback.shuffleCredibility.mode.avoidRecent': '避免最近重複',
  'settings.playback.shuffleCredibility.mode.avoidRecent.description': '預設選項，優先避開剛聽過的一批歌曲。',
  'settings.playback.shuffleCredibility.mode.library': '全曲庫',
  'settings.playback.shuffleCredibility.mode.library.description': '從歌曲頁隨機時覆蓋全曲庫；在歌單或資料夾裡則按目前範圍直接抽取。',
  'settings.playback.shuffleCredibility.mode.pseudoRandom': '偽隨機',
  'settings.playback.shuffleCredibility.mode.pseudoRandom.description': '更少重複，適合大曲庫長時間隨機。',
  'settings.playback.shuffleCredibility.off': '允許重複',
  'settings.playback.shuffleCredibility.title': '隨機播放模式',
  'settings.playback.transportFade.curve.equalPower': '等功率',
  'settings.playback.transportFade.curve.linear': '線性',
  'settings.playback.transportFade.curve.smooth': '平滑',
  'settings.playback.transportFade.description': '拖到 0 ms 關閉；開啟後目前曲目會在結束前淡出，下一首從開頭淡入。手動播放 / 暫停不觸發。',
  'settings.playback.transportFade.field.curve': '曲線',
  'settings.playback.transportFade.field.duration': '時長',
  'settings.playback.transportFade.field.fadeIn': '淡入 ms',
  'settings.playback.transportFade.field.fadeOut': '淡出 ms',
  'settings.playback.transportFade.status.disabled': '未開啟',
  'settings.playback.transportFade.status.enabled': '已開啟',
  'settings.playback.transportFade.title': '曲目切換淡入淡出',
  'settings.playback.issueDiagnostics.description': '預設關閉。排查播放問題時開啟，會彈出浮窗記錄狀態、進度、緩衝、underrun、輸出後端、警告和結束標記。',
  'settings.playback.issueDiagnostics.title': '音訊問題診斷視窗',
  'settings.playback.miniPlayer.action.hide': '隱藏',
  'settings.playback.miniPlayer.action.show': '顯示',
  'settings.playback.miniPlayer.autoHideNote': '開啟迷你播放器時將主介面隱藏到右下角系統匣',
  'settings.playback.miniPlayer.description': '獨立透明置頂小窗，只顯示封面、歌名和進度；視窗會收緊到播放器本體，避免透明空白擋住其他軟體。',
  'settings.playback.miniPlayer.status.hidden': '未顯示',
  'settings.playback.miniPlayer.status.visible': '已顯示',
  'settings.playback.miniPlayer.title': '迷你播放器',
  'settings.playback.pet.action.hide': '隱藏',
  'settings.playback.pet.action.show': '顯示',
  'settings.playback.pet.description': '在桌面顯示獨立透明置頂的像素寵物；可以直接拖曳，並會記住上次位置。',
  'settings.playback.pet.status.hidden': '未顯示',
  'settings.playback.pet.status.visible': '已顯示',
  'settings.playback.pet.size': '寵物大小',
  'settings.playback.pet.title': 'ECHO 寵物',
  'settings.playback.monoAudio.description': '把左右聲道合併後同時輸出到兩邊；預設關閉，適合單耳聽、壞聲道耳機或臨時檢查混音。',
  'settings.playback.monoAudio.title': '單聲道音訊',
  'settings.playback.nativeDecode.description': '預設關閉。開啟後，不需要重取樣的本地 WAV/FLAC/MP3 會走原生解碼；MP3 優先用 Windows Media，失敗再退回 FFmpeg。',
  'settings.playback.nativeDecode.title': '原生解碼',
  'settings.playback.hqplayer.defaultBackend.ask': '每次詢問',
  'settings.playback.hqplayer.defaultBackend.echoNative': '繼續使用 ECHO 輸出',
  'settings.playback.hqplayer.defaultBackend.hqplayer': '優先 HQPlayer',
  'settings.playback.hqplayer.description': '預留 HQPlayer 控制端點和播放交接偏好；預設不接管目前播放輸出。',
  'settings.playback.hqplayer.enable': '啟用 HQPlayer 整合',
  'settings.playback.hqplayer.field.defaultBackend': '預設交接',
  'settings.playback.hqplayer.field.endpoint': '控制端點',
  'settings.playback.hqplayer.field.lastChecked': '上次檢測',
  'settings.playback.hqplayer.field.status': '狀態',
  'settings.playback.hqplayer.host': 'Host',
  'settings.playback.hqplayer.mediaServer': 'ECHO 媒體服務',
  'settings.playback.hqplayer.mode.localDesktop': '本機 HQPlayer Desktop',
  'settings.playback.hqplayer.mode.remote': '遠端 HQPlayer',
  'settings.playback.hqplayer.note': '目前只儲存設定並測試 TCP 連通性，不會啟動 HQPlayer，也不會改變 ECHO 目前播放鏈路。',
  'settings.playback.hqplayer.port': '控制連接埠',
  'settings.playback.hqplayer.profileName': '設定檔名稱',
  'settings.playback.hqplayer.result.failed': '連線不可用',
  'settings.playback.hqplayer.result.ok': '連線可用',
  'settings.playback.hqplayer.save': '儲存',
  'settings.playback.hqplayer.saving': '儲存中',
  'settings.playback.hqplayer.status.available': '可用',
  'settings.playback.hqplayer.status.checking': '檢測中',
  'settings.playback.hqplayer.status.disabled': '未啟用',
  'settings.playback.hqplayer.status.notConfigured': '未設定連接埠',
  'settings.playback.hqplayer.status.unavailable': '不可用',
  'settings.playback.hqplayer.test': '測試連線',
  'settings.playback.hqplayer.testing': '測試中',
  'settings.playback.hqplayer.title': 'HQPlayer 整合',
  'settings.playback.audioDrawerNotice.description': '這裡會跟隨右上角音訊抽屜的即時狀態刷新；如果兩邊短時間內不一致，請以右上角抽屜為準。',
  'settings.playback.audioDrawerNotice.status': '建議從右上角音訊設定抽屜調整輸出模式、共享後端和裝置。',
  'settings.playback.audioDrawerNotice.title': '音訊設定以右上角抽屜為準',
  'settings.playback.noSoundGuide.action': '沒聲音就點我',
  'settings.playback.noSoundGuide.actionCollapse': '關閉向導',
  'settings.playback.noSoundGuide.control.manual': '這一步需要在系統或網路環境裡處理；按上面的說明調整後回到 ECHO 繼續。',
  'settings.playback.noSoundGuide.control.unavailable': '目前平台沒有可切換的共享後端。',
  'settings.playback.noSoundGuide.description': '按順序檢查輸出模式、裝置和 Windows 聲音設定；先把基礎播放救回來，再碰進階項。',
  'settings.playback.noSoundGuide.step.backend': '共享後端保持 WASAPI Shared；DirectSound 只當臨時相容兜底。',
  'settings.playback.noSoundGuide.step.backend.title': '共享後端',
  'settings.playback.noSoundGuide.step.device': '輸出裝置先選預設裝置，或明確選正在用的耳機、喇叭、DAC；藍牙裝置先確認 Windows 已連線。',
  'settings.playback.noSoundGuide.step.device.title': '選擇裝置',
  'settings.playback.noSoundGuide.step.drivers.title': '不要亂裝驅動',
  'settings.playback.noSoundGuide.step.outputMode': '先把輸出模式切到 WASAPI Shared（推薦）；Exclusive / ASIO 沒聲音時不要硬試。',
  'settings.playback.noSoundGuide.step.outputMode.title': '切回 Shared',
  'settings.playback.noSoundGuide.step.restart': '還沒聲音時，先點進階播放設定裡的重啟音訊引擎，再重新播放一首本地 MP3/FLAC 測試。',
  'settings.playback.noSoundGuide.step.restart.title': '重啟測試',
  'settings.playback.noSoundGuide.step.sampleRate': '打開 Windows 聲音設定，把預設格式先調回裝置預設、24bit / 48kHz 或 16bit / 44.1kHz。',
  'settings.playback.noSoundGuide.step.sampleRate.title': '取樣率別太高',
  'settings.playback.noSoundGuide.step.streaming.title': '串流卡頓',
  'settings.playback.noSoundGuide.step.windowsVolume': '檢查 Windows 音量混音器：系統、ECHO、目前輸出裝置都不要靜音，音量先拉到聽得見。',
  'settings.playback.noSoundGuide.step.windowsVolume.title': '系統音量',
  'settings.playback.noSoundGuide.stepsTitle': '一步一步來',
  'settings.playback.noSoundGuide.streamingNote': '串流媒體跳轉卡頓通常就是目前網路或平台回應慢，不是輸出模式能修好；如果受不了，優先用本地檔案播放。',
  'settings.playback.noSoundGuide.streamingTitle': '串流卡頓邊界',
  'settings.playback.noSoundGuide.title': '聲音排障向導',
  'settings.playback.noSoundGuide.warningDrivers': '不要使用第三方虛擬音效卡、魔改音效驅動或來路不明的 ASIO 包；優先用 Windows 內建驅動或裝置官方驅動。',
  'settings.playback.noSoundGuide.warningSampleRate': '不要在 Windows 音訊設定裡把取樣率改得過高；384kHz / 768kHz 這類設定很容易讓共享輸出、藍牙或普通音效卡無聲。',
  'settings.playback.noSoundGuide.warningTitle': '先避開兩個常見坑',
  'settings.playback.speedMode.description': '選擇播放器底部速度滑桿使用的變速方式。',
  'settings.playback.speedMode.title': '變速模式',
  'settings.playback.outputDevice.title': '輸出裝置',
  'settings.playback.outputDevice.empty': '沒有可用裝置',
  'settings.playback.sharedBackend.description': 'DirectSound 只作為手動相容模式，延遲較大；日常播放保持 WASAPI Shared。',
  'settings.playback.sharedBackend.alsa': 'ALSA',
  'settings.playback.sharedBackend.auto': '自動',
  'settings.playback.sharedBackend.directSound': 'DirectSound 相容',
  'settings.playback.sharedBackend.linuxDescription': 'Linux 下預設優先 ALSA；如果系統把 PipeWire 接到 ALSA 相容層，也會走這條共享輸出路徑。',
  'settings.playback.sharedBackend.title': '共享後端',
  'settings.playback.sharedBackend.wasapi': 'WASAPI Shared',
  'settings.playback.resetEngine.action': '重啟音訊引擎',
  'settings.playback.resetEngine.busy': '正在重啟',
  'settings.playback.resetEngine.description': '停止目前播放並釋放音訊引擎。裝置或驅動卡住時可以先試這個，不必退出軟體。',
  'settings.playback.resetEngine.done': '音訊引擎已重啟，你可以重新開始播放',
  'settings.playback.resetEngine.title': '重啟音訊引擎',
  'settings.playback.segmentLoop.description': '設定目前歌曲的 A/B 點、開啟片段循環，並儲存目前曲目的片段書籤。',
  'settings.playback.segmentLoop.title': 'A-B 循環',
  'settings.playback.replayGain.action.advanced': '進階',
  'settings.playback.replayGain.action.analyzeMissing': '分析缺失音量',
  'settings.playback.replayGain.action.analyzing': '分析中...',
  'settings.playback.replayGain.description': '開啟後自動平衡不同歌曲的聽感音量，不修改你的音樂檔案。',
  'settings.playback.replayGain.error': '音量分析錯誤 {count} 個，已略過問題檔案。',
  'settings.playback.replayGain.field.applied': '目前套用',
  'settings.playback.replayGain.field.mode': '模式',
  'settings.playback.replayGain.field.preventClipping': '防削波',
  'settings.playback.replayGain.field.preamp': '前級增益',
  'settings.playback.replayGain.field.progress': '進度',
  'settings.playback.replayGain.field.target': '目標響度',
  'settings.playback.replayGain.mode.album': '專輯',
  'settings.playback.replayGain.mode.off': '關閉',
  'settings.playback.replayGain.mode.track': '單曲',
  'settings.playback.replayGain.notRun': '尚未執行',
  'settings.playback.replayGain.preset.quiet': '安靜 (-18 LUFS)',
  'settings.playback.replayGain.preset.standard': '標準 (-14 LUFS)',
  'settings.playback.replayGain.status.disabled': '未開啟',
  'settings.playback.replayGain.status.enabled': '已開啟',
  'settings.playback.replayGain.title': '音量標準化',
  'settings.playback.replayGain.toggle.analyzeOnPlay': '播放時分析',
  'settings.playback.replayGain.toggle.analyzeOnScan': '掃描後分析',
  'settings.playback.replayGain.toggle.preventClipping': '防削波',
  'settings.playback.status.off': '關閉',
  'settings.playback.status.on': '開啟',
  'settings.playback.troubleshooting.description': '如果聲音卡住或裝置列表不正常，點這裡。軟重啟不會影響其他應用程式。',
  'settings.playback.troubleshooting.hardAction': '重啟 Windows 音訊服務',
  'settings.playback.troubleshooting.hardBusy': '正在重啟 Windows 音訊服務',
  'settings.playback.troubleshooting.hardConfirm': '這會中斷所有應用程式的聲音（Chrome、遊戲、通話），並需要系統管理員權限。是否繼續？',
  'settings.playback.troubleshooting.hardDone': 'Windows 音訊服務已恢復，你可以重新開始播放',
  'settings.playback.troubleshooting.softAction': '重啟音訊引擎',
  'settings.playback.troubleshooting.softBusy': '正在重啟音訊引擎',
  'settings.playback.troubleshooting.softDone': '音訊引擎已重啟，你可以重新開始播放',
  'settings.playback.troubleshooting.title': '音訊故障排除',
  'settings.playback.wireless.title': '無線播放',
  'settings.playback.audioStatus.title': '音訊狀態',
  'settings.playback.audioStatus.description': '分別顯示檔案取樣率和裝置取樣率，避免誤以為獨占模式被鎖在 48 kHz。',
  'settings.playback.automix.description': '目前歌曲快結束時自動準備下一首，並以平滑混音銜接。適合連續聽歌，預設關閉。',
  'settings.playback.automix.title': '智慧過渡',
  'settings.playback.stability.action.copied': '已複製',
  'settings.playback.stability.action.copy': '複製診斷資訊',
  'settings.playback.stability.action.refresh': '重新整理播放穩定性診斷',
  'settings.playback.stability.description': '查看目前輸出、緩衝和自動復原情況。只有排查卡頓或斷音時才需要展開。',
  'settings.playback.stability.details': '查看完整技術資訊',
  'settings.playback.stability.error.desktopBridgeUnavailable': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'settings.playback.stability.field.lastSharedStabilityRecoveryAt': '上次 Shared 穩定性復原時間',
  'settings.playback.stability.field.lastWatchdogRecoveryTime': '上次 watchdog 復原時間',
  'settings.playback.stability.field.nativeBufferedFrames': 'Native 緩衝影格',
  'settings.playback.stability.field.nativeBufferedMs': 'Native 緩衝毫秒',
  'settings.playback.stability.field.nativeDeviceBufferFrames': '裝置緩衝影格',
  'settings.playback.stability.field.nativeFifoCapacityFrames': 'Native FIFO 容量影格',
  'settings.playback.stability.field.nativeStartupPrebufferFrames': '啟動預緩衝影格',
  'settings.playback.stability.field.nativeUnderrunCallbacks': 'Native underrun 回呼',
  'settings.playback.stability.field.nativeUnderrunFrames': 'Native underrun 影格',
  'settings.playback.stability.field.recentWatchdogRecoveryCount': '近期 watchdog 復原次數',
  'settings.playback.stability.field.sharedStabilityTier': 'Shared 穩定性檔位',
  'settings.playback.stability.field.watchdogStatus': 'watchdog 狀態',
  'settings.playback.stability.title': '播放穩定性診斷',
  'settings.playback.stability.status.attention': '需要注意',
  'settings.playback.stability.status.idle': '等待播放',
  'settings.playback.stability.status.stable': '運作穩定',
  'settings.playback.stability.summary.buffer': '目前緩衝',
  'settings.playback.stability.summary.output': '輸出裝置',
  'settings.playback.stability.summary.playback': '播放狀態',
  'settings.playback.stability.summary.recovery': '近期復原',
  'settings.playback.stability.value.unknown': '未知',
  'settings.integrations.discord.action.refresh': '重新整理狀態',
  'settings.integrations.discord.title': 'Discord 狀態',
  'settings.integrations.obs.description': '開啟本機透明瀏覽器來源輸出，把連結加入 OBS、直播軟體或錄影工具的瀏覽器來源即可顯示目前歌詞和曲目資訊，不播放音訊。',
  'settings.integrations.obs.title': '瀏覽器來源輸出',
  'settings.integrations.stage.action.copyUrl': '複製連結',
  'settings.integrations.stage.description': '開啟本機 HTTP/SSE 介面，供 Stage 用戶端讀取目前播放與歌詞狀態。',
  'settings.integrations.stage.status.notRunning': '未執行',
  'settings.integrations.stage.title': 'Stage API',
  'settings.integrations.smtc.description': '把目前播放資訊、封面、進度和媒體鍵動作發布到 Windows 音量浮層與鎖定畫面媒體控制。',
  'settings.integrations.smtc.action.restart': '重新啟動 SMTC',
  'settings.integrations.smtc.action.restarting': '重新啟動中...',
  'settings.integrations.taskbarPlayback.description': '在 Windows 工作列圖示上顯示播放進度，並在懸停縮圖中提供上一首、播放暫停和下一首按鈕。',
  'settings.integrations.taskbarPlayback.title': '工作列音樂控制',
  'settings.integrations.taskbarMiniPlayer.description': '在 Windows 工作列中顯示封面、歌曲資訊和播放按鈕；預設關閉。',
  'settings.integrations.taskbarMiniPlayer.title': '工作列迷你播放器',
  'settings.integrations.smtc.title': 'Windows 媒體控制',
  'settings.integrations.lastfm.action.completeAuth': '完成授權',
  'settings.integrations.lastfm.action.connect': '連接 Last.fm',
  'settings.integrations.lastfm.action.disconnect': '斷開連接',
  'settings.integrations.lastfm.action.refresh': '重新整理狀態',
  'settings.integrations.lastfm.activeProgress': '{artist} - {title} · {played}/{threshold} 秒',
  'settings.integrations.lastfm.activeTrack': '目前曲目',
  'settings.integrations.lastfm.connection.description': '建議使用瀏覽器授權。在 Last.fm 點 Allow 後，回到 ECHO 完成授權。',
  'settings.integrations.lastfm.connection.title': 'Last.fm 連接',
  'settings.integrations.lastfm.description': '在主行程記錄本地播放，不傳送檔案路徑、歌詞或封面。',
  'settings.integrations.lastfm.lastNowPlaying': '上次 Now Playing',
  'settings.integrations.lastfm.lastScrobble': '上次 Scrobble',
  'settings.integrations.lastfm.never': '尚未傳送',
  'settings.integrations.lastfm.noActiveTrack': '沒有活躍曲目',
  'settings.integrations.lastfm.nowPlaying.description': '開始播放時傳送一次目前曲目資訊。',
  'settings.integrations.lastfm.nowPlaying.title': 'Last.fm Now Playing',
  'settings.integrations.lastfm.scrobbling.description': '曲目達到 Last.fm 記錄門檻後提交播放記錄。',
  'settings.integrations.lastfm.scrobbling.title': 'Last.fm Scrobbling',
  'settings.integrations.lastfm.status.connected': '已連接 {username}',
  'settings.integrations.lastfm.status.error': '錯誤：{error}',
  'settings.integrations.lastfm.status.notConnected': '未連接',
  'settings.integrations.lastfm.status.pending': '等待完成授權',
  'settings.integrations.lastfm.statusLabel': '狀態',
  'settings.integrations.lastfm.title': 'Last.fm',
  'settings.integrations.mobile.title': '手機遙控',
  'settings.integrations.mobile.description': '把手機當遙控器使用。目前版本還在準備中，不會讓手機直接控制系統資源。',
  'settings.integrations.accounts.cookieFallback': '登入帳號後會自動儲存 Cookie；手動貼上 Cookie 作為備用方式。',
  'settings.integrations.accounts.cookiePlaceholder': '貼上 Cookie 後儲存',
  'settings.integrations.accounts.description.default': '歌詞、中繼資料和下載接入預留。',
  'settings.integrations.accounts.description.bilibili': '用於 MV 解析和高畫質。',
  'settings.integrations.accounts.loginAndSync': '登入並同步',
  'settings.integrations.accounts.clickToLogin': '未登入，點此登入',
  'settings.integrations.accounts.logout': '登出',
  'settings.integrations.accounts.logoutBusy': '登出中...',
  'settings.integrations.accounts.manualSave': '手動儲存',
  'settings.integrations.accounts.manualSaveBusy': '儲存中...',
  'settings.integrations.accounts.check': '檢查',
  'settings.integrations.accounts.checkBusy': '檢查中...',
  'settings.integrations.accounts.loginBusy': '等待登入...',
  'settings.integrations.accounts.loginMeta': '登入 {loginAt} · 檢查 {checkedAt}',
  'settings.integrations.accounts.loginStatus': '登入狀態',
  'settings.integrations.accounts.soundcloudNote': 'SoundCloud 串流播放會使用這裡儲存的登入 Cookie，不需要 Artist Pro 或開發者 API。',
  'settings.integrations.accounts.osuNote': 'osu! 譜面下載會優先使用這裡儲存的登入 Cookie；官方失敗時會自動嘗試 Sayobot、Catboy 和 NeriNyan 鏡像。',
  'settings.integrations.accounts.youtube.browser': '瀏覽器',
  'settings.integrations.accounts.youtube.browserNone': '不使用',
  'settings.integrations.accounts.youtube.description': '沿用系統瀏覽器登入邏輯，供後續解析/下載使用。',
  'settings.integrations.accounts.youtube.savedStatus': '選擇瀏覽器後會儲存系統瀏覽器登入狀態。',
  'settings.integrations.accounts.spotify.description': '官方播放器接入，需要 Premium；請先在上方填寫自己的 Spotify Client ID，並在 Spotify Dashboard 註冊回調地址。',
  'settings.integrations.accounts.spotify.login': '登入 Spotify',
  'settings.integrations.accounts.spotify.loginBusy': '等待授權...',
  'settings.integrations.accounts.spotify.savedStatus': '使用 OAuth PKCE 授權，不儲存 Client Secret；下載功能不適用於 Spotify。',
  'settings.integrations.accountPanel.title': '帳號登入',
  'settings.integrations.accountPanel.description': '儲存平台登入狀態，供後續歌詞、中繼資料、MV、下載和串流媒體接入使用。Cookie 會在登入帳號後自動儲存。',
  'settings.integrations.accountPanel.refreshAll': '全部重新整理',
  'settings.integrations.accountStartupRefresh.title': '啟動時重新整理帳號登入狀態',
  'settings.integrations.accountStartupRefresh.description': '只檢查以前登入過的帳號，從未登入過的平台會保持靜默。',
  'settings.integrations.credentialPanel.title': '開發者 / API 設定',
  'settings.integrations.credentialPanel.description': 'Spotify、TIDAL、Discogs、線上歌手與 Last.fm；用不到時可以保持收起。',
  'settings.integrations.credentialPanel.expand': '展開 API 設定',
  'settings.integrations.credentialPanel.collapse': '收起 API 設定',
  'settings.integrations.smtcLyrics.title': 'SMTC 歌詞顯示',
  'settings.integrations.smtcLyrics.description': '允許把目前歌詞行附加到 Windows 媒體資訊裡；預設關閉，避免污染歌手欄位。',
  'settings.integrations.spotifyAutoLaunchOfficialPlayer.title': 'Spotify 自動啟動官方播放器',
  'settings.integrations.spotifyAutoLaunchOfficialPlayer.description': '播放 Spotify 時，如果 ECHO 內建 SDK 因 DRM 不可用，會自動打開 Spotify 桌面端或網頁版並接管 Connect 裝置。',
  'settings.integrations.networkProxy.title': '網路代理',
  'settings.integrations.networkProxy.description': '給登入頁、網路封面、歌詞、MV 搜尋和中繼資料補全使用。媒體播放流預設不走代理，避免影響緩衝和 Range 請求。',
  'settings.integrations.networkProxy.mode': '模式',
  'settings.integrations.networkProxy.modeAria': '網路代理模式',
  'settings.integrations.networkProxy.mode.off': '關閉',
  'settings.integrations.networkProxy.mode.system': '系統代理',
  'settings.integrations.networkProxy.mode.manual': '手動代理',
  'settings.integrations.networkProxy.manualUrl': '手動代理位址',
  'settings.integrations.networkProxy.manualPlaceholder': 'http://127.0.0.1:7890 或 socks5://127.0.0.1:7890',
  'settings.integrations.networkProxy.pacUrl': 'PAC 位址',
  'settings.integrations.networkProxy.bypass': '略過位址',
  'settings.integrations.networkProxy.save': '儲存並套用',
  'settings.integrations.networkProxy.saveBusy': '儲存中...',
  'settings.integrations.networkProxy.test': '測試連線',
  'settings.integrations.networkProxy.testBusy': '測試中...',
  'settings.integrations.networkProxy.note': '第一版只預設代理一般聯網能力；遠端曲庫和播放位元組流保持直連，避免影響正在播放的穩定性。',
  'settings.remote.hero.title': '網路硬碟 / 遠端音樂庫',
  'settings.remote.hero.summary': '推薦 Subsonic / Navidrome；其他協議僅保留相容，不主動維護',
  'settings.remote.hero.description': '遠端曲庫優先用 Subsonic / Navidrome。其他網盤入口只做相容，不保證後續維護；連上後仍會建立本地索引，方便歌詞、進度、收藏和歷史。',
  'settings.remote.empty.lede': '連接你的遠端音樂來源，統一瀏覽、播放與同步。',
  'settings.remote.empty.alternatives': '其他連接方式',
  'settings.remote.empty.preview.title': '連接後，你將看到',
  'settings.remote.empty.preview.browse.title': '完整的音樂瀏覽',
  'settings.remote.empty.preview.browse.description': '歌曲、專輯、藝人與資料夾統一呈現',
  'settings.remote.empty.preview.sync.title': '清楚的同步進度',
  'settings.remote.empty.preview.sync.description': '隨時查看索引、封面與中繼資料狀態',
  'settings.remote.empty.preview.playback.title': '統一管理與播放',
  'settings.remote.empty.preview.playback.description': '遠端音樂也能自然融入 ECHO 曲庫',
  'settings.remote.guardrail.aria': '遠端庫同步邊界',
  'settings.remote.guardrail.title': '本地播放優先',
  'settings.remote.guardrail.description': '遠端同步與封面/中繼資料補齊都會在背景限速執行；播放活躍時會降低並發。當前離線邊界是索引、封面和小型中繼資料快取，不會靜默鏡像整個音樂庫檔案。',
  'settings.remote.coverPerformance.aria': '遠端封面載入性能',
  'settings.remote.coverPerformance.title': '載入性能',
  'settings.remote.coverPerformance.description': '控制遠端封面預熱強度；只影響清單封面載入，不會改變播放取流。',
  'settings.remote.coverPerformance.groupAria': '遠端封面載入性能檔位',
  'settings.remote.coverPerformance.option.low': '省流量',
  'settings.remote.coverPerformance.option.low.description': '只預熱目前可見封面，適合公網或較弱的伺服器。',
  'settings.remote.coverPerformance.option.balanced': '均衡',
  'settings.remote.coverPerformance.option.balanced.description': '預熱目前與後方約半屏到一屏的封面，預設推薦。',
  'settings.remote.coverPerformance.option.aggressive': '積極預熱',
  'settings.remote.coverPerformance.option.aggressive.description': '滾動停頓時提前拉取更多封面，適合區域網內的 Navidrome。',
  'settings.remote.coverPerformance.option.lan': '區域網極速',
  'settings.remote.coverPerformance.option.lan.description': '把遠端封面預熱與背景載入並發拉滿，適合本機、NAS 或穩定的千兆區域網。',
  'settings.remote.albumMerge.aria': '遠端專輯合併策略',
  'settings.remote.albumMerge.title': '專輯合併',
  'settings.remote.albumMerge.description': '只影響網路硬碟 / 遠端專輯列表的展示分組；不改本地庫、不改檔案標籤、不影響播放。',
  'settings.remote.albumMerge.groupAria': '遠端專輯合併檔位',
  'settings.remote.albumMerge.option.conservative': '保守合併',
  'settings.remote.albumMerge.option.conservative.description': '優先使用伺服器專輯 ID；一般網路硬碟只合併同來源、同資料夾、同專輯的曲目。',
  'settings.remote.albumMerge.option.standard': '一般合併',
  'settings.remote.albumMerge.option.standard.description': '在保守邊界上放寬標題差異，會合併大小寫、符號與 - Single 這類遠端專輯拆分。',
  'settings.remote.albumMerge.stat.current': '原專輯數量',
  'settings.remote.albumMerge.stat.target': '合併後數量',
  'settings.remote.albumMerge.stat.tracks': '已索引遠端歌曲',
  'settings.remote.albumMerge.pending': '待掃描',
  'settings.remote.albumMerge.action.scan': '掃描遠端曲庫',
  'settings.remote.albumMerge.action.scanning': '掃描中...',
  'settings.remote.albumMerge.action.apply': '套用並重新整理分組',
  'settings.remote.albumMerge.action.applying': '重新整理中...',
  'settings.remote.library.title': '遠端音樂庫',
  'settings.remote.library.description': '連接 WebDAV、Jellyfin、Emby、SMB、SSHFS、Subsonic / Navidrome，並按需索引遠端音樂。',
  'settings.remote.home.add': '新增音樂庫',
  'settings.remote.home.description': '開啟音樂、檢查更新或新增來源；日常操作都在這裡完成。',
  'settings.remote.home.disconnect': '中斷連線',
  'settings.remote.home.eyebrow': '我的遠端音樂',
  'settings.remote.home.hideAdd': '收起新增',
  'settings.remote.home.lastSync': '上次同步',
  'settings.remote.home.manage': '管理與診斷',
  'settings.remote.home.open': '開啟根目錄',
  'settings.remote.home.opening': '正在開啟',
  'settings.remote.home.playing': '播放中',
  'settings.remote.home.reconnect': '重新連線',
  'settings.remote.home.reconnecting': '正在重新連線',
  'settings.remote.home.sourceManage': '管理',
  'settings.remote.home.sourcesAria': '已連接的遠端音樂庫',
  'settings.remote.home.stopOpening': '停止開啟',
  'settings.remote.home.stopPreview': '停止預覽',
  'settings.remote.home.sync': '預覽同步',
  'settings.remote.home.syncing': '正在預覽',
  'settings.remote.maintenance.close': '收起維護工具',
  'settings.remote.maintenance.description': '連線測試、中繼資料補齊、來源設定和背景參數集中放在這裡；日常聽歌不需要開啟。',
  'settings.remote.maintenance.eyebrow': '需要時再開啟',
  'settings.remote.maintenance.open': '開啟維護工具',
  'settings.remote.maintenance.title': '維護工具',
  'segmentLoop.action.clear': '清除目前 A-B 點',
  'segmentLoop.action.deleteBookmark': '刪除片段書籤 {label}',
  'segmentLoop.action.deleteBookmarkTitle': '刪除片段書籤',
  'segmentLoop.action.loopBookmark': '循環片段 {label}',
  'segmentLoop.action.loopBookmarkTitle': '循環 {label}',
  'segmentLoop.action.saveBookmark': '儲存目前片段書籤',
  'segmentLoop.action.setA': '把目前位置設為 A 點',
  'segmentLoop.action.setB': '把目前位置設為 B 點',
  'segmentLoop.action.toggle': '切換 A-B 循環',
  'segmentLoop.action.toggleTitle': '開啟或關閉 A-B 循環',
  'segmentLoop.aria.bookmarks': '目前曲目的片段書籤',
  'segmentLoop.aria.panel': 'A-B 循環和片段書籤',
  'segmentLoop.empty': '儲存片段後會顯示在這裡',
  'segmentLoop.notSet': '未設定',
  'spotifyPlayback.error.noDevice': '沒有可用的 Spotify 播放裝置。請開啟「自動啟動官方播放器」，或先打開 Spotify 桌面端/網頁版。{hint}',
  'sleepTimer.title': '睡眠定時器',
  'sleepTimer.action.pause': '暫停',
  'sleepTimer.action.stop': '停止',
  'sleepTimer.action.quit': '退出',
  'sleepTimer.fadeOut.label': '觸發前漸弱',
  'sleepTimer.custom.placeholder': '1-120',
  'sleepTimer.custom.unit': '分鐘',
  'sleepTimer.custom.confirm': '確定',
  'sleepTimer.status.active': '剩餘 {time}',
  'sleepTimer.status.inactive': '未啟動',
  'sleepTimer.status.cancel': '取消定時器',
  'sleepTimer.preset.5': '5分鐘',
  'sleepTimer.preset.10': '10分鐘',
  'sleepTimer.preset.15': '15分鐘',
  'sleepTimer.preset.30': '30分鐘',
  'sleepTimer.preset.45': '45分鐘',
  'sleepTimer.preset.60': '60分鐘',
  'sleepTimer.afterAction': '到期後{action}',
  'spotifyPlayback.error.noDrmKeysystem': '目前 Electron 建置沒有可用的 DRM/Widevine keysystem，Spotify 官方播放器無法在 ECHO 內註冊裝置。',
  'settings.eq.ab.summary': '{preset} / 峰值 {peak} / 輸出 {output} / 前級 {preamp}',
  'settings.eq.level.clips': '削波 {count}',
  'settings.eq.level.estimatedOutputPeak': '估算輸出峰值',
  'settings.eq.level.headroom': '餘量',
  'settings.eq.level.inputPeak': '輸入峰值',
  'settings.eq.level.inputRms': '輸入 RMS',
  'settings.eq.level.sourceEstimate': '解碼後 + DSP 估算',
  'settings.appearance.theme.title': '主題',
  'settings.appearance.theme.description': '選擇淺色、深色、Ambient，或跟隨系統外觀。',
  'settings.appearance.theme.ambient': '氛圍',
  'settings.appearance.theme.light': '淺色',
  'settings.appearance.theme.dark': '深色',
  'settings.appearance.theme.followSystem': '跟隨系統',
  'settings.appearance.themeSchedule.title': '定時切換深色模式',
  'settings.appearance.themeSchedule.description': '依照系統時間，到點自動切到深色，再按設定時間切回淺色。',
  'settings.appearance.themeSchedule.toggle': '啟用定時',
  'settings.appearance.themeSchedule.toggleAria': '啟用定時切換深色模式',
  'settings.appearance.themeSchedule.darkAt': '切到深色',
  'settings.appearance.themeSchedule.lightAt': '切回淺色',
  'settings.appearance.themeSchedule.status.ambient': 'Ambient 會保持生效；定時切換只影響淺色、深色和跟隨系統。',
  'settings.appearance.themeSchedule.status.enabled': '已啟用：{darkAt} 切到{mode}，{lightAt} 自動切回淺色。目前依本機時間使用{mode}。',
  'settings.appearance.themeSchedule.status.disabled': '關閉後仍使用上面的手動主題模式。',
  'settings.appearance.sidebar.title': '側邊欄',
  'settings.appearance.sidebar.description': '調整側邊入口的順序與顯示狀態，不會改動頁面或播放鏈路。',
  'settings.appearance.sidebar.expand': '展開側邊欄佈局',
  'settings.appearance.sidebar.collapse': '收起側邊欄佈局',
  'settings.appearance.sidebar.mainGroup': '主導覽',
  'settings.appearance.sidebar.utilityGroup': '底部入口',
  'settings.appearance.sidebar.reset': '恢復預設',
  'settings.appearance.sidebar.visible': '顯示',
  'settings.appearance.sidebar.hidden': '隱藏',
  'settings.appearance.sidebar.fixed': '固定顯示',
  'settings.appearance.sidebar.proLocked': 'Pro 鎖定',
  'settings.appearance.sidebar.noItems': '這一組沒有可調整的入口。',
  'settings.appearance.sidebar.summary.allVisible': '全部顯示',
  'settings.appearance.sidebar.summary.hidden': '已隱藏 {count} 個入口',
  'settings.appearance.sidebar.count': '{count} 項',
  'settings.appearance.sidebar.moveUp': '上移',
  'settings.appearance.sidebar.moveDown': '下移',
  'settings.appearance.sidebar.moveUpAria': '上移 {label}',
  'settings.appearance.sidebar.moveDownAria': '下移 {label}',
  'settings.appearance.sidebar.hideAria': '隱藏 {label}',
  'settings.appearance.sidebar.showAria': '顯示 {label}',
  'settings.appearance.sidebar.proLockedAria': '{label} 需要 Pro 外掛解鎖',
  'settings.appearance.nowPlayingCoverColor.title': '播放介面封面取色',
  'settings.appearance.nowPlayingCoverColor.description': '開啟後，正在播放頁會在閒置時從小封面取樣生成輕量背景；減輕占用模式會自動略過。預設關閉。',
  'settings.appearance.playerBarButtons.title': '右下角按鈕',
  'settings.appearance.playerBarButtons.description': '選擇底部播放器右下角要顯示的快捷按鈕。預設隱藏睡眠定時和匯出檔案，其餘顯示。',
  'settings.appearance.playerBarButtons.count': '顯示 {count} 個按鈕',
  'settings.appearance.playerBarButtons.reset': '恢復預設',
  'settings.appearance.playerBarButtons.visible': '顯示',
  'settings.appearance.playerBarButtons.hidden': '隱藏',
  'settings.appearance.playerBarButtons.sleepTimer': '睡眠定時',
  'settings.appearance.playerBarButtons.sleepTimer.description': '開啟睡眠定時器。',
  'settings.appearance.playerBarButtons.desktopLyrics': '桌面歌詞',
  'settings.appearance.playerBarButtons.desktopLyrics.description': '顯示或隱藏桌面歌詞。',
  'settings.appearance.playerBarButtons.miniPlayer': '迷你播放器',
  'settings.appearance.playerBarButtons.miniPlayer.description': '打開或收起迷你播放器。',
  'settings.appearance.playerBarButtons.volume': '音量',
  'settings.appearance.playerBarButtons.volume.description': '調整目前輸出音量。',
  'settings.appearance.playerBarButtons.speed': '播放速度',
  'settings.appearance.playerBarButtons.speed.description': '調整本機播放速度。',
  'settings.appearance.playerBarButtons.streamingDownload': '串流下載',
  'settings.appearance.playerBarButtons.streamingDownload.description': '下載支援的串流曲目。',
  'settings.appearance.playerBarButtons.audioExport': '匯出檔案',
  'settings.appearance.playerBarButtons.audioExport.description': '匯出目前本機音訊檔案。',
  'settings.appearance.windowAcrylic.title': '視窗壓克力',
  'settings.appearance.windowAcrylic.experimental': '實驗性',
  'settings.appearance.windowAcrylic.description': '開啟後會立即使用系統壓克力材質，讓桌面背景從視窗後方透出；介面會保留可讀遮罩。Windows 11 22H2 以上效果最佳。',
  'settings.appearance.windowAcrylic.themeWarning': '實驗功能：切換主題可能導致視窗透明暫時失效，如遇到請重新啟動 ECHO 或重新開關視窗壓克力。',
  'settings.appearance.windowAcrylic.keepWhenUnfocused': '失焦時保持通透',
  'settings.appearance.windowAcrylic.transparency': '透明度',
  'settings.appearance.albumCoverShape.title': '專輯封面形狀',
  'settings.appearance.albumCoverShape.description': '選擇專輯封面顯示為圓角或方角；此設定優先於主題預設。預設圓角。',
  'settings.appearance.albumCoverShape.rounded': '圓角',
  'settings.appearance.albumCoverShape.square': '方角',
  'settings.appearance.wallpaper.title': '自訂背景',
  'settings.appearance.wallpaper.description': '選擇一張圖片或本機影片，ECHO 會自動適配視窗並保持文字清晰。',
  'settings.appearance.wallpaper.choose': '選擇背景',
  'settings.appearance.wallpaper.replace': '更換背景',
  'settings.appearance.wallpaper.clear': '關閉自訂背景',
  'settings.appearance.wallpaper.current': '目前背景',
  'settings.appearance.wallpaper.emptyTitle': '讓 ECHO 更像你自己的空間',
  'settings.appearance.wallpaper.emptyDescription': '只需選擇一次；窄視窗預設使用同一張背景。',
  'settings.appearance.wallpaper.effect.title': '顯示效果',
  'settings.appearance.wallpaper.effect.balanced': '清晰自然',
  'settings.appearance.wallpaper.effect.soft': '柔和',
  'settings.appearance.wallpaper.effect.immersive': '沉浸',
  'settings.appearance.wallpaper.advanced.expand': '更多調整',
  'settings.appearance.wallpaper.advanced.collapse': '收起調整',
  'settings.appearance.wallpaper.portraitOptional': '窄視窗專用背景（可選）',
  'settings.appearance.wallpaper.portraitFallback': '未單獨設定，將自動使用目前背景。',
  'settings.appearance.wallpaper.portraitChoose': '選擇直向背景',
  'settings.appearance.wallpaper.portraitReplace': '更換直向背景',
  'settings.appearance.wallpaper.portraitClear': '清除直向背景',
  'settings.appearance.wallpaper.ambientPaused': '環境主題正在接管背景；切換到其他主題後會恢復自訂背景。',
  'settings.appearance.wallpaper.lowSpecPaused': '輕量模式已停止載入影片背景，以減少資源使用。',
  'settings.appearance.wallpaper.loadError': '背景檔案無法顯示，請更換圖片或使用 H.264 MP4 / WebM 影片。',
  'settings.appearance.wallpaper.landscapePath': '橫向',
  'settings.appearance.wallpaper.portraitPath': '直向',
  'settings.appearance.wallpaper.videoStatus': '影片桌布 · 靜音循環',
  'settings.appearance.wallpaper.videoPause.smart': '智慧暫停',
  'settings.appearance.wallpaper.videoPause.minimized': '最小化暫停',
  'settings.appearance.wallpaper.videoPause.never': '持續播放',
  'settings.appearance.wallpaper.scale': '桌布縮放',
  'settings.appearance.wallpaper.blur': '桌布模糊度',
  'settings.appearance.wallpaper.brightness': '桌布亮度',
  'settings.appearance.wallpaper.uiOpacity': 'UI 透明度',
  'settings.appearance.wallpaper.visualProtection': '可視化保護',
  'settings.appearance.wallpaper.unifiedOpacity': '統一透明度',
  'settings.appearance.themePreset.title': '主題預設',
  'settings.appearance.themePreset.description': '選擇一套全域漸層色板；目前的明暗模式仍會保留。',
  'settings.appearance.themePreset.ambientLocked': 'Ambient 是獨立主題，不能疊加其他主題預設。',
  'settings.appearance.themePreset.classic': '經典',
  'settings.appearance.themePreset.classic.description': '白灰基底、克制藍紫強調，清爽耐看。',
  'settings.appearance.themePreset.random': '隨機主題',
  'settings.appearance.themePreset.random.description': '產生一套通過可讀性校驗的隨機配色，滿意後可手動保存。',
  'settings.appearance.themePreset.echoTwilight': '暮光桃霧',
  'settings.appearance.themePreset.echoTwilight.description': '老版 ECHO 的暖粉漸層感。',
  'settings.appearance.themePreset.sakuraMilk': '櫻粉奶霜',
  'settings.appearance.themePreset.sakuraMilk.description': '奶白粉底配櫻桃紅強調。',
  'settings.appearance.themePreset.peachSoda': '蜜桃蘇打',
  'settings.appearance.themePreset.peachSoda.description': '蜜桃橙和蘇打青的輕快組合。',
  'settings.appearance.themePreset.mintCandy': '薄荷軟糖',
  'settings.appearance.themePreset.mintCandy.description': '薄荷綠、奶油白和一點桃粉。',
  'settings.appearance.themePreset.berryDream': '藍莓星糖',
  'settings.appearance.themePreset.berryDream.description': '莓紫雲白，帶一點夢幻粉光。',
  'settings.appearance.themePreset.matchaCream': '抹茶奶油',
  'settings.appearance.themePreset.matchaCream.description': '抹茶綠和奶油黃，更安靜耐看。',
  'settings.appearance.themePreset.lemonMochi': '檸檬麻糬',
  'settings.appearance.themePreset.lemonMochi.description': '奶黃和天藍，像柔軟的檸檬點心。',
  'settings.appearance.themePreset.cottonCloud': '棉花雲朵',
  'settings.appearance.themePreset.cottonCloud.description': '雲白藍配柔粉，明亮但不刺眼。',
  'settings.appearance.themePreset.melonCream': '哈密瓜奶霜',
  'settings.appearance.themePreset.melonCream.description': '蜜瓜綠與奶油底，清甜且易讀。',
  'settings.appearance.themePreset.seaSaltJelly': '海鹽果凍',
  'settings.appearance.themePreset.seaSaltJelly.description': '海鹽青配蜜桃光，清透但壓得住文字。',
  'settings.appearance.themePreset.caramelPudding': '焦糖布丁',
  'settings.appearance.themePreset.caramelPudding.description': '奶油焦糖配草莓光，甜但不膩。',
  'settings.appearance.themePreset.neonCandy': '霓虹糖果',
  'settings.appearance.themePreset.neonCandy.description': '紫色霓虹、粉色高光和薄荷泡泡。',
  'settings.appearance.themePreset.nyanCat': 'Nyan Cat',
  'settings.appearance.themePreset.nyanCat.description': '慢速流動的可愛彩虹漸層，進度條會帶著彩虹貓一起跑。',
  'settings.appearance.themePreset.childrenDoodle': '六一塗鴉',
  'settings.appearance.themePreset.childrenDoodle.description': '紙紋、蠟筆邊框和亂塗貼紙，像兒童節手帳一樣熱鬧。',
  'settings.appearance.themePreset.wisteriaBubble': '紫藤泡泡',
  'settings.appearance.themePreset.wisteriaBubble.description': '紫藤花霧配薄荷泡泡，夢幻但清爽。',
  'settings.appearance.themePreset.strawberryCookie': '草莓餅乾',
  'settings.appearance.themePreset.strawberryCookie.description': '奶油餅乾底配草莓紅和烘焙金。',
  'settings.appearance.themePreset.graphiteAurora': '石墨極光',
  'settings.appearance.themePreset.graphiteAurora.description': '石墨灰裡帶一點青綠極光，冷靜但有層次。',
  'settings.appearance.themePreset.amberNoir': '琥珀夜色',
  'settings.appearance.themePreset.amberNoir.description': '黑金唱片廳感，適合暗色長聽。',
  'settings.appearance.themePreset.oceanStudio': '海岸錄音室',
  'settings.appearance.themePreset.oceanStudio.description': '冷藍灰和海霧藍，乾淨專業。',
  'settings.appearance.themePreset.rosewoodVinyl': '玫瑰木黑膠',
  'settings.appearance.themePreset.rosewoodVinyl.description': '木質暖紅與黑膠暗調，更沉穩復古。',
  'settings.appearance.themePreset.darkSideMoon': 'The Dark Side of the Moon',
  'settings.appearance.themePreset.darkSideMoon.description': '致敬 Pink Floyd：黑月、白色稜鏡與彩虹光譜。',
  'settings.appearance.themePreset.shibuyaNight': '澀谷夜色',
  'settings.appearance.themePreset.shibuyaNight.description': '東京霓虹、夜紫街口和青色招牌光。',
  'settings.appearance.themePreset.kyotoKurenai': '京都朱印',
  'settings.appearance.themePreset.kyotoKurenai.description': '鳥居朱紅、和紙暖底和御守金色。',
  'settings.appearance.themePreset.ukiyoIndigo': '浮世靛藍',
  'settings.appearance.themePreset.ukiyoIndigo.description': '浮世繪海浪的靛藍、紙色和古金。',
  'settings.appearance.themePreset.fujiSnow': '富士初雪',
  'settings.appearance.themePreset.fujiSnow.description': '雪白、富士藍與淡櫻高光，清澈冷甜。',
  'settings.appearance.themePreset.matsuriLantern': '祭燈金魚',
  'settings.appearance.themePreset.matsuriLantern.description': '夏祭燈籠紅、夜市金光和溫暖紙色。',
  'settings.appearance.themePreset.ginzaNoir': '銀座黑曜',
  'settings.appearance.themePreset.ginzaNoir.description': '黑曜石、香檳金和櫥窗藍，成熟一點。',
  'settings.appearance.themePreset.frostJazz': '霜林爵士',
  'settings.appearance.themePreset.frostJazz.description': '冷藍爵士底色，帶一抹梅紫舞台光。',
  'settings.appearance.themePreset.celadonPorcelain': '青瓷微雨',
  'settings.appearance.themePreset.celadonPorcelain.description': '溫潤青瓷、雨後苔色和一筆淺茶金。',
  'settings.appearance.themePreset.polarDaybreak': '極光晨霧',
  'settings.appearance.themePreset.polarDaybreak.description': '冰川晨藍、柔霧白和克制的莓粉天光。',
  'settings.appearance.themePreset.plumVelvet': '梅絨夜曲',
  'settings.appearance.themePreset.plumVelvet.description': '深梅酒紅、絲絨紫和柔和舞台餘光。',
  'settings.appearance.themePreset.midnightCopper': '午夜銅金',
  'settings.appearance.themePreset.midnightCopper.description': '深夜藍黑、溫暖銅色和冷靜的青灰高光。',
  'settings.appearance.themePreset.taroBunny': '芋泥兔絨',
  'settings.appearance.themePreset.taroBunny.description': '奶油芋紫、莓粉兔耳感和柔軟奶白光。',
  'settings.appearance.themePreset.milkTeaBear': '奶茶小熊',
  'settings.appearance.themePreset.milkTeaBear.description': '烤奶茶、可可棕和一點草莓腮紅。',
  'settings.appearance.themePreset.ramuneGlass': '彈珠汽水',
  'settings.appearance.themePreset.ramuneGlass.description': '透明汽水藍、玻璃青和一顆櫻桃粉氣泡。',
  'settings.appearance.themePreset.pistachioMousse': '開心果慕斯',
  'settings.appearance.themePreset.pistachioMousse.description': '開心果奶綠、香草奶泡和溫暖杏桃收尾。',
  'settings.appearance.themePreset.sakuraWagashi': '櫻和菓子',
  'settings.appearance.themePreset.sakuraWagashi.description': '櫻花練切粉、抹茶綠和溫潤糯米白。',
  'settings.appearance.themePreset.kamakuraHydrangea': '鎌倉紫陽',
  'settings.appearance.themePreset.kamakuraHydrangea.description': '雨霧藍、紫陽花紫和潮濕石階的冷灰。',
  'settings.appearance.themePreset.ruriKintsugi': '瑠璃金繼',
  'settings.appearance.themePreset.ruriKintsugi.description': '瑠璃藍、漆黑陰影和克制的金繼光。',
  'settings.appearance.themePreset.hinokiRyokan': '檜木旅燈',
  'settings.appearance.themePreset.hinokiRyokan.description': '檜木暖白、榻榻米綠和旅館燈籠朱。',
  'settings.appearance.themePreset.nightGlass': '夜玻璃',
  'settings.appearance.themePreset.nightGlass.description': '冷夜玻璃：靛石板底、長春花強調、石青作伴。深色是母語。',
  'settings.appearance.themePreset.boneIron': '骨鐵',
  'settings.appearance.themePreset.boneIron.description': '展牆骨色，第三段掺冷灰。強調是氧化鐵，圓角收緊。',
  'settings.appearance.themePreset.harborLamp': '港燈',
  'settings.appearance.themePreset.harborLamp.description': '霧青水面、燈籠琥珀。玻璃偏高，動效略慢。',
  'settings.appearance.themePreset.ashRose': '灰薔',
  'settings.appearance.themePreset.ashRose.description': '乾薔薇配橄欖灰，第三段掺苔色，把甜味掐掉。',
  'settings.appearance.themePreset.FINAL': 'FINAL',
  'settings.appearance.themePreset.FINAL.description': '參考 final 官網的黑白產品攝影、聲學工程與 8K SOUND 語言，克制、精密、偏監聽。',
  'settings.appearance.themeCustom.title': '自訂目前主題',
  'settings.appearance.themeCustom.description': '先選一個主題，再微調顏色；每個主題都會記住自己的自訂。',
  'settings.appearance.themeCustom.ambientLocked': 'Ambient 是獨立主題，不能疊加我的主題或外掛主題。',
  'settings.appearance.themeCustom.action.autoFix': '自動修正文字',
  'settings.appearance.themeCustom.action.create': '新建我的主題',
  'settings.appearance.themeCustom.action.rename': '重新命名',
  'settings.appearance.themeCustom.action.duplicate': '複製',
  'settings.appearance.themeCustom.action.delete': '刪除',
  'settings.appearance.themeCustom.action.copyLightToDark': '複製淺色到深色',
  'settings.appearance.themeCustom.action.copyDarkToLight': '複製深色到淺色',
  'settings.appearance.themeCustom.action.export': '匯出參數',
  'settings.appearance.themeCustom.action.import': '匯入參數',
  'settings.appearance.themeCustom.action.reset': '重置目前自訂',
  'settings.appearance.themeCustom.action.save': '儲存自訂',
  'settings.appearance.themeCustom.expand': '展開我的主題',
  'settings.appearance.themeCustom.collapse': '收起我的主題',
  'settings.appearance.themeCustom.advanced.show': '展開進階設定',
  'settings.appearance.themeCustom.advanced.hide': '收起進階設定',
  'settings.appearance.themeCustom.field.appBg': '底色',
  'settings.appearance.themeCustom.field.appBg2': '漸層中段',
  'settings.appearance.themeCustom.field.appBg3': '漸層尾色',
  'settings.appearance.themeCustom.field.panel': '玻璃色調',
  'settings.appearance.themeCustom.field.panelSoft': '柔面板',
  'settings.appearance.themeCustom.field.accent': '主強調色',
  'settings.appearance.themeCustom.field.accentStrong': '次強調色',
  'settings.appearance.themeCustom.field.secondary': '第三強調色',
  'settings.appearance.themeCustom.field.heading': '主文字',
  'settings.appearance.themeCustom.field.text': '正文文字',
  'settings.appearance.themeCustom.field.muted': '次要文字',
  'settings.appearance.themeCustom.field.border': '邊界色',
  'settings.appearance.themeCustom.field.onAccent': '強調按鈕文字',
  'settings.appearance.themeCustom.field.buttonText': '一般按鈕文字',
  'settings.appearance.themeCustom.field.panelOpacity': '面板透明度',
  'settings.appearance.themeCustom.field.glass': '玻璃感',
  'settings.appearance.themeCustom.field.shadow': '陰影強度',
  'settings.appearance.themeCustom.field.titlebar': '標題列',
  'settings.appearance.themeCustom.field.sidebar': '側欄',
  'settings.appearance.themeCustom.field.player': '播放器',
  'settings.appearance.themeCustom.field.field': '輸入框',
  'settings.appearance.themeCustom.field.row': '列表列',
  'settings.appearance.themeCustom.field.rowHover': '懸停列',
  'settings.appearance.themeCustom.field.rowActive': '選中列',
  'settings.appearance.themeCustom.field.chip': '晶片',
  'settings.appearance.themeCustom.field.focus': '焦點環',
  'settings.appearance.themeCustom.field.success': '成功色',
  'settings.appearance.themeCustom.field.warning': '警告色',
  'settings.appearance.themeCustom.field.danger': '危險色',
  'settings.appearance.themeCustom.field.cornerRadius': '圓角',
  'settings.appearance.themeCustom.field.panelBlur': '面板模糊',
  'settings.appearance.themeCustom.field.saturation': '飽和度',
  'settings.appearance.themeCustom.field.motionEnabled': '啟用動效',
  'settings.appearance.themeCustom.field.motionSpeed': '動效速度',
  'settings.appearance.themeCustom.field.motionIntensity': '動效強度',
  'settings.appearance.themeCustom.field.appBg.description': '主視窗底色',
  'settings.appearance.themeCustom.field.appBg2.description': '背景漸層的柔光中段',
  'settings.appearance.themeCustom.field.appBg3.description': '背景漸層的末端停靠色',
  'settings.appearance.themeCustom.field.panel.description': '面板磨砂著色',
  'settings.appearance.themeCustom.field.panelSoft.description': '側欄和弱層級面板',
  'settings.appearance.themeCustom.field.accent.description': '主要互動',
  'settings.appearance.themeCustom.field.accentStrong.description': '漸層與層次',
  'settings.appearance.themeCustom.field.secondary.description': '高光點綴',
  'settings.appearance.themeCustom.field.heading.description': '標題與主文案',
  'settings.appearance.themeCustom.field.text.description': '正文、歌手和設定文案',
  'settings.appearance.themeCustom.field.muted.description': '輔助說明',
  'settings.appearance.themeCustom.field.border.description': '卡片邊框和分隔線',
  'settings.appearance.themeCustom.field.onAccent.description': '強調按鈕上的文字',
  'settings.appearance.themeCustom.field.buttonText.description': '一般按鈕和晶片文字',
  'settings.appearance.themeCustom.field.panelOpacity.description': '面板露出背景的程度',
  'settings.appearance.themeCustom.field.glass.description': '背景模糊和玻璃層次',
  'settings.appearance.themeCustom.field.shadow.description': '卡片、彈窗和播放器投影',
  'settings.appearance.themeCustom.field.titlebar.description': '視窗頂部列背景',
  'settings.appearance.themeCustom.field.sidebar.description': '左側導覽和弱層級區域',
  'settings.appearance.themeCustom.field.player.description': '底部播放器背景',
  'settings.appearance.themeCustom.field.field.description': '輸入框和搜尋框底色',
  'settings.appearance.themeCustom.field.row.description': '列表普通列背景',
  'settings.appearance.themeCustom.field.rowHover.description': '滑鼠懸停列背景',
  'settings.appearance.themeCustom.field.rowActive.description': '目前選中列背景',
  'settings.appearance.themeCustom.field.chip.description': '篩選晶片和小按鈕底色',
  'settings.appearance.themeCustom.field.focus.description': '鍵盤焦點和描邊高亮',
  'settings.appearance.themeCustom.field.success.description': '成功狀態提示',
  'settings.appearance.themeCustom.field.warning.description': '警告狀態提示',
  'settings.appearance.themeCustom.field.danger.description': '危險操作提示',
  'settings.appearance.themeCustom.field.cornerRadius.description': '面板和按鈕圓角大小',
  'settings.appearance.themeCustom.field.panelBlur.description': '玻璃面板模糊半徑',
  'settings.appearance.themeCustom.field.saturation.description': '介面整體色彩濃度',
  'settings.appearance.themeCustom.field.motionEnabled.description': '只影響 CSS 過渡變數',
  'settings.appearance.themeCustom.field.motionSpeed.description': 'CSS 動效時長',
  'settings.appearance.themeCustom.field.motionIntensity.description': 'CSS 位移和強調強度',
  'settings.appearance.themeCustom.preview.title': '正在編輯',
  'settings.appearance.themeCustom.preview.description': '改動會先即時預覽，儲存後才寫入設定。',
  'settings.appearance.themeCustom.myThemes.title': '我的主題',
  'settings.appearance.themeCustom.myThemes.description': '另存、切換、複製、匯入匯出安全主題參數。',
  'settings.appearance.themeCustom.myThemes.empty': '還沒有自訂主題。',
  'settings.appearance.themeCustom.group.core': '常用顏色',
  'settings.appearance.themeCustom.group.core.description': '老 ECHO 式主色板，改這裡最直觀。',
  'settings.appearance.themeCustom.group.gradient': '背景漸層',
  'settings.appearance.themeCustom.group.gradient.description': '控制老 ECHO 那種視窗底色漸層氛圍。',
  'settings.appearance.themeCustom.group.surface': '表面',
  'settings.appearance.themeCustom.group.surface.description': '標題列、側欄、播放器和列表層級。',
  'settings.appearance.themeCustom.group.state': '狀態',
  'settings.appearance.themeCustom.group.state.description': '成功、警告、危險和焦點色。',
  'settings.appearance.themeCustom.group.motion': '動效',
  'settings.appearance.themeCustom.group.motion.description': '僅寫入 CSS 變數，不增加執行時計時器。',
  'settings.appearance.themeCustom.group.advanced': '進階細節',
  'settings.appearance.themeCustom.group.advanced.description': '更細的文字、邊界和按鈕文字顏色。',
  'settings.appearance.themeCustom.message.created': '已新建我的主題。',
  'settings.appearance.themeCustom.message.copied': '已複製到目標色調，儲存後生效。',
  'settings.appearance.themeCustom.message.exported': '已匯出目前主題參數。',
  'settings.appearance.themeCustom.message.imported': '已匯入主題參數並套用。',
  'settings.appearance.themeCustom.message.importFailed': '匯入失敗，請選擇 ECHO 主題參數 JSON。',
  'settings.appearance.themeCustom.message.fixed': '已自動調整文字與按鈕顏色。',
  'settings.appearance.themeCustom.message.invalidColor': '請輸入 #RRGGBB 格式的安全顏色。',
  'settings.appearance.themeCustom.message.lowContrast': '目前文字對比不足，可能影響閱讀。可自動修正或手動調整文字顏色。',
  'settings.appearance.themeCustom.message.randomReady': '已產生隨機主題草稿，滿意後點保存。',
  'settings.appearance.themeCustom.message.reset': '已重置目前主題的自訂。',
  'settings.appearance.themeCustom.message.saved': '已儲存目前主題自訂。',
  'settings.appearance.artistAvatars.action.clear': '清除頭像快取',
  'settings.appearance.artistAvatars.action.queueing': '加入佇列中...',
  'settings.appearance.artistAvatars.action.refreshMissing': '重新整理缺失頭像',
  'settings.appearance.artistAvatars.description': '在背景慢速取得真實歌手頭像，並在藝術家牆重複使用本機快取圖片。',
  'settings.appearance.artistAvatars.fallback': '搜尋不到時使用藝術家的專輯封面',
  'settings.appearance.artistAvatars.message.cleared': '已清除 {removedRows} 筆頭像記錄和 {deletedFiles} 個檔案。',
  'settings.appearance.artistAvatars.message.desktopBridgeClear': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'settings.appearance.artistAvatars.message.desktopBridgeRefresh': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'settings.appearance.artistAvatars.message.enableFirst': '請先開啟自動取得歌手頭像。',
  'settings.appearance.artistAvatars.message.queued': '已加入 {queued} 個歌手頭像。略過 {skipped} 個。',
  'settings.appearance.artistAvatars.title': '歌手頭像',
  'settings.appearance.artistAvatars.toggle': '自動取得歌手頭像',
  'settings.devices.title': '裝置列表',
  'settings.devices.empty': '暫時還沒有收到輸出裝置列表。',
  'settings.danger.database.kicker': '曲庫資料庫安全',
  'settings.danger.database.title': '曲庫狀態',
  'settings.danger.database.badge.quarantined': '已隔離',
  'settings.danger.database.description.loading': '正在讀取健康狀態和快照。',
  'settings.danger.database.description.quarantined': '曲庫因損壞嵌入標籤或超大文字已被隔離。ECHO 會先打開恢復介面，音樂檔案不會被刪除。',
  'settings.danger.database.description.healthy': '檢查正常。可在這裡建立快照，損壞時從這裡復原。',
  'settings.danger.database.description.unrecoverable': '資料庫無法從健康快照恢復。音樂檔案不會被刪除；請先匯出診斷並查看保護目錄，再確認歸檔壞庫並重建空庫。',
  'settings.danger.database.description.recoverable': '偵測到資料庫不可用時，會先嘗試恢復健康快照；恢復前會先歸檔目前資料庫，音樂檔案不會被刪除。',
  'settings.danger.database.health.ok': '健康',
  'settings.danger.database.health.corrupt': '疑似損壞',
  'settings.danger.database.health.unreadable': '無法讀取',
  'settings.danger.database.health.idle': '待檢查',
  'settings.danger.database.steps.quarantined.1': '目前曲庫已被移出活動位置，ECHO 不會再讀取那批危險標籤。',
  'settings.danger.database.steps.quarantined.2': '優先使用「修復隔離庫副本」，它只處理歸檔副本，驗證通過後才恢復。',
  'settings.danger.database.steps.quarantined.3': '若修復失敗，再選擇恢復健康快照或歸檔並重建空曲庫；音樂檔案不會被刪除。',
  'settings.danger.database.steps.unrecoverable.1': '先確認掃描沒有執行；掃描中會拒絕恢復、重建與刪除。',
  'settings.danger.database.steps.unrecoverable.2': '優先匯出診斷並打開保護目錄，保留壞庫歸檔線索。',
  'settings.danger.database.steps.unrecoverable.3': '輸入確認詞「重建空庫」後，歸檔壞庫並重建空庫，再重新加入曲庫資料夾並掃描。',
  'settings.danger.database.steps.recoverable.1': '先確認掃描沒有執行；掃描中會拒絕恢復、重建與刪除。',
  'settings.danger.database.steps.recoverable.2': '優先點「恢復最近健康快照」，它只接受主程序列舉出的快照。',
  'settings.danger.database.steps.recoverable.3': '沒有健康快照或恢復後仍損壞時，使用「歸檔壞庫並重建空庫」。',
  'settings.danger.database.action.check': '檢查',
  'settings.danger.database.action.checking': '檢查中...',
  'settings.danger.database.action.create': '建立快照',
  'settings.danger.database.action.creating': '建立中...',
  'settings.danger.database.action.restore': '復原快照',
  'settings.danger.database.action.restoring': '復原中...',
  'settings.danger.database.action.scrub': '修復隔離庫',
  'settings.danger.database.action.scrubbing': '修復中...',
  'settings.danger.database.action.rebuild': '歸檔壞庫並重建空庫',
  'settings.danger.database.action.rebuilding': '重建中...',
  'settings.danger.database.action.discard': '歸檔問題曲目',
  'settings.danger.database.action.discarding': '歸檔中...',
  'settings.danger.database.action.relaunch': '復原模式',
  'settings.danger.database.action.relaunching': '重新啟動中...',
  'settings.danger.database.action.open': '打開目錄',
  'settings.danger.database.action.export': '匯出診斷',
  'settings.danger.database.action.exporting': '匯出中...',
  'settings.danger.database.meta.current': '目前庫',
  'settings.danger.database.meta.snapshot': '健康快照',
  'settings.danger.database.meta.snapshotHint': '可手動建立',
  'settings.danger.database.meta.archive': '壞庫歸檔',
  'settings.danger.database.meta.archiveHint': '恢復/重建前會自動歸檔',
  'settings.danger.database.meta.pending': '待載入',
  'settings.danger.database.meta.noSnapshot': '暫無健康快照',
  'settings.danger.database.meta.noArchive': '暫無壞庫歸檔',
  'settings.danger.database.unavailable.scrub': '找不到可修復的隔離庫副本；請打開保護目錄確認歸檔是否存在，或直接匯出診斷。',
  'settings.danger.database.unavailable.restore': '暫無健康快照可恢復。可以先建立健康快照；若目前曲庫不可用，請匯出診斷。',
  'settings.danger.database.confirmWord': '危險操作確認詞',
  'settings.danger.database.confirmPlaceholder': '先輸入按鈕提示的確認詞，再執行危險操作',
  'settings.danger.database.scanRunning': '曲庫掃描正在執行，恢復、重建與刪除會被拒絕。請等掃描結束後再操作。',
  'settings.danger.duplicates.title': '掃描清理',
  'settings.danger.duplicates.description': '先掃描，再把低評分副本移入資源回收筒。',
  'settings.danger.duplicates.meta.result': '掃描結果',
  'settings.danger.duplicates.meta.resultValue': '{groups} 組 / {tracks} 首待清理',
  'settings.danger.duplicates.meta.release': '預計釋放',
  'settings.danger.duplicates.meta.scanTime': '掃描時間',
  'settings.danger.duplicates.meta.notScanned': '尚未掃描',
  'settings.danger.duplicates.action.scan': '掃描',
  'settings.danger.duplicates.action.scanning': '掃描中...',
  'settings.danger.duplicates.action.clean': '清理',
  'settings.danger.duplicates.action.cleaning': '清理中...',
  'settings.danger.duplicates.progress.scan.title': '正在掃描重複歌曲',
  'settings.danger.duplicates.progress.scan.description': '分批處理，避免擠占播放',
  'settings.danger.duplicates.progress.scan.aria': '重複歌曲掃描中',
  'settings.danger.duplicates.progress.clean.title': '正在清理掃描結果',
  'settings.danger.duplicates.progress.clean.description': '移入資源回收筒並更新曲庫索引',
  'settings.danger.duplicates.progress.clean.aria': '重複歌曲清理中',
  'settings.danger.clearCache.title': '清空媒體庫快取',
  'common.unavailable': '暫不可用',
  'common.ready': '就緒',
  'common.checking': '檢查中',
  'common.search.clear': '清除搜尋',
  'common.yes': '是',
  'common.no': '否',
  // --- auto-filled missing keys (485) ---
  'accountProvider.bilibili': 'Bilibili',
  'accountProvider.osu': 'osu!',
  'accountProvider.qobuz': 'Qobuz',
  'accountProvider.qqmusic': 'QQ Music',
  'accountProvider.soundcloud': 'SoundCloud',
  'accountProvider.spotify': 'Spotify',
  'accountProvider.tidal': 'TIDAL',
  'accountProvider.youtube': 'YouTube',
  'albumDetail.credit.role.composer': '作曲',
  'app.navigation.main': '主導航',
  'app.navigation.utility': '工具導航',
  'app.toolbar.quickActions': '快捷操作',
  'app.toolbar.pluginCommands': '外掛命令（Ctrl+Shift+P）',
  'app.toolbar.windowControls': '視窗控制',
  'app.titlebar.settingsBack': '返回',
  'app.titlebar.settingsBackHint': '返回上一頁（Esc）',
  'app.window.close': '關閉',
  'app.window.maximize': '最大化',
  'app.window.minimize': '最小化',
  'app.window.hideToTray': '隱藏到系統匣',
  'common.available': '可用',
  'common.build': '構建版',
  'common.collapse': '摺疊',
  'common.dev': '開發版',
  'common.disabled': '未啟用',
  'common.enabled': '已啟用',
  'common.expand': '展開',
  'common.loading': '載入中',
  'common.na': '無',
  'downloads.error.osuOnlyUrl': 'osu! 只能下載 beatmapset 連結。',
  'downloads.folder.osuRequired': '請選擇 osu 下載資料夾',
  'downloads.settings.osuMirror.auto': '自動',
  'downloads.settings.osuMirror.autoDetail': '官方 -> Sayobot -> Catboy -> NeriNyan',
  'downloads.settings.osuMirror.catboy': 'Catboy / Mino',
  'downloads.settings.osuMirror.mirrorDetail': '映象站',
  'downloads.settings.osuMirror.nerinyan': 'NeriNyan',
  'downloads.settings.osuMirror.official': 'osu! official',
  'downloads.settings.osuMirror.officialDetail': '官方源',
  'downloads.settings.osuMirror.sayobot': 'Sayobot',
  'downloads.settings.osuMirror.title': 'osu 映象站',
  'downloads.settings.osuOutputDirectory': 'osu 下載資料夾',
  'notice.browserFilePicker': '瀏覽器預覽已選擇 {name}。請在 ECHO 桌面端通過 Audio Core 播放。',
  'notice.browserFolderPicker': '瀏覽器預覽已開啟資料夾選擇器。匯入曲庫請使用 ECHO 桌面應用。',
  'notice.windowControlsDesktop': '視窗控制只在 ECHO 桌面視窗中可用。',
  'queue.drawer.aria': '播放佇列抽屜',
  'queue.drawer.clear': '清空',
  'queue.drawer.close': '關閉播放佇列',
  'queue.drawer.fullQueue': '完整佇列',
  'queue.drawer.removeTitle': '移除',
  'queue.drawer.rowActions': '{title} 佇列操作',
  'queue.drawer.shuffleScope': '隨機範圍：{scope} · 避開最近 {count} 首',
  'queue.drawer.sourcePrefix': '來自 {source}',
  'queue.drawer.waitingCount': '{count} 首待播',
  'route.albums.description': '按專輯分組的封面牆。',
  'route.artists.description': '按藝術家瀏覽。',
  'route.genres.description': '按嵌入的類型標籤瀏覽。',
  'route.audioSettings.description': '輸出與解碼設定。',
  'route.folders.description': '本地匯入根目錄。',
  'route.history.description': '播放歷史。',
  'route.history.label': '歷史',
  'route.importFile.description': '匯入單個音訊檔案。',
  'route.liked.description': '收藏曲目。',
  'route.lyricsSettings.description': '歌詞偏好設定。',
  'route.playlists.description': '使用者歌單。',
  'route.queue.description': '播放佇列。',
  'route.remote.description': '遠端來源。',
  'route.settings.description': '應用設定。',
  'route.songs.description': '本地曲庫歌曲列表。',
  'settings.about.audioHost.description': '本機播放使用的音訊引擎程式。',
  'settings.about.audioHost.title': '音訊引擎',
  'settings.about.devMode.description': '目前是開發檢查模式，用於確認曲庫和音訊引擎工作正常。',
  'settings.about.devMode.title': '開發模式',
  'settings.about.nativeSqlite.description': '本地曲庫資料庫。開發時會自動對齊目前版本，避免掃描失敗。',
  'settings.about.nativeSqlite.title': '原生 SQLite',
  'settings.appearance.font.chinese.description': '當主字型缺少中文字元時，優先使用這個中文字型補齊。',
  'settings.appearance.font.chinese.title': '中文字型',
  'settings.appearance.font.choose': '選擇',
  'settings.appearance.font.fallback.description': '介面字型的第三組備用，優先順序最低，用於繼續補齊缺失字元。',
  'settings.appearance.font.fallback.title': '備用字型',
  'settings.appearance.font.main.description': 'ECHO 預設使用 Outfit、Microsoft YaHei 與 Noto Sans SC；也可以輸入任意已安裝字型名稱。',
  'settings.appearance.font.main.title': '主字型',
  'settings.appearance.fontSize.description': '調整全域性介面的基礎字號。',
  'settings.appearance.fontSize.title': '基礎字號',
  'settings.appearance.lineHeight.description': '調整介面文字的預設行距，讓列表和說明文本更疏朗或更緊湊。',
  'settings.appearance.lineHeight.title': '介面行距',
  'settings.appearance.reset.action': '恢復預設',
  'settings.appearance.reset.description': '恢復 Outfit、Microsoft YaHei、Noto Sans SC、字號、行距、文字深淺與圓角封面。',
  'settings.appearance.reset.title': '外觀預設值',
  'settings.appearance.textDepth.description': '調整介面文字顏色深淺；數值越低越淺。',
  'settings.appearance.textDepth.title': '文字顏色深淺',
  'settings.appearance.typography.collapse': '收起字型與排版',
  'settings.appearance.typography.expand': '展開字型與排版',
  'settings.danger.clearCache.description': '移除曲庫索引、掃描記錄和封面快取，不會刪除你的音樂檔案或曲庫資料夾。',
  'settings.eq.ab.emptySlot': '空槽',
  'settings.eq.ab.loudnessMatched': '響度匹配',
  'settings.eq.ab.title': 'A/B 對比',
  'settings.eq.action.applyA': '應用 A',
  'settings.eq.action.applyB': '應用 B',
  'settings.eq.action.applyProfile': '應用配置檔',
  'settings.eq.action.applySafePreamp': '應用安全前級',
  'settings.eq.action.bindProfile': '綁定當前輸出',
  'settings.eq.action.bypass': '旁路',
  'settings.eq.action.deleteProfile': '刪除配置檔',
  'settings.eq.action.duplicatePreset': '複製當前',
  'settings.eq.action.freqDown': '頻率 -',
  'settings.eq.action.freqFineDown': '細 -',
  'settings.eq.action.freqFineUp': '細 +',
  'settings.eq.action.freqUp': '頻率 +',
  'settings.eq.action.overwrite': '覆蓋當前',
  'settings.eq.action.redo': '重做',
  'settings.eq.action.resetAllGains': '重置全部增益',
  'settings.eq.action.resetCurve': '重置曲線',
  'settings.eq.action.resetBand': '重置 {frequency}',
  'settings.eq.action.resetEq': '重置 EQ',
  'settings.eq.action.resetFrequencies': '恢復標準頻點',
  'settings.eq.action.resetMonitorTools': '重置監聽工具',
  'settings.eq.action.resetSelected': '重置選中',
  'settings.eq.action.resetTrimsOnly': '只重置校正',
  'settings.eq.action.revertUserPreset': '還原使用者預設',
  'settings.eq.action.saveAs': '儲存到預設',
  'settings.eq.action.saveProfile': '儲存配置檔',
  'settings.eq.action.storeA': '存入 A',
  'settings.eq.action.storeB': '存入 B',
  'settings.eq.action.toggleBypassOff': '關閉旁路',
  'settings.eq.action.toggleBypassOn': '切換旁路',
  'settings.eq.action.undo': '撤銷',
  'settings.eq.action.unlockFrequency': '解鎖頻率',
  'settings.eq.band.bypassed': '旁路',
  'settings.eq.band.enabled': '啟用此段',
  'settings.eq.band.fallback': '頻段',
  'settings.eq.band.filterType': '型別',
  'settings.eq.band.frequency': '頻率',
  'settings.eq.band.frequencySnapped': '吸附到標準頻點',
  'settings.eq.band.frequencyStepper': '頻率步進',
  'settings.eq.band.frequencyUnlocked': '自由頻率',
  'settings.eq.band.gain': '增益',
  'settings.eq.band.gainStepper': '增益步進',
  'settings.eq.band.inspector': '選中頻段',
  'settings.eq.band.q': 'Q',
  'settings.eq.band.readoutsAria': '31 段 EQ 可拖動頻段讀數',
  'settings.eq.channel.balance': '平衡',
  'settings.eq.channel.bypassed': '旁路',
  'settings.eq.channel.calibrationMode': '校準模式',
  'settings.eq.channel.dsp': 'DSP',
  'settings.eq.channel.effectiveLeft': '左有效增益',
  'settings.eq.channel.effectiveRight': '右有效增益',
  'settings.eq.channel.group.balance': 'Balance',
  'settings.eq.channel.group.gainTrim': 'Gain Trim',
  'settings.eq.channel.group.monitorTools': '監聽工具',
  'settings.eq.channel.group.phaseTools': '相位工具',
  'settings.eq.channel.leftGain': '左增益',
  'settings.eq.channel.leftTotal': '左總增益',
  'settings.eq.channel.mono.left': '左',
  'settings.eq.channel.mono.right': '右',
  'settings.eq.channel.monoMode': '單聲道模式',
  'settings.eq.channel.quick.leftSolo': '左聲道 Solo',
  'settings.eq.channel.quick.monoCheck': '單聲道檢查',
  'settings.eq.channel.quick.phaseCheck': '相位檢查',
  'settings.eq.channel.quick.rightSolo': '右聲道 Solo',
  'settings.eq.channel.quick.swapCheck': '交換檢查',
  'settings.eq.channel.quickTools': '快速監聽工具',
  'settings.eq.channel.rightGain': '右增益',
  'settings.eq.channel.rightTotal': '右總增益',
  'settings.eq.channel.swap': '交換 L/R',
  'settings.eq.curve.fineEdit': 'Shift 細調',
  'settings.eq.curve.freeFrequency': '自由頻率',
  'settings.eq.curve.lockedFrequency': '頻率已鎖定，只能上下調增益',
  'settings.eq.curve.snapped': '標準吸附',
  'settings.eq.error.profileName': '請輸入配置檔名稱。',
  'settings.eq.error.profileTarget': '請選擇一個配置檔。',
  'settings.eq.preamp.aria': 'EQ 前級增益',
  'settings.eq.preamp.maxBoost': '最大提升',
  'settings.eq.preamp.metricsAria': '安全餘量指標',
  'settings.eq.preamp.recommended': '建議',
  'settings.eq.preset.approximation': '31 段近似',
  'settings.eq.preset.builtIn': '內建預設',
  'settings.eq.preset.copyName': '{name} 副本',
  'settings.eq.preset.filter.all': '全部',
  'settings.eq.preset.filter.builtIn': '內建',
  'settings.eq.preset.filter.genre': '風格',
  'settings.eq.preset.filter.target': '目標曲線',
  'settings.eq.preset.filter.user': '使用者',
  'settings.eq.preset.filter.utility': '工具',
  'settings.eq.preset.filterAria': '預設篩選',
  'settings.eq.preset.meta.approximationCaution': '這是 31 段圖示 EQ 近似，不是精確耳機校準。',
  'settings.eq.preset.meta.genrePurpose': '用於快速塑造音樂風格取向。',
  'settings.eq.preset.meta.genreScenario': '適合按曲風試聽，再按裝置微調。',
  'settings.eq.preset.meta.targetPurpose': '用於接近常見聽感目標曲線。',
  'settings.eq.preset.meta.targetScenario': '適合耳機或近場系統的目標曲線對比。',
  'settings.eq.preset.meta.tasteCaution': '這是聽感取向，不是校準結果。',
  'settings.eq.preset.meta.type.acousticSilk': '原聲音色',
  'settings.eq.preset.meta.type.animeJpop': '動漫 / J-Pop',
  'settings.eq.preset.meta.type.bassBoost': '低音增強',
  'settings.eq.preset.meta.type.bkRoomCurve': '房間曲線',
  'settings.eq.preset.meta.type.bluetoothSpeakerCleanup': '藍牙清理',
  'settings.eq.preset.meta.type.broadcastVoice': '播音人聲',
  'settings.eq.preset.meta.type.cinemaOrchestra': '電影縱深',
  'settings.eq.preset.meta.type.cityPop': '城市流行',
  'settings.eq.preset.meta.type.classicSmiley': '輕 V 形',
  'settings.eq.preset.meta.type.classical': '古典廳堂',
  'settings.eq.preset.meta.type.diffuseField': '擴散場',
  'settings.eq.preset.meta.type.femaleVocalAir': '女聲空氣感',
  'settings.eq.preset.meta.type.flat': '平坦',
  'settings.eq.preset.meta.type.harmanInEar': 'Harman 入耳',
  'settings.eq.preset.meta.type.harmanTarget': 'Harman 頭戴',
  'settings.eq.preset.meta.type.headphoneNotch': '耳機峰值修剪',
  'settings.eq.preset.meta.type.headphoneWarm': '耳機暖聲',
  'settings.eq.preset.meta.type.liveHouse': 'Live House',
  'settings.eq.preset.meta.type.lofiDusk': 'Lo-Fi',
  'settings.eq.preset.meta.type.loudness': '小音量補償',
  'settings.eq.preset.meta.type.night': '夜間聆聽',
  'settings.eq.preset.meta.type.pianoRoom': '鋼琴',
  'settings.eq.preset.meta.type.rock': '搖滾',
  'settings.eq.preset.meta.type.sibilanceTamer': '齒音抑制',
  'settings.eq.preset.meta.type.studioNeutral': '錄音室中性',
  'settings.eq.preset.meta.type.subCleanup': '低頻清理',
  'settings.eq.preset.meta.type.subsonicFilter': '次聲波濾波',
  'settings.eq.preset.meta.type.trebleSparkle': '高頻閃亮',
  'settings.eq.preset.meta.type.vinylWarmth': '黑膠暖聲',
  'settings.eq.preset.meta.type.vocalClear': '人聲清晰',
  'settings.eq.preset.meta.type.vocalDeEss': '去齒音',
  'settings.eq.preset.meta.utilityCaution': '工具型預設會改變監聽判斷，請確認後再儲存。',
  'settings.eq.preset.meta.utilityPurpose': '用於檢查、補償或更安全的監聽。',
  'settings.eq.preset.meta.utilityScenario': '適合定位問題、降低疲勞或做快速對比。',
  'settings.eq.preset.modified': '已修改',
  'settings.eq.preset.name.acoustic-silk': '原聲音色',
  'settings.eq.preset.name.anime-jpop': '動漫 / J-Pop',
  'settings.eq.preset.name.bass-boost': '低音增強',
  'settings.eq.preset.name.bk-room-curve': '房間傾斜',
  'settings.eq.preset.name.broadcast-voice': '播音人聲',
  'settings.eq.preset.name.cinema-orchestra': '電影縱深',
  'settings.eq.preset.name.city-pop': '城市流行',
  'settings.eq.preset.name.classic-smiley': '輕 V 形',
  'settings.eq.preset.name.classical': '古典廳堂',
  'settings.eq.preset.name.diffuse-field': '擴散場 · 開闊',
  'settings.eq.preset.name.diffuse-field-5128': '擴散場 · 5128',
  'settings.eq.preset.name.diffuse-field-gras-kemar': '擴散場 · KEMAR',
  'settings.eq.preset.name.diffuse-field-iso-11904-1': '擴散場 · ISO',
  'settings.eq.preset.name.female-vocal-air': '女聲空氣感',
  'settings.eq.preset.name.flat': '平坦',
  'settings.eq.preset.name.harman-in-ear': 'Harman 入耳',
  'settings.eq.preset.name.harman-in-ear-2016': 'Harman 入耳 · 暖',
  'settings.eq.preset.name.harman-in-ear-2017': 'Harman 入耳 · 亮',
  'settings.eq.preset.name.harman-in-ear-2019-no-bass': 'Harman 入耳 · 瘦',
  'settings.eq.preset.name.harman-over-ear-2013': 'Harman 頭戴 · 輕',
  'settings.eq.preset.name.harman-over-ear-2015': 'Harman 頭戴 · 暖',
  'settings.eq.preset.name.harman-over-ear-2018-no-bass': 'Harman 頭戴 · 瘦',
  'settings.eq.preset.name.harman-speaker-room-2013': '房間偏好 · 暖傾',
  'settings.eq.preset.name.harman-target': 'Harman 平衡',
  'settings.eq.preset.name.headphone-warm': '耳機暖聲',
  'settings.eq.preset.name.live-house': 'Live House',
  'settings.eq.preset.name.lofi-dusk': 'Lo-Fi',
  'settings.eq.preset.name.loudness': '小音量補償',
  'settings.eq.preset.name.night': '夜間聆聽',
  'settings.eq.preset.name.piano-room': '鋼琴',
  'settings.eq.preset.name.rock': '搖滾',
  'settings.eq.preset.name.studio-neutral': '錄音室中性',
  'settings.eq.preset.name.treble-sparkle': '高頻閃亮',
  'settings.eq.preset.name.vinyl-warmth': '黑膠暖聲',
  'settings.eq.preset.name.vocal-clear': '人聲清晰',
  'settings.eq.preset.nameAria': '預設名稱',
  'settings.eq.preset.searchAria': '搜尋預設',
  'settings.eq.preset.searchPlaceholder': '搜尋預設',
  'settings.eq.preset.selectorAria': 'EQ 預設',
  'settings.eq.preset.user': '使用者預設',
  'settings.eq.profile.bound': '{output} 已繫結 {profile}',
  'settings.eq.profile.empty': '未選擇配置檔',
  'settings.eq.profile.nameAria': 'EQ 配置檔名稱',
  'settings.eq.profile.namePlaceholder': '儲存為配置檔',
  'settings.eq.profile.noOutput': '當前輸出',
  'settings.eq.profile.selectorAria': 'EQ 配置檔',
  'settings.eq.profile.title': '配置檔',
  'settings.eq.profile.unbound': '{output} 未繫結配置檔',
  'settings.eq.state.eqDisabled': 'EQ 已關閉',
  'settings.eq.state.eqEnabled': 'EQ 已啟用',
  'settings.eq.status.bitPerfect': 'Bit-perfect',
  'settings.eq.status.eq': 'EQ',
  'settings.eq.status.estimatedPeak': '估算峰值',
  'settings.eq.status.preamp': '前級',
  'settings.eq.status.preset': '預設',
  'settings.eq.status.safe': '安全',
  'settings.eq.status.warning': '警告',
  'settings.general.backup.description': '匯出或匯入 ECHO 設定引數，用於遷移到新裝置或恢復配置。',
  'settings.general.echoProAccount.action.check': '檢查',
  'settings.general.echoProAccount.action.checking': '檢查中',
  'settings.general.echoProAccount.action.loggingIn': '登入中',
  'settings.general.echoProAccount.action.loggingOut': '退出中',
  'settings.general.echoProAccount.action.login': '登入',
  'settings.general.echoProAccount.action.logout': '退出',
  'settings.general.echoProAccount.action.redeemKey': '兌換 Key',
  'settings.general.echoProAccount.action.redeeming': '兌換中',
  'settings.general.echoProAccount.action.refreshSyncDate': '重新整理同步日期',
  'settings.general.echoProAccount.action.refreshing': '重新整理中',
  'settings.general.echoProAccount.action.register': '註冊',
  'settings.general.echoProAccount.action.registering': '註冊中',
  'settings.general.echoProAccount.action.releaseDevices': '解綁所有裝置',
  'settings.general.echoProAccount.action.releasingDevices': '解綁中',
  'settings.general.echoProAccount.action.saveCloud': '儲存設定到雲端',
  'settings.general.echoProAccount.action.saving': '儲存中',
  'settings.general.echoProAccount.action.syncCloud': '從雲端同步',
  'settings.general.echoProAccount.action.syncing': '同步中',
  'settings.general.echoProAccount.capsLock': '大寫鎖定已開啟',
  'settings.general.echoProAccount.cloudEmpty': '暫無雲端備份',
  'settings.general.echoProAccount.cloudLibrary': '雲端歌單: {playlists} 個網路歌單 / {favorites} 首流媒體收藏',
  'settings.general.echoProAccount.cloudSaved': '已儲存',
  'settings.general.echoProAccount.cloudSettings': '雲端設定: {status}',
  'settings.general.echoProAccount.cloudSyncNote': '雲端同步會儲存設定、網路歌單和流媒體收藏；不會儲存網易雲 / QQ 音樂 / Spotify 等平臺登入態。另一臺裝置同步後如不能播放，請先在賬號設定裡登入對應平臺。',
  'settings.general.echoProAccount.collapseAria': '摺疊 ECHO Pro 賬號',
  'settings.general.echoProAccount.copyHwid': '複製 HWID',
  'settings.general.echoProAccount.description': '使用 QQ 號註冊/登入後聯網檢查 Pro 資格。',
  'settings.general.echoProAccount.developmentNote': '開發提示: ECHO Pro 賬號系統仍在完善中，當前正式授權以官網生成的繫結外掛為準。',
  'settings.general.echoProAccount.deviceLine': '裝置: {count}/{max}',
  'settings.general.echoProAccount.expandAria': '展開 ECHO Pro 賬號',
  'settings.general.echoProAccount.hwidCopied': 'HWID 已複製',
  'settings.general.echoProAccount.hwidNote': 'HWID 是穩定雜湊，用於生成綁定當前裝置的 ECHO Pro 外掛。',
  'settings.general.echoProAccount.lastChecked': '上次檢查 {time}',
  'settings.general.echoProAccount.localHwid': '本機 HWID: {hwid}',
  'settings.general.echoProAccount.passwordHide': '隱藏密碼',
  'settings.general.echoProAccount.passwordPlaceholder': '密碼',
  'settings.general.echoProAccount.passwordShow': '顯示密碼',
  'settings.general.echoProAccount.registrationTip': '註冊提示: 請優先使用 QQ 號作為賬號，方便人工核對授權。',
  'settings.general.echoProAccount.showHwid': '顯示 HWID',
  'settings.general.echoProAccount.sourceVersion': '來源版本: {version}',
  'settings.general.echoProAccount.status.loggedIn': '已登入',
  'settings.general.echoProAccount.status.loggedOut': '未登入',
  'settings.general.echoProAccount.status.pluginUnlocked': '已使用外掛解鎖',
  'settings.general.echoProAccount.status.proEnabled': 'Pro 已啟用',
  'settings.general.echoProAccount.status.proUnauthorized': 'Pro 未授權',
  'settings.general.echoProAccount.status.proValid': 'Pro 有效',
  'settings.general.echoProAccount.statusLine': '狀態: {login} / {pro}',
  'settings.general.echoProAccount.syncDate': '同步日期: {date}',
  'settings.general.echoProAccount.syncStatus': '同步狀態: {status}',
  'settings.general.echoProAccount.title': 'ECHO Pro 賬號',
  'settings.general.echoProAccount.unavailable': '這功能不可用',
  'settings.general.echoProAccount.usernamePlaceholder': 'QQ號（作為賬號）',
  'settings.general.echoProAccount.verificationNote': '用雲端帳號和本機裝置識別確認授權。僅本地解鎖時介面會顯示 Pro，下載功能仍需對應授權。',
  'settings.integrations.accounts.status.checking': '檢查中',
  'settings.integrations.accounts.status.expired': '登入失效',
  'settings.integrations.accounts.status.loggedIn': '已登入',
  'settings.integrations.accounts.status.loggedOut': '未登入',
  'settings.integrations.discord.description': '將當前播放狀態同步到 Discord Rich Presence，可顯示歌曲、藝術家、進度和播放狀態。',
  'settings.library.network.description': '手動弱補全；本地內嵌後設資料始終優先。',
  'settings.library.network.title': '網路後設資料補全',
  'settings.library.networkPanel.appliedCount': '已自動補全數量',
  'settings.library.networkPanel.applyMissingOnly': '僅補缺失項',
  'settings.library.networkPanel.applySelected': '應用所選候選',
  'settings.library.networkPanel.artistField': '歌手',
  'settings.library.networkPanel.artistSource': '歌手來源',
  'settings.library.networkPanel.candidates': '候選',
  'settings.library.networkPanel.cover': '封面',
  'settings.library.networkPanel.embeddedCover': '內嵌封面',
  'settings.library.networkPanel.embeddedMetadata': '內嵌後設資料',
  'settings.library.networkPanel.kicker': '手動修復',
  'settings.library.networkPanel.localCover': '本地',
  'settings.library.networkPanel.missingCover': '缺失/預設',
  'settings.library.networkPanel.noCandidates': '暫無網路候選。',
  'settings.library.networkPanel.providerErrors': '來源錯誤',
  'settings.library.networkPanel.reject': '拒絕',
  'settings.library.networkPanel.repairMissing': '補全當前歌曲',
  'settings.library.networkPanel.repairThisTrack': '補全此曲',
  'settings.library.networkPanel.scanComplete': '掃描完成',
  'settings.library.networkPanel.scanDone': '已掃描缺失曲目',
  'settings.library.networkPanel.scanMissing': '掃描缺失資訊',
  'settings.library.networkPanel.scanPreparing': '準備掃描',
  'settings.library.networkPanel.scanProgress': '缺失後設資料掃描進度',
  'settings.library.networkPanel.scanRunning': '正在掃描網路來源',
  'settings.library.networkPanel.showCandidates': '顯示候選',
  'settings.library.networkPanel.title': '缺失後設資料修復',
  'settings.library.networkPanel.titleField': '標題',
  'settings.library.networkPanel.trackId': '曲目 ID',
  'settings.library.networkPanel.trackNotFound': '找不到該曲目。請先播放一首歌，或輸入曲目 ID。',
  'settings.library.networkPanel.unknownArtist': '未知歌手',
  'settings.library.networkPanel.untitled': '未命名',
  'settings.library.networkSources.description': '選擇手動修復和缺失掃描使用的補全源。',
  'settings.library.networkSources.title': '網路補全來源',
  'settings.nav.about.description': '版本、更新與社群資訊',
  'settings.nav.appearance.description': '主題、字型、背景',
  'settings.nav.danger.description': '復原、清理與本機資料重設',
  'settings.nav.eq.description': '均衡器與輸出安全',
  'settings.nav.eq.label': 'EQ',
  'settings.nav.library.description': '匯入、掃描與清理',
  'settings.nav.lyrics.description': '歌詞顯示與偏好',
  'settings.nav.remote.description': 'NAS、WebDAV、Subsonic',
  'settings.playback.outputDevice.description': '選擇播放裝置；留空時使用系統預設輸出。',
  'settings.playback.wireless.description': '目前版本暫未接入。無縫播放、智慧過渡和串流媒體不會走這條路徑。',
  'settings.remote.action.chooseRecommended': '選擇推薦方式',
  'settings.remote.action.deleteSource': '刪除來源',
  'settings.remote.action.openCurrentSource': '開啟當前來源',
  'settings.remote.action.startConnection': '開始連線',
  'settings.remote.background.concurrency.cover.aria': '後臺封面併發',
  'settings.remote.background.concurrency.decrease': '併發減一',
  'settings.remote.background.concurrency.durationBackfill.aria': '後臺時長回填併發',
  'settings.remote.background.concurrency.increase': '併發加一',
  'settings.remote.background.concurrency.lyrics.aria': '後臺歌詞併發',
  'settings.remote.background.concurrency.metadata.aria': '後臺後設資料併發',
  'settings.remote.background.concurrency.value': '併發',
  'settings.remote.badge.recommended': '推薦',
  'settings.remote.browser.currentDirectory': '當前目錄',
  'settings.remote.browser.refreshDirectory': '重新整理目錄',
  'settings.remote.browser.summary': '瀏覽摘要',
  'settings.remote.browser.syncIndex': '同步索引',
  'settings.remote.browser.up': '上級',
  'settings.remote.comingSoon.description': '這類來源需要處理系統掛載、憑據和跨平臺檔案訪問，會在媒體伺服器類來源穩定後接入。',
  'settings.remote.comingSoon.title': '{provider} 即將支援',
  'settings.remote.commandStatus.idle': '空閒待命',
  'settings.remote.commandStatus.paused': '後臺已暫停',
  'settings.remote.commandStatus.playbackReduced': '播放中減輕占用',
  'settings.remote.commandStatus.processing': '正在處理',
  'settings.remote.empty.aria': '空狀態',
  'settings.remote.empty.title': '還沒有 {provider} 來源',
  'settings.remote.job.cover': '封面',
  'settings.remote.job.durationBackfill': '時長回填',
  'settings.remote.job.lyrics': '歌詞',
  'settings.remote.job.metadata': '後設資料',
  'settings.remote.message.providerSelected': '已選擇 {provider}。填好連線資訊後，可以先測試，再儲存或同步；切到別的來源再回來也會保留。',
  'settings.remote.metric.abnormal': '異常',
  'settings.remote.metric.backgroundStatus': '後臺狀態',
  'settings.remote.metric.coverCompletion': '封面完成度',
  'settings.remote.metric.enabled': '啟用',
  'settings.remote.metric.error': '錯誤',
  'settings.remote.metric.failed': '失敗',
  'settings.remote.metric.indexed': '已索引',
  'settings.remote.metric.indexedTracks': '已索引歌曲',
  'settings.remote.metric.issues': '問題',
  'settings.remote.metric.knownSize': '已知容量',
  'settings.remote.metric.metadataCompletion': '後設資料完成度',
  'settings.remote.metric.missing': '缺失',
  'settings.remote.metric.queue': '佇列',
  'settings.remote.metric.size': '容量',
  'settings.remote.metric.sources': '來源',
  'settings.remote.metric.sync': '同步',
  'settings.remote.overview.aria': '網盤中心總覽',
  'settings.remote.overview.backgroundFallback': '按來源任務佇列執行',
  'settings.remote.overview.description': '集中檢視網盤索引、同步佇列、異常專案和後臺負載狀態。',
  'settings.remote.overview.title': '遠端庫控制台',
  'settings.remote.provider.emby.action': '連線 Emby',
  'settings.remote.provider.emby.emptyText': '填入 Emby 伺服器地址和憑據後，ECHO 會把可播放曲目整理到遠端來源列表。',
  'settings.remote.provider.emby.fit': '媒體伺服器',
  'settings.remote.provider.emby.intent': '我在用 Emby',
  'settings.remote.provider.emby.label': 'Emby',
  'settings.remote.provider.emby.promise': '沿用 Emby 曲庫，ECHO 負責播放體驗',
  'settings.remote.provider.emby.summary': '適合已有 Emby 媒體庫，希望 ECHO 只負責播放和本地索引。',
  'settings.remote.provider.emby.tone': '伺服器索引',
  'settings.remote.provider.jellyfin.action': '連線 Jellyfin',
  'settings.remote.provider.jellyfin.emptyText': '填入 Jellyfin 伺服器地址和賬號後，ECHO 會讀取伺服器已有音樂庫。',
  'settings.remote.provider.jellyfin.fit': '媒體伺服器',
  'settings.remote.provider.jellyfin.intent': '我在用 Jellyfin',
  'settings.remote.provider.jellyfin.label': 'Jellyfin',
  'settings.remote.provider.jellyfin.promise': '讀取已有媒體庫，不重做管理',
  'settings.remote.provider.jellyfin.summary': '適合已經用 Jellyfin 管理音樂和媒體庫的家庭伺服器。',
  'settings.remote.provider.jellyfin.tone': '伺服器索引',
  'settings.remote.provider.smb.action': '連線 NAS 共享',
  'settings.remote.provider.smb.emptyText': '先確認系統能訪問 NAS 共享路徑，再填寫連線資訊；這裡會顯示同步和可播放狀態。',
  'settings.remote.provider.smb.fit': '區域網 NAS',
  'settings.remote.provider.smb.intent': '家裡的 NAS 共享',
  'settings.remote.provider.smb.label': 'NAS / SMB',
  'settings.remote.provider.smb.promise': '適合 Windows 共享、群暉、區域網目錄',
  'settings.remote.provider.smb.summary': '適合區域網 NAS 共享，依賴系統可訪問的 SMB/UNC 路徑。',
  'settings.remote.provider.smb.tone': '系統掛載',
  'settings.remote.provider.sshfs.action': '連線掛載目錄',
  'settings.remote.provider.sshfs.emptyText': '先在系統裡完成掛載，再把可訪問路徑交給 ECHO 索引。',
  'settings.remote.provider.sshfs.fit': '掛載目錄',
  'settings.remote.provider.sshfs.intent': '我已經掛載了遠端目錄',
  'settings.remote.provider.sshfs.label': 'SSHFS',
  'settings.remote.provider.sshfs.promise': '適合已經配置好的 SSHFS / 掛載盤',
  'settings.remote.provider.sshfs.summary': '適合已經把遠端目錄掛載到系統的 SSHFS 音樂庫。',
  'settings.remote.provider.sshfs.tone': '高階入口',
  'settings.remote.provider.subsonic.action': '連線 Navidrome',
  'settings.remote.provider.subsonic.emptyText': '填入 Navidrome 或 Subsonic 伺服器地址後，ECHO 會按需索引和取流，不接管你的音樂檔案。',
  'settings.remote.provider.subsonic.fit': '首選推薦',
  'settings.remote.provider.subsonic.intent': '我有音樂伺服器',
  'settings.remote.provider.subsonic.label': 'Subsonic / Navidrome',
  'settings.remote.provider.subsonic.promise': '最穩的遠端音樂庫方式，推薦 Navidrome',
  'settings.remote.provider.subsonic.summary': '最推薦的遠端音樂庫方式。Navidrome 輕量穩定，ECHO 只索引和按需取流。',
  'settings.remote.provider.subsonic.tone': '推薦',
  'settings.remote.provider.webdav.action': '連線網盤目錄',
  'settings.remote.provider.webdav.emptyText': '左側填入網盤地址和賬號後，儲存並同步；這裡會顯示歌曲、索引進度和問題提醒。',
  'settings.remote.provider.webdav.fit': '相容優先',
  'settings.remote.provider.webdav.intent': '我有一個網盤目錄',
  'settings.remote.provider.webdav.label': '網盤 / WebDAV',
  'settings.remote.provider.webdav.promise': '適合 AList、堅果雲、群暉 WebDAV',
  'settings.remote.provider.webdav.summary': '通用網盤入口，適合 AList、堅果雲、群暉 WebDAV 和已有目錄服務。',
  'settings.remote.provider.webdav.tone': '保留相容',
  'settings.remote.providerSummary.aria': '來源摘要',
  'settings.remote.providerWorkspace.aria': '遠端來源工作區',
  'settings.remote.providerWorkspace.choiceAria': '遠端音樂庫型別',
  'settings.remote.providerWorkspace.currentEntry': '當前入口',
  'settings.remote.providerWorkspace.description': '先選最像你的情況，再填寫對應連線資訊。',
  'settings.remote.providerWorkspace.eyebrow': '從這裡開始',
  'settings.remote.providerWorkspace.title': '你的音樂在哪裡？',
  'settings.remote.sourceList.aria': '來源列表',
  'settings.remote.start.aria': '遠端來源起步',
  'settings.remote.start.description': '推薦用 Navidrome / Subsonic 做長期遠端音樂庫；已經有 AList、WebDAV 或 NAS，也可以直接接入。',
  'settings.remote.start.emptyEyebrow': '還沒有來源',
  'settings.remote.start.option.cloudDrive': '已有網盤目錄',
  'settings.remote.start.option.nas': '家裡 NAS',
  'settings.remote.start.option.server': '推薦：音樂伺服器',
  'settings.remote.start.optionsAria': '推薦入口',
  'settings.remote.start.title': '先選一個最像你音樂所在的位置',
  'settings.remote.status.disconnected': '未連線',
  'settings.remote.syncMode.browse.label': '僅瀏覽，不寫入曲庫索引',
  'settings.remote.syncMode.browse.option': '僅瀏覽',
  'settings.remote.syncMode.browse.short': '僅瀏覽',
  'settings.remote.syncMode.index.label': '建立索引，播放時按需取流',
  'settings.remote.syncMode.index.option': '建立索引，推薦',
  'settings.remote.syncMode.indexable.short': '可同步索引',
  'settings.remote.syncMode.label': '同步模式',
  'settings.remote.syncMode.mirror.label': '映象快取尚未開放，不會靜默複製整庫音訊',
  'settings.remote.syncMode.mirror.option': '映象快取，未來支援',
  'settings.remote.unit.albums': '張專輯',
  'settings.remote.unit.artists': '位藝人',
  'settings.remote.unit.songs': '首歌曲',
  'settings.remote.unit.sources': '個來源',
  'settings.remote.unit.tracks': '首',
  'sidebar.group.library': '資料庫',
  'sidebar.group.playback': '播放與創作',
  'sidebar.group.preferences': '偏好設定',
  'sidebar.group.sources': '來源',
  // --- user-facing error messages ---
  'userFacingError.fallback.audio': '播放沒有成功。ECHO 已保留詳細診斷；可以先重試播放，或在「設定 > 播放」裡暫時切換到相容輸出。',
  'userFacingError.fallback.downloads': '下載操作沒有成功。請檢查連結、網路和下載目錄後再試。',
  'userFacingError.fallback.folders': '資料夾操作沒有成功。請檢查路徑是否存在，以及 ECHO 是否有存取權限。',
  'userFacingError.fallback.generic': '操作沒有成功。ECHO 已保留詳細資訊；可以稍後重試，或開啟診斷報告排查。',
  'userFacingError.fallback.library': '曲庫操作沒有成功。請稍後重試；如果反覆出現，可以到設定裡執行曲庫診斷。',
  'userFacingError.fallback.mv': 'MV 操作沒有成功。請檢查網路或帳號狀態後再試。',
  'userFacingError.fallback.plugins': '外掛操作沒有成功。請檢查外掛狀態、權限或日誌後再試。',
  'userFacingError.fallback.settings': '設定沒有儲存成功。請稍後重試；如果反覆出現，可以重啟 ECHO 後再試。',
  'userFacingError.fallback.streaming': '串流服務暫時不可用。請檢查網路、帳號登入狀態或稍後再試。',
  'userFacingError.entitlement': '需要授權。請登入或啟用 ECHO Pro 後再試。',
  'userFacingError.desktopBridge': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'userFacingError.databaseCorrupt': '曲庫資料庫可能已損壞。請到「設定 > 診斷」執行資料庫修復，ECHO 會盡量保留你的音樂檔案和設定。',
  'userFacingError.fileNotFound': '找不到對應的檔案或資料夾。請確認路徑還存在，再重新選擇一次。',
  'userFacingError.notDirectory': '選擇的路徑不是資料夾。請重新選擇一個有效資料夾。',
  'userFacingError.permission': 'ECHO 沒有權限存取這個位置。請換一個目錄，或檢查系統權限後再試。',
  'userFacingError.diskFull': '磁碟空間不足。請清理一些空間，或換一個下載/快取目錄後再試。',
  'userFacingError.network.downloads': '下載服務連線失敗。請檢查網路、代理或連結是否可存取，然後再試。',
  'userFacingError.network.generic': '網路連線暫時失敗。請檢查網路、代理或帳號狀態後再試。',
  'userFacingError.auth': '帳號授權已失效或權限不足。請重新登入相關服務，或重新整理 Cookie 後再試。',
  // --- i18n batch2 player/queue/crash ---
  'playerBar.download.status.queued': '排隊中',
  'playerBar.download.status.probing': '解析連結',
  'playerBar.download.status.downloading': '下載中',
  'playerBar.download.status.extracting_audio': '提取音訊',
  'playerBar.download.status.importing': '匯入曲庫',
  'playerBar.download.status.binding_mv': '繫結 MV',
  'playerBar.download.status.completed': '下載完成',
  'playerBar.download.status.failed': '下載失敗',
  'playerBar.download.status.cancelled': '已取消',
  'playerBar.download.currentStreaming': '當前流媒體',
  'playerBar.download.savedToFolder': '已儲存到下載資料夾',
  'playerBar.download.retryLater': '請稍後重試',
  'playerBar.download.taskStopped': '任務已停止',
  'playerBar.download.notice.completed': '下載完成：{title}',
  'playerBar.download.notice.failed': '下載失敗：{title}',
  'playerBar.download.notice.cancelled': '下載已取消：{title}',
  'playerBar.download.notice.progress': '正在下載：{title}',
  'playerBar.download.notice.preparing': '準備下載：{title}',
  'playerBar.download.notice.resolving': '正在解析流媒體地址...',
  'playerBar.download.unsupportedPlatform': '當前平臺不支援下載',
  'playerBar.download.spotifyDetail': 'Spotify 由官方播放器播放，不提供可下載音訊 URL。',
  'playerBar.download.mockDetail': 'Mock 流媒體用於開發預覽，不寫入下載任務。',
  'playerBar.download.serviceUnavailable': '下載服務不可用',
  'playerBar.download.serviceDetail': '請在 ECHO 桌面端中使用下載功能。',
  'playerBar.download.ariaProgress': '流媒體下載進度',
  'playerBar.download.ariaDownload': '下載當前流媒體',
  'playerBar.download.title.preparing': '正在準備或下載',
  'playerBar.download.title.download': '下載當前流媒體',
  'playerBar.download.title.spotifyUnsupported': 'Spotify 不支援下載',
  'playerBar.download.title.sourceUnsupported': '當前流媒體源不支援下載',
  'playerBar.loading.streaming': '正在載入流媒體',
  'playerBar.loading.remote': '正在載入網盤音訊',
  'playerBar.loading.preparing': '正在準備音訊',
  'playerBar.loading.short': '載入中',
  'playerBar.export.noLocalFile': '沒有可匯出的本地檔案',
  'playerBar.export.sourceUnsupported': '當前來源不支援檔案匯出',
  'playerBar.export.exporting': '正在匯出當前檔案',
  'playerBar.export.exportAs': '匯出當前檔案為 {format}（{rate}）',
  'playerBar.export.cannotExport': '無法匯出當前檔案',
  'playerBar.export.notLocal': '當前來源不是本地音訊檔案。',
  'playerBar.export.noPlayingLocal': '還沒有正在播放的本地音訊檔案。',
  'playerBar.export.serviceUnavailable': '匯出服務不可用',
  'playerBar.export.serviceDetail': '請在 ECHO 桌面端中匯出音訊檔案。',
  'playerBar.export.preparing': '準備匯出：{title}',
  'playerBar.aria.playbackControls': '播放控制',
  'playerBar.aria.openLyrics': '開啟歌詞',
  'playerBar.aria.hideDesktopLyrics': '隱藏桌面歌詞',
  'playerBar.aria.showDesktopLyrics': '顯示桌面歌詞',
  'playerBar.title.hideDesktopLyrics': '隱藏桌面歌詞，右鍵自動解鎖並常駐或隱藏設定欄',
  'playerBar.title.showDesktopLyrics': '顯示桌面歌詞，右鍵自動解鎖並常駐或隱藏設定欄',
  'playerBar.aria.hideMiniPlayer': '隱藏迷你播放器',
  'playerBar.aria.showMiniPlayer': '顯示迷你播放器',
  'playerBar.openArtistDetail': '開啟藝人詳情：{name}',
  'queue.source.playlist.current': '當前播放列表：{label}',
  'queue.source.playlist.default': '播放列表',
  'queue.source.manual': '手動佇列',
  'queue.source.songs.random': '全曲庫隨機',
  'queue.source.songs.list': '歌曲列表：{label}',
  'queue.source.folder.random': '當前資料夾隨機：{label}',
  'queue.source.folder.list': '資料夾：{label}',
  'queue.source.liked': '我喜歡：{label}',
  'queue.source.album': '專輯：{label}',
  'queue.source.artist': '藝術家：{label}',
  'queue.source.streaming': '線上來源：{label}',
  'queue.source.localFile': '本地檔案：{label}',
  'queue.feedback.addedToTail': '已加入隊尾',
  'queue.feedback.addedCount': '已加入 {count} 首',
  'queue.feedback.addedNext': '已加入下一首',
  'queue.feedback.addedNextCount': '已加入下一首播放：{count} 首',
  'queue.feedback.trackDetail': '{title} · {source}',
  'queue.feedback.sourceTail': '{source} · 隊尾',
  'queue.feedback.shuffleOn': '隨機播放已開啟：{scope}',
  'queue.feedback.avoidRecent': '避開最近 {count} 首',
  'queue.feedback.fillEnough': '佇列已足夠',
  'queue.feedback.fillEnoughDetail': '當前已有 {count} 首',
  'queue.feedback.fillEmpty': '沒有可補全的歌曲',
  'queue.feedback.fillEmptyDetail': '已避開最近 {count} 首和當前佇列',
  'queue.feedback.filledCount': '已補全 {count} 首',
  'queue.feedback.noHqPlayerTrack': '沒有可交給 HQPlayer 的當前歌曲。',
  'queue.page.notice.removedPlayed': '已移除播放完成的佇列項',
  'queue.page.error.emptySave': '當前佇列為空，暫時沒有可儲存的內容。',
  'queue.page.notice.savedQueue': '已儲存佇列',
  'queue.page.error.emptySnapshot': '這個佇列快照沒有可恢復的歌曲。',
  'queue.page.notice.restoredQueue': '已恢復佇列',
  'queue.page.notice.deletedSnapshot': '已刪除佇列快照',
  'queue.page.notice.undone': '已撤銷',
  'queue.page.notice.canUndo': '可撤銷',
  'queue.page.notice.removedFromQueue': '已從佇列移除',
  'queue.page.notice.movedAfterCurrent': '已經排到當前播放後面',
  'queue.page.notice.unmarkedAfterPlay': '已取消播放後移除',
  'queue.page.notice.markedAfterPlay': '已標記 {count} 首播放後移除',
  'queue.page.error.bridgePlaylist': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'queue.page.error.noLibraryTracks': '當前佇列沒有可儲存到本地歌單的已入庫歌曲。',
  'queue.page.playlist.description': '從播放佇列儲存。',
  'queue.page.error.noneWritten': '沒有歌曲被寫入歌單。',
  'queue.page.notice.savedPlaylist': '已儲存為歌單',
  'queue.page.notice.savedPlaylistDetail': '{name}（{count} 首）',
  'queue.page.error.streamingPlaylist': '流媒體歌曲不能加入本地歌單，請在流媒體歌單中單獨管理。',
  'queue.page.notice.playNext': '已插到下一首',
  'queue.page.selection.aria': '佇列批次操作',
  'queue.page.selection.selectList': '選擇列表',
  'queue.page.selection.deselectList': '取消選擇列表',
  'queue.page.selection.selectAll': '全選列表',
  'queue.page.selection.selectedCount': '已選擇 {count} 首',
  'queue.page.selection.noneSelected': '未選擇歌曲',
  'queue.page.selection.clearAfterPlay': '取消播完移除',
  'queue.page.selection.markAfterPlay': '播放後移除',
  'queue.page.selection.done': '完成',
  'queue.page.selection.select': '選擇',
  'queue.page.receipt.affectedTracks': '受影響歌曲',
  'queue.page.receipt.closeAria': '關閉佇列操作回執',
  'queue.page.receipt.close': '關閉',
  'queue.page.saved.aria': '已儲存佇列',
  'queue.page.saved.deleteAria': '刪除佇列快照 {name}',
  'queue.page.saved.deleteTitle': '刪除佇列快照',
  'queue.page.unit.tracks': '首',
  'crashGuard.step.export.title': '先匯出診斷包',
  'crashGuard.step.export.description': '留下日誌、視窗狀態和錯誤棧，後面排查最有用。',
  'crashGuard.step.report.title': '再開啟崩潰報告',
  'crashGuard.step.report.description': '給開發者或 AI 看，通常比反覆重啟更快。',
  'crashGuard.step.reload.title': '最後再重載或重啟',
  'crashGuard.step.reload.description': '如果只是臨時閃了一下，重載介面可能就能回來。',
  'crashGuard.action.exporting': '正在準備診斷包...',
  'crashGuard.action.exported': '診斷包已匯出: {path}',
  'crashGuard.action.exportCancelled': '已取消匯出。',
  'crashGuard.action.openingReport': '正在開啟崩潰報告...',
  'crashGuard.action.openedReport': '已開啟崩潰報告: {path}',
  'crashGuard.action.reportMissing': '未找到崩潰報告。',
  'crashGuard.action.restartRequested': '已請求重啟 ECHO。若再次回到這裡，請優先匯出診斷包。',
  'crashGuard.action.quitting': '正在關閉 ECHO...',
  'crashGuard.bridge.online': '診斷可用',
  'crashGuard.bridge.offline': '診斷暫不可用',
  'crashGuard.bridge.hintOnline': '可以匯出診斷包',
  'crashGuard.bridge.hintOffline': '先截圖，或手動重啟',
  'crashGuard.status.defaultOnline': '先匯出診斷包，再開啟報告。這些資訊不會自動上傳。',
  'crashGuard.status.defaultOffline': '診斷暫時不可用。先截圖留下這一頁，再手動重啟 ECHO。',
  'crashGuard.window.main': '主視窗',
  'crashGuard.window.miniPlayer': '迷你播放器',
  'crashGuard.window.desktopLyrics': '桌面歌詞',
  'crashGuard.window.pet': 'ECHO 寵物',
  'crashGuard.brandTitle': '觀察室',
  'crashGuard.rail.kicker': '我攔住了一次介面異常',
  'crashGuard.rail.title': '先別急著重啟，把現場交給我吧。',
  'crashGuard.meta.window': '視窗',
  'crashGuard.meta.diagnostics': '診斷',
  'crashGuard.meta.type': '類型',
  'crashGuard.meta.renderError': '介面渲染異常',
  'crashGuard.sectionLabel': '先別慌，我在',
  'crashGuard.title': '介面有點不舒服，我來看著。',
  'crashGuard.lead': '這多半只是目前視窗的介面渲染失敗，播放核心通常還好好的。先把線索留下來，再決定重載或重啟。',
  'crashGuard.action.export': '匯出診斷包',
  'crashGuard.action.exportTitle': '匯出目前診斷資訊和崩潰線索',
  'crashGuard.action.openReport': '開啟報告',
  'crashGuard.action.openReportTitle': '開啟最近一次崩潰報告',
  'crashGuard.action.reload': '重載介面',
  'crashGuard.action.reloadTitle': '只重新整理目前渲染視窗',
  'crashGuard.action.feedback': '回報問題',
  'crashGuard.action.feedbackTitle': '前往 GitHub 回報問題',
  'crashGuard.action.restart': '重啟 ECHO',
  'crashGuard.action.restartTitle': '重新啟動 ECHO',
  'crashGuard.action.quit': '關閉 ECHO',
  'crashGuard.action.quitTitle': '退出 ECHO',
  'crashGuard.summary': '錯誤摘要',
  'crashGuard.action.copy': '複製摘要',
  'crashGuard.action.copyTitle': '複製錯誤資訊和堆疊，不包含本機路徑',
  'crashGuard.action.copied': '已複製崩潰摘要。',
  'crashGuard.action.copyFailed': '複製失敗，請展開摘要後手動選取。',
  'crashGuard.action.confirmRestart': '再點一次確認重啟',
  'crashGuard.action.confirmQuit': '再點一次確認關閉',
  'crashGuard.group.preserve': '先留線索',
  'crashGuard.group.recover': '再恢復',
  'crashGuard.shortcut.hint': '快捷鍵：E 匯出，R 開啟報告，C 複製摘要，L 重載介面。關閉和重啟需要點兩次。',
  'crashGuard.key.export': '匯出',
  'crashGuard.key.report': '報告',
  'crashGuard.key.copy': '複製',
  'crashGuard.key.reload': '重載',
  'crashGuard.key.confirmNote': '關閉和重啟請點兩次',
  // --- queue page extra ---
  'queue.page.notice.hiddenMore': '還有 {count} {unit}',
  'queue.page.notice.clearedCount': '已清空 {count} 首',
  'queue.page.notice.removedCount': '已移除 {count} 首',
  'queue.page.notice.movedCount': '已臨時插播 {count} 首',
  'queue.page.action.moveAfter': '臨時插播',
  'queue.page.action.removeSelected': '移除所選',
  'queue.page.action.clearSelection': '清除選擇',
  'queue.page.action.undo': '撤銷',
  'queue.page.receipt.justDone': '剛剛完成',
  'queue.page.receipt.undoThis': '撤銷這次操作',
  'queue.page.saved.heading': '已儲存佇列',
  'queue.page.saved.snapshotCount': '{count} 個快照',
  'queue.page.saved.trackMeta': '{count} 首 / {date}',
  'queue.page.saved.restore': '恢復',
  'queue.page.saved.sourceLabel': '儲存佇列：{name}',
  'queue.page.saved.nameWithTitle': '{title} 等 {count} 首',
  'queue.page.saved.nameQueue': '佇列 {date}',
  'queue.page.columns.titleArtist': '標題 / 藝術家',
  'queue.page.columns.source': '來源',
  'queue.page.columns.quality': '音質',
  'queue.page.columns.duration': '時長',
  'queue.page.undo.clear': '清空 {count} 首',
  'queue.page.undo.removeTitle': '移除 {title}',
  'queue.page.undo.removeCount': '移除 {count} 首',
  'queue.page.undo.moveCount': '臨時插播 {count} 首',
  'queue.page.undo.playNext': '插播 {title}',
  // --- error / audio error i18n ---
  'audioError.generic': '播放沒有成功。ECHO 已保留詳細診斷；你可以先重試播放，或在“設定 > 播放”裡臨時切換到相容輸出。',
  'audioError.corruptFile': '這首音訊檔案可能已經損壞或不完整，ECHO 已停止播放它。建議重新獲取這份檔案後再匯入。',
  'audioError.decodeFailed': '這首歌暫時解碼失敗。可以先重試播放；如果只在這首歌上穩定復現，再檢查檔案完整性或重新匯入。',
  'audioError.seekUnsupported': '當前位置暫時跳不過去，可能是檔案或網路來源不支援拖動。',
  'audioError.systemPlaybackFailed': '系統播放器沒有成功播放這首歌。可以重試一次，或切換到相容輸出後再播放。',
  'audioError.exclusiveFailed': 'WASAPI 獨佔輸出沒有成功。可能是裝置不支援當前格式，或正被其他應用佔用；可以先切回 WASAPI Shared 再播放。',
  'audioError.deviceNotFound': '所選音訊裝置不存在或已中斷連線。ECHO 已嘗試回退到系統預設輸出。',
  'audioError.deviceInitFailed': '音訊裝置初始化失敗。請確認輸出裝置仍線上；如果剛插拔過 DAC，可以先重啟音訊引擎或降低取樣率/緩衝設定。',
  'audioError.deviceTimeout': '音訊裝置響應太慢，可能是 USB DAC 或驅動暫時卡住。建議重新插拔 USB，或在設定裡重啟音訊引擎。',
  'audioError.hostReadyTimeout': '音訊輸出啟動超時。通常是驅動初始化太慢、裝置被佔用，或當前取樣率/緩衝設定被裝置拒絕。',
  'audioError.hostSpawnFailed': '音訊引擎無法啟動。請確認音訊引擎程式還在，且沒有被安全軟體攔截。',
  'audioError.hostInvalidExe': '音訊引擎無法啟動：程式檔案不是有效的 Windows 執行檔。建議重新安裝或重新打包，避免 echo-audio-host.exe / ffmpeg.exe 被損壞或替換。',
  'audioError.hostAccessViolation': '音訊引擎在啟動 Windows 輸出時崩潰。可以先重啟音訊引擎或 Windows 音訊服務；仍復現時，臨時切到 DirectSound 相容輸出。',
  'audioError.hostExitFailed': '音訊輸出裝置啟動失敗，可能是裝置拒絕了當前輸出模式、取樣率或緩衝設定。',
  'error.bridge.desktop': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.desktopShort': '目前無法完成此操作',
  'error.bridge.streamingSearch': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingTrack': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingAlbum': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingArtist': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingPlayback': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingBpm': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingLyrics': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingMv': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingFavorites': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.streamingDownloadResolve': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.locateAlbum': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.locateArtist': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.likeTracks': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.likeAlbums': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.exportMarkdown': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.diagnosticsZipUnsupported': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.openAudioReport': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.openLogDir': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.bridge.downloadService': '桌面下載服務不可用。',
  'error.bridge.streamingService': '桌面流媒體服務不可用，無法解析下載地址。',
  'error.bridge.playlistService': '桌面歌單服務不可用。',
  'error.bridge.spotify': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'error.devApi.unavailable.playlist': '本地開發介面未啟動，請重啟 npm run dev 後再新增歌單。',
  'error.devApi.unavailable.dailyRecommend': '本地開發介面未啟動，請重啟 npm run dev 後再重新整理每日推薦。',
  'error.devApi.unavailable.likedSongs': '本地開發介面未啟動，請重啟 npm run dev 後再同步喜歡歌單。',
  'error.devApi.unavailable.likedTrack': '本地開發介面未啟動，請重啟 npm run dev 後再同步喜歡狀態。',
  'error.devApi.importPlaylistFailed': '新增流媒體歌單失敗',
  'error.devApi.dailyRecommendFailed': '重新整理網易雲每日推薦失敗。',
  'error.devApi.syncLikedFailed': '同步線上喜歡歌單失敗。',
  'error.devApi.syncLikedTrackFailed': '同步喜歡狀態失敗。',
  'error.streaming.playlistOnlyDownload': '只有網路歌單中的流媒體歌曲可以直接下載。',
  'error.streaming.spotifyNoDownload': 'Spotify 由官方播放器播放，下載功能不適用於 Spotify。',
  'error.streaming.providerNoDownload': '這個平臺暫不支援從網路歌單直接下載。',
  'error.streaming.localPlaylistOnly': '流媒體歌曲不能加入本地歌單，請在流媒體歌單中單獨管理。',
  'error.connect.remoteSeekUnavailable': 'Connect 投送中，遠端 seek 不可用。',
  'error.hqPlayer.channelUnavailable': 'HQPlayer 接管不可用：沒有可用的 HQPlayer Connect 通道。',
  'error.plugin.systemUnavailable': '外掛系統當前不可用。',
  'error.artist.avatarUnsupported': '當前執行環境不支援設定藝術家頭像。',
  'error.artist.avatarChooseUnsupported': '當前執行環境不支援選擇藝術家頭像。',
  'error.artist.avatarNetworkUnsupported': '當前執行環境不支援從網路設定藝術家頭像。',
  'error.artist.avatarResetUnsupported': '當前執行環境不支援恢復自動頭像。',
  'error.downloads.osuAccountUnavailable': '當前執行環境無法讀取 osu! 賬號資料。',
  'error.downloads.folderRequired': '請選擇下載資料夾',
  'error.export.localOnly': '當前匯出只支援本地音訊檔案。流媒體請使用下載功能。',
  'error.export.fileMissing': '當前音訊檔案不存在，無法匯出。',
  'error.export.nothingToExport': '沒有可匯出的音訊檔案。',
  'error.export.targetExists': '匯出目標已存在。請換一個檔名，避免靜默覆蓋。',
  'error.export.cannotOverwriteSource': '匯出目標不能覆蓋當前播放的原始檔，請選擇另一個檔名。',
  'error.backup.missingManifest': '備份檔案缺少 manifest.json。',
  'error.backup.unsupported': '選中的檔案不是受支援的 ECHO 資料備份。',
  'error.backup.missingSettings': '備份檔案缺少設定檔案。',
  'error.backup.scanRunning': '曲庫掃描執行中，暫時不能匯入備份。',
  'error.backup.chooseDirectory': '請先選擇自動備份目錄。',
  'error.database.unhealthySnapshot': '曲庫資料庫當前不健康，已拒絕建立新的健康快照。',
  'error.database.snapshotMissing': '找不到這個曲庫資料庫快照，已拒絕恢復。',
  'error.database.snapshotNotRestorable': '這個快照不是可恢復的健康曲庫資料庫快照。',
  'error.database.snapshotCopyFailed': '快照複製失敗，曲庫資料庫檔案沒有恢復。',
  'error.database.quarantineMissing': '找不到可修復的隔離曲庫資料庫。',
  'error.database.quarantineCopyFailed': '隔離曲庫副本複製失敗，已拒絕修復。',
  'error.spotify.premiumRequired': 'Spotify Premium 才支援播放。',
  'error.spotify.signInExpired': 'Spotify 登入已失效，請在設定中重新登入。',
  'error.spotify.deviceNotReady': 'Spotify 播放裝置尚未就緒，請稍後再試。',
  'error.spotify.sdkTimeout': 'Spotify Web Playback SDK 載入超時。',
  'error.spotify.sdkLoadFailed': '無法載入 Spotify Web Playback SDK，請檢查網路連線。',
  'error.spotify.sdkNotReady': 'Spotify Web Playback SDK 尚未就緒。',
  'error.spotify.notPlayableTrack': '當前曲目不是可播放的 Spotify 曲目。',
  'error.spotify.stayedPaused': 'Spotify 已接受命令，但官方播放器仍處於暫停{device}。',
  'error.spotify.didNotSwitch': 'Spotify 沒有切換到請求的曲目{device}。請稍後再試。',
  'error.spotify.clientIdRequired': '請先在設定 > 整合 > Spotify OAuth 配置中填寫你自己的 Spotify Client ID，然後重新登入 Spotify。',
  'error.code.eqPresetNotFound': '找不到這個 EQ 預設。',
  'error.code.eqProfileNotFound': '找不到這個 EQ 配置檔。',
  'error.code.invalidEqPreset': 'EQ 預設無效。',
  'error.code.cannotOverwriteBuiltinEq': '不能覆蓋內建 EQ 預設。',
  'error.code.cannotDeleteBuiltinEq': '不能刪除內建 EQ 預設。',
  'error.code.invalidEqImport': 'EQ 預設匯入無效。',
  'error.code.opraUnavailable': 'OPRA 橋接不可用。',
  // --- inbox/playlists batch ---
  'inboxPage.reason.missing_cover': '缺封面',
  'inboxPage.reason.missing_title': '缺標題',
  'inboxPage.reason.missing_artist': '缺藝人',
  'inboxPage.reason.missing_album': '缺專輯',
  'inboxPage.reason.missing_album_artist': '缺專輯藝人',
  'inboxPage.reason.missing_track_no': '缺音軌號',
  'inboxPage.reason.missing_disc_no': '缺碟號',
  'inboxPage.reason.missing_year': '缺年份',
  'inboxPage.reason.missing_genre': '缺流派',
  'inboxPage.reason.unknown_artist': '未知藝人',
  'inboxPage.reason.filename_fallback': '檔名回退',
  'inboxPage.reason.unknown_field': '未知欄位',
  'inboxPage.reason.metadata_fallback': '後設資料回退',
  'inboxPage.reason.unknown_album': '未知專輯',
  'inboxPage.reason.embedded_metadata_error': '內嵌標籤讀取失敗',
  'inboxPage.reason.embedded_cover_error': '內嵌封面讀取失敗',
  'inboxPage.reason.network_metadata_candidate': '網路後設資料候選',
  'inboxPage.reason.network_cover_candidate': '網路封面候選',
  'inboxPage.reason.suspicious_file': '疑似異常',
  'inboxPage.error.desktopBridgePlaylist': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'inboxPage.error.desktopBridgeQueue': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'inboxPage.error.desktopBridgeState': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'inboxPage.error.desktopBridgeLocate': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'inboxPage.error.noLocalTracks': '當前篩選沒有可加入佇列的本地歌曲。',
  'inboxPage.playlist.name': '新歌待聽清單',
  'inboxPage.message.playlistCreated': '已生成歌單「{name}」，加入 {count} 首。',
  'inboxPage.message.playlistCreatedTruncated': '已生成歌單「{name}」，加入 {count} 首，已按效能保護加入前 {limit} 首。',
  'inboxPage.message.queueAdded': '已加入佇列 {count} 首。',
  'inboxPage.message.queueAddedTruncated': '已加入佇列 {count} 首，已按效能保護加入前 {limit} 首。',
  'inboxPage.source.label': '新歌收件箱',
  'inboxPage.bulk.aria': '收件箱批次操作',
  'inboxPage.bulk.selectPage': '選擇本頁',
  'inboxPage.bulk.deselectPage': '取消本頁選擇',
  'inboxPage.bulk.selectedCount': '已選 {count} 首',
  'inboxPage.bulk.filterCount': '當前篩選 {count} 首',
  'inboxPage.action.locateFile': '定位檔案',
  'inboxPage.action.markProcessed': '標記已處理',
  'inboxPage.action.ignore': '忽略問題',
  'inboxPage.action.setPending': '設為待處理',
  'inboxPage.action.locateTrack': '定位歌曲',
  'inboxPage.list.aria': '新歌列表',
  'inboxPage.albumWall.aria': '新增專輯牆',
  'inboxPage.albumWall.title': '新增專輯牆',
  'inboxPage.albumWall.trackCount': '{count} 首 · {duration}',
  'inboxPage.list.moreReasons': '還有 {count} 項',
  'inboxPage.loadMore': '繼續載入 {visible}/{total}',
  'inboxPage.loadingShort': '正在讀取...',
  'playlists.error.streamingOnlyDownload': '只有網路歌單中的流媒體歌曲可以直接下載。',
  'playlists.error.spotifyNoDownload': 'Spotify 由官方播放器播放，下載功能不適用於 Spotify。',
  'playlists.error.providerNoDownload': '這個平臺暫不支援從網路歌單直接下載。',
  'playlists.error.downloadService': '桌面下載服務不可用。',
  'playlists.error.streamingService': '桌面流媒體服務不可用，無法解析下載地址。',
  'playlists.error.playlistService': '桌面歌單服務不可用。',
  'inboxPage.message.stateUpdated': '已標記為{status} {count} 首。',
  'inboxPage.message.stateUpdatedTruncated': '已標記為{status} {count} 首，已按上限處理前 {limit} 首。',
  'playlists.message.addedToQueue': '已新增 {count} 首可用歌曲到佇列',
  'playlists.message.downloadQueued': '已加入下載佇列：{title}',
  'playlists.error.downloadJobFailed': '新增下載任務失敗',
  'playlists.error.playlistDownloadOnlyOnline': '只有可下載的網路歌單支援整歌單下載。',
  'playlists.error.chooseDownloadFolder': '請先在下載頁選擇下載資料夾。',
  'playlists.message.enqueueingPlaylist': '正在按歌單順序加入下載佇列：{name}',
  'playlists.error.noDownloadableTracks': '這個歌單裡沒有可下載的網路歌曲。',
  'playlists.message.playlistQueued': '已按歌單順序加入下載佇列：{count} 首',
  'playlists.message.playlistQueuedPartial': '已按歌單順序加入下載佇列：{enqueued} 首，{failed} 首未能解析。',
  'playlists.error.playlistDownloadJobFailed': '新增歌單下載任務失敗',
  'settings.remote.ux.date.never': '尚未執行',
  'settings.remote.ux.noData': '暫無資料',
  'settings.remote.ux.operationFailed': '操作失敗。',
  'settings.remote.ux.unavailableFallback': '這個來源暫時不可用。',
  'settings.remote.ux.error.timeout.title': '伺服器響應有點慢',
  'settings.remote.ux.error.timeout.description': '現有索引和快取不會消失。可以稍後重試，或先確認伺服器地址與埠仍能訪問。',
  'settings.remote.ux.error.dns.title': '暫時找不到這個伺服器',
  'settings.remote.ux.error.dns.description': '請檢查域名、家庭網路或 VPN。網路恢復後，ECHO 會自動做一次輕量重連。',
  'settings.remote.ux.error.network.title': '現在連線不上',
  'settings.remote.ux.error.network.description': '音樂索引會繼續保留。請確認服務正在執行；恢復聯網時 ECHO 會自動測試一次。',
  'settings.remote.ux.error.auth.title': '需要重新驗證身份',
  'settings.remote.ux.error.auth.description': '使用者名稱、密碼或 token 可能已變化。更新憑據不會刪除現有索引和收藏。',
  'settings.remote.ux.error.path.title': '音樂目錄搬家了',
  'settings.remote.ux.error.path.description': '請檢查根目錄設定。ECHO 會保留現有記錄，不會因為一次找不到目錄就清空曲庫。',
  'settings.remote.ux.error.generic.title': '這個來源暫時不可用',
  'settings.remote.ux.error.generic.description': '現有索引仍會保留。可以重新連線；如果問題持續，再檢視高階詳情。',
  'settings.remote.ux.error.rawReason': '原始原因：{message}',
  'settings.remote.ux.health.reconnecting.title': '正在恢復連線',
  'settings.remote.ux.health.reconnecting.description': '只進行一次輕量測試，不會啟動同步。',
  'settings.remote.ux.health.syncing.title': '正在同步',
  'settings.remote.ux.health.syncing.description': '已發現 {count} 首，播放時會自動降低後臺負載。',
  'settings.remote.ux.health.paused.title': '已暫停',
  'settings.remote.ux.health.paused.description': '連線資訊仍保留，重新啟用後可以繼續同步。',
  'settings.remote.ux.health.issues.title': '{count} 項需要留意',
  'settings.remote.ux.health.issues.description': '歌曲仍可使用；可以在來源詳情中檢視缺失檔案或後設資料問題。',
  'settings.remote.ux.health.neverSynced.title': '還沒有同步',
  'settings.remote.ux.health.neverSynced.description': '先預覽變化，確認後再建立本地索引。',
  'settings.remote.ux.health.stale.title': '一週沒有同步',
  'settings.remote.ux.health.stale.description': '上次成功同步：{date}。建議先預覽變化。',
  'settings.remote.ux.health.healthy.title': '一切正常',
  'settings.remote.ux.health.healthy.description': '上次成功同步：{date}。',
  'settings.remote.ux.issue.missing': '缺失檔案',
  'settings.remote.ux.source.enabled': '已啟用',
  'settings.remote.ux.source.disabled': '已停用',
  'settings.remote.ux.source.error': '異常',
  'settings.remote.ux.cover.progress': '已載入 {processed} / {total} · 還剩 {pending} · 執行 {running}',
  'settings.remote.ux.cover.none': '暫無封面任務',
  'settings.remote.ux.completion': '{done}/{total} · {percent}%',
  'settings.remote.ux.browser.filter.all': '全部',
  'settings.remote.ux.browser.filter.audio': '音訊',
  'settings.remote.ux.browser.filter.unindexed': '未索引',
  'settings.remote.ux.browser.filter.indexed': '已入庫',
  'settings.remote.ux.task.pending': '待處理',
  'settings.remote.ux.task.searching': '處理中',
  'settings.remote.ux.task.partial': '部分',
  'settings.remote.ux.task.ok': '完成',
  'settings.remote.ux.task.notFound': '未找到',
  'settings.remote.ux.task.error': '異常',
  'settings.remote.ux.browser.root': '根目錄',
  'settings.remote.ux.auth.none': '無需認證',
  'settings.remote.ux.auth.password': '使用者名稱密碼',
  'settings.remote.ux.auth.generic': '認證',
  'settings.remote.ux.recommend.metadata': '有 {count} 首後設資料異常，建議先只重試後設資料/時長。',
  'settings.remote.ux.recommend.cover': '有 {count} 首封面異常，可單獨重試封面補齊。',
  'settings.remote.ux.recommend.lyrics': '有 {count} 首歌詞異常，可單獨重試歌詞補齊。',
  'settings.remote.ux.recommend.missing': '有 {count} 個缺失檔案，建議檢查伺服器目錄或重新同步。',
  'settings.remote.ux.sync.discovering': '正在發現音樂',
  'streaming.error.desktopBridge': '暫時無法完成此操作，請重新啟動 ECHO 後再試。',
  'streaming.error.serviceUnavailable': '流媒體服務暫時不可用',
  'streaming.error.albumDetailUnavailable': '專輯詳情暫時不可用',
  'streaming.error.artistDetailUnavailable': '藝人詳情暫時不可用',
  'streaming.error.trackUnplayable': '這首歌暫時不可播放',
  'streaming.message.addedToQueue': '已加入佇列',
  'streaming.message.favorited': '已收藏：{title}',
  'streaming.message.unfavorited': '已取消收藏：{title}',
  'streaming.error.favoriteFailed': '收藏操作沒有成功',
  'streaming.error.platformStreamOnly': '這個平臺在 ECHO 中僅支援流播放，不提供下載任務。',
  'streaming.error.noDirectDownload': '這個平臺暫不支援從流媒體結果直接下載。',
  'streaming.error.downloadService': '桌面下載服務不可用。',
  'streaming.message.downloadQueued': '已加入下載佇列：{title}',
  'streaming.error.downloadJobFailed': '新增下載任務失敗',
  'streaming.message.downloadCompleted': '下載成功：{title}',
  'streaming.error.downloadServiceDesktop': '下載服務不可用：請在 ECHO 桌面端使用。',
  'streaming.error.albumNoDownloadable': '這張流媒體專輯沒有可下載的歌曲。',
  'streaming.message.albumDownloadFailed': '無法下載專輯：{title}',
  'streaming.message.albumDownloadPrepare': '準備下載專輯：{title}（0/{total}）',
  'streaming.message.albumDownloadProgress': '專輯下載中：{title}，{done}/{total} · {progress}%',
  'streaming.message.albumDownloadDone': '專輯下載完成：{title}（{total}/{total}）',
  'streaming.message.albumDownloadFinishedPartial': '專輯下載結束：{title}，完成 {done}/{total}，失敗 {failed}',
  'settings.remote.ux.phase.idle': '空閒',
  'settings.remote.ux.phase.testing': '測試連線',
  'settings.remote.ux.phase.scanning': '掃描檔案',
  'settings.remote.ux.phase.readingMetadata': '解析後設資料',
  'settings.remote.ux.phase.writingDatabase': '寫入索引',
  'settings.remote.ux.phase.markingMissing': '標記缺失',
  'settings.remote.ux.phase.finished': '已完成',
  'settings.remote.ux.phase.cancelled': '已取消',
  'settings.remote.ux.phase.failed': '失敗',
  'settings.remote.ux.issue.listed': '已列出 {name} 的 {kind} 問題。',
  'settings.remote.ux.issue.none': '{name} 暫時沒有 {kind} 問題。',
  'settings.remote.ux.browser.metadataLabel': '後設資料 {status}',
  'settings.remote.ux.browser.coverLabel': '封面 {status}',
  'settings.remote.ux.browser.lyricsLabel': '歌詞 {status}',
  'settings.remote.ux.issue.view': '檢視{kind}問題',
};

export default zhTW;
