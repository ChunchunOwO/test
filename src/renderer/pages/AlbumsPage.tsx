...
import { albumDetailNavigationEvent, consumePendingAlbumDetailNavigation, dispatchDetailReturnNavigation, peekPendingAlbumDetailNavigation, type DetailReturnTarget } from '../utils/albumNavigation';
...
const pageSize = 90;
const remoteAlbumPageSize = 120;
const albumWallReturnAnimationMs = 160;
const priorityAlbumWallImageCount = 6;
...
const buildAlbumWallVirtualFallbackRows = (
  rowCount: number,
  rowHeight: number,
  scrollTop: number,
  viewportHeight: number,
): Array<{ index: number; key: string; start: number }> => {
  const visibleRowCount = Math.ceil(Math.max(0, viewportHeight) / rowHeight);
  const fallbackRowCount = Math.min(rowCount, Math.max(albumWallVirtualFallbackRows, visibleRowCount + 2));
  const maxStartRow = Math.max(0, rowCount - fallbackRowCount);
  const viewportStartRow = Math.floor(Math.max(0, scrollTop) / rowHeight);
  const startRow = Math.min(maxStartRow, Math.max(0, viewportStartRow - 1));

  return Array.from({ length: fallbackRowCount }, (_, offset) => {
...
export const AlbumsPage = (): JSX.Element => {
  ...
  const [albumWallVirtualizationEnabled, setAlbumWallVirtualizationEnabled] = useState(true);
  ...
  const deferInitialAlbumWallLoadRef = useRef(Boolean(initialAlbumDetailRequest));
  const pauseDeferredAlbumImages = useScrollImagePause(pageRootRef);
  const activeAlbumPageSize = sourceMode === 'remote' ? remoteAlbumPageSize : pageSize;
  ...
  const loadAlbums = useCallback(
    async (
      ...
      try {
        ...
        const result = await library.getAlbums({
          page: nextPage,
          pageSize: options.pageSizeOverride ?? activeAlbumPageSize,
          search,
          sort,
        ...
        setPage(options.pageSizeOverride && mode === 'replace' ? Math.max(1, Math.ceil(result.items.length / activeAlbumPageSize)) : result.page);
    ...
    [activeAlbumPageSize, remoteSourceId, search, sort, sourceMode, t],
  );
  ...
  const setRemoteSource = useCallback((sourceId: string | null): void => {
    if (sourceId === remoteSourceId) {
      return;
    }
    requestIdRef.current += 1;
  ...
  useEffect(() => {
    if (deferInitialAlbumWallLoadRef.current) {
      return;
    }
  ...
  useEffect(() => {
    if (selectedAlbum || !deferInitialAlbumWallLoadRef.current) {
      return;
    }

    deferInitialAlbumWallLoadRef.current = false;
    void loadAlbums(1, 'replace');
  ...
  useEffect(() => {
    const applySettings = (settings: Partial<AppSettings> | null | undefined): void => {
      ...
      setAlbumWallVirtualizationEnabled(policy.albumWallVirtualizationEnabled);
  ...
  useEffect(() => {
    const handleLibraryChanged = (event: Event): void => {
      ...
      if (isPreserveScrollLibraryEvent(event) && scrollTop > preserveScrollThresholdPx) {
        void loadAlbums(1, 'replace', {
          pageSizeOverride: Math.min(
            sourceMode === 'remote' ? maxPreservedRemoteRefreshPageSize : maxPreservedRefreshPageSize,
            Math.max(activeAlbumPageSize, page * activeAlbumPageSize, albums.length),
          ),
  ...
  }, [activeAlbumPageSize, albums.length, loadAlbums, page, sourceMode]);
  ...
  const handleBackFromAlbumDetail = useCallback((): void => {
    if (dispatchDetailReturnNavigation(selectedAlbumReturnTo)) {
  ...
  const shouldVirtualizeAlbumWall = sourceMode === 'remote' || albumWallVirtualizationEnabled;
  const albumWallVirtualLayoutReady = shouldVirtualizeAlbumWall && albumWallViewportWidth > 0;
  ...
  const albumWallVirtualizer = useVirtualizer({
    ...
    overscan: sourceMode === 'remote' || lowSpecModeEnabled ? remoteAlbumWallVirtualOverscanRows : albumWallVirtualOverscanRows,
  });
  ...
  const renderedAlbumWallVirtualRows =
    albumWallVirtualLayoutReady && albumWallVirtualRows.length > 0
      ...
      : albumWallVirtualLayoutReady
        ? buildAlbumWallVirtualFallbackRows(
            albumWallVirtualRowCount,
            albumWallEstimatedRowHeight,
            readPageScrollTop(pageRootRef.current),
            pageRootRef.current?.clientHeight ?? 0,
          )
        : [];
  const lastAlbumWallVirtualRowIndex = renderedAlbumWallVirtualRows.at(-1)?.index ?? -1;
  const albumWallVirtualEstimatedTotalSize = albumWallVirtualRowCount * albumWallEstimatedRowHeight;
  const albumWallVirtualMeasuredTotalSize = albumWallVirtualizer.getTotalSize();
  const albumWallVirtualTotalSize = Number.isFinite(albumWallVirtualMeasuredTotalSize)
    ? Math.max(albumWallVirtualMeasuredTotalSize, albumWallVirtualEstimatedTotalSize)
    : albumWallVirtualEstimatedTotalSize;
  ...
  const renderAlbumCard = (album: LibraryAlbum, index: number): JSX.Element => {
    ...
    const isPriorityCover = index < (lowSpecModeEnabled ? 2 : priorityAlbumWallImageCount);
        ...
        <div className="album-cover" data-empty={!shouldShowCover} aria-hidden="true">
          {shouldShowCover ? (
            <DeferredWallImage
              ...
              paused={pauseDeferredAlbumImages || (shouldVirtualizeAlbumWall && albumWallVirtualizer.isScrolling)}
              priority={isPriorityCover}
              rootMargin={lowSpecModeEnabled ? '160px 0px' : albumWallImageLoadAheadMargin}
  ...
  return (
    <>
      {selectedAlbum ? <SteamAlbumDetailView album={selectedAlbum} onBack={handleBackFromAlbumDetail} /> : null}
      <div
      ...
      <div className="songs-control-row">
        ...
        <div className="songs-control-actions album-control-actions">
          <LibrarySourceSwitch value={sourceMode} onChange={setSourceMode} />
          {sourceMode === 'remote' ? <RemoteSourceFilter sources={remoteSources} value={remoteSourceId} onChange={setRemoteSource} /> : null}
      ...
      <div ref={pageRootRef} className="media-wall-scroll-shell page-scroll-container">
        {shouldVirtualizeAlbumWall ? (
          albumWallVirtualLayoutReady ? (
            ...
            <section className="album-wall" aria-label={t('library.albums.listAria')} data-virtualized="pending">
              {albums.slice(0, albumWallVirtualPendingCardLimit).map((album, index) => renderAlbumCard(album, index))}
            </section>
          )
        ) : (
        ...
        {shouldVirtualizeAlbumWall ? null : (
