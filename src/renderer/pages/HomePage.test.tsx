...
vi.mock('../stores/playbackStatusStore', () => ({
  ...
  useSharedPlaybackActivityState: () =>
    sharedPlaybackState.value.audioStatus?.state
    ?? sharedPlaybackState.value.playbackStatus?.state
    ?? 'idle',
  getSharedPlaybackStatusSnapshot: () => sharedPlaybackState.value,
  subscribeSharedPlaybackStatus: () => () => undefined,
}));
...
afterEach(() => {
  ...
  window.localStorage.removeItem('echo.locale');
  vi.restoreAllMocks();
...
describe('HomePage', () => {
  ...
  it('loads lightweight library and listening summaries', async () => {
    ...
    expect(document.querySelector('.home-recent-panel .home-cover-card img')?.getAttribute('src')).toBe('echo-cover://album/daily-cover-1');
    expect(document.querySelectorAll('.home-recommend-panel .home-cover-card')).toHaveLength(7);
    expect(document.querySelector('.home-recommend-panel .home-cover-card img')?.getAttribute('src')).toMatch(/^echo-cover:\/\/album\/daily-cover-/);
    expect(document.querySelectorAll('.home-favorite-album-panel .home-favorite-album-card')).toHaveLength(4);
    expect(document.querySelector('.home-favorite-album-panel .home-favorite-album-card img')?.getAttribute('src')).toBe('echo-cover://thumb/favorite-cover-1');
    ...
    expect(document.querySelectorAll('.home-recent-panel .home-cover-card')).toHaveLength(4);
    expect(document.querySelector('.home-recent-panel .home-played-rail img')?.getAttribute('src')).toBe('echo-cover://album/played-album-cover');
  ...
  it('persists the loaded home snapshot for the next launch', async () => {
    ...
    await waitForRecentPanelReady();
    await waitFor(() => expect(window.localStorage.getItem('echo.home-page-cache.v2')).toBeTruthy());
    const cached = JSON.parse(window.localStorage.getItem('echo.home-page-cache.v2') ?? '{}') as {
  ...
  it('renders translated home copy when locale is English', async () => {
    ...
    await waitForRecentPanelReady();
    expect(screen.getByText("Today's ECHO")).toBeTruthy();
  ...
  it('always uses the localized default home title', async () => {
    installLibraryMock();
    ...
    await waitForRecentPanelReady();
    expect(screen.getByRole('heading', { level: 1 }).textContent).toBe(translations['zh-CN']['home.hero.defaultTitle']);
  });
  ...
  it('keeps the home signal visualizer hidden by default', async () => {
    installLibraryMock();
    installAppSettingsMock({ audioVisualSpectrumEnabled: true });
  ...
  it('refreshes recently played album cards and weekly stats on playback history changes', async () => {
    ...
    expect(document.querySelector('.home-recent-panel .home-played-rail img')?.getAttribute('src')).toBe('echo-cover://album/fresh-played-cover');
  ...
  it('places the currently playing album at the front of the played rail immediately', async () => {
    ...
    await waitFor(() =>
      expect(document.querySelector('.home-recent-panel .home-played-rail img')?.getAttribute('src')).toBe('echo-cover://album/current-played-cover'),
    );
