...
const openFolderManagementMore = (): void => {
  const panel = document.querySelector<HTMLElement>('.folder-manage-section');
  if (!panel) {
    throw new Error('Folder management panel not found');
  }
  fireEvent.click(within(panel).getByText('More'));
};
...
beforeEach(() => {
  ...
  remoteSourcesMock = {
    ...
    getJobStatus: vi.fn().mockResolvedValue({
      ...
      concurrency: { metadata: 0, cover: 0, lyrics: 0, 'duration-backfill': 0 },
      current: [],
      paused: false,
      lastError: null,
      updatedAt: null,
...
describe('FoldersPage', () => {
  ...
  it('restores the expanded local tree and selected folder after a cold restart', async () => {
    ...
    expect(window.localStorage.getItem('echo.local-folder-tree-view.v1')).toContain('D:\\\\Music\\\\Rock');
  ...
  it('loads scoped tracks for the selected folder and recursive toggle', async () => {
    ...
    const scopeControl = document.querySelector<HTMLElement>('.folder-scope-control');
    expect(scopeControl).toBeTruthy();
    fireEvent.click(within(scopeControl!).getAllByRole('button')[0]!);
  ...
  it('offers track-number sorting and forwards it to the folder query', async () => {
    ...
    expect(window.localStorage.getItem('echo.folders.sort')).toBe('trackNumber');
  });
  ...
  it('plays the selected folder in folder order instead of random sort', async () => {
    window.localStorage.setItem('echo.folders.sort', 'random');
  ...
  it('keeps secondary folder playback actions in the More menu', async () => {
    renderFoldersPage();

    await screen.findByRole('heading', { name: 'Folders' });
    const actions = document.querySelector<HTMLElement>('.folder-detail-actions');
    ...
    expect(screen.getByRole('menuitem', { name: 'Add to queue tail' })).toBeTruthy();
  });
  ...
  it('remembers local root folder order after dragging folders', async () => {
    ...
    expect(window.localStorage.getItem('echo.folder-root-order.v1')).toContain('folder-2');
  ...
  it('disables album index repair while a local scan is running', async () => {
    ...
    await screen.findByRole('heading', { name: 'Folders' });
    openFolderManagementMore();
    const repairButton = screen.getByRole('button', { name: 'Repair Album Index' });
  ...
  it('shows scan percentage and expandable error details', async () => {
    const activeScan = scanStatus({
      totalFiles: 8,
      processedFiles: 2,
      errorCount: 2,
      errors: ['D:\\Music\\broken.flac: decode failed', 'D:\\Music\\missing.mp3: file missing'],
    });
    libraryMock.getScanStatus.mockResolvedValue(activeScan);
    act(() => {
      rememberLibraryScanStatus(activeScan);
    });

    renderFoldersPage();

    await screen.findByRole('heading', { name: /Folders|文件夹/ });
    const progress = await waitFor(() => {
      const element = document.querySelector<HTMLElement>('.folder-scan-progress-track[role="progressbar"]');
    ...
    const errorDetails = document.querySelector<HTMLElement>('.folder-scan-errors');
    expect(errorDetails).toBeTruthy();
    fireEvent.click(within(errorDetails!).getByText(/2/));
    expect(screen.getByText(/broken\.flac: decode failed/)).toBeTruthy();
    expect(screen.getByText(/missing\.mp3: file missing/)).toBeTruthy();
  });
  ...
  it('separates remote folder browsing from local folder APIs', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('WebDAV Music').length).toBeGreaterThan(0));
    expect(await screen.findByText('WebDAV / AList · 已启用')).toBeTruthy();
  ...
  it('shows cached indexed remote tracks for a selected folder without previewing every file again', async () => {
    const cachedTrack = track({
        ...
        mediaType: 'remote',
        sourceId: 'remote-1',
        sourceDisplayName: 'WebDAV Music',
        provider: 'webdav',
