...
const readLowSpecModeEnabled = (settings: Partial<AppSettings> | null | undefined): boolean =>
...
const extractCoverPalette = (image: HTMLImageElement): NowPlayingCoverPalette | null => {
  ...
  try {
    ...
    data = context.getImageData(0, 0, coverColorSampleSize, coverColorSampleSize).data;
  } finally {
    canvas.width = 0;
    canvas.height = 0;
  }
...
export const NowPlayingPage = (): JSX.Element => {
  ...
  const shouldUseCoverColor = nowPlayingCoverColorEnabled && !lowLoadPlaybackModeEnabled && !lowSpecModeEnabled;
  ...
  useEffect(() => {
    ...
    const applyInitialSettings = (settings: Partial<AppSettings> | null | undefined): void => {
      if (!cancelled) {
        ...
        setLowSpecModeEnabled(readLowSpecModeEnabled(settings));
    ...
    const handleSettingsChanged = (event: Event): void => {
      ...
      if (Object.prototype.hasOwnProperty.call(patch, 'lowSpecModeEnabled')) {
        setLowSpecModeEnabled(readLowSpecModeEnabled(patch));
      }
  ...
  useEffect(() => {
    ...
    let disposed = false;
    let cancelImageSampling: (() => void) | null = null;

    const cancelIdle = scheduleCoverColorRead(() => {
      const task = sampleDetachedImage(coverUrl, extractCoverPalette);
      cancelImageSampling = task.cancel;
      void task.promise.then((palette) => {
        if (disposed) {
          return;
        }
