...
const pageSize = 100;
const legacyStreamingUiEnabled = false as const;
...
const playlistListOrderMemoryKey = 'echo.playlist-list-order.v1';
const favoriteListOrderMemoryKey = 'echo.streaming-favorite-list-order.v1';
...
const playlistSourceFilterOptions: Array<{ value: PlaylistSourceFilter; labelKey: 'playlistsPage.filter.local' }> = [
...
const playlistDownloadMemoryKey = 'echo.playlist-download-session.v1';
...
export const PlaylistsPage = (): JSX.Element => {
  ...
  const [playlistSourceFilter, setPlaylistSourceFilter] = useState<PlaylistSourceFilter>('local');
  ...
  const filteredOrderedPlaylists = useMemo(
    () =>
      orderedPlaylists.filter((playlist) => playlist.sourceProvider === 'local'),
    [orderedPlaylists],
  );
  ...
  const loadPlaylists = useCallback(async (): Promise<void> => {
    ...
    if (!library) {
      setError('Desktop bridge unavailable. Open ECHO in Electron to use playlists.');
      return;
    }
  ...
  const handleGenerateSmartPlaylist = async (): Promise<void> => {
    ...
    if (!library?.createSmartPlaylist) {
      setError('Desktop bridge unavailable. Open ECHO in Electron to generate smart playlists.');
      setStatusMessage(null);
      return;
    }
  ...
  const handleDeleteStreamingPlaylistFromContextMenu = async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    ...
    setPlaylistContextMenu(null);
    if (!library || !playlist) {
      setError('Desktop bridge unavailable. Open ECHO in Electron to delete streaming playlists.');
      return;
    }
  ...
  const handleExportPlaylist = async (format: PlaylistExportFormat, playlistOverride?: LibraryPlaylist): Promise<void> => {
    ...
    if (!library?.exportPlaylist || !playlist) {
      setError('Desktop bridge unavailable. Open ECHO in Electron to export playlists.');
      return;
    }
  ...
  const handleExportStreamingFavorites = async (): Promise<void> => {
    ...
    if (!streaming?.exportFavorites) {
      setError('Desktop bridge unavailable. Open ECHO in Electron to export streaming favorites.');
      setStatusMessage(null);
      return;
    }
  ...
  const handleImportPlaylistFile = async (): Promise<void> => {
    ...
    if (!library?.importPlaylistFile) {
      setError('Desktop bridge unavailable. Open ECHO in Electron to import playlist files.');
      setStatusMessage(null);
      return;
    }
  ...
  const handleAddLocalFilesToPlaylist = async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    ...
    if (!library?.addLocalAudioFilesToPlaylist || (!library.chooseImportFiles && !playback) || !playlist) {
      setError('Desktop bridge unavailable. Open ECHO in Electron to add local songs.');
      setStatusMessage(null);
      return;
    }
          ...
          case 'remove-from-playlist':
            {
              ...
              if (!library?.removePlaylistItem || !playlistId || !itemId) {
                setError('Desktop bridge unavailable. Open ECHO in Electron to remove playlist songs.');
                return;
              }
        ...
        <AnimatePresence mode="wait" initial={false}>
          {!legacyStreamingUiEnabled ? (
            ...
            <div className="playlist-list">
              ...
              {filteredOrderedPlaylists.length === 0 ? <p className="playlist-empty">{t('playlistsPage.empty.local')}</p> : null}
            </div>
            </motion.div>
          ) : (
            <motion.div
