...
const audioCdCopyByLocale: Record<Locale, AudioCdCopy> = {
  'zh-CN': {
    ...
    errors: {
      ...
      native_cdda_unavailable: 'Windows 原生 CD 读取组件不可用，请修复应用安装。',
      no_cd_drive: '未检测到光驱。',
  ...
  'zh-TW': {
    ...
    errors: {
      ...
      native_cdda_unavailable: 'Windows 原生 CD 讀取元件不可用，請修復應用程式安裝。',
      no_cd_drive: '未偵測到光碟機。',
  ...
  'ja-JP': {
    ...
    errors: {
      ...
      native_cdda_unavailable: 'Windows のネイティブ CD 読み取りコンポーネントを利用できません。アプリを修復してください。',
  ...
  'en-US': {
    ...
    errors: {
      ...
      native_cdda_unavailable: 'The native Windows CD reader is unavailable. Repair the app installation.',
  ...
  'ko-KR': {
    ...
    errors: {
      ...
      native_cdda_unavailable: 'Windows 기본 CD 읽기 구성 요소를 사용할 수 없습니다. 앱 설치를 복구하세요.',
  ...
  return (
    <div className="page-stack audio-cd-page">
      ...
      <section className="audio-cd-metrics" aria-label="Audio CD status">
        ...
        <span data-state={status.inputAvailable ? 'available' : 'unavailable'}>
          <strong>{status.inputAvailable ? copy.available : copy.unavailable}</strong>
          {copy.cdInput}
        </span>
      </section>
