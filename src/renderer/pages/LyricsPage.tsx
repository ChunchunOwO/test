...
import "../styles/lyrics-folded.css";
import "../styles/lyrics-cinema-stage.css";
import "../styles/lyrics-kinetic-poster.css";
import "../styles/lyrics-cover-stage.css";
import "../styles/lyrics-cut-board.css";
import "../styles/lyrics-theme-filter.css";
import "../styles/lyrics-background-source.css";
import "../styles/lyrics-route-motion.css";
...
import { containsOnlyInstrumentalPlaceholderLines } from "../../shared/utils/instrumentalLyrics";
...
import { neteaseDjRadioPlaylistPrefix, streamingProviderNames } from "../../shared/mediaProviderIdentity";
import type { PlaybackStatus } from "../../shared/types/playback";
import { decodeTextFileBytes } from "../../shared/utils/decodeTextFile";
import { LyricsBackdrop } from "../components/lyrics/LyricsBackdrop";
import { LyricsCoverStageArtwork } from "../components/lyrics/LyricsCoverStageArtwork";
import { LyricsCinemaStageChrome } from "../components/lyrics/LyricsCinemaStageChrome";
import { LyricsCinemaStageParticles } from "../components/lyrics/LyricsCinemaStageParticles";
import { KineticPosterChrome } from "../components/lyrics/KineticPosterChrome";
import { LyricsCutBoardChrome } from "../components/lyrics/LyricsCutBoardChrome";
import {
  fallbackLyricsCutBoardLayout,
  sampleLyricsCutBoardImage,
  type LyricsCutBoardLayout,
} from "../components/lyrics/lyricsCutBoardLayout";
import { resolveLyricsBackgroundPolicy } from "../components/lyrics/lyricsBackgroundPolicy";
import { resolveVisibleLyricsPageStyle } from "../components/lyrics/lyricsPageStyleAvailability";
import {
  isFullAppSettingsSnapshot,
  resolveLyricsPageStyleFromLoadedSettings,
  resolveLyricsPageStyleFromSettingsEvent,
  type LyricsPageStyleWrite,
} from "../components/lyrics/lyricsPageStyleSettings";
import { LyricsView, getActiveLyricIndex, getEstimatedPlainLyricIndex } from "../components/lyrics/LyricsView";
import { getLyricsWordTimingMode, type LyricsWordTimingMode } from "../components/lyrics/lyricsWordTimingMode";
import {
...
import { requestPlaybackCommand } from "../components/player/playbackCommandBus";
import { titleFromPath } from "../components/player/playerFormat";
import { usePlaybackQueue } from "../stores/PlaybackQueueProvider";
import { beginPlaybackSeekSnapshot, refreshPlaybackStatus, useThrottledSharedPlaybackStatus } from "../stores/playbackStatusStore";
...
import { sampleDetachedImage } from "../utils/detachedImageSampling";
import {
...
import { WorkshopLyricsSceneRenderer } from "../workshop/WorkshopLyricsSceneRenderer";
import { useActiveWorkshopLyricsScene } from "../workshop/useActiveWorkshopLyricsScene";
import { useWorkshopSceneVolume } from "../workshop/useWorkshopSceneVolume";
...
type LyricsDisplaySettings = Pick<
  ...
  | "lyricsRoseVinylGradientEnabled"
...
const normalizeInstrumentalPlaceholderLyrics = (lyrics: LyricsState): LyricsState => {
  if (
    lyrics.kind === "instrumental" ||
    lyrics.lines.length === 0 ||
    !containsOnlyInstrumentalPlaceholderLines(lyrics.lines.map((line) => line.text))
  ) {
    return lyrics;
  }
...
const lyricsPageSessionMemoryPrefix = "echo.lyrics-page.state.v1:";
const lyricsPageSessionMemoryIndexKey = "echo.lyrics-page.state.index.v1";
  ...
  window.dispatchEvent(new CustomEvent("lyrics:current-provider-changed", {
    detail: lyrics
      ? {
          ...
          title: lyrics.title,
          kind: lyrics.kind,
          wordTimingMode: getLyricsWordTimingMode(lyrics),
        }
      : {
          ...
          wordTimingMode: fallback?.wordTimingMode ?? 'unavailable',
        },
  }));
};
...
const lyricsPlaybackStatusUiIntervalMs = 200;
...
const LyricsTrackMarqueeText = ({
  ...
  useEffect(() => {
    ...
    const updateOverflow = (): void => {
      ...
      frameId = window.requestAnimationFrame(() => {
        frameId = null;
        const distance = maxLines === 2
          ? Math.max(0, innerElement.scrollHeight - element.clientHeight)
          : Math.max(0, innerElement.scrollWidth - element.clientWidth);
        element.style.setProperty("--lyrics-track-marquee-distance", `${distance + (maxLines === 2 ? 8 : 24)}px`);
  ...
  const sharedProps = {
    ...
    "data-max-lines": String(maxLines),
...
const lyricsProviderLabels: Partial<Record<LyricsSource, string>> = {
  local: "本地",
...
const isNetworkLyricsSource = (source: TrackLyrics["provider"] | LyricsProviderId | null | undefined): boolean =>
  source === "subsonic" || Boolean(source && networkLyricsProviderIds.has(source as LyricsProviderId));
...
const lyricsSourceDisplayLabel = (source: TrackLyrics["provider"] | LyricsProviderId | null | undefined, fallback?: string | null): string | null => {
  ...
  return lyricsProviderLabels[source as LyricsSource] ?? null;
};
...
const streamingLyricsProviderDetail = (
  ...
  if (!result || lyrics.kind === "empty") {
    return null;
  }

  const provider = result.provider as LyricsSource;
  const knownProvider = lyricsProviderLabels[provider] ? provider : null;
  const providerLabel = lyricsSourceDisplayLabel(knownProvider, result.sourceLabel);
  return {
...
const safeBackgroundCoverUrl = (track: LibraryTrack | null): string | null => {
  const displayCoverUrl = safeDisplayCoverUrl(track);
  return localCoverBackgroundUrl(track?.coverId, displayCoverUrl) ?? displayCoverUrl;
};
...
const selectLyricsDisplaySettings = (
  ...
  lyricsBackgroundMode: performancePolicy.lowSpecModeEnabled && settings.lyricsBackgroundMode === "coverColor"
    ? "cover"
    : settings.lyricsBackgroundMode,
  ...
  lyricsWordHighlightEnabled: performancePolicy.lyricsWordHighlightEnabled,
  lyricsWordHighlightClarityPercent:
  ...
  lyricsAutoSearch: settings.lowLoadPlaybackModeEnabled === true || performancePolicy.lowSpecModeEnabled ? false : settings.lyricsAutoSearch,
  ...
  lyricsSmartReadableColorsEnabled: performancePolicy.lyricsSmartReadableColorsEnabled,
  lyricsPageStyle: settings.lyricsPageStyle ?? fallbackLyricsDisplaySettings.lyricsPageStyle,
  lyricsThemeFilterEnabled: performancePolicy.lyricsThemeFilterEnabled,
  lyricsImmersiveCoverStyleEnabled: performancePolicy.lyricsImmersiveCoverStyleEnabled,
  lyricsImmersiveCoverGlassEnabled: performancePolicy.lyricsImmersiveCoverGlassEnabled,
  lyricsImmersiveCoverGlassBlurPx: performancePolicy.lowSpecModeEnabled
    ? 0
    : settings.lyricsImmersiveCoverGlassBlurPx ?? fallbackLyricsDisplaySettings.lyricsImmersiveCoverGlassBlurPx,
  lyricsRoseVinylBackgroundBlurPx: performancePolicy.lowSpecModeEnabled
    ? 0
    : settings.lyricsRoseVinylBackgroundBlurPx ?? fallbackLyricsDisplaySettings.lyricsRoseVinylBackgroundBlurPx,
  lyricsRoseVinylGradientEnabled: !performancePolicy.lowSpecModeEnabled && settings.lyricsRoseVinylGradientEnabled === true,
  lyricsMusicReactiveVisualsEnabled: performancePolicy.lyricsMusicReactiveVisualsEnabled,
  lyricsCoverBlurPx: performancePolicy.lyricsCoverBlurPx,
...
const createCoverColorCssVars = (
  ...
  const textTint = mixCoverRgb(averageMix, { r: 255, g: 255, b: 255 }, 0.86);
  if (themeMode === null) {
    return {
      "--lyrics-cover-color-rgb": rgbToCssChannels(primary),
      "--lyrics-cover-color-soft-rgb": rgbToCssChannels(mixCoverRgb(averageMix, { r: 255, g: 255, b: 255 }, 0.28)),
      "--lyrics-cover-color-deep-rgb": rgbToCssChannels(mixCoverRgb(averageMix, { r: 8, g: 10, b: 14 }, 0.58)),
      "--lyrics-cover-color-glow-rgb": rgbToCssChannels(mixCoverRgb(primary, sample.averageRgb, 0.36)),
      "--lyrics-cover-text-rgb": rgbToCssChannels(textTint),
    };
  }
...
const mergeLyricsDisplaySettingsFromEvent = (
  current: LyricsDisplaySettings,
  patch: Partial<LyricsDisplaySettings>,
  detail: Record<string, unknown>,
  recentWrite: LyricsPageStyleWrite | null,
  nowMs: number,
): LyricsDisplaySettings => {
  const resolvedStyle = resolveLyricsPageStyleFromSettingsEvent({
    currentStyle: current.lyricsPageStyle ?? "default",
    incomingStyle: patch.lyricsPageStyle,
    isFullSnapshot: isFullAppSettingsSnapshot(detail),
    recentWrite,
    nowMs,
  });
  const merged = {
    ...current,
    ...patch,
    lyricsPageStyle: resolvedStyle,
  };
  if (!Object.prototype.hasOwnProperty.call(detail, "lowSpecModeEnabled")) {
    return merged;
  }

  return selectLyricsDisplaySettings({
    ...merged,
    ...detail,
    lyricsPageStyle: merged.lyricsPageStyle,
  } as AppSettings);
};
...
const shouldReduceLyricsMvGraphicsForMemoryPressure = (
  ...
  (guardEnabled || event.rendererMitigationRecommended !== false);
...
export const LyricsPage = ({ initialLyrics, isActive = true, usePlayerDrawerHeader = false }: LyricsPageProps): JSX.Element => {
  ...
  const workshopSceneVolume = useWorkshopSceneVolume(activeWorkshopLyricsScene !== null);
  const isDocumentVisible = useDocumentVisibilityState();
  const sharedPlaybackStatus = useThrottledSharedPlaybackStatus(lyricsPlaybackStatusUiIntervalMs);
  ...
  const [cutBoardLayout, setCutBoardLayout] = useState<LyricsCutBoardLayout>(fallbackLyricsCutBoardLayout);
  ...
  const lyricsPageStyleWriteRef = useRef<LyricsPageStyleWrite | null>(null);
  ...
  const customLyricsApplyRequestRef = useRef(0);
  ...
  const publishCurrentLyricsProvider = useCallback((trackLyrics: TrackLyrics | null, fallback?: CurrentLyricsProviderDetail | null): void => {
    const nextDetail = trackLyrics
      ? {
          ...
          wordTimingMode: getLyricsWordTimingMode(trackLyrics),
        }
      : fallback ?? null;
  ...
  const workshopSceneTrackTech = useMemo(
    () =>
      currentTrack
        ? {
            codec: currentTrack.codec ?? null,
            sampleRateHz: currentTrack.sampleRate ?? null,
            bitDepth: currentTrack.bitDepth ?? null,
            // Library stores bitrate in bits per second.
            bitrateKbps: currentTrack.bitrate ? currentTrack.bitrate / 1000 : null,
  ...
  const lyricsRenderPressureReduced =
    ...
    lyricsDisplaySettings.lowSpecModeEnabled === true ||
    sessionGraphicsPressureReduced;
  const lyricsRenderBudget = useRenderBudget({ pressureReduced: lyricsRenderPressureReduced });
  const lyricsVisualMotionActive =
    isActive &&
    isDocumentVisible &&
    lyricsRenderBudget.allowDecorativeMotion &&
  ...
  // Lightweight mode keeps the same cover surface but uses the display-sized
  // variant instead of the original file, which is a full-window decode.
  const backgroundCoverUrl = lyricsDisplaySettings.lowSpecModeEnabled
    ? (safeDisplayCoverUrl(currentTrack) ?? airPlayArtworkUrl ?? headerCoverUrl ?? coverUrl)
    : (safeBackgroundCoverUrl(currentTrack) ?? airPlayArtworkUrl ?? headerCoverUrl ?? coverUrl);
  ...
  const lyricsImportIdentityRef = useRef(lyricsMemoryKey);
  lyricsImportIdentityRef.current = lyricsMemoryKey;
  ...
  const normalizedDisplayedLyrics = useMemo(
    () => normalizeInstrumentalPlaceholderLyrics(displayedLyrics),
    [displayedLyrics],
  );
  ...
  const effectiveDisplayedLyrics = useMemo(
    () =>
      lyricsDisplaySettings.lyricsTimelineCorrectionEnabled !== false
        ? normalizedDisplayedLyrics
        : { ...normalizedDisplayedLyrics, offsetMs: 0 },
    [lyricsDisplaySettings.lyricsTimelineCorrectionEnabled, normalizedDisplayedLyrics],
  );
  const hasSettledEmptyLyricsState =
    effectiveDisplayedLyrics.lines.length === 0 &&
    (effectiveDisplayedLyrics.kind === "instrumental" || (!isLyricsLoading && !isCandidateLoading));
  const shouldHideSettledEmptyLyricsState =
    lyricsDisplaySettings.lyricsEmptyStateHidden && hasSettledEmptyLyricsState;
  ...
  const requestedLyricsPageStyle = resolveVisibleLyricsPageStyle(lyricsDisplaySettings.lyricsPageStyle ?? "default");
  const activeLyricsPageStyle =
    !activeWorkshopLyricsScene &&
    lyricsViewMode === "lyrics" &&
    (requestedLyricsPageStyle === "editorial" ||
      requestedLyricsPageStyle === "folded" ||
      requestedLyricsPageStyle === "roseVinyl" ||
      requestedLyricsPageStyle === "cinemaStage" ||
      requestedLyricsPageStyle === "kineticPoster" ||
      requestedLyricsPageStyle === "coverStage" ||
      requestedLyricsPageStyle === "cutBoard")
      ? requestedLyricsPageStyle
      : "default";
  const shouldUseEditorialStyle = activeLyricsPageStyle === "editorial";
  const shouldUseFoldedStyle = activeLyricsPageStyle === "folded";
  const shouldUseRoseVinylStyle = activeLyricsPageStyle === "roseVinyl";
  const shouldUseCinemaStageStyle = activeLyricsPageStyle === "cinemaStage";
  const shouldUseKineticPosterStyle = activeLyricsPageStyle === "kineticPoster";
  const shouldUseCoverStageStyle = activeLyricsPageStyle === "coverStage";
  const shouldUseCutBoardStyle = activeLyricsPageStyle === "cutBoard";
  const shouldUseImmersiveCoverStyle =
    !lyricsRenderPressureReduced &&
    activeLyricsPageStyle !== "folded" &&
    activeLyricsPageStyle !== "roseVinyl" &&
    activeLyricsPageStyle !== "cinemaStage" &&
    activeLyricsPageStyle !== "kineticPoster" &&
    activeLyricsPageStyle !== "coverStage" &&
    activeLyricsPageStyle !== "cutBoard" &&
    lyricsDisplaySettings.lyricsImmersiveCoverStyleEnabled === true &&
    lyricsViewMode === "lyrics";
  const lyricsBackgroundModeOverrideEnabled = lyricsDisplaySettings.lyricsBackgroundModeOverrideEnabled === true;
  const lyricsBackgroundPolicy = resolveLyricsBackgroundPolicy({
    immersiveCoverStyleEnabled: shouldUseImmersiveCoverStyle,
    pageStyle: activeLyricsPageStyle,
    savedMode: lyricsDisplaySettings.lyricsBackgroundMode,
    userOverrideEnabled: lyricsBackgroundModeOverrideEnabled,
  });
  const requestedLyricsBackgroundMode = lyricsBackgroundPolicy.sourceMode;
  const effectiveLyricsBackgroundMode =
    ...
    !lyricsDisplaySettings.lyricsCustomWallpaperPath
      ...
      : requestedLyricsBackgroundMode === "cover" &&
          ...
          !shouldUseKineticPosterStyle &&
          !shouldUseCutBoardStyle
        ? "theme"
        : requestedLyricsBackgroundMode === "coverColor" && coverColorSampleUrls.length === 0
          ? "theme"
          : requestedLyricsBackgroundMode;
  const lyricsBackgroundLayoutMode = activeLyricsPageStyle === "default"
    ? effectiveLyricsBackgroundMode
    : lyricsBackgroundPolicy.layoutMode;
  ...
  const shouldUseMusicReactiveVisuals =
    lyricsVisualMotionActive &&
    musicReactiveVisualsFeatureEnabled &&
    lyricsDisplaySettings.lyricsMusicReactiveVisualsEnabled === true &&
    lyricsViewMode === "lyrics";
  const shouldRenderCinemaStageParticles =
    shouldUseCinemaStageStyle &&
    !lyricsRenderPressureReduced &&
    lyricsViewMode === "lyrics";
  const cinemaStageParticleMode =
    shouldRenderCinemaStageParticles && lyricsVisualMotionActive
      ? musicReactiveScene.mode
      : "idle";
  ...
  const lyricsWallpaperUrl = lyricsDisplaySettings.lyricsCustomWallpaperPath
    ...
    : null;
  const lyricsBackgroundCoverUrl = backgroundCoverUrl;
  ...
  const lyricsSmartReadableEnabled =
    ...
    shouldUseRoseVinylStyle ||
    shouldUseCoverStageStyle ||
    shouldUseCutBoardStyle ||
    lyricsDisplaySettings.lyricsSmartReadableColorsEnabled === true;
  const shouldUseFixedImmersiveCoverLyrics =
  ...
  const lyricsSmartReadableImageUrls = useMemo<readonly string[]>(
    () => {
      if (!lyricsSmartReadableEnabled || shouldUseFixedImmersiveCoverLyrics) {
    ...
    [
      ...
      lyricsWallpaperUrl,
      shouldUseFixedImmersiveCoverLyrics,
    ],
  );
  ...
  const coverColorCssVars = useMemo<CoverColorCssVars | null>(
    () =>
      effectiveLyricsBackgroundMode === "coverColor" || shouldUseCoverStageStyle
        ? createCoverColorCssVars(imageReadableSample, shouldUseCoverStageStyle ? null : documentThemeMode)
        : null,
    [documentThemeMode, effectiveLyricsBackgroundMode, imageReadableSample, shouldUseCoverStageStyle],
  );
  ...
  const cutBoardCssVars = useMemo<CSSProperties>(
    () => ({
      "--lyrics-cut-board-accent": cutBoardLayout.accentColor,
      "--lyrics-cut-board-columns": cutBoardLayout.columns.map((column) => `${column}%`).join(" "),
    } as CSSProperties),
    [cutBoardLayout],
  );
  ...
  const lyricsPageStyle = useMemo(
    () =>
      ({
        "--lyrics-cover": isActive && effectiveLyricsBackgroundMode === "cover" && lyricsBackgroundCoverUrl
          ? cssUrl(lyricsBackgroundCoverUrl)
          : "none",
        "--lyrics-wallpaper": isActive && lyricsWallpaperUrl
        ...
        ...(coverColorCssVars ?? {}),
        ...(smartReadableColors ?? {}),
        ...(shouldUseCutBoardStyle ? cutBoardCssVars : {}),
        ...(shouldUseMusicReactiveVisuals || shouldRenderCinemaStageParticles ? musicReactiveCssVars : {}),
      }) as CSSProperties,
    [
      coverColorCssVars,
      effectiveLyricsBackgroundMode,
      ...
      shouldRenderCinemaStageParticles,
      smartReadableColors,
    ],
  );
  ...
  useEffect(() => {
    if (!isActive || !lyricsSampleImageKey || lyricsDisplaySettings.lowSpecModeEnabled) {
  ...
  }, [isActive, lyricsDisplaySettings.lowSpecModeEnabled, lyricsSampleImageKey]);

  useEffect(() => {
    if (!isActive || !shouldUseCutBoardStyle || !lyricsBackgroundCoverUrl) {
      setCutBoardLayout(fallbackLyricsCutBoardLayout);
      return undefined;
    }

    let disposed = false;
    let cancelImage = (): void => undefined;
    const cancelSchedule = scheduleLyricsImageSampling(() => {
      const task = sampleDetachedImage(
        lyricsBackgroundCoverUrl,
        sampleLyricsCutBoardImage,
        /^https?:\/\//iu.test(lyricsBackgroundCoverUrl) ? { crossOrigin: "anonymous" } : undefined,
      );
      cancelImage = task.cancel;
      void task.promise.then((layout) => {
        if (!disposed && layout) {
          setCutBoardLayout(layout);
        }
      });
    });
  ...
  }, [isActive, lyricsBackgroundCoverUrl, shouldUseCutBoardStyle]);
  ...
  const handleOpenAlbumDetail = useCallback((): void => {
    ...
    albumNavigationTimeoutRef.current = window.setTimeout(() => {
      ...
      void openAlbumDetailForTrack(currentTrack, { returnTo: 'lyrics' })
        .then((locatedAlbum) => {
          if (!locatedAlbum) {
  ...
  const lyricsSeekTimelineOffsetMs =
    ...
    (lyricsDisplaySettings.lyricsTimelineCorrectionEnabled !== false
      ...
      : 0);
  // Scrubbing the timeline stays valid without synced lyrics; only clicking a
  // lyric line needs per-line timestamps.
  const lyricsTimelineSeekEnabled =
    isActive &&
    !trackTransition &&
  ...
  const lyricsLineSeekEnabled =
    lyricsTimelineSeekEnabled && effectiveDisplayedLyrics.kind === "synced";
  ...
  const loadLyricsDisplaySettings = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const nextDisplaySettings = selectLyricsDisplaySettings(nextSettings);
      setLyricsDisplaySettings({
        ...nextDisplaySettings,
        lyricsPageStyle: resolveLyricsPageStyleFromLoadedSettings({
          loadedStyle: nextDisplaySettings.lyricsPageStyle ?? "default",
          recentWrite: lyricsPageStyleWriteRef.current,
          nowMs: Date.now(),
        }),
      });
    } catch {
  ...
  useEffect(() => {
    return window.echo?.diagnostics?.onMemoryPressure?.((event) => {
      if (shouldReduceLyricsMvGraphicsForMemoryPressure(
        event,
        lyricsDisplaySettings.lyricsMvGraphicsPressureGuardEnabled !== false,
      )) {
  ...
  useEffect(() => {
    const handleSettingsChanged = (event: Event): void => {
      ...
      const patch = pickLyricsDisplaySettingsPatch(detail);
      if (Object.keys(patch).length > 0 && detail) {
        lyricsDisplaySettingsLoadVersionRef.current += 1;
        if (patch.lyricsPageStyle !== undefined && !isFullAppSettingsSnapshot(detail)) {
          lyricsPageStyleWriteRef.current = { style: patch.lyricsPageStyle, atMs: Date.now() };
        }
        setLyricsDisplaySettings((current) =>
          mergeLyricsDisplaySettingsFromEvent(
            current,
            patch,
            detail,
            lyricsPageStyleWriteRef.current,
            Date.now(),
          ),
        );
  ...
  useEffect(() => {
    const handleLyricsPageWheel = (event: WheelEvent): void => {
      const page = lyricsPageRef.current;
      if (!page || !(event.target instanceof Node) || !page.contains(event.target)) {
        return;
      }
    ...
    window.addEventListener("wheel", handleLyricsPageWheel, { capture: true, passive: false });
    return () => window.removeEventListener("wheel", handleLyricsPageWheel, true);
  }, [handleLyricsBackgroundWheel]);
  ...
  useEffect(() => {
    ...
    const setPendingLyrics = (fallbackOffsetMs = 0): void => {
      // Session memory is only an error fallback. Painting it before the main
      // process confirms the current lyrics can briefly resurrect lyrics that
      // were later cleared or marked as instrumental when this page remounts.
    ...
    if (streamingTarget && isCurrentNeteaseDjRadioTrack === false) {
      ...
      void (async () => {
        ...
        .then(async (streamingLyrics) => {
          if (!streamingLyrics) {
            if (lyricsMemoryKey) {
              forgetRememberedLyricsState(lyricsMemoryKey);
            }
            return;
          }
          if (lyricsRequestRef.current !== requestId) {
          ...
          if (!isRememberableLyricsState(nextLyrics) && lyricsMemoryKey) {
            forgetRememberedLyricsState(lyricsMemoryKey);
          }
          setLyrics(nextLyrics);
          publishCurrentLyricsProvider(null, streamingLyricsProviderDetail(streamingLyrics, title, nextLyrics));
          setError(null);
        ...
        .catch((lyricsError) => {
          ...
          setLyrics(rememberedLyrics ?? emptyLyrics(0));
    ...
    void getLyricsForActiveTrack()
      .then(async (trackLyrics) => {
        ...
        const nextLyrics = trackLyricsToState(trackLyrics);
        if (!isRememberableLyricsState(nextLyrics) && lyricsMemoryKey) {
          forgetRememberedLyricsState(lyricsMemoryKey);
        }
        setLyrics(nextLyrics);
  ...
  const applyCustomLyricsFile = useCallback(
    async (file: File): Promise<void> => {
      ...
      const requestSnapshot = lyricsSnapshotRequest;
      const requestIdentity = lyricsMemoryKey;
      const canApplyCustomLyrics = requestSnapshot
        ? Boolean(lyricsApi?.applyCustomLrcForSnapshot)
        : Boolean(lyricsApi?.applyCustomLrc);
      if (!lyricsApi || !canApplyCustomLyrics || !requestTrackId || !requestIdentity) {
        setError("Desktop bridge unavailable");
        return;
      }
      ...
      const requestId = lyricsRequestRef.current + 1;
      lyricsRequestRef.current = requestId;
      const applyRequestId = customLyricsApplyRequestRef.current + 1;
      customLyricsApplyRequestRef.current = applyRequestId;
      const isCurrentRequest = (): boolean =>
        lyricsRequestRef.current === requestId && lyricsImportIdentityRef.current === requestIdentity;
      ...
      try {
        ...
        const trackLyrics = requestSnapshot
          ? await lyricsApi.applyCustomLrcForSnapshot!(requestSnapshot, lrcText, file.name)
          : await lyricsApi.applyCustomLrc!(requestTrackId, lrcText, file.name);
        if (!isCurrentRequest()) {
          return;
        }
        ...
        setError(null);
        if (lyricsDisplaySettings.lyricsRestartOnApplyEnabled === true) {
          await restartCurrentPlaybackForLyrics(requestTrackId);
        }
        ...
        if (customLyricsApplyRequestRef.current === applyRequestId) {
  ...
  const handleLyricSeekRequest = useCallback((timeMs: number): void => {
    void handleLyricSeek(timeMs);
  }, [handleLyricSeek]);
  ...
  return (
    <div
      ...
      data-background={lyricsBackgroundLayoutMode}
      data-background-source={effectiveLyricsBackgroundMode}
      data-background-overridden={lyricsBackgroundModeOverrideEnabled ? "true" : undefined}
      data-render-budget={lyricsRenderBudget.mode}
      data-render-pressure-reduced={lyricsRenderPressureReduced ? "true" : undefined}
      data-visual-motion={lyricsVisualMotionActive ? "running" : "idle"}
      ...
      data-theme-filter={lyricsDisplaySettings.lyricsThemeFilterEnabled === false ? "false" : "true"}
      data-rose-vinyl-gradient={
        shouldUseRoseVinylStyle
          ? lyricsDisplaySettings.lyricsRoseVinylGradientEnabled
            ? "true"
            : "false"
          : undefined
      }
      data-lyrics-color-mode={lyricsUsesManualColor ? "manual" : "theme"}
      data-empty-state={hasSettledEmptyLyricsState ? "true" : undefined}
      data-empty-state-hidden={shouldHideSettledEmptyLyricsState ? "true" : undefined}
      ...
      data-cinema-particles={shouldRenderCinemaStageParticles ? cinemaStageParticleMode : undefined}
      data-cinema-particles-telemetry={shouldRenderCinemaStageParticles ? musicReactiveScene.visualTelemetryState : undefined}
      data-corner-controls-visible={areCornerControlsVisible ? "true" : "false"}
      data-track-transition={trackTransition ? "true" : undefined}
      data-window-maximized={isWindowMaximized}
      data-file-drop-owner="lyrics"
      style={lyricsPageStyle}
      onDragLeave={handleLyricsDragLeave}
      onDragOver={handleLyricsDragOver}
      onDrop={handleLyricsDrop}
    >
      {shouldUseCoverStageStyle ? null : (
        <LyricsBackdrop
          backgroundMode={effectiveLyricsBackgroundMode}
          coverUrl={lyricsBackgroundCoverUrl}
          isActive={isActive}
          showCover={effectiveLyricsBackgroundMode === "cover"}
          previousCover={
            trackTransition?.previousBackgroundCoverUrl && effectiveLyricsBackgroundMode === "cover"
              ? { id: trackTransition.id, style: trackTransitionStyle }
              : null
          }
        />
      )}
      {isActive && shouldRenderCinemaStageParticles ? <LyricsCinemaStageParticles /> : null}
      {isActive && shouldUseCinemaStageStyle ? <LyricsCinemaStageChrome /> : null}
      {isActive && shouldUseKineticPosterStyle ? <KineticPosterChrome /> : null}
      {shouldUseCoverStageStyle ? (
        <LyricsCoverStageArtwork artworkUrl={headerCoverUrl} isActive={isActive} />
      ) : null}
      {isActive && shouldUseCutBoardStyle ? (
        <LyricsCutBoardChrome
          durationSeconds={displayDurationSeconds}
          layout={cutBoardLayout}
          positionSeconds={Math.floor(lyricsPositionSeconds)}
        />
      ) : null}
      {isActive && shouldUseMusicReactiveVisuals ? (
      ...
      {activeWorkshopLyricsScene && lyricsViewMode === "lyrics" ? (
        <WorkshopLyricsSceneRenderer
          activeScene={activeWorkshopLyricsScene}
          title={title}
          artist={artist}
          album={album}
          coverUrl={isActive ? headerCoverUrl : null}
          ...
          spectrumBands={Array.from(activeAudioStatus?.audioLevels?.visualSpectrum ?? musicReactiveScene.bands)}
          defaultShowRomanization={isLyricsDisplaySettingsReady && lyricsDisplaySettings.lyricsRomanizationEnabled}
          defaultShowTranslation={isLyricsDisplaySettingsReady && lyricsDisplaySettings.lyricsTranslationEnabled}
          defaultWordHighlightEnabled={lyricsDisplaySettings.lyricsWordHighlightEnabled === true}
          highFrequencyUpdatesEnabled={isActive && !lyricsRenderPressureReduced}
          motionEnabled={lyricsVisualMotionActive}
          seekEnabled={lyricsLineSeekEnabled}
          timelineSeekEnabled={lyricsTimelineSeekEnabled}
          seekTimelineOffsetMs={lyricsSeekTimelineOffsetMs}
          canGoNext={queue.canGoNext}
          canGoPrevious={queue.canGoPrevious}
          trackTech={workshopSceneTrackTech}
          volume={workshopSceneVolume.volume}
          volumeInteractive={workshopSceneVolume.interactive}
          onVolumeChange={workshopSceneVolume.commitVolume}
          onSeek={handleLyricSeekRequest}
          onTogglePlay={() => requestPlaybackCommand("playPause")}
          onNext={() => requestPlaybackCommand("nextTrack")}
          onPrevious={() => requestPlaybackCommand("previousTrack")}
          onBack={() => window.dispatchEvent(new Event("app:navigate:lyrics-back"))}
        />
      ) : (
        <>
      <section className="lyrics-left-panel">
        ...
        {lyricsDisplaySettings.lyricsHeaderHidden && !shouldUseKineticPosterStyle && !shouldUseCoverStageStyle && !shouldUseCutBoardStyle ? null : (
          <header className={`lyrics-track-header${shouldUseCoverStageStyle ? " lyrics-cover-stage-track-info" : ""}`}>
            <div
              className="lyrics-track-cover"
              data-empty={!headerCoverUrl}
              title="右键复制封面原图"
            ...
            >
              ...
              <LyricsTrackMarqueeText
                as="h1"
                maxLines={shouldUseCoverStageStyle ? 2 : 1}
                text={title}
                title="右键复制歌名"
                onContextMenu={handleTrackTitleContextMenu}
              />
              {album ? (
                <LyricsTrackMarqueeText
                  as="button"
        ...
        {lyricsDisplaySettings.lyricsEnabled && !shouldHideLyricsInMv ? (
          <LyricsView
            durationMs={displayDurationSeconds * 1000}
            hideEmptyState={shouldHideSettledEmptyLyricsState}
            ...
            wordHighlightEnabled={
              !shouldUseImmersiveCoverStyle &&
              (lyricsDisplaySettings.lyricsWordHighlightEnabled === true ||
                shouldUseCinemaStageStyle ||
                shouldUseKineticPosterStyle ||
                shouldUseCutBoardStyle) &&
              !lyricsRenderPressureReduced
            ...
            wordHighlightProgressMode={
              shouldUseFoldedStyle || shouldUseKineticPosterStyle || shouldUseCutBoardStyle ? "discrete" : "continuous"
            }
      ...
      >
        {isActive && shouldUseRoseVinylStyle && headerCoverUrl ? (
