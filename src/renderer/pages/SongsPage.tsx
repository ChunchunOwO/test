...
import { ArrowUpDown, Check, ChevronDown, FilePlus2, FolderPlus, ListFilter, Play, RotateCw, Search, Trash2, X } from 'lucide-react';
import type { DuplicateTrackIndexSummary, DuplicateTrackMember, EditableTrackTags, LibraryAudioFormatFilter, LibraryFileFormatFilter, LibraryPlaylist, LibraryScanStatus, LibrarySort, LibraryTrack } from '../../shared/types/library';
...
import { toggleSongMultiSort } from '../utils/songSortSelection';
...
const audioFormatFilterOptions: Array<{ value: LibraryAudioFormatFilter; labelKey: TranslationKey }> = [
  { value: 'all', labelKey: 'songs.filter.sampleRate.all' },
...
const fileFormatFilterOptions: Array<{ value: LibraryFileFormatFilter; labelKey: TranslationKey }> = [
  { value: 'all', labelKey: 'songs.filter.audioFormat.all' },
  { value: 'flac', labelKey: 'songs.filter.audioFormat.flac' },
  { value: 'mp3', labelKey: 'songs.filter.audioFormat.mp3' },
  { value: 'wav', labelKey: 'songs.filter.audioFormat.wav' },
  { value: 'aiff', labelKey: 'songs.filter.audioFormat.aiff' },
  { value: 'alac', labelKey: 'songs.filter.audioFormat.alac' },
  { value: 'aac', labelKey: 'songs.filter.audioFormat.aac' },
  { value: 'ogg', labelKey: 'songs.filter.audioFormat.ogg' },
  { value: 'opus', labelKey: 'songs.filter.audioFormat.opus' },
  { value: 'dsd', labelKey: 'songs.filter.audioFormat.dsd' },
  { value: 'other', labelKey: 'songs.filter.audioFormat.other' },
];
...
const readInitialSongsState = (): InitialSongsState => {
  ...
  const fileFormatFilter: LibraryFileFormatFilter = 'all';
...
export const SongsPage = ({ isActive = true }: { isActive?: boolean } = {}): JSX.Element => {
  const { t } = useI18n();
  ...
  const [selectedSorts, setSelectedSorts] = useState<LibrarySort[]>(() => [initialSongsState.sort]);
  ...
  const [fileFormatFilter, setFileFormatFilter] = useState<LibraryFileFormatFilter>(() => initialSongsState.fileFormatFilter);
  ...
  const visibleRemoteMetadataHydrationSeenAtRef = useRef<Record<string, number>>({});
  const visibleRemoteCoverRecoverySeenAtRef = useRef<Record<string, number>>({});
  ...
  const effectiveSorts = useMemo(() => isMultiSortEnabled ? selectedSorts : [sort], [isMultiSortEnabled, selectedSorts, sort]);
  const activeSortLabel = effectiveSorts.length > 1
    ? t('songs.sort.multiCount', { count: effectiveSorts.length })
    : t(sortOptions.find((option) => option.value === effectiveSorts[0])?.labelKey ?? 'songs.sort.default');
  const activeAudioFormatFilterLabel = t(audioFormatFilterOptions.find((option) => option.value === audioFormatFilter)?.labelKey ?? 'songs.filter.sampleRate.all');
  const activeFileFormatFilterLabel = t(fileFormatFilterOptions.find((option) => option.value === fileFormatFilter)?.labelKey ?? 'songs.filter.audioFormat.all');
  const activeFilterLabels = useMemo(() => {
    ...
    if (fileFormatFilter !== 'all') {
      labels.push(activeFileFormatFilterLabel);
    }
    return labels;
  }, [activeAudioFormatFilterLabel, activeFileFormatFilterLabel, audioFormatFilter, fileFormatFilter, showDuplicatesOnly, showOsuOnly, t]);
  ...
  const queueSource = useMemo(
    () => ({
      ...
      sort,
      sorts: isMultiSortEnabled ? selectedSorts : undefined,
      hideDuplicates: effectiveHideDuplicates,
      showDuplicatesOnly,
      showOsuOnly,
      audioFormatFilter: audioFormatFilter === 'all' ? undefined : audioFormatFilter,
      fileFormatFilter: fileFormatFilter === 'all' ? undefined : fileFormatFilter,
    }),
    [audioFormatFilter, effectiveHideDuplicates, fileFormatFilter, isMultiSortEnabled, search, selectedSorts, showDuplicatesOnly, showOsuOnly, sort, sourceMode, t],
  );
  ...
  useEffect(() => {
    ...
    const selected = sortMenuRef.current?.querySelector<HTMLElement>('.sort-option[aria-selected="true"]');
  ...
  const handleFileFormatFilterChange = useCallback(
    (nextFileFormatFilter: LibraryFileFormatFilter): void => {
      setFileFormatFilter(nextFileFormatFilter);
      closeFilterMenu();
      setSelectedTrackIds({});
      clearSongsFirstPageSnapshot();
      clearListMetadataCache();
  ...
  const loadTracks = useCallback(
    ...
    [audioFormatFilter, clearListMetadataCache, effectiveHideDuplicates, fileFormatFilter, isMultiSortEnabled, remoteSourceId, reportSongsError, search, selectedSorts, showDuplicatesOnly, showOsuOnly, sort, sourceMode, t],
  );
  ...
  useEffect(() => {
    ...
    visibleRemoteMetadataHydrationSeenAtRef.current = {};
    visibleRemoteCoverRecoverySeenAtRef.current = {};
    clearVisibleRemoteHydrationTimers();
  ...
  const handlePlayTrack = useCallback(
    ...
    [playTrack, queueSource, t, tracks],
  );
  ...
  useEffect(() => {
    ...
    const trackIds = uniqueIds(visibleRemoteIds)
      .filter((trackId) => now - (visibleRemoteMetadataHydrationSeenAtRef.current[trackId] ?? 0) > 30_000)
      .slice(0, 24);
    ...
    for (const trackId of trackIds) {
      visibleRemoteMetadataHydrationSeenAtRef.current[trackId] = now;
    }
    void remoteApi
      .hydrateVisibleTracks(trackIds, { metadata: true, cover: false, priority: 12 })
      .then((updatedTracks) => {
        if (visibleRemoteHydrationRequestIdRef.current !== requestId) {
          return;
        }
    ...
    (trackIds: string[]): void => {
      ...
      const targetIds = uniqueIds(trackIds)
        ...
        .filter((track): track is LibraryTrack => Boolean(
            ...
            !track.coverThumb &&
            now - (visibleRemoteCoverRecoverySeenAtRef.current[track.id] ?? 0) > 30_000,
        ))
        .map((track) => track.id);
      ...
      for (const trackId of targetIds) {
        visibleRemoteCoverRecoverySeenAtRef.current[trackId] = now;
      }

      void remoteApi
        .hydrateVisibleTracks(targetIds, { metadata: true, cover: false, priority: 18 })
  ...
  useRemoteCoverPreloader({
    active: isActive && sourceMode === 'remote',
    tracks,
    visibleTrackIds,
      ...
      <div className="songs-control-row">
        ...
        <div className="songs-control-actions">
          ...
          <div className="sort-select" ref={filterMenuRef}>
            ...
            {isFilterOpen ? (
              <div className="sort-menu" role="listbox" aria-label={t('songs.filter.menuAria')} data-state="open">
                ...
                {fileFormatFilterOptions.map((option) => (
                  <button
                    key={`file-format-${option.value}`}
                    className="sort-option sort-option--filter"
                    type="button"
                    role="option"
                    aria-selected={fileFormatFilter === option.value}
                    onClick={() => handleFileFormatFilterChange(option.value)}
                  >
                    <span>{t(option.labelKey)}</span>
                    {fileFormatFilter === option.value ? <Check size={14} /> : null}
                  </button>
                ))}
                <div className="sort-menu-divider" role="presentation" />
                <div className="sort-menu-section-title" role="presentation">{t('songs.filter.sampleRate')}</div>
                {audioFormatFilterOptions.map((option) => (
                  <button
                    key={`sample-rate-${option.value}`}
                    className="sort-option sort-option--filter"
                    type="button"
                    role="option"
          ...
          <div className="sort-select" ref={sortMenuRef}>
            ...
            {isSortOpen ? (
              <div className="sort-menu sort-menu--songs" role="dialog" aria-label={t('songs.sort.menuAria')} data-state={isSortClosing ? 'closing' : 'open'}>
                <div className="sort-multi-toggle">
                  <span className="sort-multi-toggle-copy">
                    <strong>{t('songs.sort.multi.label')}</strong>
                    <small>{t('songs.sort.multi.hint')}</small>
                  </span>
                  <button
                    className="sort-multi-switch"
                    type="button"
                    role="switch"
                    aria-checked={isMultiSortEnabled}
                    aria-label={t('songs.sort.multi.label')}
                    onClick={() => setIsMultiSortEnabled((current) => !current)}
                  >
                ...
                <div
                  ...
                  role="listbox"
                  aria-label={t('songs.sort.menuAria')}
                  aria-multiselectable={isMultiSortEnabled}
                >
                  {sortGroups.map((group, groupIndex) => (
                    <Fragment key={group.labelKey}>
                      ...
                      {group.options.map((option) => {
                        const priority = effectiveSorts.indexOf(option.value);
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (!isMultiSortEnabled) {
                                ...
                                setSelectedSorts([option.value]);
                                closeSortMenu();
                                return;
                              }

                              const nextSorts = toggleSongMultiSort(selectedSorts, option.value);
                              setSelectedSorts(nextSorts);
                              setSort(nextSorts[0] ?? 'default');
                            }}
                          >
                            <span>{t(option.labelKey)}</span>
                            {isSelected ? (
                              isMultiSortEnabled
                                ? <span className="sort-option-priority" aria-label={String(priority + 1)}>{priority + 1}</span>
                                : <Check size={14} />
                            ) : null}
                          </button>
                        );
