...
const stats = (): PlaybackStatsDashboard => ({
  ...
  insights: {
    comparison: {
      current: { playCount: 3, playedSeconds: 540, uniqueTracks: 1 },
      previous: { playCount: 2, playedSeconds: 360, uniqueTracks: 1 },
    },
    sessions: {
      sessionCount: 2,
      averagePlayedSeconds: 270,
      longestPlayedSeconds: 360,
      averageTrackCount: 1.5,
      activeDays: 1,
      currentStreakDays: 1,
      longestStreakDays: 4,
    },
...
describe('HistoryPage', () => {
  it('renders period comparison, listening sessions, and discovery insights', async () => {
    installLibraryMock();
    renderHistoryPage();

    expect(await screen.findByText('Listening insights')).toBeTruthy();
    expect(screen.getByText('Compared with previous period')).toBeTruthy();
    expect(screen.getByText('Listening sessions')).toBeTruthy();
    expect(screen.getByText('Discovery and return')).toBeTruthy();
  ...
  it('persists the first history page after refresh', async () => {
    ...
    await waitFor(() => {
      const cached = JSON.parse(window.localStorage.getItem('echo.history-page-cache.v1') ?? '{}') as {
