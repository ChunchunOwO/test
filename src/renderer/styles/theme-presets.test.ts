...
describe('theme presets stylesheet', () => {
  it('keeps supporting preset text readable on its strongest panel surface', () => {
    const css = [
      readFileSync('src/renderer/styles/theme-presets.css', 'utf8'),
      readFileSync('src/renderer/styles/theme-presets-fable.css', 'utf8'),
    ].join('\n');
    const blocks = css.matchAll(
      /html\[data-theme="(light|dark)"\]\[data-theme-preset="([^"]+)"\]\s*\{([\s\S]*?)\n\}/g,
    );
    ...
    for (const [, mode, preset, body] of blocks) {
      const variables = new Map(
        Array.from(body.matchAll(/--([\w-]+):\s*([^;]+);/g), (match) => [match[1], match[2]]),
      );
      const panel = variables.get('preset-panel-rgb')?.trim().split(/\s+/).map(Number) as [number, number, number] | undefined;
      const muted = variables.get('preset-muted');
      const subtle = variables.get('preset-subtle');
      if (!panel || !muted || !subtle) {
        continue;
      }

      expect(contrast(toRgb(muted), panel), `${mode}/${preset} muted text`).toBeGreaterThanOrEqual(4.5);
      expect(contrast(toRgb(subtle), panel), `${mode}/${preset} subtle text`).toBeGreaterThanOrEqual(3);
      checkedPresets += 1;
    }

    expect(checkedPresets).toBeGreaterThanOrEqual(60);
  });
  ...
  it('clears the startup color from every renderer root while acrylic is active', () => {
    const css = readFileSync('src/renderer/styles/theme.css', 'utf8').replace(/\r\n/g, '\n');

    expect(css).toContain(
      'html:has(.app-shell--acrylic),\nbody:has(.app-shell--acrylic),\n#root:has(.app-shell--acrylic) {\n  background: transparent;',
    );
  });
  ...
  it('keeps classic lyrics backdrops from flattening preset gradients', () => {
    const lyricsCss = readFileSync('src/renderer/styles/lyrics.css', 'utf8');
    const presetCss = readFileSync('src/renderer/styles/theme-presets.css', 'utf8');
    const classicOnlyGuard =
      ':not(:is([data-theme-custom="true"], [data-theme-preset]:not([data-theme-preset="classic"])))';

    expect(lyricsCss).toContain(
      `html[data-theme="light"]${classicOnlyGuard} .lyrics-page:has(.lyrics-mv-panel[data-mv-enabled="false"]) .lyrics-backdrop`,
    );
    expect(lyricsCss).toContain(
      `html[data-theme="dark"]${classicOnlyGuard} .lyrics-page:has(.lyrics-mv-panel[data-mv-enabled="false"]) .lyrics-backdrop`,
    );
    expect(lyricsCss).toContain(
      'radial-gradient(ellipse at 78% -4%, rgba(100, 181, 255, 0.17), transparent 39%)',
    );
    expect(presetCss).toMatch(
      /\.lyrics-page:not\(\[data-theme-filter="false"\]\)\[data-background="theme"\] \.lyrics-backdrop \{\r?\n {2}background:\r?\n {4}radial-gradient\(ellipse at 8% -6%, rgb\(var\(--preset-panel-rgb\) \/ 0\.44\), transparent 34%\),[\s\S]*?var\(--preset-app-bg-3\) 100%\);/,
    );
  });
  ...
  it('keeps FINAL metric tiles aligned and adapts the lyrics page', () => {
    ...
    expect(css).toContain('html[data-theme-preset="FINAL"] .page-surface:has(.lyrics-page:not([data-theme-filter="false"]))');
    expect(css).toContain('html[data-theme-preset="FINAL"] .lyrics-page:not([data-theme-filter="false"]) .lyrics-backdrop');
