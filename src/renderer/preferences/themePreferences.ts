...
const storageKey = 'echo:appearance-theme';
const presetStorageKey = 'echo:appearance-theme-preset';
const presetOverridesStorageKey = 'echo:appearance-theme-preset-overrides';
const customThemesStorageKey = 'echo:appearance-custom-themes';
const customThemeIdStorageKey = 'echo:appearance-theme-custom-id';
...
const applyThemeToneOverride = (root: HTMLElement, tone: EffectiveTheme, override: AppThemeToneOverride): void => {
  ...
  root.style.setProperty('--theme-scrollbar-thumb', rgb(borderRgb, 0.42 + glassAlpha * 0.08));
  root.style.setProperty('--theme-scrollbar-thumb-hover', rgb(borderRgb, 0.62 + glassAlpha * 0.08));
...
const runThemeTransition = (callback: () => void, options: ThemeApplyOptions = {}): void => {
  if (!options.animate || prefersReducedMotion() || document.querySelector('.app-shell[data-low-spec-mode="true"]')) {
    callback();
    return;
  }

  const root = document.documentElement;
  const transitionId = ++activeThemeTransition;
  const startViewTransition = (document as Document & {
    startViewTransition?: (update: () => void) => { finished: Promise<unknown> };
  }).startViewTransition?.bind(document);
  const clearTransitionState = (): void => {
    if (activeThemeTransition === transitionId) {
  ...
  callback();
  window.setTimeout(clearTransitionState, themeTransitionMs);
};
...
export const watchThemeSettings = (loadSettings: () => Promise<Partial<AppSettings> | null | undefined>): (() => void) => {
  ...
  const getThemeSettingsSignature = (settings: Partial<AppSettings> | null): string => JSON.stringify({
    appearanceTheme: settings?.appearanceTheme,
    appearanceThemeScheduleEnabled: settings?.appearanceThemeScheduleEnabled,
    appearanceThemeScheduleDarkAt: settings?.appearanceThemeScheduleDarkAt,
    appearanceThemeScheduleLightAt: settings?.appearanceThemeScheduleLightAt,
    appearanceThemePreset: settings?.appearanceThemePreset,
    appearanceThemePresetOverrides: settings?.appearanceThemePresetOverrides,
    appearanceCustomThemes: settings?.appearanceCustomThemes,
    appearanceThemeCustomId: settings?.appearanceThemeCustomId,
    finalThemeUnlockVersion: settings?.finalThemeUnlockVersion,
  });
  ...
  const handleSettingsChanged = (event: Event): void => {
    ...
    const previousThemeSignature = getThemeSettingsSignature(latestSettings);
    latestSettings = { ...(latestSettings ?? {}), ...patch };
    if (getThemeSettingsSignature(latestSettings) === previousThemeSignature) {
      return;
    }
