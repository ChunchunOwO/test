...
describe('theme preferences', () => {
  ...
  it('applies and caches independent theme presets', () => {
    ...
    expect(readThemePreset({ finalThemeUnlocked: true })).toBe('classic');
    expect(document.documentElement.dataset.themePreset).toBe('classic');

    updateThemePreset('nyanCat', { finalThemeUnlocked: true });
    ...
    const customThemes = [
      {
        id: 'theme-safe',
        name: 'Safe Theme',
        basePreset: 'echoTwilight' as const,
        light: {
          appBg: '#101820',
          titlebar: '#203040',
          accent: '#ff66aa',
  ...
  it('uses one snapshot transition for an interactive theme change', async () => {
    ...
    expect(document.documentElement.dataset.themeTransition).toBe('view');
  ...
  it('does not start a whole-page theme transition for unrelated settings changes', async () => {
    Object.defineProperty(window, 'matchMedia', {
      configurable: true,
      value: vi.fn((query: string) => ({
        matches: false,
    ...
    const stopWatching = watchThemeSettings(async () => settings);
    ...
    window.dispatchEvent(new CustomEvent('settings:changed', {
      detail: { ...settings, playerVolume: 0.6 },
    }));
    expect(startViewTransition).not.toHaveBeenCalled();

    window.dispatchEvent(new CustomEvent('settings:changed', {
      detail: { ...settings, appearanceTheme: 'light' },
    }));
    expect(startViewTransition).toHaveBeenCalledOnce();
