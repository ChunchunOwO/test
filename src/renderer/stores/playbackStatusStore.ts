...
import { getPlaybackChromeRevision, getPlaybackUiRevision } from './playbackStatusSelectors';
import {
...
export const subscribeSharedPlaybackStatus = subscribe;
export const getSharedPlaybackStatusSnapshot = getSnapshot;

const getPlaybackIsPlayingSnapshot = (): boolean =>
  snapshot.audioStatus?.state === 'playing' || snapshot.playbackStatus?.state === 'playing';

export const useSharedPlaybackStatus = (): PlaybackStatusSnapshot => {
  return useSyncExternalStore(subscribe, getSnapshot, getSnapshot);
};

const useRevisionedPlaybackSnapshot = (
  getRevision: (statusSnapshot: PlaybackStatusSnapshot) => string,
): PlaybackStatusSnapshot => {
  const getRevisionRef = useRef(getRevision);
  getRevisionRef.current = getRevision;
  const cacheRef = useRef<{ source: PlaybackStatusSnapshot; revision: string } | null>(null);

  const getSelected = useCallback((): PlaybackStatusSnapshot => {
    ...
    const revision = getRevisionRef.current(source);
    if (cache && cache.revision === revision) {
      return cache.source;
    }

    cacheRef.current = { source, revision };
    return source;
  }, []);
...
export const useSharedPlaybackStatusSelector = <T,>(
  selector: (statusSnapshot: PlaybackStatusSnapshot) => T,
  ...
  const cacheRef = useRef<{ source: PlaybackStatusSnapshot; value: T } | null>(null);
  ...
  const getSelected = useCallback((): T => {
    ...
    if (cache && isEqualRef.current(cache.value, nextValue)) {
      cache.source = source;
      return cache.value;
    }

    const retained = (nextValue === source ? { ...source } : nextValue) as T;
    cacheRef.current = { source, value: retained };
    return retained;
  }, []);
...
export const useSharedPlaybackStatusForUi = (): PlaybackStatusSnapshot =>
  useRevisionedPlaybackSnapshot(getPlaybackUiRevision);

export const useSharedPlaybackStatusForChrome = (): PlaybackStatusSnapshot =>
  useRevisionedPlaybackSnapshot(getPlaybackChromeRevision);

export const useSharedPlaybackIsPlaying = (): boolean =>
  useSyncExternalStore(subscribe, getPlaybackIsPlayingSnapshot, getPlaybackIsPlayingSnapshot);
