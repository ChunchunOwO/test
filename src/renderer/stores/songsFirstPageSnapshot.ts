...
const snapshotStorageKey = 'echo.songs.first-page-snapshot';
const diagnosticsStorageKey = 'echo.songs.startup-load';
const snapshotVersion = 1;
const maxSnapshotItems = 100;

type SnapshotQuery = Pick<LibraryPageQuery, 'search' | 'sort' | 'sorts' | 'hideDuplicates' | 'showDuplicatesOnly' | 'showOsuOnly' | 'duplicateMode' | 'audioFormatFilter' | 'fileFormatFilter'> & {
  pageSize: number;
...
export const createSongsFirstPageSnapshotQueryKey = (query: SnapshotQuery): string =>
  JSON.stringify({
    ...
    fileFormatFilter: query.fileFormatFilter ?? 'all',
