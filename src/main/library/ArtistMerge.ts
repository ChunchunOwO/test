...
const standardSuffixPatterns: RegExp[] = [
  /\s*(?:\+|\uff0b)\s*81$/iu,
  /\s*(?:-|–|—|:|\|)\s*(?:official|topic|vevo)$/iu,
];
const compactSlashBandNamePattern = /^[A-Z0-9]{1,3}\/[A-Z0-9]{1,3}$/u;
...
export const splitArtistCreditParts = (value: string): string[] =>
  compactSlashBandNamePattern.test(value.trim()) ? [value.trim()] : value.split(artistNameSeparatorPattern);
...
export const findArtistMergeKey = (
  ...
  for (const existing of existingArtists) {
    ...
    const threshold = overlap ? 0.93 : 0.96;
