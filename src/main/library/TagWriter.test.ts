...
describe('writeEmbeddedTrackTags', () => {
  ...
  it('runs embedded tag writes in a worker', async () => {
    ...
    expect(workerMockState.workers[0].source).toContain('totalTracks: tags.totalTracks');
    expect(workerMockState.workers[0].source).toContain('bpm: tags.bpm === undefined ? undefined : tags.bpm ?? 0');
    expect(workerMockState.workers[0].source).toContain("comment: tags.comment === undefined ? undefined : tags.comment ?? ''");
