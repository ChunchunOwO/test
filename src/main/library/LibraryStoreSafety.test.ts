...
describe('LibraryStore track metadata safety', () => {
  ...
  it('keeps the local import timestamp on direct track lookups used by achievements', () => {
    const store = makeStore();
    const folder = store.addFolder('D:\\Music');
    const path = 'D:\\Music\\Imported.flac';
    store.upsertTrack(baseTrack(folder.id, path, { createdAt: '2025-06-01T00:00:00.000Z' }));

    expect(store.getTrack('track-1')?.createdAt).toBe('2025-06-01T00:00:00.000Z');
    expect(store.getTrackByPath(path)?.createdAt).toBe('2025-06-01T00:00:00.000Z');
    expect(store.getActiveTracks()[0]?.createdAt).toBe('2025-06-01T00:00:00.000Z');
  });
  ...
  it('rebuilds the same album index cooperatively as the synchronous path', async () => {
    const store = makeStore();
    const folder = store.addFolder('D:\\Music');
    ...
    store.refreshAlbums(albumService, '2026-01-01T00:00:00.000Z');
    const syncTitles = store.getAlbums({ pageSize: 20 }).items.map((album) => album.title).sort();

    await store.refreshAlbumsCooperatively(albumService, '2026-01-02T00:00:00.000Z', {}, 1);
    const cooperativeTitles = store.getAlbums({ pageSize: 20 }).items.map((album) => album.title).sort();

    expect(syncTitles).toEqual(['Alpha', 'Beta']);
    expect(cooperativeTitles).toEqual(syncTitles);
  });
