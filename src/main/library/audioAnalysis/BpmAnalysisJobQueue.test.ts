...
const makeTempAudioPath = (): string => {
  const root = mkdtempSync(join(tmpdir(), 'echo-bpm-job-'));
  tempRoots.push(root);
...
describe('BpmAnalysisJobQueue', () => {
  ...
  it('measures one track without updating the library or writing tags', async () => {
    ...
    const result = await queue.measureTrack(track.id);
    ...
    expect(analyzer.analyze).toHaveBeenCalledWith(track.path, track.duration);
    expect(store.markTrackAnalyzing).not.toHaveBeenCalled();
    expect(store.updateTrackBpmAnalysis).not.toHaveBeenCalled();
