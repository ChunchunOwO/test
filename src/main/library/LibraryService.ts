...
import { getSharedLibraryDatabasePath } from '../app/editionDataSync';
...
import { GenreReadModel } from './genre/GenreReadModel';
...
import { createWorkerBackedLibraryScanWorkers, type RemoteAlbumReader } from './workers/WorkerBackedLibraryScan';
...
const artistInsightsLocalCacheTtlMs = 5_000;
...
export class LibraryService {
  ...
  private readonly artistInsightsLocalCache = new Map<string, { at: number; value: ArtistInsights }>();
  ...
  private readonly genreReadModel: GenreReadModel;
  ...
  private remoteAlbumWorkerFallbackWarned = false;
  ...
  constructor(
    ...
    private readonly remoteAlbumReader: RemoteAlbumReader | null = null,
  ) {
    ...
    this.genreReadModel = new GenreReadModel(this.database);
  ...
  async getAlbumsResponsive(query?: LibraryPageQuery): Promise<LibraryPage<LibraryAlbum>> {
    if (query?.sourceProvider !== 'remote' || !this.remoteAlbumReader) {
      return this.getAlbums(query);
    }

    const settings = this.readAppSettings();
    try {
      return await this.remoteAlbumReader.getAlbums({
      ...
      if (!this.remoteAlbumWorkerFallbackWarned) {
        this.remoteAlbumWorkerFallbackWarned = true;
        console.warn(`[library-worker] remote albums worker failed; falling back to the main process: ${error instanceof Error ? error.message : String(error)}`);
      }
      return this.getAlbums(query);
    }
  }
  ...
  async getArtistInsights(artistId: string, options?: ArtistInsightsOptions): Promise<ArtistInsights> {
    const localCacheKey = playbackSafeCacheKey({ artistId, limit: options?.limit });
    const cachedLocalInsights = this.artistInsightsLocalCache.get(localCacheKey);
    const localInsights = cachedLocalInsights && Date.now() - cachedLocalInsights.at < artistInsightsLocalCacheTtlMs
      ? cachedLocalInsights.value
      : isAlbumArtistId(artistId)
        ? this.albumArtistReadModel.getArtistInsights(artistId, options)
        : this.store.getArtistInsights(artistId, options);
    if (localInsights !== cachedLocalInsights?.value) {
      this.artistInsightsLocalCache.delete(localCacheKey);
      this.artistInsightsLocalCache.set(localCacheKey, { at: Date.now(), value: localInsights });
      trimMapToLimit(this.artistInsightsLocalCache, playbackSafeReadCacheLimit);
    }
  ...
  getPlaybackStatsDashboard(query?: PlaybackHistoryQuery): PlaybackStatsDashboard {
    ...
    const result = query?.search?.trim()
      ? dashboard
      : { ...dashboard, insights: this.store.getPlaybackStatsInsights(query) };
    this.rememberPlaybackStatsDashboard(query, result);
    return result;
  }

  getSteamAchievementHistoryStats() {
    return this.store.getSteamAchievementHistoryStats();
  }

  recordSteamAchievementPlaybackFact(input: Parameters<LibraryStore['recordSteamAchievementPlaybackFact']>[0]): void {
    this.store.recordSteamAchievementPlaybackFact(input);
  }

  getSteamAchievementPlaybackFacts(query?: Parameters<LibraryStore['getSteamAchievementPlaybackFacts']>[0]) {
    return this.store.getSteamAchievementPlaybackFacts(query);
  }

  getSteamLeaderboardHistoryStats() {
    const history = this.store.getSteamAchievementHistoryStats();
    const insights = this.store.getPlaybackStatsInsights({ mediaType: 'local' });
    return {
      ...history,
      listeningSessionCount: insights.sessions.sessionCount,
      longestListeningSessionSeconds: insights.sessions.longestPlayedSeconds,
      rediscoveredTrackCount: insights.discovery.rediscoveredTrackCount,
    };
  }
  ...
  refreshAlbumGrouping(request: LibraryAlbumGroupingRefreshRequest = {}): LibrarySummary {
    ...
    try {
      const summary = this.store.transaction(() => {
        this.store.refreshAlbums(this.albumService, new Date().toISOString(), this.albumRefreshOptions(request));
        this.store.refreshArtists({ artistMergeStrategy: request.artistMergeStrategy });
        return this.store.getSummary();
      });
  ...
  refreshAlbumGroupingPlaybackSafe(request: LibraryAlbumGroupingRefreshRequest = {}): Promise<LibraryAlbumGroupingRefreshResult> {
    if (this.hasRunningJobs()) {
      throw new Error('Cannot refresh album grouping while a library scan is running.');
    }
    ...
    return runNonCriticalMainWork<LibraryAlbumGroupingRefreshResult>({
      ...
      work: () => ({
        ...
        summary: this.refreshAlbumGrouping(request),
      }),
      fallback: () => {
        this.groupingRefreshDelayedForPlaybackCount += 1;
        if (request.albumMergeStrategy === undefined && request.artistMergeStrategy === undefined) {
  ...
  async loadEmbeddedTrackTags(trackId: string): Promise<EmbeddedTrackTagsLoadResult> {
    ...
    return {
      tags: {
        ...
        album: metadata.fields.album,
        albumArtist: metadata.fields.albumArtist,
        composer: metadata.fields.composer ?? null,
        trackNo: metadata.fields.trackNo,
        totalTracks: metadata.fields.totalTracks ?? null,
        discNo: metadata.fields.discNo,
        totalDiscs: metadata.fields.totalDiscs ?? null,
        year: metadata.fields.year,
        genre: metadata.fields.genre,
        bpm: metadata.fields.bpm ?? null,
        comment: metadata.fields.comment ?? null,
      },
      coverId,
  ...
  async updateTrackTags(request: LibraryTrackTagUpdateRequest): Promise<LibraryTrack> {
    ...
    if (tags.composer !== undefined) fieldSources.composer = 'manual';
    if (tags.totalTracks !== undefined) fieldSources.totalTracks = 'manual';
    if (tags.totalDiscs !== undefined) fieldSources.totalDiscs = 'manual';
    if (tags.bpm !== undefined) fieldSources.bpm = 'manual';
    if (tags.comment !== undefined) fieldSources.comment = 'manual';
  ...
  async updateAlbumTags(request: LibraryAlbumTagUpdateRequest): Promise<LibraryAlbum> {
    ...
    for (const track of tracks) {
      ...
      const fieldSources = {
        ...
        albumArtist: 'manual',
        year: 'manual',
        ...(tags.genre !== undefined ? { genre: 'manual' } : {}),
      };
      ...
      const tagUpdate = {
        ...
        discNo: track.discNo,
        year: tags.year,
        genre: tags.genre !== undefined ? tags.genre : track.genre,
        sizeBytes: fileStat.size,
        mtimeMs: fileStat.mtimeMs,
        fieldSources,
      };
  ...
  clearCache(): LibraryCacheClearResult {
    ...
    if (!isCoverCacheDirectorySafeToClear(this.databasePath, this.coverCacheDir)) {
      throw new Error('Cannot clear an external cover cache directory that is not marked as owned by ECHO.');
    }
  ...
  measureTrackBpm(trackId: string): Promise<BpmAnalysisResult> {
    ...
    return this.bpmAnalysisJobQueue.measureTrack(trackId);
  }
  ...
  private albumRefreshOptions(request: LibraryAlbumGroupingRefreshRequest = {}): { albumMergeStrategy: AlbumMergeStrategy } {
    return { albumMergeStrategy: request.albumMergeStrategy ?? this.readAppSettings().albumMergeStrategy };
  }
  ...
  private async runScheduledGroupingRefresh(): Promise<void> {
    ...
    const startedAtMs = Date.now();
    try {
      await runMainBackgroundTask('library-scan:grouping_albums', async () => {
        await this.store.refreshAlbumsCooperatively(this.albumService, undefined, this.albumRefreshOptions());
        await this.store.refreshArtistsCooperatively();
      });
      this.artistsDirty = false;
      this.lastGroupingRefreshDurationMs = Date.now() - startedAtMs;
      this.lastGroupingRefreshAt = new Date().toISOString();
...
export const createLibraryService = (
  ...
  return new LibraryService(
    ...
    workerBackedScanWorkers?.remoteAlbumReader ?? null,
  );
};
...
const closeDefaultDatabaseUsersBeforeRecovery = async (): Promise<void> => {
  ...
  const [lyrics, remote] = await Promise.all([
...
export const getLibraryService = (): LibraryService => {
  ...
  if (!defaultLibraryService) {
    const electronApp = (electron as unknown as { app?: { getPath: (name: 'appData' | 'userData') => string } }).app;

    if (!electronApp) {
      throw new Error('Electron app module is unavailable outside the Electron main process');
    }

    const databasePath = getSharedLibraryDatabasePath(electronApp);
    const manager = getLibraryDatabaseManager();
...
const normalizeEditableTags = (tags: EditableTrackTags, previous: LibraryTrack): EditableTrackTags => {
  ...
  const normalized: EditableTrackTags = {
    title,
    artist,
  ...
  if (tags.composer !== undefined) normalized.composer = cleanNullableText(tags.composer);
  if (tags.totalTracks !== undefined) normalized.totalTracks = cleanNullableNumber(tags.totalTracks);
  if (tags.totalDiscs !== undefined) normalized.totalDiscs = cleanNullableNumber(tags.totalDiscs);
  if (tags.bpm !== undefined) normalized.bpm = typeof tags.bpm === 'number' && Number.isFinite(tags.bpm) && tags.bpm > 0 ? tags.bpm : null;
  if (tags.comment !== undefined) normalized.comment = cleanNullableText(tags.comment);
...
const normalizeEditableAlbumTags = (tags: EditableAlbumTags, previous: LibraryAlbum): EditableAlbumTags => ({
  ...
  year: cleanNullableNumber(tags.year),
  genre: tags.genre === undefined ? undefined : cleanNullableText(tags.genre),
});
