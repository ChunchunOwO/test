...
import {
  isMp4IlstFallbackPath,
  readMp4IlstMetadataFallback,
  type Mp4IlstMetadataFallback,
} from './Mp4IlstMetadataFallback';
...
const textMetadataFields = new Set<keyof MetadataFields>(['title', 'artist', 'album', 'albumArtist', 'composer', 'genre', 'comment']);
...
const commonTextMetadataKeys = new Set(['title', 'artist', 'artists', 'album', 'albumartist', 'composer', 'comment', 'genre', 'date']);
...
const containsUnsafeMetadataText = (value: unknown): boolean => {
  ...
  if (Array.isArray(value)) {
    return value.some(containsUnsafeMetadataText);
  }

  return Boolean(value && typeof value === 'object' && Object.values(value).some(containsUnsafeMetadataText));
};
...
const isTextMetadataField = (field: keyof MetadataFields): field is 'title' | 'artist' | 'album' | 'albumArtist' | 'composer' | 'genre' | 'comment' =>
...
const cleanCommentList = (value: unknown): string | null => {
  if (!Array.isArray(value)) {
    return cleanText(value);
  }

  const cleaned = value
    .map((item) => cleanText(item && typeof item === 'object' && 'text' in item ? item.text : item))
    .filter((item): item is string => Boolean(item));
  const unique = [...new Set(cleaned)];
...
const shouldInspectNativeTextTag = (common: IAudioMetadata['common'], rawId: string): boolean => {
  ...
  if (id === 'composer' || id === 'tcom') {
    return !cleanJoinedTextList(common.composer);
  }
  if (id === 'comment' || id === 'comments' || id === 'comm') {
    return !cleanCommentList(common.comment);
  }
  if (id === 'genre' || id === 'tcon') {
  ...
  try {
    ...
    return {
      fields: {
        ...
        composer: skipTagMetadata ? null : firstText(tagValue(tags, ['composer'])),
        trackNo: skipTagMetadata ? null : firstNumber(tagValue(tags, ['track', 'trackNumber', 'tracknumber'])),
        totalTracks: skipTagMetadata ? null : firstNumber(tagValue(tags, ['totalTracks', 'trackTotal', 'tracktotal'])),
        discNo: skipTagMetadata ? null : firstNumber(tagValue(tags, ['discNumber', 'discnumber', 'disc', 'disk'])),
        totalDiscs: skipTagMetadata ? null : firstNumber(tagValue(tags, ['totalDiscs', 'discTotal', 'disctotal'])),
        ...
        comment: skipTagMetadata ? null : firstText(tagValue(tags, ['comment', 'comments'])),
        duration,
...
export class TsMetadataReader implements MetadataReader {
  ...
  private async readInternal(filePath: string, readCover: boolean): Promise<MetadataResult> {
    ...
    try {
      ...
      if (!cueTrack) {
        ...
        const normalized = await this.normalizeWithFallback(filePath, metadataPath, metadata, waveInfoTags);
        return this.applyMp4IlstFallbackIfNeeded(normalized, metadataPath, readCover);
      }
      ...
      const mp4Fallback = await readMp4IlstMetadataFallback(filePath, readCover);
      if (mp4Fallback) {
        result = this.applyMp4IlstFallback(result, mp4Fallback);
      }
      ...
      const recovered =
        merged.embeddedMetadataStatus === 'present' ||
  ...
  private async applyMp4IlstFallbackIfNeeded(
    ...
    const mp4TechnicalFallbackFields = result.fields.codec === 'ALAC'
      ? (['duration', 'codec', 'sampleRate', 'bitDepth'] as const)
      : (['duration', 'codec', 'sampleRate'] as const);
    const missingTechnicalMetadata = mp4TechnicalFallbackFields.some((field) =>
      replaceableTechnicalSources.has(result.fieldSources[field] ?? 'unknown'),
    );
    if ((!readCover || result.embeddedCover) && !missingCoreMetadata && !missingTechnicalMetadata) {
      return result;
    }
    const fallback = await readMp4IlstMetadataFallback(filePath, readCover);
    return fallback ? this.applyMp4IlstFallback(result, fallback) : result;
  }

  private applyMp4IlstFallback(result: MetadataResult, fallback: Mp4IlstMetadataFallback): MetadataResult {
    ...
    const applyEmbedded = <Key extends keyof MetadataFields>(field: Key): void => {
      const value = fallback.fields[field];
    ...
    const applyTechnical = <Key extends keyof MetadataFields>(field: Key): void => {
      const value = fallback.fields[field];
      if (value === null || value === undefined || value === '' || value === 0) {
        return;
      }
      const mp4AlacCanOverride =
        fallback.fields.codec === 'ALAC' &&
        fields.codec === 'ALAC' &&
        (field === 'codec' || field === 'sampleRate' || field === 'bitDepth');
      if (!mp4AlacCanOverride && !replaceableTechnicalSources.has(fieldSources[field] ?? 'unknown')) {
        return;
      }
      fields[field] = value;
    ...
    for (const field of ['duration', 'codec', 'sampleRate', 'bitDepth', 'bitrate'] as const) {
      applyTechnical(field);
    }

    const embeddedCover = result.embeddedCover ?? fallback.embeddedCover;
    return {
      ...result,
      fields,
      fieldSources,
  ...
  private applyTagLibFallback(result: MetadataResult, tagLibMetadata: TagLibFallbackMetadata, filePath: string): MetadataResult {
    ...
    applyEmbedded('composer', tagLibMetadata.fields.composer);
    applyEmbedded('trackNo', tagLibMetadata.fields.trackNo);
    applyEmbedded('totalTracks', tagLibMetadata.fields.totalTracks);
    applyEmbedded('discNo', tagLibMetadata.fields.discNo);
    applyEmbedded('totalDiscs', tagLibMetadata.fields.totalDiscs);
    applyEmbedded('year', tagLibMetadata.fields.year);
    applyEmbedded('genre', tagLibMetadata.fields.genre);
    applyEmbedded('comment', tagLibMetadata.fields.comment);
  ...
  normalize(filePath: string, metadata: IAudioMetadata, waveInfoTags: WaveInfoTags = {}): MetadataResult {
    ...
    const embeddedComposer = skipEmbeddedMetadata
      ? null
      : (cleanJoinedTextList(common.composer) ?? firstNativeText(metadata, ['COMPOSER', 'TCOM']));
    const embeddedComment = skipEmbeddedMetadata
      ? null
      : (cleanCommentList(common.comment) ?? firstNativeText(metadata, ['COMMENT', 'COMMENTS', 'COMM']));
    ...
    const composer = embeddedComposer;
    fieldSources.composer = composer ? 'embedded' : 'unknown';
    const trackNo = pickNumber('trackNo', embeddedTrackNo);
    const totalTracks = pickNumber('totalTracks', skipEmbeddedMetadata ? null : (numberOrNull(common.track?.of) ?? firstNativeNumber(metadata, ['TRACKTOTAL', 'TOTALTRACKS'])));
    ...
    const totalDiscs = pickNumber('totalDiscs', skipEmbeddedMetadata ? null : (numberOrNull(common.disk?.of) ?? firstNativeNumber(metadata, ['DISCTOTAL', 'TOTALDISCS'])));
    ...
    fieldSources.comment = comment ? 'embedded' : 'unknown';
