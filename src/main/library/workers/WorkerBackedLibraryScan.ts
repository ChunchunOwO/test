...
class LibraryScanWorkerPool {
  ...
  constructor(options: WorkerBackedLibraryScanOptions = {}) {
    ...
    this.idleTimeoutMs = Math.max(1_000, Math.round(options.idleTimeoutMs ?? defaultIdleTimeoutMs));
  ...
  run(request: Omit<Extract<LibraryScanWorkerRequest, { type: 'library:remote-albums' }>, 'requestId'>): Promise<LibraryPage<LibraryAlbum>>;
  ...
  private scheduleIdleShutdown(): void {
    ...
    if (
      ...
      this.queue.length > 0 ||
      this.workers.some((slot) => slot.currentTask !== null)
    ) {
      return;
    }
    ...
    this.idleTimer = setTimeout(() => {
      ...
      const idleWorkers = this.workers.splice(0);
      this.started = false;
      for (const slot of idleWorkers) {
        slot.retired = true;
...
type RemoteAlbumRequest = Omit<
  Extract<LibraryScanWorkerRequest, { type: 'library:remote-albums' }>,
  'requestId' | 'type'
>;

export type RemoteAlbumReader = {
  getAlbums(request: RemoteAlbumRequest): Promise<LibraryPage<LibraryAlbum>>;
};

export class WorkerBackedRemoteAlbumReader implements RemoteAlbumReader {
  constructor(private readonly pool: LibraryScanWorkerPool) {}
...
export type WorkerBackedLibraryScanWorkers = {
  ...
  remoteAlbumReader: RemoteAlbumReader;
  close: () => void;
};
...
export const createWorkerBackedLibraryScanWorkers = (
  ...
  const remoteAlbumPool = new LibraryScanWorkerPool({
    ...options,
    workerCount: 1,
    workerUrl: options.searchWorkerUrl ?? defaultSearchWorkerUrl,
  });
  ...
  return {
    ...
    remoteAlbumReader: new WorkerBackedRemoteAlbumReader(remoteAlbumPool),
    close: () => {
      pool.close();
      identityPool.close();
      searchPool.close();
      remoteAlbumPool.close();
    },
  };
};
