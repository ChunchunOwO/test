...
describe('WorkerBackedLibraryScan', () => {
  ...
  it('releases idle local scan workers and recreates them for the next task', async () => {
    const workers: FakeWorker[] = [];
    const scanWorkers = createWorkerBackedLibraryScanWorkers({
      workerCount: 1,
      idleTimeoutMs: 1_000,
    ...
    try {
      await scanWorkers.metadataReader.read('D:\\Music\\First.flac');
      expect(workers).toHaveLength(1);

      await vi.waitFor(() => expect(workers[0]?.terminated).toBe(true), { timeout: 1_500 });

      await scanWorkers.metadataReader.read('D:\\Music\\Second.flac');
      expect(workers).toHaveLength(2);
      expect(workers[1]?.requests).toEqual([
        expect.objectContaining({
          type: 'metadata:read',
          filePath: 'D:\\Music\\Second.flac',
        }),
      ]);
    } finally {
      scanWorkers.close();
  ...
  it('reads remote albums through a dedicated worker without occupying a scan worker', async () => {
    const workers: FakeWorker[] = [];
    const scanWorkers = createWorkerBackedLibraryScanWorkers({
      workerCount: 4,
      workerFactory: () => {
        const worker = new FakeWorker((request) => ({
          requestId: request.requestId,
          ok: true,
          result: { items: [], page: 1, pageSize: 120, total: 849, hasMore: true },
        }));
        workers.push(worker);
        return worker;
      },
    ...
    try {
      await expect(scanWorkers.remoteAlbumReader.getAlbums({
        databasePath: 'D:\\Library\\echo-library.sqlite',
        query: { page: 1, pageSize: 120, sourceProvider: 'remote' },
        searchOptions: {
          chineseCrossScriptSearchEnabled: true,
          artistMergeStrategy: 'standard',
          remoteAlbumMergeStrategy: 'conservative',
        },
      })).resolves.toMatchObject({ total: 849, hasMore: true });
      ...
      expect(workers[0]?.requests).toEqual([
        expect.objectContaining({
          ...
          databasePath: 'D:\\Library\\echo-library.sqlite',
          query: expect.objectContaining({ sourceProvider: 'remote', pageSize: 120 }),
        }),
      ]);
    } finally {
      scanWorkers.close();
