...
const makeTempRoot = (): string => {
  const root = join(tmpdir(), `echo-metadata-reader-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
const writeFlacWithCueSheet = (filePath: string, cueSheet: string): void => {
  const vendor = Buffer.from('ECHO', 'utf8');
...
describe('TsMetadataReader parser fallbacks', () => {
  ...
  it('reads extended editable tags from common metadata', async () => {
    parseFileMock.mockResolvedValue(emptyMetadata({
      common: {
        title: 'Extended Tags',
        artist: 'Primary Artist',
        composer: ['Composer A', 'Composer B'],
        comment: [{ text: 'Studio note' }],
        track: { no: 3, of: 12 },
        disk: { no: 1, of: 2 },
      },
      format: {
    ...
    const result = await new TsMetadataReader().read('D:\\Music\\Extended Tags.flac');
