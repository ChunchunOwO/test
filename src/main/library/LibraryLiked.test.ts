...
describe('LibraryStore liked media', () => {
  ...
  it('keeps liked album ids available across an unchanged album regroup', () => {
    const store = createStore();
    seedTrack(store, 'track-1', {
      title: 'Track One',
      album: 'Stable Album',
      albumArtist: 'Stable Artist',
      fieldSources: { albumArtist: 'embedded' },
    });
    const albumService = new AlbumService();
    store.refreshAlbums(albumService);
    const album = store.getAlbums({ pageSize: 1 }).items[0]!;
    store.likeAlbum(album.id);

    store.refreshAlbums(albumService);
    const liked = store.getLikedAlbums().items[0]!;

    expect(liked.mediaId).toBe(album.id);
    expect(liked.unavailable).toBe(false);
    expect(liked.album?.id).toBe(album.id);
  });
