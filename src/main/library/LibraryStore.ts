...
import type { ArtistMergeStrategy, RemoteAlbumMergeStrategy } from '../../shared/types/appSettings';
...
import {
  ...
  coverBackedLooseAlbumKey,
  isKnownLooseAlbumTitle,
  makeCoverBackedLooseAlbumKeys,
  mapPreviousAlbumsToNextIds,
  type AlbumGroupingIdentity,
  type PreviousAlbumGroupingIdentity,
} from './AlbumGrouping';
import {
...
import { unifiedArtistOrderSql } from './artistOrderSql';
...
import { readRemoteServerAlbum, readRemoteServerAlbumTracks, remoteServerAlbumIdForTrack, remoteServerAlbumLocatorFromId } from './remote/RemoteServerAlbumReadModel';
import * as remoteServerLibrary from './remote/RemoteServerLibraryReadModel';
import {
...
import {
  ...
  type SteamAchievementPlaybackFactQuery,
  type SteamAchievementPlaybackFactInput,
...
const albumIndexTrackSelectSql = `SELECT
...
const reliableAlbumArtistDisplaySources = new Set(['embedded', 'manual', 'network', 'sidecar', 'technical']);
...
const flacFileFormatSql = "(UPPER(TRIM(COALESCE(codec, ''))) = 'FLAC' OR LOWER(COALESCE(path, '')) LIKE '%.flac')";
const mp3FileFormatSql = "(UPPER(TRIM(COALESCE(codec, ''))) = 'MP3' OR UPPER(COALESCE(codec, '')) LIKE 'MPEG%LAYER 3%' OR UPPER(COALESCE(codec, '')) LIKE 'MPEG%LAYER III%' OR LOWER(COALESCE(path, '')) LIKE '%.mp3')";
const wavFileFormatSql = "(UPPER(TRIM(COALESCE(codec, ''))) IN ('WAV', 'WAVE') OR LOWER(COALESCE(path, '')) LIKE '%.wav')";
const aiffFileFormatSql = "(UPPER(TRIM(COALESCE(codec, ''))) IN ('AIFF', 'AIF', 'AIFC') OR LOWER(COALESCE(path, '')) LIKE '%.aiff' OR LOWER(COALESCE(path, '')) LIKE '%.aif' OR LOWER(COALESCE(path, '')) LIKE '%.aifc')";
const alacFileFormatSql = "(UPPER(TRIM(COALESCE(codec, ''))) IN ('ALAC', 'APPLE LOSSLESS') OR LOWER(COALESCE(path, '')) LIKE '%.alac')";
const aacFileFormatSql = "(UPPER(TRIM(COALESCE(codec, ''))) IN ('AAC', 'MP4A') OR UPPER(COALESCE(codec, '')) LIKE 'AAC %' OR LOWER(COALESCE(path, '')) LIKE '%.aac')";
const oggFileFormatSql = "(UPPER(TRIM(COALESCE(codec, ''))) = 'OGG' OR UPPER(COALESCE(codec, '')) LIKE '%VORBIS%' OR LOWER(COALESCE(path, '')) LIKE '%.ogg')";
const opusFileFormatSql = "(UPPER(TRIM(COALESCE(codec, ''))) = 'OPUS' OR LOWER(COALESCE(path, '')) LIKE '%.opus')";
const knownFileFormatSql = [
  flacFileFormatSql,
  mp3FileFormatSql,
  wavFileFormatSql,
  aiffFileFormatSql,
...
const unifiedTrackFileFormatFilterSql = (filter?: LibraryFileFormatFilter): string => {
  switch (filter) {
    case 'flac':
      return flacFileFormatSql;
    case 'mp3':
      return mp3FileFormatSql;
    case 'wav':
      return wavFileFormatSql;
    case 'aiff':
      return aiffFileFormatSql;
    case 'alac':
      return alacFileFormatSql;
    case 'aac':
      return aacFileFormatSql;
    case 'ogg':
      return oggFileFormatSql;
    case 'opus':
      return opusFileFormatSql;
    case 'dsd':
      return dsdAudioFormatSql;
    case 'other':
      return `NOT (${knownFileFormatSql})`;
    case 'all':
    default:
      return '';
  }
...
export type LibraryStoreOptions = {
  skipSearchTermsBackfill?: boolean;
};
...
export class LibraryStore {
  ...
  constructor(
    ...
    if (this.options.skipSearchTermsBackfill !== true) {
  ...
  getPlaybackStatsInsights(query?: PlaybackHistoryQuery) {
    return getPlaybackStatsInsights(this.database, query);
  }

  getSteamAchievementHistoryStats() {
    return getSteamAchievementHistoryStats(this.database);
  }

  recordSteamAchievementPlaybackFact(input: SteamAchievementPlaybackFactInput): void {
    recordSteamAchievementPlaybackFact(this.database, input);
  }

  getSteamAchievementPlaybackFacts(query?: SteamAchievementPlaybackFactQuery) {
    return getSteamAchievementPlaybackFacts(this.database, query);
  }
  ...
  getTrack(trackId: string): LibraryTrack | null {
    const row = this.getRow(
      `SELECT
        ...
        tracks.replay_gain_status, tracks.replay_gain_updated_at,
        tracks.cover_id, tracks.metadata_status, tracks.embedded_metadata_status, tracks.embedded_cover_status,
        tracks.network_metadata_status, tracks.field_sources_json, tracks.created_at
      FROM tracks
  ...
  refreshArtists(options: { artistMergeStrategy?: ArtistMergeStrategy } = {}): void {
    this.transaction(() => {
      const timestamp = nowIso();
      const artistMergeStrategy = normalizeArtistMergeStrategy(options.artistMergeStrategy ?? this.readSearchOptions().artistMergeStrategy);
  ...
  async refreshArtistsCooperatively(options: { yieldEveryRows?: number; artistMergeStrategy?: ArtistMergeStrategy } = {}): Promise<void> {
  ...
  refreshAlbums(
    ...
    const tracks = this.loadAlbumIndexTrackRows();
    this.commitAlbumIndexRebuild(this.buildStandardAlbumGroupsForRows(tracks, albumService), albumService, now, options);
  }

  async refreshAlbumsCooperatively(
    albumService: AlbumService,
    now = nowIso(),
    options: { albumMergeStrategy?: AlbumMergeStrategy } = {},
    yieldEveryRows = 300,
    ...
    const yieldEvery = Math.max(50, Math.floor(yieldEveryRows));
    for (let index = 0; index < tracks.length; index += 1) {
      this.accumulateStandardAlbumGroup(standardGroups, tracks[index]!, index, albumService);
      if ((index + 1) % yieldEvery === 0) {
        await yieldToMainLoop();
      }
    }
    this.commitAlbumIndexRebuild(standardGroups, albumService, now, options);
  }

  private loadAlbumIndexTrackRows(): DbRow[] {
    return this.allRows(
      `${albumIndexTrackSelectSql}
       WHERE tracks.missing = 0
       ORDER BY tracks.album_artist COLLATE NOCASE, tracks.album COLLATE NOCASE, tracks.disc_no, tracks.track_no, tracks.title COLLATE NOCASE`,
  ...
  private commitAlbumIndexRebuild(
    standardGroups: Map<string, StandardAlbumGroup>,
    albumService: AlbumService,
    now: string,
    options: { albumMergeStrategy?: AlbumMergeStrategy },
  ): void {
    this.transaction(() => {
      const previousById = new Map<string, { id: string; albumKey: string; trackIds: Set<string> }>();
      const previousCreatedAtById = new Map<string, string>();
      for (const row of this.allRows(
        `SELECT albums.id, albums.album_key, albums.created_at, album_tracks.track_id
         FROM albums
         LEFT JOIN album_tracks ON album_tracks.album_id = albums.id`,
      )) {
        const id = String(row.id);
        const previous =
          previousById.get(id) ?? { id, albumKey: String(row.album_key), trackIds: new Set<string>() };
        const trackId = textOrNull(row.track_id);
        if (trackId) previous.trackIds.add(trackId);
        previousById.set(id, previous);
        previousCreatedAtById.set(id, String(row.created_at || now));
      }
      const previousAlbums = Array.from(previousById.values());
      const looseAlbumKeys =
        options.albumMergeStrategy === 'sameTitleAndCover'
          ? this.makeLooseAlbumKeys(standardGroups, albumService)
          : new Map<string, string>();
      const nextTrackIdsByKey = new Map<string, Set<string>>();
      for (const group of standardGroups.values()) {
        const albumKey = looseAlbumKeys.get(group.albumKey) ?? group.albumKey;
        const trackIds = nextTrackIdsByKey.get(albumKey) ?? new Set<string>();
        for (const link of group.links) trackIds.add(link.trackId);
        nextTrackIdsByKey.set(albumKey, trackIds);
      }
      const nextIdentities: AlbumGroupingIdentity[] = Array.from(nextTrackIdsByKey, ([albumKey, trackIds]) => ({ albumKey, trackIds }));
      const albumIdsByKey = assignStableAlbumIds(nextIdentities, previousAlbums, randomUUID);
      this.run('DELETE FROM album_tracks');
      this.run('DELETE FROM albums');
      ...
      for (const standardGroup of standardGroups.values()) {
        ...
        const albumId = albumIdsByKey.get(albumKey)!;
        const stats =
        ...
        for (const link of standardGroup.links) albumTrackLinks.push({ albumId, ...link });
      }
      ...
      for (const album of albumStats.values()) {
        this.run(
          ...
          album.trackCount,
          album.duration,
          previousCreatedAtById.get(album.id) ?? now,
          now,
        );
      }
      ...
      this.migrateAlbumPlaylistItems(
        previousAlbums,
        nextIdentities.map((album) => ({ ...album, id: albumIdsByKey.get(album.albumKey)! })),
        albumStats,
        now,
      );
    });
  ...
  private migrateAlbumPlaylistItems(
    previousAlbums: readonly PreviousAlbumGroupingIdentity[],
    nextAlbums: readonly (AlbumGroupingIdentity & { id: string })[],
    albumsByKey: ReadonlyMap<string, AlbumIndexStats>,
    now: string,
  ): void {
    const targetsByPreviousId = mapPreviousAlbumsToNextIds(previousAlbums, nextAlbums);
    const albumsById = new Map(Array.from(albumsByKey.values(), (album) => [album.id, album]));
    const claimedByPlaylist = new Map<string, Set<string>>();
    const touchedPlaylists = new Set<string>();
    const items = this.allRows(
      `SELECT * FROM playlist_items
       WHERE media_type = 'album' AND source_provider = 'local'
       ORDER BY playlist_id, position, added_at`,
    );

    for (const item of items) {
      const playlistId = String(item.playlist_id);
      const claimed = claimedByPlaylist.get(playlistId) ?? new Set<string>();
      const targets = (targetsByPreviousId.get(String(item.media_id)) ?? []).filter((id) => albumsById.has(id) && !claimed.has(id));
      if (targets.length === 0) {
        if ((targetsByPreviousId.get(String(item.media_id)) ?? []).length > 0) {
          this.run('DELETE FROM playlist_items WHERE id = ?', String(item.id));
          touchedPlaylists.add(playlistId);
        }
        continue;
      }

      const writeSnapshot = (itemId: string, albumId: string): void => {
        const album = albumsById.get(albumId)!;
        this.run(
          `UPDATE playlist_items SET media_id = ?, title_snapshot = ?, artist_snapshot = ?, album_snapshot = ?,
             duration_snapshot = ?, cover_id = ?, unavailable = 0 WHERE id = ?`,
          albumId,
          album.title,
          album.albumArtist,
          album.title,
      ...
      writeSnapshot(String(item.id), targets[0]!);
      claimed.add(targets[0]!);
      touchedPlaylists.add(playlistId);

      for (const albumId of targets.slice(1)) {
        const cloneId = randomUUID();
        const nextPosition = Number(
          this.getRow('SELECT COALESCE(MAX(position), -1) + 1 AS next_position FROM playlist_items WHERE playlist_id = ?', playlistId)
            ?.next_position ?? 0,
        );
        this.run(
          ...
          playlistId,
          albumId,
          textOrNull(item.source_item_id),
          textOrNull(item.title_snapshot),
          textOrNull(item.artist_snapshot),
          textOrNull(item.album_snapshot),
          numberOrNull(item.duration_snapshot),
          textOrNull(item.cover_id),
          nextPosition,
          String(item.added_at),
          textOrNull(item.added_from),
        );
        writeSnapshot(cloneId, albumId);
        claimed.add(albumId);
      }
      claimedByPlaylist.set(playlistId, claimed);
    }
    for (const playlistId of touchedPlaylists) this.refreshPlaylistItemCount(playlistId, now);
  }

  private buildStandardAlbumGroupsForRows(tracks: DbRow[], albumService: AlbumService): Map<string, StandardAlbumGroup> {
    const standardGroups = new Map<string, StandardAlbumGroup>();
    tracks.forEach((track, index) => {
      this.accumulateStandardAlbumGroup(standardGroups, track, index, albumService);
    });
    return standardGroups;
  }

  private accumulateStandardAlbumGroup(
    standardGroups: Map<string, StandardAlbumGroup>,
    track: DbRow,
    index: number,
    albumService: AlbumService,
    ...
    const coverSourceType = textOrNull(track.cover_source_type);
    ...
    if (coverSourceHash && coverSourceType !== 'default') {
  ...
  private makeLooseAlbumKeys(standardGroups: Map<string, StandardAlbumGroup>, albumService: AlbumService): Map<string, string> {
    const groupsByKey = new Map(Array.from(standardGroups.values(), (group) => [group.albumKey, group]));
    return makeCoverBackedLooseAlbumKeys(
      Array.from(standardGroups.values(), (group) => ({
        albumKey: group.albumKey,
        title: group.title,
        coverMatchKey: mostCommonMapKey(group.coverFingerprints) ?? mostCommonMapKey(group.coverSourceHashes),
      })),
      (candidate) => {
        const group = groupsByKey.get(candidate.albumKey)!;
        return candidate.coverMatchKey && isKnownLooseAlbumTitle(candidate.title)
          ? coverBackedLooseAlbumKey(candidate.title, candidate.coverMatchKey)
          : albumService.makeAlbumKey({
              ...group.keyInput,
              coverId: group.coverId,
              coverSourceHash: mostCommonMapKey(group.coverSourceHashes),
              mergeStrategy: 'standard',
            });
      },
    );
  ...
  getTracks(query?: LibraryPageQuery): LibraryPage<LibraryTrack> {
    const startedAt = performance.now();
    const { page, pageSize, search, sort, sorts: requestedSorts, sourceProvider, sourceId, excludeTrackIds, randomWindow } = pageFromQuery(query);
    const sorts = requestedSorts?.length ? requestedSorts : [sort as LibrarySort];
    const searchOptions = this.readSearchOptions();
    const offset = (page - 1) * pageSize;
    ...
    const remoteWhereSql = showDuplicatesOnly
      ? 'WHERE 0 = 1'
      : searchQuery
      ? "WHERE remote_tracks.availability != 'missing' AND remote_sources.status = 'enabled' AND remote_sources.provider IN ('webdav', 'jellyfin', 'emby', 'smb', 'sshfs', 'subsonic') AND remote_tracks_fts MATCH ?"
      : "WHERE remote_tracks.availability != 'missing' AND remote_sources.status = 'enabled' AND remote_sources.provider IN ('webdav', 'jellyfin', 'emby', 'smb', 'sshfs', 'subsonic')";
    ...
    const fileFormatFilterSql = unifiedTrackFileFormatFilterSql(query?.fileFormatFilter);
    ...
    const includePlaybackStats = sorts.some((value) => value === 'lastPlayed' || value === 'playCountAsc' || value === 'playCountDesc');
    ...
    const orderSql = unifiedTrackOrderSql(sorts, Boolean(searchQuery));
  ...
  getAlbums(query?: LibraryPageQuery): LibraryPage<LibraryAlbum> {
    ...
    const useServerReadModel = mediaTypeFilter === 'remote' && remoteServerLibrary.supportsRemoteServerAlbumSort(sort)
      && remoteServerLibrary.canUseRemoteServerLibraryReadModel(this.database, sourceId, remoteServerLibrary.remoteServerLibraryConfigurationKey(this.readSearchOptions()));
    const albumsSql = useServerReadModel
      ? remoteServerLibrary.remoteServerAlbumsSql(sourceId, { includePlaybackStats, includeSearchBlob: Boolean(search) })
      : this.unifiedAlbumsSql(includePlaybackStats, mediaTypeFilter, sourceId, Boolean(search));
    const rows = this.allRows(
      `${albumsSql}
      SELECT
        id, media_type, source_id, source_display_name, provider, album_key, title, album_artist, year, track_count,
        duration, cover_id, cover_art, cover_track_id, COUNT(*) OVER () AS total_count
      FROM library_albums
      ${whereSql}
      ${orderSql}
      LIMIT ? OFFSET ?`,
  ...
  private unifiedAlbumsSql(includePlaybackStats = false, mediaType: 'local' | 'remote' | null = null, sourceId: string | null = null, includeSearchBlob = true): string {
    ...
    return `WITH ${playbackStatsCtes} remote_album_rows AS (
      ...
      WHERE remote_tracks.availability != 'missing'
        ...
        AND remote_sources.provider IN ('webdav', 'jellyfin', 'emby', 'smb', 'sshfs', 'subsonic')
        AND ${mediaType === 'local' ? '0 = 1' : '1 = 1'}${sourceId ? ` AND remote_tracks.source_id = ${sqlString(sourceId)}` : ''}
    ),
    local_albums AS (
      SELECT
        albums.id,
      ...
      FROM albums
      ${localPlaybackJoin}
      WHERE ${mediaType === 'remote' ? '0 = 1' : '1 = 1'} AND EXISTS (
        SELECT 1
        FROM album_tracks
        INNER JOIN tracks ON tracks.id = album_tracks.track_id
        WHERE album_tracks.album_id = albums.id
    ...
    remote_albums AS (
      SELECT
        ...
        ${includeSearchBlob ? "GROUP_CONCAT(COALESCE(search_terms, '') || ' ' || title || ' ' || artist || ' ' || album_artist || ' ' || COALESCE(genre, '') || ' ' || remote_path, ' ')" : "''"} AS search_blob
    ...
    library_albums AS (
      ...
      SELECT
        ...
        remote_albums.cover_art,
        remote_albums.cover_track_id,
  ...
  getAlbum(albumId: string): LibraryAlbumDetail | null {
    if (remoteServerAlbumLocatorFromId(albumId)) {
      const remoteRow = readRemoteServerAlbum(this.database, albumId);
      return remoteRow ? this.mapAlbumDetail(remoteRow) : null;
    }
    const row = this.getRow(
      `${this.unifiedAlbumsSql()}
      SELECT
        id, media_type, source_id, source_display_name, provider, album_key, title, album_artist, year, track_count,
        duration, cover_id, cover_art, cover_track_id
      FROM library_albums
      WHERE id = ?`,
      albumId,
    );
  ...
  getAlbumForTrack(trackId: string): LibraryAlbum | null {
    ...
    const remoteServerAlbumId = remoteServerAlbumIdForTrack(this.database, trackId);
    if (remoteServerAlbumId) {
      return this.getAlbum(remoteServerAlbumId);
    }
    ...
    const remoteRow = this.getRow(
      `${this.unifiedAlbumsSql()}
       SELECT
        ...
        library_albums.cover_art, library_albums.cover_track_id
       FROM remote_album_rows
  ...
  getArtists(query?: LibraryPageQuery): LibraryPage<LibraryArtist> {
    ...
    const whereSql = whereParts.length > 0 ? `WHERE ${whereParts.join(' AND ')}` : '';
    const whereParams = [...searchFilter.params, ...(mediaTypeFilter ? [mediaTypeFilter] : []), ...(sourceId ? [sourceId] : [])];
    const orderSql = unifiedArtistOrderSql(sort, query?.prioritizeArtistAvatars === true);
    const artistsSql = mediaTypeFilter === 'remote' && remoteServerLibrary.canUseRemoteServerLibraryReadModel(this.database, sourceId, remoteServerLibrary.remoteServerLibraryConfigurationKey(this.readSearchOptions()))
      ? remoteServerLibrary.remoteServerArtistsSql(sourceId)
    ...
    const rows = this.allRows(
      ...
      SELECT
        ...
        avatar_source_hash, avatar_thumb_path, avatar_medium_path, avatar_large_path, COUNT(*) OVER () AS total_count
  ...
  getArtist(artistId: string): LibraryArtist | null {
    const remoteLocator = remoteServerLibrary.remoteServerArtistLocatorFromId(artistId);
    const artistsSql = remoteLocator && remoteServerLibrary.canUseRemoteServerLibraryReadModel(this.database, remoteLocator.sourceId, remoteServerLibrary.remoteServerLibraryConfigurationKey(this.readSearchOptions()))
      ? remoteServerLibrary.remoteServerArtistsSql(remoteLocator.sourceId)
      : this.unifiedArtistsSql();
    const row = this.getRow(
      `${artistsSql}
       SELECT
  ...
  getArtistInsights(artistId: string, options: ArtistInsightsOptions = {}): ArtistInsights {
    ...
    if (artist.mediaType === 'remote') {
      return remoteServerLibrary.remoteServerArtistInsights(artist, options, generatedAt);
    }
  ...
  getArtistTracks(artistId: string, query?: Pick<LibraryPageQuery, 'page' | 'pageSize' | 'sort'>): LibraryPage<LibraryTrack> {
    ...
    if (artist.mediaType === 'remote') {
      const result = remoteServerLibrary.readRemoteArtistTracks(this.database, artist.id, pageSize, offset);
      return remoteServerLibrary.mapRemoteServerPage(result ?? { rows: [], total: 0 }, page, pageSize, offset, (row) => this.mapTrack(row));
    }
  ...
  getArtistAlbums(artistId: string, query?: Pick<LibraryPageQuery, 'page' | 'pageSize' | 'sort'>): LibraryPage<LibraryAlbum> {
    ...
    if (artist.mediaType === 'remote') {
      const serverResult = remoteServerLibrary.readRemoteServerArtistAlbums(this.database, artist.id, pageSize, offset, sort);
      if (serverResult) {
        return remoteServerLibrary.mapRemoteServerPage(serverResult, page, pageSize, offset, (row) => this.mapAlbum(row));
      }
    ...
    const orderSql = albumOrderSql(sort);
    const totalRow = this.getRow(
      `SELECT COUNT(*) AS total
       FROM artist_albums
  ...
  getAlbumTracks(albumId: string, query?: Pick<LibraryPageQuery, 'page' | 'pageSize'>): LibraryPage<LibraryTrack> {
    ...
    const offset = (page - 1) * pageSize;
    const serverAlbumPage = readRemoteServerAlbumTracks(this.database, albumId, pageSize, offset);
    if (serverAlbumPage) {
      return { items: serverAlbumPage.rows.map((row) => this.mapTrack(row)), page, pageSize, total: serverAlbumPage.total, hasMore: offset + serverAlbumPage.rows.length < serverAlbumPage.total };
    }
  ...
  getLikedSongsPlaylist(): LibraryPlaylist {
    return this.ensureSystemPlaylist(likedSongsSourcePlaylistId, 'Liked Songs', 'Tracks you liked in ECHO.');
  }

  getLikedAlbumsPlaylist(): LibraryPlaylist {
    return this.ensureSystemPlaylist(likedAlbumsSourcePlaylistId, 'Liked Albums', 'Albums you liked in ECHO.');
  }
  ...
  private mapTrack(row: DbRow): LibraryTrack {
    ...
    return {
      ...
      coverThumb: this.toCoverUrl(row.cover_id, 'thumb')
        ?? subsonicDirectCoverUrlFor(row.id, row.source_id, row.provider, row.cover_id, fieldSources, row.remote_path, row.stable_key, 160),
      createdAt: textOrNull(row.created_at),
  ...
  private mapAlbum(row: DbRow): LibraryAlbum {
    ...
    const coverId = textOrNull(row.cover_id);
    const coverArt = textOrNull(row.cover_art);
    ...
    return {
      ...
      coverId,
      coverThumb: this.toCoverUrl(coverId, 'album')
        ?? subsonicDirectCoverUrlFor(row.cover_track_id ?? row.id, row.source_id, row.provider, coverId, coverArt ? { coverArt } : null, null, null, 320),
    };
  }
  ...
  private mapAlbumDetail(row: DbRow): LibraryAlbumDetail {
    ...
    return {
      ...album,
      coverLarge: this.toCoverUrl(row.cover_id, 'large') ?? album.coverThumb,
    };
  }
