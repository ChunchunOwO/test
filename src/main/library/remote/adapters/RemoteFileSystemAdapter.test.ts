...
const makeRoot = async (): Promise<string> => {
  const root = join(tmpdir(), `echo-remote-fs-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
describe('RemoteFileSystemAdapter', () => {
  ...
  it('keeps mounted path canonicalization off the Electron main thread', async () => {
    const implementation = await readFile(
      fileURLToPath(new URL('./RemoteFileSystemAdapter.ts', import.meta.url)),
      'utf8',
    );

    expect(implementation).not.toContain('realpathSync');
    expect(implementation).not.toContain("from 'node:fs'");
  });
  ...
  it('rejects an absolute root override outside the configured mount', async () => {
    const root = await makeRoot();
    const outside = await makeRoot();
    const adapter = new RemoteFileSystemAdapter('smb');
    const mountedSource = source(root);
    mountedSource.config = { ...mountedSource.config, rootPath: outside };

    const result = await adapter.testConnection({ source: mountedSource });

    expect(result.ok).toBe(false);
    expect(result.message).toContain('rootPath must be relative');
  });
});
