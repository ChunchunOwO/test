...
describe('MediaServerRemoteSourceAdapter', () => {
  ...
  it('refuses credentials over public HTTP before making a request', async () => {
    ...
    const result = await adapter.testConnection({
      source: source(80, { baseUrl: 'http://public.example', authType: 'basic', username: 'alice', secret: 'wonderland' }),
    });
  ...
  it('authenticates, scans server metadata, and proxies streams without leaking credentials', async () => {
    ...
    adapter.clearSourceState('source-jellyfin');
    expect((await adapter.testConnection({ source: source(port) })).ok).toBe(true);
