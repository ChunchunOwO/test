...
type SourceRevisionContext = Pick<QueueJob, 'sourceId' | 'revision'>;
...
const maxRememberedCoverIds = 4096;
...
const playbackDeferredKinds = new Set<RemoteBackgroundJobKind>(['cover', 'lyrics']);
...
export class RemoteBackgroundJobQueue {
  ...
  enqueueSource(sourceId: string, kinds: RemoteBackgroundJobKind[] = ['metadata', 'lyrics'], options: { failedOnly?: boolean; priority?: number } = {}): RemoteBackgroundJobStatus {
    ...
    this.sourceContinuations.set(this.sourceContinuationKey(sourceId, normalizedKinds, options, revision), {
      revision,
      kinds: normalizedKinds,
      options,
      seenTrackIds: new Set(),
    });
    this.scheduleSourceEnqueue(sourceId, normalizedKinds, options, revision);
    return this.getStatus(sourceId);
  }

  enqueueTrack(track: QueueableTrack, kinds: RemoteBackgroundJobKind[] = ['metadata'], priority = 0): void {
    if (this.disposing || this.cancelledSources.has(track.sourceId)) return;
    const jobs = this.kindsForTrack(track, this.normalizeKinds(kinds), false).map((kind) => ({
      sourceId: track.sourceId,
      revision: this.getSourceRevision(track.sourceId),
      kind,
      trackId: track.id,
      priority,
      retry: false,
  ...
  enqueueTrackWrites(tracks: RemoteTrackWrite[], kinds: RemoteBackgroundJobKind[] = ['metadata'], priority = 0): void {
    ...
    for (const track of tracks) {
      for (const kind of this.kindsForTrack(track, normalizedKinds, false)) {
        jobs.push({ sourceId: track.sourceId, revision: this.getSourceRevision(track.sourceId), kind, trackId: track.id, priority, retry: false, groupKey: this.jobGroupKey(track, kind) });
      }
    }
  ...
  cancelSource(sourceId: string): RemoteBackgroundJobStatus {
    ...
    this.pausedSources.add(sourceId);
    this.cancelledSources.add(sourceId);
    this.abortRunningJobs((job) => job.sourceId === sourceId);
    ...
    this.coverIdsByKey.clear();
    this.coverPromisesByKey.clear();
    this.touch();
    return this.getStatus(sourceId);
  }
  ...
  resume(sourceId: string): RemoteBackgroundJobStatus {
    ...
    this.cancelledSources.delete(sourceId);
    this.touch();
    this.schedule();
    return this.getStatus(sourceId);
  }
  ...
  getStatus(sourceId: string): RemoteBackgroundJobStatus {
    ...
    const running = this.runningCountsForSource(sourceId);
  ...
  async runTrackMetadataNow(trackId: string): Promise<RemoteLibraryTrack | null> {
    ...
    const revision = this.getSourceRevision(track.sourceId);
    this.requireEnabledSource(track.sourceId, revision);
    await this.runMetadataJob(track, 'duration-backfill', undefined, { sourceId: track.sourceId, revision });
  ...
  async runTrackCoverNow(trackId: string): Promise<RemoteLibraryTrack | null> {
    ...
    const revision = this.getSourceRevision(track.sourceId);
    this.requireEnabledSource(track.sourceId, revision);
    await this.runCoverJob(track, undefined, { sourceId: track.sourceId, revision });
    return this.store.getTrack(trackId);
  }
  ...
  private scheduleSourceEnqueue(
    ...
    if (this.disposing) return;
    const key = this.sourceContinuationKey(sourceId, kinds, options, revision);
    ...
    this.touch();
    setImmediate(() => {
      if (this.disposing || !this.isSourceRevisionCurrent(sourceId, revision)) {
        this.sourceEnqueueingKeys.delete(key);
        return;
      }
      let enqueuedCount = 0;
      void this.enqueueSourceInChunks(sourceId, kinds, options, revision)
        .then((count) => {
          enqueuedCount = count;
          if (count === 0) {
            this.sourceContinuations.delete(key);
        ...
        .catch((error) => {
          this.lastErrors.set(sourceId, sanitizeRemoteErrorMessage(error));
          this.sourceContinuations.delete(key);
          this.touch();
        })
        .finally(() => {
  ...
  private async enqueueSourceInChunks(
    ...
    const continuation = this.sourceContinuations.get(this.sourceContinuationKey(sourceId, kinds, options, revision));
    ...
    for (let index = 0; index < trackIds.length; index += chunkSize) {
      if (!this.isSourceRevisionCurrent(sourceId, revision) || this.cancelledSources.has(sourceId)) {
  ...
  private sourceContinuationKey(
    sourceId: string,
    kinds: RemoteBackgroundJobKind[],
    options: { failedOnly?: boolean },
    revision: number,
  ): string {
    return `${sourceId}:${revision}:${kinds.join(',')}:${options.failedOnly === true ? 'failed' : 'all'}`;
  }
  ...
  private scheduleSourceContinuation(sourceId: string): void {
    ...
    for (const [key, continuation] of this.sourceContinuations) {
      if (
        ...
        || this.sourceEnqueueingKeys.has(key)
        || !this.isSourceRevisionCurrent(sourceId, continuation.revision)
      ) {
        continue;
      }

      this.scheduleSourceEnqueue(sourceId, continuation.kinds, continuation.options, continuation.revision);
      break;
    }
  }
  ...
  private enqueueMany(jobs: QueueJob[]): void {
    ...
    for (const job of jobs) {
      if (!this.isSourceRevisionCurrent(job.sourceId, job.revision)) {
        continue;
      }
  ...
  private drain(): void {
    ...
    for (const kind of jobKinds) {
      ...
      while (pending.length > 0 && startedCount < maxJobsStartedPerDrain) {
        const staleIndex = pending.findIndex((job) => !this.isSourceRevisionCurrent(job.sourceId, job.revision));
        if (staleIndex >= 0) {
          const [staleJob] = pending.splice(staleIndex, 1);
          this.queuedKeys.delete(this.jobKey(staleJob));
          this.adjust(this.pendingBySource, staleJob.sourceId, staleJob.kind, -1);
          continue;
        }
        ...
        const runningPromise = (async () => {
          ...
          if (
            outcome === 'aborted'
            && !this.disposing
            && !this.cancelledSources.has(job.sourceId)
            && this.isSourceRevisionCurrent(job.sourceId, job.revision)
          ) {
  ...
  private async run(job: QueueJob, track: RemoteLibraryTrack, signal?: AbortSignal): Promise<'done' | 'aborted'> {
    try {
      this.assertSourceWorkCurrent(job, signal);
      this.requireEnabledSource(job.sourceId, job.revision, signal);
      let shouldCooldownCover = false;
      let shouldCooldownLyrics = false;
      if (job.kind === 'metadata' || job.kind === 'duration-backfill') {
        const updated = await this.runMetadataJob(track, job.kind, signal, job);
        this.assertSourceWorkCurrent(job, signal);
        if (updated) {
          this.enqueueTrack(updated, ['lyrics']);
        }
      } else if (job.kind === 'cover') {
        const covered = await this.runCoverJob(track, signal, job);
        this.assertSourceWorkCurrent(job, signal);
        if (!covered) {
          this.increment(this.skippedBySource, job.sourceId, job.kind);
          return 'done';
        ...
        const lyrics = await getLyricsService().getLyricsForTrack(track.id, { signal });
        this.assertSourceWorkCurrent(job, signal);
      ...
      if (signal?.aborted || isAbortError(error)) {
        if (
          this.isSourceRevisionCurrent(job.sourceId, job.revision)
          && !this.cancelledSources.has(job.sourceId)
          && (job.kind === 'cover' || job.kind === 'lyrics' || job.kind === 'metadata' || job.kind === 'duration-backfill')
        ) {
      ...
      this.lastErrors.set(job.sourceId, sanitizeRemoteErrorMessage(error));
      if (job.kind === 'cover' || job.kind === 'lyrics' || job.kind === 'metadata' || job.kind === 'duration-backfill') {
  ...
  private async runCoverJob(
    track: RemoteLibraryTrack,
    signal?: AbortSignal,
    context: SourceRevisionContext = { sourceId: track.sourceId, revision: this.getSourceRevision(track.sourceId) },
  ): Promise<boolean> {
    this.assertSourceWorkCurrent(context, signal);
    ...
    if (cachedCoverId) {
      this.assertSourceWorkCurrent(context, signal);
      this.applyCoverId(track, cachedCoverId, context, signal);
      if (coverKey) {
        this.rememberCoverId(coverKey, cachedCoverId);
      }
      return true;
    }
    ...
    if (coverKey && this.coverIdsByKey.has(coverKey)) {
      ...
      this.assertSourceWorkCurrent(context, signal);
      if (coverId) {
        this.applyCoverId(track, coverId, context, signal);
      } else {
    ...
    const coverPromise = this.readAndCacheCover(track, this.coverService, signal, context);
    if (coverKey) {
    ...
    try {
      ...
      this.assertSourceWorkCurrent(context, signal);
      if (coverKey) {
        this.rememberCoverId(coverKey, coverId);
      }
      if (coverId) {
        this.applyCoverId(track, coverId, context, signal);
      } else {
      ...
      if (coverKey && this.coverPromisesByKey.get(coverKey) === coverPromise) {
        this.coverPromisesByKey.delete(coverKey);
      }
    }
  }

  private rememberCoverId(coverKey: string, coverId: string | null): void {
    this.coverIdsByKey.delete(coverKey);
    this.coverIdsByKey.set(coverKey, coverId);
    while (this.coverIdsByKey.size > maxRememberedCoverIds) {
      const oldestKey = this.coverIdsByKey.keys().next().value;
      if (typeof oldestKey !== 'string') {
        break;
      }
      this.coverIdsByKey.delete(oldestKey);
    }
  }

  private applyCoverId(
    track: RemoteLibraryTrack,
    coverId: string,
    context: SourceRevisionContext,
    signal?: AbortSignal,
  ): void {
    this.assertSourceWorkCurrent(context, signal);
  ...
  private async readAndCacheCover(
    track: RemoteLibraryTrack,
  ...
  private async runMetadataJob(
    track: RemoteLibraryTrack,
    kind: RemoteBackgroundJobKind,
    signal?: AbortSignal,
    context: SourceRevisionContext = { sourceId: track.sourceId, revision: this.getSourceRevision(track.sourceId) },
  ): Promise<RemoteLibraryTrack | null> {
    this.assertSourceWorkCurrent(context, signal);
    const source = this.store.getSourceWithSecret(track.sourceId);
    ...
    const searchTerms = await this.store.prepareMetadataUpdateSearchTerms(track.id, update);
    this.assertSourceWorkCurrent(context, signal);
    return this.store.updateTrackMetadata(track.id, update, searchTerms);
  }
  ...
  private jobKey(job: Pick<QueueJob, 'sourceId' | 'revision' | 'trackId' | 'kind' | 'groupKey'>): string {
    return `${job.sourceId}:${job.revision}:${job.kind}:${job.groupKey ?? job.trackId}`;
  }
  ...
  private canStart(job: QueueJob): boolean {
    if (!this.isSourceRevisionCurrent(job.sourceId, job.revision) || this.globalPaused || this.pausedSources.has(job.sourceId)) {
      return false;
    }
    ...
    const runningForSource = this.runningCountsForSource(job.sourceId)[job.kind];
  ...
  private runningCountsForSource(sourceId: string): Record<RemoteBackgroundJobKind, number> {
    const counts = zeroCounts();
    const revision = this.getSourceRevision(sourceId);
    for (const job of this.running.values()) {
      if (job.sourceId === sourceId && job.revision === revision) {
        counts[job.kind] += 1;
      }
    }
    return counts;
  ...
  private getSourceRevision(sourceId: string): number {
    return this.sourceRevisions.get(sourceId) ?? 0;
  }

  private isSourceRevisionCurrent(sourceId: string, revision: number): boolean {
    return this.getSourceRevision(sourceId) === revision;
  }

  private assertSourceWorkCurrent(context: SourceRevisionContext, signal?: AbortSignal): void {
    if (signal?.aborted || !this.isSourceRevisionCurrent(context.sourceId, context.revision)) {
      throw abortError();
    }
  }

  private requireEnabledSource(sourceId: string, revision: number, signal?: AbortSignal): void {
    this.assertSourceWorkCurrent({ sourceId, revision }, signal);
