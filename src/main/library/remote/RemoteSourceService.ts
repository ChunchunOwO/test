...
import type { RemoteLyricsResult, RemoteSourceAdapter } from './remoteTypes';
...
import { subscribeRemoteSourcePlaybackActivity } from './RemoteSourcePlaybackActivity';
...
export class RemoteSourceService {
    ...
    this.syncService = new RemoteLibrarySyncService(this.store, (provider) => this.getAdapter(provider), () => this.invalidateSourceListCache(), (sourceId, status, options) => {
      ...
      if (status.status === 'completed') {
        ...
        const kinds: RemoteBackgroundJobKind[] = options.includeCover === false || shouldDeferProviderCovers
          ? ['metadata', 'duration-backfill', 'lyrics']
          : ['metadata', 'duration-backfill', 'cover', 'lyrics'];
  ...
  async testSource(sourceIdOrInput: string | RemoteSourceInput): Promise<TestRemoteSourceResult> {
    ...
    const revision = typeof sourceIdOrInput === 'string' ? this.syncService.getSourceRevision(source.id) : null;
    const result = await adapter.testConnection({ source });
    if (
      typeof sourceIdOrInput === 'string'
      && revision !== null
      && this.syncService.isSourceRevisionCurrent(source.id, revision)
    ) {
  ...
  refreshTrackMetadata(trackId: string): Promise<RemoteLibraryTrack | null> {
    ...
    this.requireEnabledSource(track.sourceId);
  ...
  async createStreamUrl(input: { trackId: string }): Promise<RemoteStreamUrlResult> {
    const track = this.store.getTrack(input.trackId);
    if (!track || track.availability !== 'available') {
      throw new Error(`Remote track ${input.trackId} is not available`);
    }

    const source = this.requireEnabledSource(track.sourceId);
    return this.getAdapter(source.provider).createStreamUrl({ source, remotePath: track.remotePath, stableKey: track.stableKey });
  }
  ...
  async readRemoteCover(trackId: string, size = 512, signal?: AbortSignal): Promise<RemoteCoverResult> {
    const track = this.store.getTrack(trackId);
    ...
    return adapter.readCover({ source, item, size, signal });
  }

  async readRemoteLyrics(trackId: string, signal?: AbortSignal): Promise<RemoteLyricsResult | null> {
    const track = this.store.getTrack(trackId);
    if (!track || track.provider !== 'subsonic') {
      return null;
    }
    ...
    const source = this.requireEnabledSource(track.sourceId);
    const adapter = this.getAdapter(track.provider);
    if (!adapter.readLyrics) {
      return null;
    }

    return adapter.readLyrics({
      source,
  ...
  async readSubsonicCoverByIdentity(sourceId: string, coverArt: string, size = 512, signal?: AbortSignal): Promise<RemoteCoverResult> {
    const source = this.requireEnabledSource(sourceId);
  ...
  private cancelSourceWork(sourceId: string): void {
    this.syncService.invalidateSource(sourceId);
    this.backgroundQueue.cancelSource(sourceId);
    const adapters: RemoteSourceAdapter[] = [this.webdavAdapter, this.jellyfinAdapter, this.embyAdapter, this.subsonicAdapter, this.smbAdapter, this.sshfsAdapter];
    for (const adapter of adapters) {
      adapter.clearSourceState?.(sourceId);
    }
  }
  ...
  private inputToTransientSource(input: RemoteSourceInput) {
    const authType = input.provider === 'subsonic' ? 'basic' : input.authType ?? 'basic';
    ...
    return {
      ...
      baseUrl: normalizeRemoteSourceBaseUrl(input.provider, input.baseUrl, authType),
      username: input.username ?? null,
      authType,
      config: sanitizeRemoteSourceConfig(input.provider, input.config),
...
let unsubscribeDefaultRemoteSourcePlaybackActivity: (() => void) | null = null;
...
export const getRemoteSourceService = (): RemoteSourceService => {
  ...
  if (!defaultRemoteSourceService) {
    ...
    unsubscribeDefaultRemoteSourcePlaybackActivity = subscribeRemoteSourcePlaybackActivity((activity) => {
      defaultRemoteSourceService?.setPlaybackActive(activity.active, {
        lowLoadEnhanced: activity.lowLoadEnhanced,
      });
    });
  }
...
export const closeDefaultRemoteSourceService = async (): Promise<void> => {
  ...
  unsubscribeDefaultRemoteSourcePlaybackActivity?.();
  unsubscribeDefaultRemoteSourcePlaybackActivity = null;
  await service.close();
};
