...
import { invalidateRemoteServerLibraryReadModel } from './RemoteServerLibraryReadModel';
import {
...
const supportedRemoteSourceProviderSql = "'webdav', 'jellyfin', 'emby', 'smb', 'sshfs', 'subsonic'";
const supportedRemoteSourceProviders = new Set<RemoteSourceProvider>(['webdav', 'jellyfin', 'emby', 'smb', 'sshfs', 'subsonic']);
...
export class RemoteLibraryStore {
  constructor(
    ...
    this.hardenPersistedSources();
  }

  private invalidateServerLibraryReadModel(): void {
    invalidateRemoteServerLibraryReadModel(this.database);
  }
    ...
    return this.database
      .prepare<[], DbRow>(
        `SELECT remote_sources.*,
         ...
         FROM remote_sources
         WHERE provider IN (${supportedRemoteSourceProviderSql})
  ...
  getOverview(sourceId?: string | null): RemoteSourceOverview {
    const sourceFilter = textOrNull(sourceId)
      ? `WHERE remote_sources.provider IN (${supportedRemoteSourceProviderSql}) AND remote_sources.id = ?`
      : `WHERE remote_sources.provider IN (${supportedRemoteSourceProviderSql})`;
      ...
      .prepare<string[], DbRow>(
        `SELECT
         ...
         WHERE remote_tracks.availability != 'missing'
           ...
           AND remote_sources.provider IN (${supportedRemoteSourceProviderSql})
           ${sourceFilter}`,
      )
      .all(...params)
    ...
    const row = this.database
      .prepare<[string], DbRow>(
        `SELECT remote_sources.*,
          (SELECT COUNT(*) FROM remote_tracks WHERE remote_tracks.source_id = remote_sources.id AND availability != 'missing') AS indexed_track_count
         FROM remote_sources
         WHERE id = ? AND provider IN (${supportedRemoteSourceProviderSql})`,
      )
      .get(id);
  ...
  hasSource(id: string): boolean {
    const row = this.database
      .prepare<[string], { found: number }>(`SELECT 1 AS found FROM remote_sources WHERE id = ? AND provider IN (${supportedRemoteSourceProviderSql})`)
      .get(id);
    return row?.found === 1;
  }
  ...
  getSourceConfig(id: string): Record<string, unknown> | null {
    const row = this.database
      .prepare<[string], { provider: string; config_json: string | null }>(`SELECT provider, config_json FROM remote_sources WHERE id = ? AND provider IN (${supportedRemoteSourceProviderSql})`)
      .get(id);
    return row ? sanitizeRemoteSourceConfig(providerOrWebdav(row.provider), parseJsonObject(row.config_json)) : null;
  }

  getSourceWithSecret(id: string): RemoteSourceSecret | null {
    const row = this.database
      .prepare<[string], DbRow>(`SELECT * FROM remote_sources WHERE id = ? AND provider IN (${supportedRemoteSourceProviderSql})`)
      .get(id);
    if (!row) {
      return null;
    }

    const encryptedSecret = textOrNull(row.encrypted_secret);
    const secret = this.secretStore.decrypt(encryptedSecret);
    if (encryptedSecret && !secret) {
      throw new Error('Stored remote source credentials cannot be decrypted. Re-enter the credentials before reconnecting.');
    }
    ...
    source.baseUrl = normalizeRemoteSourceBaseUrl(source.provider, source.baseUrl, source.authType);
    return source;
  }
  ...
  createSource(input: RemoteSourceInput): RemoteSource {
    ...
    const id = randomUUID();
    if (!supportedRemoteSourceProviders.has(input.provider)) {
      throw new Error(`Unsupported remote source provider ${input.provider}`);
    }
    const provider = input.provider;
    const displayName = input.displayName.trim().slice(0, 200) || provider.toUpperCase();
    const username = input.username?.trim().slice(0, 512) || null;
    const requestedAuthType = authTypeOrBasic(input.authType);
    const authType = provider !== 'subsonic' && requestedAuthType === 'basic' && !username && !input.secret ? 'none' : requestedAuthType;
    const baseUrl = normalizeRemoteSourceBaseUrl(provider, input.baseUrl, authType);
    assertRemoteSourceConfigInput(input.config);
    const config = sanitizeRemoteSourceConfig(provider, input.config);
    ...
    this.invalidateServerLibraryReadModel();
  ...
  updateSource(input: RemoteSourceUpdate): RemoteSource {
    ...
    if (input.provider !== undefined && input.provider !== current.provider) {
      throw new Error('Remote source provider cannot be changed; create a new source instead.');
    }

    const provider = current.provider;
    const displayName = input.displayName !== undefined ? input.displayName.trim().slice(0, 200) || current.displayName : current.displayName;
    const authType = input.authType ? authTypeOrBasic(input.authType) : current.authType;
    const baseUrl = normalizeRemoteSourceBaseUrl(provider, input.baseUrl !== undefined ? input.baseUrl : current.baseUrl, authType);
    const username = input.username !== undefined ? input.username?.trim().slice(0, 512) || null : current.username;
    const credentialScopeChanged = baseUrl !== current.baseUrl || username !== current.username || authType !== current.authType;
    const encryptedSecret = this.getEncryptedSecret(input.id);
    if (credentialScopeChanged && encryptedSecret && input.secret === undefined) {
      throw new Error('Credentials must be re-entered when the endpoint, username, or authentication type changes.');
    }
    const secret =
      input.secret !== undefined ? this.secretStore.encrypt(input.secret) : encryptedSecret;
    assertRemoteSourceConfigInput(input.config);
    const config = sanitizeRemoteSourceConfig(provider, input.config ?? current.config);
    ...
    return this.database
      .prepare<unknown[], DbRow>(
        `SELECT id, source_id, remote_path, stable_key, title, artist, album, duration, codec, cover_id,
          metadata_status, cover_status, lyrics_status, availability, provider, field_sources_json
         FROM remote_tracks
  ...
  beginSyncEnumeration(): string {
    this.database.exec(
      `CREATE TEMP TABLE IF NOT EXISTS remote_sync_seen_paths (
         run_id TEXT NOT NULL,
         source_id TEXT NOT NULL,
         remote_path TEXT NOT NULL,
         PRIMARY KEY (run_id, source_id, remote_path)
       ) WITHOUT ROWID`,
    );
    return randomUUID();
  }

  recordSyncEnumerationPaths(runId: string, sourceId: string, remotePaths: string[]): void {
    if (remotePaths.length === 0) {
      return;
    }

    const statement = this.database.prepare(
      'INSERT OR IGNORE INTO remote_sync_seen_paths (run_id, source_id, remote_path) VALUES (?, ?, ?)',
    );
    this.database.transaction(() => {
      for (const remotePath of remotePaths) {
        statement.run(runId, sourceId, remotePath);
      }
    })();
  }

  countMissingFromSyncEnumeration(runId: string, sourceId: string): number {
    const row = this.database
      .prepare<[string, string], { count: number }>(
        `SELECT COUNT(*) AS count
         ...
         WHERE remote_tracks.source_id = ?
           AND NOT EXISTS (
             SELECT 1
             FROM remote_sync_seen_paths
             WHERE remote_sync_seen_paths.run_id = ?
               AND remote_sync_seen_paths.source_id = remote_tracks.source_id
               AND remote_sync_seen_paths.remote_path = remote_tracks.remote_path
           )`,
      )
      .get(sourceId, runId);
  ...
  markMissingFromSyncEnumeration(runId: string, sourceId: string): number {
    const missingCount = this.countMissingFromSyncEnumeration(runId, sourceId);
    if (missingCount === 0) {
      return 0;
    }
    ...
    this.database
      ...
      .run(nowIso(), sourceId, runId);
    this.invalidateServerLibraryReadModel();
    return missingCount;
  }

  finishSyncEnumeration(runId: string): void {
    this.database.prepare('DELETE FROM remote_sync_seen_paths WHERE run_id = ?').run(runId);
  }
  ...
  updateTrackJobStatus(trackId: string, kind: RemoteBackgroundJobKind, status: RemoteLibraryTrack['metadataStatus']): void {
    const column = kind === 'cover' ? 'cover_status' : kind === 'lyrics' ? 'lyrics_status' : 'metadata_status';
  ...
  removeMissingTracks(sourceId: string): number {
    const changed = this.database.prepare('DELETE FROM remote_tracks WHERE source_id = ? AND availability = ?').run(sourceId, 'missing').changes;
    if (changed > 0) {
      this.invalidateServerLibraryReadModel();
    }
    return changed;
  ...
  private hardenPersistedSources(): void {
    const rows = this.database
      .prepare<[], {
        id: string;
        provider: string;
      ...
      }>('SELECT id, provider, base_url, auth_type, encrypted_secret, config_json, status, last_error FROM remote_sources')
      .all();
    const update = this.database.prepare(
      `UPDATE remote_sources SET base_url = ?, encrypted_secret = ?, config_json = ?, status = ?, last_error = ?, updated_at = ? WHERE id = ?`,
    );
    const scrubUnsupportedBaidu = this.database.prepare(
      `UPDATE remote_sources
       SET base_url = NULL, username = NULL, auth_type = 'none', encrypted_secret = NULL, config_json = '{}', status = 'disabled',
           last_error = ?, updated_at = ?
       WHERE id = ?`,
    );
    const deleteRetiredProviderTracks = this.database.prepare('DELETE FROM remote_tracks WHERE source_id = ?');
    const deleteRetiredProviderScanCache = this.database.prepare('DELETE FROM remote_provider_scan_cache WHERE source_id = ?');
    const deleteRetiredProviderCoverCache = this.database.prepare('DELETE FROM remote_cover_cache WHERE source_id = ?');

    this.database.transaction(() => {
      for (const row of rows) {
        if (row.provider === 'baidu') {
          deleteRetiredProviderTracks.run(row.id);
          deleteRetiredProviderScanCache.run(row.id);
          deleteRetiredProviderCoverCache.run(row.id);
          scrubUnsupportedBaidu.run('Baidu Netdisk support has been removed. Saved credentials were cleared.', nowIso(), row.id);
          continue;
        }
        if (!supportedRemoteSourceProviders.has(row.provider as RemoteSourceProvider)) {
          continue;
        }

        const provider = row.provider as RemoteSourceProvider;
        const authType = authTypeOrBasic(row.auth_type);
        const migratedSecret = this.secretStore.migrateLegacyPlaintext(row.encrypted_secret);
        const encryptedSecret = migratedSecret === undefined ? row.encrypted_secret : migratedSecret;
        const configJson = JSON.stringify(sanitizeRemoteSourceConfig(provider, parseJsonObject(row.config_json)));
        let baseUrl = row.base_url;
        let status: RemoteSourceStatus = statusOrEnabled(row.status);
        let lastError = row.last_error;
        try {
          baseUrl = normalizeRemoteSourceBaseUrl(provider, row.base_url, authType);
        } catch (error) {
          status = 'error';
          lastError = error instanceof Error ? error.message : 'Remote source configuration was blocked.';
        }
        if (migratedSecret === null && row.encrypted_secret?.startsWith('plain:')) {
          status = 'error';
          lastError = 'Legacy plaintext credentials were cleared because secure credential storage is unavailable.';
        }
        if (
          ...
          update.run(baseUrl, encryptedSecret, configJson, status, lastError, nowIso(), row.id);
        }
      }
    })();
  }
  ...
  private mapSource(row: DbRow): RemoteSource {
    const provider = providerOrWebdav(row.provider);
    return {
      id: String(row.id),
      provider,
      ...
      config: sanitizeRemoteSourceConfig(provider, parseJsonObject(row.config_json)),
  ...
  private mapTrackLookup(row: DbRow): RemoteTrackLookupItem {
    ...
    return {
      ...
      coverThumb: coverId
        ...
        : subsonicDirectCoverUrlFor(row.id, row.source_id, row.provider, coverId, fieldSources, row.remote_path, row.stable_key, 160),
