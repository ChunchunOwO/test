...
export class LyricsBackfillJobQueue {
  ...
  private finishJob(job: MutableJobStatus, status: MutableJobStatus['status']): void {
    ...
    this.flushAttemptedTrackIds(job.id);
    this.persistJob(job, true);
    this.releaseFinishedJobState(job.id);
  }

  private releaseFinishedJobState(jobId: string): void {
    this.runOptions.delete(jobId);
    this.targetTrackIds.delete(jobId);
    this.attemptedTrackIds.delete(jobId);
    this.pendingAttemptedTrackIds.delete(jobId);
    this.lastPersistedAt.delete(jobId);
  }
