...
// `technical` is a legacy source label used by older scans even when the value
// came from an embedded ALBUMARTIST tag. Treat the value as usable for grouping
// so a partially refreshed folder does not split one physical album in two.
const reliableAlbumArtistSources = new Set(['embedded', 'manual', 'network', 'sidecar', 'technical']);
