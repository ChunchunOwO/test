...
describe('AlbumService', () => {
  ...
  it('standard strategy keeps legacy technical and embedded album artist rows together', () => {
    const service = new AlbumService();
    const legacy = service.makeAlbumKey(makeInput({ albumArtist: '星街すいせい', albumArtistSource: 'technical' }));
    const refreshed = service.makeAlbumKey(
      makeInput({
        albumArtist: '星街すいせい',
        albumArtistSource: 'embedded',
        filePath: 'D:\\Other\\Specter\\B.flac',
        trackId: 'track-b',
      }),
    );

    expect(legacy).toBe(refreshed);
  });
