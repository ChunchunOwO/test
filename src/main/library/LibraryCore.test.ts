...
const makeTempRoot = (): string => {
  const root = join(tmpdir(), `echo-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
describe('Library Core', () => {
  ...
  it('filters songs independently by sample rate and audio file format', async () => {
    ...
    const mp3Path = writeAudioFile(harness.folder, 'Compressed Track.mp3');
    const uncommonPath = writeAudioFile(harness.folder, 'Legacy Track.wma');
    [cdPath, hiResPath, dsdPath, otherPath, mp3Path, uncommonPath].forEach((path, index) => {
      files.push({
        path,
        sizeBytes: 100 + index,
    ...
    metadataByPath.set(mp3Path, metadataResult({
      title: 'Compressed Track',
      codec: 'MPEG 1 Layer 3',
      sampleRate: 44_100,
      bitDepth: null,
    }));
    metadataByPath.set(uncommonPath, metadataResult({
      title: 'Legacy Track',
      codec: 'WMA',
      sampleRate: 48_000,
      bitDepth: null,
    ...
    expect(harness.service.getTracks({ pageSize: 10, fileFormatFilter: 'flac' }).items.map((track) => track.title).sort()).toEqual(['CD Track', 'Hi Res Track']);
    expect(harness.service.getTracks({ pageSize: 10, fileFormatFilter: 'mp3' }).items.map((track) => track.title)).toEqual(['Compressed Track']);
    expect(harness.service.getTracks({ pageSize: 10, fileFormatFilter: 'wav' }).items.map((track) => track.title)).toEqual(['Podcast Rate']);
    expect(harness.service.getTracks({ pageSize: 10, fileFormatFilter: 'dsd' }).items.map((track) => track.title)).toEqual(['Native DSD']);
    expect(harness.service.getTracks({ pageSize: 10, fileFormatFilter: 'other' }).items.map((track) => track.title)).toEqual(['Legacy Track']);
    expect(harness.service.getTracks({ pageSize: 10, fileFormatFilter: 'flac', audioFormatFilter: 'sampleRate192000' }).items.map((track) => track.title)).toEqual(['Hi Res Track']);
    expect(harness.service.getTracks({ pageSize: 10, audioFormatFilter: 'all', fileFormatFilter: 'all' }).total).toBe(6);
  ...
  it('loading embedded track tags refreshes persisted text tags and embedded cover', async () => {
    ...
    expect(result.tags.composer).toBe('草东没有派对');
    expect(result.tags.totalTracks).toBe(12);
    expect(result.tags.totalDiscs).toBe(1);
    expect(result.tags.bpm).toBe(108);
    expect(result.tags.comment).toBe('studio release');
  ...
  it('sameTitleAndCover album grouping keeps exact titles split across artists without matching covers', async () => {
    ...
    expect(albums.total).toBe(4);
    expect(albums.items.map((album) => album.trackCount)).toEqual([1, 1, 1, 1]);
    harness.cleanup();
  });
  ...
  it('sameTitleAndCover album grouping keeps normalized titles split without matching covers', async () => {
    const harness = createHarness({ coverExtractor: new FakeCoverExtractor() });
    harness.setAlbumMergeStrategy('sameTitleAndCover');
    const firstFolder = join(harness.folder, 'disc-a');
    const secondFolder = join(harness.folder, 'disc-b');
  ...
  it('sameTitleAndCover album grouping keeps similar titles split when covers differ', async () => {
    const harness = createHarness({ coverExtractor: new FakeCoverExtractor() });
    harness.setAlbumMergeStrategy('sameTitleAndCover');
    const first = writeAudioFile(harness.folder, 'A.flac');
    const second = writeAudioFile(harness.folder, 'B.flac');
  ...
  it('sameTitleAndCover album grouping keeps same title split when covers differ', async () => {
    const harness = createHarness({ coverExtractor: new FakeCoverExtractor() });
    harness.setAlbumMergeStrategy('sameTitleAndCover');
    const first = writeAudioFile(harness.folder, 'A.flac');
    const second = writeAudioFile(harness.folder, 'B.flac');
  ...
  it('sameTitleAndCover album grouping keeps same title split when cover hash is missing', async () => {
  ...
  it('getTracks sorts discovery fields in SQLite and keeps unknown metadata last', () => {
    ...
    try {
      ...
      expect(service.getTracks({ pageSize: 10, sort: 'playCountAsc', sorts: ['playCountAsc', 'yearDesc'] }).items.map((track) => track.title))
        .toEqual(['Classic', 'Unknown', 'Recent', 'Modern']);
    } finally {
      service.close();
    }
  });
  ...
  it('playback stats dashboard aggregates top tracks, artists, formats, and quality', async () => {
    ...
    expect(stats.insights?.sessions).toMatchObject({
      sessionCount: 1,
      activeDays: 1,
      longestStreakDays: 1,
    });
    expect(stats.insights?.discovery).toMatchObject({
      hasBoundedRange: false,
      newTrackCount: 2,
      replayedTrackCount: 1,
      rediscoveredTrackCount: 0,
    });
    expect(stats.insights?.discovery.repeatPlayRate).toBeCloseTo(1 / 3);

    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);
    const todayStats = harness.service.getPlaybackStatsDashboard({
      from: today.toISOString(),
      to: tomorrow.toISOString(),
    });
    expect(todayStats.insights?.comparison).toMatchObject({
      current: { playCount: 3, playedSeconds: 145, uniqueTracks: 2 },
      previous: { playCount: 0, playedSeconds: 0, uniqueTracks: 0 },
    });
    expect(todayStats.insights?.discovery).toMatchObject({
      hasBoundedRange: true,
      newTrackCount: 2,
      returningTrackCount: 0,
    });
    harness.cleanup();
  });
