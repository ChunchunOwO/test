...
type LyricsBackfillQueueTransientState = {
  runOptions: Map<string, unknown>;
  targetTrackIds: Map<string, string[]>;
  attemptedTrackIds: Map<string, Set<string>>;
  pendingAttemptedTrackIds: Map<string, Set<string>>;
  lastPersistedAt: Map<string, number>;
};

const transientStateFor = (queue: LyricsBackfillJobQueue): LyricsBackfillQueueTransientState =>
  queue as unknown as LyricsBackfillQueueTransientState;
...
describe('LyricsBackfillJobQueue', () => {
  ...
  it('releases completed job target and attempt collections after the final persistence flush', async () => {
    const tracks = [track('one'), track('two')];
    const persistedAttemptedTrackIds = new Set<string>();
    const persistence = {
      saveJob: vi.fn(),
      saveTargetIds: vi.fn(),
      markTrackAttempted: vi.fn((_jobId: string, trackId: string) => {
        persistedAttemptedTrackIds.add(trackId);
      }),
      loadLatestStatus: vi.fn(() => null),
      loadLatestResumableJob: vi.fn(() => null),
    } as unknown as LyricsBackfillJobPersistence;
    const queue = new LyricsBackfillJobQueue(
      (query) => pageFor(tracks, query),
      {
        getLyricsService: async () => ({
          hasCachedLyricsForTrack: () => false,
          getLyricsForTrack: async (trackId) => syncedLyrics(trackId),
        }),
        persistence,
        isPlaybackActive: () => false,
      },
    ...
    const started = queue.start({ mode: 'quick', limit: 10, concurrency: 1 });
    const finished = await waitForTerminalStatus(queue, started.id);
    const transientState = transientStateFor(queue);

    expect(finished.status).toBe('completed');
    expect(persistedAttemptedTrackIds).toEqual(new Set(['one', 'two']));
    expect(persistence.saveJob).toHaveBeenLastCalledWith(
      expect.objectContaining({ id: started.id, status: 'completed' }),
      expect.any(Object),
    );
    expect(transientState.runOptions.has(started.id)).toBe(false);
    expect(transientState.targetTrackIds.has(started.id)).toBe(false);
    expect(transientState.attemptedTrackIds.has(started.id)).toBe(false);
    expect(transientState.pendingAttemptedTrackIds.has(started.id)).toBe(false);
    expect(transientState.lastPersistedAt.has(started.id)).toBe(false);
    expect(queue.getStatus(started.id)).toEqual(finished);
  });

  it('releases transient collections after an in-flight job is cancelled', async () => {
    let releaseLookup = (): void => undefined;
    const lookupGate = new Promise<void>((resolve) => {
      releaseLookup = () => resolve();
    });
    const getLyricsForTrack = vi.fn(async (trackId: string) => {
      await lookupGate;
      return syncedLyrics(trackId);
    });
    const queue = new LyricsBackfillJobQueue(
      (query) => pageFor([track('cancelled')], query),
      {
        getLyricsService: async () => ({
          hasCachedLyricsForTrack: () => false,
          getLyricsForTrack,
    ...
    const started = queue.start({ mode: 'quick', limit: 10, concurrency: 1 });
    await vi.waitFor(() => expect(getLyricsForTrack).toHaveBeenCalledTimes(1));
    expect(queue.cancel(started.id).status).toBe('cancelled');
    releaseLookup();
    ...
    expect(queue.getStatus(started.id).status).toBe('cancelled');
  });
});
