...
import {
  ...
  type PluginPlayerBarActionContribution,
  type PluginPlayerBarActionIcon,
...
const pluginPlayerBarActionIconSet = new Set<PluginPlayerBarActionIcon>(pluginPlayerBarActionIcons);
const settingTypes = new Set<PluginSettingType>(['string', 'select', 'boolean', 'number', 'secret']);
const pluginIconExtensions = new Set(['.svg']);
  ...
  try {
    ...
    return {
      ...
      placement: typeof input.placement === 'string'
        && (pluginPanelPlacements as readonly string[]).includes(input.placement)
        ? input.placement as PluginPanelContribution['placement']
        : 'main',
    };
  } catch {
    return null;
...
const normalizePlayerBarAction = (value: unknown): PluginPlayerBarActionContribution | null => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return null;
  }

  const input = value as Partial<PluginPlayerBarActionContribution>;
  try {
    if (input.icon !== undefined
      && (typeof input.icon !== 'string' || !pluginPlayerBarActionIconSet.has(input.icon as PluginPlayerBarActionIcon))) {
      return null;
    }
    return {
      id: normalizePluginId(input.id),
      title: asText(input.title, 'player bar action title', 80),
      commandId: normalizePluginId(input.commandId),
      description: typeof input.description === 'string' && input.description.trim()
        ? input.description.trim().slice(0, 180)
        : undefined,
      icon: input.icon as PluginPlayerBarActionIcon | undefined ?? 'blocks',
    };
  } catch {
    return null;
  }
...
const normalizeAgent = (value: unknown): PluginAgentContribution | null => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return null;
  }

  const input = value as Partial<PluginAgentContribution>;
  try {
    return {
      id: normalizePluginId(input.id),
      title: asText(input.title, 'agent title', 80),
      description: typeof input.description === 'string' && input.description.trim()
        ? input.description.trim().slice(0, 180)
        : undefined,
      inputPlaceholder: typeof input.inputPlaceholder === 'string' && input.inputPlaceholder.trim()
        ? input.inputPlaceholder.trim().slice(0, 120)
        : undefined,
    };
  } catch {
    return null;
...
const normalizeContributes = (value: unknown): PluginManifestContributes => {
  ...
  return {
    ...
    sourceProviders: Array.isArray(input.sourceProviders)
      ...
      : [],
    playerBarActions: Array.isArray(input.playerBarActions)
      ? input.playerBarActions
          .map(normalizePlayerBarAction)
          .filter((item): item is PluginPlayerBarActionContribution => Boolean(item))
      : [],
    agents: Array.isArray(input.agents)
      ? input.agents
          .map(normalizeAgent)
          .filter((item): item is PluginAgentContribution => Boolean(item))
          .slice(0, 8)
      : [],
...
export const normalizePluginManifest = (value: unknown, directoryName = ''): PluginManifest => {
  ...
  const icon = normalizeRelativeFilePath(input.icon, 'icon', null);
  ...
  if (icon && !pluginIconExtensions.has(extname(icon).toLowerCase())) {
  ...
  return {
    ...
    ...(typeof input.description === 'string' && input.description.trim() ? { description: input.description.trim().slice(0, 600) } : {}),
    ...(icon ? { icon } : {}),
    entry,
    panel,
