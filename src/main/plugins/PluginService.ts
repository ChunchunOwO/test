...
const maxPluginUploadBytes = 2 * 1024 * 1024 * 1024;
...
const pluginPackageType = 'echo-plugin-package';
const legacyPluginPackageTypes = new Set(['echo-plugin-package', 'echo-next-plugin-package']);
const pluginPackageVersion = 1;
const pluginPackageExtension = '.echomod';
const legacyPluginPackageExtensions = ['.echo', '.echo-plugin.json'];
...
const exportablePluginFileExtensions = new Set(['.js', '.mjs', '.cjs', '.html', '.css', '.json', '.md', '.txt', '.sig', '.svg']);
...
const isPluginPackagePath = (value: string): boolean =>
  value.endsWith(pluginPackageExtension) || legacyPluginPackageExtensions.some((extension) => value.endsWith(extension));
...
const assertEchoProActivationPackage = (packageText: string): void => {
  ...
  if (
    !isRecord(parsed) ||
    (typeof parsed.type !== 'string' || !legacyPluginPackageTypes.has(parsed.type)) ||
    parsed.version !== pluginPackageVersion
  ) {
    throw new Error('echo_pro_activation_package_invalid');
  }
...
export class PluginService {
    ...
    return Object.freeze({
      ...
      playback: Object.freeze({
        ...
        load: async (request: unknown) => {
          ...
          if (!filePath || filePath.length > 4_000) {
            throw new Error('plugin_playback_file_invalid');
          }
          const trackId = typeof input.trackId === 'string' && input.trackId.trim() ? input.trackId.trim().slice(0, 240) : filePath;
          const metadata = isRecord(input.metadata) ? input.metadata : undefined;
          return jsonClone(await getAudioSession().playLocalFile({
            filePath,
            trackId,
            mimeType: typeof input.mimeType === 'string' ? input.mimeType.slice(0, 120) : undefined,
            metadata,
            startSeconds: typeof input.startSeconds === 'number' && Number.isFinite(input.startSeconds) ? Math.max(0, input.startSeconds) : undefined,
          }));
        },
      }),
      ...
      net: Object.freeze({
        ...
        getFileInfo: async (request: unknown) => this.getPluginFileInfo(record, request),
        uploadFile: async (request: unknown) => this.uploadPluginFile(record, request),
      }),
  ...
  private async uploadPluginFile(record: PluginRecord, request: unknown): Promise<unknown> {
    if (!record.manifest) {
      throw new Error('plugin_manifest_invalid');
    }
    if (record.manifest.apiVersion < 2) {
    ...
    if (!record.trustedPermissions.includes('network') || !record.trustedPermissions.includes('playback:read')) {
      throw new Error('plugin_permission_denied:network');
    }
    const input = isRecord(request) ? request : {};
    const url = typeof input.url === 'string' ? input.url.trim() : '';
    const filePath = typeof input.filePath === 'string' ? input.filePath.trim() : '';
    if (!url || !isSafeHttpUrl(url) || !filePath || filePath.length > 4_000) {
      throw new Error('plugin_upload_request_invalid');
    }
    const currentFilePath = getAudioSession().getStatus().currentFilePath;
    if (currentFilePath !== filePath) {
      throw new Error('plugin_upload_file_must_be_current_track');
    }
    const stats = statSync(filePath);
    if (!stats.isFile() || stats.size <= 0 || stats.size > maxPluginUploadBytes) {
      throw new Error('plugin_upload_file_size_invalid');
    }
  ...
  private getPluginFileInfo(record: PluginRecord, request: unknown): { filePath: string; sizeBytes: number; fileName: string } {
    if (!record.manifest || !record.trustedPermissions.includes('playback:read')) {
      throw new Error('plugin_permission_denied:playback:read');
    }
    const input = isRecord(request) ? request : {};
    const filePath = typeof input.filePath === 'string' ? input.filePath.trim() : '';
    if (!filePath || getAudioSession().getStatus().currentFilePath !== filePath) {
      throw new Error('plugin_upload_file_must_be_current_track');
    }
    const stats = statSync(filePath);
    if (!stats.isFile() || stats.size <= 0 || stats.size > maxPluginUploadBytes) {
    ...
    return {
      ...
      fileName: basename(filePath).slice(0, 180),
    };
  }
  ...
  private toSummary(record: PluginRecord): PluginSummary {
    ...
    const echoProLicense = isEchoProUnlockManifest(manifest)
      ...
      : null;
    const iconDataUrl = this.readPluginIconDataUrl(record);
  ...
  private readPluginIconDataUrl(record: PluginRecord): string | null {
    const icon = record.manifest?.icon;
    if (!icon) {
      return null;
    }
    try {
      const iconPath = join(record.directory, icon);
      const stats = statSync(iconPath);
      if (!stats.isFile() || stats.size <= 0 || stats.size > 512 * 1024) {
        return null;
      }
      const mime = extname(iconPath).toLowerCase() === '.svg' ? 'image/svg+xml' : 'image/*';
  ...
  private async chooseExportPath(manifest: PluginManifest): Promise<string | null> {
    const result = await dialog.showSaveDialog({
      ...
      filters: [
        { name: 'ECHO mod package', extensions: ['echomod', 'echo'] },
