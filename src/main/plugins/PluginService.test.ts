...
vi.mock('electron', () => {
  const electronMock = {
    app: {
      getPath: () => join(tmpdir(), 'echo-plugin-service-userdata'),
...
describe('PluginService', () => {
  ...
  beforeEach(() => {
    ...
    pluginRoot = mkdtempSync(join(tmpdir(), 'echo-plugin-service-'));
  ...
  it('exports and imports plugin packages without runtime storage', async () => {
    ...
    expect(payload.type).toBe('echo-plugin-package');
    ...
    const importRoot = mkdtempSync(join(tmpdir(), 'echo-plugin-import-'));
    try {
