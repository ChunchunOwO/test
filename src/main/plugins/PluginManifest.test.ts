...
describe('plugin manifest validation', () => {
  it('normalizes a valid editable local plugin manifest with v2 contributions', () => {
    const manifest = normalizePluginManifest({
      ...
      contributes: {
        ...
        playerBarActions: [{ id: 'Quick_Status', title: 'Quick Status', commandId: 'Show_Status', icon: 'sparkles' }],
        panels: [{ id: 'Main', title: 'Main Panel', path: 'panel.html', placement: 'home' }],
    ...
    expect(manifest.contributes?.playerBarActions?.[0]).toMatchObject({
      id: 'quick_status',
      commandId: 'show_status',
      icon: 'sparkles',
    });
    expect(manifest.contributes?.panels?.[0]).toMatchObject({ id: 'main', path: 'panel.html', placement: 'home' });
  ...
  it('rejects host-controlled plugin panel contributions', () => {
    const manifest = normalizePluginManifest({
      id: 'osu.downloader',
      name: 'osu!downloader',
      version: '1.0.0',
