...
export type HqPlayerMediaResolver = {
  createRemoteStreamUrl: (input: { trackId: string }) => Promise<RemoteStreamUrlResult>;
...
const defaultMediaResolver: HqPlayerMediaResolver = {
  ...
  resolveStreamingPlayback: async () => {
    throw new Error('Streaming playback is not available in the Steam distribution.');
  },
};
