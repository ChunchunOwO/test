...
const togglePetWindowMock = vi.fn();
let minimized = false;
let destroyed = false;
...
vi.mock('./petWindow', () => ({
  togglePetWindow: togglePetWindowMock,
}));
...
describe('global playback shortcuts', () => {
  ...
  it('toggles the pet directly from the main process', async () => {
    currentSettings = createSettings({
      globalShortcuts: {
        ...createDefaultGlobalShortcuts(),
        togglePet: { enabled: true, accelerator: 'Ctrl+Alt+P' },
      },
    });
    const shortcuts = await import('./backgroundPlaybackShortcuts');

    shortcuts.refreshBackgroundSpaceRegistration();
    callbacks.get('Ctrl+Alt+P')?.();

    expect(togglePetWindowMock).toHaveBeenCalledTimes(1);
