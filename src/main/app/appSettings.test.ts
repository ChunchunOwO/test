...
describe('app settings normalization', () => {
  ...
  it('preserves supported lyrics page styles', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({ lyricsPageStyle: 'editorial' }).lyricsPageStyle).toBe('editorial');
    expect(normalizeSettings({ lyricsPageStyle: 'folded' }).lyricsPageStyle).toBe('folded');
    expect(normalizeSettings({ lyricsPageStyle: 'cinemaStage' }).lyricsPageStyle).toBe('cinemaStage');
    expect(normalizeSettings({ lyricsPageStyle: 'kineticPoster' }).lyricsPageStyle).toBe('kineticPoster');
    expect(normalizeSettings({ lyricsPageStyle: 'coverStage' }).lyricsPageStyle).toBe('coverStage');
    expect(normalizeSettings({ lyricsPageStyle: 'cutBoard' }).lyricsPageStyle).toBe('cutBoard');
    expect(normalizeSettings({ lyricsPageStyle: 'unsupported' }).lyricsPageStyle).toBe('default');
  });

  it('keeps the lyrics theme filter enabled by default and preserves an explicit opt-out', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).lyricsThemeFilterEnabled).toBe(true);
    expect(normalizeSettings({ lyricsThemeFilterEnabled: false }).lyricsThemeFilterEnabled).toBe(false);
  });

  it('normalizes accessibility preferences including UI scale', async () => {
    const { normalizeSettings } = await import('./appSettings');
  ...
  it('keeps DSD passthrough and window acrylic free while Pro DSP stays locked until DLC ownership is verified', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).audioDsdOutputMode).toBe('dop');
    expect(normalizeSettings({}).audioAsioNativeDsdExperimentalEnabled).toBe(false);
    const locked = normalizeSettings({
    ...
    expect(locked.audioDsdOutputMode).toBe('dop');
    expect(locked.audioAsioNativeDsdExperimentalEnabled).toBe(true);
    expect(locked.audioSdmMode).toBe('off');
    expect(locked.audioEchoSrcMode).toBe('off');
    expect(locked.appWindowAcrylicEnabled).toBe(true);
    expect(normalizeSettings({ audioSdmMode: 'dsdPassthrough' }).audioSdmMode).toBe('dsdPassthrough');

    const unlocked = normalizeSettings({
      audioDsdOutputMode: 'dop',
      audioSdmMode: 'pcmToDsd',
      audioEchoSrcMode: 'family4x',
      appWindowAcrylicEnabled: true,
    }, { proFeaturesUnlocked: true });
    expect(unlocked.audioDsdOutputMode).toBe('dop');
    expect(unlocked.audioSdmMode).toBe('pcmToDsd');
    expect(unlocked.audioEchoSrcMode).toBe('family4x');
    expect(unlocked.appWindowAcrylicEnabled).toBe(true);
  });
  ...
  it('keeps old settings files compatible when coverCacheDir is missing', async () => {
    ...
    expect(settings.hiddenPlayerBarButtonIds).toEqual(['sleepTimer', 'audioExport']);
    ...
    expect(settings.lyricsMvGraphicsPressureGuardEnabled).toBe(true);
    expect(settings.dataProtectionDisabled).toBe(true);
    expect(settings.autoAccountCheckOnStartup).toBe(false);
    expect(settings.preventSleepWhilePlaying).toBe(false);
    expect(settings.autoPlayOnStartup).toBe(false);
    expect(settings.spotifyAutoLaunchOfficialPlayer).toBe(false);
    ...
    expect(settings.settingsHideSidebarEnabled).toBe(false);
    ...
    expect(settings.globalShortcuts?.togglePet).toEqual({ enabled: false, accelerator: null });
    ...
    expect(settings.audioDsdOutputMode).toBe('dop');
    expect(settings.audioAsioNativeDsdExperimentalEnabled).toBe(false);
    ...
    expect(settings.lyricsEmptyStateBackgroundEnabled).toBe(false);
    expect(settings.lyricsPlayerBarDrawerEnabled).toBe(true);
    expect(settings.lyricsPlayerBarDrawerAutoHideEnabled).toBe(true);
    expect(settings.lyricsPlayerBarDrawerCompactOnIdleEnabled).toBe(true);
    ...
    expect(settings.lyricsWordHighlightEnabled).toBe(false);
    ...
    expect(settings.lyricsThemeFilterEnabled).toBe(true);
    ...
    expect(settings.lyricsRoseVinylGradientEnabled).toBe(false);
    expect(settings.lyricsMusicReactiveVisualsEnabled).toBe(false);
    expect(settings.lyricsBackgroundMode).toBe('theme');
    expect(settings.lyricsBackgroundModeOverrideEnabled).toBe(false);
    ...
    expect(settings.miniPlayerUsesUltraLightMode).toBe(false);
    ...
    expect(settings.audioTransportFadeInMs).toBe(1500);
    expect(settings.audioTransportFadeOutMs).toBe(1500);
  ...
  it('normalizes hidden bottom-right player buttons', async () => {
    ...
    expect(normalizeSettings({}).hiddenPlayerBarButtonIds).toEqual(['sleepTimer', 'audioExport']);
    expect(
      normalizeSettings({
  ...
  it('keeps the lyrics/MV graphics pressure guard on by default with an explicit opt-out', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).lyricsMvGraphicsPressureGuardEnabled).toBe(true);
    expect(normalizeSettings({ lyricsMvGraphicsPressureGuardEnabled: true }).lyricsMvGraphicsPressureGuardEnabled).toBe(true);
    expect(normalizeSettings({ lyricsMvGraphicsPressureGuardEnabled: false }).lyricsMvGraphicsPressureGuardEnabled).toBe(false);
    expect(normalizeSettings({ lyricsMvGraphicsPressureGuardEnabled: 'true' as never }).lyricsMvGraphicsPressureGuardEnabled).toBe(true);
  });

  it('keeps album wall virtualization on by default with an explicit opt-out', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).albumWallVirtualizationEnabled).toBe(true);
    expect(normalizeSettings({ albumWallVirtualizationEnabled: true }).albumWallVirtualizationEnabled).toBe(true);
    expect(normalizeSettings({ albumWallVirtualizationEnabled: false }).albumWallVirtualizationEnabled).toBe(false);
    expect(normalizeSettings({ albumWallVirtualizationEnabled: 'true' as never }).albumWallVirtualizationEnabled).toBe(true);
  });
  ...
  it('loads settings files that were saved with a UTF-8 BOM', async () => {
    vi.resetModules();
    userDataPath = join(tmpdir(), `echo-settings-${Date.now()}-${Math.random().toString(16).slice(2)}`);
  ...
  it('keeps the home waveform visualizer disabled unless explicitly enabled', async () => {
    ...
    expect(normalizeSettings({ homeWaveformVisualizerEnabled: 'true' as never }).homeWaveformVisualizerEnabled).toBe(false);
  });
  ...
  it('normalizes track transition fade as a default-off customizable opt-in', async () => {
    const { normalizeSettings } = await import('./appSettings');
  ...
  it('normalizes online artist info provider settings as optional local text', async () => {
    ...
    expect(settings.onlineArtistInfoBandsintownAppId).toBe('echo');
  ...
  it('normalizes appearance theme presets', async () => {
    ...
    expect(normalizeSettings({ appearanceThemePreset: 'celadonPorcelain' }).appearanceThemePreset).toBe('celadonPorcelain');
    expect(normalizeSettings({ appearanceThemePreset: 'polarDaybreak' }).appearanceThemePreset).toBe('polarDaybreak');
    expect(normalizeSettings({ appearanceThemePreset: 'plumVelvet' }).appearanceThemePreset).toBe('plumVelvet');
    expect(normalizeSettings({ appearanceThemePreset: 'midnightCopper' }).appearanceThemePreset).toBe('midnightCopper');
    expect(normalizeSettings({ appearanceThemePreset: 'taroBunny' }).appearanceThemePreset).toBe('taroBunny');
    expect(normalizeSettings({ appearanceThemePreset: 'milkTeaBear' }).appearanceThemePreset).toBe('milkTeaBear');
    expect(normalizeSettings({ appearanceThemePreset: 'ramuneGlass' }).appearanceThemePreset).toBe('ramuneGlass');
    expect(normalizeSettings({ appearanceThemePreset: 'pistachioMousse' }).appearanceThemePreset).toBe('pistachioMousse');
    expect(normalizeSettings({ appearanceThemePreset: 'sakuraWagashi' }).appearanceThemePreset).toBe('sakuraWagashi');
    expect(normalizeSettings({ appearanceThemePreset: 'kamakuraHydrangea' }).appearanceThemePreset).toBe('kamakuraHydrangea');
    expect(normalizeSettings({ appearanceThemePreset: 'ruriKintsugi' }).appearanceThemePreset).toBe('ruriKintsugi');
    expect(normalizeSettings({ appearanceThemePreset: 'hinokiRyokan' }).appearanceThemePreset).toBe('hinokiRyokan');
    expect(normalizeSettings({ appearanceThemePreset: 'nightGlass' }).appearanceThemePreset).toBe('nightGlass');
    expect(normalizeSettings({ appearanceThemePreset: 'boneIron' }).appearanceThemePreset).toBe('boneIron');
    expect(normalizeSettings({ appearanceThemePreset: 'harborLamp' }).appearanceThemePreset).toBe('harborLamp');
    expect(normalizeSettings({ appearanceThemePreset: 'ashRose' }).appearanceThemePreset).toBe('ashRose');
  ...
  it('keeps Pro themes locked unless the donator unlock plugin is valid for the current marker', async () => {
    ...
    expect(normalizeSettings({ appearanceThemePreset: 'nyanCat', finalThemeUnlockVersion }, { finalThemeUnlocked: true }).appearanceThemePreset).toBe('classic');
    expect(normalizeSettings({ appearanceThemePreset: 'darkSideMoon', finalThemeUnlockVersion }, { finalThemeUnlocked: true }).appearanceThemePreset).toBe('classic');
    ...
    expect(normalizeSettings({ appearanceThemePreset: 'nyanCat', finalThemeUnlockVersion: null }, { finalThemeUnlocked: true }).appearanceThemePreset).toBe('classic');
    expect(normalizeSettings({ appearanceThemePreset: 'darkSideMoon', finalThemeUnlockVersion: null }, { finalThemeUnlocked: true }).appearanceThemePreset).toBe('classic');
  });
  ...
  it('keeps a retired Pro theme persisted for migration while never restoring it at runtime', async () => {
    vi.resetModules();
    const { finalThemeUnlockVersion } = await import('../../shared/constants/featureUnlocks');
    userDataPath = join(tmpdir(), `echo-settings-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    tempRoots.push(userDataPath);
    ...
    expect(unlockedRuntimeView.appearanceThemePreset).toBe('classic');
  ...
  it('keeps GPU disabling scoped to the ultra-light runtime unless explicitly enabled', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).ultraLightGpuDisabled).toBe(false);
    expect(normalizeSettings({ ultraLightGpuDisabled: true }).ultraLightGpuDisabled).toBe(true);
    expect(normalizeSettings({ ultraLightGpuDisabled: 'yes' as never }).ultraLightGpuDisabled).toBe(false);
  });
  ...
  it('keeps startup account checks disabled in the Steam distribution', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).autoAccountCheckOnStartup).toBe(false);
    expect(normalizeSettings({ autoAccountCheckOnStartup: true }).autoAccountCheckOnStartup).toBe(false);
    expect(normalizeSettings({ autoAccountCheckOnStartup: false }).autoAccountCheckOnStartup).toBe(false);
    expect(normalizeSettings({ autoAccountCheckOnStartup: 'no' as never }).autoAccountCheckOnStartup).toBe(false);
  });
  ...
  it('keeps streaming and online downloader entries disabled in the Steam distribution', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).streamingFeatureEnabled).toBe(false);
    expect(normalizeSettings({ streamingFeatureEnabled: true }).streamingFeatureEnabled).toBe(false);
    expect(normalizeSettings({ streamingFeatureEnabled: false }).streamingFeatureEnabled).toBe(false);
    expect(normalizeSettings({ streamingFeatureEnabled: 'yes' as never }).streamingFeatureEnabled).toBe(false);
    expect(normalizeSettings({}).osuDownloaderFeatureEnabled).toBe(false);
    expect(normalizeSettings({ osuDownloaderFeatureEnabled: true }).osuDownloaderFeatureEnabled).toBe(false);
  ...
  it('keeps Spotify official player launch disabled in the Steam distribution', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).spotifyAutoLaunchOfficialPlayer).toBe(false);
    expect(normalizeSettings({ spotifyAutoLaunchOfficialPlayer: true }).spotifyAutoLaunchOfficialPlayer).toBe(false);
    expect(normalizeSettings({ spotifyAutoLaunchOfficialPlayer: false }).spotifyAutoLaunchOfficialPlayer).toBe(false);
    expect(normalizeSettings({ spotifyAutoLaunchOfficialPlayer: 'no' as never }).spotifyAutoLaunchOfficialPlayer).toBe(false);
  });
  ...
  it('normalizes settings hide-sidebar as a default-off opt-in', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).settingsHideSidebarEnabled).toBe(false);
    expect(normalizeSettings({ settingsHideSidebarEnabled: true }).settingsHideSidebarEnabled).toBe(true);
    expect(normalizeSettings({ settingsHideSidebarEnabled: 'true' as never }).settingsHideSidebarEnabled).toBe(false);
  });
  ...
  it('keeps Discord Rich Presence optional and disabled by default', async () => {
    const { normalizeSettings } = await import('./appSettings');
  ...
  it('uses detailed Steam Rich Presence by default and migrates the legacy boolean', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({})).toMatchObject({
      steamRichPresenceEnabled: true,
      steamRichPresenceMode: 'detailed',
      steamRichPresencePreset: 'music',
      steamRichPresenceShowAlbum: true,
      steamRichPresenceShowProgress: true,
      steamRichPresenceShowGenre: false,
      steamRichPresenceShowPlaybackOrder: false,
      steamRichPresenceShowBpm: false,
      steamRichPresenceShowQuality: false,
      steamRichPresenceShowFormat: false,
      steamRichPresenceShowBitPerfect: false,
      steamListeningStatsEnabled: true,
      steamLeaderboardsEnabled: false,
    });
    expect(normalizeSettings({ steamListeningStatsEnabled: true }).steamListeningStatsEnabled).toBe(true);
    expect(normalizeSettings({ appMemoryVersion: 7, steamListeningStatsEnabled: false }).steamListeningStatsEnabled).toBe(true);
    expect(normalizeSettings({ appMemoryVersion: 8, steamListeningStatsEnabled: false }).steamListeningStatsEnabled).toBe(false);
    expect(normalizeSettings({ steamLeaderboardsEnabled: true }).steamLeaderboardsEnabled).toBe(true);
    expect(normalizeSettings({ steamRichPresenceEnabled: true }).steamRichPresenceMode).toBe('detailed');
    expect(normalizeSettings({ steamRichPresenceEnabled: false })).toMatchObject({ steamRichPresenceEnabled: false, steamRichPresenceMode: 'off' });
    expect(normalizeSettings({ steamRichPresenceMode: 'detailed' })).toMatchObject({ steamRichPresenceEnabled: true, steamRichPresenceMode: 'detailed' });
    expect(normalizeSettings({ steamRichPresenceMode: 'unsupported' as never }).steamRichPresenceMode).toBe('detailed');
    expect(normalizeSettings({ steamRichPresenceMode: 'basic' }).steamRichPresencePreset).toBe('privacy');
    expect(normalizeSettings({ steamRichPresencePreset: 'hifi' as never }).steamRichPresencePreset).toBe('music');
    expect(normalizeSettings({ steamRichPresenceShowAlbum: false, steamRichPresenceShowProgress: false })).toMatchObject({
      steamRichPresenceShowAlbum: false,
      steamRichPresenceShowProgress: false,
    });
    expect(normalizeSettings({
      steamRichPresenceShowBpm: true,
      steamRichPresenceShowQuality: true,
      steamRichPresenceShowFormat: true,
      steamRichPresenceShowBitPerfect: true,
    })).toMatchObject({
      steamRichPresenceShowBpm: true,
      steamRichPresenceShowQuality: true,
      steamRichPresenceShowFormat: true,
  ...
  it('enables the Stage API and taskbar mini player by default without overriding explicit choices', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).stageApiEnabled).toBe(true);
    expect(normalizeSettings({}).taskbarMiniPlayerEnabled).toBe(true);
    expect(normalizeSettings({ stageApiEnabled: false }).stageApiEnabled).toBe(false);
    expect(normalizeSettings({ taskbarMiniPlayerEnabled: false }).taskbarMiniPlayerEnabled).toBe(false);
  });
  ...
  it('keeps streaming download actions disabled regardless of unlock state', async () => {
    const { normalizeSettings } = await import('./appSettings');
  ...
  it('does not derive a forbidden downloads unlock from plugin state', async () => {
    const { normalizeSettings } = await import('./appSettings');
  ...
  it('normalizes lyrics settings', async () => {
    ...
    expect(normalizeSettings({ lyricsWordHighlightEnabled: true })).toMatchObject({
  ...
  it('hides genres in the sidebar for settings saved before that default', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({ appMemoryVersion: 6, sidebarHiddenRouteIds: ['audio-cd'] }).sidebarHiddenRouteIds).toEqual([
      'audio-cd',
      'genres',
    ]);
    expect(normalizeSettings({ appMemoryVersion: 7, sidebarHiddenRouteIds: ['audio-cd'] }).sidebarHiddenRouteIds).toEqual([
      'audio-cd',
    ]);
  });
});
