...
describe('dataProtection', () => {
  ...
  it('pins userData to a stable appData folder', () => {
    ...
    expect(getProtectedUserDataPath(fakeApp)).toBe(join(tempDir, 'ECHO Steam'));
    expect(initializeProtectedUserDataPath(fakeApp)).toBe(join(tempDir, 'ECHO Steam'));
    expect(calls).toEqual([['userData', join(tempDir, 'ECHO Steam')]]);
  });
  ...
  it('migrates stronger legacy echo data over a fresh protected directory', async () => {
    const targetDir = join(tempDir, 'ECHO Steam');
    const legacyDir = join(tempDir, 'echo');
    ...
    createHealthyLibrary(legacyLibraryPath);
    ...
    const migratedLibrary = new Database(targetLibraryPath, { readonly: true });
    expect(migratedLibrary.prepare('SELECT COUNT(*) AS count FROM tracks').get()).toMatchObject({ count: 1 });
    migratedLibrary.close();
    expect(existsSync(join(targetDir, 'data-protection', 'snapshots'))).toBe(true);
  });

  it('migrates explicitly selected legacy settings and account data', async () => {
    const targetDir = join(tempDir, 'ECHO Steam');
    const legacyDir = join(tempDir, 'echo');
    ...
    createHealthyLibrary(join(legacyDir, 'echo-library.sqlite'));
    writeFileSync(join(legacyDir, 'accounts.json'), '{"providers":["spotify"]}\n', 'utf8');
    writeFileSync(join(legacyDir, 'echo-download-settings.json'), '{"enabled":true}\n', 'utf8');
    writeFileSync(join(legacyDir, 'eq-presets.json'), '{"presets":["my-eq"]}\n', 'utf8');

    const migration = await migrateLegacyProtectedData(targetDir, [legacyDir]);

    expect(migration.sourcePath).toBe(legacyDir);
    expect(migration.migrated).toEqual(expect.arrayContaining(['echo-settings.json', 'accounts.json', 'echo-download-settings.json', 'eq-presets.json']));
