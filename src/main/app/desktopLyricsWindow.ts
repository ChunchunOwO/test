...
const desktopLyricsSessionPartition = 'echo-desktop-lyrics';
...
export const createDesktopLyricsWindow = (): BrowserWindow => {
  ...
  const window = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // Chromium stores page zoom by session and origin. Desktop lyrics must
      // remain at its designed size when the main window is enlarged.
      partition: desktopLyricsSessionPartition,
    },
  });
  ...
  window.once('ready-to-show', () => {
    if (desktopLyricsWindow !== window || window.isDestroyed()) {
      return;
    }
    desktopLyricsRendererReady = true;
...
export const showDesktopLyricsWindow = (): DesktopLyricsState => {
  ...
  applyDesktopLyricsLockState(window);
  if (desktopLyricsRendererReady && !window.isVisible()) {
    window.showInactive();
  }
  applyDesktopLyricsAlwaysOnTop(window);
  emitDesktopLyricsStateChanged();
...
export const hideDesktopLyricsWindow = (): DesktopLyricsState => {
  ...
  destroyDesktopLyricsWindow(false);
  emitDesktopLyricsStateChanged();
  return getDesktopLyricsState();
};
...
const destroyDesktopLyricsWindow = (preserveEnabledState: boolean): void => {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) {
    desktopLyricsWindow = null;
    desktopLyricsMousePassthrough = false;
    return;
...
export const closeDesktopLyricsWindow = (): void => {
  destroyDesktopLyricsWindow(true);
};
