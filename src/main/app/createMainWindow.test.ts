...
const makeOutMainDir = (): string => {
  const root = mkdtempSync(join(tmpdir(), 'echo-preload-'));
...
describe('createMainWindowWebPreferences', () => {
  it('keeps renderer timers running until guarded runtime throttling takes ownership', () => {
  ...
  it('passes the normalized accessibility scale to Electron before the first paint', () => {
    expect(createMainWindowWebPreferences(95)).toMatchObject({ zoomFactor: 0.95 });
    expect(createMainWindowWebPreferences(999)).toMatchObject({ zoomFactor: 1.5 });
  });
...
describe('mainWindowMinimumSize', () => {
  it('keeps the main window above an unusable compact size', () => {
