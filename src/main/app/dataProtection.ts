...
import { getLibraryEntryPathForUserData } from './editionDataSync';
...
export class LibraryDatabaseUnavailableError extends Error {
  constructor(readonly recovery: LibraryRecoveryResult | null = lastDataProtectionResult?.recovery ?? null) {
    ...
    super(
      confirmedCorruption
        ? 'SQLite 明确报告音乐库数据库损坏，ECHO 已进入保护模式。音乐文件不会被删除，请前往设置里的数据库恢复工具处理。'
        : '音乐库数据库暂时无法读取，健康检查未完成；这不代表数据库已经损坏。请重启 ECHO，若仍然出现请检查运行环境。',
    );
...
const protectedUserDataFolderName = 'ECHO Steam';
// Steam settings stay in their own profile. The library has a separate shared
// path resolver and must never be copied as part of legacy profile migration.
const legacyUserDataFolderNames: readonly string[] = [];
...
const libraryEntryPathFor = (rootPath: string, name: string): string => getLibraryEntryPathForUserData(rootPath, name);
const libraryPathFor = (rootPath: string): string => libraryEntryPathFor(rootPath, libraryFileName);
const libraryWalPathFor = (rootPath: string): string => libraryEntryPathFor(rootPath, libraryWalFileName);
const libraryShmPathFor = (rootPath: string): string => libraryEntryPathFor(rootPath, libraryShmFileName);
...
const copyLibraryTriplet = (sourceRoot: string, targetRoot: string): string[] => {
  ...
  for (const name of [libraryFileName, libraryWalFileName, libraryShmFileName]) {
    const sourcePath = libraryEntryPathFor(sourceRoot, name);
    if (!existsSync(sourcePath)) {
      continue;
    }
    try {
      copyProtectedEntry(sourcePath, libraryEntryPathFor(targetRoot, name), 'file');
      copied.push(name);
    } catch {
...
const copyLibraryTripletAsync = async (sourceRoot: string, targetRoot: string): Promise<string[]> => {
  ...
  for (const name of [libraryFileName, libraryWalFileName, libraryShmFileName]) {
    ...
    try {
      await copyProtectedEntryAsync(sourcePath, libraryEntryPathFor(targetRoot, name), 'file');
      copied.push(name);
    } catch {
      // A failed archive/snapshot copy should not block the rest of startup.
    }
...
const tryRemoveLibraryTriplet = (rootPath: string): LibraryTripletRemoveResult => {
  ...
  for (const name of [libraryFileName, libraryWalFileName, libraryShmFileName]) {
    const filePath = libraryEntryPathFor(rootPath, name);
...
export const repairProtectedLibraryDatabase = (userDataPath = app.getPath('userData')): LibraryDatabaseRepairResult => {
  ...
  const removedDatabaseFiles = [libraryFileName, libraryWalFileName, libraryShmFileName].filter((name) =>
    existsSync(libraryEntryPathFor(userDataPath, name)),
  );
...
export const migrateLegacyProtectedData = async (
  ...
  for (const entry of protectedDataEntries) {
    ...
    if (entry.name === libraryWalFileName || entry.name === libraryShmFileName) {
      skipped.push(entry.name);
      continue;
    }
    ...
    await runDataProtectionPhase(phaseContext, `file copy/hash ${entry.name}`, async () => {
      try {
        if (entry.name === libraryFileName) {
          // SQLite backup takes a consistent snapshot even if regular ECHO is
          // open and its source database currently has an active WAL.
          await sqliteBackup(sourceEntryPath, join(targetUserDataPath, entry.name));
        } else {
...
export const restoreMissingProtectedData = (userDataPath = app.getPath('userData')): RestoreResult => {
  ...
  for (const entry of protectedDataEntries) {
    const targetPath = libraryEntryNames.has(entry.name)
      ? libraryEntryPathFor(userDataPath, entry.name)
...
export const writeDataProtectionManifest = (userDataPath = app.getPath('userData')): void => {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        formatVersion: 1,
        appName: typeof app.getName === 'function' ? app.getName() : 'ECHO',
        appVersion: typeof app.getVersion === 'function' ? app.getVersion() : null,
        protectedUserDataPath: userDataPath,
        protectedEntries: protectedDataEntries.map((entry) => ({
          ...entry,
          path: libraryEntryNames.has(entry.name)
            ? libraryEntryPathFor(userDataPath, entry.name)
            : join(userDataPath, entry.name),
        })),
