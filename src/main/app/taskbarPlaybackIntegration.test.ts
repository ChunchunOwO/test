...
describe('TaskbarPlaybackIntegration', () => {
  ...
  it('sets playback progress and the current track title on Windows', async () => {
    ...
    expect(window.setTitle).toHaveBeenCalledWith('Song A - Artist A | ECHO');
    expect(window.setThumbnailClip).toHaveBeenCalledWith({ x: 0, y: 624, width: 1280, height: 96 });
    expect(window.setThumbnailToolTip).toHaveBeenCalledWith('Song A - Artist A | ECHO');
  ...
  it('uses audio status metadata for streaming tracks that are not in the library', async () => {
    ...
    expect(window.setTitle).toHaveBeenCalledWith('Streaming Song - Streaming Artist | ECHO');
    expect(window.setThumbnailToolTip).toHaveBeenCalledWith('Streaming Song - Streaming Artist | ECHO');
    integration.dispose();
  });
  ...
  it('keeps paused progress but marks the taskbar progress as paused', async () => {
    ...
    expect(window.setTitle).toHaveBeenCalledWith('Paused Song | ECHO');
  ...
  it('clears taskbar state when playback stops', async () => {
    ...
    expect(window.setTitle).toHaveBeenCalledWith('ECHO');
