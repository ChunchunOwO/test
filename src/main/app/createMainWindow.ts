import { existsSync } from 'node:fs';
import { join } from 'node:path';
import { pathToFileURL } from 'node:url';
import { app, BrowserWindow, shell } from 'electron';
import { mainWindowMinimumSize, type AppSettings, type RememberedWindowSize } from '../../shared/types/appSettings';
import { resolveEffectivePerformancePolicy } from '../../shared/utils/performancePolicy';
import { defaultAccessibilityPreferences, getAppSettings, setAppSettings } from './appSettings';
import { bindBackgroundPlaybackShortcutsToWindow } from './backgroundPlaybackShortcuts';
import { bindTaskbarPlaybackIntegration } from './taskbarPlaybackIntegration';
import { ensureTray, isAppQuitRequested, requestAppQuit } from './tray';
import { clearMainWindow, setMainWindow } from './windowManager';
import { IpcChannels } from '../../shared/constants/ipcChannels';
import {
  isWorkshopUiRuntimeActive,
  isWorkshopUiRuntimeEmergencyExitInput,
} from '../workshop/WorkshopUiRuntimeAuthority';
import { getCrashReportService } from '../diagnostics/CrashReportService';
import { closeDevConsoleWindow, recordMainRuntimeIssue, recordRendererConsoleMessage } from '../diagnostics/DevConsoleService';
import { markStartupStage } from '../diagnostics/StartupDiagnostics';
import {
  applyMainWindowBackgroundMaterial,
  isMainWindowAcrylicSupportedPlatform,
  resolveMainWindowBackgroundColor,
} from './windowBackgroundMaterial';
import { areDeveloperToolsAllowed } from './securityPolicy';
import { resolveAppIconPath } from './appIcon';
import { hideTaskbarMiniPlayerOnly, showTaskbarMiniPlayerOnly } from './taskbarMiniPlayerWindow';
import { isUltraLightModeActive } from './UltraLightModeService';
import { isAllowedExternalUrl, isAllowedWorkshopFrameNavigation, isTrustedRendererUrl, isWorkshopFrameUrl } from './trustedRenderer';
import {
  adjustMainWindowUiScalePercent,
  applyMainWindowUiScale,
  resolveMainWindowUiScaleShortcut,
  resolveMainWindowZoomFactor,
  type MainWindowUiScaleShortcutAction,
} from './mainWindowUiScale';
import { bindMainWindowBackgroundThrottling } from './mainWindowBackgroundThrottling';
import { hideMainWindowToTray } from './mainWindowTrayVisibility';

const mainOutputDir = import.meta.dirname;
export const defaultMainWindowSize = {
  width: 1680,
  height: 1050,
} as const;
export { mainWindowMinimumSize };
const rememberWindowSizeDebounceMs = 350;

export const resolvePreloadPath = (baseDir = mainOutputDir): string => {
  const mjsPreload = join(baseDir, '../preload/index.mjs');

  if (existsSync(mjsPreload)) {
    return mjsPreload;
  }

  return join(baseDir, '../preload/index.js');
};

export const createMainWindowWebPreferences = (
  uiScalePercent: unknown = 100,
): Electron.BrowserWindowConstructorOptions['webPreferences'] => ({
  preload: resolvePreloadPath(),
  contextIsolation: true,
  nodeIntegration: false,
  devTools: areDeveloperToolsAllowed(),
  sandbox: false,
  backgroundThrottling: false,
  zoomFactor: resolveMainWindowZoomFactor(uiScalePercent),
});

export const resolveInitialMainWindowSize = (settings: AppSettings = getAppSettings()): RememberedWindowSize => {
  const rememberedSize = settings.rememberWindowSizeEnabled === false ? null : settings.rememberedWindowSize;

  return {
    width: Math.max(mainWindowMinimumSize.width, rememberedSize?.width ?? defaultMainWindowSize.width),
    height: Math.max(mainWindowMinimumSize.height, rememberedSize?.height ?? defaultMainWindowSize.height),
  };
};

export const resolveMainWindowBackgroundOptions = (
  settings: Pick<AppSettings, 'appWindowAcrylicEnabled' | 'lowSpecModeEnabled'>,
  acrylicSupported = isMainWindowAcrylicSupportedPlatform(),
): Pick<Electron.BrowserWindowConstructorOptions, 'backgroundColor' | 'backgroundMaterial'> => {
  const acrylicEnabled = acrylicSupported && resolveEffectivePerformancePolicy(settings).appWindowAcrylicEnabled;

  return {
    // Match the commissioned startup artwork so the native window never
    // exposes a light flash while Chromium prepares its first frame.
    backgroundColor: resolveMainWindowBackgroundColor(acrylicEnabled, '#74d3e5'),
    ...(acrylicSupported
      ? {
          backgroundMaterial: acrylicEnabled ? 'acrylic' : 'none',
        }
      : {}),
  };
};

const rememberMainWindowSize = (window: BrowserWindow): void => {
  if (window.isDestroyed() || window.isMinimized() || window.isMaximized() || window.isFullScreen()) {
    return;
  }

  if (getAppSettings().rememberWindowSizeEnabled === false) {
    return;
  }

  const [width, height] = window.getSize();
  setAppSettings({
    rememberedWindowSize: {
      width,
      height,
    },
  });
};

export const createMainWindow = (): BrowserWindow => {
  markStartupStage('main-window:create:start');
  const settings = getAppSettings();
  const initialSize = resolveInitialMainWindowSize(settings);
  const appIconPath = resolveAppIconPath();
  const mainWindowBackgroundOptions = resolveMainWindowBackgroundOptions(settings);
  const window = new BrowserWindow({
    width: initialSize.width,
    height: initialSize.height,
    minWidth: mainWindowMinimumSize.width,
    minHeight: mainWindowMinimumSize.height,
    title: 'ECHO',
    icon: appIconPath ?? undefined,
    ...mainWindowBackgroundOptions,
    frame: false,
    show: false,
    webPreferences: createMainWindowWebPreferences(settings.accessibilityPreferences?.uiScalePercent),
  });
  window.setMinimumSize(mainWindowMinimumSize.width, mainWindowMinimumSize.height);
  window.on('will-resize', (event, nextBounds) => {
    if (
      nextBounds.width < mainWindowMinimumSize.width ||
      nextBounds.height < mainWindowMinimumSize.height
    ) {
      event.preventDefault();
    }
  });
  const rendererEntryPath = join(mainOutputDir, '../renderer/index.html');
  const configuredRendererUrl = process.env.ELECTRON_RENDERER_URL?.trim() || null;
  const trustedRendererUrl = configuredRendererUrl ?? pathToFileURL(rendererEntryPath).toString();
  const acrylicStartupQuery = mainWindowBackgroundOptions.backgroundMaterial === 'acrylic'
    ? { windowAcrylic: '1' }
    : undefined;
  let rememberSizeTimer: ReturnType<typeof setTimeout> | null = null;
  const commitMainWindowUiScale = (action: MainWindowUiScaleShortcutAction): void => {
    const latestSettings = getAppSettings();
    const currentPreferences = latestSettings.accessibilityPreferences ?? defaultAccessibilityPreferences;
    const nextUiScalePercent = adjustMainWindowUiScalePercent(currentPreferences.uiScalePercent, action);
    if (nextUiScalePercent === currentPreferences.uiScalePercent) {
      return;
    }

    setAppSettings({
      accessibilityPreferences: {
        ...currentPreferences,
        uiScalePercent: nextUiScalePercent,
      },
    });
    applyMainWindowUiScale(window, nextUiScalePercent);
    window.webContents.send(IpcChannels.AppSharedSettingsChanged);
  };

  window.webContents.on('before-input-event', (event, input) => {
    const action = resolveMainWindowUiScaleShortcut(input);
    if (!action) {
      return;
    }
    event.preventDefault();
    commitMainWindowUiScale(action);
  });
  window.webContents.on('zoom-changed', (event, direction) => {
    event.preventDefault();
    commitMainWindowUiScale(direction === 'in' ? 'increase' : 'decrease');
  });

  const publishMaximizedState = (): void => {
    if (!window.isDestroyed()) {
      window.webContents.send(IpcChannels.AppWindowMaximizedChanged, window.isMaximized() || window.isFullScreen());
    }
  };
  const publishFullscreenState = (): void => {
    if (!window.isDestroyed()) {
      window.webContents.send(IpcChannels.AppWindowFullscreenChanged, window.isFullScreen());
    }
  };
  const publishMinimizedState = (): void => {
    if (!window.isDestroyed()) {
      window.webContents.send(IpcChannels.AppWindowMinimizedChanged, window.isMinimized());
    }
  };
  const publishHiddenState = (): void => {
    if (!window.isDestroyed()) {
      window.webContents.send(IpcChannels.AppWindowHiddenChanged, !window.isVisible());
    }
  };
  const toggleFullscreenFromShortcut = (): void => {
    if (!window.isDestroyed()) {
      window.setFullScreen(!window.isFullScreen());
    }
  };

  window.webContents.on('console-message', (details) => {
    recordRendererConsoleMessage(details);
    const { level, message, lineNumber, sourceId } = details;

    if (!message.includes('[SpotifySDK]')) {
      return;
    }

    getCrashReportService().getLogger()?.info('renderer', 'renderer console', {
      level,
      message,
      line: lineNumber,
      sourceId,
    });
  });
  window.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    recordMainRuntimeIssue('renderer-load-failed', errorDescription || 'Renderer failed to load', {
      reason: validatedURL,
      exitCode: errorCode,
      sourceId: isMainFrame ? 'main-frame' : 'sub-frame',
    });
  });
  window.webContents.on('preload-error', (_event, preloadPath, error) => {
    recordMainRuntimeIssue('preload-error', error.message, {
      stack: error.stack,
      sourceId: preloadPath,
    });
  });
  window.webContents.on('before-input-event', (event, input) => {
    if (
      isWorkshopUiRuntimeEmergencyExitInput(input) && isWorkshopUiRuntimeActive(window.webContents)
    ) {
      event.preventDefault();
      window.webContents.send(IpcChannels.WorkshopUiRuntimeEmergencyExit);
      return;
    }
    if (input.type === 'keyDown' && input.key === 'F11') {
      event.preventDefault();
      toggleFullscreenFromShortcut();
    }
  });
  const preventUntrustedNavigation = (event: { preventDefault: () => void }, url: string): void => {
    if (!isTrustedRendererUrl(url, trustedRendererUrl)) {
      event.preventDefault();
    }
  };
  window.webContents.on('will-navigate', preventUntrustedNavigation);
  window.webContents.on('will-redirect', preventUntrustedNavigation);
  window.webContents.on('will-frame-navigate', (details) => {
    if (details.isMainFrame) return;
    if (!isAllowedWorkshopFrameNavigation(details.frame?.url ?? '', details.url)) {
      details.preventDefault();
    }
  });
  window.webContents.setWindowOpenHandler(({ url, referrer }) => {
    if (isWorkshopFrameUrl(referrer.url)) {
      return { action: 'deny' };
    }
    if (isAllowedExternalUrl(url)) {
      void shell.openExternal(url).catch(() => undefined);
    }
    return { action: 'deny' };
  });

  const scheduleRememberSize = (): void => {
    if (rememberSizeTimer !== null) {
      clearTimeout(rememberSizeTimer);
    }

    rememberSizeTimer = setTimeout(() => {
      rememberSizeTimer = null;
      rememberMainWindowSize(window);
    }, rememberWindowSizeDebounceMs);
  };
  const refreshAcrylicAfterFocusChange = (): void => {
    const latestSettings = getAppSettings();
    if (latestSettings.appWindowAcrylicEnabled === true && latestSettings.appWindowAcrylicKeepWhenUnfocusedEnabled === true) {
      applyMainWindowBackgroundMaterial(window, latestSettings);
    }
  };

  window.once('ready-to-show', () => {
    const settings = getAppSettings();
    if (settings.miniPlayerEnabled === true && settings.miniPlayerAutoHideMainWindow === true) {
      window.hide();
    } else {
      window.show();
    }

    // Create and show the taskbar mini player if enabled.
    // It is a permanent overlay, not tied to main window focus/blur.
    if (settings.taskbarMiniPlayerEnabled === true) {
      try {
        showTaskbarMiniPlayerOnly();
      } catch {
        // Taskbar mini player is best-effort; don't block main window startup.
      }
    }

    markStartupStage('main-window:ready-to-show');
  });

  window.on('resize', () => {
    if (!window.isDestroyed() && !window.isMaximized() && !window.isFullScreen()) {
      const [width, height] = window.getSize();
      const nextWidth = Math.max(mainWindowMinimumSize.width, width);
      const nextHeight = Math.max(mainWindowMinimumSize.height, height);
      if (nextWidth !== width || nextHeight !== height) {
        window.setSize(nextWidth, nextHeight);
      }
    }
    scheduleRememberSize();
  });
  window.on('blur', refreshAcrylicAfterFocusChange);
  window.on('focus', refreshAcrylicAfterFocusChange);
  window.on('maximize', publishMaximizedState);
  window.on('unmaximize', publishMaximizedState);

  // Taskbar mini player management
  // The taskbar mini player is a PERMANENT overlay. It stays visible
  // regardless of main window state (focus, blur, minimize, restore).
  //
  // The ONLY conditions that hide it:
  // 1. taskbarMiniPlayerEnabled is false (user toggled it off)
  // 2. miniPlayerEnabled is true (desktop mini player takes over)
  // 3. Main window is focused and in fullscreen

  const shouldShowTaskbarMiniPlayer = (): boolean =>
    getAppSettings().taskbarMiniPlayerEnabled === true &&
    getAppSettings().miniPlayerEnabled !== true;

  const syncTaskbarMiniPlayerForMainWindowState = (): void => {
    if (window.isDestroyed()) {
      return;
    }

    if (window.isFullScreen() && window.isFocused()) {
      try { hideTaskbarMiniPlayerOnly(); } catch { /* best-effort */ }
      return;
    }

    if (shouldShowTaskbarMiniPlayer()) {
      try { showTaskbarMiniPlayerOnly(); } catch { /* best-effort */ }
    }
  };

  window.on('enter-full-screen', () => {
    publishMaximizedState();
    publishFullscreenState();
    syncTaskbarMiniPlayerForMainWindowState();
  });
  window.on('leave-full-screen', () => {
    publishMaximizedState();
    publishFullscreenState();
    syncTaskbarMiniPlayerForMainWindowState();
  });
  window.on('focus', syncTaskbarMiniPlayerForMainWindowState);
  window.on('blur', syncTaskbarMiniPlayerForMainWindowState);

  window.on('close', (event) => {
    if (rememberSizeTimer !== null) {
      clearTimeout(rememberSizeTimer);
      rememberSizeTimer = null;
    }

    rememberMainWindowSize(window);
    closeDevConsoleWindow();

    if (!isAppQuitRequested() && getAppSettings().hideToTrayOnClose) {
      event.preventDefault();
      ensureTray();
      hideMainWindowToTray(window);
    }
  });

  window.on('closed', () => {
    if (rememberSizeTimer !== null) {
      clearTimeout(rememberSizeTimer);
      rememberSizeTimer = null;
    }

    clearMainWindow();
    closeDevConsoleWindow();

    if (!isAppQuitRequested() && !isUltraLightModeActive() && !getAppSettings().hideToTrayOnClose) {
      requestAppQuit();
      app.quit();
    }
  });

  applyMainWindowBackgroundMaterial(window, settings);

  setMainWindow(window, trustedRendererUrl);
  if (configuredRendererUrl) {
    const rendererUrl = new URL(configuredRendererUrl);
    if (acrylicStartupQuery) {
      rendererUrl.searchParams.set('windowAcrylic', acrylicStartupQuery.windowAcrylic);
    }
    void window.loadURL(rendererUrl.toString());
  } else {
    void window.loadFile(rendererEntryPath, acrylicStartupQuery ? { query: acrylicStartupQuery } : undefined);
  }

  ensureTray();
  bindMainWindowBackgroundThrottling(window);
  // Register after the throttling controller so show/restore re-enable
  // Chromium timers before the renderer receives its foreground budget.
  window.on('minimize', publishMinimizedState);
  window.on('restore', publishMinimizedState);
  window.on('hide', publishHiddenState);
  window.on('show', publishHiddenState);
  bindBackgroundPlaybackShortcutsToWindow();
  bindTaskbarPlaybackIntegration(window);
  markStartupStage('main-window:create:complete', initialSize);

  return window;
};
