...
import {
  artistOnlineInfoSources,
  artistStreamingAlbumProviders,
...
import { defaultSidebarHiddenRouteIds, defaultSidebarRouteOrder, migrateSidebarHiddenRouteIds, normalizeSidebarRouteOrder } from '../../shared/types/sidebar';
import type {
...
const defaultAudioTransportFadeDurationMs = 1500;
...
const normalizeRememberedWindowSize = (value: unknown): RememberedWindowSize | null => {
  ...
  return {
    width: Math.round(clamp(width, mainWindowMinimumSize.width, 3840)),
    height: Math.round(clamp(height, mainWindowMinimumSize.height, 2160)),
  };
};
...
export const defaultSettings: AppSettings = {
  ...
  hiddenPlayerBarButtonIds: ['sleepTimer', 'audioExport'],
  ...
  miniPlayerUsesUltraLightMode: false,
...
let regularEditionSettingsMtimeMs = -1;
...
export const getAppSettingsUpdatedAt = (): string | null => {
  const settingsPath = getSettingsPath();
  if (!existsSync(settingsPath)) {
    return null;
  }
  try {
    return statSync(settingsPath).mtime.toISOString();
  } catch {
    return null;
  }
};
...
const readLocalSettingsSource = (): Partial<AppSettings> => {
  const settingsPath = getSettingsPath();
  if (!existsSync(settingsPath)) {
  ...
  try {
    return JSON.parse(readJsonFileText(settingsPath)) as Partial<AppSettings>;
  } catch {
    return { ...defaultSettings };
  }
};

const refreshSharedSettingsSource = (): boolean => {
  const shared = readRegularEditionSharedSettings();
  if (!shared || shared.mtimeMs === regularEditionSettingsMtimeMs) {
    return false;
  }

  cachedSettingsSource = {
    ...(cachedSettingsSource ?? readLocalSettingsSource()),
    ...shared.settings,
  };
  cachedSettings = null;
  regularEditionSettingsMtimeMs = shared.mtimeMs;
  return true;
};
...
const normalizeAudioTransportFadeDurationMs = (value: unknown): number => {
  const numeric = Number(value);
  return Number.isFinite(numeric)
    ? Math.round(clamp(numeric, 0, 5000))
...
const normalizeAccessibilityPreferences = (value: unknown): AccessibilityPreferences => {
  ...
  const rawUiScalePercent = Number(input.uiScalePercent);
  const uiScalePercent = Number.isFinite(rawUiScalePercent)
    ? Math.round(clamp(rawUiScalePercent, uiScalePercentMin, uiScalePercentMax) / uiScalePercentStep) * uiScalePercentStep
...
const normalizeRememberedAudioOutput = (value: unknown): RememberedAudioOutput => {
  ...
  const outputMode =
    input.outputMode === 'shared' || input.outputMode === 'exclusive' || input.outputMode === 'asio' || input.outputMode === 'ks' || input.outputMode === 'system'
      ? input.outputMode
...
const normalizeLyricsPageStyle = (value: unknown): LyricsPageStyle =>
  value === 'editorial' || value === 'folded' || value === 'roseVinyl' || value === 'cinemaStage' || value === 'kineticPoster' || value === 'coverStage' || value === 'cutBoard'
    ? value
...
export const normalizeSettings = (value: unknown, options: NormalizeSettingsOptions = {}): AppSettings => {
  ...
  const proFeaturesUnlocked = options.proFeaturesUnlocked === true;
  const normalizedAudioSdmMode = normalizeAudioSdmMode(settings.audioSdmMode);
  ...
  return {
    ...
    sidebarHiddenRouteIds: migrateSidebarHiddenRouteIds(settings.sidebarHiddenRouteIds, sourceAppMemoryVersion),
    ...
    settingsHideSidebarEnabled: settings.settingsHideSidebarEnabled === true,
    ...
    audioSdmMode: proFeaturesUnlocked || normalizedAudioSdmMode === 'dsdPassthrough' ? normalizedAudioSdmMode : 'off',
    ...
    audioEchoSrcMode: proFeaturesUnlocked ? normalizeAudioEchoSrcMode(settings.audioEchoSrcMode) : 'off',
    audioEchoSrcQualityProfile: normalizeAudioEchoSrcQualityProfile(settings.audioEchoSrcQualityProfile),
    audioEchoSrcAdvancedModeEnabled: proFeaturesUnlocked && settings.audioEchoSrcAdvancedModeEnabled === true,
    ...
    artistMergeStrategy,
    chineseCrossScriptSearchEnabled: true,
    albumWallVirtualizationEnabled: settings.albumWallVirtualizationEnabled !== false,
    ...
    lyricsMvGraphicsPressureGuardEnabled: settings.lyricsMvGraphicsPressureGuardEnabled !== false,
    ...
    lyricsEmptyStateBackgroundEnabled: settings.lyricsEmptyStateBackgroundEnabled === true,
    ...
    lyricsThemeFilterEnabled: settings.lyricsThemeFilterEnabled !== false,
    ...
    lyricsRoseVinylGradientEnabled: settings.lyricsRoseVinylGradientEnabled === true,
    ...
    lyricsBackgroundModeOverrideEnabled:
      typeof settings.lyricsBackgroundModeOverrideEnabled === 'boolean'
        ? settings.lyricsBackgroundModeOverrideEnabled
        : normalizeLyricsBackgroundMode(settings.lyricsBackgroundMode) !== defaultSettings.lyricsBackgroundMode,
    ...
    miniPlayerUsesUltraLightMode: settings.miniPlayerUsesUltraLightMode === true,
    ...
    taskbarMiniPlayerEnabled:
      settings.taskbarMiniPlayerEnabled === undefined
        ? defaultSettings.taskbarMiniPlayerEnabled
        : settings.taskbarMiniPlayerEnabled === true,
    ...
    steamRichPresenceMode:
      settings.steamRichPresenceMode === 'off' || settings.steamRichPresenceMode === 'basic' || settings.steamRichPresenceMode === 'detailed'
        ? settings.steamRichPresenceMode
        : settings.steamRichPresenceEnabled === false
          ? 'off'
          : 'detailed',
    steamRichPresenceEnabled:
      settings.steamRichPresenceMode === 'off'
        ? false
        : settings.steamRichPresenceMode === 'basic' || settings.steamRichPresenceMode === 'detailed'
          ? true
          : settings.steamRichPresenceEnabled !== false,
    steamRichPresencePreset:
      settings.steamRichPresenceMode === 'basic'
        ? 'privacy'
        : settings.steamRichPresencePreset === 'music' || settings.steamRichPresencePreset === 'minimal' || settings.steamRichPresencePreset === 'privacy'
          ? settings.steamRichPresencePreset
          : defaultSettings.steamRichPresencePreset,
    steamRichPresenceShowAlbum:
      settings.steamRichPresenceShowAlbum === undefined
        ? defaultSettings.steamRichPresenceShowAlbum
        : settings.steamRichPresenceShowAlbum === true,
    steamRichPresenceShowProgress:
      settings.steamRichPresenceShowProgress === undefined
        ? defaultSettings.steamRichPresenceShowProgress
        : settings.steamRichPresenceShowProgress === true,
    steamRichPresenceShowGenre: settings.steamRichPresenceShowGenre === true,
    steamRichPresenceShowPlaybackOrder: settings.steamRichPresenceShowPlaybackOrder === true,
    steamRichPresenceShowBpm: settings.steamRichPresenceShowBpm === true,
    steamRichPresenceShowQuality: settings.steamRichPresenceShowQuality === true,
    steamRichPresenceShowFormat: settings.steamRichPresenceShowFormat === true,
    steamRichPresenceShowBitPerfect: settings.steamRichPresenceShowBitPerfect === true,
    steamListeningStatsEnabled:
      sourceAppMemoryVersion < 8 ? true : settings.steamListeningStatsEnabled !== false,
    steamLeaderboardsEnabled: settings.steamLeaderboardsEnabled === true,
    ...
    stageApiEnabled:
      settings.stageApiEnabled === undefined
        ? defaultSettings.stageApiEnabled
        : settings.stageApiEnabled === true,
  };
};
...
export const setAppSettings = (patch: Partial<AppSettings>, options: NormalizeSettingsOptions = {}): AppSettings => {
  const previousSettings = getAppSettings(options);
  const nextSettings = normalizeSettings({ ...previousSettings, ...patch }, options);
  const changedPatch = Object.fromEntries(
    Object.keys(patch)
      .filter((key) => JSON.stringify(previousSettings[key as keyof AppSettings]) !== JSON.stringify(nextSettings[key as keyof AppSettings]))
      .map((key) => [key, nextSettings[key as keyof AppSettings]]),
  ...
  try {
    writeRegularEditionSharedSettingsPatch(changedPatch);
    const shared = readRegularEditionSharedSettings();
    if (shared) {
      ...
      persistNormalizedSettings(mergedSettings);
      cachedSettingsSource = mergedSettings;
      cachedSettings = mergedSettings;
      regularEditionSettingsMtimeMs = shared.mtimeMs;
      return mergedSettings;
    }
  } catch (error) {
    console.warn(`[settings-sync] failed to update regular ECHO settings: ${error instanceof Error ? error.message : String(error)}`);
  }
