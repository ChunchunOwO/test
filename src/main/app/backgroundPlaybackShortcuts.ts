...
import { togglePetWindow } from './petWindow';
...
const registerActionShortcut = (
  ...
  const mouseButton = parseMouseButtonAccelerator(accelerator);
  if (mouseButton) {
    registeredMouseAccelerators.set(mouseButton, action);

    if (!ensureMouseShortcutHook()) {
      registeredMouseAccelerators.delete(mouseButton);
