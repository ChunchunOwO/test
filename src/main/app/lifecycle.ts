...
import { registerWorkshopAssetProtocolHandler } from '../protocol/workshopAssetProtocol';
import { disposeSmtcIntegration, initializeSmtcIntegration } from '../integrations/smtc/SmtcStatusSync';
import { disposeDiscordPresenceIntegration, syncDiscordPresenceIntegrationFromSettings } from '../integrations/discord/DiscordPresenceStatusSync';
import { disposeSteamRichPresenceIntegration, syncSteamRichPresenceIntegrationFromSettings } from '../integrations/steam/SteamRichPresenceStatusSync';
import {
  disposeSteamAchievementIntegration,
  initializeSteamAchievementIntegration,
  initializeSteamStartupAchievementIntegration,
} from '../integrations/steam/SteamAchievementStatusSync';
import { disposeSteamListeningStatsIntegration, initializeSteamListeningStatsIntegration } from '../integrations/steam/SteamListeningStatsStatusSync';
import { disposeSteamCloudSettingsIntegration } from '../integrations/steam/SteamCloudSettingsSync';
import { disposeSteamLeaderboardIntegration, initializeSteamLeaderboardIntegration } from '../integrations/steam/SteamLeaderboardStatusSync';
import { disposeSteamListenTogetherProbeService } from '../integrations/steam/SteamworksService';
import { disposeLastFmIntegration, syncLastFmIntegrationFromSettings } from '../integrations/lastfm/LastFmStatusSync';
...
import { enterUltraLightMode, isUltraLightModeActive, restoreUltraLightMode } from './UltraLightModeService';
import { isUltraLightGpuRuntime } from './ultraLightGpuRuntime';
...
import { applyMainWindowUiScale } from './mainWindowUiScale';
import { handleMacosOpenFile } from './macosOpenFile';
...
import { startExternalModLoader, stopExternalModLoader } from '../mods/ExternalModLoaderService';

const notifyLibraryDatabaseProtected = (): void => {
  void dialog.showMessageBox({
    type: 'warning',
    title: '曲库数据库进入保护模式',
    message: 'SQLite 明确报告音乐库数据库损坏，ECHO 已停止继续写入。',
...
export const registerAppLifecycle = (): void => {
  ...
  let canCreateMacosWindow = false;
  syncNativeThemeSource(startupSettings);

  const showMacosMainWindow = (): BrowserWindow | null => {
    ...
    if (!window && canCreateMacosWindow) {
  ...
  const sendMacosApplicationCommand = (window: BrowserWindow, action: GlobalShortcutAction): void => {
    const send = (): void => {
      if (!window.isDestroyed()) {
  ...
  const dispatchMacosApplicationCommand = (action: GlobalShortcutAction): void => {
    if (isUltraLightModeActive()) {
      void restoreUltraLightMode().then(() => {
        const window = showMacosMainWindow();
        if (window) {
          sendMacosApplicationCommand(window, action);
        }
      });
      return;
    }
  ...
  if (process.platform === 'darwin') {
    app.on('open-file', (event, filePath) => {
      handleMacosOpenFile({
        ...
        event,
        filePath,
        appReady: canCreateMacosWindow,
        recoveryMode: libraryRecoveryMode,
        getWindow: getMainWindow,
        createWindow: createMainWindow,
        dispatchFiles: (paths) => {
          if (isUltraLightModeActive()) {
            void restoreUltraLightMode().then(() => dispatchLocalAudioFilesOpened(paths));
          } else {
            dispatchLocalAudioFilesOpened(paths);
          }
        },
      });
    });
  ...
  if (startupSettings.hardwareAccelerationDisabled === true || ultraLightGpuRuntime) {
    ...
    if (ultraLightGpuRuntime) {
      // Electron otherwise starts a D3D11 WARP GPU process even after hardware
      // acceleration is disabled. Ultra-light mode has no Chromium surface, so
      // disable that software fallback and GPU compositing as well.
      app.commandLine.appendSwitch('disable-gpu');
      app.commandLine.appendSwitch('disable-gpu-compositing');
      app.commandLine.appendSwitch('disable-software-rasterizer');
    }
    markStartupStage(ultraLightGpuRuntime
      ? 'hardware-acceleration:disabled-for-ultra-light-runtime'
      : 'hardware-acceleration:disabled-by-setting');
  }

  app.commandLine.appendSwitch('autoplay-policy', 'no-user-gesture-required');
  if (process.platform === 'win32') {
    app.setAppUserModelId(app.isPackaged ? 'app.echo.steam' : 'app.echo.steam.dev');
  }
  ...
  app.whenReady().then(async () => {
    ...
    if (process.platform === 'darwin') {
      Menu.setApplicationMenu(Menu.buildFromTemplate(createMacosApplicationMenuTemplate(app.name, {
        dispatchCommand: dispatchMacosApplicationCommand,
        showMainWindow: () => {
          showMacosMainWindow();
        },
      })));
      markStartupStage('macos:application-menu:installed');
    }
    ...
    if (ultraLightGpuRuntime) {
      ...
      const ultraLightStatus = await enterUltraLightMode();
      markStartupStage('startup:ready', {
        mode: ultraLightStatus.active ? 'ultra-light-gpu-disabled' : 'ultra-light-gpu-disabled-failed',
    ...
    void syncMqttIntegrationFromSettings(appSettings).catch((error) => {
    ...
    if (dataProtection.libraryHealth.status === 'ok' && !libraryRecoveryMode) {
      void initializeSmtcIntegration();
      void syncLastFmIntegrationFromSettings(appSettings);
      void syncDiscordPresenceIntegrationFromSettings(appSettings);
      void syncSteamRichPresenceIntegrationFromSettings(appSettings);
      initializeSteamAchievementIntegration();
      initializeSteamListeningStatsIntegration();
      initializeSteamLeaderboardIntegration();
      void initializeStageBridgeIntegration();
      markStartupStage('startup-integrations:init:scheduled', { eventHub: true, echoLinkBasic: true, smtc: true, lastfm: true, discord: true, steamPresence: true, stage: true });
    ...
    canCreateMacosWindow = true;
    markStartupStage('main-window:create:request');
    const mainWindow = createMainWindow();
    markStartupStage('main-window:create:returned');
    startExternalModLoader();
    markStartupStage('external-mod-loader:start:requested');
    initializeSharedSettingsWatcher(() => {
      applyMainWindowUiScale(mainWindow, getAppSettings().accessibilityPreferences?.uiScalePercent);
  ...
  const cleanupBeforeQuit = async (): Promise<void> => {
    ...
    await disposeSteamRichPresenceIntegration();
    disposeSteamAchievementIntegration();
    disposeSteamListeningStatsIntegration();
    disposeSteamCloudSettingsIntegration();
    disposeSteamLeaderboardIntegration();
    disposeSteamListenTogetherProbeService();
