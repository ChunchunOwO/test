...
describe('desktop lyrics window bounds', () => {
  beforeEach(() => {
    ...
    mocks.browserWindowOptions.splice(0);
    mocks.displays = [
      {
        bounds: { x: 0, y: 0, width: 1920, height: 1080 },
        workArea: { x: 0, y: 110, width: 1920, height: 970 },
  ...
  it('isolates desktop lyrics from main-window accessibility zoom', async () => {
    const { createDesktopLyricsWindow } = await import('./desktopLyricsWindow');

    createDesktopLyricsWindow();

    expect(mocks.browserWindowOptions[0]?.webPreferences?.partition).toBe('echo-desktop-lyrics');
  });
...
describe('desktop lyrics startup memory', () => {
  ...
  it('restores the desktop lyrics window when it was open before exit', async () => {
    ...
    expect(mocks.createdWindows[0].showInactive).not.toHaveBeenCalled();
    mocks.createdWindows[0].emit('ready-to-show');
  ...
  it('destroys the renderer without preserving enabled state when the user hides it', async () => {
    const { hideDesktopLyricsWindow, showDesktopLyricsWindow } = await import('./desktopLyricsWindow');
    showDesktopLyricsWindow();
    const window = mocks.createdWindows[0];
    mocks.setAppSettings.mockClear();

    const state = hideDesktopLyricsWindow();
    hideDesktopLyricsWindow();

    expect(mocks.setAppSettings).toHaveBeenCalledWith({ desktopLyricsEnabled: false });
    expect(mocks.setAppSettings).not.toHaveBeenCalledWith({ desktopLyricsEnabled: true });
  ...
  it('creates a fresh renderer when desktop lyrics are enabled again', async () => {
    const { hideDesktopLyricsWindow, showDesktopLyricsWindow } = await import('./desktopLyricsWindow');
    showDesktopLyricsWindow();
    ...
    expect(mocks.createdWindows[0].destroy).toHaveBeenCalledTimes(1);
    expect(mocks.createdWindows[1].showInactive).not.toHaveBeenCalled();
    mocks.createdWindows[1].emit('ready-to-show');
    expect(mocks.createdWindows[1].showInactive).toHaveBeenCalledTimes(1);
  });
});
