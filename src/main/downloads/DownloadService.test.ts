...
const makeTempRoot = (): string => {
  const root = join(tmpdir(), `echo-download-service-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
describe('DownloadService', () => {
  ...
  it('rejects Spotify playback-only URLs before creating a download job', () => {
    ...
    expect(() => service.createUrlJob('https://open.spotify.com/track/spotify-track-id')).toThrow(
      'This streaming platform is playback-only in ECHO',
    );
    ...
    expect(fetchMock.mock.calls[1]?.[1]).toEqual(
      expect.objectContaining({
        headers: expect.objectContaining({
          ...
          'User-Agent': expect.stringContaining('ECHO'),
        }),
      }),
    );
