...
const makeTempRoot = (): string => {
  const root = join(tmpdir(), `echo-lyrics-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
const createHarness = ({
  ...
  remoteLyrics?: RemoteLyricsResult | null;
} = {}) => {
  const database = createDatabase(':memory:');
  const library = {
    getTrack: vi.fn((trackId: string) => (currentTrack.id === trackId ? currentTrack : null)),
    getRemoteLyrics: vi.fn(async () => remoteLyrics ?? null),
  };
...
describe('LyricsService', () => {
  it('uses and caches Navidrome native lyrics before external matching', async () => {
    const remoteTrack = {
      ...track(),
      mediaType: 'remote' as const,
      sourceId: 'source-1',
      stableKey: 'song-1',
    };
    const { library, service } = createHarness({
      currentTrack: remoteTrack,
      remoteLyrics: {
        provider: 'subsonic',
        providerLyricsId: 'subsonic:song-1:ja:synced',
    ...
    const match = vi.spyOn(
      (service as unknown as { matchEngine: LyricsMatchEngine }).matchEngine,
      'match',
    );

    const first = await service.getLyricsForTrack(remoteTrack.id);
    const cached = await service.getLyricsForTrack(remoteTrack.id);
    ...
    expect(cached?.provider).toBe('subsonic');
    expect(library.getRemoteLyrics).toHaveBeenCalledTimes(1);
    expect(match).not.toHaveBeenCalled();
  });

  it('uses relaxed multi-provider matching by default for remote tracks', async () => {
    const remoteTrack = {
      ...track(),
      mediaType: 'remote' as const,
      sourceId: 'source-1',
    ...
    await service.getLyricsForTrack(remoteTrack.id);

    expect(library.getRemoteLyrics).not.toHaveBeenCalled();
    expect(match).toHaveBeenCalledWith(
      expect.objectContaining({ mediaType: 'remote', sourceId: 'source-1' }),
      expect.objectContaining({ preferPrimaryProvider: false, relaxedAutoAccept: true }),
    );
  });
  ...
  it('applies custom lyrics for a track snapshot that is not in the library', async () => {
    const { library, service } = createHarness();

    const lyrics = await service.applyCustomLrcForSnapshot({
      trackId: 'airplay:living-room',
      title: 'Snapshot Song',
      ...
      stableKey: 'airplay:living-room',
    }, '[00:01.00]Snapshot line', 'snapshot.lrc');

    expect(library.getTrack).not.toHaveBeenCalledWith('airplay:living-room');
    ...
    expect(lyrics.lines).toEqual([{ timeMs: 1000, text: 'Snapshot line' }]);
    await expect(service.getLyricsForSnapshot({
      trackId: 'airplay:living-room',
      title: 'Snapshot Song',
      artist: 'Snapshot Artist',
      album: 'Snapshot Album',
      ...
      sourceId: 'living-room',
      stableKey: 'airplay:living-room',
    })).resolves.toMatchObject({ provider: 'manual' });
  });
