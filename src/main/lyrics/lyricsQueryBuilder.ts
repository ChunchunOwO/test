...
const trailingBracketPattern = /\s*(?:\(([^()]*)\)|\[([^\[\]]*)\]|\uFF08([^\uFF08\uFF09]*)\uFF09|\u3010([^\u3010\u3011]*)\u3011)\s*$/u;
const remoteArtistSeparatorPattern = /\s*(?:&|＆|\/|／|;|；|、|×)\s*/u;
const remoteVersionNoisePattern = /\b(?:ver(?:sion)?\.?|remaster(?:ed)?|live|mix|edit|demo|acoustic|instrumental|karaoke|off[\s-]?vocal|tv[\s-]?size|sfc)\b|ライブ|伴奏|カラオケ|重制/iu;
...
const primaryRemoteArtist = (artist: string): string | null => {
  const [primary] = artist.split(remoteArtistSeparatorPattern);
  const trimmed = cleanLooseTitle(primary);
  return trimmed && normalizeTextForIdentity(trimmed) !== normalizeTextForIdentity(artist) ? trimmed : null;
};

const stripRemoteVersionSuffix = (title: string): string | null => {
  const normalized = title.normalize('NFKC');
  const match = normalized.match(trailingBracketPattern);
  const descriptor = match?.slice(1).find((value): value is string => Boolean(value)) ?? '';
  if (!match || match.index === undefined || !remoteVersionNoisePattern.test(descriptor)) {
    return null;
  }

  const stripped = cleanLooseTitle(normalized.slice(0, match.index));
  return stripped && normalizeTextForIdentity(stripped) !== normalizeTextForIdentity(title) ? stripped : null;
};
...
export const buildNormalizedLyricsQuery = (query: LyricsQuery): NormalizedLyricsQuery => {
  ...
  const remotePrimaryArtist = query.mediaType === 'remote' ? primaryRemoteArtist(rawArtist) : null;
  const remoteVersionlessTitle = query.mediaType === 'remote' ? stripRemoteVersionSuffix(rawTitle) : null;
  ...
  if (remoteVersionlessTitle) {
    pushVariant(variants, {
      title: remoteVersionlessTitle,
      artist: rawArtist,
      album: rawAlbum,
  ...
  if (remotePrimaryArtist) {
    pushVariant(variants, {
      title: featuredTitle ?? remoteVersionlessTitle ?? rawTitle,
      artist: remotePrimaryArtist,
      album: rawAlbum,
