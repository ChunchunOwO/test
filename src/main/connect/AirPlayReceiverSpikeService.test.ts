...
import { airPlay2PtpFeatureMask } from './AirPlayMdnsAdvertiser';
...
const createAvailablePtpClock = () => ({
  start: vi.fn(async () => undefined),
  stop: vi.fn(async () => undefined),
  setPeerAddresses: vi.fn(),
  getStatus: vi.fn(() => ({
    running: true,
  ...
  localTimeForMasterTime: vi.fn(() => null),
});
...
const decryptControlFrameSequence = (
  key: Buffer,
  firstCounter: number,
  frames: Buffer,
): { plaintext: Buffer; frameCount: number; consumed: number } => {
  const chunks: Buffer[] = [];
  let offset = 0;
  let frameCount = 0;
  while (offset + 2 <= frames.length) {
    const payloadLength = frames.readUInt16LE(offset);
    const frameLength = 2 + payloadLength + 16;
    if (offset + frameLength > frames.length) break;
    chunks.push(decryptControlFrame(
      key,
      firstCounter + frameCount,
      frames.subarray(offset, offset + frameLength),
    ));
    offset += frameLength;
    frameCount += 1;
  }
  return { plaintext: Buffer.concat(chunks), frameCount, consumed: offset };
};
...
const dmapAtom = (tag: string, value: Buffer | string | number): Buffer => {
...
const testBplistIntByteLength = (value: number | bigint): 1 | 2 | 4 | 8 => {
...
const encodeTestBplistInt = (value: number | bigint): Buffer => {
...
const encodeTestBplistObject = (record: TestBplistRecord, refSize: number): Buffer => {
  ...
  if (typeof value === 'number' || typeof value === 'bigint') return encodeTestBplistInt(value);
...
describe('AirPlayReceiverSpikeService', () => {
  it('uses the current Electron executable for the AirPlay helper in packaged builds', () => {
    const nodePath = resolveAirPlayHelperNodePath({
      isPackaged: true,
      processExecPath: 'C:\\Program Files\\ECHO\\ECHO.exe',
      npmNodeExecPath: 'C:\\Program Files\\ECHO\\ECHO.exe',
      nodeEnvPath: 'C:\\stale\\node.exe',
    });

    expect(nodePath).toBe('C:\\Program Files\\ECHO\\ECHO.exe');
  });
  ...
  it('keeps the normal AirPlay receiver toggle on AirPlay 1 discovery by default', async () => {
    const mdnsStarts: Array<{ airPlay2Experimental?: boolean; airPlayPort?: number | null; airPlayPublicKey?: string | null; timingMode?: 'ntp' | 'ptp' }> = [];
    const service = new AirPlayReceiverSpikeService({
      audioSession: new FakeAudioSession() as never,
      getAdvertiseInterfaces: () => [
        { name: 'Wi-Fi', address: '192.168.31.214', mac: '60:CF:84:CB:1E:D1' },
      ...
      createMdnsAdvertiser: () => ({
        start: vi.fn(async (advertisement) => {
          const start: (typeof mdnsStarts)[number] = {
          ...
          if (advertisement.airPlay2Experimental) start.timingMode = advertisement.airPlay2TimingMode;
          mdnsStarts.push(start);
        }),
        stop: vi.fn(async () => undefined),
      }),
      loadRaopModule: async () => ({
  ...
  it('advertises AirPlay 2 discovery when the receiver protocol setting selects the experimental mode', async () => {
    ...
    const service = new AirPlayReceiverSpikeService({
      ...
      ptpClock: createAvailablePtpClock(),
      getAdvertiseInterfaces: () => [
        { name: 'Wi-Fi', address: '192.168.31.214', mac: '60:CF:84:CB:1E:D1' },
      ],
      createMdnsAdvertiser: () => ({
        start: vi.fn(async (advertisement) => {
          mdnsStarts.push({
            ...
            airPlayPort: advertisement.airPlayPort,
            airPlayPublicKey: advertisement.airPlayPublicKey,
            timingMode: advertisement.airPlay2TimingMode,
          });
        }),
        stop: vi.fn(async () => undefined),
      }),
  ...
  it('answers AirPlay 2 experimental info probes on the AirPlay control port', async () => {
    ...
    expect(body).toContain(`<integer>${airPlay2PtpFeatureMask}</integer>`);
    expect(body).toContain('<key>audioFormats</key>');
    expect(body).toContain('<integer>13107196</integer>');
    expect(body).toContain('<key>supportedFormats</key>');
    expect(body).toContain('<key>audioStream</key><integer>4458496</integer>');
    expect(body).toContain('<key>bufferStream</key><integer>12845056</integer>');
    ...
    try {
      ...
      expect(responseBody.toString('binary')).toContain('supportedFormats');
      expect(responseBody.toString('binary')).toContain('bufferStream');
      expect(responseBody.toString('binary')).toContain('txtAirPlay');
      expect(responseBody.toString('binary')).toContain('featuresEx');
  ...
  it('answers AirPlay 2 Pair-Setup M1-M6 exchange', async () => {
    ...
    expect(saveController).toHaveBeenCalledWith({
  ...
  it('enables encrypted control frames after transient AirPlay 2 Pair-Setup M1-M4 with the fallback cipher', async () => {
    ...
    try {
      ...
      socket.write(encryptControlFrame(controlWriteKey, 3, rawRequest(
        'SETUP',
        `rtsp://127.0.0.1/${Date.now()}`,
        encodeTestBplist({
          timingProtocol: 'NTP',
          timingPort: 7000,
          name: 'Transient iPhone',
          model: 'iPhone16,2',
          sourceVersion: '409.16',
        }),
        ['CSeq: 35'],
      )));
      const encryptedSetupResponse = await readUntil(socket, hasCompleteControlFrame);
      const setupResponse = decryptControlFrame(controlReadKey, 2, encryptedSetupResponse);
      expect(setupResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(setupResponse.toString('utf8')).toContain('CSeq: 35');
      const setupEvent = [...service.getStatus().debugEvents].reverse().find((event) =>
        event.action === 'setup' && event.message?.includes('session setup acknowledged'),
      );
      const eventPort = Number(setupEvent?.message?.match(/eventPort=(\d+)/u)?.[1]);
      expect(eventPort).toBeGreaterThan(0);
      eventSocket = connect({ host: '127.0.0.1', port: eventPort });
      await new Promise<void>((resolve, reject) => {
        eventSocket!.once('connect', resolve);
        eventSocket!.once('error', reject);
      });
      const eventWriteKey = derivePairSetupKey(
        srpClient.sessionKey,
        'Events-Salt',
        'Events-Write-Encryption-Key',
      );
      const encryptedUpdate = await readUntil(eventSocket, (buffer) => {
        const decrypted = decryptControlFrameSequence(eventWriteKey, 0, buffer);
        return decrypted.consumed === buffer.length && hasCompleteTextResponse(decrypted.plaintext);
      });
      const update = decryptControlFrameSequence(eventWriteKey, 0, encryptedUpdate);
      expect(update.frameCount).toBeGreaterThan(1);
      expect(update.plaintext.toString('binary')).toContain('updateInfo');
      const eventReadKey = derivePairSetupKey(
        srpClient.sessionKey,
        'Events-Salt',
        'Events-Read-Encryption-Key',
      );
      eventSocket.write(encryptControlFrame(
        eventReadKey,
        0,
        Buffer.from('RTSP/1.0 200 OK\r\nContent-Length: 0\r\n\r\n', 'utf8'),
      ));
      await vi.waitFor(() => expect(service.getStatus().debugEvents.some((event) =>
        event.action === 'event' && event.message?.includes('RTSP/1.0 200 OK'),
      )).toBe(true));
      ...
      expect(service.getStatus().debugEvents.some((event) =>
        event.action === 'crypto' && event.message?.includes('key=srp-session-key'),
      )).toBe(true);
      expect(service.getStatus().debugEvents.some((event) =>
  ...
  it('answers the AirPlay 2 Pair-Verify M1/M3 cryptographic exchange', async () => {
    const mdnsStarts: Array<{ airPlayPort?: number | null }> = [];
    const controllerIdentifier = Buffer.from('test-client', 'utf8');
    const controllerKeys = generateKeyPairSync('ed25519');
    const controllerPublicKey = exportEd25519PublicKey(controllerKeys.publicKey);
    const service = new AirPlayReceiverSpikeService({
      audioSession: new FakeAudioSession() as never,
      ...
      pairingStore: {
        getController: vi.fn((identifier) => identifier === controllerIdentifier.toString('utf8')
          ? { identifier, publicKey: controllerPublicKey, permissions: 0 }
          : null),
        saveController: vi.fn(),
      },
    });
    ...
    const controllerSignature = sign(
      null,
      Buffer.concat([clientPublicKey, controllerIdentifier, serverPublicKey]),
      controllerKeys.privateKey,
    );
  ...
  it('falls back to an NTP/realtime advertisement when PTP ports are unavailable', async () => {
    const timingModes: Array<'ntp' | 'ptp' | undefined> = [];
    const ptpClock = {
      start: vi.fn(async () => { throw new Error('EADDRINUSE 0.0.0.0:319'); }),
      stop: vi.fn(async () => undefined),
      setPeerAddresses: vi.fn(),
      getStatus: vi.fn(() => ({
        running: false,
    ...
    const service = new AirPlayReceiverSpikeService({
      ...
      createMdnsAdvertiser: () => ({
        start: vi.fn(async (advertisement) => {
          timingModes.push(advertisement.airPlay2TimingMode);
        }),
        stop: vi.fn(async () => undefined),
      }),
      loadRaopModule: async () => ({
    ...
    const status = await service.setEnabled(true);
    expect(timingModes).toEqual(['ntp']);
    expect(status.debugEvents.some((event) =>
      event.action === 'timing' && event.message?.includes('using realtime/NTP profile'),
    )).toBe(true);
    await service.setEnabled(false);
    expect(ptpClock.stop).toHaveBeenCalled();
  });

  it('captures AirPlay 2 DACP headers and routes computer transport controls to the sender', async () => {
    let airPlayPort: number | null = null;
    const nativeRemoteCommand = vi.fn(() => true);
    const dacpRemote = {
      updateSender: vi.fn(),
      clear: vi.fn(),
    ...
    const service = new AirPlayReceiverSpikeService({
      ...
      createMdnsAdvertiser: () => ({
        start: vi.fn(async (advertisement) => {
          airPlayPort = advertisement.airPlayPort ?? null;
        }),
        stop: vi.fn(async () => undefined),
      }),
      loadRaopModule: async () => ({
        startReceiver: vi.fn(() => 24),
        stopReceiver: vi.fn(),
        sendRemoteCommand: nativeRemoteCommand,
      }),
    });

    await service.setEnabled(true);
    expect(airPlayPort).not.toBeNull();
    const response = await fetch(`http://127.0.0.1:${airPlayPort}/info`, {
      headers: {
    ...
    expect(response.status).toBe(200);
    expect(dacpRemote.updateSender).toHaveBeenCalledWith({
    ...
    expect(dacpRemote.send).toHaveBeenNthCalledWith(1, 'pause');
    expect(dacpRemote.send).toHaveBeenNthCalledWith(2, 'play');
    expect(nativeRemoteCommand).not.toHaveBeenCalled();

    await service.setEnabled(false);
    expect(dacpRemote.clear).toHaveBeenCalled();
  });

  it('plays the advertised unencrypted AirPlay 2 realtime RTP path without a shared key', async () => {
    const mdnsStarts: Array<{ airPlayPort?: number | null }> = [];
    const audio = new FakeAudioSession();
    const service = new AirPlayReceiverSpikeService({
      audioSession: audio as never,
    ...
    try {
      ...
      socket.write(rawRequest('SETUP', `rtsp://127.0.0.1/${Date.now()}`, encodeTestBplist({
        et: 0,
        name: 'Clear RTP sender',
      }), ['CSeq: 1']));
      const sessionResponse = await readUntil(socket, hasCompleteTextResponse);
      expect(sessionResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(sessionResponse.includes(Buffer.from('127.0.0.1', 'utf8'))).toBe(true);
      expect(sessionResponse.includes(Buffer.from('192.168.31.214', 'utf8'))).toBe(false);
      ...
      const streamResponse = await readUntil(socket, hasCompleteTextResponse);
      expect(streamResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      const setupEvent = service.getStatus().debugEvents.find((event) =>
        event.action === 'setup' && event.message?.includes('clear realtime UDP LPCM RTP ready'),
      );
      const dataPort = Number(setupEvent?.message?.match(/dataPort=(\d+)/u)?.[1]);
      expect(dataPort).toBeGreaterThan(0);

      const rtpHeader = Buffer.alloc(12);
      rtpHeader[0] = 0x80;
      rtpHeader[1] = 96;
      rtpHeader.writeUInt16BE(9, 2);
      rtpHeader.writeUInt32BE(654_321, 4);
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          udpSocket.send(Buffer.concat([rtpHeader, lpcmPayload]), dataPort, '127.0.0.1', (error) => {
      ...
      await vi.waitFor(() => expect(audio.playPcmStream).toHaveBeenCalledTimes(1));
      expect(service.getStatus().debugEvents.some((event) =>
        event.action === 'rtp' && event.message?.includes('clear=8b; pcm=f32le'),
      )).toBe(true);
    } finally {
      socket.destroy();
      await service.setEnabled(false);
  ...
  it('buffers type 103 RTP over TCP until SETRATEANCHORTIME starts playback', async () => {
    const mdnsStarts: Array<{ airPlayPort?: number | null }> = [];
    const audio = new FakeAudioSession();
    const service = new AirPlayReceiverSpikeService({
      audioSession: audio as never,
    ...
    await service.setEnabled(true);
    const controlSocket = connect({ host: '127.0.0.1', port: mdnsStarts[0]!.airPlayPort! });
    ...
    try {
      ...
      const sendRequest = async (method: string, path: string, body: Buffer, cseq: number): Promise<Buffer> => {
        controlSocket.write(rawRequest(method, path, body, [`CSeq: ${cseq}`], 'application/x-apple-binary-plist'));
        return readUntil(controlSocket, hasCompleteTextResponse);
      };

      const sessionResponse = await sendRequest('SETUP', `rtsp://127.0.0.1/${Date.now()}`, encodeTestBplist({
        et: 0,
      ...
      const eventPort = Number(sessionSetupEvent?.message?.match(/eventPort=(\d+)/u)?.[1]);
      expect(eventPort).toBeGreaterThan(0);
      eventSocket = connect({ host: '127.0.0.1', port: eventPort });
      await new Promise<void>((resolve, reject) => {
        eventSocket!.once('connect', resolve);
      ...
      const streamResponse = await sendRequest('SETUP', `rtsp://127.0.0.1/${Date.now()}`, encodeTestBplist({
        streams: [{
          type: 103,
          ct: 1,
          audioFormat: 0x800,
      ...
      expect(streamResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(streamResponse.toString('binary')).toContain('audioBufferSize');
      expect(streamResponse.toString('binary')).toContain('streamID');
      const setupEvent = service.getStatus().debugEvents.find((event) =>
        event.action === 'setup' && event.message?.includes('buffered TCP LPCM RTP ready'),
      );
      const dataPort = Number(setupEvent?.message?.match(/dataPort=(\d+)/u)?.[1]);
      expect(dataPort).toBeGreaterThan(0);

      const feedbackResponse = await sendRequest('POST', '/feedback', encodeTestBplist({}), 3);
      expect(feedbackResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(feedbackResponse.toString('binary')).toContain('audioBufferSize');

      const configureResponse = await sendRequest('POST', '/configure', encodeTestBplist({}), 4);
      expect(configureResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(configureResponse.toString('utf8')).toContain('Content-Type: application/x-apple-binary-plist');
      expect(configureResponse.toString('binary')).toContain('bplist00');

      dataSocket = connect({ host: '127.0.0.1', port: dataPort });
      await new Promise<void>((resolve, reject) => {
      ...
      packet[0] = 0x80;
      packet.writeUIntBE(9, 1, 3);
      packet.writeUInt32BE(654_321, 4);
      packet.writeUInt32BE(0x01020304, 8);
      Buffer.from([0x00, 0x00, 0x40, 0x00, 0xc0, 0x00, 0x7f, 0xff]).copy(packet, 12);
      const frameLength = Buffer.alloc(2);
      frameLength.writeUInt16BE(packet.length + 2);
      dataSocket.write(Buffer.concat([frameLength, packet]));
      await new Promise((resolve) => setTimeout(resolve, 20));
      expect(audio.playPcmStream).not.toHaveBeenCalled();

      const anchorResponse = await sendRequest('SETRATEANCHORTIME', '*', encodeTestBplist({
        rate: 1,
        rtpTime: 654_321,
      }), 5);
      expect(anchorResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      await vi.waitFor(() => expect(audio.playPcmStream).toHaveBeenCalledTimes(1));
      expect(service.getStatus().debugEvents.some((event) =>
        event.action === 'anchor' && event.message?.includes('queuedPackets=1') && event.message?.includes('remainingPackets=0'),
      )).toBe(true);

      const pauseResponse = await sendRequest('PAUSE', '*', encodeTestBplist({}), 6);
      expect(pauseResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(audio.pause).toHaveBeenCalledTimes(1);

      packet.writeUIntBE(10, 1, 3);
      packet.writeUInt32BE(654_673, 4);
      dataSocket.write(Buffer.concat([frameLength, packet]));
      await new Promise((resolve) => setTimeout(resolve, 20));
      expect(audio.playPcmStream).toHaveBeenCalledTimes(1);

      const flushResponse = await sendRequest('FLUSHBUFFERED', '*', encodeTestBplist({
        flushFromSeq: 10,
        flushUntilSeq: 10,
      }), 7);
      expect(flushResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      const resumeResponse = await sendRequest('SETRATEANCHORTIME', '*', encodeTestBplist({
        rate: 1,
        rtpTime: 654_673,
      }), 8);
      expect(resumeResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      await new Promise((resolve) => setTimeout(resolve, 20));
      expect(audio.playPcmStream).toHaveBeenCalledTimes(1);

      packet.writeUIntBE(11, 1, 3);
      packet.writeUInt32BE(655_025, 4);
      dataSocket.write(Buffer.concat([frameLength, packet]));
      await vi.waitFor(() => expect(audio.playPcmStream).toHaveBeenCalledTimes(2));
    } finally {
      eventSocket?.destroy();
      dataSocket?.destroy();
  ...
  it('accepts advertised buffered AAC-LC and routes decoded float PCM into Audio Core', async () => {
    const mdnsStarts: Array<{ airPlayPort?: number | null }> = [];
    const audio = new FakeAudioSession();
    const writeFrame = vi.fn<(frame: Buffer) => boolean>();
    const decoderStop = vi.fn(async () => undefined);
    const decoderReset = vi.fn(async () => undefined);
    const pauseOutput = vi.fn();
    const resumeOutput = vi.fn();
    const formats: Array<{ audioFormat: number; sampleRate: number; channels: number }> = [];
    const service = new AirPlayReceiverSpikeService({
      audioSession: audio as never,
      airPlay2Experimental: true,
      ptpClock: {
        start: vi.fn(async () => { throw new Error('test PTP unavailable'); }),
        stop: vi.fn(async () => undefined),
        setPeerAddresses: vi.fn(),
        getStatus: vi.fn(() => ({
          running: false,
      ...
      createAirPlay2AacDecoder: async (format, handlers) => {
        formats.push(format);
        writeFrame.mockImplementation((frame) => {
          handlers.onPcm(Buffer.from([0, 0, 0, 0, 0, 0, 0, 0]));
          return frame.length > 0;
        });
        return { writeFrame, reset: decoderReset, stop: decoderStop, pauseOutput, resumeOutput };
      },
      getAdvertiseInterfaces: () => [
        { name: 'Wi-Fi', address: '192.168.31.214', mac: '60:CF:84:CB:1E:D1' },
      ],
    ...
    try {
      ...
      expect((await sendRequest('SETUP', '*', encodeTestBplist({ et: 0 }), 1)).toString('utf8'))
        .toContain('RTSP/1.0 200 OK');
      const streamResponse = await sendRequest('SETUP', '*', encodeTestBplist({
        streams: [{ type: 103, ct: 4, audioFormat: 0x800000, spf: 1024 }],
      }), 2);
      expect(streamResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(formats).toEqual([{ audioFormat: 0x800000, sampleRate: 48_000, channels: 2 }]);
      const setupEvent = service.getStatus().debugEvents.find((event) =>
        event.action === 'setup' && event.message?.includes('buffered TCP AAC-LC RTP ready'),
      );
      const dataPort = Number(setupEvent?.message?.match(/dataPort=(\d+)/u)?.[1]);
      expect(dataPort).toBeGreaterThan(0);
      ...
      packet[0] = 0x80;
      packet.writeUIntBE(1, 1, 3);
      packet.writeUInt32BE(1_024, 4);
      packet.writeUInt32BE(0x01020304, 8);
      Buffer.from([0x21, 0x10, 0x04, 0x60]).copy(packet, 12);
      const frameLength = Buffer.alloc(2);
      frameLength.writeUInt16BE(packet.length + 2);
      dataSocket.write(Buffer.concat([frameLength, packet]));
      ...
      expect((await sendRequest('SETRATEANCHORTIME', '*', encodeTestBplist({
        rate: 1,
        rtpTime: 1_024,
      }), 3)).toString('utf8')).toContain('RTSP/1.0 200 OK');
      await vi.waitFor(() => expect(audio.playPcmStream).toHaveBeenCalledTimes(1));
      expect(writeFrame).toHaveBeenCalledWith(Buffer.from([0x21, 0x10, 0x04, 0x60]));
      expect(audio.playPcmStream.mock.calls[0]?.[0]).toEqual(expect.objectContaining({ sampleRate: 48_000, channels: 2 }));

      expect((await sendRequest('FLUSHBUFFERED', '*', encodeTestBplist({
        flushFromSeq: 1,
        flushUntilSeq: 1,
      }), 4)).toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(decoderReset).toHaveBeenCalledTimes(1);
      expect(audio.pause).toHaveBeenCalledTimes(1);

      packet.writeUIntBE(2, 1, 3);
      packet.writeUInt32BE(2_048, 4);
      dataSocket.write(Buffer.concat([frameLength, packet]));
      expect((await sendRequest('SETRATEANCHORTIME', '*', encodeTestBplist({
        rate: 1,
        rtpTime: 2_048,
      }), 5)).toString('utf8')).toContain('RTSP/1.0 200 OK');
      await vi.waitFor(() => expect(audio.playPcmStream).toHaveBeenCalledTimes(2));
    } finally {
      dataSocket?.destroy();
      controlSocket.destroy();
    ...
    expect(decoderStop).toHaveBeenCalledTimes(1);
  });

  it('uses PTP clock and 64-bit timeline anchors to schedule buffered playback', async () => {
    const mdnsStarts: Array<{ airPlayPort?: number | null; timingMode?: 'ntp' | 'ptp' }> = [];
    const audio = new FakeAudioSession();
    const timelineId = 0x8877_6655_4433_2211n;
    const localAnchorTime = BigInt(Date.now() + 260) * 1_000_000n;
    const localTimeForMasterTime = vi.fn(() => localAnchorTime);
    const ptpClock = {
      start: vi.fn(async () => undefined),
      stop: vi.fn(async () => undefined),
      setPeerAddresses: vi.fn(),
      getStatus: vi.fn(() => ({
        running: true,
        masterClockIdentity: timelineId.toString(16),
        sourceAddress: '127.0.0.1',
        lastSampleAtNanoseconds: BigInt(Date.now()) * 1_000_000n,
        smoothedOffsetNanoseconds: 0n,
        sampleCount: 1,
      })),
    ...
    await service.setEnabled(true);
    expect(mdnsStarts[0]?.timingMode).toBe('ptp');
    const controlSocket = connect({ host: '127.0.0.1', port: mdnsStarts[0]!.airPlayPort! });
    let dataSocket: Socket | null = null;
    try {
      await new Promise<void>((resolve, reject) => {
      ...
      const sessionResponse = await sendRequest('SETUP', '*', encodeTestBplist({
        et: 0,
        timingProtocol: 'PTP',
      }), 1);
      expect(sessionResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(ptpClock.setPeerAddresses).toHaveBeenCalledWith(['127.0.0.1']);
      ...
      const packet = Buffer.alloc(20);
      packet[0] = 0x80;
      packet.writeUIntBE(12, 1, 3);
      packet.writeUInt32BE(900_000, 4);
      packet.writeUInt32BE(0x01020304, 8);
      Buffer.from([0x00, 0x00, 0x40, 0x00, 0xc0, 0x00, 0x7f, 0xff]).copy(packet, 12);
      const frameLength = Buffer.alloc(2);
      frameLength.writeUInt16BE(packet.length + 2);
      ...
      const anchorResponse = await sendRequest('SETRATEANCHORTIME', '*', encodeTestBplist({
        rate: 1,
        rtpTime: 900_000,
        networkTimeSecs: 1_800_000_000n,
        networkTimeFrac: 0n,
        networkTimeTimelineID: timelineId,
      }), 3);
      expect(anchorResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(localTimeForMasterTime).toHaveBeenCalledWith(1_800_000_000_000_000_000n, timelineId);
      await new Promise((resolve) => setTimeout(resolve, 20));
      expect(audio.playPcmStream).not.toHaveBeenCalled();
      await vi.waitFor(() => expect(audio.playPcmStream).toHaveBeenCalledTimes(1), { timeout: 1_000 });
    } finally {
      dataSocket?.destroy();
      controlSocket.destroy();
      await service.setEnabled(false);
  ...
  it('decrypts AirPlay 2 control/RTP frames after Pair-Verify with the fallback cipher', async () => {
    ...
    const controllerIdentifier = Buffer.from('rtsp-client', 'utf8');
    const controllerKeys = generateKeyPairSync('ed25519');
    const controllerPublicKey = exportEd25519PublicKey(controllerKeys.publicKey);
    const service = new AirPlayReceiverSpikeService({
      audioSession: audio as never,
    ...
    try {
      ...
      const encryptedUpdateInfo = await readUntil(eventSocket, (buffer) => {
        const decrypted = decryptControlFrameSequence(eventWriteKey, 0, buffer);
        return decrypted.consumed === buffer.length && hasCompleteTextResponse(decrypted.plaintext);
      });
      const updateInfo = decryptControlFrameSequence(eventWriteKey, 0, encryptedUpdateInfo);
      expect(updateInfo.frameCount).toBeGreaterThan(1);
      expect(updateInfo.plaintext.toString('utf8')).toContain('POST /command RTSP/1.0');
      expect(updateInfo.plaintext.toString('binary')).toContain('bplist00');
      expect(updateInfo.plaintext.toString('binary')).toContain('updateInfo');
      expect(updateInfo.plaintext.toString('binary')).toContain('supportedFormats');
      const eventReadKey = derivePairSetupKey(
        sharedSecret,
        'Events-Salt',
        'Events-Read-Encryption-Key',
      ...
      writeCounter += 1;
      const encryptedInitialInfo = await readUntil(socket, (buffer) => {
        const decrypted = decryptControlFrameSequence(controlReadKey, readCounter, buffer);
        return decrypted.consumed === buffer.length && hasCompleteTextResponse(decrypted.plaintext);
      });
      const decryptedInitialInfo = decryptControlFrameSequence(controlReadKey, readCounter, encryptedInitialInfo);
      const initialInfoResponse = decryptedInitialInfo.plaintext;
      readCounter += decryptedInitialInfo.frameCount;
      expect(decryptedInitialInfo.frameCount).toBeGreaterThan(1);
      ...
      expect(initialInfoResponse.toString('binary')).toContain('supportedFormats');
      expect(initialInfoResponse.toString('binary')).toContain('txtAirPlay');
      ...
      const unsupportedSetupEvent = service.getStatus().debugEvents.find((event) =>
        event.action === 'setup' && event.message?.includes('supported path requires LPCM or ALAC'),
      );
      expect(unsupportedSetupEvent?.message).toContain('ct=2');
      expect(unsupportedSetupEvent?.message).toContain('audioFormat=131072');
      ...
      const missingKeySetupEvent = service.getStatus().debugEvents.find((event) =>
        event.action === 'setup' && event.message?.includes('encrypted RTP requires a 32-byte shared key'),
      );
      ...
      const bufferedSetupEvent = service.getStatus().debugEvents.find((event) =>
        event.action === 'setup' && event.message?.includes('supported audio stream types are realtime 96 and buffered 103'),
      );
      expect(bufferedSetupEvent?.message).toContain('type=110');
      ...
      const dmapMetadata = dmapAtom('mlit', Buffer.concat([
        dmapAtom('minm', 'AirPlay 2 Song'),
        dmapAtom('asar', 'AirPlay Artist'),
        dmapAtom('asal', 'AirPlay Album'),
        dmapAtom('asaa', 'Album Artist'),
        dmapAtom('astm', 181_500),
      ]));
      socket.write(encryptControlFrame(controlWriteKey, writeCounter, rawRequest(
        'SET_PARAMETER',
        '*',
      ...
      writeCounter += 1;
      const encryptedMetadata = await readUntil(socket, hasCompleteControlFrame);
      const metadataResponse = decryptControlFrame(controlReadKey, readCounter, encryptedMetadata);
      readCounter += 1;
      expect(metadataResponse.toString('utf8')).toContain('RTSP/1.0 200 OK');
      expect(metadataResponse.toString('utf8')).toContain('CSeq: 22a');
      expect(service.getStatus()).toEqual(expect.objectContaining({
        metadata: expect.objectContaining({
          title: 'AirPlay 2 Song',
      ...
      const alacSetupEvent = [...service.getStatus().debugEvents].reverse().find((event) =>
        event.action === 'setup' && event.message?.includes('encrypted realtime UDP ALAC RTP ready'),
      );
      ...
      expect(alacSetupEvent?.message).toContain('audioFormat=unknown');
      expect(alacSetupEvent?.message).toContain('sr=44100');
      expect(alacSetupEvent?.message).toContain('alac=44100/16/2');
      expect(audio.stop).toHaveBeenCalledTimes(1);
      expect(service.getStatus().currentSourceId).toBeNull();
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        ...
        const alacPcmRequest = audio.playPcmStream.mock.calls[1]?.[0];
        alacPcmChunk = alacPcmRequest?.stream.read() as Buffer | null;
      ...
      expect(audio.playPcmStream.mock.calls[1]?.[0]).toEqual(expect.objectContaining({
        sampleRate: 44_100,
        channels: 2,
      }));
      expect(audio.playPcmStream.mock.calls[1]?.[0].sourceId).not.toBe(pcmRequest?.sourceId);
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        const resumedPcmRequest = audio.playPcmStream.mock.calls[2]?.[0];
      ...
      expect(audio.playPcmStream).toHaveBeenCalledTimes(3);
  ...
  it('sends AirPlay remote commands for computer transport controls', async () => {
    ...
    await vi.waitFor(() => expect(service.getStatus().state).toBe('idle'));
    expect(audio.stop).toHaveBeenCalledTimes(1);
    expect(sendRemoteCommand).not.toHaveBeenCalledWith(15, 'stop');
  });
  ...
  it('maps AirPlay dB volume events without muting normal playback', async () => {
    ...
    await vi.waitFor(() => expect(audio.setOutput).toHaveBeenCalledWith({ volume: 0.1 }));
    expect(service.getStatus().volume).toBe(10);
  });

  it('does not report a sender volume change when Audio Core rejects it', async () => {
    const audio = new FakeAudioSession();
    audio.setOutput.mockRejectedValueOnce(new Error('output unavailable'));
    const harness: { handler?: (event: Record<string, unknown>) => void } = {};
    const service = new AirPlayReceiverSpikeService({
      audioSession: audio as never,
      loadRaopModule: async () => ({
    ...
    expect(service.getStatus().volume).toBe(100);
    expect(service.getStatus().error).toContain('output unavailable');
  });
  ...
  it('releases the AirPlay session when local playback takes over', async () => {
    ...
    await vi.waitFor(() => expect(sendRemoteCommand).toHaveBeenCalledWith(8, 'stop'));
    const status = service.getStatus();
    expect(audio.playPcmStream).toHaveBeenCalledTimes(1);
    expect(status.state).toBe('idle');
