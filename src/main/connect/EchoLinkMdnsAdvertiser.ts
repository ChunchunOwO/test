...
export class EchoLinkMdnsAdvertiser {
  ...
  private queryResponseTimer: ReturnType<typeof setTimeout> | null = null;

  private readonly handleSocketError = (): void => {
    // Keep a permanent error boundary so a late UDP error cannot terminate the main process.
  };

  private readonly handleMessage = (message: Buffer): void => {
    if (!this.isRelevantQuery(message) || this.queryResponseTimer) {
      return;
    }
    this.queryResponseTimer = setTimeout(() => {
      this.queryResponseTimer = null;
      void this.announce(false).catch(() => undefined);
    }, 20);
    this.queryResponseTimer.unref?.();
  };
  ...
  async stop(goodbye = true): Promise<void> {
    ...
    if (this.queryResponseTimer) {
      clearTimeout(this.queryResponseTimer);
      this.queryResponseTimer = null;
    }
    if (goodbye) {
  ...
  private isRelevantQuery(message: Buffer): boolean {
    const advertisement = this.advertisement;
    if (!advertisement || message.byteLength < 12 || (message[2] & 0x80) !== 0) {
      return false;
    }
    ...
    return [
      ...
      instance,
      host,
      `${safeDnsLabel(advertisement.name)}.local`,
    ].some((name) => message.includes(writeName(name)));
  }
