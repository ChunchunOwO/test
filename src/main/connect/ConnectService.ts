...
export class ConnectService extends EventEmitter<ConnectEvents> {
  ...
  async preflight(request: ConnectStartRequest): Promise<ConnectPreflightResult> {
    const checkedAt = new Date().toISOString();
    if (request.deviceId === airPlayPlaceholder.id) {
      return {
        deviceId: airPlayPlaceholder.id,
        deviceName: airPlayPlaceholder.name,
        protocol: 'airplay',
        ready: false,
        checkedAt,
        source: null,
        delivery: 'unsupported',
        capabilities: airPlayPlaceholder.capabilities,
        issues: ['airplay_not_ready'],
        warnings: [],
      };
    }

    if (request.deviceId === hqPlayerConnectDeviceId) {
      const device = this.hqPlayerDevice();
      let item: PlayableTrack;
      try {
        item = this.createHqPlayerPlayableTrack(request);
      } catch {
        return {
          deviceId: device.id,
      ...
      const settings = this.hqPlayerService.getSettings();
      const connection = await this.hqPlayerService.testConnection({ ...settings, enabled: true });
      return {
        deviceId: device.id,
        deviceName: device.name,
        protocol: 'hqplayer',
        ...
        source: {
          title: item.title,
          mimeType: null,
          remote: item.mediaType !== 'local',
        },
        delivery: 'hqplayer',
        capabilities: device.capabilities,
        issues: connection.ok ? [] : ['hqplayer_unavailable'],
        warnings: settings.enabled ? [] : ['hqplayer_will_enable'],
      };
    }

    const device = await this.refreshAndFindDevice(request.deviceId);
    const publicDevice = device ? this.publicDevice(device) : null;
    const capabilities = publicDevice?.capabilities ?? {
      canPlay: false,
      canPause: false,
      canStop: false,
      canSeek: false,
    ...
    const filePath = this.resolvePlaybackFilePath(request, track, status.currentFilePath);
    const issues: ConnectPreflightResult['issues'] = [];
    if (!device || device.state === 'unavailable') {
      issues.push('device_unavailable');
    }
    if (!filePath) {
      issues.push('source_missing');
    } else if (!isHttpUrl(filePath) && !existsSync(filePath)) {
      issues.push('source_not_found');
    }

    const mimeType = filePath ? mimeTypeForAudioPath(filePath) : null;
    const directMimeType = device && mimeType ? this.selectDeviceMimeType(device, mimeType) : null;
    const delivery = mimeType && directMimeType ? 'direct' : mimeType ? 'transcode' : 'unsupported';
    const warnings: ConnectPreflightResult['warnings'] = [];
    if (delivery === 'transcode') {
      warnings.push('transcode_required');
    }
    if (!capabilities.canPause || !capabilities.canSeek || !capabilities.canSetVolume) {
      warnings.push('limited_controls');
    }

    return {
      deviceId: request.deviceId,
      deviceName: publicDevice?.name ?? request.deviceId,
      protocol: 'dlna',
      ready: issues.length === 0,
      checkedAt,
      source: filePath ? {
        title: track?.title?.trim() || filePath.split(/[\\/]/u).pop() || 'ECHO',
        mimeType,
        remote: isHttpUrl(filePath),
      } : null,
      delivery,
      capabilities,
      issues,
  ...
  private resolvePlaybackFilePath(
    ...
    return request.filePath ?? sourceUrl ?? remotePath ?? track?.path ?? currentFilePath;
  }
