...
describe('EchoLinkService', () => {
  ...
  let executePlaybackControl: ReturnType<typeof vi.fn<(request: MainWindowPlaybackControlRequest) => Promise<unknown>>>;
  ...
  beforeEach(async () => {
    ...
    executePlaybackControl = vi.fn(async () => undefined);
  ...
  it('serves the web control page through the Echo Link token URL', async () => {
    ...
    expect(body).toContain("const trackParams = new URLSearchParams({ page: '1', pageSize: '500' })");
    expect(body).toContain("tracks.forEach((track, index) =>");
    expect(body).not.toContain('tracks.slice(0, 16)');
    expect(body).toContain("viewMode: 'tracks'");
    expect(body).toContain('id="tracksViewBtn" type="button" data-active="true"');
    expect(body).toContain("const activeItems = () => state.viewMode === 'tracks' ? state.tracks : state.albums");
    expect(body).toContain("api('/echo-link/v1/library/tracks?' + trackParams.toString())");
    expect(body).toContain("await command({ command: 'queueReplace', trackIds, startTrackId: track.id, output: 'pc' })");
    expect(body).toContain('const compactCollection = count <= 24');
    expect(body).toContain('0 0 0 1px rgba(255, 255, 255, 0.48)');
    expect(body).not.toContain('album-glass-tint');
    expect(body).toContain('.album-card[data-spotlight="true"][data-selected="true"]');
    expect(body).toContain('card.dataset.sizeTier = readStyleNumber');
    expect(body).toContain('border-radius: 20px');
    expect(body).toContain('backdrop-filter: blur(22px) saturate(1.62)');
    expect(body).toContain('rgba(var(--glass-rgb, 128, 154, 236), 0.24)');
    expect(body).toContain('.album-card[data-featured="true"]');
    expect(body).toContain('rgba(var(--glass-rgb, 128, 154, 236), 0.3)');
    expect(body).toContain("const syncArtworkGlass = (card, image, index) =>");
    expect(body).toContain('const albumStyle = (index, featured = false) =>');
    expect(body).toContain("card.dataset.featured = featured ? 'true' : 'false'");
    expect(body).toContain('const size = wide && featured');
    expect(body).toContain('? (heroTier ? 354 : 174)');
    expect(body).toContain('const isHeroTierAt = (index, compactCollection) =>');
    expect(body).toContain('const anchoredHero = heroTier && priorHeroCount < 3');
    expect(body).toContain('Math.exp(-Math.pow(distance / focusRadius, 2))');
    expect(body).toContain(': 0.45 + 0.55 * focus');
    expect(body).toContain('0.35 + 0.65 * Math.exp(-Math.pow(distance / (2.5 * focusRadius), 2))');
    expect(body).toContain('-330 + 580 * focus');
    expect(body).toContain('layout.cellW = wide ? 180 : 138');
    expect(body).toContain('layout.cellH = wide ? 232 : 184');
    expect(body).toContain('layout.cellW * 1.45');
    expect(body).toContain('vertical * (far ? 46 : 38)');
    expect(body).toContain('-horizontal * (far ? 46 : 40)');
    expect(body).toContain('compactCollection ? 1.28 : 1.18');
    expect(body).toContain('const placeHeroAnchors = (plans) =>');
    expect(body).toContain('placeHeroAnchors(placedCards)');
    expect(body).toContain('focusPinned: true');
    expect(body).toContain('const schedulePinnedFocus = () =>');
    expect(body).toContain('const baseBox = planVisualBox(meta)');
    expect(body).toContain("displayDepthZ: item.field.depthZ");
    expect(body).toContain('</div><div class="album-mini-controls">');
    expect(body).not.toContain('background: rgba(6, 12, 22, 0.76)');
    expect(body).not.toContain('Math.max(13, Math.ceil(Math.sqrt(count');
    const inlineScript = body.match(/<script>([\s\S]*?)<\/script>/u)?.[1];
    expect(inlineScript).toBeTruthy();
    expect(() => new Function(inlineScript ?? '')).not.toThrow();
    expect(body).toContain('album-selection');
    expect(body).toContain('selected-album-card');
    expect(body).toContain('detail-back');
    expect(body).toContain("'audio-lines'");
    expect(body).toContain('selectAlbum(album).catch');
    ...
    expect(body).toContain("没有找到' + (trackMode ? '歌曲' : '专辑')");
  ...
  it('routes playback controls through the awaited provider-aware control path', async () => {
    ...
    expect(executePlaybackControl).toHaveBeenCalledWith({ type: 'seek', positionSeconds: 42 });
    expect(executePlaybackControl).toHaveBeenCalledWith({ type: 'setVolume', volume: 0.5 });
    expect(executePlaybackControl).toHaveBeenCalledWith({ type: 'playPause' });
    expect(executePlaybackControl).toHaveBeenCalledWith({ type: 'stop' });
    expect(audioSession.seek).not.toHaveBeenCalled();
    expect(audioSession.setOutput).not.toHaveBeenCalled();
  });
  ...
  it('handles queueReplace and advances within the ECHO Link queue', async () => {
    ...
    expect(executePlaybackControl).not.toHaveBeenCalledWith({ type: 'next' });
  });
  ...
  it('awaits provider-aware queue navigation when no ECHO Link queue is active', async () => {
    ...
    expect(executePlaybackControl).toHaveBeenCalledWith({ type: 'next' });
    expect(executePlaybackControl).toHaveBeenCalledWith({ type: 'previous' });
  });
  ...
  it('redacts media tokens, query secrets, and internal error details from diagnostics', async () => {
    const stream = service.createStream('track-1', baseUrl());
    const token = stream.streamUrl.split('/').at(-1)!;
    const rangeResponse = await fetch(`${stream.streamUrl}?access_token=phone-secret`, {
      headers: { Range: 'bytes=99-100' },
    });

    expect(rangeResponse.status).toBe(416);
    let diagnostic = service.getServerStatus().diagnostics.recentHttpErrors[0];
    expect(diagnostic).toMatchObject({
      path: '/echo-link/media/[redacted]',
      statusCode: 416,
      message: 'invalid_range',
    });
    expect(JSON.stringify(diagnostic)).not.toContain(token);
    expect(JSON.stringify(diagnostic)).not.toContain('phone-secret');

    rmSync(audioPath, { force: true });
    const missingFileResponse = await fetch(`${stream.streamUrl}?access_token=another-secret`);
    expect(missingFileResponse.status).toBe(500);
    await expect(missingFileResponse.json()).resolves.toMatchObject({ error: 'internal_error' });
    diagnostic = service.getServerStatus().diagnostics.recentHttpErrors[0];
    expect(diagnostic.message).toBe('internal_error');
    expect(JSON.stringify(diagnostic)).not.toContain(tempRoot);
    expect(JSON.stringify(diagnostic)).not.toContain('another-secret');
  });
