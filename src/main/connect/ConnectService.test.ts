...
describe('ConnectService HQPlayer output device', () => {
  ...
  it('preflights HQPlayer without changing the saved enabled setting', async () => {
    const { ConnectService } = await import('./ConnectService');
    const hqPlayer = createHqPlayerService({ enabled: false });
    const service = new ConnectService(hqPlayer);
    ...
    expect(result).toMatchObject({
      ...
      delivery: 'hqplayer',
      issues: [],
      warnings: ['hqplayer_will_enable'],
    });
    expect(hqPlayer.testConnection).toHaveBeenCalledWith(expect.objectContaining({ enabled: true }));
    expect(hqPlayer.setSettings).not.toHaveBeenCalled();
  });

  it('preflights a discovered DLNA source and reports direct delivery', async () => {
    const tempRoot = mkdtempSync(join(tmpdir(), 'echo-connect-preflight-'));
    try {
      ...
      const merge = service as unknown as { mergeDiscoveredDlnaDevices: (devices: DlnaDevice[]) => void };
      merge.mergeDiscoveredDlnaDevices([dlnaDevice()]);
      vi.spyOn(service, 'refreshDevices').mockResolvedValue(service.listDevices());
      ...
      expect(result).toMatchObject({
        ready: true,
        delivery: 'direct',
        source: { mimeType: 'audio/flac', remote: false },
        issues: [],
      });
    } finally {
