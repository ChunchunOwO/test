...
describe('AirPlayMdnsAdvertiser', () => {
  ...
  it('can advertise an opt-in experimental AirPlay 2 discovery service', () => {
    ...
    expect(payload).toContain('features=0x405f8a00,0x16040');
    expect(payload).toContain('ft=0x405f8a00,0x16040');
    expect(payload).toContain('et=0');
    expect(payload).not.toContain('et=0,1,3');
    expect(payload).toContain('cn=0,1,2');
    expect(payload).toContain('vn=65537');
    expect(payload).toContain('vs=354.0');
    expect(payload).toContain('srcvers=354.0');
  ...
  it('advertises buffered/PTP only when the PTP timing backend is active', () => {
    const advertiser = new AirPlayMdnsAdvertiser() as unknown as PacketFactory;
    const packet = advertiser.createPacket({
      name: 'ECHO (AirPlay)',
      address: '192.168.31.214',
      ...
      model: 'ECHO-AirPlay-Spike',
      airPlay2Experimental: true,
      airPlay2TimingMode: 'ptp',
    }, 120);
    const payload = packet.toString('utf8');

    expect(payload).toContain('features=0x405f8a00,0x1c340');
    expect(payload).toContain('ft=0x405f8a00,0x1c340');
    expect(payload).toContain('vs=366.0');
    expect(payload).toContain('srcvers=366.0');
    expect(Number.isSafeInteger(airPlay2PtpFeatureMask)).toBe(true);
  });

  it('advertises realtime NTP without claiming PTP, buffered audio, or FairPlay', () => {
    expect([9, 11, 15, 16, 17, 18, 19, 20, 22, 30, 38, 45, 46, 48].every(hasAirPlay2FeatureBit)).toBe(true);

    expect(hasAirPlay2FeatureBit(14)).toBe(false);
    expect(hasAirPlay2FeatureBit(21)).toBe(false);
    expect(hasAirPlay2FeatureBit(40)).toBe(false);
    expect(hasAirPlay2FeatureBit(41)).toBe(false);
