...
const localProUnlockedMock = vi.fn((_feature?: string) => false);
const requireLocalProMock = vi.fn((feature = 'echo-pro') => {
  if (!localProUnlockedMock(feature)) {
    throw new Error('echo_authorization_required');
  }
});
const applyDefaultAudioRuntimeBaselineMock = vi.fn(async () => undefined);
const reconcileEchoProAudioEntitlementMock = vi.fn(async () => ({
  unlocked: localProUnlockedMock(),
  source: localProUnlockedMock() ? 'steam-dlc' : 'none',
  checkedAt: '2026-08-15T00:00:00.000Z',
}));
const lockedFeatureSettingsOptions = {
  finalThemeUnlocked: false,
...
const hideMainWindowToTrayMock = vi.fn();
...
const appPathMock = vi.fn((name: string) =>
  name === 'downloads' ? tmpdir() : join(tmpdir(), 'echo-register-ipc-test-userdata'),
);
const nativeThemeMock = { themeSource: 'system' as 'system' | 'light' | 'dark' };
const syncSteamRichPresenceIntegrationFromSettingsMock = vi.fn();

vi.mock('electron', () => ({
  default: {
    app: {
      getAppPath: () => 'D:\\ECHO\\ECHO',
      getPath: appPathMock,
      isPackaged: false,
    },
  },
...
vi.mock('../app/tray', () => ({
  ...
  hideMainWindowToTray: hideMainWindowToTrayMock,
...
vi.mock('../plugins/LocalProEntitlements', () => ({
  isLocalProUnlocked: localProUnlockedMock,
  requireLocalPro: requireLocalProMock,
}));

vi.mock('../audio/AudioEntitlementRuntime', () => ({
  applyDefaultAudioRuntimeBaseline: applyDefaultAudioRuntimeBaselineMock,
  reconcileEchoProAudioEntitlement: reconcileEchoProAudioEntitlementMock,
}));
...
vi.mock('../integrations/discord/DiscordPresenceStatusSync', () => ({
  syncDiscordPresenceIntegrationFromSettings: vi.fn(),
}));

vi.mock('../integrations/steam/SteamRichPresenceStatusSync', () => ({
  syncSteamRichPresenceIntegrationFromSettings: syncSteamRichPresenceIntegrationFromSettingsMock,
}));
...
vi.mock('../integrations/lastfm/LastFmStatusSync', () => ({
  syncLastFmIntegrationFromSettings: vi.fn(),
}));

vi.mock('../integrations/mqtt/MqttIntegrationService', () => ({
  syncMqttIntegrationFromSettings: vi.fn(),
}));
...
describe('app IPC cover cache directory', () => {
  beforeEach(async () => {
    ...
    localProUnlockedMock.mockReset();
    localProUnlockedMock.mockReturnValue(false);
    requireLocalProMock.mockClear();
    applyDefaultAudioRuntimeBaselineMock.mockClear();
    reconcileEchoProAudioEntitlementMock.mockClear();
    ...
    hideMainWindowToTrayMock.mockClear();
    ...
    syncSteamRichPresenceIntegrationFromSettingsMock.mockClear();
    const module = await import('./registerIpc');
    module.registerIpc();
  });

  it('ensures the tray exists before hiding the main window', () => {
    handlers[IpcChannels.AppWindowHideToTray]!();

    expect(ensureTrayMock).toHaveBeenCalledTimes(1);
    expect(hideMainWindowToTrayMock).toHaveBeenCalledTimes(1);
    expect(ensureTrayMock.mock.invocationCallOrder[0]).toBeLessThan(
      hideMainWindowToTrayMock.mock.invocationCallOrder[0]!,
    );
  });
  ...
  it('resets app settings and restores the default cover cache directory', async () => {
    ...
    expect(applyDefaultAudioRuntimeBaselineMock).toHaveBeenCalledTimes(1);
  });
  ...
  it('blocks enabling optional audio processing through settings without Pro', async () => {
    await expect(handlers[IpcChannels.AppSetSettings]!(null, { replayGainEnabled: true }))
      .rejects.toThrow();
    expect(setAppSettingsMock).not.toHaveBeenCalled();
  });

  it('resynchronizes Steam Rich Presence when optional audio fields change', async () => {
    setAppSettingsMock.mockReturnValue({
      coverCacheDir: null,
      hideToTrayOnClose: false,
    ...
    expect(syncSteamRichPresenceIntegrationFromSettingsMock).toHaveBeenCalledWith(expect.objectContaining({
      steamRichPresenceShowBpm: true,
      steamRichPresenceShowQuality: true,
      steamRichPresenceShowFormat: true,
      steamRichPresenceShowBitPerfect: true,
  ...
  it('allows Pro theme settings when the local Steam Pro entitlement is valid', async () => {
    localProUnlockedMock.mockReturnValue(true);
    ...
    expect(setAppSettingsMock).toHaveBeenCalledWith(
      ...
      { finalThemeUnlocked: true, proFeaturesUnlocked: true, downloadsFeatureUnlocked: false },
    );
  });

  it('does not treat the legacy ECHO Pro unlock plugin license as a Steam entitlement', async () => {
  ...
  it('keeps removed download features locked even when the legacy plugin reports unlocked', async () => {
  ...
  it('does not register plugin activation handlers in the Steam main process', () => {
    expect(handlers[IpcChannels.AppEchoProPluginActivate]).toBeUndefined();
    expect(handlers[IpcChannels.AppEchoProPluginReleaseCurrentDevice]).toBeUndefined();
  });
  ...
  it('exports app settings to a selected JSON backup', async () => {
    ...
    expect(payload.format).toBe('echo-settings-backup');
  ...
  it('backs up current settings before importing a selected JSON backup', async () => {
    ...
    writeFileSync(importPath, JSON.stringify({ format: 'echo-settings-backup', version: 1, settings: { locale: 'en-US', coverCacheDir: null } }), 'utf8');
  ...
  it('runs a manual data backup through the configured service', async () => {
    runDataBackupNowMock.mockResolvedValue({
      filePath: 'D:\\EchoBackups\\ECHO-backup.zip',
    ...
    expect(result).toMatchObject({ filePath: 'D:\\EchoBackups\\ECHO-backup.zip' });
  });
