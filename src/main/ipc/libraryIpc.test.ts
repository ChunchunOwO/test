...
const makeTempRoot = (): string => {
  const root = join(tmpdir(), `echo-library-ipc-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
describe('library IPC', () => {
  ...
  it('preserves extended editable tags through the IPC boundary', async () => {
    const service = installLibraryService();

    await handlers[IpcChannels.LibraryUpdateTrackTags]!(null, {
      trackId: 'track-1',
      tags: {
        title: 'Song',
        artist: 'Artist',
    ...
    expect(service.updateTrackTags).toHaveBeenCalledWith(expect.objectContaining({
      trackId: 'track-1',
  ...
  it('measures one track BPM through the preview-only IPC path', async () => {
    const service = installLibraryService();
    const result = {
      trackId: 'track-1',
      bpm: 127.6,
    ...
    service.measureTrackBpm.mockResolvedValue(result);
    (appSettingsMock.current as Record<string, unknown>).audioAnalysisEnabled = true;

    await expect(handlers[IpcChannels.LibraryMeasureTrackBpm]!(null, 'track-1')).resolves.toEqual(result);
    expect(service.measureTrackBpm).toHaveBeenCalledWith('track-1');
  });
  ...
  it('returns and exports a sanitized library health report', async () => {
    ...
    expect(markdown).toContain('ECHO 曲库体检报告');
    expect(markdown).not.toContain(root);
    expect(markdown).not.toContain('token=secret');
    expect(markdown).not.toContain('do-not-export');
  });

  it('stores pathless dropped audio in the managed import directory', async () => {
    const root = makeTempRoot();
    const managedImportRoot = join(root, 'Imported Media');
    const { app } = await import('electron');
    ...
    expect(result).toMatchObject({
      ...
      failedCount: 0,
      storedFileCount: 1,
      outputDirectory: managedImportRoot,
    });
    expect(existsSync(join(managedImportRoot, 'song.flac'))).toBe(true);
    expect(getLibraryServiceMock().importAudioFile).toHaveBeenCalledWith(
      join(managedImportRoot, 'song.flac'),
      { folderPath: managedImportRoot, deferGroupingRefresh: true },
    );
  });

  it('imports dropped local audio from its original path without copying it', async () => {
    const service = installLibraryService();
    const root = makeTempRoot();
    const audioPath = join(root, 'song.flac');
    writeFileSync(audioPath, 'audio');

    const result = await handlers[IpcChannels.LibraryImportDroppedFiles]!(null, [
      { name: 'song.flac', type: 'audio/flac', path: audioPath, bytes: null },
    ]);
  ...
  it('adds dropped folders and starts the existing library folder scan', async () => {
    const service = installLibraryService();
    const root = makeTempRoot();
    const folderPath = join(root, 'Album');
    const scanStatus = {
      id: 'scan-dropped-folder',
      folderId: 'folder-dropped',
      status: 'running',
      phase: 'discovering',
      totalFiles: 0,
      processedFiles: 0,
    ...
    service.addFolder.mockReturnValue({ id: 'folder-dropped', path: folderPath });
    service.scanFolder.mockReturnValue(scanStatus);

    const result = await handlers[IpcChannels.LibraryImportDroppedFiles]!(null, [
      { name: 'Album', type: '', path: folderPath, bytes: null },
    ]);

    expect(service.addFolder).toHaveBeenCalledWith(folderPath);
    expect(service.scanFolder).toHaveBeenCalledWith('folder-dropped');
    expect(result).toMatchObject({
      addedFolderCount: 1,
      importedCount: 0,
      ignoredCount: 0,
      failedCount: 0,
      folderScanStatuses: [scanStatus],
    });
  });
  ...
  it('imports dropped osu archives through the shared importer without treating them as unsupported', async () => {
    ...
    expect(getLibraryServiceMock().importAudioFile).toHaveBeenCalledWith(
      `${managedImportRoot}\\Artist - Song.mp3`,
      expect.objectContaining({
        folderPath: managedImportRoot,
  ...
  it('refreshes album grouping through the playback-safe IPC path', async () => {
    ...
    const result = await handlers[IpcChannels.LibraryRefreshAlbumGroupingPlaybackSafe]!(null, {
      albumMergeStrategy: 'sameTitleAndCover',
      artistMergeStrategy: 'conservative',
      ignored: true,
    });

    expect(service.refreshAlbumGroupingPlaybackSafe).toHaveBeenCalledWith({
      albumMergeStrategy: 'sameTitleAndCover',
      artistMergeStrategy: 'conservative',
    });
  ...
  it('accepts file modified and artist-album sorting for songs and albums', async () => {
    ...
    expect(service.getAlbumsResponsive).toHaveBeenCalledWith({ page: 1, pageSize: 50, sort: 'fileModifiedAsc', excludeOsuAlbums: true });
  });

  it('normalizes multi-sort values for song queries', async () => {
    const service = installLibraryService();

    await handlers[IpcChannels.LibraryGetTracks]!(null, {
      page: 1,
      sort: 'artist',
      sorts: ['artist', 'yearDesc', 'artist', 'invalid'],
    });
  ...
  it('registers genre browse IPC handlers with normalized queries', async () => {
    const service = installLibraryService();
    service.getGenre.mockReturnValue({
      genreKey: 'rock',
      name: 'Rock',
      unclassified: false,
      trackCount: 2,
    ...
    service.getGenreTracks.mockReturnValue({ items: [], page: 1, pageSize: 50, total: 0, hasMore: false });
    service.getGenreAlbums.mockReturnValue({ items: [], page: 1, pageSize: 12, total: 0, hasMore: false });

    await handlers[IpcChannels.LibraryGetGenres]!(null, { page: 1, pageSize: 96, sort: 'titleAsc', extra: true });
    await handlers[IpcChannels.LibraryGetGenre]!(null, 'Rock', { sourceProvider: 'local' });
    await handlers[IpcChannels.LibraryGetGenreTracks]!(null, '__unclassified__', { page: 2, pageSize: 50, sort: 'durationDesc' });
    await handlers[IpcChannels.LibraryGetGenreAlbums]!(null, 'j-pop/anime', { page: 1, pageSize: 12, sourceProvider: 'remote' });

    expect(service.getGenres).toHaveBeenCalledWith({ page: 1, pageSize: 96, sort: 'titleAsc' });
    expect(service.getGenre).toHaveBeenCalledWith('Rock', { sourceProvider: 'local' });
    expect(service.getGenreTracks).toHaveBeenCalledWith('__unclassified__', { page: 2, pageSize: 50, sort: 'durationDesc' });
    expect(service.getGenreAlbums).toHaveBeenCalledWith('j-pop/anime', { page: 1, pageSize: 12, sourceProvider: 'remote' });
  });
  ...
  it('keeps an unowned external cache directory intact during factory reset', async () => {
    ...
    try {
      ...
      expect(result.failedPaths).toEqual([
        {
          path: externalCache,
          error: 'Skipped external cover cache directory because it is not marked as owned by ECHO.',
        },
      ]);
