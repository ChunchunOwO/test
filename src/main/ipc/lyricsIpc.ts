...
export const registerLyricsIpc = (): void => {
  ...
  ipcMain.handle(IpcChannels.LyricsApplyCustomLrcForSnapshot, (
    _event,
    request: unknown,
    lrcText: unknown,
    fileName?: unknown,
    ...
    return runWithLyricsChanged(normalizedRequest.trackId, () =>
      getLyricsService().applyCustomLrcForSnapshot(normalizedRequest, normalizedLrcText, normalizedFileName),
    );
  });
