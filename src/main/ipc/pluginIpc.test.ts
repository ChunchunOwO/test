...
const serviceMock = {
  ...
  exportPackage: vi.fn(async () => ({ type: 'echo-plugin-package', version: 1, exportedAt: '2026-06-21T00:00:00.000Z', manifest: { id: 'echo.playback-panel', name: 'Playback Panel', version: '1.0.0', apiVersion: 2 }, files: [] })),
...
describe('plugin IPC', () => {
  ...
  it('exports plugin packages without checking a Pro entitlement', async () => {
    requireLocalProMock.mockImplementation(() => {
      throw new Error('echo_pro_required');
    });

    await expect(handlers[IpcChannels.PluginsExportPackage]!(null, 'echo.playback-panel')).resolves.toMatchObject({ type: 'echo-plugin-package' });
    expect(serviceMock.exportPackage).toHaveBeenCalledWith('echo.playback-panel');
    expect(requireLocalProMock).not.toHaveBeenCalled();
  });

  it('installs market plugins without checking a Pro entitlement', async () => {
    requireLocalProMock.mockImplementation(() => {
      throw new Error('echo_pro_required');
    });
