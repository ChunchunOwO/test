...
/** All channel strings defined in the IpcChannels const. */
...
const PUSH_ONLY_CHANNELS = new Set<string>([
  ...
  IpcChannels.AppSharedSettingsChanged,
  IpcChannels.AppWindowFullscreenChanged,
  IpcChannels.AppWindowHiddenChanged,
  IpcChannels.AppWindowMaximizedChanged,
  IpcChannels.AppWindowMinimizedChanged,
...
const INTENTIONALLY_UNREGISTERED_CHANNELS = new Set<string>([
  // Steam distribution keeps the private Pro plugin activation surface disconnected.
  IpcChannels.AppEchoProPluginActivate,
...
describe('IPC Registry — channel registration completeness', () => {
  ...
  describe('1. All defined channels referenced in handler files', () => {
    it('every channel from IpcChannels has a corresponding reference in registration source files', () => {
      ...
      const unaccounted = missing.filter((ch) =>
        !PUSH_ONLY_CHANNELS.has(ch) && !INTENTIONALLY_UNREGISTERED_CHANNELS.has(ch));
  ...
  describe('2. All handler channels registered via ipcMain.handle() or ipcMain.on()', () => {
    it('every non-push channel has a handler registration in IPC source files', () => {
      ...
      for (const channel of ALL_CHANNELS) {
        if (allPushChannels.has(channel) || INTENTIONALLY_UNREGISTERED_CHANNELS.has(channel)) {
          continue;
        }
  ...
  describe('3. Handler count matches channel count', () => {
    it('handler registration count is within expected range of defined channels', () => {
      ...
      const expectedHandlerCount =
        ALL_CHANNELS.length - allPushChannels.size - INTENTIONALLY_UNREGISTERED_CHANNELS.size;
