...
const normalizeAlbumGroupingRefreshRequest = (value: unknown): LibraryAlbumGroupingRefreshRequest => {
  const input = value && typeof value === 'object' && !Array.isArray(value) ? (value as Record<string, unknown>) : {};
  return {
    albumMergeStrategy:
      input.albumMergeStrategy === 'standard' || input.albumMergeStrategy === 'sameTitleAndCover'
        ? input.albumMergeStrategy
        : undefined,
    artistMergeStrategy:
      input.artistMergeStrategy === 'conservative' || input.artistMergeStrategy === 'standard'
        ? input.artistMergeStrategy
        : undefined,
  };
};
...
const fileFormatFilterValues = new Set<LibraryFileFormatFilter>([
  'all',
  'flac',
  'mp3',
  'wav',
...
const scheduleLibraryRecoveryRelaunch = () => {
  ...
  return {
    ...
    message: 'ECHO 将退出并重启到恢复模式，重启时会在服务占用数据库前先运行曲库保护检查和自动修复。',
  };
};
...
const normalizeQuery = (value: unknown): LibraryPageQuery => {
  ...
  if (Array.isArray(input.sorts)) {
    query.sorts = Array.from(new Set(
      input.sorts.filter((sort): sort is LibrarySort => typeof sort === 'string' && sortValues.has(sort as LibrarySort)),
    ));
  }
  ...
  if (typeof input.fileFormatFilter === 'string' && fileFormatFilterValues.has(input.fileFormatFilter as LibraryFileFormatFilter)) {
    query.fileFormatFilter = input.fileFormatFilter as LibraryFileFormatFilter;
  }
...
const normalizeTagUpdateRequest = (value: unknown): LibraryTrackTagUpdateRequest => {
  ...
  const hasTag = (key: keyof EditableTrackTags): boolean => Object.prototype.hasOwnProperty.call(tagsRecord, key);

  if (hasTag('composer')) {
    tags.composer = typeof tagsRecord.composer === 'string' && tagsRecord.composer.trim() ? tagsRecord.composer : null;
  }
  if (hasTag('totalTracks')) {
    tags.totalTracks = optionalNumber(tagsRecord.totalTracks);
  }
  if (hasTag('totalDiscs')) {
    tags.totalDiscs = optionalNumber(tagsRecord.totalDiscs);
  }
  if (hasTag('comment')) {
    tags.comment = typeof tagsRecord.comment === 'string' && tagsRecord.comment.trim() ? tagsRecord.comment : null;
  }
  if (hasTag('bpm')) {
    tags.bpm = optionalNumber(tagsRecord.bpm);
  }
...
const normalizeAlbumTagUpdateRequest = (value: unknown): LibraryAlbumTagUpdateRequest => {
  ...
  return {
    ...
    tags: {
      ...
      albumArtist: readText('albumArtist'),
      year: optionalNumber(tagsRecord.year),
      genre: Object.prototype.hasOwnProperty.call(tagsRecord, 'genre')
        ? typeof tagsRecord.genre === 'string' && tagsRecord.genre.trim().length > 0 ? tagsRecord.genre.trim() : null
        : undefined,
    },
    coverPath: typeof input.coverPath === 'string' && input.coverPath.trim().length > 0 ? input.coverPath : null,
    coverUrl: typeof input.coverUrl === 'string' && input.coverUrl.trim().length > 0 ? input.coverUrl : null,
...
const getImportOutputDirectory = (): string => {
  return join(app.getPath('userData'), 'Imported Media');
};
...
const importDroppedFiles = async (value: unknown): Promise<{
  ...
  return runMainBackgroundTask('library-import-files', async () => {
    ...
    const folderScanStatuses: LibraryScanStatus[] = [];
    let addedFolderCount = 0;
    let ignoredCount = 0;
    let failedCount = 0;
    let storedFileCount = 0;

    const ensureOutputDirectory = (): string => {
    ...
    for (const file of files) {
      ...
      if (inputPath) {
        try {
          if (statSync(inputPath).isDirectory()) {
            const folder = service.addFolder(inputPath);
            folderScanStatuses.push(service.scanFolder(folder.id));
            addedFolderCount += 1;
            continue;
          }
        } catch {
      ...
      try {
        if (inputPath) {
          const track = await service.importAudioFile(inputPath, { deferGroupingRefresh: true });
          importedTrackIds.push(track.id);
        } else {
          ensureOutputDirectory();
...
const importPlaylistFile = async (): Promise<ImportPlaylistFileResult | null> => {
  ...
  throw new Error('Online playlist import is not available in the Steam distribution.');
};
...
export const registerLibraryIpc = (): void => {
  ...
  ipcMain.handle(IpcChannels.LibraryGetAlbums, (_event, query: unknown) =>
    getLibraryService().getAlbumsResponsive(normalizeQuery(query)),
  );
  ...
  ipcMain.handle(IpcChannels.LibraryGetGenres, (_event, query: unknown) =>
    getLibraryService().getGenres(normalizeQuery(query)),
  );
  ipcMain.handle(IpcChannels.LibraryGetGenre, (_event, genreKey: unknown, query: unknown) =>
    getLibraryService().getGenre(requireText(genreKey, 'genreKey'), normalizeQuery(query)),
  );
  ipcMain.handle(IpcChannels.LibraryGetGenreTracks, (_event, genreKey: unknown, query: unknown) =>
    getLibraryService().getGenreTracks(requireText(genreKey, 'genreKey'), normalizeQuery(query)),
  );
  ipcMain.handle(IpcChannels.LibraryGetGenreAlbums, (_event, genreKey: unknown, query: unknown) =>
    getLibraryService().getGenreAlbums(requireText(genreKey, 'genreKey'), normalizeQuery(query)),
  );
  ...
  ipcMain.handle(IpcChannels.LibraryRefreshAlbumGroupingPlaybackSafe, (_event, request: unknown) =>
    getLibraryService().refreshAlbumGroupingPlaybackSafe(normalizeAlbumGroupingRefreshRequest(request)),
  );
  ...
  ipcMain.handle(IpcChannels.LibraryMeasureTrackBpm, (_event, trackId: unknown) => {
    const settings = getAppSettings();
    if (!settings.audioAnalysisEnabled) {
      throw new Error('BPM analysis is disabled in Settings');
    }
    return getLibraryService().measureTrackBpm(requireText(trackId, 'trackId'));
  });
