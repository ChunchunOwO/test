...
const mocks = vi.hoisted(() => ({
  ...
  applyCustomLrcForSnapshotMock: vi.fn(),
  audioStatus: {
    state: 'idle',
    outputMode: 'shared',
...
vi.mock('../lyrics/LyricsService', () => ({
  getLyricsService: () => ({
    ...
    applyCustomLrcForSnapshot: mocks.applyCustomLrcForSnapshotMock,
  }),
...
describe('lyrics IPC', () => {
  beforeEach(() => {
    ...
    mocks.applyCustomLrcForSnapshotMock.mockReset();
    mocks.audioStatus = {
      state: 'idle',
      outputMode: 'shared',
      nativeUnderrunCallbacks: 0,
  ...
  it('applies custom lyrics through the snapshot contract', async () => {
    const request = {
    ...
    mocks.applyCustomLrcForSnapshotMock.mockResolvedValue(trackLyrics(request.trackId));
    registerLyricsIpc();
    const handler = mocks.handlers.get(IpcChannels.LyricsApplyCustomLrcForSnapshot);

    await expect(handler?.(null, request, '[00:01.00]Line', 'custom.lrc')).resolves.toMatchObject({
      trackId: request.trackId,
    });
    expect(mocks.applyCustomLrcForSnapshotMock).toHaveBeenCalledWith(request, '[00:01.00]Line', 'custom.lrc');
  });
});
