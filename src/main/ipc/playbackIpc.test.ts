...
describe('playback media prepare IPC', () => {
  ...
  it('does not let a stale streaming resolve interrupt a newer local playback request', async () => {
    ...
    const { readRemoteSourcePlaybackActivity } = await import('../library/remote/RemoteSourcePlaybackActivity');
    expect(readRemoteSourcePlaybackActivity()).toEqual({ active: true, lowLoadEnhanced: false });
    expect(setPlaybackActive).not.toHaveBeenCalled();
  });
