...
const createSmoothstepExpression = (progress: string): string =>
...
const createAutomixCurrentGainFilter = (gainDb: number, transitionStartSeconds: number, transitionSeconds: number): string | null => {
  ...
  return createVolumeExpressionFilter(`if(lt(t,${start}),1,if(lt(t,${end}),1+(${gain}-1)*${createSmoothstepExpression(progress)},${gain}))`);
};
...
const createAutomixNextGainFilter = (gainDb: number, transitionSeconds: number): string | null => {
  ...
  const progress = `(t-${overlap})/${release}`;
  return createVolumeExpressionFilter(`if(lt(t,${overlap}),${gain},if(lt(t,${releaseEnd}),${gain}+(1-${gain})*${createSmoothstepExpression(progress)},1))`);
};
