...
const visualSpectrumComputeIntervalMs = 100;
...
export const createNativeAudioLevelTelemetry = (
  ...
  const visualSpectrumReady = visualSpectrum.length > 0;
  const normalizedVisualSpectrum = visualSpectrumReady
    ? normalizeVisualSpectrum(visualSpectrum)
    : emptyVisualSpectrum();
  const visualEnergy = visualSpectrumReady
    ? roundUnit(normalizedVisualSpectrum.reduce((sum, value) => sum + value, 0) / visualSpectrumBucketCount)
    : 0;
  const visualTransient = visualSpectrumReady
    ? roundUnit(Math.max(0, normalizedPeakDb - normalizedRmsDb) / 30)
    : 0;
  ...
  return {
    ...
    visualSpectrum: normalizedVisualSpectrum,
    visualSpectrumVersion,
    visualEnergy,
    visualTransient,
    visualTelemetryState: visualSpectrumReady ? 'pcm' : 'fallback',
