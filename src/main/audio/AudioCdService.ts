...
type AudioCdCapabilities = {
  ...
  nativeCddaAvailable: boolean;
  inputAvailable: boolean;
  inputBackend: AudioCdStatus['inputBackend'];
};
...
type ResolvedAudioCdTrack = {
  drive: AudioCdDrive;
  track: AudioCdTrack;
  backendImpl: 'windows-native-cdda-pcm' | 'ffmpeg-libcdio-pcm';
};

type WindowsCddaTocTrack = {
  index: number;
  startLba: number;
  endLba: number;
  audio: boolean;
...
export const parseWindowsCddaToc = (
  drive: Pick<AudioCdDrive, 'id' | 'device'>,
  jsonText: string,
): AudioCdTrack[] => {
  const parsed = JSON.parse(jsonText) as { tracks?: unknown };
  if (!Array.isArray(parsed.tracks)) {
    throw new Error('audio_cd_invalid_native_toc');
  }
  ...
  for (const value of parsed.tracks) {
    ...
    const record = value as Partial<WindowsCddaTocTrack>;
    const index = Number(record.index);
    const startLba = Number(record.startLba);
    const endLba = Number(record.endLba);
    if (
      record.audio !== true
      || !Number.isInteger(index)
      || index < 1
      || index > 99
      || !Number.isInteger(startLba)
      || !Number.isInteger(endLba)
      || startLba < 0
      || endLba <= startLba
    ) {
      continue;
    ...
    tracks.push(createTrack(
      drive,
      index,
      startLba / cddaFramesPerSecond,
      endLba / cddaFramesPerSecond,
    ));
  }
  return tracks;
};
...
export class AudioCdService {
  ...
  private readonly nativeHostPath?: string | null;
  ...
  constructor(dependencies: AudioCdServiceDependencies = {}) {
    ...
    this.nativeHostPath = dependencies.nativeHostPath;
  ...
  async getStatus(driveId?: string | null): Promise<AudioCdStatus> {
    ...
    if (!capabilities.inputAvailable) {
      return {
        ...capabilities,
        drives,
        ...
        error: this.platform === 'win32'
          ? 'native_cdda_unavailable'
          : capabilities.ffmpegAvailable ? 'libcdio_unavailable' : 'ffmpeg_unavailable',
      };
    }
  ...
  async createTrackPcmStream(request: AudioCdPlayTrackRequest): Promise<ResolvedAudioCdTrack & { stream: Readable }> {
    ...
    if (!status.inputBackend) {
      throw new Error(status.error ?? 'audio_cd_input_unavailable');
    }
    ...
    return {
      drive,
      track,
      backendImpl: status.inputBackend === 'windows-native-cdda'
        ? 'windows-native-cdda-pcm'
        : 'ffmpeg-libcdio-pcm',
      stream: this.spawnTrackDecode(drive, track, status.inputBackend),
    };
  }
  ...
  private getNativeHostPath(): string | null {
    if (this.platform !== 'win32') {
      return null;
    }
    if (this.nativeHostPath !== undefined) {
      return this.nativeHostPath;
    }
    if (this.platform !== process.platform) {
      return null;
    }
    return resolveHostBinary({ cwd: this.cwd });
  }
  ...
  private async detectCapabilities(): Promise<AudioCdCapabilities> {
    const nativeHostPath = this.getNativeHostPath();
    const [ffmpegResult, nativeResult] = await Promise.all([
      this.runProcess(this.getFfmpegPath(), ['-hide_banner', '-devices'], 5_000).catch(() => null),
      nativeHostPath
        ? this.runProcess(nativeHostPath, ['--cdda-capabilities'], 2_000).catch(() => null)
        : Promise.resolve(null),
    ]);
    const ffmpegOutput = ffmpegResult ? `${ffmpegResult.stdout}\n${ffmpegResult.stderr}` : '';
    const ffmpegAvailable = ffmpegResult?.code === 0;
    const libcdioAvailable = ffmpegAvailable && /\blibcdio\b/iu.test(ffmpegOutput);
    const nativeCddaAvailable = nativeResult?.code === 0
      && /"windowsNativeCdda"\s*:\s*true/iu.test(nativeResult.stdout);
    return {
      ffmpegAvailable,
      libcdioAvailable,
      nativeCddaAvailable,
      inputAvailable: nativeCddaAvailable || (ffmpegAvailable && libcdioAvailable),
      inputBackend: nativeCddaAvailable
        ? 'windows-native-cdda'
        : ffmpegAvailable && libcdioAvailable ? 'ffmpeg-libcdio' : null,
    };
  }
  ...
  private async inspectDisc(drive: AudioCdDrive): Promise<AudioCdTrack[]> {
    const nativeHostPath = this.getNativeHostPath();
    if (nativeHostPath) {
      const result = await this.runProcess(nativeHostPath, ['--cdda-toc', drive.device], 8_000);
      if (result.code !== 0) {
        throw new Error(this.resolveNativeCddaError(result.stderr, result.code));
      }
      return parseWindowsCddaToc(drive, result.stdout);
    }
  ...
  private spawnTrackDecode(
    drive: AudioCdDrive,
    track: AudioCdTrack,
    inputBackend: AudioCdStatus['inputBackend'],
  ): Readable {
    const stream = new PassThrough();
    const nativeHostPath = inputBackend === 'windows-native-cdda' ? this.getNativeHostPath() : null;
    const file = nativeHostPath ?? this.getFfmpegPath();
    const args = nativeHostPath ? [
      '--cdda-read',
      drive.device,
      '--track',
      String(track.index),
    ...
    if (!child.stdout) {
      stream.destroy(new Error('audio_cd_decoder_stdout_unavailable'));
      return stream;
    }
    ...
    child.once('close', (code, signal) => {
      ...
      if (code && code !== 0) {
        const message = nativeHostPath
          ? this.resolveNativeCddaError(stderr, code)
          : `audio_cd_ffmpeg_exit_${code}: ${stderr.trim()}`;
  ...
  private resolveNativeCddaError(stderr: string, code: number | null): string {
    const match = stderr.match(/\b(audio_cd_[a-z0-9_]+)\b/iu);
    return match?.[1] ?? `audio_cd_native_exit_${code ?? 'unknown'}`;
  }
