...
export const createEchoSrcFirStagePlans = (
  ...
  if (factor !== 2 && factor !== 4 && factor !== 8 && factor !== 16 && factor !== 32) {
    return [];
  }
...
export const measureEchoSrcFirMagnitudeDb = (
  taps: Float32Array,
  sampleRate: number,
  frequencyHz: number,
): number => {
  const safeSampleRate = Math.max(1, Number(sampleRate));
  const safeFrequencyHz = clamp(Number(frequencyHz), 0, safeSampleRate / 2);
  return magnitudeDb(magnitudeAt(taps, safeFrequencyHz / safeSampleRate));
};
