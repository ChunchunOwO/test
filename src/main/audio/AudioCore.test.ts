...
import { automixAnalysisVersion, type AutomixAnalysisV2 } from '../../shared/types/automix';
import {
...
const automixV2Analysis = (fingerprint: string): AutomixAnalysisV2 => ({
  ...
  bpmConfidence: 0.95,
  beatOffsetMs: 0,
  beatGridSeconds: Array.from({ length: 240 }, (_value, index) => index * 0.5),
  downbeatGridSeconds: Array.from({ length: 60 }, (_value, index) => index * 2),
  phraseBoundaries: [{ seconds: 108, bars: 8, confidence: 0.9 }],
  key: {
    tonic: 9,
    mode: 'minor',
    camelot: '8A',
  ...
  introLufs: -14,
  outroLufs: -14,
  segmentRmsDb: new Array<number>(12).fill(-14),
  energyCurve: new Array<number>(12).fill(0.7),
...
const createDaemonAudioBackendStub = (
  ...
  getPositionBaseSeconds: vi.fn(() => 0),
  getOutputFrame: vi.fn(() => 0),
  onPosition: vi.fn(),
  onEnded: vi.fn(),
  onError: vi.fn(),
  ...
  prepareGapless: vi.fn(async (request) => ({
    prepared: true,
    operationId: 1,
    ...request,
  })),
  prepareAutomixV2: vi.fn(async (request) => ({
    acknowledged: true,
  ...
  cancelAutomixV2: vi.fn(async () => undefined),
  getAutomixStateV2: vi.fn(async () => ({
    state: 'idle' as const,
    planId: null,
    queueRevision: null,
    operationId: null,
...
const createSharedDaemonAudioBackendStub = (
  filePath: string,
  outputSampleRate: number,
  deviceName: string,
): DaemonAudioBackend => {
  const backend = createDaemonAudioBackendStub(filePath, outputSampleRate, deviceName);
  backend.getOutputReady = vi.fn(() => ({
    ...
    backendImpl: 'test-wasapi-shared',
    deviceType: 'Windows Audio (Shared Mode)',
    deviceName,
  }));
  return backend;
};
...
describe('AudioSession CUDA capability status', () => {
  it('reports the runtime probe while CUDA processing is inactive', () => {
    const cudaStatus = {
      available: true,
    ...
    const { session } = createSessionHarness([], [], [], {
      resolveCudaRuntimeStatus: () => cudaStatus,
    });

    try {
      expect(session.getStatus()).toMatchObject({
        echoSrcCudaActive: false,
        echoSrcCudaStatus: cudaStatus,
        sdmCudaStatus: cudaStatus,
      });
    } finally {
      session.dispose();
    }
...
describe('Audio Core sample-rate regression guard', () => {
  ...
  it('keeps ordinary daemon playback across an exclusive gapless sample-rate boundary', async () => {
    ...
    try {
      ...
      expect(startAudioDaemon).toHaveBeenCalledOnce();
      expect(decoder.decodeRequests).toHaveLength(0);
      expect(backends[0]?.prepareGapless).not.toHaveBeenCalled();
      expect(status).toMatchObject({
        state: 'playing',
        automix: {
          active: false,
  ...
  it.each(['.wave', '.aifc', '.oga', '.au', '.snd', '.w64', '.voc', '.bwf'])(
    'routes low-risk local audio format %s through the native daemon',
    async (extension) => {
      const filePath = `local-audio${extension}`;
      const { backends, decoder, session } = createDaemonPcmSrcHarness([
        { ...probe(filePath, 48_000), bitDepth: 16 },
      ]);
  ...
  it('retries daemon playback once on the system-default Shared output when the selected device is missing', async () => {
    const filePath = 'missing-daemon-output.flac';
    const missingBackend = createDaemonAudioBackendStub(filePath, 48_000, 'Missing ASIO DAC');
    missingBackend.openSource = vi.fn(async () => { throw new Error('ASIO device not found'); });
    const sharedBackend = createSharedDaemonAudioBackendStub(filePath, 48_000, 'System Default Speakers');
    const backends = [missingBackend, sharedBackend];
    const createDaemonAudioBackend = vi.fn(async () => backends.shift() ?? null);
    const session = createAudioSessionForTest({
      decoder: new FakeDecoder(new Map([[filePath, probe(filePath, 48_000)]])),
      deviceService: { listDevices: () => [] },
      isNativeHostAvailable: () => true,
      startAudioDaemon: vi.fn(async () => undefined),
      stopAudioDaemon: vi.fn(async () => undefined),
      createDaemonAudioBackend,
      logger: noopLogger,
    });
    ...
    try {
      const status = await session.playLocalFile({
        filePath,
        output: { outputMode: 'asio', deviceIndex: 2, deviceName: 'Missing ASIO DAC' },
      });

      expect(status).toMatchObject({
        state: 'playing',
        outputMode: 'shared',
        outputDeviceName: 'System Default Speakers',
      });
      expect(status.warnings).toContain('output_device_not_found_fell_back_to_system_shared');
      expect(createDaemonAudioBackend).toHaveBeenNthCalledWith(2, '', expect.objectContaining({
        outputMode: 'shared',
  ...
  it('keeps same-rate native gapless active through the ECHO SRC FIR path', async () => {
    const currentFile = 'gapless-src-current.flac';
    const nextFile = 'gapless-src-next.flac';
    const { backends, decoder, session, startAudioDaemon } = createDaemonPcmSrcHarness([
      probe(currentFile, 44_100),
      probe(nextFile, 44_100),
    ]);

    try {
      await session.syncQueueToBackend([
        { itemId: 'current-item', trackId: 'current-track', filePath: currentFile, sampleRate: 44_100 },
        { itemId: 'next-item', trackId: 'next-track', filePath: nextFile, sampleRate: 44_100 },
      ], 'off', 'current-item');
      ...
      expect(startAudioDaemon).toHaveBeenCalledOnce();
      expect(decoder.decodeRequests).toHaveLength(0);
      expect(backends[0]?.prepareGapless).toHaveBeenCalledWith(expect.objectContaining({
        filePath: nextFile,
        trackId: 'next-track',
        itemId: 'next-item',
      }));
      ...
      expect(status.warnings).not.toContain('native_gapless_unavailable:native_processing_active');
    } finally {
      session.dispose();
    }
  });

  it('keeps ECHO SRC daemon playback active when the next gapless track has a different source rate', async () => {
    const currentFile = 'gapless-src-441.flac';
    const nextFile = 'gapless-src-480.flac';
    const { backends, decoder, session, startAudioDaemon } = createDaemonPcmSrcHarness([
      probe(currentFile, 44_100),
      probe(nextFile, 48_000),
    ]);
    ...
    try {
      await session.syncQueueToBackend([
        { itemId: 'current-item', trackId: 'current-track', filePath: currentFile, sampleRate: 44_100 },
        { itemId: 'next-item', trackId: 'next-track', filePath: nextFile, sampleRate: 48_000 },
      ], 'off', 'current-item');
  ...
  it.runIf(process.env.ECHO_AUTOMIX_V2_PHASE === 'native_beta')(
    'arms Smart Transition on an already playing daemon source without reopening it',
    async () => {
      ...
      const currentProbe = probe('current.flac', 48_000);
      const nextProbe = probe('next.flac', 48_000);
      const analyzeV2 = vi.fn(async ({ filePath }: { filePath: string }) => automixV2Analysis(filePath));
      const { backends, session } = createDaemonPcmSrcHarness(
        [currentProbe, nextProbe],
        {
          automixAnalyzer: {
            analyze: vi.fn(async () => createEstimatedAutomixAnalysis({ durationSeconds: 120 })),
            analyzeV2,
          },
        },
      );
      ...
      try {
        await session.syncQueueToBackend([
          { itemId: 'queue-current', trackId: 'current-track', filePath: 'current.flac', sampleRate: 48_000 },
          { itemId: 'queue-next', trackId: 'next-track', filePath: 'next.flac', sampleRate: 48_000 },
        ], 'off', 'queue-current');
        await session.playLocalFile({
          filePath: 'current.flac',
          trackId: 'current-track',
        ...
        vi.mocked(backend.getPositionSeconds).mockReturnValue(7);
        vi.mocked(backend.getOutputFrame).mockReturnValue(9_600_000);
        const openSourceCalls = vi.mocked(backend.openSource).mock.calls.length;

        const status = await session.configureAutomixForCurrentPlayback({
          enabled: true,
        ...
        expect(backend.prepareAutomixV2).toHaveBeenCalledOnce();
        expect(vi.mocked(backend.openSource).mock.calls).toHaveLength(openSourceCalls);
        expect(status.automix).toMatchObject({
          active: true,
          mode: 'armed',
          nextTrackId: 'next-track',
          engine: 'nativeDualDeck',
        });

        const plan = vi.mocked(backend.prepareAutomixV2).mock.calls[0]?.[0].plan;
        const commitTransition = vi.mocked(backend.onAutomixTransitionCommitted).mock.calls[0]?.[0];
        expect(plan).toBeDefined();
        expect(plan!.fadeStartOutputFrame).toBeGreaterThan(9_600_000);
        expect(commitTransition).toBeTypeOf('function');
        const trackAdvance = vi.fn();
        session.on('track-advance', trackAdvance);
        const sourcePositionAtFadeEnd = plan!.nextStartSeconds
        ...
        commitTransition?.({
          ...
          outputFrame: plan!.fadeEndOutputFrame - 1,
          sourcePositionSeconds: sourcePositionAtFadeEnd,
        });
        expect(session.getStatus()).toMatchObject({
          currentTrackId: 'current-track',
          warnings: expect.arrayContaining(['automix_v2_premature_identity_commit_blocked']),
        });
        ...
        commitTransition?.({
          ...
          toItemId: plan!.toItemId,
          toTrackId: plan!.toTrackId,
          outputFrame: plan!.fadeEndOutputFrame,
          sourcePositionSeconds: sourcePositionAtFadeEnd,
        });
        expect(session.getStatus()).toMatchObject({
          currentTrackId: 'next-track',
          currentTrackTitle: 'Next title',
          currentTrackArtist: 'Next artist',
          automix: {
            mode: 'off',
            runtimeState: 'committed',
            nextTrackId: null,
            transitionStartedAtSeconds: plan!.currentEndSeconds
              - (plan!.overlapFrames / plan!.mixSampleRate),
            advanceAtSeconds: plan!.currentEndSeconds,
          },
        });
        expect(trackAdvance).toHaveBeenCalledOnce();
        expect(trackAdvance).toHaveBeenCalledWith(expect.objectContaining({
          nextTrackId: 'next-track',
          gapless: false,
          status: expect.objectContaining({ currentTrackId: 'current-track' }),
        }));
      } finally {
        session.dispose();
      }
  ...
  it.runIf(process.env.ECHO_AUTOMIX_V2_PHASE === 'native_beta')(
    're-arms Smart Transition from the reset native clock after seeking',
    async () => {
      disableAudioVisualSpectrumForDirectPlaybackTest();
      audioCoreAppSettingsMock.current = {
        ...audioCoreAppSettingsMock.current,
      ...
      const currentProbe = probe('seek-current.flac', 48_000);
      const nextProbe = probe('seek-next.flac', 48_000);
      const { backends, session } = createDaemonPcmSrcHarness(
        [currentProbe, nextProbe],
        {
          automixAnalyzer: {
            analyze: vi.fn(async () => createEstimatedAutomixAnalysis({ durationSeconds: 120 })),
            analyzeV2: vi.fn(async ({ filePath }: { filePath: string }) => automixV2Analysis(filePath)),
          },
        },
      );
      ...
      try {
        await session.syncQueueToBackend([
          { itemId: 'seek-current-item', trackId: 'seek-current-track', filePath: 'seek-current.flac', sampleRate: 48_000 },
          { itemId: 'seek-next-item', trackId: 'seek-next-track', filePath: 'seek-next.flac', sampleRate: 48_000 },
        ], 'off', 'seek-current-item');
        await session.playLocalFile({
          filePath: 'seek-current.flac',
          trackId: 'seek-current-track',
          output: { outputMode: 'shared' },
          probe: currentProbe,
        });
        const backend = backends[0]!;
        const request: AudioSessionAutomixRequest = {
          enabled: true,
          maxTransitionSeconds: 12,
          beatAlignEnabled: true,
          next: {
            filePath: 'seek-next.flac',
            trackId: 'seek-next-track',
            probe: nextProbe,
          },
        };
        await session.configureAutomixForCurrentPlayback(request);
        expect(backend.prepareAutomixV2).toHaveBeenCalledOnce();

        vi.mocked(backend.getPositionBaseSeconds).mockReturnValue(60);
        vi.mocked(backend.getPositionSeconds).mockReturnValue(60);
        vi.mocked(backend.getOutputFrame).mockReturnValue(0);
        const status = await session.seek(60);

        expect(backend.seek).toHaveBeenCalledWith(60);
        expect(backend.prepareAutomixV2).toHaveBeenCalledTimes(2);
        const rearmedPlan = vi.mocked(backend.prepareAutomixV2).mock.calls[1]?.[0].plan;
        expect(rearmedPlan?.currentStartSeconds).toBe(60);
        expect(rearmedPlan?.fadeStartOutputFrame).toBeGreaterThan(0);
        expect(status).toMatchObject({
          currentTrackId: 'seek-current-track',
          positionSeconds: 60,
          automix: {
            mode: 'armed',
            runtimeState: 'armed',
            nextTrackId: 'seek-next-track',
          },
        });
      } finally {
        session.dispose();
  ...
  it('falls back to the system default Shared output when the selected shared device disappears', async () => {
    vi.stubEnv('ECHO_FORCE_LEGACY_LOCAL_PLAYBACK', '1');
  ...
  it('keeps non-device selected-output failures in error state when default-device fallback is disabled', async () => {
    vi.stubEnv('ECHO_FORCE_LEGACY_LOCAL_PLAYBACK', '1');
    const failingBridges = [
      new ConfigurableStartupFailingBridge('output open failed: permission denied'),
      new ConfigurableStartupFailingBridge('output open failed: permission denied'),
      new ConfigurableStartupFailingBridge('output open failed: permission denied'),
    ];
    const fallbackBridge = new FakeBridge(48000);
  ...
  it('forces a full routing-device refresh during playback when explicitly requested', async () => {
    const refreshedDevices: AudioDeviceInfo[] = [{
      id: 'asio:1',
      index: 1,
      name: 'Hot-plugged ASIO Driver',
      outputMode: 'asio',
      sampleRate: null,
      isDefault: false,
      sharedDeviceSampleRate: null,
    }];
    const refreshRoutingDevicesAsync = vi.fn(async () => refreshedDevices);
    const session = createAudioSessionForTest({
      decoder: new FakeDecoder(new Map()),
      deviceService: {
        listDevices: () => [],
    ...
    (session as unknown as { state: AudioStatus['state'] }).state = 'playing';

    await expect(session.listDevicesAsync({ forceRefresh: true })).resolves.toEqual(refreshedDevices);
    expect(refreshRoutingDevicesAsync).toHaveBeenCalledOnce();
  });
  ...
  it('fails safe from experimental WDM-KS to WASAPI shared after a KS open error', async () => {
    const previousLegacyPlayback = process.env.ECHO_FORCE_LEGACY_LOCAL_PLAYBACK;
    process.env.ECHO_FORCE_LEGACY_LOCAL_PLAYBACK = '1';
    try {
      const decoder = new FakeDecoder(new Map([['song.flac', probe('song.flac', 48000)]]));
      const failingKsBridge = new ConfigurableStartupFailingBridge('WDM-KS format unsupported or render pin unavailable');
      const fallbackBridge = new FakeBridge(48000);
      const bridges = [failingKsBridge, fallbackBridge];
      const session = createAudioSessionForTest({
        decoder,
        deviceService: { listDevices: () => [] },
        createBridge: () => bridges.shift() as unknown as FakeBridge,
      ...
      const status = await session.playLocalFile({
        filePath: 'song.flac',
        output: { outputMode: 'ks', deviceIndex: 2, deviceName: 'KS DAC' },
      });

      expect(failingKsBridge.startOptions).toMatchObject({
      ...
      expect(fallbackBridge.startOptions).not.toHaveProperty('kernelStreaming');
      expect(status.outputMode).toBe('shared');
      expect(status.outputBackend).toBe('wasapi-shared');
      expect(status.warnings).toContain('ks_output_fell_back_to_shared');
      expect(status.error).toBeNull();
    } finally {
      if (previousLegacyPlayback === undefined) {
        delete process.env.ECHO_FORCE_LEGACY_LOCAL_PLAYBACK;
      } else {
        process.env.ECHO_FORCE_LEGACY_LOCAL_PLAYBACK = previousLegacyPlayback;
      }
    }
  });

  it('fails safe from WDM-KS daemon startup to the default WASAPI shared endpoint', async () => {
    const decoder = new FakeDecoder(new Map([['song.flac', probe('song.flac', 48000)]]));
    const fallbackBridge = new FakeBridge(48000);
    const session = createAudioSessionForTest({
      decoder,
      deviceService: { listDevices: () => [] },
      createBridge: () => fallbackBridge,
      logger: noopLogger,
    });
  ...
  it('routes automatic output through the native CoreAudio shared backend on macOS', async () => {
    const refresh = vi.fn().mockResolvedValue(undefined);
    const session = createAudioSessionForTest({
      decoder: new FakeDecoder(new Map()),
      deviceService: {
    ...
    const status = await session.setOutput({
      ...
      outputMode: 'system',
      deviceIndex: 3,
      deviceName: 'Stale Mac output',
    });

    expect(refresh).toHaveBeenCalledTimes(1);
    expect(status).toMatchObject({
      automaticOutputEnabled: true,
      automaticOutputStage: 'default-shared',
      outputMode: 'shared',
      sharedBackend: 'auto',
    });
    expect(status.outputDeviceName).toBeNull();
    session.dispose();
  });
  ...
  it('uses DSF bitstream DoP over WASAPI exclusive when explicitly enabled', async () => {
    ...
    try {
      const status = await session.playLocalFile({
        filePath,
        output: { outputMode: 'exclusive', dsdOutputMode: 'dop', pcmDitherMode: 'tpdf' },
      });
      ...
      expect(bridges[0].sessionChunks.map((item) => [...item.chunk])).toEqual([
        [0x40, 0x80, 0x05, 0x60, 0xa0, 0x05, 0x20, 0xc0, 0xfa, 0x10, 0xe0, 0xfa],
      ]);
  ...
  it('keeps ASIO DSD on DoP unless the Native DSD experiment is enabled', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'echo-asio-dop-default-'));
    const filePath = join(tempDir, 'asio-dop-default.dsf');
    await writeFile(filePath, createDsfDopFixture());
    const bridges: FakeBridge[] = [];
    const { decoder, session } = createSessionHarness([dsdProbe(filePath)], [], [], {
      createBridge: () => {
  ...
  it('fails closed when generated PCM to SDM output cannot start', async () => {
    const filePath = 'generated-sdm-start-failure.flac';
    const pcmFallbackBridge = new FakeBridge(48_000);
    const daemonBackend = createDaemonAudioBackendStub(filePath, 352_800, 'Matrix ASIO Driver');
    vi.spyOn(daemonBackend, 'openSource').mockRejectedValue(new Error('ASIO DoP format unsupported'));
    const createBridge = vi.fn(() => pcmFallbackBridge);
    const session = createAudioSessionForTest({
      decoder: new FakeDecoder(new Map([[filePath, probe(filePath, 44_100)]])),
      deviceService: { listDevices: () => [] },
      createBridge,
      startAudioDaemon: async () => undefined,
      stopAudioDaemon: async () => undefined,
      createDaemonAudioBackend: async () => daemonBackend,
      logger: noopLogger,
    });
    ...
    })).rejects.toThrow('sdm_pcm_to_dsd_output_start_failed:ASIO DoP format unsupported');

    expect(daemonBackend.openSource).toHaveBeenCalledTimes(1);
    expect(createBridge).not.toHaveBeenCalled();
    expect(pcmFallbackBridge.startOptions).toBeNull();
    expect(session.getStatus().state).toBe('error');
    expect(session.getStatus().outputMode).toBe('asio');
    expect(session.getStatus().warnings).toContain(
      'sdm_pcm_to_dsd_output_start_failed:ASIO DoP format unsupported',
    );
    expect(session.getStatus().warnings.some((warning) => warning.startsWith('sdm_pcm_to_dsd_fell_back_to_pcm'))).toBe(false);
  });

  it('routes generated DSD512 to native ASIO DSD without an environment switch', async () => {
    const filePath = 'generated-native-asio-sdm.flac';
    const { daemonOutputSettings, session } = createDaemonPcmSrcHarness([
      probe(filePath, 44_100),
    ]);
    ...
    try {
      ...
      expect(daemonOutputSettings[0]).toMatchObject({
        outputMode: 'asio',
        requestedOutputSampleRate: 22_579_200,
        nativeProcessing: {
          outputFormat: 'dsd-native-raw',
          sdm: {
            sourceSampleRate: 44_100,
            targetSampleRate: 1_411_200,
            computeBackend: 'cpu',
            stages: expect.arrayContaining([
              expect.objectContaining({ upsampleFactor: 2, taps: expect.any(Array) }),
            ]),
      ...
      expect(status.activeDsdOutputMode).toBe('native');
      expect(daemonOutputSettings[0]?.nativeProcessing?.sdm?.stages).toHaveLength(5);
      expect(status.sdmNativeSampleRate).toBe(22_579_200);
      expect(status.sdmTransportSampleRate).toBe(1_411_200);
      expect(status.warnings).toContain('sdm_generated_native_asio_experimental');
    } finally {
      session.dispose();
    }
  });

  it('routes 48 kHz-family generated DSD512 to 24.576 MHz native ASIO DSD', async () => {
    const filePath = 'generated-native-asio-sdm-48k.flac';
    const { daemonOutputSettings, session } = createDaemonPcmSrcHarness([
      probe(filePath, 48_000),
    ]);
    ...
    try {
      ...
      expect(daemonOutputSettings[0]).toMatchObject({
        outputMode: 'asio',
        requestedOutputSampleRate: 24_576_000,
        nativeProcessing: {
          outputFormat: 'dsd-native-raw',
          sdm: {
            sourceSampleRate: 48_000,
            targetSampleRate: 1_536_000,
            computeBackend: 'cuda',
          },
        },
      });
      expect(daemonOutputSettings[0]?.nativeProcessing?.sdm?.stages).toHaveLength(5);
      expect(status.activeDsdOutputMode).toBe('native');
      expect(status.sdmNativeSampleRate).toBe(24_576_000);
      expect(status.sdmTransportSampleRate).toBe(1_536_000);
    } finally {
      session.dispose();
    }
  });

  it('refuses generated DSD512 outside ASIO because DoP cannot carry it', async () => {
    const filePath = 'generated-dsd512-without-asio.flac';
    const { daemonOutputSettings, session } = createDaemonPcmSrcHarness([
      probe(filePath, 48_000),
    ]);
    ...
    try {
      const status = await session.playLocalFile({
        ...
        output: {
          outputMode: 'exclusive',
          deviceIndex: 0,
          deviceName: 'Matrix USB Audio 2.0',
          sdmMode: 'pcmToDsd',
          sdmTargetRate: 'dsd512',
          sdmQualityProfile: 'hifi',
          sdmComputeBackend: 'cpu',
      ...
      expect(daemonOutputSettings[0]?.nativeProcessing?.sdm).toBeUndefined();
      expect(status.sdmRuntimeState).toBe('pcm_to_sdm_not_routed');
      expect(status.sdmActive).toBe(false);
      expect(status.sdmRuntime?.fallbackReason).toBe('sdm_pcm_to_dsd_target_unsupported:dsd512');
      expect(status.warnings).toContain('sdm_pcm_to_dsd_target_unsupported:dsd512');
    } finally {
      session.dispose();
    }
  });
  ...
  it('keeps DSF bitstream DoP when seeking while playing', async () => {
    ...
    try {
      ...
      expect(lastChunk ? [...lastChunk] : []).toEqual([0x20, 0xc0, 0xfa, 0x10, 0xe0, 0xfa]);
      expect(status.activeDecodeBackendImpl).toBe('dsf-bitstream-dop');
      expect(status.activeDsdOutputMode).toBe('dop');
    } finally {
      await rm(tempDir, { recursive: true, force: true });
  ...
  it('keeps the native DSD128 rate when the daemon reports its decoded PCM source rate', async () => {
    const filePath = 'daemon-dsd128.dsf';
    const nativeSampleRate = 5_644_800;
    const outputSampleRate = 352_800;
    const daemonBackend = createDaemonAudioBackendStub(filePath, outputSampleRate, 'Matrix ASIO Driver');
    daemonBackend.openSource = vi.fn(async () => ({
      status: 'ok',
      filePath,
      sampleRate: outputSampleRate,
      sourceSampleRate: 705_600,
      channels: 2,
      durationSeconds: 120,
    ...
    const decoder = new FakeDecoder(new Map([[filePath, dsdProbe(filePath, nativeSampleRate)]]));
    const session = createAudioSessionForTest({
      decoder,
      deviceService: { listDevices: () => [] },
      isNativeHostAvailable: () => true,
    ...
    try {
      ...
      expect(status).toMatchObject({
        state: 'playing',
        fileSampleRate: nativeSampleRate,
        decoderOutputSampleRate: outputSampleRate,
        requestedOutputSampleRate: outputSampleRate,
        actualDeviceSampleRate: outputSampleRate,
        activeDecodeBackendImpl: 'native-direct-daemon-libav',
      });
      expect(status.warnings).toContain(`dsd_source_decoded_to_pcm:${nativeSampleRate}->${outputSampleRate}`);
      expect(status.error).toBeNull();
    } finally {
      session.dispose();
    }
  ...
  it('keeps manual native pause immediate when track transition fade is enabled', async () => {
  ...
  it('keeps native manual controls immediate by default', async () => {
    const fadeWait = vi.fn(async () => undefined);
    const { bridges, session } = createSessionHarness([probe('song.flac', 44100)], [], [], {
      transportFadeDurationMs: 30,
      transportFadeStepMs: 10,
  ...
  it('keeps manual native resume immediate when track transition fade is enabled', async () => {
    const fadeWait = vi.fn(async () => undefined);
    audioCoreAppSettingsMock.current = {
      ...audioCoreAppSettingsMock.current,
      audioTransportFadeEnabled: true,
    ...
    expect(bridges[1].setVolume).not.toHaveBeenCalled();
    expect(fadeWait).not.toHaveBeenCalled();
  });
...
describe('NativeOutputBridge host arguments', () => {
  it('resolves the ECHO electron-app build host before migration fallbacks', () => {
    const exe = process.platform === 'win32' ? 'echo-audio-host.exe' : 'echo-audio-host';
    const cwd = join('tmp', 'echo');
...
describe('DeviceService diagnostics', () => {
  it('uses async native host enumeration for UI device lists', async () => {
    ...
    const execFileMock = vi.fn((_bin, args, _options, callback) => {
      const output = Array.isArray(args) && args.includes('-ks')
        ? '0\tKernel Streaming DAC\n'
        : Array.isArray(args) && args.includes('-asio')
        ? ''
    ...
    expect(execFileMock.mock.calls.map((call) => call[1])).toEqual(
      expect.arrayContaining([['-list'], ['-exclusive', '-list'], ['-asio', '-list'], ['-ks', '-list']]),
    );
  });
  ...
  it('returns UI routing devices without waiting for WASAPI exclusive capability enumeration', async () => {
    ...
    await expect(service.listRoutingDevicesAsync()).resolves.toMatchObject([
      ...
      { name: 'Kernel Streaming DAC', outputMode: 'ks' },
    ]);
  ...
  it('refreshes shared endpoints without probing cached ASIO or exclusive devices', async () => {
    ...
    expect(service.listKernelStreamingDevices()).toMatchObject([
      { name: 'Kernel Streaming DAC', outputMode: 'ks' },
    ]);
  });
  ...
  it('keeps the last device snapshot available after the refresh TTL', async () => {
    ...
    try {
      const execFileMock = vi.fn((_bin, args, _options, callback) => {
        const output = Array.isArray(args) && (args.includes('-asio') || args.includes('-ks'))
          ? ''
          : Array.isArray(args) && args.includes('-exclusive')
            ? '0\tExclusive DAC\t192000\t1\t48000\n'
            : '0\tSpeakers\t48000\t1\t48000\n';
...
describe('DecoderPipeline ffmpeg resolution', () => {
  ...
  it('falls back to dev-bundled ffmpeg before system ffmpeg', () => {
    expect(
      resolveDecoderFfmpegPath({
        env: {},
        resourcesPath: null,
        cwd: join('C:', 'Project', 'ECHO'),
        systemFfmpegPath: 'system-ffmpeg',
        existsSync: (path) => path === join('C:', 'Project', 'ECHO', 'electron-app', 'tools', 'ffmpeg.exe'),
      }),
    ).toBe(join('C:', 'Project', 'ECHO', 'electron-app', 'tools', 'ffmpeg.exe'));
  });
  ...
  it('builds Automix ffmpeg filters that skip next-track leading silence and use smooth curves', async () => {
    ...
    expect(filter).toContain("volume='if(lt(t\\,82.000)\\,1\\,if(lt(t\\,98.000)\\,1+(0.891251-1)*(((t-82.000)/16.000)*((t-82.000)/16.000)*(3-2*((t-82.000)/16.000)))\\,0.891251))':eval=frame");
    expect(filter).toContain("volume='if(lt(t\\,16.000)\\,1.258925\\,if(lt(t\\,20.000)\\,1.258925+(1-1.258925)*(((t-16.000)/4.000)*((t-16.000)/4.000)*(3-2*((t-16.000)/4.000)))\\,1))':eval=frame");
