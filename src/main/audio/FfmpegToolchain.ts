...
const collectCandidates = (dependencies: FfmpegToolchainDependencies = {}): FfmpegCandidate[] => {
  ...
  const platformDevToolsDirectory = platform === 'linux'
    ? 'tools-linux'
    : platform === 'darwin'
      ? 'tools-macos'
      : null;
  ...
  const candidates: Array<FfmpegCandidate | null> = [
    ...
    resourcesPath
      ...
      : null,
    platformDevToolsDirectory
      ? { path: join(cwd, 'electron-app', platformDevToolsDirectory, executableName), source: 'dev-bundled', mustExist: true }
      : null,
