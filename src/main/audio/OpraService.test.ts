...
describe('OpraService', () => {
  ...
  it('automatically refreshes a cached OPRA database after 24 hours', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'echo-opra-'));
    tempDirs.push(dir);
    const cachePath = join(dir, 'database_v1.jsonl');
    writeFileSync(cachePath, jsonLine({ type: 'vendor', id: 'old', data: { name: 'Old cache' } }), 'utf8');
    writeFileSync(`${cachePath}.meta.json`, JSON.stringify({ fetchedAt: '2026-08-04T23:59:59.000Z' }), 'utf8');
    const fetchText = vi.fn(async () => jsonLine({ type: 'vendor', id: 'current', data: { name: 'Current catalog' } }));
    const service = new OpraService({
      cachePath,
      fetchText,
    ...
    expect(fetchText).toHaveBeenCalledWith('https://opra.roonlabs.net/database_v1.jsonl');
    expect(result.status).toMatchObject({ source: 'network', fetchedAt: '2026-08-06T00:00:00.000Z', vendorCount: 1 });
    expect(result.vendors).toEqual([]);
  });

  it('keeps a fresh OPRA cache without downloading the catalog again', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'echo-opra-'));
    tempDirs.push(dir);
    const cachePath = join(dir, 'database_v1.jsonl');
    writeFileSync(cachePath, jsonLine({ type: 'vendor', id: 'cached', data: { name: 'Cached catalog' } }), 'utf8');
    writeFileSync(`${cachePath}.meta.json`, JSON.stringify({ fetchedAt: '2026-08-05T12:00:01.000Z' }), 'utf8');
    ...
    expect(result.status).toMatchObject({ source: 'cache', fetchedAt: '2026-08-05T12:00:01.000Z', vendorCount: 1 });
  });

  it('falls back to a stale OPRA cache when the automatic refresh fails', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'echo-opra-'));
    tempDirs.push(dir);
    const cachePath = join(dir, 'database_v1.jsonl');
    writeFileSync(cachePath, jsonLine({ type: 'vendor', id: 'fallback', data: { name: 'Fallback catalog' } }), 'utf8');
    writeFileSync(`${cachePath}.meta.json`, JSON.stringify({ fetchedAt: '2026-08-01T00:00:00.000Z' }), 'utf8');
    const fetchText = vi.fn(async () => {
    ...
    expect(result.status).toMatchObject({ source: 'cache', fetchedAt: '2026-08-01T00:00:00.000Z', vendorCount: 1 });
  });
});
