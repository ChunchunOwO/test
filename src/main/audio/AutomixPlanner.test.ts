...
describe('AutomixPlanner', () => {
  it('uses the requested fixed fade only at the track boundary', () => {
    const plan = planAutomixTransition({
      ...
      nextAnalysis: makeAnalysis({ durationSeconds: 210 }),
      trackBoundaryFadeMs: 1500,
    });

    expect(plan).toMatchObject({
      mode: 'energyFade',
      currentFadeStartSeconds: 178.5,
    ...
    expect(plan?.advanceAtSeconds).toBe(180);
  });
  ...
  it('skips next-track intro silence and trims current trailing silence before a beat-aligned transition', () => {
    ...
    expect(plan?.advanceAtSeconds).toBeCloseTo(
      (plan?.currentFadeStartSeconds ?? 0) + (plan?.overlapSeconds ?? 0),
      3,
    );
  });
  ...
  it('uses a short conservative fade for reliably detected incompatible tempos', () => {
    const plan = planAutomixTransition({
      currentProbe: { durationSeconds: 180 },
      nextProbe: { durationSeconds: 180 },
      currentAnalysis: makeAnalysis({ bpm: 96, beatConfidence: 0.92 }),
      nextAnalysis: makeAnalysis({ bpm: 132, beatConfidence: 0.9 }),
  ...
  it('keeps full-length high-energy transitions long enough to blend without overcrowding', () => {
    const plan = planAutomixTransition({
    ...
    expect(plan?.mode).toBe('energyFade');
    expect(plan?.overlapSeconds).toBeGreaterThanOrEqual(7);
    expect(plan?.overlapSeconds).toBeLessThanOrEqual(12);
  });
  ...
  it('sizes energy overlap from the skipped intro entry point instead of the quiet opening', () => {
    ...
    expect(plan?.overlapSeconds).toBeGreaterThanOrEqual(7);
    expect(plan?.overlapSeconds).toBeLessThanOrEqual(10);
  });
