...
import {
  builtInEqPresetDefinitions,
  createBuiltInEqBands,
  refreshBuiltInEqState,
} from '../../shared/audio/eqBuiltInPresets';
...
const builtInPresets: EqPreset[] = builtInEqPresetDefinitions.map((preset) => ({
  ...
  bands: createBuiltInEqBands(preset),
  createdAt: 'built-in',
  updatedAt: 'built-in',
  readonly: true,
}));
...
const cloneEqState = (state: EqState): EqState => ({
  ...state,
  bands: state.bands.map((band) => ({ ...band })),
});

const cloneChannelBalanceState = (state: ChannelBalanceState): ChannelBalanceState => {
  const bandGains = state.bandGains ?? {
    low: { leftGainDb: 0, rightGainDb: 0 },
    mid: { leftGainDb: 0, rightGainDb: 0 },
    high: { leftGainDb: 0, rightGainDb: 0 },
  };
...
const cloneRoomCorrectionState = (state: RoomCorrectionState): RoomCorrectionState => ({ ...state });

type PathCachedValue<T> = {
  path: string;
  value: T;
};
...
const getDefaultConfigPath = (): string | null => {
  if (process.platform === 'linux') {
    ...
    return configRoot ? join(configRoot, 'echo-steam') : null;
  }

  const homeDirectory = getHomeDirectory();

  if (process.platform === 'darwin') {
    return homeDirectory ? join(homeDirectory, 'Library', 'Application Support', 'ECHO Steam') : null;
  }
  ...
  return homeDirectory ? join(homeDirectory, '.config', 'echo-steam') : null;
};
...
export class EqStateStore {
  ...
  private static eqStateCache: PathCachedValue<EqState> | null = null;
  private static channelBalanceStateCache: PathCachedValue<ChannelBalanceState> | null = null;
  private static roomCorrectionStateCache: PathCachedValue<RoomCorrectionState & { irPath: string | null }> | null = null;
  ...
  static loadEqState(): EqState {
    const path = EqStateStore.statePath;
    if (EqStateStore.eqStateCache?.path === path) {
      return cloneEqState(EqStateStore.eqStateCache.value);
    }

    if (!existsSync(path)) {
      const value = defaultState();
      EqStateStore.eqStateCache = { path, value };
      return cloneEqState(value);
    }

    try {
      const value = refreshBuiltInEqState(normalizeState(JSON.parse(readFileSync(path, 'utf8'))) ?? defaultState());
      EqStateStore.eqStateCache = { path, value };
      return cloneEqState(value);
    } catch {
      return defaultState();
  ...
  static saveEqState(state: EqState): void {
    try {
      ...
      EqStateStore.eqStateCache = {
        path,
        value: refreshBuiltInEqState(normalizeState(state) ?? defaultState()),
      };
    } catch {
      EqStateStore.eqStateCache = null;
    }
  }

  static loadChannelBalanceState(): ChannelBalanceState {
    const path = EqStateStore.channelBalanceStatePath;
    if (EqStateStore.channelBalanceStateCache?.path === path) {
      return cloneChannelBalanceState(EqStateStore.channelBalanceStateCache.value);
    }

    if (!existsSync(path)) {
      const value = defaultChannelBalanceState();
      EqStateStore.channelBalanceStateCache = { path, value };
      return cloneChannelBalanceState(value);
    }

    try {
      const parsed = JSON.parse(readFileSync(path, 'utf8')) as unknown;
      const value = normalizeChannelBalancePatch(
  ...
  static saveChannelBalanceState(state: ChannelBalanceState): void {
    try {
      ...
      mkdirSync(dirname(path), { recursive: true });
      writeFileSync(path, `${JSON.stringify(state, null, 2)}\n`, 'utf8');
      EqStateStore.channelBalanceStateCache = { path, value: cloneChannelBalanceState(state) };
    } catch {
      EqStateStore.channelBalanceStateCache = null;
    }
  }
  ...
  static loadRoomCorrectionState(): RoomCorrectionState {
    const path = EqStateStore.roomCorrectionStatePath;
    if (EqStateStore.roomCorrectionStateCache?.path === path) {
      const cached = EqStateStore.roomCorrectionStateCache.value;
      EqStateStore.irPath = cached.irPath;
      const { irPath: _cachedIrPath, ...state } = cached;
      return cloneRoomCorrectionState(state);
    }

    if (!existsSync(path)) {
      EqStateStore.irPath = null;
      const value = defaultRoomCorrectionState();
      EqStateStore.roomCorrectionStateCache = { path, value: { ...value, irPath: null } };
      return cloneRoomCorrectionState(value);
    }

    try {
      const parsed = JSON.parse(readFileSync(path, 'utf8')) as PersistedRoomCorrectionState;
      ...
      const value = EqStateStore.irPath && !existsSync(EqStateStore.irPath)
        ? {
            ...state,
            enabled: false,
        ...
        : state;
      EqStateStore.roomCorrectionStateCache = {
        path,
        value: { ...value, irPath: EqStateStore.irPath },
      };
      return cloneRoomCorrectionState(value);
    } catch {
      EqStateStore.irPath = null;
  ...
  static saveRoomCorrectionState(state: RoomCorrectionState): void {
    try {
      ...
      EqStateStore.roomCorrectionStateCache = {
        path,
        value: { ...cloneRoomCorrectionState(state), irPath: EqStateStore.irPath },
      };
    } catch {
      EqStateStore.roomCorrectionStateCache = null;
    }
  }
}
