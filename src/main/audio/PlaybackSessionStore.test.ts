...
import { createResumeFromAudioStatus, PlaybackSessionStore } from './PlaybackSessionStore';
...
describe('PlaybackSessionStore', () => {
  ...
  it('does not persist remote proxy capability URLs as resume paths', () => {
    const session = makeSession();
    session.items[0].track = {
      ...session.items[0].track,
      mediaType: 'remote',
      sourceId: 'source-1',
      path: 'remote://source-1/song.flac',
    };

    expect(createResumeFromAudioStatus(session, makeStatus({
      currentFilePath: 'http://127.0.0.1:19000/remote-stream/secret-token',
    }), '2026-05-21T01:02:03.000Z')).toBeNull();
  });
});
