...
const opraCacheMaxAgeMs = 24 * 60 * 60 * 1_000;
const opraFetchTimeoutMs = 30_000;
...
export class OpraService {
  ...
  constructor(dependencies: OpraServiceDependencies) {
    ...
    this.fetchText = dependencies.fetchText ?? (async (url) => {
      const response = await fetch(url, { signal: AbortSignal.timeout(opraFetchTimeoutMs) });
      if (!response.ok) {
  ...
  private async loadDatabase(refresh: boolean): Promise<OpraDatabase> {
    const cacheIsFresh = this.isCacheFresh();
    if (this.database && !refresh && cacheIsFresh) {
    ...
    if (refresh || !existsSync(this.cachePath) || !cacheIsFresh) {
      try {
  ...
  private isCacheFresh(): boolean {
    ...
    const fetchedAt = this.database?.status.fetchedAt ?? this.readCachedFetchedAt();
    if (!fetchedAt) {
      return false;
    }
    ...
    return ageMs >= 0 && ageMs < opraCacheMaxAgeMs;
  }
