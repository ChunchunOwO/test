...
describe('EchoSrcFirEngine', () => {
  ...
  it('plans the five-stage 32x FIR carrier required by DSD512', () => {
    const stagePlans = createEchoSrcFirStagePlans('poly-sinc-hb', 48_000, 1_536_000);

    expect(stagePlans).toHaveLength(5);
    expect(stagePlans.map((stage) => [stage.plan.sourceSampleRate, stage.plan.targetSampleRate])).toEqual([
      [48_000, 96_000],
      [96_000, 192_000],
      [192_000, 384_000],
    ...
    expect(stagePlans.every((stage) => stage.upsampleFactor === 2)).toBe(true);
  });

  it('measures FIR response at an audible frequency in output-rate coordinates', () => {
    const taps = new Float32Array([1]);
    expect(measureEchoSrcFirMagnitudeDb(taps, 1_411_200, 20_000)).toBeCloseTo(0, 8);
  });
