...
const quantizeOverlapToBars = (seconds: number, bpm: number, maximumSeconds: number): number => {
  ...
  const bars = clamp(Math.round(seconds / barSeconds), 1, 4);
...
const selectEnergyOverlap = (
  ...
  const density = (currentEnergy + nextEnergy) / 2;
  const densityLimit = density >= 0.68
    ? 7
    : density >= 0.52
      ? 9
      : density >= 0.36
        ? 11
        : 12;
  const mismatchAdjustment = energyDelta > 0.35 ? -1 : 0;

  return Math.min(baseSeconds, densityLimit) + mismatchAdjustment;
};
...
export const planAutomixTransition = (input: AutomixPlanInput): AutomixTransitionPlan | null => {
  ...
  const trackBoundaryFadeSeconds = clamp((finiteOrNull(input.trackBoundaryFadeMs) ?? 0) / 1000, 0, 5);
  if (trackBoundaryFadeSeconds > 0) {
    const overlapSeconds = Math.min(trackBoundaryFadeSeconds, currentRemainingSeconds - 0.2, nextDuration - 0.2);
    if (overlapSeconds <= 0) {
      return null;
    }
    const currentEndSeconds = currentDuration;
    const currentFadeStartSeconds = currentEndSeconds - overlapSeconds;
    return {
      mode: 'energyFade',
      ...
      nextGainDb: 0,
      tempoRatio: 1,
      advanceAtSeconds: roundToMillis(currentEndSeconds),
  ...
  const tempoIncompatible = currentBpm !== null
    && nextBpm !== null
    && !areBpmsCompatible(currentBpm, nextBpm);
  const conservativeFallback = shortTrackFallback || unavailableAnalysis || tempoIncompatible;
  const beatAligned =
  ...
  if (conservativeFallback) {
    ...
    fallbackReason = unavailableAnalysis
  ...
  const nominalOverlap = conservativeFallback
  ...
  const minOverlap = conservativeFallback ? 0.8 : 2;
  ...
  return {
    ...
    tempoRatio,
    // Public track identity changes only after the outgoing envelope reaches
    // silence. The overlap may be audible before then, but it is not yet owned
    // exclusively by the incoming track.
    advanceAtSeconds: roundToMillis(currentFadeStartSeconds + overlapSeconds),
    skipIntroSilence,
