...
export const rendererMemoryPressureThresholdBytes = 768 * 1024 * 1024;
export const rendererEmergencyMemoryPressureThresholdBytes = 1536 * 1024 * 1024;

const defaultCheckIntervalMs = 10_000;
...
const rendererCleanupRequestCooldownMs = 5 * 60 * 1000;
const mib = 1024 * 1024;
const gib = 1024 * mib;
...
let lastRendererCleanupRequestAtMs = 0;
...
export const isRendererProbeLyricsPageVisible = (
  activeRouteId: string | null,
  lyricsPage: Element | null,
): boolean => {
  if (
    activeRouteId !== 'lyrics' ||
    !lyricsPage ||
    lyricsPage.closest('[hidden], [aria-hidden="true"]')
  ) {
    return false;
  }

  const style = lyricsPage.ownerDocument?.defaultView?.getComputedStyle(lyricsPage);
  return !style || (
    style.display !== 'none' &&
...
(async () => {
  ...
  const lyricsPageVisible = (${isRendererProbeLyricsPageVisible.toString()})(activeRouteId, lyricsPage);
  const visibleLyricsPage = lyricsPageVisible ? lyricsPage : null;
  const visibleLyricsExists = (selector) => Boolean(visibleLyricsPage?.querySelector(selector));
  const visibleLyricsCount = (selector) => visibleLyricsPage?.querySelectorAll(selector).length || 0;
  const visibleMvPanel = visibleLyricsPage?.querySelector('.lyrics-mv-panel') || null;
  ...
  const routeDetail = {
    ...
    pageMode: interactionDiagnostics?.routeId === activeRouteId
      ? interactionDiagnostics?.pageMode
      : visibleLyricsPage
        ? \`lyrics:\${visibleLyricsPage.getAttribute('data-view-mode') || 'unknown'}\`
        : null,
    activeRouteId,
    ...
    lyricsViewMode: visibleLyricsPage ? visibleLyricsPage.getAttribute('data-view-mode') : null,
  ...
  return {
    ...
    visibleState: {
      ...
      lyricsPageVisible,
      lyricsViewMode: visibleLyricsPage ? visibleLyricsPage.getAttribute('data-view-mode') : null,
      lyricsBackgroundMode: visibleLyricsPage
        ? visibleLyricsPage.getAttribute('data-background-source') ?? visibleLyricsPage.getAttribute('data-background')
        : null,
      lyricsRenderPressureReduced: visibleLyricsPage?.getAttribute('data-render-pressure-reduced') === 'true',
      lyricsImmersiveCoverStyle: visibleLyricsPage?.getAttribute('data-immersive-cover-style') === 'true',
      lyricsImmersiveCoverGlass: visibleLyricsPage?.getAttribute('data-immersive-cover-glass') === 'true',
      lyricsSpectrumVisible: visibleLyricsExists('.lyrics-music-reactive-spectrum'),
      lyricsBackdropVisible: visibleLyricsExists('.lyrics-backdrop'),
      lyricsPreviousBackdropVisible: visibleLyricsExists('.lyrics-backdrop-previous-cover'),
      lyricsSettingsDrawerVisible: exists('.lyrics-settings-drawer-root[data-open="true"] .lyrics-settings-drawer:not(.lyrics-visual-settings-drawer)'),
      lyricsVisualSettingsDrawerVisible: exists('.lyrics-visual-settings-drawer-root[data-open="true"] .lyrics-visual-settings-drawer'),
      ...
      mvPanelVisible: visibleLyricsExists('.lyrics-mv-panel[data-mv-enabled="true"]'),
      mvImmersiveActive: visibleMvPanel?.getAttribute('data-immersive-active') === 'true',
      mvRenderPressureReduced: visibleMvPanel?.getAttribute('data-render-pressure-reduced') === 'true',
      mvForegroundVideoCount: visibleLyricsCount('video.lyrics-mv-video'),
      mvBackgroundVideoCount: visibleLyricsCount('video.lyrics-mv-background-video'),
      mvForegroundYoutubeFrameCount: visibleLyricsCount('iframe.lyrics-mv-video--youtube'),
      mvBackgroundYoutubeFrameCount: visibleLyricsCount('iframe.lyrics-mv-background-video--youtube'),
    },
...
export const getLargestRendererMemoryBytes = (
  snapshot: Pick<DiagnosticMemorySnapshot, 'metrics'>,
): number => snapshot.metrics.reduce((largest, metric) => {
  const label = `${metric.type} ${metric.name ?? ''}`.toLowerCase();
  if (!label.includes('tab') && !label.includes('renderer')) {
    return largest;
  }
...
const requestSoftMemoryCleanup = (reason: string): void => {
  void releaseSoftMemoryPressure({ reason })
    .then((summary) => {
...
const maybeReleaseSoftMemoryPressure = (): void => {
  if (shouldReleaseSoftMemoryPressure(getRecentMemoryTrendSamples())) {
    requestSoftMemoryCleanup('sustained-soft-memory-pressure');
  }
};

const maybeReleaseRendererMemoryPressure = (
  reason = 'renderer-memory-pressure',
  timestampMs = Date.now(),
): void => {
  if (
    lastRendererCleanupRequestAtMs > 0 &&
    timestampMs - lastRendererCleanupRequestAtMs < rendererCleanupRequestCooldownMs
  ) {
    return;
  }

  lastRendererCleanupRequestAtMs = timestampMs;
  requestSoftMemoryCleanup(reason);
};
...
export const checkMemoryPressureNow = async (): Promise<DiagnosticMemoryPressureEvent | null> => {
  ...
  try {
    ...
    const rendererPressureBytes = getLargestRendererMemoryBytes(snapshot);
    const gpuPressureBytes = getLargestGpuMemoryBytes(snapshot.metrics);
    const recentSamples = getRecentMemoryTrendSamples();
    const totalPressureDetected = totalPressureBytes >= memoryPressureThresholdBytes;
    const rendererPressureDetected = rendererPressureBytes >= rendererMemoryPressureThresholdBytes;
    const rendererEmergencyDetected = rendererPressureBytes >= rendererEmergencyMemoryPressureThresholdBytes;
    const gpuPressureDetected = gpuPressureBytes >= gpuMemoryPressureThresholdBytes &&
      hasSustainedGpuMemoryPressure(recentSamples);
    if (rendererPressureDetected) {
      maybeReleaseRendererMemoryPressure();
    } else if (gpuPressureDetected) {
      maybeReleaseRendererMemoryPressure('sustained-gpu-memory-pressure');
    } else {
      // Keep releasing disposable caches after the first hard-pressure report.
      // The janitor owns cooldowns and never touches playback-owned state.
      maybeReleaseSoftMemoryPressure();
    }
    if (!totalPressureDetected && !rendererPressureDetected && !gpuPressureDetected) {
      return null;
    }

    const enrichedSnapshot: DiagnosticMemorySnapshot = {
      ...snapshot,
      thresholdBytes: totalPressureDetected
        ? memoryPressureThresholdBytes
          : rendererEmergencyDetected
    ...
    const reportedEvent = hasReportedMemoryPressure
    ...
    const event: DiagnosticMemoryPressureEvent = {
      ...reportedEvent,
      // Renderer/GPU-only pressure is handled silently. GPU pressure only
      // changes visible rendering when the report confirms lyrics/MV graphics
      // pressure; generic GPU spikes merely release disposable caches.
      userNoticeRecommended: totalPressureDetected,
      rendererMitigationRecommended: totalPressureDetected || rendererEmergencyDetected || (
        gpuPressureDetected && Boolean(reportedEvent.graphicsPressure)
      ),
    };
