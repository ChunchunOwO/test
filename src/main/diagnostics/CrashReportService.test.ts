...
import { CrashReportService, createMemoryPressureEventFromSnapshot } from './CrashReportService';
...
describe('CrashReportService', () => {
  ...
  it('opens a markdown report for the previous abnormal session', async () => {
    ...
    expect(report).toContain('Previous ECHO session did not close normally.');
  ...
  it('writes audio crash reports with safe audio status snapshots', async () => {
    ...
    service.reportAudioError({
      message: 'echo-audio-host timeout_waiting_for_ready; host="D:\\Program Files\\ECHO\\echo-audio-host.exe"',
      phase: 'output-start',
    ...
    expect(readableReport).toContain('# ECHO Audio Crash Report');
  ...
  it('opens the dedicated audio crash report file', async () => {
    ...
    expect(readFileSync(reportPath, 'utf8')).toContain('# ECHO Audio Crash Report');
  ...
  it('opens the dedicated audio crash text report file', async () => {
    ...
    expect(existsSync(reportPath)).toBe(true);
    const report = readFileSync(reportPath, 'utf8');
    expect(report).toContain('ECHO Audio Crash Report');
  ...
  it('opens the dedicated normal crash report file', async () => {
    ...
    expect(report).toContain('# ECHO Crash Report');
  ...
  it('writes and opens a dedicated memory pressure report', async () => {
    ...
    const report = readFileSync(reportPath, 'utf8');
    expect(report).toContain('# ECHO Memory Pressure Report');
  ...
  it('flags sustained GPU pressure on a visible lyrics page below the renderer threshold', () => {
    const mib = 1024 * 1024;
    const gpuProcess = {
      pid: 330,
      type: 'GPU',
      serviceName: 'GPU',
      workingSetBytes: 1480 * mib,
      peakWorkingSetBytes: 1500 * mib,
      privateBytes: 1820 * mib,
      cpuPercent: 0.8,
    };
    const rendererProcess = {
      pid: 220,
      type: 'Tab',
      name: 'renderer',
      workingSetBytes: 654 * mib,
      peakWorkingSetBytes: 700 * mib,
      privateBytes: 713 * mib,
      cpuPercent: 3.5,
    };
    const snapshot: DiagnosticMemorySnapshot = {
      timestamp: '2026-08-12T02:51:00.000Z',
      thresholdBytes: 1024 * mib,
      totalWorkingSetBytes: 2470 * mib,
      totalPrivateBytes: 3100 * mib,
      processCount: 4,
      source: 'electron-app-metrics',
      currentProcess: {
        pid: 100,
        rssBytes: 324 * mib,
        heapTotalBytes: 203 * mib,
        heapUsedBytes: 174 * mib,
        externalBytes: 20 * mib,
        arrayBuffersBytes: 10 * mib,
      },
      metrics: [gpuProcess, rendererProcess],
      topProcesses: [gpuProcess, rendererProcess],
      rendererProcesses: [
        {
          timestamp: '2026-08-12T02:51:00.000Z',
          pid: 220,
          ...
          process: rendererProcess,
          heap: {
            usedJSHeapSize: 142 * mib,
            totalJSHeapSize: 180 * mib,
            jsHeapSizeLimit: 4_000_000_000,
          },
          dom: {
            nodeCount: 8_742,
      ...
      recentSamples: [
        {
          timestamp: '2026-08-12T02:50:50.000Z',
          totalWorkingSetBytes: 2300 * mib,
          totalPrivateBytes: 2900 * mib,
          topProcesses: [{ ...gpuProcess, privateBytes: 1600 * mib }],
        },
        {
          timestamp: '2026-08-12T02:51:00.000Z',
          totalWorkingSetBytes: 2470 * mib,
    ...
    expect(createMemoryPressureEventFromSnapshot(snapshot, 'memory-pressure-report.md').graphicsPressure).toMatchObject({
      kind: 'lyrics-mv-render-pressure',
      reason: 'gpu-process-memory-high-on-lyrics-or-mv-page',
      rendererPid: 220,
      rendererPrivateBytes: 713 * mib,
      lyricsPageVisible: true,
    });

    expect(createMemoryPressureEventFromSnapshot({
      ...snapshot,
      recentSamples: snapshot.recentSamples?.slice(-1),
    }, 'memory-pressure-report.md').graphicsPressure).toBeNull();
  });
