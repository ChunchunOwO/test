...
const createLyricsMvGraphicsPressureRecommendation = (
  ...
  const topProcessIsRenderer = topProcessLabel.includes('tab') || topProcessLabel.includes('renderer');
  const rendererSnapshots = snapshot.rendererProcesses ?? [];
  ...
  const rendererPrivateBytes = matchedRenderer?.process?.privateBytes ?? (
    topProcessIsRenderer ? topProcess?.privateBytes ?? 0 : 0
  );
  ...
  const mib = 1024 * 1024;
  const rendererGraphicsPressureThresholdBytes = 768 * mib;
  const gpuPressureBytes = getLargestGpuMemoryBytes(snapshot.metrics.length > 0
    ? snapshot.metrics
    : snapshot.topProcesses);
  const sustainedGpuPressure = gpuPressureBytes >= gpuMemoryPressureThresholdBytes &&
    hasSustainedGpuMemoryPressure(snapshot.recentSamples ?? []);

  if (
    !topProcessIsRenderer &&
  ...
  if (
    (rendererPrivateBytes < rendererGraphicsPressureThresholdBytes && !sustainedGpuPressure) ||
  ...
  return {
    ...
    reason: duplicateMvVideoDecode
      ...
      : sustainedGpuPressure && rendererPrivateBytes < rendererGraphicsPressureThresholdBytes
        ? 'gpu-process-memory-high-on-lyrics-or-mv-page'
...
export class CrashReportService {
  ...
  private createCrashReportMarkdown(
    ...
    const summaryMessage = record?.message ?? (
      isLastAbnormalReport
        ? 'Previous ECHO session did not close normally.'
  ...
  private async chooseDiagnosticsMarkdownPath(): Promise<string | null> {
    const defaultPath = join(
      ...
      `ECHO-Diagnostics-${new Date().toISOString().replace(/[:.]/g, '-')}.md`,
    );
  ...
  private async chooseDiagnosticsZipPath(): Promise<string | null> {
    const defaultPath = join(
      app.getPath('downloads'),
      `ECHO-Diagnostics-${new Date().toISOString().replace(/[:.]/g, '-')}.zip`,
    );
    const result: SaveDialogReturnValue = await dialog.showSaveDialog({
  ...
  private getSafeAccountStatus(): unknown {
    return { disabled: true, distribution: 'steam' };
  }
